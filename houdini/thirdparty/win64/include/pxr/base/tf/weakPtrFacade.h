...
// previous overload in the hboost namespace.
namespace hboost {
...
public:
    ...
    /// must be polymorphic.
    template <class T>
    typename hboost::enable_if<hboost::is_base_of<T, DataType> >::type
    PointsToA() const {
        return _FetchPointer();  // points to a T if not null.
    }

    template <class T>
    typename hboost::disable_if<hboost::is_base_of<T, DataType>, bool>::type
    PointsToA() const {
...
struct TfTypeFunctions<Ptr<T>,
                       typename hboost::enable_if<
                           hboost::is_base_of<TfWeakPtrFacadeBase, Ptr<T> >
                           >::type>
{
...
struct TfTypeFunctions<Ptr<const T>,
                       typename hboost::enable_if<
                           hboost::is_base_of<TfWeakPtrFacadeBase, Ptr<const T> >
                           >::type>
{
...
// Extend hboost::hash to support TfWeakPtrFacade.
template <template <class> class X, class T>
inline size_t
hash_value(TfWeakPtrFacade<X, T> const &ptr)
{
    // Make the hboost::hash type depend on T so that we don't have to always
    // include hboost/functional/hash.hpp in this header for the definition of
    // hboost::hash.
    auto uniqueId = ptr.GetUniqueIdentifier();
    return hboost::hash<decltype(uniqueId)>()(uniqueId);
}
