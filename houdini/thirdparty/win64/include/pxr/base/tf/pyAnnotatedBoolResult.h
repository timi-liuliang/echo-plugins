...
struct TfPyAnnotatedBoolResult :
    hboost::equality_comparable<TfPyAnnotatedBoolResult<Annotation>, bool>
{
    ...
    static hboost::python::class_<Derived>
    Wrap(char const *name, char const *annotationName) {
        ...
        return class_<Derived>(name, no_init)
            ...
            // and hboost python doesn't know the former is-a latter because
...
private:
    ...
    template <class Derived>
    static hboost::python::object _GetItem(const Derived& x, int i)
    {
        if (i == 0) {
            return hboost::python::object(x._val);
        }
        if (i == 1) {
            return hboost::python::object(x._annotation);
        }
