...
struct TfMakePyPtr {
    ...
    typedef hboost::python::objects::pointer_holder<Ptr, Pointee> Holder;
    ...
    static Result Execute(Ptr const& p)
    {
        ...
        if (!p.GetUniqueIdentifier())
            return Result(hboost::python::detail::none(), false);
        ...
        PyObject *res = hboost::python::objects::make_ptr_instance
...
namespace mpl = hboost::mpl;

using namespace hboost::python;

using hboost::disable_if;
using hboost::enable_if;
using hboost::is_abstract;

template <typename Ptr>
struct _PtrInterface {
    typedef typename Ptr::DataType Pointee;
    typedef typename hboost::add_const<Pointee>::type ConstPointee;
    typedef typename hboost::remove_const<Pointee>::type NonConstPointee;
...
struct _PtrToPythonWrapper {
    ...
    static PyObject *Convert(void const *x) {
        // See hboost/python/converter/as_to_python_function.hpp
...
struct WeakPtr : def_visitor<WeakPtr> {
    ...
    static void _RegisterConversionsHelper() {
        ...
        // hboost python for doing this sort of thing.
        //
        // We do this for wrapper because this is the "wrapped type" -- the type
        // for which hboost python has already registered a to-python
