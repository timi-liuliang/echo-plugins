...
///
/// This class is reimplemented from the hboost class simply because the
/// provided class only allows construction from it's friended class
/// hboost::python::override, which we also reimplement below.
///
/// \see TfPyOverride
class TfPyMethodResult
{
...
public:
    ...
    {
        TfPyLock lock;
        hboost::python::converter::return_from_python<T> converter;
    ...
    operator T&() const
    {
        ...
        hboost::python::converter::return_from_python<T&> converter;
        return converter(
            const_cast<hboost::python::handle<>&>(m_obj).release());
    }
    ...
    {
        TfPyLock lock;
        return hboost::python::extract<T>(m_obj)();
    }

private:
    mutable hboost::python::handle<> m_obj;
};
...
/// hboost::python::wrapper::get_override(). Unfortunately, that method 
...
public:
    ...
    TfPyOverride(hboost::python::handle<> callable)
        : TfPyObjWrapper(hboost::python::object(callable))
    {}
    ...
    operator()(Args const&... args) const
    {
        ...
        TfPyMethodResult x(
            PyEval_CallFunction(
                ...
                hboost::python::converter::arg_to_python<Args>(args).get()...));
        return x;
    }
};
