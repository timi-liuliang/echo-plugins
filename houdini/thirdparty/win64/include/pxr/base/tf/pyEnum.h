...
class Tf_PyEnumRegistry {
  ...
  public:
    ...
    TF_API
    void RegisterValue(TfEnum const &e, hboost::python::object const &obj);

    template <typename T>
    void RegisterEnumConversions() {
        // Register conversions to and from python.
        hboost::python::to_python_converter<T, _EnumToPython<T> >();
        _EnumFromPython<T>();
    }
  ...
  private:
    ...
    struct _EnumFromPython {
        _EnumFromPython() {
            hboost::python::converter::registry::insert
                (&convertible, &construct, hboost::python::type_id<T>());
        }
        static void *convertible(PyObject *obj) {
            ...
            if (hboost::is_same<T, TfEnum>::value ||
                (hboost::is_integral<T>::value && !hboost::is_enum<T>::value))
                return i != o2e.end() ? obj : 0;
            else
                return (i != o2e.end() && i->second.IsA<T>()) ? obj : 0;
        }
        static void construct(PyObject *src, hboost::python::converter::
                              rvalue_from_python_stage1_data *data) {
            void *storage =
                ((hboost::python::converter::
...
TF_API
std::string Tf_PyEnumRepr(hboost::python::object const &self);
...
Tf_PyEnumRegistry::_EnumToPython<T>::convert(T const &t)
{
    ...
    if (!Tf_PyEnumRegistry::GetInstance()._enumsToObjects.count(e)) {
        ...
        hboost::python::object wrappedVal =
            hboost::python::object(Tf_PyEnumWrapper(name, e));
...
/// may be manually wrapped using hboost::python::enum_ instead.
///
...
private:
    typedef hboost::python::class_<
        Tf_TypedPyEnumWrapper<T>, hboost::python::bases<Tf_PyEnumWrapper> >
    _EnumPyClassType;
  ...
  private:
    ...
    void _ExportValues(bool cleanNames, _EnumPyClassType &enumClass) {
        ...
        TF_FOR_ALL(name, names) {
            ...
            hboost::python::object pyValue(wrappedValue);
        ...
        enumClass.setattr("allValues", hboost::python::tuple(valueList));
    }
