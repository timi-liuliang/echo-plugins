...
// In hboost version 1.51, they seem to have neglected to define this. 
// Without it, some functions will get confused about macros with no arguments
#ifndef HBOOST_PP_TUPLE_TO_SEQ_0
...
#if defined(ARCH_OS_WINDOWS)
#define TF_NUM_ARGS(...) \
    HBOOST_PP_IIF(HBOOST_VMD_IS_EMPTY(__VA_ARGS__),\
        0, HBOOST_PP_VARIADIC_SIZE(__VA_ARGS__))
#else
#define TF_NUM_ARGS(...)                                        \
    _TF_NUM_ARGS_CHECK(__VA_ARGS__)                             \
    HBOOST_PP_IIF(HBOOST_PP_EQUAL(1, _TF_NUM_ARGS1(__VA_ARGS__)), \
        HBOOST_PP_EXPAND(TF_ARG_2 HBOOST_PP_LPAREN()              \
        HBOOST_PP_EXPAND(_TF_NUM_ARGS_0X TF_ARG_1(__VA_ARGS__)   \
        (HBOOST_PP_REPEAT(TF_MAX_ARITY, _TF_NUM_ARGS_REP, _TF))) \
        HBOOST_PP_COMMA() 1 HBOOST_PP_RPAREN()),                  \
...
// We can't use
// HBOOST_PP_STRINGIZE because on MSVC passing no arguments will report "not
...
// 0.  No other values of c are allowed.  We can't use HBOOST_PP_IFF() because
...
#define TF_PP_IS_TUPLE(arg) \
    HBOOST_PP_CAT(_TF_PP_IS_TUPLE, HBOOST_PP_EXPAND(_TF_PP_IS_TUPLE arg)) )
...
#define TF_PP_TUPLE_SIZE(tuple) \
    HBOOST_PP_EXPAND(TF_NUM_ARGS tuple)
...
#define TF_PP_TUPLE_TO_LIST(tuple) \
    HBOOST_PP_IIF(                                               \
        HBOOST_PP_EQUAL(TF_PP_TUPLE_SIZE(tuple), 0),             \
        HBOOST_PP_LIST_NIL,                                      \
        HBOOST_PP_TUPLE_TO_LIST(TF_PP_TUPLE_SIZE(tuple), tuple))
...
#define TF_PP_TUPLE_TO_SEQ(tuple) \
    HBOOST_PP_IIF(                                               \
        HBOOST_PP_EQUAL(TF_PP_TUPLE_SIZE(tuple), 0),             \
        HBOOST_PP_EMPTY(),                                       \
        HBOOST_PP_TUPLE_TO_SEQ(TF_PP_TUPLE_SIZE(tuple), tuple))
