...
// be a hboost::python::tuple and dict. These will contain the remaining
...
//     const hboost::python::tuple& args, const hboost::python::dict& kwargs);
//
// class_<MyClass, MyClassPtr>("MyClass", no_init)
//    .def(TfPyRefAndWeakPtr())
...
//    .def(...)
//
// NOTE: The current implementation does not handle hboost::python::arg for
//       specifying keywords for required arguments.

namespace Tf_MakePyConstructor {

namespace bp = hboost::python;
...
struct NewVisitor : bp::def_visitor<NewVisitor<CTOR> > {
    ...
    void visit(CLS &c) const {
        ...
        // hboost python needs to have all overloads exported before you say
        // .staticmethod.
...
struct InstallPolicy<TfRefPtr<T> > {
    HBOOST_MPL_ASSERT_MSG(Tf_SupportsUniqueChanged<T>::Value,
...
struct _RefPtrFactoryConverter {
    typedef typename hboost::remove_reference<P>::type Ptr;
    ...
    // Required for hboost.python signature generator, in play when
    // HBOOST_PYTHON_NO_PY_SIGNATURES is undefined.
    PyTypeObject const *get_pytype() const {
        return hboost::python::objects::registered_class_object(
            hboost::python::type_id<typename WeakPtr::DataType>()).get();
    }
};
...
struct RefPtrFactory {
    ...
    struct apply {
        typedef typename hboost::mpl::if_<hboost::is_same<WeakPtr, void>,
...
/// A \c hboost::python result converter generator which converts standard
...
struct Tf_PySequenceToListConverterRefPtrFactory {
    typedef typename hboost::remove_reference<T>::type SeqType;
    bool convertible() const {
        return true;
    }
...
#define SIGNATURE R (HBOOST_PP_ENUM_PARAMS(N, A))
#define PARAMLIST HBOOST_PP_ENUM_TRAILING_BINARY_PARAMS(N, A, a)
#define ARGLIST HBOOST_PP_ENUM_PARAMS(N, a)
...
#define VAR_SIGNATURE                                   \
    R (HBOOST_PP_ENUM_PARAMS(N, A) HBOOST_PP_COMMA_IF(N)  \
...
#define EXTRACT_REQ_ARG_A(z, n, data)                                     \
    ...
    bp::extract<typename hboost::remove_reference<A##n>::type>(data[n + 1])
...
struct InitCtorWithVarArgs<VAR_SIGNATURE> : CtorBase<VAR_SIGNATURE> {
    ...
    static bp::object __init__(const bp::tuple& args, const bp::dict& kwargs) {
        ...
        if (numArgs - 1 < N) {
            ...
            TfPyThrowTypeError(
                TfStringPrintf(
                    ...
                    "\t__init__(" HBOOST_PP_REPEAT(N, FORMAT_STR, 0) "...)"
                    HBOOST_PP_COMMA_IF(N) HBOOST_PP_ENUM(N, ARG_TYPE_STR_A, 0)
                )
            );
            return bp::object();
        }
        ...
        Install<CLS>(
            ...
            Base::_func(
                HBOOST_PP_ENUM(N, EXTRACT_REQ_ARG_A, args) HBOOST_PP_COMMA_IF(N)
...
#if N > 0
#undef PARAMLIST
#define PARAMLIST HBOOST_PP_ENUM_BINARY_PARAMS(N, A, a)
