...
#define TF_PY_REPR_PREFIX \
    std::string(HBOOST_PP_STRINGIZE(MFB_PACKAGE_MODULE) ".")
...
hboost::python::object TfPyObject(T const &t, bool complainOnFailure = true) {
    ...
    try {
        return hboost::python::object(t);
    } catch (hboost::python::error_already_set const &) {
        Tf_PyObjectError(complainOnFailure);
        return hboost::python::object();
   }
}
...
hboost::python::object TfPyObject(PyObject* t, bool complainOnFailure = true) {
    TfPyLock pyLock;
    return hboost::python::object(hboost::python::handle<>(t));
}
...
TF_API
void
Tf_PyWrapOnceImpl(hboost::python::type_info const &,
                  hboost::function<void()> const&,
                  bool *);

/// Invokes \p wrapFunc to wrap type \c T if \c T is not already wrapped.
///
/// Executing \p wrapFunc *must* register \c T with hboost python.  Otherwise,
...
template <typename T>
void
TfPyWrapOnce(hboost::function<void()> const &wrapFunc)
{
    ...
    Tf_PyWrapOnceImpl(hboost::python::type_id<T>(), wrapFunc, &isTypeWrapped);
}
...
/// hboost::python::error_already_set.
template <class Seq>
hboost::python::object TfPyCopySequenceToSet(Seq const &seq) {
    TfPyLock lock;
    hboost::python::handle<> set{hboost::python::allow_null(PySet_New(nullptr))};
    if (!set) {
        hboost::python::throw_error_already_set();
    }
    for (auto const& item : seq) {
        hboost::python::object obj(item);
        if (PySet_Add(set.get(), obj.ptr()) == -1) {
            hboost::python::throw_error_already_set();
        }
    }
    return hboost::python::object(set);
}

template<class Seq>
hboost::python::tuple TfPyCopySequenceToTuple(Seq const &seq) {
    return hboost::python::tuple(TfPyCopySequenceToList(seq));
}
...
TF_API bool Tf_PyEvaluateWithErrorCheck(
    const std::string & expr, hboost::python::object * obj);
...
bool TfPyEvaluateAndExtract(const std::string & expr, T * t)
{
    ...
    // Take the lock before doing anything with hboost::python.
    TfPyLock lock;
    ...
    // hboost::python::object, since it will increment and decrement ref counts 
    // outside of the call to TfPyEvaluate.
    hboost::python::object obj;
    if (!Tf_PyEvaluateWithErrorCheck(expr, &obj))
        return false;

    hboost::python::extract<T> extractor(obj);
