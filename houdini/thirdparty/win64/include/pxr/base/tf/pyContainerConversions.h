...
struct TfPySequenceToPython
{
    ...
    {
        hboost::python::list result;
        TF_FOR_ALL(i, c) {
            result.append(*i);
        }
        return hboost::python::incref(result.ptr());
    }
};
...
struct TfPyMapToPythonDict
{
    static PyObject* convert(ContainerType const &c)
    {
        return hboost::python::incref(TfPyCopyMapToDictionary(c).ptr());
    }    
};
...
namespace TfPyContainerConversions {
  ...
  struct to_tuple
  {
    static PyObject* convert(ContainerType const& a)
    {
      ...
      for(const_iter p=a.begin();p!=a.end();p++) {
        result.append(hboost::python::object(*p));
      }
      return hboost::python::incref(hboost::python::tuple(result).ptr());
    }
  };
  ...
  struct to_tuple<std::pair<First, Second> > {
    ...
    {
      hboost::python::tuple result =
        hboost::python::make_tuple(a.first, a.second);
      return hboost::python::incref(result.ptr());
    }
  };
  ...
  struct fixed_size_policy
  {
    ...
    static void assert_size(hboost::type<ContainerType>, std::size_t sz)
    {
      if (!check_size(hboost::type<ContainerType>(), sz)) {
        PyErr_SetString(PyExc_RuntimeError,
          "Insufficient elements for fixed-size array.");
        hboost::python::throw_error_already_set();
      }
    }
  ...
  struct from_python_sequence
  {
    ...
    from_python_sequence()
    {
      hboost::python::converter::registry::push_back(
        &convertible,
        &construct,
        hboost::python::type_id<ContainerType>());
    }
    ...
    static void* convertible(PyObject* obj_ptr)
    {
      ...
      hboost::python::handle<> obj_iter(
        hboost::python::allow_null(PyObject_GetIter(obj_ptr)));
      ...
      if (ConversionPolicy::check_convertibility_per_element()) {
        ...
        if (!ConversionPolicy::check_size(
          hboost::type<ContainerType>(), obj_size)) return 0;
    ...
    {
      for(;;i++) {
        hboost::python::handle<> py_elem_hdl(
          hboost::python::allow_null(PyIter_Next(obj_iter.get())));
        if (PyErr_Occurred()) {
          PyErr_Clear();
          return false;
        }
        if (!py_elem_hdl.get()) break; // end of iteration
        hboost::python::object py_elem_obj(py_elem_hdl);
        hboost::python::extract<container_element_type>
    ...
    {
      hboost::python::handle<> obj_iter(PyObject_GetIter(obj_ptr));
      void* storage = (
        (hboost::python::converter::rvalue_from_python_storage<ContainerType>*)
          data)->storage.bytes;
      ...
      for(;;i++) {
        hboost::python::handle<> py_elem_hdl(
          hboost::python::allow_null(PyIter_Next(obj_iter.get())));
        if (PyErr_Occurred()) hboost::python::throw_error_already_set();
        if (!py_elem_hdl.get()) break; // end of iteration
        hboost::python::object py_elem_obj(py_elem_hdl);
        hboost::python::extract<container_element_type> elem_proxy(py_elem_obj);
        ConversionPolicy::set_value(result, i, elem_proxy());
      }
      ConversionPolicy::assert_size(hboost::type<ContainerType>(), i);
    }
  };
  ...
  struct from_python_tuple_pair {
    ...
    from_python_tuple_pair()
    {
      hboost::python::converter::registry::push_back(
        &convertible,
        &construct,
        hboost::python::type_id<PairType>());
    }
    ...
    static void* convertible(PyObject* obj_ptr)
    {
      ...
      hboost::python::extract<first_type> e1(PyTuple_GetItem(obj_ptr, 0));
      hboost::python::extract<second_type> e2(PyTuple_GetItem(obj_ptr, 1));
    ...
    {
      void* storage = (
        (hboost::python::converter::rvalue_from_python_storage<PairType>*)
          data)->storage.bytes;
      hboost::python::extract<first_type>  e1(PyTuple_GetItem(obj_ptr, 0));
      hboost::python::extract<second_type> e2(PyTuple_GetItem(obj_ptr, 1));
  ...
  {
    to_tuple_mapping() {
      hboost::python::to_python_converter<
        ContainerType,
