...
class TfRefPtr {
    ...
    typedef typename hboost::mpl::if_c<
        ...
        !hboost::is_convertible<T*, TfSimpleRefBase*>::value,
...
// Extend hboost::hash to support TfRefPtr.
template <class T>
inline size_t
hash_value(const TfRefPtr<T>& ptr)
{
    ...
    return hboost::hash<decltype(refBase)>()(refBase);
}

#endif // !doxygen

#define TF_SUPPORTS_REFPTR(T)   hboost::is_base_of<TfRefBase, T >::value
