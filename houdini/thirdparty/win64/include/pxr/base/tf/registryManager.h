...
///
class TfRegistryManager : hboost::noncopyable {
public:
...
struct Tf_RegistryStaticInit {
    Tf_RegistryStaticInit() {
        Tf_RegistryInitCtor(HBOOST_PP_STRINGIZE(MFB_ALT_PACKAGE_NAME));
    }
    ~Tf_RegistryStaticInit() {
        Tf_RegistryInitDtor(HBOOST_PP_STRINGIZE(MFB_ALT_PACKAGE_NAME));
    }
};
}
...
#define TF_REGISTRY_DEFINE_WITH_TYPE(KEY_TYPE, TAG)                            \
    ...
    ARCH_CONSTRUCTOR(HBOOST_PP_CAT(_Tf_RegistryAdd, __LINE__),                  \
                     TF_REGISTRY_PRIORITY, KEY_TYPE*, TAG*)                    \
    {                                                                          \
        Tf_RegistryInit::Add(HBOOST_PP_STRINGIZE(MFB_ALT_PACKAGE_NAME),         \
                             (void(*)(KEY_TYPE*, TAG*))_Tf_RegistryFunction,   \
                             HBOOST_PP_STRINGIZE(KEY_TYPE));                    \
    }                                                                          \
...
#define TF_REGISTRY_DEFINE(KEY_TYPE, NAME)                                     \
    static void HBOOST_PP_CAT(_Tf_RegistryFunction, NAME)(KEY_TYPE*, void*);    \
    ARCH_CONSTRUCTOR(HBOOST_PP_CAT(_Tf_RegistryAdd, NAME),                      \
                     TF_REGISTRY_PRIORITY, KEY_TYPE*)                          \
    {                                                                          \
        Tf_RegistryInit::Add(HBOOST_PP_STRINGIZE(MFB_ALT_PACKAGE_NAME),         \
                             (void(*)(KEY_TYPE*, void*))                       \
                             HBOOST_PP_CAT(_Tf_RegistryFunction, NAME),         \
                             HBOOST_PP_STRINGIZE(KEY_TYPE));                    \
    }                                                                          \
    _ARCH_ENSURE_PER_LIB_INIT(Tf_RegistryStaticInit, _tfRegistryInit);         \
    static void HBOOST_PP_CAT(_Tf_RegistryFunction, NAME)(KEY_TYPE*, void*)
...
#define TF_REGISTRY_FUNCTION_WITH_TAG(KEY_TYPE, TAG) \
    TF_REGISTRY_DEFINE(KEY_TYPE, HBOOST_PP_CAT(TAG, __LINE__))
