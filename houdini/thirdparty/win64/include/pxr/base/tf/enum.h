...
/// \endcode
///
class TfEnum : hboost::totally_ordered<TfEnum>
{
public:
    ...
    template <class T>
    TfEnum(T value,
           typename hboost::enable_if<hboost::is_enum<T> >::type * = 0)
    ...
    template <class T>
    typename hboost::enable_if<hboost::is_enum<T>, bool>::type
    ...
    template <class T>
    friend typename hboost::enable_if<hboost::is_enum<T>, bool>::type
...
#define TF_ADD_ENUM_NAME(VAL, ...)                                      \
    TfEnum::_AddName(VAL,                                               \
                     HBOOST_PP_STRINGIZE(VAL)                            \
                     HBOOST_PP_COMMA_IF(TF_NUM_ARGS(__VA_ARGS__))        \
                     __VA_ARGS__)
