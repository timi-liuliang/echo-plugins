...
// Specializations for hboost::python::pointee and get_pointer for TfRefPtr and
// TfWeakPtr.
...
struct Tf_PyOwnershipHelper<Ptr,
    typename hboost::enable_if<
        hboost::mpl::and_<hboost::is_same<TfRefPtr<typename Ptr::DataType>, Ptr>,
            hboost::is_base_of<TfRefBase, typename Ptr::DataType> > >::type>
{
    struct _RefPtrHolder {
        static hboost::python::object
        Get(Ptr const &refptr) {
            TfPyLock pyLock;
            _WrapIfNecessary();
            return hboost::python::object(_RefPtrHolder(refptr));
        }
        static void _WrapIfNecessary() {
            ...
            if (TfPyIsNone(TfPyGetClassObject<_RefPtrHolder>())) {
                ...
                name = TfStringReplace(name, ">", "_");
                name = TfStringReplace(name, "::", "_");
                hboost::python::class_<_RefPtrHolder>(name.c_str(),
                                                     hboost::python::no_init);
            }
        }
      private:
...
template <class Ptr>
typename hboost::enable_if<Tf_PyIsRefPtr<Ptr> >::type
Tf_PySetPythonIdentity(Ptr const &, PyObject *)
{
}

template <class Ptr>
typename hboost::disable_if<Tf_PyIsRefPtr<Ptr> >::type
