...
struct Tf_PyNoticeObjectGenerator {
    ...
    // Produce a hboost::python::object for the correct derived type of \a n.
    TF_API static hboost::python::object Invoke(TfNotice const &n);

private:

    template <typename T>
    static hboost::python::object _Generate(TfNotice const &n) {
        // Python locking is left to the caller.
        return hboost::python::object(static_cast<T const &>(n));
    }
...
public:
    ...
    typedef typename hboost::mpl::if_<
        hboost::is_same<NoticeType, TfNotice>
        , hboost::python::bases<>, hboost::python::bases<BaseType> >::type Bases;
    ...
    static ClassType Wrap(std::string const &name = std::string()) {
        ...
        return ClassType(wrappedName.c_str(), hboost::python::no_init)
    ...
    virtual hboost::python::handle<> GetNoticePythonObject() const {
        TfPyLock lock;
        return hboost::python::handle<>(hboost::python::borrowed(_self));
    }
