...
#ifdef PXR_PYTHON_SUPPORT_ENABLED
VT_VALUE_SET_STORED_TYPE(hboost::python::object, TfPyObjWrapper);
#endif // PXR_PYTHON_SUPPORT_ENABLED
...
struct Vt_ValueGetStored 
    : Vt_ValueStoredType<typename hboost::decay<T>::type> {};
...
/// checks such as provided by hboost::numeric_cast(), and will fail, returning
...
class VtValue
{
    ...
    template <class T>
    struct _IsTriviallyCopyable : hboost::mpl::and_<
        hboost::has_trivial_constructor<T>,
        hboost::has_trivial_copy<T>,
        hboost::has_trivial_assign<T>,
        hboost::has_trivial_destructor<T> > {};

    // Metafunction that returns true if T should be stored locally, false if it
    // should be stored remotely.
    template <class T>
    struct _UsesLocalStore : hboost::mpl::bool_<
    ...
    struct _ArrayHelper<Array,
                        typename hboost::enable_if<VtIsArray<Array> >::type> {
    ...
    private:
        ...
        static TfPyObjWrapper _GetPyObj(_Storage const &storage) {
            TfPyLock lock;
            return hboost::python::api::object(GetObj(storage));
        }
#endif // PXR_PYTHON_SUPPORT_ENABLED
    ...
    struct _RemoteTypeInfo : _TypeInfoImpl<
        T,                                  // type
        hboost::intrusive_ptr<_Counted<T> >, // container
        _RemoteTypeInfo<T>                  // CRTP
        >
    {
        constexpr _RemoteTypeInfo()
            : _TypeInfoImpl<
                  T, hboost::intrusive_ptr<_Counted<T>>, _RemoteTypeInfo<T>>()
        {}

        typedef hboost::intrusive_ptr<_Counted<T> > Ptr;
    ...
    struct _TypeInfoFor {
        ...
        typedef typename hboost::mpl::if_<_UsesLocalStore<T>,
                                         _LocalTypeInfo<T>,
    ...
    struct _TypeInfoFor<char[N]> {
        // return _UsesLocalStore(T) ? _LocalTypeInfo<T> : _RemoteTypeInfo<T>;
        typedef typename hboost::mpl::if_<_UsesLocalStore<std::string>,
    ...
    typename hboost::enable_if<
        hboost::is_same<T, typename Vt_ValueGetStored<T>::Type> >::type
    _Init(T const &obj) {
        _info = GetTypeInfo<T>();
...
public:
    ...
    /// std::string. If T is hboost::python::object, produce a VtValue holding
    /// a TfPyObjWrapper.
    template <class T>
...
private:
    ...
    typename hboost::enable_if<VtIsValueProxy<Proxy>, Proxy &>::type
    _GetMutable() {
    ...
    template <class T>
    typename hboost::disable_if<VtIsValueProxy<T>, T &>::type
    _GetMutable() {
    ...
    template <class Proxy>
    typename hboost::enable_if<VtIsValueProxy<Proxy>, Proxy const &>::type
    _Get() const {
        typedef typename _TypeInfoFor<Proxy>::Type TypeInfo;
    ...
    template <class T>
    typename hboost::disable_if<VtIsValueProxy<T>, T const &>::type
    _Get() const {
        typedef typename _TypeInfoFor<T>::Type TypeInfo;
    ...
    // hboost::python wrapped object corresponding to the held type.  This
...
struct Vt_DefaultValueFactory {
    ...
    static Vt_DefaultValueHolder Invoke() {
        return Vt_DefaultValueHolder::Create<T>(
            hboost::value_initialized<T>().data());
    }
};
...
// arrays of objects.  However, hboost::value_initialized<T>() no
