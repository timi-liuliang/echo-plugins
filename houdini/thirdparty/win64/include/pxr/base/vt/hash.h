...
// We make unqualified calls, intending to pick up hboost::hash_value if an
// overload isn't found by ADL.
using hboost::hash_value;
...
/// (hboost::)hash_value() or TfHash()() on a T instance.
template <class T>
constexpr bool
VtIsHashable() {
...
/// Compute a hash code for \p val by invoking (hboost::)hash_value(val) if
