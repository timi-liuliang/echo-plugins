...
HBOOST_PP_CAT(Vt, HBOOST_PP_CAT(VT_TYPE_NAME(elem), Array)) ;
HBOOST_PP_SEQ_FOR_EACH(VT_ARRAY_TYPEDEF, ~, VT_SCALAR_VALUE_TYPES)

// The following preprocessor code generates the hboost pp sequence for
// all array value types (VT_ARRAY_VALUE_TYPES)
#define VT_ARRAY_TYPE_TUPLE(r, unused, elem) \
(( HBOOST_PP_CAT(Vt, HBOOST_PP_CAT(VT_TYPE_NAME(elem), Array)) , \
   HBOOST_PP_CAT(VT_TYPE_NAME(elem), Array) ))
#define VT_ARRAY_VALUE_TYPES \
HBOOST_PP_SEQ_FOR_EACH(VT_ARRAY_TYPE_TUPLE, ~, VT_SCALAR_VALUE_TYPES)
