...
HBOOST_PP_SEQ_FOR_EACH(MAKE_STREAM_FUNC, ~, _OPTIMIZED_STREAM_INTEGRAL_TYPES)
...
{
    return hboost::lexical_cast<std::string>(self);
}
...
#define VtCat_DEF(z, n, unused) \
    ...
    HBOOST_PP_REPEAT_FROM_TO(1, VT_FUNCTIONS_MAX_ARGS, VtCat_DEF, ~)
#undef VtCat_DEF
...
Vt_ConvertFromPySequence(TfPyObjWrapper const &obj)
{
    ...
    if (PySequence_Check(obj.ptr())) {
        ...
        for (size_t i = 0; i != len; ++i) {
            hboost::python::handle<> h(PySequence_ITEM(obj.ptr(), i));
            if (!h) {
                if (PyErr_Occurred())
                    PyErr_Clear();
                return VtValue();
            }
            hboost::python::extract<ElemType> e(h.get());
            if (!e.check())
                return VtValue();
            *elem++ = e();
        }
