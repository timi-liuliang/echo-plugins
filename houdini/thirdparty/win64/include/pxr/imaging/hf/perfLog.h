...
#define HF_MALLOC_TAG_FUNCTION() \
    TfAutoMallocTag2 tagFunc(HBOOST_PP_STRINGIZE(MFB_PACKAGE_NAME), \
...
#define HF_MALLOC_TAG(x) \
    TfAutoMallocTag2 tag2(HBOOST_PP_STRINGIZE(MFB_PACKAGE_NAME), x);

///
/// Overrides operator new/delete and injects malloc tags.
///
#define HF_MALLOC_TAG_NEW(x) \
    TF_MALLOC_TAG_NEW(HBOOST_PP_STRINGIZE(MFB_PACKAGE_NAME), x);
