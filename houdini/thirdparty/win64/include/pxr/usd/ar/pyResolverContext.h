...
{
    hboost::python::extract<const Context&> x(obj);
    if (x.check()) {
        if (context) {
    ...
    TfPyObjWrapper* obj)
{
    if (const Context* contextObj = context.Get<Context>()) {
        if (obj) {
            TfPyLock lock;
            *obj = hboost::python::object(*contextObj);
        }
        return true;
    }
    return false;
...
typedef hboost::function<bool(PyObject*, ArResolverContext*)> 
    Ar_MakeResolverContextFromPythonFn;
typedef hboost::function<bool(const ArResolverContext&, TfPyObjWrapper*)>
