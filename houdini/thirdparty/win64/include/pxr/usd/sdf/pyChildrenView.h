...
private:
    struct _ExtractItem {
        static hboost::python::object Get(const View& x, const const_iterator& i)
        {
            return hboost::python::make_tuple(x.key(i), *i);
        }
    };
    ...
    struct _ExtractKey {
        static hboost::python::object Get(const View& x, const const_iterator& i)
        {
            return hboost::python::object(x.key(i));
        }
    };
    ...
    public:
        _Iterator(const hboost::python::object& object) :
            _object(object),
            _owner(hboost::python::extract<const View&>(object)),
            _cur(_owner.begin()),
            _end(_owner.end())
        {
            // Do nothing
        ...
        hboost::python::object GetNext()
        {
            ...
            hboost::python::object result = E::Get(_owner, _cur);
            ++_cur;
            return result;
        }
    ...
    static hboost::python::object _PyGet(const View& x, const key_type& key)
    {
        const_iterator i = x.find(key);
    ...
    template <class E>
    static hboost::python::list _Get(const View& x)
    {
    ...
    static hboost::python::list _GetItems(const View& x)
    {
        return _Get<_ExtractItem>(x);
    }

    static hboost::python::list _GetKeys(const View& x)
    {
        return _Get<_ExtractKey>(x);
    }

    static hboost::python::list _GetValues(const View& x)
    {
