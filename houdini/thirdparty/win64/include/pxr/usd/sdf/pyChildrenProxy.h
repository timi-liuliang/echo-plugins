...
private:
    ...
    struct _ExtractItem {
        static hboost::python::object Get(const _const_iterator& i)
        {
    ...
    public:
        _Iterator(const hboost::python::object& object) :
            _object(object),
            _owner(hboost::python::extract<const This&>(object)()._proxy)
        {
            _cur = _owner.begin();
        }
    ...
    void _SetItemBySlice(const hboost::python::slice& slice,
    ...
    hboost::python::object _PyGet(const key_type& key) const
    {
        _view_const_iterator i = _GetView().find(key);
        return i == _GetView().end() ? hboost::python::object() :
                                       hboost::python::object(*i);
    }

    hboost::python::object _PyGetDefault(const key_type& key,
                                        const mapped_type& def) const
    {
        _view_const_iterator i = _GetView().find(key);
        return i == _GetView().end() ? hboost::python::object(def) :
                                       hboost::python::object(*i);
    }
    ...
    static
    _Iterator<_ExtractItem> _GetItemIterator(const hboost::python::object &x)
    {
        return _Iterator<_ExtractItem>(x);
    }

    static
    _Iterator<_ExtractKey> _GetKeyIterator(const hboost::python::object &x)
    {
        return _Iterator<_ExtractKey>(x);
    }

    static
    _Iterator<_ExtractValue> _GetValueIterator(const hboost::python::object &x)
    {
        return _Iterator<_ExtractValue>(x);
    }

    template <class E>
    hboost::python::list _Get() const
    {
    ...
    hboost::python::list _GetItems() const
    {
        return _Get<_ExtractItem>();
    }

    hboost::python::list _GetKeys() const
    {
        return _Get<_ExtractKey>();
    }

    hboost::python::list _GetValues() const
    {
