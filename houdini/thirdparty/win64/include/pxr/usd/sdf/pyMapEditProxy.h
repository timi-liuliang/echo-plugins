...
private:
    ...
    struct _ExtractItem {
        static hboost::python::object Get(const const_iterator& i)
        {
            return hboost::python::make_tuple(i->first, i->second);
        }
    };
    ...
    struct _ExtractKey {
        static hboost::python::object Get(const const_iterator& i)
        {
            return hboost::python::object(i->first);
        }
    };
    ...
    struct _ExtractValue {
        static hboost::python::object Get(const const_iterator& i)
        {
            return hboost::python::object(i->second);
        }
    };
    ...
    class _Iterator {
    public:
        _Iterator(const hboost::python::object& object) :
            _object(object),
            _owner(hboost::python::extract<const Type&>(object)),
            _cur(_owner.begin()),
            _end(_owner.end())
        {
            // Do nothing
        ...
        hboost::python::object GetNext()
        {
            if (_cur == _end) {
                TfPyThrowStopIteration("End of MapEditProxy iteration");
            }
            hboost::python::object result = E::Get(_cur);
            ++_cur;
            return result;
        }

    private:
        hboost::python::object _object;
        const Type& _owner;
        const_iterator _cur;
        const_iterator _end;
    };
    ...
    _GetItemIterator(const hboost::python::object& x)
    {
        return _Iterator<_ExtractItem>(x);
    }

    static _Iterator<_ExtractKey> 
    _GetKeyIterator(const hboost::python::object& x)
    {
        return _Iterator<_ExtractKey>(x);
    }

    static _Iterator<_ExtractValue> 
    _GetValueIterator(const hboost::python::object& x)
    {
        return _Iterator<_ExtractValue>(x);
    }

    static hboost::python::object _PyGet(const Type& x, const key_type& key)
    {
        const_iterator i = x.find(key);
        return i == x.end() ? hboost::python::object() :
                              hboost::python::object(i->second);
    }
    ...
    template <class E>
    static hboost::python::list _Get(const Type& x)
    {
    ...
    static hboost::python::list _GetItems(const Type& x)
    {
        return _Get<_ExtractItem>(x);
    }

    static hboost::python::list _GetKeys(const Type& x)
    {
        return _Get<_ExtractKey>(x);
    }

    static hboost::python::list _GetValues(const Type& x)
    {
    ...
    static hboost::python::tuple _PopItem(Type& x)
    {
        if (x.empty()) {
            TfPyThrowKeyError("MapEditProxy is empty");
            return hboost::python::tuple();
        }
        else {
            iterator i = x.begin();
            value_type result = *i;
            x.erase(i);
            return hboost::python::make_tuple(result.first, result.second);
        }
    }
    ...
    static void _UpdateDict(Type& x, const hboost::python::dict& d)
    {
        _UpdateList(x, d.items());
    }

    static void _UpdateList(Type& x, const hboost::python::list& pairs)
    {
        using namespace hboost::python;
