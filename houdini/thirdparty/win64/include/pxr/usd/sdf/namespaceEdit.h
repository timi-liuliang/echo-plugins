...
struct SdfNamespaceEdit :
    hboost::equality_comparable<SdfNamespaceEdit> {
public:
...
///
struct SdfNamespaceEditDetail :
    hboost::equality_comparable<SdfNamespaceEditDetail> {
public:
    /// Validity of an edit.
    enum Result {
...
public:
    ...
    typedef hboost::function<bool(const SdfPath&)> HasObjectAtPath;

    /// Functor that returns \c true iff the namespace edit will succeed.
    /// If not it returns \c false and sets the string argument.
    typedef hboost::function<bool(const SdfNamespaceEdit&,std::string*)> CanEdit;
