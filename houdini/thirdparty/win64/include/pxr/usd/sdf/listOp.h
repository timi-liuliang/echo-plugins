...
public:
    ...
    typedef hboost::function<
        hboost::optional<ItemType>(SdfListOpType, const ItemType&)
    > ApplyCallback;
    ...
    friend inline size_t hash_value(const SdfListOp &op) {
        size_t h = 0;
        hboost::hash_combine(h, op._isExplicit);
        hboost::hash_combine(h, op._explicitItems);
        hboost::hash_combine(h, op._addedItems);
        hboost::hash_combine(h, op._prependedItems);
        hboost::hash_combine(h, op._appendedItems);
        hboost::hash_combine(h, op._deletedItems);
        hboost::hash_combine(h, op._orderedItems);
        return h;
    }
