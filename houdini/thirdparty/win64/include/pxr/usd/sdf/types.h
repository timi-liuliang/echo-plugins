...
#define _SDF_UNITSLIST_ENUM(elem) HBOOST_PP_CAT(HBOOST_PP_CAT(Sdf, \
...
enum _SDF_UNITSLIST_ENUM(elem) {                         \
    HBOOST_PP_SEQ_FOR_EACH(_SDF_DECLARE_UNIT_ENUMERANT,   \
                          _SDF_UNITSLIST_CATEGORY(elem), \
                          _SDF_UNITSLIST_TUPLES(elem))   \
};
HBOOST_PP_LIST_FOR_EACH(_SDF_DECLARE_UNIT_ENUM, ~, _SDF_UNITS)
...
#define _SDF_UNIT_MAX_UNITS \
    HBOOST_PP_LIST_FOLD_LEFT(_SDF_MAX_UNITS_OP, 0, _SDF_UNITS)
...
#define SDF_VALUE_TAG(tup) HBOOST_PP_TUPLE_ELEM(4, 0, tup)
#define SDF_VALUE_TYPENAME(tup) HBOOST_PP_TUPLE_ELEM(4, 1, tup)
#define SDF_VALUE_CPP_TYPE(tup) HBOOST_PP_TUPLE_ELEM(4, 2, tup)
#define SDF_VALUE_TUPLE_DIM(tup) \
    TF_PP_TUPLE_TO_LIST(HBOOST_PP_TUPLE_ELEM(4, 3, tup))
#define SDF_VALUE_TRAITS_TYPE(tup) \
    HBOOST_PP_CAT(SdfValueType, SDF_VALUE_TAG(tup))
...
struct SDF_VALUE_TRAITS_TYPE(elem) {                                        \
    ...
    static std::string Name() {                                             \
        return HBOOST_PP_STRINGIZE(SDF_VALUE_TYPENAME(elem));                \
    }                                                                       \
    ...
    static SdfTupleDimensions Dimensions() {                                \
        ...
        static_assert(                                                      \
            HBOOST_PP_LIST_SIZE(SDF_VALUE_TUPLE_DIM(elem)) <= 2,             \
...
HBOOST_PP_SEQ_FOR_EACH(SDF_DECLARE_VALUE_TYPE_S, ~, _SDF_SCALAR_VALUE_TYPES);
HBOOST_PP_SEQ_FOR_EACH(SDF_DECLARE_VALUE_TYPE, ~, _SDF_DIMENSIONED_VALUE_TYPES);
...
HBOOST_PP_COMMA() SDF_VALUE_TRAITS_TYPE(elem)

/// An mpl sequence of all available traits types for value types
/// defined by Sdf.
typedef ::hboost::mpl::vector<
    SDF_VALUE_TRAITS_TYPE(HBOOST_PP_SEQ_HEAD(_SDF_SCALAR_VALUE_TYPES))
    HBOOST_PP_SEQ_FOR_EACH(_SDF_WRITE_VALUE_TRAITS_TYPE, ~,
                          HBOOST_PP_SEQ_TAIL(_SDF_SCALAR_VALUE_TYPES))
    > Sdf_ScalarValueTraitsTypesVector;
typedef ::hboost::mpl::vector<
    SDF_VALUE_TRAITS_TYPE(HBOOST_PP_SEQ_HEAD(_SDF_DIMENSIONED_VALUE_TYPES))
    HBOOST_PP_SEQ_FOR_EACH(_SDF_WRITE_VALUE_TRAITS_TYPE, ~,
                          HBOOST_PP_SEQ_TAIL(_SDF_DIMENSIONED_VALUE_TYPES))
    > Sdf_DimensionedValueTraitsTypesVector;

typedef ::hboost::mpl::joint_view<
...
/// by Sdf.
typedef hboost::mpl::transform_view<
...
class SdfUnregisteredValue : 
    public hboost::equality_comparable<SdfUnregisteredValue>
{
public:
...
class Sdf_ValueTypeNamesType : hboost::noncopyable {
public:
