...
///
class SdfReference : hboost::totally_ordered<SdfReference> {
public:
    ...
    friend inline size_t hash_value(const SdfReference &r) {
        size_t h = 0;
        hboost::hash_combine(h, r._assetPath);
        hboost::hash_combine(h, r._primPath);
        hboost::hash_combine(h, r._layerOffset);
        hboost::hash_combine(h, r._customData);
        return h;
    }
