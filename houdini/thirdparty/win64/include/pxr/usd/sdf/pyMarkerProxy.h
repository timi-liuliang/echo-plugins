...
private:
    struct _ExtractItem {
        static hboost::python::object Get(const OwnerSpecHandle& spec,
                                         const SdfPath& markerPath)
        {
            return hboost::python::make_tuple(
    ...
    struct _ExtractKey {
        static hboost::python::object Get(const OwnerSpecHandle& spec,
                                         const SdfPath& markerPath)
        {
            return hboost::python::object(markerPath);
        }
    };
    ...
    public:
        ...
        hboost::python::object GetNext()
        {
            ...
            hboost::python::object result = E::Get(_spec, _markerPaths.front());
    ...
    hboost::python::object _PyGet(const key_type& key)
    {
        if (!_Validate()) {
            return hboost::python::object();
        }
        ...
        const std::string marker = MarkerPolicy::GetMarker(_spec, key);
        return marker.empty() ? hboost::python::object() :
                                hboost::python::object(marker);
    }
    ...
    hboost::python::list _GetItems()
    {
        return Get<_ExtractItem>();
    }

    hboost::python::list _GetKeys()
    {
        return Get<_ExtractKey>();
    }

    hboost::python::list _GetValues()
    {
    ...
    hboost::python::tuple _PopItem()
    {
        ...
        if (markerPaths.empty()) {
            ...
            return hboost::python::make_tuple(markerPaths.front(), marker);
        }
    }
    ...
    void _UpdateDict(const hboost::python::dict& d)
    {
        _UpdateList(d.items());
    }

    void _UpdateList(const hboost::python::list& pairs)
    {
    ...
    void Copy(const hboost::python::dict& other)
    {
        if (!_Validate()) {
            return;
        }
        ...
        hboost::python::list keys = other.keys();
        size_t numKeys = len(other);
        for (size_t i = 0; i != numKeys; i++) {
            key_type key    = hboost::python::extract<key_type>(keys[i]);
            mapped_type val = hboost::python::extract<mapped_type>(other[keys[i]]);
            markerMap[key] = val;
        }
