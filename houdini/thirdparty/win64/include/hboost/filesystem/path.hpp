...
#ifndef HBOOST_FILESYSTEM_PATH_HPP
#define HBOOST_FILESYSTEM_PATH_HPP
...
#include <hboost/filesystem/path_traits.hpp>  // includes <cwchar>
...
#include <hboost/io/detail/quoted_manip.hpp>
...
namespace filesystem
{
  ...
  class HBOOST_FILESYSTEM_DECL path
  {
  public:
...
# ifdef HBOOST_WINDOWS_API
    ...
    HBOOST_STATIC_CONSTEXPR value_type      preferred_separator = L'\\';
# else 
    typedef char                           value_type;
    HBOOST_STATIC_CONSTEXPR value_type      preferred_separator = '/';
# endif
    ...
    path() HBOOST_NOEXCEPT {}                                          
    path(const path& p) : m_pathname(p.m_pathname) {}

    template <class Source>
    path(Source const& source,
      typename hboost::enable_if<path_traits::is_pathable<
        typename hboost::decay<Source>::type> >::type* =0)
    {
...
# if !defined(HBOOST_NO_CXX11_RVALUE_REFERENCES)
    path(path&& p) HBOOST_NOEXCEPT { m_pathname = std::move(p.m_pathname); }
    path& operator=(path&& p) HBOOST_NOEXCEPT
...
# endif
    ...
    template <class Source>
      typename hboost::enable_if<path_traits::is_pathable<
        typename hboost::decay<Source>::type>, path&>::type
    ...
    template <class CharT>
      typename hboost::enable_if<is_integral<CharT>, path&>::type
    operator+=(CharT c)
    {
      CharT tmp[2];
      tmp[0] = c;
    ...
    void   clear() HBOOST_NOEXCEPT             { m_pathname.clear(); }
    path&  make_preferred()
#   ifdef HBOOST_POSIX_API
      { return *this; }  // POSIX no effect
#   else // HBOOST_WINDOWS_API
      ;  // change slashes to backslashes
#   endif
    path&  remove_filename();
    path&  remove_trailing_separator();
    path&  replace_extension(const path& new_extension = path());
    void   swap(path& rhs) HBOOST_NOEXCEPT     { m_pathname.swap(rhs.m_pathname); }
    ...
    const string_type&  native() const HBOOST_NOEXCEPT  { return m_pathname; }
    const value_type*   c_str() const HBOOST_NOEXCEPT   { return m_pathname.c_str(); }
    string_type::size_type size() const HBOOST_NOEXCEPT { return m_pathname.size(); }
...
#   endif
    ...
    int compare(const path& p) const HBOOST_NOEXCEPT;  // generic, lexicographical
    ...
    bool empty() const HBOOST_NOEXCEPT{ return m_pathname.empty(); }
...
# if defined(HBOOST_FILESYSTEM_DEPRECATED) && defined(HBOOST_FILESYSTEM_NO_DEPRECATED)
#   error both HBOOST_FILESYSTEM_DEPRECATED and HBOOST_FILESYSTEM_NO_DEPRECATED are defined
# endif
...
# if defined(HBOOST_FILESYSTEM_DEPRECATED)
...
# endif
  ...
  private:
    ...
    friend class hboost::filesystem::path;
    friend class hboost::filesystem::path::reverse_iterator;
...
# endif
  ...
  //    use hboost::io::quoted() to handle spaces in paths
  ...
  {
    return os
      << hboost::io::quoted(p.template string<std::basic_string<Char> >(), static_cast<Char>('&'));
  }
  ...
  operator>>(std::basic_istream<Char, Traits>& is, path& p)
  {
    ...
    is >> hboost::io::quoted(str, static_cast<Char>('&'));
    p = str;
    return is;
  }
  ...
  HBOOST_FILESYSTEM_DECL bool portable_posix_name(const std::string & name);
  HBOOST_FILESYSTEM_DECL bool windows_name(const std::string & name);
  HBOOST_FILESYSTEM_DECL bool portable_name(const std::string & name);
  HBOOST_FILESYSTEM_DECL bool portable_directory_name(const std::string & name);
  HBOOST_FILESYSTEM_DECL bool portable_file_name(const std::string & name);
  HBOOST_FILESYSTEM_DECL bool native(const std::string & name);
...
#endif  // HBOOST_FILESYSTEM_PATH_HPP
