...
//      workaround for HBOOST_NO_STDC_NAMESPACE (the library
...
#ifndef HBOOST_TOKEN_FUNCTIONS_JRB120303_HPP_
#define HBOOST_TOKEN_FUNCTIONS_JRB120303_HPP_
...
// MSVC does not like the following typename
  template <class Char,
    class Traits = HBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
  ...
  private:
    ...
    void do_escape(iterator& next,iterator end,Token& tok) {
      if (++next == end)
        HBOOST_THROW_EXCEPTION(escaped_list_error(std::string("cannot end with escape")));
      if (Traits::eq(*next,'n')) {
        tok+='\n';
        return;
      }
        ...
        HBOOST_THROW_EXCEPTION(escaped_list_error(std::string("unknown escape sequence")));
    }
  ...
  public:
    ...
    bool operator()(InputIterator& next, InputIterator end, Token& tok)
    {
      typedef tokenizer_detail::assign_or_plus_equal<
        HBOOST_DEDUCED_TYPENAME tokenizer_detail::get_iterator_category<
          InputIterator
        >::iterator_category
      > assigner;

      HBOOST_ASSERT(!offsets_.empty());
  ...
  template <typename Char,
    typename Tr = HBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
  ...
  // The out of the box GCC 2.95 on cygwin does not have a char_traits class.
  template <class Char,
    class Tr = HBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
