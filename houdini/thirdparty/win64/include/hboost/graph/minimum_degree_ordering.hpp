...
#include <hboost/pending/bucket_sorter.hpp>
#include <hboost/detail/numeric_traits.hpp> // for integer_traits
  ...
  namespace detail {
    ...
    public:
        ...
        work_space(n_)
      {
        ...
        size_type vid = 0;
        for (hboost::tie(v, vend) = vertices(G); v != vend; ++v, ++vid)
        ...
        for (hboost::tie(v, vend) = vertices(G); v != vend; ++v) {
      ...
      void eliminate(vertex_t node)
      {
        ...
        while (!element_neighbor.empty()) {
          ...
          adj_iter i, i_end;
          for (hboost::tie(i, i_end) = adjacent_vertices(element, G); i != i_end; ++i){
            vertex_t i_node = *i;
        ...
        adj_iter v, ve;
        for (hboost::tie(v, ve) = adjacent_vertices(node, G); v != ve; ++v) {
          vertex_t v_node = *v;
      ...
      void update(Stack llist, size_type& min_degree)
      {
        ...
        while (! llist.empty()) {
          ...
          adj_iter i, ie;
          for (hboost::tie(i,ie) = adjacent_vertices(current, G); i != ie; ++i) {
            vertex_t i_node = *i;
          ...
          while (!q2list.empty()) {
            ...
            if (numbering.is_numbered(neighbor)) {
              adj_iter i, ie;
              for (hboost::tie(i,ie) = adjacent_vertices(neighbor, G);
                   i != ie; ++i) {
          ...
          while (!qxlist.empty()) {
            ...
            deg = deg0;
            adj_iter i, ie;
            for (hboost::tie(i, ie) = adjacent_vertices(u_node, G); i != ie; ++i) {
              vertex_t i_node = *i;
              if (marker.is_tagged(i_node)) 
                continue;
              marker.mark_tagged(i_node);
              ...
              if (numbering.is_numbered(i_node)) {
                adj_iter j, je;
                for (hboost::tie(j, je) = adjacent_vertices(i_node, G); j != je; ++j) {
