...
#ifndef HBOOST_GRAPH_R_C_SHORTEST_PATHS_HPP
#define HBOOST_GRAPH_R_C_SHORTEST_PATHS_HPP
  ...
  Visitor vis )
{
  ...
  while( !unprocessed_labels.empty()  && vis.on_enter_loop(unprocessed_labels, g) )
  {
    ...
    {
      typename hboost::graph_traits<Graph>::vertex_descriptor
    ...
    if( !cur_label->b_is_dominated )
    {
      ...
      for( hboost::tie( oei, oei_end ) = out_edges( cur_vertex, g ); 
           oei != oei_end; 
           ++oei )
      {
        b_feasible = true;
...
#endif // HBOOST_GRAPH_R_C_SHORTEST_PATHS_HPP
