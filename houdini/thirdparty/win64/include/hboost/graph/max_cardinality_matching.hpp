...
#ifndef HBOOST_GRAPH_MAXIMUM_CARDINALITY_MATCHING_HPP
#define HBOOST_GRAPH_MAXIMUM_CARDINALITY_MATCHING_HPP
...
namespace hboost
{
  ...
  public:
    ...
    { 
      typedef hboost::iterator_property_map<typename std::vector<X>::iterator,
                                           VertexIndexMap> type; 
    };
    ...
    bool augment_matching()
    {
      ...
      while(!even_edges.empty() && !found_alternating_path)
      {
        ...
        if (vertex_state[w_prime] == graph::detail::V_UNREACHED)
        {
          ...
          out_edge_iterator_t ei, ei_end;
          for( hboost::tie(ei,ei_end) = out_edges(w_prime_mate, g); ei != ei_end; ++ei)
          {
                  ...
                  vertex_pair_t the_bridge)
    {
      for(vertex_descriptor_t v = x; v != stop_vertex; v = parent(v))
      {
        ...
        if (vertex_state[v] == graph::detail::V_ODD)
        {
          ...
          for(hboost::tie(oei, oei_end) = out_edges(v,g); oei != oei_end; ++oei)
          {
            if (target(*oei,g) != v)
            {
...
#endif //HBOOST_GRAPH_MAXIMUM_CARDINALITY_MATCHING_HPP
