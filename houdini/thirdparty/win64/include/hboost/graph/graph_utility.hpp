...
//=======================================================================
//
#ifndef HBOOST_GRAPH_UTILITY_HPP
#define HBOOST_GRAPH_UTILITY_HPP
...
namespace hboost {
  ...
  void print_in_edges(const IncidenceGraph& G, Name name)
  {
    ...
    for (hboost::tie(ui,ui_end) = vertices(G); ui != ui_end; ++ui) {
      ...
      for(hboost::tie(ei,ei_end) = in_edges(*ui,G); ei != ei_end; ++ei)
  ...
  void print_graph_dispatch(const IncidenceGraph& G, Name name, directed_tag)
  {
    ...
    for (hboost::tie(ui,ui_end) = vertices(G); ui != ui_end; ++ui) {
      ...
      for(hboost::tie(ei,ei_end) = out_edges(*ui,G); ei != ei_end; ++ei)
  ...
  bool is_adj_dispatch(Graph& g, Vertex a, Vertex b, bidirectional_tag)
  {
    typename graph_traits<Graph>::adjacency_iterator vi, viend, 
      adj_found;
    hboost::tie(vi, viend) = adjacent_vertices(a, g);
    ...
    typename graph_traits<Graph>::out_edge_iterator oi, oiend, 
      out_found;
    hboost::tie(oi, oiend) = out_edges(a, g);
    ...
    typename graph_traits<Graph>::in_edge_iterator ii, iiend, 
      in_found;
    hboost::tie(ii, iiend) = in_edges(b, g);
  ...
  bool in_vertex_set(Graph& g, Vertex v)
  {
    ...
    hboost::tie(vi, vi_end) = vertices(g);
  ...
  inline bool is_connected(const VertexListGraph& g, VertexColorMap color)
  {
    ...
    for (hboost::tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui)
      for (hboost::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
        if (*ui != *vi) {
          for (hboost::tie(ci, ci_end) = vertices(g); ci != ci_end; ++ci) 
  ...
  namespace graph {
    ...
    bool has_no_vertices(const Graph& g) {
      typedef typename hboost::graph_traits<Graph>::vertex_iterator vi;
    ...
    bool has_no_edges(const Graph& g) {
      typedef typename hboost::graph_traits<Graph>::edge_iterator ei;
      std::pair<ei, ei> p = edges(g);
      return (p.first == p.second);
    }

    template <typename Graph>
    bool has_no_out_edges(const typename hboost::graph_traits<Graph>::vertex_descriptor& v, const Graph& g) {
      typedef typename hboost::graph_traits<Graph>::out_edge_iterator ei;
...
#endif /* HBOOST_GRAPH_UTILITY_HPP*/
