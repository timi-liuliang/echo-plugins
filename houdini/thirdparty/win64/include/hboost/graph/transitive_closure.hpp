...
#ifndef HBOOST_GRAPH_TRANSITIVE_CLOSURE_HPP
#define HBOOST_GRAPH_TRANSITIVE_CLOSURE_HPP
...
#include <hboost/graph/strong_components.hpp>
...
namespace hboost
{
  ...
  namespace detail
  {
                           ...
                           std::vector < std::size_t > &s3)
    {
      ...
      for (std::size_t k = 0; k < s1.size(); ++k)
        s3[k] = min HBOOST_PREVENT_MACRO_SUBSTITUTION(s1[k], s2[k]);
    }
  }                             // namespace detail
                            ...
                            VertexIndexMap index_map)
  {
    ...
    HBOOST_CONCEPT_ASSERT(( VertexMutableGraphConcept < GraphTC > ));
    HBOOST_CONCEPT_ASSERT(( EdgeMutableGraphConcept < GraphTC > ));
    HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept < VertexIndexMap,
      vertex > ));
    ...
    typedef hboost::adjacency_list<hboost::vecS, hboost::vecS, hboost::directedS> CG_t;
    CG_t CG(num_scc);
    ...
    for (size_type i = 0; i < num_vertices(CG); ++i) {
      typedef typename hboost::graph_traits<CG_t>::adjacency_iterator cg_adj_iter;
      ...
      std::sort(CG_vec[i].begin(), CG_vec[i].end(),
                hboost::bind(std::less<cg_vertex>(),
                            hboost::bind(detail::subscript(topo_number), _1),
                            hboost::bind(detail::subscript(topo_number), _2)));
    }
      ...
      for (hboost::tie(i, i_end) = vertices(g); i != i_end; ++i)
        {
          ...
          for (hboost::tie(ab, ae) = adjacent_vertices(*i, g); ab != ae; ++ab)
            {
              if (*ab == *i)
  ...
  template < typename G > void warshall_transitive_closure(G & g)
  {
    ...
    HBOOST_CONCEPT_ASSERT(( AdjacencyMatrixConcept < G > ));
    HBOOST_CONCEPT_ASSERT(( EdgeMutableGraphConcept < G > ));
    ...
    for (hboost::tie(ki, ke) = vertices(g); ki != ke; ++ki)
      for (hboost::tie(ii, ie) = vertices(g); ii != ie; ++ii)
        if (edge(*ii, *ki, g).second)
          for (hboost::tie(ji, je) = vertices(g); ji != je; ++ji)
  ...
  template < typename G > void warren_transitive_closure(G & g)
  {
    ...
    for (hboost::tie(ic, ie) = vertices(g), ++ic; ic != ie; ++ic)
      for (hboost::tie(kc, ke) = vertices(g); *kc != *ic; ++kc)
        if (edge(*ic, *kc, g).second)
          for (hboost::tie(jc, je) = vertices(g); jc != je; ++jc)
    ...
    for (hboost::tie(ic, ie) = vertices(g), --ie; ic != ie; ++ic)
...
#endif // HBOOST_GRAPH_TRANSITIVE_CLOSURE_HPP
