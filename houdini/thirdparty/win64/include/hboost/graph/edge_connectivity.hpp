...
#ifndef HBOOST_EDGE_CONNECTIVITY
#define HBOOST_EDGE_CONNECTIVITY
...
#include <set>
#include <algorithm>
#include <hboost/graph/edmonds_karp_max_flow.hpp>
...
namespace hboost {
  ...
  namespace detail {
    ...
    min_degree_vertex(Graph& g)
    {
      ...
      for (hboost::tie(i, iend) = vertices(g); i != iend; ++i)
        if (degree(*i, g) < delta) {
          delta = degree(*i, g);
          p = *i;
        }
                   ...
                   OutputIterator result)
    {
      ...
      for (hboost::tie(ai, aend) = adjacent_vertices(u, g); ai != aend; ++ai)
        *result++ = *ai;
    }
  ...
  edge_connectivity(VertexListGraph& g, OutputIterator disconnecting_set)
  {
    ...
    for (hboost::tie(ei, ei_end) = edges(g); ei != ei_end; ++ei) {
      ...
      hboost::tie(e1, inserted) = add_edge(u, v, flow_g);
      cap[e1] = 1;
      hboost::tie(e2, inserted) = add_edge(v, u, flow_g);
    ...
    hboost::tie(p, delta) = detail::min_degree_vertex(g);
    S_star.push_back(p);
    alpha_star = delta;
    S.insert(p);
    neighbor_S.insert(p);
    ...
    while (!non_neighbor_S.empty()) { // at most n - 1 times
      ...
      if (alpha_S_k < alpha_star) {
        ...
        S_star.clear();
        for (hboost::tie(vi, vi_end) = vertices(flow_g); vi != vi_end; ++vi)
    ...
    for (si = S_star.begin(); si != S_star.end(); ++si) {
      ...
      for (hboost::tie(ei, ei_end) = out_edges(*si, g); ei != ei_end; ++ei)
...
#endif // HBOOST_EDGE_CONNECTIVITY
