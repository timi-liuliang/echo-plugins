...
//=======================================================================
#ifndef HBOOST_GRAPHVIZ_HPP
#define HBOOST_GRAPHVIZ_HPP
...
#include <hboost/graph/subgraph.hpp>
...
#include <hboost/graph/dll_import_export.hpp>
...
#include <hboost/algorithm/string/replace.hpp>
#include <hboost/xpressive/xpressive_static.hpp>
...
namespace hboost {
  ...
  inline std::string escape_dot_string(const T& obj) {
    using namespace hboost::xpressive;
    static sregex valid_unquoted_id = (((alpha | '_') >> *_w) | (!as_xpr('-') >> (('.' >> *_d) | (+_d >> !('.' >> *_d)))));
    std::string s(hboost::lexical_cast<std::string>(obj));
    if (regex_match(s, valid_unquoted_id)) {
      return s;
    } else {
      hboost::algorithm::replace_all(s, "\"", "\\\"");
      return "\"" + s + "\"";
    }
  }
  ...
  HBOOST_INSTALL_PROPERTY(edge, attribute);
  HBOOST_INSTALL_PROPERTY(vertex, attribute);
  HBOOST_INSTALL_PROPERTY(graph, graph_attribute);
  HBOOST_INSTALL_PROPERTY(graph, vertex_attribute);
  HBOOST_INSTALL_PROPERTY(graph, edge_attribute);
  ...
  {
    HBOOST_CONCEPT_ASSERT((EdgeListGraphConcept<Graph>));
    ...
    for(hboost::tie(ei, edge_end) = edges(g); ei != edge_end; ++ei) {
  ...
  namespace detail {
                                  ...
                                  VertexID vertex_id)
    {
      ...
      //print subgraph
      for ( hboost::tie(i_child,j_child) = g.children();
  ...
  bool is_directed() const
  {
    return
      hboost::is_convertible<
        typename hboost::graph_traits<MutableGraph>::directed_category,
        hboost::directed_tag>::value;
  }
  ...
  do_add_edge(const edge_t& edge, const node_t& source, const node_t& target)
  {
    ...
    if(!result.second) {
      // In the case of no parallel edges allowed
        hboost::throw_exception(bad_parallel_edge(source, target));
    } else {
  ...
  : public mutate_graph
{
 ...
 public:
  ...
  void finish_building_graph() {
    ...
    typedef hboost::tuple<id_t, bgl_vertex_t, id_t> v_prop;
    HBOOST_FOREACH(const v_prop& t, vertex_props) {
      put(hboost::get<0>(t), dp_, hboost::get<1>(t), hboost::get<2>(t));
    }
    typedef hboost::tuple<id_t, bgl_edge_t, id_t> e_prop;
    HBOOST_FOREACH(const e_prop& t, edge_props) {
      put(hboost::get<0>(t), dp_, edge_permutation_from_sorting[hboost::get<1>(t)], hboost::get<2>(t));
    }
  }
  ...
  bool is_directed() const
  {
    return
      hboost::is_convertible<
        typename hboost::graph_traits<CSRGraph>::directed_category,
        hboost::directed_tag>::value;
  }
  ...
  virtual void do_add_vertex(const node_t& node)
  {
    ...
    // node_id_prop_ allows the caller to see the real id names for nodes.
    vertex_props.push_back(hboost::make_tuple(node_id_prop_, v, node));
  }
  ...
  set_node_property(const id_t& key, const node_t& node, const id_t& value)
  {
    vertex_props.push_back(hboost::make_tuple(key, bgl_nodes[node], value));
  }

  void
  set_edge_property(const id_t& key, const edge_t& edge, const id_t& value)
  {
    edge_props.push_back(hboost::make_tuple(key, bgl_edges[edge], value));
  }
 ...
 protected:
  ...
  std::string node_id_prop_;
  std::vector<hboost::tuple<id_t, bgl_vertex_t, id_t> > vertex_props;
  std::vector<hboost::tuple<id_t, bgl_edge_t, id_t> > edge_props;
  std::vector<std::pair<bgl_vertex_t, bgl_vertex_t> > edges_to_add;
  std::map<node_t, bgl_vertex_t> bgl_nodes;
  std::map<edge_t, bgl_edge_t> bgl_edges;
};

} } } // end namespace hboost::detail::graph

#ifdef HBOOST_GRAPH_USE_SPIRIT_PARSER
#  ifndef HBOOST_GRAPH_READ_GRAPHVIZ_ITERATORS
#    define HBOOST_GRAPH_READ_GRAPHVIZ_ITERATORS
#  endif
#  include <hboost/graph/detail/read_graphviz_spirit.hpp>
#else // New default parser
#  include <hboost/graph/detail/read_graphviz_new.hpp>
#endif // HBOOST_GRAPH_USE_SPIRIT_PARSER
...
bool read_graphviz(InputIterator user_first,
  ...
  typedef hboost::spirit::classic::multi_pass<is_t> iterator_t;

  iterator_t first(hboost::spirit::classic::make_multi_pass(user_first));
  iterator_t last(hboost::spirit::classic::make_multi_pass(user_last));
...
#  include <hboost/graph/distributed/graphviz.hpp>
#endif

#endif // HBOOST_GRAPHVIZ_HPP
