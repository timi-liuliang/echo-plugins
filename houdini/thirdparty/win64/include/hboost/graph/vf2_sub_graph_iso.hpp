...
#ifndef HBOOST_VF2_SUB_GRAPH_ISO_HPP
#define HBOOST_VF2_SUB_GRAPH_ISO_HPP
...
#include <hboost/graph/graph_utility.hpp>
#include <hboost/graph/graph_traits.hpp>
#include <hboost/graph/mcgregor_common_subgraphs.hpp> // for always_equivalent
...
#ifndef HBOOST_GRAPH_ITERATION_MACROS_HPP
#define HBOOST_ISO_INCLUDED_ITER_MACROS // local macro, see bottom of file
...
namespace hboost {
  ...
  namespace detail {
    ...
    public:
      ...
      hboost::tuple<size_type, size_type, size_type>
      term_set() const {
        return hboost::make_tuple(term_in_count_, term_out_count_, 
                                 term_both_count_);
      }
    ...
    struct equivalent_edge_exists {
      typedef typename hboost::graph_traits<Graph>::edge_descriptor edge_type;

      HBOOST_CONCEPT_ASSERT(( LessThanComparable<edge_type> ));
    ...
    struct equivalent_edge_exists<Graph, typename hboost::disable_if<is_multigraph<Graph> >::type> {
      template<typename EdgePredicate>
      bool operator()(typename graph_traits<Graph>::vertex_descriptor s,
                      typename graph_traits<Graph>::vertex_descriptor t, 
                      EdgePredicate is_valid_edge, const Graph& g) {
    ...
    class state {
      ...
      inline bool comp_term_sets(graph1_size_type a, 
                                 graph2_size_type b,
                                 hboost::mpl::int_<subgraph_mono>) const {
        return a <= b;
      }
      ...
      inline bool comp_term_sets(graph1_size_type a, 
                                 graph2_size_type b,
                                 hboost::mpl::int_<subgraph_iso>) const {
        return a <= b;
      }
      ...
      inline bool comp_term_sets(graph1_size_type a, 
                                 graph2_size_type b,
                                 hboost::mpl::int_<isomorphism>) const {
        return a == b;
      }
    ...
    public:
      ...
      bool feasible(const vertex1_type& v_new, const vertex2_type& w_new) {
        ...
        if (problem_selection != subgraph_mono) { // subgraph_iso and isomorphism
          return comp_term_sets(term_in1_count, term_in2_count,
                                hboost::mpl::int_<problem_selection>()) &&
      ...
      bool valid() const {
        hboost::tuple<graph1_size_type, graph1_size_type, graph1_size_type> term1;
        hboost::tuple<graph2_size_type, graph2_size_type, graph2_size_type> term2;
        
        term1 = state1_.term_set();
        term2 = state2_.term_set();
        
        return comp_term_sets(hboost::get<0>(term1), hboost::get<0>(term2),
                              hboost::mpl::int_<problem_selection>()) &&
               comp_term_sets(hboost::get<1>(term1), hboost::get<1>(term2),
                              hboost::mpl::int_<problem_selection>()) &&
               comp_term_sets(hboost::get<2>(term1), hboost::get<2>(term2),
                              hboost::mpl::int_<problem_selection>()); 
      }
    ...
    bool match(const Graph1& graph1, const Graph2& graph2, 
      ...
      hboost::tie(graph2_verts_iter, graph2_verts_iter_end) = vertices(graph2);
    ...
    void sort_vertices(const Graph& graph, IndexMap index_map, VertexOrder& order) {
      ...
      hboost::range::sort(order, vertex_in_out_degree_cmp<Graph>(graph));
      ...
      hboost::range::sort(order, vertex_frequency_degree_cmp<Graph, frequency_map_type>(graph, freq));
    ...
    bool vf2_subgraph_morphism(const GraphSmall& graph_small, const GraphLarge& graph_large,
      ...
      // Graph requirements
      HBOOST_CONCEPT_ASSERT(( BidirectionalGraphConcept<GraphSmall> ));
      HBOOST_CONCEPT_ASSERT(( VertexListGraphConcept<GraphSmall> ));
      HBOOST_CONCEPT_ASSERT(( EdgeListGraphConcept<GraphSmall> ));
      HBOOST_CONCEPT_ASSERT(( AdjacencyMatrixConcept<GraphSmall> ));

      HBOOST_CONCEPT_ASSERT(( BidirectionalGraphConcept<GraphLarge> ));
      HBOOST_CONCEPT_ASSERT(( VertexListGraphConcept<GraphLarge> ));
      HBOOST_CONCEPT_ASSERT(( EdgeListGraphConcept<GraphLarge> ));
      HBOOST_CONCEPT_ASSERT(( AdjacencyMatrixConcept<GraphLarge> ));
      ...
      HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<IndexMapSmall, vertex_small_type> ));
      typedef typename property_traits<IndexMapSmall>::value_type IndexMapSmallValue;
      HBOOST_STATIC_ASSERT(( is_convertible<IndexMapSmallValue, size_type_small>::value ));
        
      HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<IndexMapLarge, vertex_large_type> ));
      typedef typename property_traits<IndexMapLarge>::value_type IndexMapLargeValue;
      HBOOST_STATIC_ASSERT(( is_convertible<IndexMapLargeValue, size_type_large>::value ));
      ...
      HBOOST_CONCEPT_ASSERT(( BinaryPredicateConcept<EdgeEquivalencePredicate, 
                             edge_small_type, edge_large_type> ));

      HBOOST_CONCEPT_ASSERT(( BinaryPredicateConcept<VertexEquivalencePredicate, 
                             vertex_small_type, vertex_large_type> ));

      // Vertex order requirements
      HBOOST_CONCEPT_ASSERT(( ContainerConcept<VertexOrderSmall> )); 
      typedef typename VertexOrderSmall::value_type order_value_type;
      HBOOST_STATIC_ASSERT(( is_same<vertex_small_type, order_value_type>::value ));
      HBOOST_ASSERT( num_vertices(graph_small) == vertex_order_small.size() );
  ...
  bool vf2_graph_iso(const Graph1& graph1, const Graph2& graph2,
    ...
    // Graph requirements
    HBOOST_CONCEPT_ASSERT(( BidirectionalGraphConcept<Graph1> ));
    HBOOST_CONCEPT_ASSERT(( VertexListGraphConcept<Graph1> ));
    HBOOST_CONCEPT_ASSERT(( EdgeListGraphConcept<Graph1> ));
    HBOOST_CONCEPT_ASSERT(( AdjacencyMatrixConcept<Graph1> ));

    HBOOST_CONCEPT_ASSERT(( BidirectionalGraphConcept<Graph2> ));
    HBOOST_CONCEPT_ASSERT(( VertexListGraphConcept<Graph2> ));
    HBOOST_CONCEPT_ASSERT(( EdgeListGraphConcept<Graph2> ));
    HBOOST_CONCEPT_ASSERT(( AdjacencyMatrixConcept<Graph2> ));
    ...
    // Property map requirements
    HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<IndexMap1, vertex1_type> ));
    typedef typename property_traits<IndexMap1>::value_type IndexMap1Value;
    HBOOST_STATIC_ASSERT(( is_convertible<IndexMap1Value, size_type1>::value ));
        
    HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<IndexMap2, vertex2_type> ));
    typedef typename property_traits<IndexMap2>::value_type IndexMap2Value;
    HBOOST_STATIC_ASSERT(( is_convertible<IndexMap2Value, size_type2>::value ));
    ...
    // Vertex order requirements
    HBOOST_CONCEPT_ASSERT(( ContainerConcept<VertexOrder1> )); 
    typedef typename VertexOrder1::value_type order_value_type;
    HBOOST_STATIC_ASSERT(( is_same<vertex1_type, order_value_type>::value ));
    HBOOST_ASSERT( num_vertices(graph1) == vertex_order1.size() );
...
#ifdef HBOOST_ISO_INCLUDED_ITER_MACROS
#undef HBOOST_ISO_INCLUDED_ITER_MACROS
#include <hboost/graph/iteration_macros_undef.hpp>
#endif

#endif // HBOOST_VF2_SUB_GRAPH_ISO_HPP
