...
#ifndef HBOOST_GRAPH_DIRECTED_GRAPH_HPP
#define HBOOST_GRAPH_DIRECTED_GRAPH_HPP
...
public:
        ...
        , m_max_vertex_index(n), m_max_edge_index(0)
    {
        ...
        m_num_edges = m_max_edge_index = hboost::num_edges(m_graph);
    }
...
private:
    ...
    vertex_descriptor make_index(vertex_descriptor v) {
        hboost::put(vertex_index, m_graph, v, m_max_vertex_index);
        m_num_vertices++;
        m_max_vertex_index++;
        return v;
    }
public:
    ...
    { return make_index(hboost::add_vertex(m_graph)); }

    vertex_descriptor add_vertex(vertex_property_type const& p)
    { return make_index(hboost::add_vertex(internal_vertex_property(0u, p), m_graph)); }

    void clear_vertex(vertex_descriptor v)
    {
        m_num_edges -= hboost::degree(v, m_graph);
        hboost::clear_vertex(v, m_graph);
    }

    void remove_vertex(vertex_descriptor v)
    {
        hboost::remove_vertex(v, m_graph);
        --m_num_vertices;
    }
...
private:
    ...
    {
        if(x.second) {
            hboost::put(edge_index, m_graph, x.first, m_max_edge_index);
            ++m_num_edges;
            ++m_max_edge_index;
        }
        return x;
...
public:
    ...
    { return make_index(hboost::add_edge(u, v, m_graph)); }

    std::pair<edge_descriptor, bool>
    add_edge(vertex_descriptor u, vertex_descriptor v, edge_property_type const& p)
    { return make_index(hboost::add_edge(u, v, internal_edge_property(0u, p), m_graph)); }
    ...
    void remove_edge(vertex_descriptor u, vertex_descriptor v)
    {
        ...
        for(hboost::tie(i, i_end) = hboost::out_edges(u, m_graph); i != i_end; ++i) {
            if(hboost::target(*i, m_graph) == v) {
                edges.push_back(*i);
            }
        }
    ...
    renumber_vertex_indices()
    {
        ...
        hboost::tie(i, end) = vertices(m_graph);
    ...
    {
        edge_iterator i, end;
        hboost::tie(i, end) = edges(m_graph);
