...
#ifndef HBOOST_GRAPH_COMPRESSED_SPARSE_ROW_STRUCT_HPP
#define HBOOST_GRAPH_COMPRESSED_SPARSE_ROW_STRUCT_HPP

#ifndef HBOOST_GRAPH_COMPRESSED_SPARSE_ROW_GRAPH_HPP
#error This file should only be included from hboost/graph/compressed_sparse_row_graph.hpp
#endif
...
#include <hboost/graph/detail/histogram_sort.hpp>
...
namespace detail {
  ...
  class compressed_sparse_row_structure :
    ...
    assign_unsorted_multi_pass_edges(MultiPassInputIterator edge_begin,
      ...
      typedef hboost::transform_iterator<hboost::graph::detail::project1st<edge_type>, MultiPassInputIterator> source_iterator;
      typedef hboost::transform_iterator<hboost::graph::detail::project2nd<edge_type>, MultiPassInputIterator> target_iterator;
      source_iterator sources_begin(edge_begin, hboost::graph::detail::project1st<edge_type>());
      source_iterator sources_end(edge_end, hboost::graph::detail::project1st<edge_type>());
      target_iterator targets_begin(edge_begin, hboost::graph::detail::project2nd<edge_type>());
      target_iterator targets_end(edge_end, hboost::graph::detail::project2nd<edge_type>());

      hboost::graph::detail::count_starts
        (sources_begin, sources_end, m_rowstart.begin(), numlocalverts,
         source_pred, hboost::make_property_map_function(global_to_local));

      m_column.resize(m_rowstart.back());
      inherited_edge_properties::resize(m_rowstart.back());

      hboost::graph::detail::histogram_sort
        (sources_begin, sources_end, m_rowstart.begin(), numlocalverts,
    ...
    void assign_from_sorted_edges(
      ...
      if (numedges == 0) {
        numedges = hboost::graph::detail::reserve_count_for_single_pass(edge_begin, edge_end);
      }
      m_column.clear();
      m_column.reserve(numedges_or_zero);
    ...
    void assign_sources_and_targets_global(std::vector<vertex_descriptor>& sources,
      ...
      HBOOST_ASSERT (sources.size() == targets.size());
      ...
      hboost::graph::detail::count_starts
        (sources.begin(), sources.end(), m_rowstart.begin(), numverts,
         keep_all(), hboost::make_property_map_function(global_to_local));
      hboost::graph::detail::histogram_sort_inplace
        (sources.begin(), m_rowstart.begin(), numverts,
         targets.begin(), hboost::make_property_map_function(global_to_local));
    ...
    void assign_sources_and_targets_global(std::vector<vertex_descriptor>& sources,
                                           ...
                                           GlobalToLocal global_to_local) {
      HBOOST_ASSERT (sources.size() == targets.size());
      HBOOST_ASSERT (sources.size() == edge_props.size());
      // Do an in-place histogram sort (at least that's what I think it is) to
      // sort sources and targets
      m_rowstart.clear();
      m_rowstart.resize(numverts + 1);
      ...
      hboost::graph::detail::histogram_sort_inplace
        (sources.begin(), m_rowstart.begin(), numverts,
         ...
         hboost::make_property_map_function(global_to_local));
      // Now targets is the correct vector (properly sorted by source) for
           ...
           vertices_size_type numverts, edges_size_type numedges)
    {
      ...
      EdgeIndex current_edge = 0;
      typedef typename hboost::graph_traits<Graph>::vertex_descriptor g_vertex;
      typedef typename hboost::graph_traits<Graph>::out_edge_iterator
        g_out_edge_iter;
      ...
      for (Vertex i = 0; i != numverts; ++i) {
        ...
        for (hboost::tie(ei, ei_end) = out_edges(v, g); ei != ei_end; ++ei) {
    ...
    add_edges_sorted_internal(
        ...
        const GlobalToLocal& global_to_local) {
      typedef hboost::reverse_iterator<BidirectionalIteratorOrig> BidirectionalIterator;
      typedef hboost::reverse_iterator<EPIterOrig> EPIter;
      // Flip sequence
  ...
  struct transpose_iterator_gen {
    ...
    typedef hboost::transform_iterator<transpose, Iter> type;
  ...
  {
    typedef typename hboost::graph_traits<GraphT>::vertices_size_type
      vertices_size_type;
    typedef typename hboost::graph_traits<GraphT>::edge_descriptor edge_descriptor;
  ...
  edge_to_index_pair
    <GraphT,
     typename hboost::property_map<GraphT,hboost::vertex_index_t>::const_type>
  make_edge_to_index_pair(const GraphT& g)
  {
    typedef typename hboost::property_map<GraphT,
                                         hboost::vertex_index_t>::const_type
      VertexIndexMap;
    return edge_to_index_pair<GraphT, VertexIndexMap>(g,
                                                     get(hboost::vertex_index,
                                                         g));
  }

  template<typename GraphT, typename VertexIndexMap, typename Iter>
  hboost::transform_iterator<edge_to_index_pair<GraphT, VertexIndexMap>, Iter>
  make_edge_to_index_pair_iter(const GraphT& g, const VertexIndexMap& index,
                               Iter it) {
    return hboost::transform_iterator<edge_to_index_pair<GraphT, VertexIndexMap>, Iter>(it, edge_to_index_pair<GraphT, VertexIndexMap>(g, index));
  }
...
#endif // HBOOST_GRAPH_COMPRESSED_SPARSE_ROW_STRUCT_HPP
