...
#ifndef HBOOST_READ_GRAPHVIZ_SPIRIT_HPP
#define HBOOST_READ_GRAPHVIZ_SPIRIT_HPP

// Phoenix/Spirit set these limits to 3, but I need more.
#define PHOENIX_LIMIT 6
#define HBOOST_SPIRIT_CLOSURE_LIMIT 6


#include <hboost/spirit/include/classic_multi_pass.hpp>
#include <hboost/spirit/include/classic_core.hpp>
#include <hboost/spirit/include/classic_confix.hpp>
#include <hboost/spirit/include/classic_distinct.hpp>
#include <hboost/spirit/include/classic_lists.hpp>
#include <hboost/spirit/include/classic_escape_char.hpp>
#include <hboost/spirit/include/classic_attribute.hpp>
#include <hboost/spirit/include/classic_dynamic.hpp>
#include <hboost/spirit/include/classic_actor.hpp>
#include <hboost/spirit/include/classic_closure.hpp>
#include <hboost/spirit/include/phoenix1.hpp>
#include <hboost/spirit/include/phoenix1_binders.hpp>
#include <hboost/ref.hpp>
#include <hboost/function/function2.hpp>
#include <hboost/type_traits/is_same.hpp>
#include <hboost/property_map/dynamic_property_map.hpp>
...
#include <utility>
#include <map>
#include <hboost/graph/graphviz.hpp>
#include <hboost/throw_exception.hpp>

namespace hboostphoenix {
...
typedef hboost::function2<void, id_t const&, id_t const&> actor_t;
...
/////////////////////////////////////////////////////////////////////////////
struct id_closure : hboost::spirit::classic::closure<id_closure, node_t> {
  member1 name;
};


struct node_id_closure : hboost::spirit::classic::closure<node_id_closure, node_t> {
  member1 name;
};

struct attr_list_closure : hboost::spirit::classic::closure<attr_list_closure, actor_t> {
  member1 prop_actor;
};

struct property_closure : hboost::spirit::classic::closure<property_closure, id_t, id_t> {
  member1 key;
  member2 value;
};

struct data_stmt_closure : hboost::spirit::classic::closure<data_stmt_closure,
...
struct subgraph_closure : hboost::spirit::classic::closure<subgraph_closure,
...
struct dot_grammar : public hboost::spirit::classic::grammar<dot_grammar> { 
  mutate_graph& graph_;
  ...
  struct definition {
    ...
    keyword_p("0-9a-zA-Z_") {
      using namespace hboost::spirit::classic;
      using namespace hboostphoenix;
      ...
      a_list
          = list_p( ID[(a_list.key = arg1),
                    ...
                    >> !( ch_p('=')
                          ...
                          [hboostphoenix::bind(&definition::call_prop_actor)
      ...
      node_id
          = ( ID[node_id.name = arg1] >> (!port) )
             [hboostphoenix::bind(&definition::memoize_node)(var(*this))];

      graph_stmt
          = (ID[graph_stmt.key = arg1] >>
             ch_p('=') >>
             ID[graph_stmt.value = arg1])
        [hboostphoenix::bind(&definition::call_graph_prop)
         (var(*this),graph_stmt.key,graph_stmt.value)]
        ; // Graph property.

      attr_stmt
          = (as_lower_d[keyword_p("graph")]
             >> attr_list(actor_t(hboostphoenix::bind(&definition::default_graph_prop)
                                     (var(*this),arg1,arg2))))
          | (as_lower_d[keyword_p("node")]
             >> attr_list(actor_t(hboostphoenix::bind(&definition::default_node_prop)
                                     (var(*this),arg1,arg2))))
          | (as_lower_d[keyword_p("edge")]
             >> attr_list(actor_t(hboostphoenix::bind(&definition::default_edge_prop)
                                     (var(*this),arg1,arg2))))
          ;

      // edge_head is set depending on the graph type (directed/undirected)
      edgeop = ch_p('-') >> ch_p(hboost::ref(edge_head));

      edgeRHS
          =  +(    edgeop[(data_stmt.sources = data_stmt.dests),
                          (data_stmt.dests = construct_<nodes_t>())]
                   >> ( subgraph[data_stmt.dests = arg1]
                      | node_id[hboostphoenix::bind(&definition::insert_node)
                                (var(*this),data_stmt.dests,arg1)]
                      )
                   [hboostphoenix::bind(&definition::activate_edge)
...
#ifdef HBOOST_GRAPH_DEBUG
                      (std::cout << val("AcTive Node: ") << arg1 << "\n"),
#endif // HBOOST_GRAPH_DEBUG
                      (data_stmt.active_node = arg1)]
            ) >> if_p(edgeRHS)[
                     !attr_list(
                       actor_t(hboostphoenix::bind(&definition::edge_prop)
                               (var(*this),arg1,arg2)))
                  ].else_p[
                     if_p(data_stmt.saw_node)[
                         !attr_list(
                           actor_t(hboostphoenix::bind(&definition::node_prop)
                                   (var(*this),arg1,arg2)))
      ...
      the_grammar
          = (!as_lower_d[keyword_p("strict")])
            >>  ( as_lower_d[keyword_p("graph")][
                   (var(edge_head) = '-'),
                   (hboostphoenix::bind(&definition::check_undirected)(var(*this)))]
                | as_lower_d[keyword_p("digraph")][
                   (var(edge_head) = '>'),
                   (hboostphoenix::bind(&definition::check_directed)(var(*this)))]
                )
            >> (!ID) >> ch_p('{') >> stmt_list >> ch_p('}');

    } // definition()

    typedef hboost::spirit::classic::rule<ScannerT> rule_t;
    ...
    void check_undirected() {
      if(self.graph_.is_directed())
          hboost::throw_exception(hboost::undirected_graph_error());
    }

    void check_directed() {
      if(!self.graph_.is_directed())
          hboost::throw_exception(hboost::directed_graph_error());
    }
   ...
   void default_edge_prop(id_t const& key, id_t const& value) {
    ...
    // Keywords;
    const hboost::spirit::classic::distinct_parser<> keyword_p;
    //
    // rules that make up the grammar
    //
    hboost::spirit::classic::rule<ScannerT,id_closure::context_t> ID;
    hboost::spirit::classic::rule<ScannerT,property_closure::context_t> a_list;
    hboost::spirit::classic::rule<ScannerT,attr_list_closure::context_t> attr_list;
    rule_t port_location;
    rule_t port_angle;
    rule_t port;
    hboost::spirit::classic::rule<ScannerT,node_id_closure::context_t> node_id;
    hboost::spirit::classic::rule<ScannerT,property_closure::context_t> graph_stmt;
    rule_t attr_stmt;
    hboost::spirit::classic::rule<ScannerT,data_stmt_closure::context_t> data_stmt;
    hboost::spirit::classic::rule<ScannerT,subgraph_closure::context_t> subgraph;
    rule_t edgeop;
    rule_t edgeRHS;
    rule_t stmt;
    rule_t stmt_list;
...
//
struct dot_skipper : public hboost::spirit::classic::grammar<dot_skipper>
{
    dot_skipper() {}
...
#ifdef HBOOST_SPIRIT_DEBUG
               HBOOST_SPIRIT_DEBUG_RULE(skip);
#endif
        }

      hboost::spirit::classic::rule<ScannerT>  skip;
      hboost::spirit::classic::rule<ScannerT> const&
...
bool read_graphviz_spirit(MultiPassIterator begin, MultiPassIterator end,
  ...
  typedef skip_parser_iteration_policy< hboost::detail::graph::dot_skipper>
    iter_policy_t;
  typedef scanner_policies<iter_policy_t> scanner_policies_t;
  typedef scanner<iterator_t, scanner_policies_t> scanner_t;

  ::hboost::detail::graph::mutate_graph_impl<MutableGraph> 
      m_graph(graph, dp, node_id);

  ::hboost::detail::graph::dot_grammar p(m_graph);
  ::hboost::detail::graph::dot_skipper skip_p;
...
#endif // HBOOST_READ_GRAPHVIZ_SPIRIT_HPP
