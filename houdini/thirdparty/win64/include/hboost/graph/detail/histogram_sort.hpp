...
#ifndef HBOOST_GRAPH_DETAIL_HISTOGRAM_SORT_HPP
#define HBOOST_GRAPH_DETAIL_HISTOGRAM_SORT_HPP
...
count_starts
  ...
  for (KeyIterator i = begin; i != end; ++i) {
    if (key_filter(*i)) {
      HBOOST_ASSERT (key_transform(*i) < numkeys);
...
histogram_sort_inplace(KeyIterator key_begin,
  ...
  for (size_t i = 0; i < rowstart[numkeys]; ++i) {
    HBOOST_ASSERT (key_transform(key_begin[i]) < numkeys);
    ...
    while (!(i >= rowstart[key_transform(key_begin[i])] && i < insert_positions[key_transform(key_begin[i])])) {
      ...
      HBOOST_ASSERT (target_pos < rowstart[key_transform(key_begin[i]) + 1]);
...
#endif // HBOOST_GRAPH_DETAIL_HISTOGRAM_SORT_HPP
