...
//=======================================================================
//
#ifndef HBOOST_D_ARY_HEAP_HPP
#define HBOOST_D_ARY_HEAP_HPP
...
namespace hboost {
  ...
  inline void property_map_swap(
         PropMap prop_map,
         const typename hboost::property_traits<PropMap>::key_type& ka, 
         const typename hboost::property_traits<PropMap>::key_type& kb) {
    typename hboost::property_traits<PropMap>::value_type va = get(prop_map, ka);
  ...
  namespace detail {
    ...
    class fixed_max_size_vector {
      hboost::shared_array<Value> m_data;
      std::size_t m_size;
  ...
  class d_ary_heap_indirect {
    HBOOST_STATIC_ASSERT (Arity >= 2);

    public:
    typedef typename Container::size_type size_type;
    typedef Value value_type;
    typedef typename hboost::property_traits<DistanceMap>::value_type key_type;
    ...
    // distance map
    typedef typename hboost::property_traits<DistanceMap>::value_type distance_type;
    ...
    void verify_heap() const {
      ...
      for (size_t i = 1; i < data.size(); ++i) {
        if (compare_indirect(data[i], data[parent(i)])) {
          HBOOST_ASSERT (!"Element is smaller than its parent");
        }
      }
#endif
    }
