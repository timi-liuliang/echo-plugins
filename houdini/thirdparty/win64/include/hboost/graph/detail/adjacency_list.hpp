...
#ifndef HBOOST_GRAPH_DETAIL_ADJACENCY_LIST_HPP
#define HBOOST_GRAPH_DETAIL_ADJACENCY_LIST_HPP
...
#include <hboost/range/irange.hpp>
...
#include <hboost/pending/container_traits.hpp>
#include <hboost/graph/detail/adj_list_edge_iterator.hpp>
...
#define HBOOST_GRAPH_MOVE_IF_POSSIBLE(x) (x)
#else
#include <utility>
#define HBOOST_GRAPH_MOVE_IF_POSSIBLE(x) (std::move((x)))
#endif
...
namespace hboost {
  ...
  namespace detail {
    ...
    {
      hboost::graph_detail::erase_if(el, detail::target_is<vertex_descriptor>(v));
    }
    // O(log(E/V))
    template <class EdgeList, class vertex_descriptor>
    void erase_from_incidence_list(EdgeList& el, vertex_descriptor v,
    ...
    template <class Vertex>
    class stored_edge
      : public hboost::equality_comparable1< stored_edge<Vertex>,
        hboost::less_than_comparable1< stored_edge<Vertex> > >
    {
    public:
...
#if defined(HBOOST_NO_CXX11_RVALUE_REFERENCES) || defined(HBOOST_NO_CXX11_DEFAULTED_FUNCTIONS)
    ...
    public:
      ...
      inline stored_edge_property(Vertex target,
                                  const Property& p = Property())
        : stored_edge<Vertex>(target), m_property(new Property(p)) { }
#if defined(HBOOST_MSVC) || (defined(HBOOST_GCC) && (HBOOST_GCC / 100) < 406)
    ...
    public:
      ...
      inline Property& get_property() { HBOOST_ASSERT ((m_vec != 0)); return (*m_vec)[m_i].get_property(); }
      inline const Property& get_property() const {
        HBOOST_ASSERT ((m_vec != 0));
        return (*m_vec)[m_i].get_property();
      }
      inline Iter get_iter() const { HBOOST_ASSERT ((m_vec != 0)); return m_vec->begin() + m_i; }
    protected:
      std::size_t m_i;
      EdgeVec* m_vec;
    };
      ...
      const detail::stored_ra_edge_iter<Vertex, EdgeVec, Property>& e)
  {
    ...
    namespace detail {
      ...
      remove_directed_edge_if_dispatch(incidence_iterator first,
                                       ...
                                       hboost::allow_parallel_edge_tag)
      {
        // remove_if
        ...
        if (first != last)
          for (++i; i != last; ++i)
            if (!pred(*i)) {
              *first.base() = HBOOST_GRAPH_MOVE_IF_POSSIBLE(*i.base());
              ++first;
            }
      ...
      remove_directed_edge_if_dispatch(incidence_iterator first,
                                       ...
                                       EdgeList& el,
                                       Predicate pred,
                                       hboost::disallow_parallel_edge_tag)
      {
                                             ...
                                             hboost::allow_parallel_edge_tag)
      {
        ...
        HBOOST_STATIC_ASSERT((!is_same<EdgeListS, vecS>::value));
                       ...
                       directed_graph_helper<Config>& g_)
    {
      ...
      hboost::tie(first, last) = out_edges(u, g);
    ...
    remove_edge_if(Predicate pred, directed_graph_helper<Config>& g_)
    {
      ...
      for (hboost::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
                 ...
                 directed_graph_helper<Config>& g_)
    {
      ...
      for (hboost::tie(vi, viend) = vertices(g); vi != viend; ++vi)
             ...
             directed_graph_helper<Config>& g_)
    {
      ...
      bool inserted;
      hboost::tie(i, inserted) = hboost::graph_detail::push(g.out_edge_list(u),
                                            StoredEdge(v, p));
    ...
    namespace detail {
      ...
      remove_edge_and_property(Graph& g, EdgeList& el, Vertex v,
                               hboost::allow_parallel_edge_tag cat)
      {
        typedef typename Graph::global_edgelist_selector EdgeListS;
        HBOOST_STATIC_ASSERT((!is_same<EdgeListS, vecS>::value));
        ...
        for (; i != end; ++i) {
          if ((*i).get_target() == v) {
            ...
            bool skip = (hboost::next(i) != end && i->get_iter() == hboost::next(i)->get_iter());
    ...
    struct list_edge // short name due to VC++ truncation and linker problems
      : public hboost::detail::edge_base<hboost::undirected_tag, Vertex>
    {
      typedef EdgeProperty property_type;
      typedef hboost::detail::edge_base<hboost::undirected_tag, Vertex> Base;
                 ...
                 undirected_graph_helper<Config>& g_)
    {
      ...
      while (true) {
        ...
        hboost::tie(ei, ei_end) = out_edges(u, g);
        if (ei == ei_end) break;
        remove_edge(*ei, g);
      }
    }
             ...
             undirected_graph_helper<Config>& g_)
    {
      ...
      if (inserted) {
        hboost::graph_detail::push(g.out_edge_list(v), StoredEdge(u, p_iter, &g.m_edges));
      ...
      : public bidirectional_graph_helper<Config>
    {
      ...
      remove_edge(typename Config::edge_descriptor e)
      {
        ...
        HBOOST_ASSERT(rng.first != rng.second);
                       ...
                       bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      for (hboost::tie(out_i, out_end) = out_edges(u, g); out_i != out_end; ++out_i)
        if (pred(*out_i)) {
          detail::remove_directed_edge_dispatch
                      ...
                      bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      for (hboost::tie(in_i, in_end) = in_edges(v, g); in_i != in_end; ++in_i)
        if (pred(*in_i)) {
      ...
      hboost::tie(first, last) = in_edges(v, g);
      typedef typename Config::edge_parallel_category Cat;
      detail::remove_directed_edge_if_dispatch
             ...
             bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      hboost::tie(i, inserted) = hboost::graph_detail::push(g.out_edge_list(u),
                                        StoredEdge(v, p_iter, &g.m_edges));
      if (inserted) {
        hboost::graph_detail::push(in_edge_list(g, v), StoredEdge(u, p_iter, &g.m_edges));
                          ...
                          const adj_list_helper<Config, Base>& g_)
    {
      ...
      typedef typename Config::inv_adjacency_iterator inv_adjacency_iterator;
      typename Config::in_edge_iterator first, last;
      hboost::tie(first, last) = in_edges(u, g);
               ...
               const adj_list_helper<Config, Base>& g_)
    {
      ...
      hboost::tie(first, last) = graph_detail::
    ...
    namespace detail {
      ...
      inline
      typename hboost::property_map<typename Config::graph_type,
        Property>::type
      get_dispatch(adj_list_helper<Config,Base>&, Property p,
                   hboost::edge_property_tag) {
        typedef typename Config::graph_type Graph;
        typedef typename hboost::property_map<Graph, Property>::type PA;
        return PA(p);
      }
      template <class Config, class Base, class Property>
      inline
      ...
      get_dispatch(const adj_list_helper<Config,Base>&, Property p,
                   hboost::edge_property_tag) {
        typedef typename Config::graph_type Graph;
        typedef typename hboost::property_map<Graph, Property>::const_type PA;
        return PA(p);
      }
    ...
    template <class Config, class Base, class Property>
    inline
    typename hboost::property_map<typename Config::graph_type, Property>::type
    ...
    inline
    typename hboost::property_traits<
      typename hboost::property_map<typename Config::graph_type,
        Property>::type
    >::reference
    ...
    {
      typedef typename Config::graph_type Graph;
      typedef typename hboost::property_map<Graph, Property>::type Map;
    ...
    public:
      ...
      inline void copy_impl(const adj_list_impl& x_)
      {
        ...
        for (hboost::tie(vi, vi_end) = vertices(x); vi != vi_end; ++vi) {
        ...
        for (hboost::tie(ei, ei_end) = edges(x); ei != ei_end; ++ei) {
          edge_descriptor e;
          bool inserted;
          vertex_descriptor s = source(*ei,x), t = target(*ei,x);
          hboost::tie(e, inserted) = add_edge(vertex_map[(stored_vertex*)s],
    ...
    add_vertex(adj_list_impl<Derived, Config, Base>& g_)
    {
      ...
      bool inserted;
      hboost::tie(pos,inserted) = hboost::graph_detail::push(g.m_vertices, v);
      v->m_position = pos;
      g.added_vertex(v);
      return v;
    }
                              ...
                              adj_list_impl<Derived, Config, Base>& g_)
    {
      typedef typename Config::stored_vertex stored_vertex;
      Derived& g = static_cast<Derived&>(g_);
      g.removing_vertex(u, hboost::graph_detail::iterator_stability(g_.m_vertices));
    ...
    public:
      ...
      //    protected:
      inline hboost::integer_range<vertex_descriptor> vertex_set() const {
        return hboost::integer_range<vertex_descriptor>(0, m_vertices.size());
      }
      inline OutEdgeList& out_edge_list(vertex_descriptor v) {
      ...
      inline void copy_impl(const vec_adj_list_impl& x_)
      {
        ...
        for (hboost::tie(ei, ei_end) = edges(x); ei != ei_end; ++ei) {
          edge_descriptor e;
          bool inserted;
          hboost::tie(e, inserted) = add_edge(source(*ei,x), target(*ei,x) , *this);
          *((edge_property_type*)e.m_eproperty)
            = *((edge_property_type*)(*ei).m_eproperty);
        }
      }
             ...
             vec_adj_list_impl<Graph, Config, Base>& g_)
    {
      ...
      typename Config::vertex_descriptor x = max HBOOST_PREVENT_MACRO_SUBSTITUTION(u, v);
                              ...
                              vec_adj_list_impl<Graph, Config, Base>& g_)
    {
      ...
      Graph& g = static_cast<Graph&>(g_);
      g.removing_vertex(v, hboost::graph_detail::iterator_stability(g_.m_vertices));
  ...
  namespace detail {
    ...
    struct adj_list_gen
    {
      ...
      struct config
      {
        ...
        typedef hboost::integer_range<vertex_descriptor> RandVertexList;
        ...
        typedef hboost::detail::iterator_traits<OutEdgeIter> OutEdgeIterTraits;
        ...
        typedef hboost::detail::iterator_traits<EdgeIter> EdgeIterTraits;
    ...
    struct adj_list_vertex_all_properties_map
      : public hboost::put_get_helper<PropRef,
        ...
        >
    {
      typedef ValueType value_type;
      typedef Reference reference;
      typedef typename hboost::graph_traits<Graph>::vertex_descriptor key_type;
      typedef hboost::lvalue_property_map_tag category;
    ...
    struct vec_adj_list_vertex_all_properties_map
      : public hboost::put_get_helper<PropertyRef,
        ...
        >
    {
      ...
      typedef hboost::readable_property_map_tag category;
  ...
  namespace detail {
    ...
    struct adj_list_choose_vertex_pa
      : hboost::mpl::if_<
          hboost::is_same<Tag, vertex_all_t>,
...
namespace hboost {
  ...
  template <typename V>
  struct hash< hboost::detail::stored_edge<V> >
  {
    std::size_t
    operator()(const hboost::detail::stored_edge<V>& e) const
    {
  ...
  struct hash< hboost::detail::stored_edge_property <V,P> >
  {
    std::size_t
    operator()(const hboost::detail::stored_edge_property<V,P>& e) const
    {
      return hash<V>()(e.m_target);
    }
  };
  ...
  struct hash< hboost::detail::stored_edge_iter<V,I, P> >
  {
    std::size_t
    operator()(const hboost::detail::stored_edge_iter<V,I,P>& e) const
    {
      return hash<V>()(e.m_target);
    }
  };
...
#endif // HBOOST_GRAPH_DETAIL_DETAIL_ADJACENCY_LIST_CCT
