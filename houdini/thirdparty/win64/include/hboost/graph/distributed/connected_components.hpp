...
//           Douglas Gregor
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_PARALLEL_CC_HPP
#define HBOOST_GRAPH_PARALLEL_CC_HPP
...
#include <hboost/graph/connected_components.hpp>
...
namespace hboost { namespace graph { namespace distributed {
  namespace cc_detail {
                          ...
                          RootIterator r_end, AdjacencyMap& adj)
    {
      ...
      HBOOST_STATIC_ASSERT((is_same<typename AdjacencyMap::mapped_type,
      ...
      if (id == 0) {
        ...
        typedef hboost::adjacency_list<vecS, vecS, undirectedS, 
        ...
        while (optional<std::pair<process_id_type, int> > m = probe(pg)) {
          HBOOST_ASSERT(m->second == root_adj_msg);
    ...
    {
      typedef typename hboost::graph::parallel::process_group_type<Graph>
    ...
    {
      using hboost::connected_components;
      ...
      typedef typename hboost::graph::parallel::process_group_type<DistributedGraph>
        ::type process_group_type;
      typedef typename process_group_type::process_id_type process_id_type;
      ...
      BGL_FORALL_VERTICES_T(v, g, DistributedGraph) {
        ...
        for (hboost::tie(av1, av2) = adjacent_vertices(v, g);
             av1 != av2; ++av1) {
            ...
            if ( get(owner, new_parent) == id )
              {
                ...
                HBOOST_ASSERT(::hboost::detail::is_sorted(my_adj.begin(),
                                                  my_adj.end() - adj[*liter].size(),
                                                  std::less<vertex_descriptor>()));
                HBOOST_ASSERT(::hboost::detail::is_sorted(my_adj.end() - adj[*liter].size(),
                                                  my_adj.end(),
                                                  std::less<vertex_descriptor>()));
#endif
        ...
        while (optional<std::pair<process_id_type, int> > m
               = probe(pg))
          {
            ...
            HBOOST_ASSERT(::hboost::detail::is_sorted(my_adj.begin(),
                                              ...
                                              std::less<vertex_descriptor>()));
            HBOOST_ASSERT(::hboost::detail::is_sorted(my_adj.end() - (num_incoming_edges-1),
                                              my_adj.end(),
                                              std::less<vertex_descriptor>()));
#endif
            std::inplace_merge(my_adj.begin(),
...
#endif // HBOOST_GRAPH_PARALLEL_CC_HPP
