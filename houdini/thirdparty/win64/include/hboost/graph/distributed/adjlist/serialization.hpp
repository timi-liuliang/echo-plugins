...
#ifndef HBOOST_GRAPH_DISTRIBUTED_ADJLIST_SERIALIZATION_070925_HPP
#define HBOOST_GRAPH_DISTRIBUTED_ADJLIST_SERIALIZATION_070925_HPP
...
namespace detail { namespace parallel
{
      ...
      available_process_files(std::string const& filename)
      {
          ...
          for (filesystem::directory_iterator i(filename), end; i != end; ++i)
          {
              if (!filesystem::is_regular(*i))
                  hboost::throw_exception(std::runtime_error("directory contains non-regular entries"));
              ...
              for (std::string::size_type i = 0; i < process_name.size(); ++i)
                if (!is_digit(process_name[i]))
                  hboost::throw_exception(std::runtime_error("directory contains files with invalid names"));

              result.push_back(hboost::lexical_cast<int>(process_name));
          }
  ...
  void graph_loader<Graph, Archive, VertexListS>::load_prefix()
  {
      ...
      if (num_processes_ != num_processes(m_pg))
          hboost::throw_exception(std::runtime_error("number of processes mismatch"));
...
# endif
          HBOOST_ASSERT(m_id_mapping[old_ids[i]] == -1);
  ...
  {
      int V;
      m_ar >> HBOOST_SERIALIZATION_NVP(V); 
...
# endif
  ...
  {
      int E;
      m_ar >> HBOOST_SERIALIZATION_NVP(E);
...
# endif
  ...
  {
      std::size_t I;
      m_ar >> HBOOST_SERIALIZATION_NVP(I);
...
# endif
      ...
      hboost::parallel::inplace_all_to_all(m_pg, m_property_ptrs);
  }
  ...
  void graph_loader<Graph, Archive, VertexListS>::resolve_remote_vertices(directedS)
  {
      ...
      hboost::parallel::inplace_all_to_all(
          m_pg, m_requested_vertices, m_remote_vertices);

      for (int i = 0; i < num_processes(m_pg); ++i)
      {
          HBOOST_FOREACH(serializable_vertex_descriptor& u, m_remote_vertices[i])
          {
              u = m_local_vertices[u];
          }
      }

      hboost::parallel::inplace_all_to_all(m_pg, m_remote_vertices);
  }
...
# endif
      ...
      for (int i = 0; i < num_processes(m_pg); ++i)
          HBOOST_ASSERT(m_remote_vertices[i].size() == m_requested_vertices[i].size());
  }
  ...
  void graph_loader<Graph, Archive, VertexListS>::commit_pending_edges(Anything)
  {
      ...
      HBOOST_FOREACH(pending_edge_type const& e, m_pending_edges)
      {
  ...
  void graph_loader<Graph, Archive, VertexListS>::commit_pending_in_edges(bidirectionalS)
  {
      ...
      HBOOST_FOREACH(pending_in_edge const& e, m_pending_in_edges)
      {
...
void save_in_edges(Archive& ar, Graph const& g, bidirectionalS)
{
    ...
    {
        HBOOST_FOREACH(edge_descriptor const& e, in_edges(v, g))
        {
    ...
    ar << HBOOST_SERIALIZATION_NVP(I);

    HBOOST_FOREACH(edge_descriptor const& e, saved_in_edges)
    {
...
void PBGL_DISTRIB_ADJLIST_TYPE::load(std::string const& filename)
{
    ...
    if (available_files.empty())
        hboost::throw_exception(std::runtime_error("no file available"));
...
void PBGL_DISTRIB_ADJLIST_TYPE::save(std::string const& filename) const
{
    ...
    if (filesystem::exists(filename) && !filesystem::is_directory(filename))
        hboost::throw_exception(std::runtime_error("entry exists, but is not a directory"));
...
#endif // HBOOST_GRAPH_DISTRIBUTED_ADJLIST_SERIALIZATION_070925_HPP
