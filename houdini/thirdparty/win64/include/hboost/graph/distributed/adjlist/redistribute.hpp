...
private:
  ...
  {
    ar & hboost::serialization::base_object<inherited>(*this) 
...
{
  using hboost::parallel::inplace_all_to_all;
  ...
  for (hboost::tie(vi, vi_end) = vertices(*this); vi != vi_end; ++vi) {
  ...
  // an all-to-all transfer.
  for (hboost::tie(ei, ei_end) = edges(*this); ei != ei_end; ++ei) {
  ...
  if (has_stable_descriptors) {
    ...
    for (hboost::tie(vi, vi_end) = vertices(*this); vi != vi_end; /* in loop */) {
