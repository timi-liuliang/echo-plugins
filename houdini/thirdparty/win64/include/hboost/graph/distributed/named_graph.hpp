...
#ifndef HBOOST_GRAPH_DISTRIBUTED_NAMED_GRAPH_HPP
#define HBOOST_GRAPH_DISTRIBUTED_NAMED_GRAPH_HPP
...
#include <hboost/graph/named_graph.hpp>
#include <hboost/functional/hash.hpp>
#include <hboost/variant.hpp>
#include <hboost/graph/parallel/simple_trigger.hpp>
...
using hboost::parallel::trigger_receive_context;
using hboost::detail::parallel::pair_with_property;
...
public:
   ...
   *  function uses the @c hboost::hash value of the name, modulo @c p.
   */
                           ...
                           ProcessGroup, void>
{
  ...
  typedef typename mpl::if_<is_same<InDistribution, defaultS>,
                            hboost::parallel::variant_distribution<ProcessGroup, 
                                                                  VertexSize>,
  ...
  typedef typename mpl::if_<is_same<InDistribution, defaultS>,
                            hboost::parallel::block, type>::type
    default_type;
};
...
protected:
  ...
  hboost::parallel::detail::untracked_pair<vertex_descriptor, bool>
  handle_find_vertex(int source, int tag, const vertex_name_type& msg,
                     trigger_receive_context);

  template<typename U, typename V>
  void handle_add_edge(int source, int tag, const hboost::parallel::detail::untracked_pair<U, V>& msg,
                       trigger_receive_context);

  template<typename U, typename V>
  hboost::parallel::detail::untracked_pair<edge_descriptor, bool> 
  handle_add_edge_with_reply(int source, int tag, const hboost::parallel::detail::untracked_pair<U, V>& msg,
                             trigger_receive_context);
...
{
  using hboost::parallel::detail::make_untracked_pair;
  ...
  if (vertex_name_type* v_name = hboost::get<vertex_name_type>(&v)) {
    ...
    if (vertex_name_type* u_name = hboost::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*v_name),
           ...
           make_untracked_pair(hboost::get<vertex_descriptor>(u), *v_name));
  } else {
    if (vertex_name_type* u_name = hboost::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*u_name),
           ...
           make_untracked_pair(*u_name, hboost::get<vertex_descriptor>(v)));
    else
      ...
      add_edge(hboost::get<vertex_descriptor>(u), 
               hboost::get<vertex_descriptor>(v), 
               self.derived());
  }
}
...
BGL_NAMED_GRAPH::lazy_add_edge::commit() const
{
  ...
  if (const vertex_name_type* u_name = hboost::get<vertex_name_type>(&u)) {
    /// We haven't resolved the source vertex to a descriptor yet, so
    ...
    if (const vertex_name_type* v_name = hboost::get<vertex_name_type>(&v))
    ...
    u_owner 
      = get(vertex_owner, self.derived(),
            hboost::get<vertex_descriptor>(u));
    if (u_owner == rank) {
      ...
      return add_edge(hboost::get<vertex_descriptor>(u), 
                      hboost::get<vertex_descriptor>(v), 
                      self.derived());
    } else {
...
{
  using hboost::detail::parallel::make_pair_with_property;
  ...
  if (vertex_name_type* v_name = hboost::get<vertex_name_type>(&v)) {
    ...
    if (vertex_name_type* u_name = hboost::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*v_name),
           ...
           make_pair_with_property(hboost::get<vertex_descriptor>(u), *v_name, 
                                   property));
  } else {
    if (vertex_name_type* u_name = hboost::get<vertex_name_type>(&u))
      // We haven't resolved the source vertex to a descriptor yet, so
      ...
      send(self.process_group(), self.distribution_(*u_name),
           ...
           make_pair_with_property(*u_name, hboost::get<vertex_descriptor>(v), 
                                   property));
    else
      // We have descriptors for both of the vertices, either of which
      // may be remote or local. Tell the owner of the source vertex
...
{
  using hboost::graph::parallel::simple_trigger;
            ...
            const BGL_NAMED_GRAPH& g)
{
  ...
  if (owner == process_id(g.process_group())) {
    ...
    hboost::parallel::detail::untracked_pair<vertex_descriptor, bool> result;
...
template<BGL_NAMED_GRAPH_PARAMS>
hboost::parallel::detail::untracked_pair<typename BGL_NAMED_GRAPH::vertex_descriptor, bool>
BGL_NAMED_GRAPH::
handle_find_vertex(int source, int /*tag*/, const vertex_name_type& msg, 
                   trigger_receive_context)
{
...
void
BGL_NAMED_GRAPH::
handle_add_edge(int source, int /*tag*/, const hboost::parallel::detail::untracked_pair<U, V>& msg, 
                trigger_receive_context)
{
...
template<BGL_NAMED_GRAPH_PARAMS>
template<typename U, typename V>
hboost::parallel::detail::untracked_pair<typename BGL_NAMED_GRAPH::edge_descriptor, bool>
BGL_NAMED_GRAPH::
handle_add_edge_with_reply(int source, int /*tag*/, const hboost::parallel::detail::untracked_pair<U, V>& msg,
                           trigger_receive_context)
{
...
#endif // HBOOST_GRAPH_DISTRIBUTED_NAMED_GRAPH_HPP
