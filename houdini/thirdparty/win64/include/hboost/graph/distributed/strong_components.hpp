...
//           Douglas Gregor
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_DISTRIBUTED_SCC_HPP
#define HBOOST_GRAPH_DISTRIBUTED_SCC_HPP
...
#include <hboost/property_map/parallel/caching_property_map.hpp>
#include <hboost/graph/parallel/algorithm.hpp>
#include <hboost/graph/parallel/process_group.hpp>
#include <hboost/graph/distributed/queue.hpp>
#include <hboost/graph/distributed/filtered_graph.hpp>
#include <hboost/pending/indirect_cmp.hpp>
#include <hboost/graph/breadth_first_search.hpp>
#include <hboost/graph/graph_traits.hpp>
#include <hboost/graph/overloading.hpp>
#include <hboost/graph/distributed/concepts.hpp>
#include <hboost/graph/distributed/local_subgraph.hpp>
#include <hboost/graph/parallel/properties.hpp>
#include <hboost/graph/named_function_params.hpp>
#include <hboost/graph/random.hpp>
#include <hboost/graph/distributed/reverse_graph.hpp>
#include <hboost/optional.hpp>
#include <hboost/graph/distributed/detail/filtered_queue.hpp>
#include <hboost/graph/distributed/adjacency_list.hpp>
#ifdef PBGL_SCC_DEBUG
  #include <iostream>
  #include <cstdlib>
  #include <iomanip>
  #include <sys/time.h>
  #include <hboost/graph/distributed/graphviz.hpp> // for ostringstream
#endif
#include <vector>
#include <map>
...
#ifdef PBGL_SCC_DEBUG
#  include <hboost/graph/accounting.hpp>
...
struct vertex_identity_property_map
  : public hboost::put_get_helper<T, vertex_identity_property_map<T> >
{
  typedef T key_type;
...
namespace hboost { namespace graph { namespace distributed {
  ...
  accounting::time_type start = accounting::get_time();
      ...
      for( hboost::tie(vstart, vend) = vertices(g); vstart != vend; vstart++ )
      ...
      // and may help in the case where there are lots of small strong components.
      {
        ...
        int num_comp = hboost::strong_components(ls, ls_component);
        ...
        for( hboost::tie(vstart,vend) = vertices(ls); vstart != vend; vstart++ )
        ...
        for( std::size_t i = 0; i < num_comp; ++i ) {
          ...
          for( std::size_t j = 0; j < local_comp_map[i].size(); j++ ) {
            for( hboost::tie(abegin,aend) = adjacent_vertices(local_comp_map[i][j], g);
                 abegin != aend; abegin++ )
            ...
            if( local )
              for( hboost::tie(rev_abegin,rev_aend) = adjacent_vertices(get(fr, local_comp_map[i][j]), gr);
  ...
  if(id == 0) {
    ...
    start = accounting::get_time();
       ...
       for (std::size_t i = 0; i < vertex_sets.size(); ++i)
        ...
        typedef filtered_queue<queue<vertex_descriptor>, hboost::detail::has_not_been_seen<VertexIndexMap> >
          local_queue_type;
        typedef hboost::graph::distributed::distributed_queue<process_group_type, OwnerMap, local_queue_type>
          queue_t;

        typedef typename property_map<ReverseGraph, vertex_owner_t>::const_type
          RevOwnerMap;

        typedef filtered_queue<queue<rev_vertex_descriptor>, hboost::detail::has_not_been_seen<VertexIndexMap> >
          rev_local_queue_type;
        typedef hboost::graph::distributed::distributed_queue<process_group_type, RevOwnerMap, rev_local_queue_type>
          rev_queue_t;

        queue_t Q(process_group(g),
                  owner,
                  make_filtered_queue(queue<vertex_descriptor>(),
                                      hboost::detail::has_not_been_seen<VertexIndexMap>
  ...
  start = accounting::get_time();
        ...
        for( hboost::tie(rev_vstart, rev_vend) = vertices(gr); rev_vstart != rev_vend; rev_vstart++ ) {
        ...
        while (optional<std::pair<process_id_type, int> > m = probe(pg)) {
          HBOOST_ASSERT(m->second == fhp_succ_size_msg || m->second == fhp_pred_size_msg);
          ...
          if (m->second == fhp_succ_size_msg) {
            ...
            if (hboost::is_same<detail::vertex_identity_property_map<vertex_descriptor>, IsoMapRF>::value)
              for (std::size_t i = 0; i < local_start.size(); ++i)
                if (added[local_start[i]] > 0)
  ...
  start = accounting::get_time();
    ...
    build_reverse_graph( const Graph& g, ReverseGraph& gr, IsoMap& fr, IsoMap& rf )
    {
      ...
      typedef typename graph_traits<Graph>::vertex_descriptor vertex_descriptor;
      typedef typename graph_traits<Graph>::out_edge_iterator out_edge_iterator;
      typedef typename hboost::graph::parallel::process_group_type<Graph>::type process_group_type;
      typedef typename process_group_type::process_id_type process_id_type;
      typedef std::vector<std::pair<vertex_descriptor, vertex_descriptor> > VertexPairVec;
      ...
      for( hboost::tie(vstart, vend) = vertices(g); vstart != vend; vstart++ )
        for( hboost::tie(oestart, oeend) = out_edges(*vstart, g); oestart != oeend; oestart++ )
      ...
      while (optional<std::pair<process_id_type, int> > m = probe(pg)) {
        HBOOST_ASSERT(m->second == fhp_edges_size_msg);
        std::size_t num_requests;
        receive(pg, m->first, m->second, num_requests);
        VertexPairVec requests(num_requests);
       ...
       incidence_graph_tag)
    {
      ...
      typename hboost::graph::parallel::process_group_type<Graph>::type pg = process_group(g);
  ...
  accounting::time_type start = accounting::get_time();
      ...
      typedef adjacency_list<listS,
                             distributedS<typename hboost::graph::parallel::process_group_type<Graph>::type, vecS>,
  ...
  fleischer_hendrickson_pinar_strong_components(g, r, gr, fr, rf, 
       ...
       VertexIndexMap vertex_index_map)
    {
      ...
      typename hboost::graph::parallel::process_group_type<Graph>::type pg 
        = process_group(g);

      if (num_processes(pg) == 1) {
        local_subgraph<const Graph> ls(g);
        return hboost::strong_components(ls, c);
      }
...
#endif // HBOOST_GRAPH_DISTRIBUTED_SCC_HPP
