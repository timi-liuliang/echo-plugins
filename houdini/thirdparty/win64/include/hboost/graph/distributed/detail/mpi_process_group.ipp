...
#include <queue>
#include <stack>
#include <hboost/graph/distributed/detail/tag_allocator.hpp>
#include <stdio.h>
...
struct mpi_process_group::impl
{
   ...
   */
  hboost::mpi::communicator             comm;

  /**
   * The MPI communicator used to transmit out-of-band replies.
   */
  hboost::mpi::communicator             oob_reply_comm;
...
{
  HBOOST_ASSERT(tag <  msg_reserved_first || tag > msg_reserved_last);
  ...
  hboost::mpi::packed_oarchive oa(impl_->comm, outgoing.buffer);
  oa << value;
                             ...
                             mpl::false_ /*is_mpi_datatype*/) const
{
  ...
  hboost::mpi::packed_oarchive out(impl_->comm, outgoing.buffer);
  out << value;
     ...
     int tag, const T& value)
{
  pg.send_impl(dest, pg.encode_tag(pg.my_block_number(), tag), value,
               hboost::mpi::is_mpi_datatype<T>());
}

template<typename T>
typename enable_if<hboost::mpi::is_mpi_datatype<T>, void>::type
send(const mpi_process_group& pg, mpi_process_group::process_id_type dest,
     int tag, const T values[], std::size_t n)
{
  pg.send_impl(dest, pg.encode_tag(pg.my_block_number(), tag),
                 hboost::serialization::make_array(values,n), 
                 hboost::mpl::true_());
}

template<typename T>
typename disable_if<hboost::mpi::is_mpi_datatype<T>, void>::type
mpi_process_group::
                                ...
                                mpl::true_ /*is_mpi_datatype*/) const
{
  ...
  // Unpack the data
  if (header->bytes > 0) {
    hboost::mpi::packed_iarchive ia(impl_->comm, incoming.buffer, 
                                ...
                                mpl::false_ /*is_mpi_datatype*/) const
{
  ...
  hboost::mpi::packed_iarchive in(impl_->comm, incoming.buffer, 
                                 archive::no_header, header->offset);
  in >> value;
...
template<typename T>
typename disable_if<hboost::mpi::is_mpi_datatype<T>, bool>::type
mpi_process_group::
...
{
  using hboost::mpi::get_mpi_datatype;
    ...
    int source, int tag, Type& data, mpl::false_ /*is_mpi_datatype*/) 
{
  ...
  hboost::mpi::status status;
  status = self.impl_->comm.probe(source, tag);

#if HBOOST_VERSION >= 103600
  int size = status.count<hboost::mpi::packed>().get();
#else
  int size;
  ...
  hboost::mpi::packed_iarchive in(self.impl_->comm);
  in.resize(size);
...
void do_oob_receive(mpi_process_group const& self, int source, int tag, Type& data) 
{
  do_oob_receive(self, source, tag, data,
                           hboost::mpi::is_mpi_datatype<Type>());
}
        ...
        trigger_receive_context context, int block) const
{
  ...
  if (context == trc_out_of_band) {
    ...
    hboost::graph::distributed::receive(self, source, tag, data);
        ...
        trigger_receive_context context, int block) const
{
  ...
  std::cerr << (out_of_band? "OOB reply trigger" : "Reply trigger") 
        ...
        << " in block " << (block == -1 ? self.my_block_number() : block) << std::endl;
#endif
  HBOOST_ASSERT(context == trc_out_of_band);

  hboost::parallel::detail::untracked_pair<int, Type> data;
        ...
        trigger_receive_context context, int block) const
{
  ...
  HBOOST_ASSERT (context == trc_irecv_out_of_band);

  // force posting of new MPI_Irecv, even though buffer is already allocated
  hboost::mpi::packed_iarchive ia(self.impl_->comm,self.impl_->buffers[tag]);
  ia >> data;
  // Start a new receive
...
prepare_receive(mpi_process_group const& self, int tag, bool force) const
{
 ...
 std::cerr << ("Posting Irecv for trigger") 
  ...
  HBOOST_ASSERT(static_cast<int>(self.impl_->buffers[tag].size()) >= buffer_size);
  
  //HBOOST_MPL_ASSERT(mpl::not_<is_mpi_datatype<Type> >);
  if (force) {
...
receive(const mpi_process_group& pg, int tag, T& value)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
    if (pg.receive_impl(source, pg.encode_tag(pg.my_block_number(), tag),
                        value, hboost::mpi::is_mpi_datatype<T>()))
      return source;
  }
  HBOOST_ASSERT (false);
}
...
template<typename T>
typename 
  enable_if<hboost::mpi::is_mpi_datatype<T>, 
...
template<typename T>
typename 
  disable_if<hboost::mpi::is_mpi_datatype<T>, 
             std::pair<mpi_process_group::process_id_type, std::size_t> >::type
receive(const mpi_process_group& pg, int tag, T values[], std::size_t n)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
           ...
           BinaryOperation bin_op)
{
  ...
  hboost::mpi::all_reduce(hboost::mpi::communicator(communicator(pg),
                                                  hboost::mpi::comm_attach), 
...
{
  // broadcast the seed  
  hboost::mpi::communicator comm(communicator(pg),hboost::mpi::comm_attach);
  hboost::mpi::broadcast(comm,val,root);
}
           ...
           BinaryOperation bin_op)
{
  ...
  hboost::mpi::scan(communicator(pg), first, last-first, out, bin_op);
           ...
           InputIterator last, std::vector<T>& out)
{
  ...
  int result = MPI_Allgather(&size, 1, MPI_INT,
                             &sizes[0], 1, MPI_INT,
                             communicator(pg));
  HBOOST_ASSERT(result == MPI_SUCCESS);
...
{
/*
  hboost::mpi::group current_group = communicator(pg).group();
  hboost::mpi::group new_group = current_group.include(first,last);
  hboost::mpi::communicator new_comm(communicator(pg),new_group);
...
*/
  ...
  if (new_comm != MPI_COMM_NULL) {
    mpi_process_group result_pg(hboost::mpi::communicator(new_comm,hboost::mpi::comm_attach));
...
template<typename T>
typename enable_if<hboost::mpi::is_mpi_datatype<T> >::type
            ...
            mpi_process_group::process_id_type source, int tag, T& value, int block)
{
  ...
  std::pair<hboost::mpi::communicator, int> actual
...
template<typename T>
typename disable_if<hboost::mpi::is_mpi_datatype<T> >::type
receive_oob(const mpi_process_group& pg, 
            mpi_process_group::process_id_type source, int tag, T& value, int block)
{
  // Determine the actual message we expect to receive, and which
  ...
  hboost::optional<hboost::mpi::status> status;
  do {
  ...
  hboost::mpi::packed_iarchive in(actual.first);

#if HBOOST_VERSION >= 103600
  in.resize(status->count<hboost::mpi::packed>().get());
#else
  int size;
  MPI_Status mpi_status = *status;
  MPI_Get_count(&mpi_status, MPI_PACKED, &size);
...
typename enable_if<hboost::mpi::is_mpi_datatype<ReplyT> >::type
                    ...
                    int block)
{
  ...
  send_oob(pg, dest, tag, hboost::parallel::detail::make_untracked_pair(
        (int)reply_tag, send_value), block);
  receive_oob(pg, dest, reply_tag, reply_value);
}

template<typename SendT, typename ReplyT>
typename disable_if<hboost::mpi::is_mpi_datatype<ReplyT> >::type
send_oob_with_reply(const mpi_process_group& pg, 
                    mpi_process_group::process_id_type dest,
                    int tag, const SendT& send_value, ReplyT& reply_value,
                    int block)
{
  ...
  send_oob(pg, dest, tag, 
           hboost::parallel::detail::make_untracked_pair((int)reply_tag, 
                                                        send_value), block);
  receive_oob(pg, dest, reply_tag, reply_value);
}

} } } // end namespace hboost::graph::distributed
