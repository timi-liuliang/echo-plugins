...
#ifndef HBOOST_DEHNE_GOTZ_MIN_SPANNING_TREE_HPP
#define HBOOST_DEHNE_GOTZ_MIN_SPANNING_TREE_HPP
...
#include <utility>
#include <hboost/pending/disjoint_sets.hpp>
#include <hboost/pending/indirect_cmp.hpp>
#include <hboost/property_map/parallel/caching_property_map.hpp>
#include <hboost/graph/vertex_and_edge_range.hpp>
#include <hboost/graph/kruskal_min_spanning_tree.hpp>
...
namespace detail {
                     ...
                     EdgeList& edge_list)
  {
    ...
    typedef typename property_traits<WeightMap>::value_type
                                                           weight_type;
    typedef hboost::parallel::detail::untracked_pair<edge_descriptor, 
                                       weight_type>        w_edge;
                                    ...
                                    SupervertexMap supervertex_map)
{
  ...
  HBOOST_STATIC_ASSERT((is_convertible<traversal_category*,
...
merge_local_minimum_spanning_trees(const Graph& g, WeightMap weight,
                                   ...
                                   GlobalIndexMap global_index)
{
  using hboost::graph::parallel::process_group_type;
  using hboost::graph::parallel::process_group;
  ...
  kruskal_minimum_spanning_tree
    (make_vertex_and_edge_range(g, vertices(g).first, vertices(g).second,
                                edges(g).first, edges(g).second),
     std::back_inserter(edge_list),
     hboost::weight_map(weight).vertex_index_map(global_index));
...
#endif // HBOOST_DEHNE_GOTZ_MIN_SPANNING_TREE_HPP
