...
#ifndef HBOOST_GRAPH_DISTRIBUTED_ADJACENCY_LIST_HPP
#define HBOOST_GRAPH_DISTRIBUTED_ADJACENCY_LIST_HPP
...
#include <hboost/graph/adjacency_iterator.hpp>
...
#include <hboost/graph/parallel/detail/property_holders.hpp>
...
#include <hboost/pending/property_serialize.hpp>
#include <hboost/graph/distributed/unsafe_serialize.hpp>

// Named vertices
#include <hboost/graph/distributed/named_graph.hpp>

#include <hboost/graph/distributed/shuffled_distribution.hpp>
...
namespace hboost {
  ...
  namespace detail {
    ...
    serialize(Archiver& ar, edge_desc_impl<Directed, Vertex>& e,
              const unsigned int /*version*/)
    {
      ar & hboost::serialization::base_object<edge_base<Directed, Vertex> >(e)
  ...
  namespace detail { namespace parallel {
    ...
    struct edge_descriptor
    {
      ...
      public:
        typedef typename hboost::mpl::if_<is_same<DirectedS, bidirectionalS>,
                                         stored_in_edge<Edge>,
    ...
    struct adjacency_list_config
    {
      ...
      typedef typename hboost::mpl::if_<is_same<DirectedS, undirectedS>,
      ...
      typedef typename inherited::vertices_size_type vertices_size_type;
          ...
          typedef typename ::hboost::graph::distributed::select_distribution<
              in_distribution_type, VertexProperty, vertices_size_type, 
              ProcessGroup>::type 
        base_distribution_type;

          typedef ::hboost::graph::distributed::shuffled_distribution<
    ...
    maybe_initialize_vertex_indices(IteratorPair p,
                                    ::hboost::detail::error_property_not_found)
    { }
    ...
    public:
      ...
      void serialize(Archiver& ar, const unsigned int /*version*/)
      {
        ar & hboost::serialization::base_object<inherited_data>(*this) 
           & hboost::serialization::base_object<inherited_property>(*this);
      }
    };
    ...
    struct get_adj_list_pmap<vertex_local_index_t>
    {
      template<typename Graph>
      struct apply
        : ::hboost::property_map<typename Graph::inherited, vertex_index_t>
      { };
    };
    ...
    remove_in_edge(typename Graph::edge_descriptor e, Graph& g, undirectedS)
    {
      ...
      for (hboost::tie(ei, ei_end) = out_edges(v.local, bg); ei != ei_end; ++ei)
      {
    ...
    add_local_edge(typename Config::vertex_descriptor u,
                   typename Config::vertex_descriptor v,
                   const typename Config::edge_property_type& p,
                   hboost::adj_list_impl<Graph, Config, Base>& g)
    {
      return add_edge(u, v, p, g);
    }
      ...
      : public detail::parallel::maybe_store_property<EdgeProperty>
    {
      ...
      void serialize(Archiver& ar, const unsigned int /*version*/)
      {
        ar & hboost::serialization::base_object<inherited>(*this) & e;
      }
    };
  ...
  public:
    ...
    typedef typename hboost::mpl::if_<typename DirectedS::is_bidir_t,
      bidirectional_tag,
      typename hboost::mpl::if_<typename DirectedS::is_directed_t,
...
//    typedef typename inherited::edge_descriptor   edge_descriptor;
    ...
    HBOOST_STATIC_ASSERT((is_same<edge_parallel_category,
    ...
    typedef typename hboost::mpl::if_<
              ...
              typename hboost::mpl::if_<is_same<DirectedS, bidirectionalS>,
  ...
  public:
    ...
    typedef hboost::adjacency_iterator<
  ...
  public:
    adjacency_list(const ProcessGroup& pg = ProcessGroup())
      : named_graph_mixin(pg, default_distribution_type(pg, 0)),
        ...
        process_group_(pg, hboost::parallel::attach_distributed_object())
    {
      setup_triggers();
    }
    ...
    {
      HBOOST_ASSERT(v.owner == processor());
      return base()[v.local];
    }
    ...
    {
      HBOOST_ASSERT(e.owner() == processor());
      return base()[e.local];
    }
    ...
    hboost::function<void(vertex_descriptor, adjacency_list&)> on_add_vertex;

    // Callback that will be invoked whenever a new edge is added locally
    hboost::function<void(edge_descriptor, adjacency_list&)> on_add_edge;
  ...
  public:
    ...
    typedef  hboost::detail::parallel::msg_nonlocal_edge_data<
      edge_property_type,local_edge_descriptor> msg_nonlocal_edge_data;

    typedef hboost::detail::parallel::msg_remove_edge_data<edge_descriptor>
      msg_remove_edge_data;
    ...
    hboost::parallel::detail::untracked_pair<edge_descriptor, bool>
  ...
  protected:
                    ...
                    processor_id_type other_proc, bidirectionalS)
    {
      ...
      hboost::graph_detail::push(get(vertex_in_edges, base())[v], edge);
    }
                      ...
                      processor_id_type other_proc, bidirectionalS)
    {
      ...
      if (src.owner == process_id(process_group_)) {
        ...
        for (hboost::tie(ei, ei_end) = out_edges(src.local, base());
             ei != ei_end; ++ei) {
        ...
        if (ei != ei_end) hboost::remove_edge(ei, base());

        remove_local_edge_from_list(src, tgt, undirectedS());
      } else {
        HBOOST_ASSERT(tgt.owner == process_id(process_group_));
        in_edge_list_type& in_edges =
                      ...
                      processor_id_type other_proc, undirectedS)
    {
      ...
      {
        base_out_edge_iterator ei, ei_end;
        for (hboost::tie(ei, ei_end) = out_edges(local_vertex.local, base());
             ei != ei_end; ++ei) {
          // TBD: can't check the descriptor here, because it could
          // have changed if we're allowing the removal of
          // edges. Egads!
  ...
  PBGL_DISTRIB_ADJLIST_TYPE::lazy_add_edge::commit() const
  {
    ...
    if (source.owner == self.processor())
      ...
      hboost::parallel::detail::untracked_pair<edge_descriptor, bool> result;
  ...
  add_local_edge(const edge_property_type& property, bidirectionalS) const
  {
    ...
    if (result.second) {
      if (target.owner == self.processor()) {
        ...
        hboost::graph_detail::push(get(vertex_in_edges, 
                                      self.base())[target.local], e);
      } 
      else {
        // Edge is remote, so notify the target's owner that an edge
        // has been added.
        if (self.process_group_.trigger_context() == hboost::parallel::trc_out_of_band)
  ...
  out_edges(typename PBGL_DISTRIB_ADJLIST_TYPE::vertex_descriptor v,
            const PBGL_DISTRIB_ADJLIST_TYPE& g)
  {
    HBOOST_ASSERT(v.owner == g.processor());
       ...
       const PBGL_DISTRIB_ADJLIST_TYPE_CONFIG(directedS)& g)
  {
    ...
    HBOOST_ASSERT(u.owner == process_id(g.process_group()));

    typename PBGL_DISTRIB_ADJLIST_TYPE_CONFIG(directedS)
        ::out_edge_iterator ei, ei_end;
    for (hboost::tie(ei, ei_end) = out_edges(u, g); ei != ei_end; ++ei) {
       ...
       const PBGL_DISTRIB_ADJLIST_TYPE& g)
  {
    ...
    if (u.owner == process_id(g.process_group())) {
      ...
      for (hboost::tie(ei, ei_end) = in_edges(v, g); ei != ei_end; ++ei) {
  ...
  remove_edge(typename PBGL_DISTRIB_ADJLIST_TYPE::edge_descriptor e,
              PBGL_DISTRIB_ADJLIST_TYPE& g)
  {
    HBOOST_ASSERT(source(e, g).owner == g.processor()
  ...
  {
    HBOOST_ASSERT(source(*ei, g).owner == g.processor());
      ...
      bool operator()(const argument_type& le)
      {
        ...
        if (predicate(e)) {
          if (source(e, g).owner != target(e, g).owner
            ...
            ::hboost::detail::parallel::remove_in_edge(e, g,
                                                      directed_selector());
      ...
      PBGL_DISTRIB_ADJLIST_TYPE& g)
  {
    ...
    HBOOST_ASSERT(u.owner == g.processor());
      ...
      bool operator()(const argument_type& e)
      {
        if (predicate(e)) {
          if (source(e, g).owner != target(e, g).owner)
            g.send_remove_edge_request(e);
          if (target(e, g).owner == g.processor())
            ::hboost::detail::parallel::remove_in_edge(e, g, undirectedS());
          if (source(e, g).owner == g.processor())
            remove_edge(e.local, g.base());
          return true;
        } else return false;
                ...
                PBGL_DISTRIB_ADJLIST_TYPE& g)
  {
    ...
    static_cast<named_graph_mixin&>(static_cast<graph_type&>(g))
      .removing_vertex(u, hboost::graph_detail::iterator_stability(g.base().m_vertices));
  ...
  struct property_map<PBGL_DISTRIB_ADJLIST_TYPE const, Property>
          : hboost::detail::parallel::get_adj_list_pmap<Property>
// FIXME: in the original code the following was not const
  ...
  namespace mpi {
    ...
    struct is_mpi_datatype<hboost::detail::edge_base<Directed, Vertex> >
      : is_mpi_datatype<Vertex> { };

    template<typename Directed, typename Vertex>
    struct is_mpi_datatype<hboost::detail::edge_desc_impl<Directed, Vertex> >
      : is_mpi_datatype<hboost::detail::edge_base<Directed, Vertex> > { };

    template<typename LocalDescriptor>
    struct is_mpi_datatype<hboost::detail::parallel::global_descriptor<LocalDescriptor> >
      : is_mpi_datatype<LocalDescriptor> { };

    template<typename Edge>
    struct is_mpi_datatype<hboost::detail::parallel::edge_descriptor<Edge> >
      : is_mpi_datatype<Edge> { };

    template<typename Vertex, typename LocalVertex>
    struct is_mpi_datatype<hboost::detail::parallel::
                             msg_add_edge_data<Vertex, LocalVertex> >
      : is_mpi_datatype<Vertex> { };
   ...
   struct is_mpi_datatype<hboost::detail::parallel::msg_nonlocal_edge_data<
                          EdgeProperty,EdgeDescriptor> >
           : mpl::and_<
               is_mpi_datatype<hboost::detail::parallel::maybe_store_property<
                           EdgeProperty> >,
  ...
  {};
   ...
   struct is_mpi_datatype<
            hboost::detail::parallel::msg_remove_edge_data<EdgeDescriptor> >
           : is_mpi_datatype<EdgeDescriptor> {};
  }
  ...
  namespace serialization {
    template<typename Directed, typename Vertex>
    struct is_bitwise_serializable<hboost::detail::edge_base<Directed, Vertex> >
      : is_bitwise_serializable<Vertex> { };

    template<typename Directed, typename Vertex>
    struct is_bitwise_serializable<hboost::detail::edge_desc_impl<Directed, Vertex> >
      : is_bitwise_serializable<hboost::detail::edge_base<Directed, Vertex> > { };

    template<typename LocalDescriptor>
    struct is_bitwise_serializable<hboost::detail::parallel::global_descriptor<LocalDescriptor> >
      : is_bitwise_serializable<LocalDescriptor> { };

    template<typename Edge>
    struct is_bitwise_serializable<hboost::detail::parallel::edge_descriptor<Edge> >
      : is_bitwise_serializable<Edge> { };

    template<typename Vertex, typename LocalVertex>
    struct is_bitwise_serializable<hboost::detail::parallel::
                             msg_add_edge_data<Vertex, LocalVertex> >
      : is_bitwise_serializable<Vertex> { };
   ...
   template<typename EdgeProperty, typename EdgeDescriptor>
   struct is_bitwise_serializable<hboost::detail::parallel::msg_nonlocal_edge_data<
                                  EdgeProperty,EdgeDescriptor> >
           : mpl::and_<
               is_bitwise_serializable<
                hboost::detail::parallel::maybe_store_property<EdgeProperty> >,
  ...
  {};
   ...
   struct is_bitwise_serializable<
    ...
    template<typename Directed, typename Vertex>
    struct implementation_level<hboost::detail::edge_base<Directed, Vertex> >
      : mpl::int_<object_serializable> {};

    template<typename Directed, typename Vertex>
    struct implementation_level<hboost::detail::edge_desc_impl<Directed, Vertex> >
      : mpl::int_<object_serializable> {};

    template<typename LocalDescriptor>
    struct implementation_level<hboost::detail::parallel::global_descriptor<LocalDescriptor> >
      : mpl::int_<object_serializable> {};

    template<typename Edge>
    struct implementation_level<hboost::detail::parallel::edge_descriptor<Edge> >
      : mpl::int_<object_serializable> {};

    template<typename Vertex, typename LocalVertex>
    struct implementation_level<hboost::detail::parallel::
                             msg_add_edge_data<Vertex, LocalVertex> >
      : mpl::int_<object_serializable> {};
   ...
   template<typename EdgeProperty, typename EdgeDescriptor>
   struct implementation_level<hboost::detail::parallel::msg_nonlocal_edge_data<
                               EdgeProperty,EdgeDescriptor> >
           : mpl::int_<object_serializable> {};
   ...
   struct implementation_level<
    ...
    template<typename Directed, typename Vertex>
    struct tracking_level<hboost::detail::edge_base<Directed, Vertex> >
      : mpl::int_<track_never> {};

    template<typename Directed, typename Vertex>
    struct tracking_level<hboost::detail::edge_desc_impl<Directed, Vertex> >
      : mpl::int_<track_never> {};

    template<typename LocalDescriptor>
    struct tracking_level<hboost::detail::parallel::global_descriptor<LocalDescriptor> >
      : mpl::int_<track_never> {};

    template<typename Edge>
    struct tracking_level<hboost::detail::parallel::edge_descriptor<Edge> >
      : mpl::int_<track_never> {};

    template<typename Vertex, typename LocalVertex>
    struct tracking_level<hboost::detail::parallel::
                             msg_add_edge_data<Vertex, LocalVertex> >
      : mpl::int_<track_never> {};
   ...
   template<typename EdgeProperty, typename EdgeDescriptor>
   struct tracking_level<hboost::detail::parallel::msg_nonlocal_edge_data<
                         EdgeProperty,EdgeDescriptor> >
           : mpl::int_<track_never> {};
...
#include <hboost/graph/distributed/adjlist/handlers.hpp>
#include <hboost/graph/distributed/adjlist/initialize.hpp>
#include <hboost/graph/distributed/adjlist/redistribute.hpp>
#include <hboost/graph/distributed/adjlist/serialization.hpp>

#endif // HBOOST_GRAPH_DISTRIBUTED_ADJACENCY_LIST_HPP
