...
//           Douglas Gregor
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_PARALLEL_CC_PS_HPP
#define HBOOST_GRAPH_PARALLEL_CC_PS_HPP
...
namespace hboost { namespace graph { namespace distributed {
  namespace cc_ps_detail {
    ...
    public:
      ...
      {
        HBOOST_ASSERT(collisions.count(a) == 0);
        collisions[a] = a;
      }
    ...
    struct update_reducer {
      HBOOST_STATIC_CONSTANT(bool, non_default_resolver = false);
    ...
    private:
      ...
      hboost::processor_id_type pg_id;
    };
  ...
  connected_components_ps(const Graph& g, ComponentMap c)
  {
    ...
    hboost::tie(current, end) = vertices(g);
    ...
    while (!global_done) {
      ...
      synchronize(pg);
      global_done = all_reduce(pg, (q.empty() && (current == end)), hboost::parallel::minimum<bool>());
