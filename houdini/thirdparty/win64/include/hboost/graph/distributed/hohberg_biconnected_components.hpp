...
//
#ifndef HBOOST_GRAPH_DISTRIBUTED_HOHBERG_BICONNECTED_COMPONENTS_HPP
#define HBOOST_GRAPH_DISTRIBUTED_HOHBERG_BICONNECTED_COMPONENTS_HPP
...
namespace hohberg_detail {
  ...
  {
    HBOOST_ASSERT(!p1.empty());
    HBOOST_ASSERT(!p2.empty());
    HBOOST_ASSERT(p1.front() == p2.front());
   ...
   VertexProcessorMap vertex_processor)
{
  ...
  HBOOST_CONCEPT_ASSERT(( IncidenceGraphConcept<Graph> ));
  ...
  do {
      ...
      case msg_path_header:
        {
          ...
          HBOOST_ASSERT(path_length == header.path_length);
    ...
    minimum_phase = all_reduce(pg, minimum_phase, hboost::mpi::minimum<char>());
    ...
    if (minimum_phase == 4
        && all_reduce(pg, maximum_phase, hboost::mpi::maximum<char>()) == 4) {
...
#endif // HBOOST_GRAPH_DISTRIBUTED_HOHBERG_BICONNECTED_COMPONENTS_HPP
