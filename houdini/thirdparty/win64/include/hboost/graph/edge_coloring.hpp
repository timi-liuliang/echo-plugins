...
//=======================================================================
#ifndef HBOOST_GRAPH_EDGE_COLORING_HPP
#define HBOOST_GRAPH_EDGE_COLORING_HPP
...
namespace hboost {
  namespace detail {
    ...
    is_free(const Graph &g,
            ...
            typename hboost::property_traits<ColorMap>::value_type free_color)
    {
      typedef typename hboost::property_traits<ColorMap>::value_type color_t;
    ...
    template<typename Graph, typename ColorMap>
    std::vector<typename hboost::graph_traits<Graph>::vertex_descriptor>
    maximal_fan(const Graph &g,
                ColorMap color,
                typename hboost::graph_traits<Graph>::vertex_descriptor x,
                typename hboost::graph_traits<Graph>::vertex_descriptor y)
    {
    ...
    template<typename Graph, typename ColorMap>
    typename hboost::property_traits<ColorMap>::value_type
    find_free_color(const Graph &g,
                    ColorMap color,
                    typename hboost::graph_traits<Graph>::vertex_descriptor u)
    {
      typename hboost::property_traits<ColorMap>::value_type c = 0;
    ...
    invert_cd_path(const Graph &g,
                   ColorMap color,
                   typename hboost::graph_traits<Graph>::vertex_descriptor x,
                   typename hboost::graph_traits<Graph>::edge_descriptor eold,
                   typename hboost::property_traits<ColorMap>::value_type c,
                   typename hboost::property_traits<ColorMap>::value_type d)
    {
      put(color, eold, d);
      BGL_FORALL_OUTEDGES_T(x, e, g, Graph) {
    ...
    rotate_fan(const Graph &g,
               ...
               ForwardIterator end)
    {
      typedef typename hboost::graph_traits<Graph>::edge_descriptor edge_t;
      if (begin == end) {
        return;
      }
    ...
    public:
      find_free_in_fan(const Graph &graph,
        ...
        : graph(graph),
          color(color),
          d(d) {}
      bool operator()(const typename hboost::graph_traits<Graph>::vertex_descriptor u) const {
        return is_free(graph, color, u, d);
      }
    private:
      const Graph &graph;
      const ColorMap color;
      const typename hboost::property_traits<ColorMap>::value_type d;
    };
  }
