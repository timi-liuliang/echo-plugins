...
#ifndef HBOOST_GRAPH_STRONG_COMPONENTS_HPP
#define HBOOST_GRAPH_STRONG_COMPONENTS_HPP
...
namespace hboost {
  ...
  namespace detail {
       ...
       const bgl_named_params<P, T, R>& params)
    {
      ...
      HBOOST_CONCEPT_ASSERT(( ReadWritePropertyMapConcept<ComponentMap, Vertex> ));
      HBOOST_CONCEPT_ASSERT(( ReadWritePropertyMapConcept<RootMap, Vertex> ));
      typedef typename property_traits<RootMap>::value_type RootV;
      HBOOST_CONCEPT_ASSERT(( ConvertibleConcept<RootV, Vertex> ));
      HBOOST_CONCEPT_ASSERT(( ReadWritePropertyMapConcept<DiscoverTime, Vertex> ));
...
#include <queue>
#include <vector>
#include <hboost/graph/transpose_graph.hpp>
#include <hboost/pending/indirect_cmp.hpp>
#include <hboost/graph/connected_components.hpp> // for components_recorder
...
namespace hboost {
  ...
  {
    HBOOST_CONCEPT_ASSERT(( MutableGraphConcept<Graph> ));
    // ...
    ...
    for (hboost::tie(ui, ui_end) = vertices(G_T); ui != ui_end; ++ui)
    ...
    hboost::tie(i, iend) = vertices(G_T);
    hboost::tie(j, jend) = vertices(G);
    for ( ; i != iend; ++i, ++j) {
...
#  include <hboost/graph/distributed/strong_components.hpp>
#endif

#endif // HBOOST_GRAPH_STRONG_COMPONENTS_HPP
