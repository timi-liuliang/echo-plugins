...
#ifndef HBOOST_GRAPH_CLIQUE_HPP
#define HBOOST_GRAPH_CLIQUE_HPP
...
namespace hboost {
    namespace concepts {
        HBOOST_concept(CliqueVisitor,(Visitor)(Clique)(Graph))
        {
            HBOOST_CONCEPT_USAGE(CliqueVisitor)
            {
                vis.clique(k, g);
            }
        private:
...
struct max_clique_visitor
{
    ...
    inline void clique(const Clique& p, const Graph& g)
    {
        ...
        maximum = max HBOOST_PREVENT_MACRO_SUBSTITUTION (maximum, p.size());
    }
    std::size_t& maximum;
};
...
namespace detail
{
    ...
    {
        HBOOST_CONCEPT_ASSERT(( GraphConcept<Graph> ));
        HBOOST_CONCEPT_ASSERT(( CliqueVisitorConcept<Visitor,Clique,Graph> ));
...
bron_kerbosch_all_cliques(const Graph& g, Visitor vis, std::size_t min)
{
    ...
    HBOOST_CONCEPT_ASSERT(( AdjacencyMatrixConcept<Graph> )); // Structural requirement only
    typedef typename graph_traits<Graph>::vertex_descriptor Vertex;
