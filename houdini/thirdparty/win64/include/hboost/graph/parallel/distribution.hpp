...
//           Peter Gottschling
//           Andrew Lumsdaine
#ifndef HBOOST_PARALLEL_DISTRIBUTION_HPP
#define HBOOST_PARALLEL_DISTRIBUTION_HPP
...
struct uneven_block
{
  ...
  { 
    HBOOST_ASSERT(local_sizes.size() == p);
  ...
  SizeType operator()(SizeType i) const
  {
    HBOOST_ASSERT (i >= (SizeType) 0 && i < (SizeType) n); // check for valid range
...
#endif // HBOOST_PARALLEL_DISTRIBUTION_HPP
