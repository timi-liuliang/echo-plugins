...
#ifndef HBOOST_SUBGRAPH_HPP
#define HBOOST_SUBGRAPH_HPP
...
public:
        ...
        : m_graph(n, p), m_parent(0), m_edge_counter(0), m_global_vertex(n)
    {
        ...
        for(hboost::tie(v, v_end) = vertices(m_graph); v != v_end; ++v)
            m_global_vertex[i++] = *v;
    }
        ...
        , m_global_vertex(x.m_global_vertex), m_global_edge(x.m_global_edge)
    {
        ...
        hboost::tie(i,i_end) = x.children();
        for(; i != i_end; ++i)
        {         
         subgraph<Graph> child = this->create_subgraph();
         child = *i;
         vertex_iterator vi,vi_end;   
         hboost::tie(vi,vi_end) = vertices(*i);
         for (;vi!=vi_end;++vi)  
         {
          add_vertex(*vi,child);
         }
    ...
    vertex_descriptor global_to_local(vertex_descriptor u_global) const {
        ...
        hboost::tie(u_local, in_subgraph) = this->find_vertex(u_global);
        HBOOST_ASSERT(in_subgraph == true);
        return u_local;
    }
    ...
    HBOOST_STATIC_ASSERT((!is_same<edge_index_type,
                        hboost::detail::error_property_not_found>::value));
...
public: // Probably shouldn't be public....
                                   ...
                                   edge_descriptor e_global)
    {
        ...
        bool inserted;
        hboost::tie(e_local, inserted) = add_edge(u_local, v_local, m_graph);
           ...
           subgraph<G>& g)
{
    ...
    // remember edge global and local maps
    {
        ...
        for (hboost::tie(ei, ei_end) = out_edges(u_global, r);
            ei != ei_end; ++ei) {
            e_global = *ei;
    ...
    if (is_directed(g)) { // not necessary for undirected graph
        ...
        typename subgraph<G>::out_edge_iterator ei, ei_end;
        for(hboost::tie(vi, vi_end) = vertices(r); vi != vi_end; ++vi) {
            v_global = *vi;
            ...
            for(hboost::tie(ei, ei_end) = out_edges(*vi, r); ei != ei_end; ++ei) {
                e_global = *ei;
...
namespace detail {
                             ...
                             subgraph<Graph>& g, subgraph<Graph>* orig)
    {
        if(&g != orig ) {
            ...
            hboost::tie(u_local, u_in_subgraph) = g.find_vertex(u_global);
            hboost::tie(v_local, v_in_subgraph) = g.find_vertex(v_global);
                      ...
                      subgraph<Graph>& g, subgraph<Graph>* orig)
    {
        if(g.is_root()) {
            ...
            bool inserted;
            hboost::tie(e_global, inserted) = add_edge(u_global, v_global, ep, g.m_graph);
...
remove_edge(typename subgraph<G>::edge_descriptor e, subgraph<G>& g)
{
    ...
    HBOOST_ASSERT(fe.second && fe.first == e);
#endif //NDEBUG
