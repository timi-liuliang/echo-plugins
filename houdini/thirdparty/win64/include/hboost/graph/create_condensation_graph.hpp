...
#ifndef HBOOST_CREATE_CONDENSATION_GRAPH_HPP
#define HBOOST_CREATE_CONDENSATION_GRAPH_HPP
                                 ...
                                 EdgeMultiplicityMap edge_mult_map)
  {
    ...
    for (size_type si = 0; si < components.size(); ++si) {
      ...
      for (size_type i = 0; i < components[si].size(); ++i) {
        ...
        for (hboost::tie(v, v_end) = adjacent_vertices(u, g); v != v_end; ++v) {
      ...
      if (! adj.empty()) {
        ...
        bool inserted;
        hboost::tie(e, inserted) = add_edge(s, t, cg);
...
#endif // HBOOST_CREATE_CONDENSATION_GRAPH_HPP
