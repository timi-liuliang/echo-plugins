...
#ifndef HBOOST_REVERSE_GRAPH_DWA092300_H_
# define HBOOST_REVERSE_GRAPH_DWA092300_H_
...
class reverse_graph {
 ...
 public:
    ...
    { return m_g[detail::get_underlying_descriptor_from_reverse_descriptor<Descriptor>::convert(x)]; }
#endif // HBOOST_GRAPH_NO_BUNDLED_PROPERTIES
...
struct vertex_property_type<reverse_graph<BidirectionalGraph, GraphRef> > {
  typedef typename hboost::vertex_property_type<BidirectionalGraph>::type type;
};

template <class BidirectionalGraph, class GraphRef>
struct edge_property_type<reverse_graph<BidirectionalGraph, GraphRef> > {
  typedef typename hboost::edge_property_type<BidirectionalGraph>::type type;
};

template <class BidirectionalGraph, class GraphRef>
struct graph_property_type<reverse_graph<BidirectionalGraph, GraphRef> > {
  typedef typename hboost::graph_property_type<BidirectionalGraph>::type type;
};
...
struct property_map<reverse_graph<BidirGraph, GRef>, Property> {
  typedef hboost::is_same<typename detail::property_kind_from_graph<BidirGraph, Property>::type, edge_property_tag> is_edge_prop;
  typedef hboost::is_const<typename hboost::remove_reference<GRef>::type> is_ref_const;
  ...
  typedef typename hboost::mpl::if_<is_edge_prop, detail::reverse_graph_edge_property_map<orig_type>, orig_type>::type type;
  typedef typename hboost::mpl::if_<is_edge_prop, detail::reverse_graph_edge_property_map<orig_const_type>, orig_const_type>::type const_type;
};
...
template <typename T> struct is_reverse_graph: hboost::mpl::false_ {};
template <typename G, typename R> struct is_reverse_graph<reverse_graph<G, R> >: hboost::mpl::true_ {};
...
typename hboost::mpl::if_<
           hboost::is_const<typename hboost::remove_reference<GRef>::type>,
