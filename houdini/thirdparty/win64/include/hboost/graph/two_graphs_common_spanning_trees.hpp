...
#ifndef HBOOST_GRAPH_TWO_GRAPHS_COMMON_SPANNING_TREES_HPP
#define HBOOST_GRAPH_TWO_GRAPHS_COMMON_SPANNING_TREES_HPP


#include <hboost/config.hpp>

#include <hboost/bimap.hpp>
#include <hboost/type_traits.hpp>
#include <hboost/concept/requires.hpp>
#include <hboost/graph/graph_traits.hpp>
#include <hboost/graph/undirected_dfs.hpp>
...
public:
  HBOOST_CONCEPT_ASSERT((BackInsertionSequence<Coll>));
  HBOOST_CONCEPT_ASSERT((RandomAccessContainer<Seq>));
  HBOOST_CONCEPT_ASSERT((CopyConstructible<Seq>));

  typedef typename Coll::value_type coll_value_type;
  typedef typename Seq::value_type seq_value_type;

  HBOOST_STATIC_ASSERT((is_same<coll_value_type, Seq>::value));
  HBOOST_STATIC_ASSERT((is_same<seq_value_type, bool>::value));
  ...
  )
{
  ...
  HBOOST_STATIC_ASSERT((is_same<order_value_type, edge_descriptor>::value));
  HBOOST_CONCEPT_ASSERT((Convertible<order_size_type, edges_size_type>));

  HBOOST_CONCEPT_ASSERT((Convertible<seq_size_type, edges_size_type>));
  HBOOST_STATIC_ASSERT((is_same<seq_value_type, bool>::value));

  HBOOST_STATIC_ASSERT((is_same<directed_category, undirected_tag>::value));
  ...
  hboost::tuples::tie(current, last) = edges(iG);
  for(; current != last; ++current)
    if(iG_bimap.right.find(*current) == iG_bimap.right.end())
      return;
  hboost::tuples::tie(current, last) = edges(vG);
  for(; current != last; ++current)
  ...
  )
{
  ...
  hboost::tuples::tie(curr, last) = edges(iG);
  for(; curr != last; ++curr)
    iGO.push_back(*curr);

  hboost::tuples::tie(curr, last) = edges(vG);
  for(; curr != last; ++curr)
    vGO.push_back(*curr);
...
#endif // HBOOST_GRAPH_TWO_GRAPHS_COMMON_SPANNING_TREES_HPP
