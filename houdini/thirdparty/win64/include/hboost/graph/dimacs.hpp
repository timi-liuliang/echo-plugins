...
//  Authors: Alex Breuer
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_DIMACS_HPP
#define HBOOST_GRAPH_DIMACS_HPP
...
public:
  ...
  dimacs_basic_reader( std::istream& in, bool want_weights = true ) : 
    ...
    if( buf[0] != 'p' ) {
        hboost::throw_exception(dimacs_exception());
    }
  ...
  edge_type edge_deref() {
    HBOOST_ASSERT( !read_edges.empty() );
  ...
  inline edge_weight_type edge_weight_deref() {
    HBOOST_ASSERT( !read_edge_weights.empty() );
  ...
  inline dimacs_basic_reader incr( incr_mode mode ) {
    ...
    if( (mode == edge && read_edges.empty()) ||
      ...
      HBOOST_ASSERT( read_edges.size() < 100 );
      HBOOST_ASSERT( read_edge_weights.size() < 100 );
    }
