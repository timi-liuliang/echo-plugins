...
//           Douglas Gregor
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_TOPOLOGY_HPP
#define HBOOST_GRAPH_TOPOLOGY_HPP

#include <hboost/config/no_tr1/cmath.hpp>
#include <cmath>
#include <hboost/random/uniform_01.hpp>
#include <hboost/random/linear_congruential.hpp>
#include <hboost/math/constants/constants.hpp> // For root_two
#include <hboost/algorithm/minmax.hpp>
#include <hboost/config.hpp> // For HBOOST_STATIC_CONSTANT
#include <hboost/math/special_functions/hypot.hpp>
...
class convex_topology 
{
  ...
  struct point 
  {
    HBOOST_STATIC_CONSTANT(std::size_t, dimensions = Dims);
    point() { }
 ...
 public:
  ...
  double distance(point a, point b) const 
  {
    ...
    for (std::size_t i = 0; i < Dims; ++i) {
      double diff = b[i] - a[i];
      dist = hboost::math::hypot(dist, diff);
    }
  ...
  point pointwise_min(point a, point b) const {
    ...
    point result;
    for (std::size_t i = 0; i < Dims; ++i)
      result[i] = min HBOOST_PREVENT_MACRO_SUBSTITUTION (a[i], b[i]);
    return result;
  }
  ...
  point pointwise_max(point a, point b) const {
    ...
    point result;
    for (std::size_t i = 0; i < Dims; ++i)
      result[i] = max HBOOST_PREVENT_MACRO_SUBSTITUTION (a[i], b[i]);
    return result;
  }
...
class hypercube_topology : public convex_topology<Dims>
{
 ...
 public:
  ...
  point_type bound(point_type a) const
  {
    ...
    point_type p;
    for (std::size_t i = 0; i < Dims; ++i)
      p[i] = min HBOOST_PREVENT_MACRO_SUBSTITUTION (scaling, max HBOOST_PREVENT_MACRO_SUBSTITUTION (-scaling, a[i]));
    return p;
  }
  ...
  double distance_from_boundary(point_type a) const
  {
    ...
    using std::abs;
#endif
    HBOOST_STATIC_ASSERT (Dims >= 1);
    double dist = abs(scaling - a[0]);
    for (std::size_t i = 1; i < Dims; ++i)
      dist = min HBOOST_PREVENT_MACRO_SUBSTITUTION (dist, abs(scaling - a[i]));
    return dist;
  }
...
class rectangle_topology : public convex_topology<2>
{
  ...
  rectangle_topology(double left, double top, double right, double bottom)
    : gen_ptr(new RandomNumberGenerator), rand(new rand_t(*gen_ptr)),
      left(std::min HBOOST_PREVENT_MACRO_SUBSTITUTION (left, right)),
      top(std::min HBOOST_PREVENT_MACRO_SUBSTITUTION (top, bottom)),
      right(std::max HBOOST_PREVENT_MACRO_SUBSTITUTION (left, right)),
      bottom(std::max HBOOST_PREVENT_MACRO_SUBSTITUTION (top, bottom)) { }
  ...
  point_type bound(point_type a) const
  {
    ...
    HBOOST_USING_STD_MAX();
    point_type p;
    p[0] = min HBOOST_PREVENT_MACRO_SUBSTITUTION (right, max HBOOST_PREVENT_MACRO_SUBSTITUTION (left, a[0]));
    p[1] = min HBOOST_PREVENT_MACRO_SUBSTITUTION (bottom, max HBOOST_PREVENT_MACRO_SUBSTITUTION (top, a[1]));
    return p;
  }
  ...
  double distance_from_boundary(point_type a) const
  {
    ...
    dist = min HBOOST_PREVENT_MACRO_SUBSTITUTION (dist, abs(right - a[0]));
    dist = min HBOOST_PREVENT_MACRO_SUBSTITUTION (dist, abs(top - a[1]));
    dist = min HBOOST_PREVENT_MACRO_SUBSTITUTION (dist, abs(bottom - a[1]));
    return dist;
  }
...
class heart_topology 
{
  ...
  bool in_heart(point p) const 
  {
    ...
    if (hboost::math::hypot(p[0] - -500, p[1] - -500) <= 500. * hboost::math::constants::root_two<double>())
 ...
 public:
  ...
  point random_point() const 
  {
    point result;
    do {
      result[0] = (*rand)() * (1000 + 1000 * hboost::math::constants::root_two<double>()) - (500 + 500 * hboost::math::constants::root_two<double>());
      result[1] = (*rand)() * (2000 + 500 * (hboost::math::constants::root_two<double>() - 1)) - 2000;
  ...
  double distance(point a, point b) const 
  {
    if (segment_within_heart(a, b)) {
      // Straight line
      return hboost::math::hypot(b[0] - a[0], b[1] - a[1]);
    } else {
      // Straight line bending around (0, 0)
      return hboost::math::hypot(a[0], a[1]) + hboost::math::hypot(b[0], b[1]);
    }
  }
  ...
  point move_position_toward(point a, double fraction, point b) const 
  {
    if (segment_within_heart(a, b)) {
      ...
      double distance_to_point_a = hboost::math::hypot(a[0], a[1]);
      double distance_to_point_b = hboost::math::hypot(b[0], b[1]);
...
#endif // HBOOST_GRAPH_TOPOLOGY_HPP
