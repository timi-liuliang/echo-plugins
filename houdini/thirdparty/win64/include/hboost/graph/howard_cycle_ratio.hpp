...
#ifndef HBOOST_GRAPH_CYCLE_RATIO_HOWARD_HPP
#define HBOOST_GRAPH_CYCLE_RATIO_HOWARD_HPP
...
namespace hboost {
  ...
  namespace detail {
    ...
    public:
      ...
      typedef typename hboost::iterator_property_map<
    ...
    private:
      ...
      float_t mcr_bound()
      {
        ...
        for (hboost::tie(vi, vie) = vertices(m_g); vi != vie; ++vi)
          {
            for (hboost::tie(oei, oeie) = out_edges(*vi, m_g); oei != oeie; ++oei)
              {
      ...
      void construct_policy_graph()
      {
        ...
        for ( hboost::tie(vi, vie) = vertices(m_g); vi != vie; ++vi )
          {
            hboost::tie(oei, oeie) = out_edges(*vi, m_g);
            typename graph_traits<Graph>::out_edge_iterator mei =
              std::max_element(oei, oeie,
                               hboost::bind(m_cmp,
                                           hboost::bind(&EdgeWeight1::operator[], m_ew1m, _1),
                                           hboost::bind(&EdgeWeight1::operator[], m_ew1m, _2)
                                          )
                               );
            if (mei == oeie)
              {
      ...
      float_t policy_mcr()
      {
        ...
        hboost::tie(uv_itr, vie) = vertices(m_g);
        float_t mcr = m_bound;
        while ( (uv_itr = std::find_if(uv_itr, vie,
                                       hboost::bind(std::equal_to<my_color_type>(),
                                                   my_white,
                                                   hboost::bind(&color_map_t::operator[], vcm_, _1)
                                                   )
                                       )
                 ) != vie )
 ...
 optimum_cycle_ratio(const TG &g, TVIM vim, TEW1 ewm, TEW2 ew2m, EV* pcc)
    {
      ...
      HBOOST_STATIC_ASSERT((is_convertible<DirCat*, directed_tag*>::value == true));
      HBOOST_CONCEPT_ASSERT(( IncidenceGraphConcept<TG> ));
      HBOOST_CONCEPT_ASSERT(( VertexListGraphConcept<TG> ));
      typedef typename graph_traits<TG>::vertex_descriptor Vertex;
      HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<TVIM, Vertex> ));
      typedef typename graph_traits<TG>::edge_descriptor Edge;
      HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<TEW1, Edge> ));
      HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<TEW2, Edge> ));
                   ...
                   FloatTraits ft = FloatTraits())
{
    ...
    >::type Weight;
    typename std::vector<Weight> ed_w2(hboost::num_edges(g), 1);
