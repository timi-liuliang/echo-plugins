...
#ifndef HBOOST_GRAPH_CLUSTERING_COEFFICIENT_HPP
#define HBOOST_GRAPH_CLUSTERING_COEFFICIENT_HPP
...
namespace detail
{
    ...
    {
        HBOOST_CONCEPT_ASSERT(( AdjacencyMatrixConcept<Graph> ));
...
{
    HBOOST_CONCEPT_ASSERT(( AdjacencyGraphConcept<Graph> ));
    ...
    hboost::tie(i, end) = adjacent_vertices(v, g);
...
num_triangles_on_vertex(const Graph& g, Vertex v)
{
    ...
    for(hboost::tie(i, end) = adjacent_vertices(v, g); i != end; ++i) {
        for(j = hboost::next(i); j != end; ++j) {
...
all_clustering_coefficients(const Graph& g, ClusteringMap cm)
{
    ...
    HBOOST_CONCEPT_ASSERT(( WritablePropertyMapConcept<ClusteringMap,Vertex> ));
...
mean_clustering_coefficient(const Graph& g, ClusteringMap cm)
{
    ...
    typedef typename graph_traits<Graph>::vertex_descriptor Vertex;
    typedef typename graph_traits<Graph>::vertex_iterator VertexIterator;
    HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<ClusteringMap,Vertex> ));
    typedef typename property_traits<ClusteringMap>::value_type Coefficient;
