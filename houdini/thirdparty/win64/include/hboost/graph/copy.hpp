...
#ifndef HBOOST_GRAPH_COPY_HPP
#define HBOOST_GRAPH_COPY_HPP
...
namespace hboost {
  ...
  namespace detail {
    ...
    struct add_reverse_edge_descriptor<Desc, hboost::reverse_graph<G, GR> > {
    ...
    template <typename Desc, typename G, typename GR>
    struct add_reverse_edge_descriptor<reverse_graph_edge_descriptor<Desc>, hboost::reverse_graph<G, GR> > {
      typedef reverse_graph_edge_descriptor<Desc> type;
    ...
    template <> struct copy_graph_impl<0>
    {
                        ...
                        Orig2CopyVertexIndexMap orig2copy, IndexMap)
      {
        ...
        for (hboost::tie(vi, vi_end) = vertices(g_in); vi != vi_end; ++vi) {
          typename graph_traits<MutableGraph>::vertex_descriptor
        ...
        for (hboost::tie(ei, ei_end) = edges(g_in); ei != ei_end; ++ei) {
          typename graph_traits<MutableGraph>::edge_descriptor new_e;
          bool inserted;
          hboost::tie(new_e, inserted) = add_edge(get(orig2copy, source(*ei, g_in)), 
    ...
    template <> struct copy_graph_impl<1>
    {
                        ...
                        Orig2CopyVertexIndexMap orig2copy, IndexMap)
      {
        ...
        for (hboost::tie(vi, vi_end) = vertices(g_in); vi != vi_end; ++vi) {
          ...
          for (hboost::tie(ei, ei_end) = out_edges(*vi, g_in); ei != ei_end; ++ei) {
            typename graph_traits<MutableGraph>::edge_descriptor new_e;
            bool inserted;
            hboost::tie(new_e, inserted) = add_edge(get(orig2copy, source(*ei, g_in)), 
                                                   get(orig2copy, target(*ei, g_in)),
                  ...
                  const bgl_named_params<P, T, R>& params)
  {
    ...
    if (n == 0)
      return;
    std::vector<HBOOST_DEDUCED_TYPENAME graph_traits<MutableGraph>::vertex_descriptor> 
      orig2copy(n);
  ...
  namespace detail {
    ...
    struct graph_copy_visitor : public bfs_visitor<>
    {
      ...
      void tree_edge(Edge e, const Graph& g_in) const {
        ...
        bool inserted;
        hboost::tie(new_e, inserted) = add_edge(get(orig2copy, source(e, g_in)), 
