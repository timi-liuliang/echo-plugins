...
#ifndef HBOOST_VECTOR_AS_GRAPH_HPP
#define HBOOST_VECTOR_AS_GRAPH_HPP
...
namespace hboost {
  ...
  namespace detail {
    ...
    // need rewrite this using hboost::iterator_adaptor
    template <class V, class Iter>
    class val_out_edge_iterator
      : public hboost::iterator<std::input_iterator_tag, std::pair<V,V>,
  ...
  template <class EdgeList, class Alloc>
  std::pair<hboost::counting_iterator<typename EdgeList::value_type>,
            hboost::counting_iterator<typename EdgeList::value_type> >
  vertices(const std::vector<EdgeList, Alloc>& v)
  {
    typedef hboost::counting_iterator<typename EdgeList::value_type> Iter;
...
#endif // HBOOST_VECTOR_AS_GRAPH_HPP
