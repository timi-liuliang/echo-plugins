...
#ifndef HBOOST_PUSH_RELABEL_MAX_FLOW_HPP
#define HBOOST_PUSH_RELABEL_MAX_FLOW_HPP
...
namespace hboost {
   ...
   // not blocked).
    ...
    public:
      ...
      void add_to_active_list(vertex_descriptor u, Layer& layer) {
        ...
        max_active = max HBOOST_PREVENT_MACRO_SUBSTITUTION(get(distance, u), max_active);
        min_active = min HBOOST_PREVENT_MACRO_SUBSTITUTION(get(distance, u), min_active);
          ...
          work_since_last_update(0)
      {
        ...
        for (hboost::tie(u_iter, u_end) = vertices(g); u_iter != u_end; ++u_iter)
          for (hboost::tie(ei, e_end) = out_edges(*u_iter, g); ei != e_end; ++ei) {
        ...
        for (hboost::tie(a_iter, a_end) = out_edges(src, g); a_iter != a_end; ++a_iter)
        ...
        if (overflow_detected)
          ...
          for (hboost::tie(a_iter, a_end) = out_edges(src, g); 
               a_iter != a_end; ++a_iter) {
      ...
      void global_distance_update()
      {
        ...
        while (! Q.empty()) {
          ...
          for (hboost::tie(ai, a_end) = out_edges(u, g); ai != a_end; ++ai) {
            ...
            if (get(color, v) == ColorTraits::white()
              ...
              max_distance = max HBOOST_PREVENT_MACRO_SUBSTITUTION(d_v, max_distance);
      ...
      {
        HBOOST_ASSERT(get(excess_flow, u) > 0);
        while (1) {
          out_edge_iterator ai, ai_end;
          for (hboost::tie(ai, ai_end) = current[u]; ai != ai_end; ++ai) {
            edge_descriptor a = *ai;
      ...
      void push_flow(edge_descriptor u_v)
      {
        ...
        FlowValue flow_delta
          = min HBOOST_PREVENT_MACRO_SUBSTITUTION(get(excess_flow, u), get(residual_capacity, u_v));
      ...
      distance_size_type relabel_distance(vertex_descriptor u)
      {
        ...
        if (min_distance < n) {
          ...
          max_distance = max HBOOST_PREVENT_MACRO_SUBSTITUTION(min_distance, max_distance);
        }
        return min_distance;
      } // relabel_distance()
      ...
      void convert_preflow_to_flow()
      {
        ...
        // handle self-loops
        for (hboost::tie(u_iter, u_end) = vertices(g); u_iter != u_end; ++u_iter)
          for (hboost::tie(ai, a_end) = out_edges(*u_iter, g); ai != a_end; ++ai)
              ...
              for (; current[u].first != current[u].second; ++current[u].first) {
                ...
                if (get(capacity, a) == 0 && is_residual_edge(a)) {
                  ...
                  if (get(color, v) == ColorTraits::white()) {
                    ...
                    while (1) {
                      HBOOST_USING_STD_MIN();
                      delta = min HBOOST_PREVENT_MACRO_SUBSTITUTION(delta, get(residual_capacity, *current[v].first));
                      if (v == u)
                        break;
                      else
        ...
        if (! bos_null) {
          for (u = tos; u != bos; u = topo_next[get(index, u)]) {
            hboost::tie(ai, a_end) = out_edges(u, g);
      ...
      hboost::queue<vertex_descriptor> Q;
     ...
     ReverseEdgeMap rev, VertexIndexMap index_map)
  {
    ...
    HBOOST_ASSERT(algo.is_flow());
    HBOOST_ASSERT(algo.is_optimal());
...
#endif // HBOOST_PUSH_RELABEL_MAX_FLOW_HPP
