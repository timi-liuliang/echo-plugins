...
#ifndef HBOOST_GRAPH_BELLMAN_FORD_SHORTEST_PATHS_HPP
#define HBOOST_GRAPH_BELLMAN_FORD_SHORTEST_PATHS_HPP
...
namespace hboost {
  ...
  {
    HBOOST_CONCEPT_ASSERT(( EdgeListGraphConcept<EdgeListGraph> ));
    typedef graph_traits<EdgeListGraph> GTraits;
    typedef typename GTraits::edge_descriptor Edge;
    typedef typename GTraits::vertex_descriptor Vertex;
    HBOOST_CONCEPT_ASSERT(( ReadWritePropertyMapConcept<DistanceMap, Vertex> ));
    HBOOST_CONCEPT_ASSERT(( ReadablePropertyMapConcept<WeightMap, Edge> ));

    typename GTraits::edge_iterator i, end;

    for (Size k = 0; k < N; ++k) {
      bool at_least_one_edge_relaxed = false;
      for (hboost::tie(i, end) = edges(g); i != end; ++i) {
        v.examine_edge(*i, g);
...
#endif // HBOOST_GRAPH_BELLMAN_FORD_SHORTEST_PATHS_HPP
