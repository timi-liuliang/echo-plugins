...
//           Douglas Gregor
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_GURSOY_ATUN_LAYOUT_HPP
#define HBOOST_GRAPH_GURSOY_ATUN_LAYOUT_HPP
...
struct update_position_visitor {
  ...
  typedef hboost::on_examine_vertex event_filter;
  ...
  void operator()(vertex_descriptor v, const Graph&) const 
  {
    ...
    if (get(node_distance, v) > distance_limit)
      HBOOST_THROW_EXCEPTION(over_distance_limit());
...
struct gursoy_shortest
{
  ...
  {
    hboost::dijkstra_shortest_paths(g, s, weight_map(weight).
      visitor(hboost::make_dijkstra_visitor(std::make_pair(
       hboost::record_distances(node_distance, hboost::on_edge_relaxed()),
        update_position))));
  }
};
...
struct gursoy_shortest<dummy_property_map>
{
  ...
  run(const Graph& g, typename graph_traits<Graph>::vertex_descriptor s,
      ...
      dummy_property_map)
  {
    hboost::breadth_first_search(g, s,
      visitor(hboost::make_bfs_visitor(std::make_pair(
        hboost::record_distances(node_distance, hboost::on_tree_edge()),
        update_position))));
  }
};
   ...
   EdgeWeightMap weight)
{
  ...
  typedef hboost::iterator_property_map<std::vector<double>::iterator, 
                                       VertexIndexMap,
                                       double, double&>
  ...
  for (hboost::tie(i, iend) = vertices(graph); i != iend; ++i) {
  ...
  HBOOST_ASSERT (!min_distance_unset); // Graph must have at least one vertex
  hboost::detail::update_position_visitor<
                        ...
                        EdgeWeightMap weight) 
{
  ...
  typedef hboost::iterator_property_map<std::vector<int>::iterator, 
...
#endif // HBOOST_GRAPH_GURSOY_ATUN_LAYOUT_HPP
