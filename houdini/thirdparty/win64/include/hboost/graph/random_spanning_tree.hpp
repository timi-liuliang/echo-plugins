...
#ifndef HBOOST_GRAPH_RANDOM_SPANNING_TREE_HPP
#define HBOOST_GRAPH_RANDOM_SPANNING_TREE_HPP

#include <vector>
#include <hboost/assert.hpp>
#include <hboost/graph/loop_erased_random_walk.hpp>
...
namespace hboost {
  ...
  namespace detail {
    ...
    void random_spanning_tree_internal(const Graph& g, typename graph_traits<Graph>::vertex_descriptor s, PredMap pred, ColorMap color, NextEdge next_edge) {
      ...
      HBOOST_ASSERT (num_vertices(g) >= 1); // g must also be undirected (or symmetric) and connected
      ...
      BGL_FORALL_VERTICES_T(v, g, Graph) {
        ...
        for (typename std::vector<vertex_descriptor>::const_reverse_iterator i = path.rbegin();
          ...
          ++j;
          HBOOST_ASSERT (get(color, *j) == color_gen::gray());
...
#endif // HBOOST_GRAPH_RANDOM_SPANNING_TREE_HPP
