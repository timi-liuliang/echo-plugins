...
#ifndef HBOOST_GRAPH_WAVEFRONT_HPP
#define HBOOST_GRAPH_WAVEFRONT_HPP
...
namespace hboost {
    ...
    rows_active[index_i] = true;
      ...
      for (hboost::tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui)
      {
        v = *ui;
          if(index[v] <= index_i)
            {
              for (hboost::tie(edge_it2, edge_it2_end) = out_edges(v, g); edge_it2 != edge_it2_end; ++edge_it2)
              {
                w = target(*edge_it2, g);
  ...
  max_wavefront(const Graph& g, VertexIndexMap index)
  {
    ...
    for (hboost::tie(i, end) = vertices(g); i != end; ++i)
      b = max HBOOST_PREVENT_MACRO_SUBSTITUTION(b, ith_wavefront(*i, g, index));
    return b;
  }
...
#endif // HBOOST_GRAPH_WAVEFRONT_HPP
