...
#ifndef HBOOST_GRAPH_TEST_HPP
#define HBOOST_GRAPH_TEST_HPP

#include <vector>
#include <hboost/test/minimal.hpp>
#include <hboost/graph/filtered_graph.hpp>
#include <hboost/graph/iteration_macros.hpp>
#include <hboost/graph/isomorphism.hpp>
#include <hboost/graph/copy.hpp>
#include <hboost/graph/graph_utility.hpp> // for connects
#include <hboost/range.hpp>
...
namespace hboost {
  ...
  struct graph_test
  {
        ...
        const Graph& g)
    {
      ...
      for (vertex_iter ui = vertex_set.begin(); ui != vertex_set.end(); ++ui) {
        ...
        HBOOST_CHECK(out_degree(u, g) == adj.size());
        HBOOST_CHECK(deg_size_t(std::distance(p.first, p.second))
                   == out_degree(u, g));
        for (; p.first != p.second; ++p.first) {
          edge_t e = *p.first;
          HBOOST_CHECK(source(e, g) == u);
          HBOOST_CHECK(container_contains(adj, target(e, g)) == true);
        }
      }
    }
       ...
       const Graph& g)
    {
      ...
      for (vertex_iter vi = vertex_set.begin(); vi != vertex_set.end(); ++vi) {
        ...
        HBOOST_CHECK(in_degree(v, g) == inv_adj.size());
        HBOOST_CHECK(deg_size_t(std::distance(p.first, p.second))
                   == in_degree(v, g));
        for (; p.first != p.second; ++p.first) {
          edge_t e = *p.first;
          HBOOST_CHECK(target(e, g) == v);
          HBOOST_CHECK(container_contains(inv_adj, source(e, g)) == true);
        }
      }
    }
       ...
       const Graph& g)
    {
      ...
      for (vertex_iter ui = vertex_set.begin(); ui != vertex_set.end(); ++ui) {
        ...
        HBOOST_CHECK(deg_size_t(std::distance(p.first, p.second)) == adj.size());
        for (; p.first != p.second; ++p.first) {
          vertex_t v = *p.first;
          HBOOST_CHECK(container_contains(adj, v) == true);
        }
      }
    }      
      ...
      (const std::vector<vertex_t>& vertex_set, const Graph& g)
    {
      ...
      HBOOST_CHECK(num_vertices(g) == vertex_set.size());
      v_size_t n = (size_t)std::distance(p.first, p.second);
      HBOOST_CHECK(n == num_vertices(g));
      for (; p.first != p.second; ++p.first) {
        vertex_t v = *p.first;
        HBOOST_CHECK(container_contains(vertex_set, v) == true);
      }
    }
       ...
       const Graph& g)
    {
      ...
      HBOOST_CHECK(num_edges(g) == edge_set.size());
      e_size_t m = std::distance(p.first, p.second);
      HBOOST_CHECK(m == num_edges(g));
      for (; p.first != p.second; ++p.first) {
        edge_t e = *p.first;
        HBOOST_CHECK(find_if(edge_set, connects(source(e, g), target(e, g), g)) != hboost::end(edge_set));
        HBOOST_CHECK(container_contains(vertex_set, source(e, g)) == true);
        HBOOST_CHECK(container_contains(vertex_set, target(e, g)) == true);
      }
    }
       ...
       const Graph& g)
    {
      ...
      for (typename std::vector<std::pair<vertex_t, vertex_t> >
        ...
        HBOOST_CHECK(p.second == true);
        HBOOST_CHECK(source(p.first, g) == i->first);
        HBOOST_CHECK(target(p.first, g) == i->second);
      }
      ...
      for (j = vertex_set.begin(); j != vertex_set.end(); ++j)
        for (k = vertex_set.begin(); k != vertex_set.end(); ++k) {
          p = edge(*j, *k, g);
          if (p.second == true)
            HBOOST_CHECK(find_if(edge_set, 
              connects(source(p.first, g), target(p.first, g), g)) != hboost::end(edge_set));
        }
    }
    ...
    void test_add_vertex(Graph& g)
    {
      ...
      HBOOST_CHECK((verify_isomorphism(g, cpy, iso_map)));

      vertex_t v = add_vertex(g);
      
      HBOOST_CHECK(num_vertices(g) == num_vertices(cpy) + 1);

      HBOOST_CHECK(out_degree(v, g) == 0);

      // Make sure the rest of the graph stayed the same
      HBOOST_CHECK((verify_isomorphism
    ...
    void test_add_edge(vertex_t u, vertex_t v, Graph& g)
    {
      ...
      if (is_undirected(g) && u == v) // self edge
        HBOOST_CHECK(added == false);
      else if (parallel_edge_exists)
        HBOOST_CHECK(allows_parallel_edges(g) && added == true
                   || !allows_parallel_edges(g) && added == false);
      else
        HBOOST_CHECK(added == true);

      if (p.second == true) { // edge added
        HBOOST_CHECK(num_edges(g) == num_edges(cpy) + 1);
        
        HBOOST_CHECK(container_contains(out_edges(u, g), e) == true);
    ...
    void test_remove_edge(vertex_t u, vertex_t v, Graph& g)
    {
      ...
      HBOOST_CHECK(num_edges(g) + occurances == num_edges(cpy));
      HBOOST_CHECK((verify_isomorphism
    ...
    void test_remove_edge(edge_t e, Graph& g)
    {
      ...
      HBOOST_CHECK(num_edges(g) + 1 == num_edges(cpy));
      HBOOST_CHECK(count(adjacent_vertices(u, g), v) + 1 == occurances);
      HBOOST_CHECK((verify_isomorphism
    ...
    void test_clear_vertex(vertex_t v, Graph& g)
    {
      ...
      HBOOST_CHECK(out_degree(v, g) == 0);
      HBOOST_CHECK(num_vertices(g) == num_vertices(cpy));
      HBOOST_CHECK((verify_isomorphism
  ...
  for (typename hboost::graph_traits<Graph>::vertex_iterator 
           bgl_first_9 = vertices(g).first, bgl_last_9 = vertices(g).second;
       bgl_first_9 != bgl_last_9; bgl_first_9 = bgl_last_9)
    for (typename hboost::graph_traits<Graph>::vertex_descriptor v;
      ...
      //BGL_FORALL_VERTICES_T(v, g, Graph) {
        ...
        HBOOST_CHECK(pval1 == pval2);
        HBOOST_CHECK(pval1 == *i++);
      }
    }
