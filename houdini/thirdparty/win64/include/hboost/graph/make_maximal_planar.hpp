...
#include <hboost/tuple/tuple.hpp>   //for tie
#include <hboost/graph/biconnected_components.hpp>
#include <hboost/property_map/property_map.hpp>
#include <vector>
#include <iterator>
#include <algorithm>

#include <hboost/graph/planar_face_traversal.hpp>
#include <hboost/graph/planar_detail/add_edge_visitors.hpp>
...
namespace hboost
{
  ...
  struct triangulation_visitor : public planar_face_traversal_visitor
  {
    ...
    void end_face()
    {
      ...
      for(hboost::tie(ai,ai_end) = adjacent_vertices(vertices_on_face.front(),g); 
          ai != ai_end; ++ai
          )
        {
      ...
      for(face_iterator fi = hboost::next(hboost::next(vertices_on_face.begin())); 
          fi != fi_end; ++fi
          )
        {
      ...
      if (marked_neighbor == vertices_on_face.end())
        {
          add_edge_range(
                         vertices_on_face[0],
                         hboost::next(hboost::next(vertices_on_face.begin())),
