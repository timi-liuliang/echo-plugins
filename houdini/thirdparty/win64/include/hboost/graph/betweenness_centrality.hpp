...
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_BRANDES_BETWEENNESS_CENTRALITY_HPP
#define HBOOST_GRAPH_BRANDES_BETWEENNESS_CENTRALITY_HPP
...
namespace detail { namespace graph {
                                      ...
                                      ShortestPaths shortest_paths)
  {
    ...
    for (hboost::tie(s, s_end) = vertices(g); s != s_end; ++s) {
      // Initialize for this iteration
      vertex_iterator w, w_end;
      for (hboost::tie(w, w_end) = vertices(g); w != w_end; ++w) {
        incoming[*w].clear();
        put(path_count, *w, 0);
        put(dependency, *w, 0);
      }
...
#endif // HBOOST_GRAPH_BRANDES_BETWEENNESS_CENTRALITY_HPP
