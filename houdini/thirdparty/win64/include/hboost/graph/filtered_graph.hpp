...
namespace hboost {
  ...
  // hboost::tie(vi, viend) = vertices(g);
            ...
            const filtered_graph<G, EP, VP>& g)
  {
    ...
    hboost::tie(f, l) = out_edges(u, g.m_g);
    return std::make_pair(iter(pred, f, l), iter(pred, l, l));
  }
            ...
            const filtered_graph<G, EP, VP>& g)
  {
    ...
    hboost::tie(f, l) = in_edges(u, g.m_g);
    return std::make_pair(iter(pred, f, l), iter(pred, l, l));
  }
       ...
       const filtered_graph<G, EP, VP>& g)
  {
    ...
    bool exists;
    hboost::tie(e, exists) = edge(u, v, g.m_g);
             ...
             const filtered_graph<G, EP, VP>& g)
  {
    ...
    typedef typename Graph::out_edge_iterator iter;
    typename graph_traits<G>::out_edge_iterator f, l;
    hboost::tie(f, l) = edge_range(u, v, g.m_g);
    return std::make_pair(iter(pred, f, l), iter(pred, l, l));
  }
...
#endif // HBOOST_FILTERED_GRAPH_HPP
