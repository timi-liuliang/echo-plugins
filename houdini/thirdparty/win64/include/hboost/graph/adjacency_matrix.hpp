...
#ifndef HBOOST_ADJACENCY_MATRIX_HPP
#define HBOOST_ADJACENCY_MATRIX_HPP
...
#include <hboost/property_map/function_property_map.hpp>
...
namespace hboost {
  ...
  public:
    ...
    // in_degree, etc.).
    HBOOST_STATIC_ASSERT(!(is_same<Directed, bidirectionalS>::value));
             ...
             const adjacency_matrix<D,VP,EP,GP,A>& g)
  {
    ...
    for (hboost::tie(f, l) = out_edges(u, g); f != l; ++f)
      ++n;
    return n;
  }
             ...
             const adjacency_matrix<D,VP,EP,GP,A>& g)
  {
    ...
    for (hboost::tie(f, l) = in_edges(u, g); f != l; ++f)
      ++n;
    return n;
  }
  ...
  struct adj_mat_pm_helper<D, VP, EP, GP, A, Tag, edge_property_tag> {
    ...
    struct lookup_property_from_edge {
      ...
      typedef typename hboost::mpl::if_<IsConst, const EP, EP>::type ep_type_nonref;
    ...
    typedef function_property_map<
              lookup_property_from_edge<hboost::mpl::false_>,
              typename graph_traits<adjacency_matrix<D, VP, EP, GP, A> >::edge_descriptor> type;
    typedef function_property_map<
              lookup_property_from_edge<hboost::mpl::true_>,
              typename graph_traits<adjacency_matrix<D, VP, EP, GP, A> >::edge_descriptor> const_type;
    typedef edge_descriptor arg_type;
    typedef typename lookup_property_from_edge<hboost::mpl::false_>::result_type single_nonconst_type;
    typedef typename lookup_property_from_edge<hboost::mpl::true_>::result_type single_const_type;
...
#endif // HBOOST_ADJACENCY_MATRIX_HPP
