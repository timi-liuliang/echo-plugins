...
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_FRUCHTERMAN_REINGOLD_FORCE_DIRECTED_LAYOUT_HPP
#define HBOOST_GRAPH_FRUCHTERMAN_REINGOLD_FORCE_DIRECTED_LAYOUT_HPP
...
#include <hboost/graph/topology.hpp> // For topology concepts
#include <vector>
#include <list>
...
struct all_force_pairs
{
  ...
  void operator()(const Graph& g, ApplyForce apply_force)
  {
    ...
    for (hboost::tie(v, end) = vertices(g); v != end; ++v) {
      vertex_iterator u = v;
      for (++u; u != end; ++u) {
        apply_force(*u, *v);
        apply_force(*v, *u);
...
struct grid_force_pairs
{
  ...
  HBOOST_STATIC_ASSERT (Point::dimensions == 2);
  ...
  DisplacementMap displacement)
{
  ...
  do {
    ...
    for (hboost::tie(e, e_end) = edges(g); e != e_end; ++e) {
      ...
      // When the vertices land on top of each other, move the
      // first vertex away from the boundaries.
      ::hboost::detail::maybe_jitter_point(topology, position, u, get(position, v));
    ...
    if (double temp = cool()) {
      ...
      BGL_FORALL_VERTICES_T (v, g, Graph) {
        ...
        put(position, v, topology.adjust(get(position, v), get(displacement, v) * (min HBOOST_PREVENT_MACRO_SUBSTITUTION (disp_size, temp) / disp_size)));
...
#  include <hboost/graph/distributed/fruchterman_reingold.hpp>
#endif

#endif // HBOOST_GRAPH_FRUCHTERMAN_REINGOLD_FORCE_DIRECTED_LAYOUT_HPP
