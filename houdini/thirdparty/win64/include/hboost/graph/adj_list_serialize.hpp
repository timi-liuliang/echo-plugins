...
struct tracking_level<hboost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> > {
  typedef mpl::integral_c_tag tag;
  typedef mpl::int_<track_never> type;
  HBOOST_STATIC_CONSTANT(int, value = tracking_level::type::value);
};
...
inline void save(
    Archive & ar,
    const hboost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> &graph,
...
inline void load(
    Archive & ar,
    hboost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> &graph,
    const unsigned int /* file_version */
){
  typedef adjacency_list<OEL,VL,D,VP,EP,GP,EL> Graph;
  typedef typename graph_traits<Graph>::vertex_descriptor Vertex;
  ...
  while(E-- > 0){
    int u; int v;
    ar >> HBOOST_SERIALIZATION_NVP(u);
    ar >> HBOOST_SERIALIZATION_NVP(v);
    Edge e; bool inserted;
    hboost::tie(e,inserted) = add_edge(verts[u], verts[v], graph);
    ar >> serialization::make_nvp("edge_property", get(edge_all_t(), graph, e) );
  }
  ar >> serialization::make_nvp("graph_property", get_property(graph, graph_all_t()) );
}
...
inline void serialize(
    ...
    hboost::serialization::split_free(ar, graph, file_version);
}
