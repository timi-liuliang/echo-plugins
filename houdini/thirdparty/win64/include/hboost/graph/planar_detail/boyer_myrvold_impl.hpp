...
#include <hboost/config.hpp>    //for std::min macros
...
#include <hboost/graph/planar_detail/face_handles.hpp>
#include <hboost/graph/planar_detail/face_iterators.hpp>
#include <hboost/graph/planar_detail/bucket_sort.hpp>
...
namespace hboost
{
  ...
  struct planar_dfs_visitor : public dfs_visitor<>
  {
    ...
    void back_edge(const Edge& e, Graph& g)
    {
      ...
      if ( t != get(parent, s) ) {
        ...
        put(low, s, 
            min HBOOST_PREVENT_MACRO_SUBSTITUTION(s_low_df_number,
                                                 t_df_number)
            );
        
        put(least_ancestor, s, 
            min HBOOST_PREVENT_MACRO_SUBSTITUTION(s_least_ancestor_df_number, 
                                                 t_df_number
                                                 )
            );
    ...
    void finish_vertex(const Vertex& u, Graph&)
    {
      ...
      if (u_parent != u)
        {
          put(low, u_parent, 
              min HBOOST_PREVENT_MACRO_SUBSTITUTION(u_lowpoint, 
                                                   u_parent_lowpoint
                                                   )
              );
        }
  ...
  class boyer_myrvold_impl
  {
    ...
    typedef hboost::shared_ptr< face_handle_list_t > face_handle_list_ptr_t;
    typedef hboost::shared_ptr< vertex_list_t > vertex_list_ptr_t;
    typedef hboost::tuple<vertex_t, bool, bool> merge_stack_frame_t;
  ...
  private:
    ...
    void walkup(vertex_t v)
    {
      ...
      for(hboost::tie(oi,oi_end) = out_edges(v,g); oi != oi_end; ++oi)
        {
          edge_t e(*oi);
                      ...
                      if (first_tail != v)
                        {
                          ...
                          hboost::tie(first_side_vertex, first_tail) 
                      ...
                      else if (second_tail != v)
                        {
                          ...
                          hboost::tie(second_side_vertex, second_tail) 
    ...
    void clean_up_embedding(graph::detail::store_embedding)
    {
      ...
      for(hboost::tie(xi,xi_end) = vertices(g); xi != xi_end; ++xi)
        {
  ...
  public:
    ...
    void extract_kuratowski_subgraph(OutputIterator o_itr, EdgeIndexMap em)
    {
      ...
      for(hboost::tie(ei, ei_end) = edges(g); ei != ei_end; ++ei)
        {
          edge_t e(*ei);
          goal_edge[e] 
      ...
      if (bicomp_root != v)
        {
          ...
          for(hboost::tie(vi,vi_end) = vertices(g); vi != vi_end; ++vi)
            if (lower_face_vertex[*vi])
              for(hboost::tie(oei,oei_end) = out_edges(*vi,g); oei != oei_end; ++oei)
      ...
      else 
        {
              ...
              for(; old_face_itr != old_face_end; ++old_face_itr)
                {
                  ...
                  if (w == graph_traits<Graph>::null_vertex() && 
                      ...
                      outer_face_edge[e] &&
                      outer_face_edge[*hboost::next(old_face_itr)] &&
                      !seen_x_or_y
                      )
                    {
                      w = current_vertex;
          ...
          for(hboost::tie(v_edge_itr,v_edge_end) = out_edges(v,g); 
          ...
          if (chosen_case == detail::BM_CASE_E)
            {
              ...
              for(hboost::tie(oei, oei_end) = out_edges(w,g); oei != oei_end; ++oei)
                {
                  if (!outer_face_edge[*oei])
                    goal_edge[*oei] = true;
                }
      ...
      while(child != parent)
        {
          ...
          hboost::tie(parent, child) = std::make_pair( dfs_parent[parent], parent );
        }
