...
namespace hboost
{
                                 ...
                                 VertexIndexMap vm)
  {
    ...
    for(hboost::tie(ai,ai_end) = adjacent_vertices(first_vertex,g); ai != ai_end; ++ai)
      {
        if (*ai == first_vertex)
          continue;
        second_vertex = *ai;
        ...
        for(ei = ei_start; ei != ei_end; ++ei)
          {
            ...
            edge_t e(*ei); // e = (u,v)
            next_edge_itr = hboost::next(ei) == ei_end ? ei_start : hboost::next(ei);
            ...
            while (next_vertex == v || next_vertex == u)
              {
                next_edge_itr = hboost::next(next_edge_itr) == ei_end ?
                  ei_start : hboost::next(next_edge_itr);
