...
#ifndef HBOOST_GRAPH_FLOYD_WARSHALL_HPP
#define HBOOST_GRAPH_FLOYD_WARSHALL_HPP
...
namespace hboost
{
  namespace detail {
      ...
      const Zero& zero)
    {
      ...
      for (hboost::tie(k, lastk) = vertices(g); k != lastk; k++)
        for (hboost::tie(i, lasti) = vertices(g); i != lasti; i++)
          if(d[*i][*k] != inf)
            for (hboost::tie(j, lastj) = vertices(g); j != lastj; j++)
              if(d[*k][*j] != inf)
                d[*i][*j] = 
  ...
  bool floyd_warshall_initialized_all_pairs_shortest_paths(
    ...
    const Zero& zero)
  {
    HBOOST_CONCEPT_ASSERT(( VertexListGraphConcept<VertexListGraph> ));
  ...
  {
    HBOOST_CONCEPT_ASSERT(( VertexListGraphConcept<VertexAndEdgeListGraph> ));
    HBOOST_CONCEPT_ASSERT(( EdgeListGraphConcept<VertexAndEdgeListGraph> ));
    HBOOST_CONCEPT_ASSERT(( IncidenceGraphConcept<VertexAndEdgeListGraph> ));
    ...
    for(hboost::tie(firstv, lastv) = vertices(g); firstv != lastv; firstv++)
      for(hboost::tie(firstv2, lastv2) = vertices(g); firstv2 != lastv2; firstv2++)
    ...
    for(hboost::tie(first, last) = edges(g); first != last; first++)
    {
      ...
      const bgl_named_params<P, T, R>& params)
    {
      ...
      WM inf =
        choose_param(get_param(params, distance_inf_t()), 
          std::numeric_limits<WM>::max HBOOST_PREVENT_MACRO_SUBSTITUTION());
