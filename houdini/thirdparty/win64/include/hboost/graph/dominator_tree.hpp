...
#ifndef HBOOST_GRAPH_DOMINATOR_HPP
#define HBOOST_GRAPH_DOMINATOR_HPP
...
namespace hboost {
  namespace detail {
    ...
    public :
                 ...
                 const PredMap& parentMap, const Graph& g)
      {
        ...
        for (hboost::tie(inItr, inEnd) = in_edges(n, g); inItr != inEnd; ++inItr)
          {
     ...
     DomTreePredMap domTreePredMap)
  {
    ...
    HBOOST_CONCEPT_ASSERT(( BidirectionalGraphConcept<Graph> ));
        ...
        for (hboost::tie(vi, viend) = vertices(g); vi != viend; ++vi)
          {
            ...
            typename graph_traits<Graph>::in_edge_iterator inItr, inEnd;
            for (hboost::tie(inItr, inEnd) = in_edges(*vi, g); inItr != inEnd; ++inItr)
              {
          ...
          } // end of for (hboost::tie(vi, viend) = vertices(g)
      } // end of while(change)
...
#endif // HBOOST_GRAPH_DOMINATOR_HPP
