...
#ifndef HBOOST_GRAPH_HAWICK_CIRCUITS_HPP
#define HBOOST_GRAPH_HAWICK_CIRCUITS_HPP
...
#include <hboost/tuple/tuple.hpp> // for hboost::tie
...
struct get_all_adjacent_vertices {
    ...
    typename result<
        get_all_adjacent_vertices(HBOOST_FWD_REF(Vertex), HBOOST_FWD_REF(Graph))
    >::type
    operator()(HBOOST_FWD_REF(Vertex) v, HBOOST_FWD_REF(Graph) g) const {
        return adjacent_vertices(hboost::forward<Vertex>(v),
                                 hboost::forward<Graph>(g));
    }
};
...
bool contains(Container const& c, Value const& v) {
    return std::find(hboost::begin(c), hboost::end(c), v) != hboost::end(c);
}
...
private:
    ...
    bool blocked_map_starts_all_unblocked() const {
        HBOOST_FOREACH(Vertex v, vertices(graph_))
            if (is_blocked(v))
                return false;
        return true;
    }
    ...
    bool all_closed_rows_are_empty() const {
        HBOOST_FOREACH(typename ClosedMatrix::reference row, closed_)
            if (!row.empty())
                return false;
        return true;
    }
...
public:
    hawick_circuits_from(Graph const& graph, Visitor& visitor,
        ...
        {
            HBOOST_ASSERT(blocked_map_starts_all_unblocked());
            ...
            HBOOST_ASSERT(stack_.empty());
            HBOOST_ASSERT(closed_.size() == n_vertices);
            HBOOST_ASSERT(all_closed_rows_are_empty());
        }
...
private:
    ...
    void close_to(Vertex u, Vertex v) {
        HBOOST_ASSERT(!is_closed_to(u, v));
    ...
    void unblock(Vertex u) {
        ...
        HBOOST_ASSERT(closed_to_u.empty());
    }
    ...
    bool circuit(Vertex start, Vertex v) {
        ...
        AdjacencyIterator const w_end = hboost::end(adj_vertices);

        for (AdjacencyIterator w_it = hboost::begin(adj_vertices);
             w_it != w_end;
             ++w_it)
        {
            Vertex const w = *w_it;
...
void call_hawick_circuits(Graph const& graph,
    ...
    for (hboost::tie(start, last) = vertices(graph); start != last; ++start) {
...
void call_hawick_circuits(Graph const& graph, HBOOST_FWD_REF(Visitor) visitor) {
    call_hawick_circuits<GetAdjacentVertices>(
        graph, hboost::forward<Visitor>(visitor), get(vertex_index, graph)
    );
}
...
void hawick_circuits(HBOOST_FWD_REF(Graph) graph,
                     HBOOST_FWD_REF(Visitor) visitor,
                     HBOOST_FWD_REF(VertexIndexMap) vertex_index_map) {
    hawick_circuits_detail::call_hawick_circuits<
        hawick_circuits_detail::get_all_adjacent_vertices
    >(
        hboost::forward<Graph>(graph),
        hboost::forward<Visitor>(visitor),
        hboost::forward<VertexIndexMap>(vertex_index_map)
    );
}
...
void hawick_circuits(HBOOST_FWD_REF(Graph) graph,
    ...
    hawick_circuits_detail::call_hawick_circuits<
        hawick_circuits_detail::get_all_adjacent_vertices
    >(hboost::forward<Graph>(graph), hboost::forward<Visitor>(visitor));
}

/*!
 * Same as `hboost::hawick_circuits`, but duplicate circuits caused by parallel
 * edges will not be considered. Each circuit will be considered only once.
 */
template <typename Graph, typename Visitor, typename VertexIndexMap>
void hawick_unique_circuits(HBOOST_FWD_REF(Graph) graph,
                            HBOOST_FWD_REF(Visitor) visitor,
                            HBOOST_FWD_REF(VertexIndexMap) vertex_index_map) {
    hawick_circuits_detail::call_hawick_circuits<
...
#endif // !HBOOST_GRAPH_HAWICK_CIRCUITS_HPP
