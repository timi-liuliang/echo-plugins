...
//=======================================================================
#ifndef HBOOST_GRAPH_RANDOM_HPP
#define HBOOST_GRAPH_RANDOM_HPP
...
namespace hboost {
  ...
  random_out_edge(Graph& g, typename graph_traits<Graph>::vertex_descriptor src, RandomNumGen& gen) {
    ...
    typedef hboost::uniform_int<degree_size_type> ui_type;
    ui_type ui(0, out_degree(src, g) - 1);
    hboost::variate_generator<RandomNumGen&, ui_type>
      variate(gen, ui);
  ...
  weighted_random_out_edge(Graph& g, typename graph_traits<Graph>::vertex_descriptor src, WeightMap weight, RandomNumGen& gen) {
    ...
    typedef hboost::uniform_real<> ur_type;
    ur_type ur(0, weight_sum);
    hboost::variate_generator<RandomNumGen&, ur_type>
      variate(gen, ur);
    ...
    HBOOST_ASSERT (false); // Should not get here
  }
     ...
     bool self_edges = false)
  {
    ...
    if (!allow_parallel) {
      ...
      typedef typename hboost::graph_traits<MutableGraph>::directed_category dir;
      ...
      for (e_size_t j = 0; j < E; /* Increment in body */) {
        ...
        hboost::tie(e, inserted) = add_edge(a, b, g);
        if (inserted) {
          ++j;
        } else {
  ...
  namespace detail {
                            ...
                            Property, vertex_property_tag)
    {
      ...
      for (hboost::tie(vi, ve) = vertices(g); vi != ve; ++vi) {
        pm[*vi] = rg();
      }
    }
                            ...
                            Property, edge_property_tag)
    {
      ...
      for (hboost::tie(ei, ee) = edges(g); ei != ee; ++ei) {
        pm[*ei] = rg();
      }
    }
  }
