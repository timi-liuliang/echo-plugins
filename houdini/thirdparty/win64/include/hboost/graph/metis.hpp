...
//           Andrew Lumsdaine
#ifndef HBOOST_GRAPH_METIS_HPP
#define HBOOST_GRAPH_METIS_HPP

#ifdef HBOOST_GRAPH_METIS_NO_INLINE
#  define HBOOST_GRAPH_METIS_INLINE_KEYWORD
#else
#  define HBOOST_GRAPH_METIS_INLINE_KEYWORD inline
#endif
...
#if !defined(HBOOST_GRAPH_METIS_NO_INLINE) || defined(HBOOST_GRAPH_METIS_SOURCE)
HBOOST_GRAPH_METIS_INLINE_KEYWORD
...
void metis_reader::edge_iterator::advance(bool skip_initial_read)
{
  do {
      ...
      if (self->line_in >> std::ws >> self->edge.second) {
        ...
        if (self->has_edge_weights()) {
          if (!(self->line_in >> self->edge_weight))
            hboost::throw_exception(metis_input_exception());
        }
        return;
      }
    ...
    if (!self->in) hboost::throw_exception(metis_input_exception());
    ...
    while (weights_left > 0) {
      ...
      else hboost::throw_exception(metis_input_exception());
      --weights_left;
    }
...
void metis_reader::start()
{
  ...
  if (!in || line.empty()) hboost::throw_exception(metis_input_exception());

  // Determine number of vertices and edges in the graph
  line_in.str(line);
  if (!(line_in >> n_vertices >> n_edges)) hboost::throw_exception(metis_input_exception());
