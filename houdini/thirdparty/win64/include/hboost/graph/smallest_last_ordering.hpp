...
#ifndef HBOOST_SMALLEST_LAST_VERTEX_ORDERING_HPP
#define HBOOST_SMALLEST_LAST_VERTEX_ORDERING_HPP
/*
...
namespace hboost {
  ...
  smallest_last_vertex_ordering(const VertexListGraph& G, Order order, 
    ...
    typedef typename hboost::graph_traits<VertexListGraph> GraphTraits;
    ...
    typedef typename hboost::property_map<VertexListGraph, vertex_index_t>::type ID;
  ...
  smallest_last_vertex_ordering(const VertexListGraph& G, Order order, 
    ...
    while ( 1 ) {
      ...
      for (hboost::tie(v,vend) = adjacent_vertices(node, G); v != vend; ++v)
        ...
        if ( get(marker,*v) > current_order ) { //*v is unordered vertex
          ...
          minimum_degree = min HBOOST_PREVENT_MACRO_SUBSTITUTION(minimum_degree, get(degree, *v)); 
