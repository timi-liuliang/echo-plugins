...
#ifndef HBOOST_GRAPH_LOOP_ERASED_RANDOM_WALK_HPP
#define HBOOST_GRAPH_LOOP_ERASED_RANDOM_WALK_HPP
...
namespace hboost {
  ...
  void loop_erased_random_walk(
         const Graph& g,
         typename hboost::graph_traits<Graph>::vertex_descriptor s,
         NextEdge next_edge,
         ColorMap color,
         std::vector<typename hboost::graph_traits<Graph>::vertex_descriptor>& path
  ) {
    ...
    typedef hboost::color_traits<color_t> color_gen;
    
    HBOOST_ASSERT (get(color, s) == color_gen::white());
    path.clear();
    path.push_back(s);
  ...
  class unweighted_random_out_edge_gen {
    ...
    typedef hboost::graph_traits<Graph> gt;
    ...
    operator()(typename gt::vertex_descriptor src, const Graph& g) const {
      ...
      return hboost::random_out_edge(g, src, gen);
    }
  };
  ...
  class weighted_random_out_edge_gen {
    ...
    operator()(typename gt::vertex_descriptor src, const Graph& g) const {
      if (out_degree(src, g) == 0) throw loop_erased_random_walk_stuck();
      return hboost::weighted_random_out_edge(g, src, weight, gen);
    }
  };
}

#endif // HBOOST_GRAPH_LOOP_ERASED_RANDOM_WALK_HPP
