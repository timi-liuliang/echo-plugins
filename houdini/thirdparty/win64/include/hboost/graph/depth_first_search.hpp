...
#ifndef HBOOST_GRAPH_RECURSIVE_DFS_HPP
#define HBOOST_GRAPH_RECURSIVE_DFS_HPP
...
#include <hboost/tti/has_member_function.hpp>
...
namespace hboost {
  ...
  public:
    void constraints() {
      HBOOST_CONCEPT_ASSERT(( CopyConstructibleConcept<Visitor> ));
  ...
  namespace detail {
    ...
    HBOOST_TTI_HAS_MEMBER_FUNCTION(finish_edge)
...
// Define HBOOST_RECURSIVE_DFS to use older, recursive version.
// It is retained for a while in order to perform performance
// comparison.
#ifndef HBOOST_RECURSIVE_DFS
    ...
    //
    // See http://lists.boost.org/MailArchives/hboost/msg48752.php for FAQ.
    ...
    {
      HBOOST_CONCEPT_ASSERT(( IncidenceGraphConcept<IncidenceGraph> ));
      HBOOST_CONCEPT_ASSERT(( DFSVisitorConcept<DFSVisitor, IncidenceGraph> ));
      typedef typename graph_traits<IncidenceGraph>::vertex_descriptor Vertex;
      typedef typename graph_traits<IncidenceGraph>::edge_descriptor Edge;
      HBOOST_CONCEPT_ASSERT(( ReadWritePropertyMapConcept<ColorMap, Vertex> ));
      typedef typename property_traits<ColorMap>::value_type ColorValue;
      HBOOST_CONCEPT_ASSERT(( ColorValueConcept<ColorValue> ));
      typedef color_traits<ColorValue> Color;
      typedef typename graph_traits<IncidenceGraph>::out_edge_iterator Iter;
      typedef std::pair<Vertex, std::pair<hboost::optional<Edge>, std::pair<Iter, Iter> > > VertexInfo;

      hboost::optional<Edge> src_e;
      Iter ei, ei_end;
      ...
      if (func(u, g)) {
          ...
          stack.push_back(std::make_pair(u, std::make_pair(hboost::optional<Edge>(), std::make_pair(ei_end, ei_end))));
      } else {
          stack.push_back(std::make_pair(u, std::make_pair(hboost::optional<Edge>(), std::make_pair(ei, ei_end))));
      }
      while (!stack.empty()) {
        VertexInfo& back = stack.back();
        u = back.first;
        src_e = back.second.first;
        hboost::tie(ei, ei_end) = back.second.second;
        stack.pop_back();
        while (ei != ei_end) {
...
#else // HBOOST_RECURSIVE_DFS is defined
                     ...
                     typename graph_traits<VertexListGraph>::vertex_descriptor start_vertex)
  {
    ...
    HBOOST_CONCEPT_ASSERT(( DFSVisitorConcept<DFSVisitor, VertexListGraph> ));
    typedef typename property_traits<ColorMap>::value_type ColorValue;
    typedef color_traits<ColorValue> Color;

    typename graph_traits<VertexListGraph>::vertex_iterator ui, ui_end;
    for (hboost::tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui) {
  ...
  depth_first_search(const VertexListGraph& g, DFSVisitor vis, ColorMap color)
  {
    typedef typename hboost::graph_traits<VertexListGraph>::vertex_iterator vi;
  ...
  public:
    ...
    void discover_vertex(Vertex u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::hboost::on_discover_vertex());
    }
    template <class Edge, class Graph>
    void examine_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::hboost::on_examine_edge());
    }
    template <class Edge, class Graph>
    void tree_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::hboost::on_tree_edge());
    }
    template <class Edge, class Graph>
    void back_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::hboost::on_back_edge());
    }
    template <class Edge, class Graph>
    void forward_or_cross_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::hboost::on_forward_or_cross_edge());
    }
    template <class Edge, class Graph>
    void finish_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::hboost::on_finish_edge());
    }
    template <class Vertex, class Graph>
    ...
    HBOOST_GRAPH_EVENT_STUB(on_initialize_vertex,dfs)
    HBOOST_GRAPH_EVENT_STUB(on_start_vertex,dfs)
    HBOOST_GRAPH_EVENT_STUB(on_discover_vertex,dfs)
    HBOOST_GRAPH_EVENT_STUB(on_examine_edge,dfs)
    HBOOST_GRAPH_EVENT_STUB(on_tree_edge,dfs)
    HBOOST_GRAPH_EVENT_STUB(on_back_edge,dfs)
    HBOOST_GRAPH_EVENT_STUB(on_forward_or_cross_edge,dfs)
    HBOOST_GRAPH_EVENT_STUB(on_finish_edge,dfs)
    HBOOST_GRAPH_EVENT_STUB(on_finish_vertex,dfs)
    ...
    namespace detail {
      ...
      struct depth_first_search_impl {
        ...
        void operator()(const Graph& g, const ArgPack& arg_pack) const {
          ...
          hboost::depth_first_search(g,
                                    arg_pack[_visitor | make_dfs_visitor(null_visitor())],
                                    hboost::detail::make_color_map_from_arg_pack(g, arg_pack),
                                    arg_pack[_root_vertex || hboost::detail::get_default_starting_vertex_t<Graph>(g)]);
        }
      };
    }
    HBOOST_GRAPH_MAKE_FORWARDING_FUNCTION(depth_first_search, 1, 4)
  }

  HBOOST_GRAPH_MAKE_OLD_STYLE_PARAMETER_FUNCTION(depth_first_search, 1)
...
#  include <hboost/graph/distributed/depth_first_search.hpp>
#endif
