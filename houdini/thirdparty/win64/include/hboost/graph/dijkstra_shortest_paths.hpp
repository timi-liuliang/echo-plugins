...
#ifdef HBOOST_GRAPH_DIJKSTRA_TESTING
#  include <hboost/pending/mutable_queue.hpp>
#endif // HBOOST_GRAPH_DIJKSTRA_TESTING
...
namespace hboost {
  ...
  namespace detail {
    ...
    struct dijkstra_bfs_visitor
    {
      ...
      void examine_edge(Edge e, Graph& g) {
        ...
        //    https://svn.boost.org/trac/hboost/ticket/8398).
        ...
        //    https://svn.boost.org/trac/hboost/ticket/9012).
        ...
        if (m_compare(m_combine(m_zero, get(m_weight, e)), m_zero)) 
            hboost::throw_exception(negative_edge());
  ...
  namespace detail {
    ...
    struct vertex_property_map_generator_helper<Graph, IndexMap, Value, true> {
      typedef hboost::iterator_property_map<Value*, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index, hboost::scoped_array<Value>& array_holder) {
    ...
    struct vertex_property_map_generator_helper<Graph, IndexMap, Value, false> {
      typedef hboost::vector_property_map<Value, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index, hboost::scoped_array<Value>& array_holder) {
        return hboost::make_vector_property_map<Value>(index);
      }
    };

    template <class Graph, class IndexMap, class Value>
    struct vertex_property_map_generator {
      typedef hboost::is_base_and_derived<
                hboost::vertex_list_graph_tag,
                typename hboost::graph_traits<Graph>::traversal_category>
              known_num_vertices;
  ...
  namespace detail {
    ...
    struct default_color_map_generator_helper<Graph, IndexMap, true> {
      typedef hboost::two_bit_color_map<IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        size_t nv = num_vertices(g);
        return hboost::two_bit_color_map<IndexMap>(nv, index);
      }
    };

    template <class Graph, class IndexMap>
    struct default_color_map_generator_helper<Graph, IndexMap, false> {
      typedef hboost::vector_property_map<hboost::two_bit_color_type, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        return hboost::make_vector_property_map<hboost::two_bit_color_type>(index);
      }
    };
...
#ifdef HBOOST_GRAPH_DIJKSTRA_USE_RELAXED_HEAP
    typedef relaxed_heap<Vertex, IndirectCmp, IndexMap> MutableQueue;
    MutableQueue Q(num_vertices(g), icmp, index_map);
#else // Now the default: use a d-ary heap
      hboost::scoped_array<std::size_t> index_in_heap_map_holder;
      typedef
  ...
  dijkstra_shortest_paths
    (const VertexListGraph& g,
     ...
     HBOOST_GRAPH_ENABLE_IF_MODELS_PARM(VertexListGraph,vertex_list_graph_tag))
  {
    hboost::two_bit_color_map<IndexMap> color(num_vertices(g), index_map);
...
#  include <hboost/graph/distributed/dijkstra_shortest_paths.hpp>
#endif

#endif // HBOOST_GRAPH_DIJKSTRA_HPP
