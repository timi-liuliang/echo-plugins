...
#ifndef HBOOST_GRAPH_BICONNECTED_COMPONENTS_HPP
#define HBOOST_GRAPH_BICONNECTED_COMPONENTS_HPP
...
namespace hboost
{
  namespace detail
  {
    ...
    struct biconnected_components_visitor : public dfs_visitor<>
    {
      ...
      {
        typename hboost::graph_traits<Graph>::vertex_descriptor src = source(e, g);
        typename hboost::graph_traits<Graph>::vertex_descriptor tgt = target(e, g);
      ...
      void back_edge(const Edge& e, Graph& g)
      {
        ...
        if ( tgt != get(pred, src) ) {
          S.push(e);
          put(lowpt, src,
              min HBOOST_PREVENT_MACRO_SUBSTITUTION(get(lowpt, src),
                                                   get(dtm, tgt)));
        }
        vis.back_edge(e, g);
      }
      ...
      void finish_vertex(const Vertex& u, Graph& g)
      {
        ...
        if (parent == u) { // Root of tree is special
          ...
          put(lowpt, parent,
              min HBOOST_PREVENT_MACRO_SUBSTITUTION(get(lowpt, parent),
                                                 get(lowpt, u)));
          if ( get(lowpt, u) >= get(dtm, parent) ) {
            ...
            HBOOST_ASSERT (source(S.top(), g) == parent);
            HBOOST_ASSERT (target(S.top(), g) == u);
            put(comp, S.top(), c);
            S.pop();
            ++c;
          }
        ...
        LowPointMap lowpt, PredecessorMap pred, DFSVisitor dfs_vis)
  {
    ...
    HBOOST_CONCEPT_ASSERT(( WritablePropertyMapConcept<ComponentMap, edge_t> ));
    HBOOST_CONCEPT_ASSERT(( ReadWritePropertyMapConcept<DiscoverTimeMap,
                                                  vertex_t> ));
    HBOOST_CONCEPT_ASSERT(( ReadWritePropertyMapConcept<LowPointMap, vertex_t > ));
    HBOOST_CONCEPT_ASSERT(( ReadWritePropertyMapConcept<PredecessorMap,
                                                  vertex_t> ));
...
#endif  /* HBOOST_GRAPH_BICONNECTED_COMPONENTS_HPP */
