...
#ifndef HBOOST_GRAPH_TRAITS_HPP
#define HBOOST_GRAPH_TRAITS_HPP
...
namespace hboost {
    ...
    namespace detail {
#define HBOOST_GRAPH_MEMBER_OR_VOID(name) \
      HBOOST_MPL_HAS_XXX_TRAIT_DEF(name) \
      template <typename T> struct HBOOST_JOIN(get_member_, name) {typedef typename T::name type;}; \
      template <typename T> struct HBOOST_JOIN(get_opt_member_, name): \
        hboost::mpl::eval_if_c< \
          HBOOST_JOIN(has_, name)<T>::value, \
          HBOOST_JOIN(get_member_, name)<T>, \
          hboost::mpl::identity<void> > \
        {};
      HBOOST_GRAPH_MEMBER_OR_VOID(adjacency_iterator)
      HBOOST_GRAPH_MEMBER_OR_VOID(out_edge_iterator)
      HBOOST_GRAPH_MEMBER_OR_VOID(in_edge_iterator)
      HBOOST_GRAPH_MEMBER_OR_VOID(vertex_iterator)
      HBOOST_GRAPH_MEMBER_OR_VOID(edge_iterator)
      HBOOST_GRAPH_MEMBER_OR_VOID(vertices_size_type)
      HBOOST_GRAPH_MEMBER_OR_VOID(edges_size_type)
      HBOOST_GRAPH_MEMBER_OR_VOID(degree_size_type)
    }

    template <typename G>
    struct graph_traits {
#define HBOOST_GRAPH_PULL_OPT_MEMBER(name) \
        typedef typename detail::HBOOST_JOIN(get_opt_member_, name)<G>::type name;

        typedef typename G::vertex_descriptor      vertex_descriptor;
        typedef typename G::edge_descriptor        edge_descriptor;
        HBOOST_GRAPH_PULL_OPT_MEMBER(adjacency_iterator)
        HBOOST_GRAPH_PULL_OPT_MEMBER(out_edge_iterator)
        HBOOST_GRAPH_PULL_OPT_MEMBER(in_edge_iterator)
        HBOOST_GRAPH_PULL_OPT_MEMBER(vertex_iterator)
        HBOOST_GRAPH_PULL_OPT_MEMBER(edge_iterator)

        typedef typename G::directed_category      directed_category;
        typedef typename G::edge_parallel_category edge_parallel_category;
        typedef typename G::traversal_category     traversal_category;

        HBOOST_GRAPH_PULL_OPT_MEMBER(vertices_size_type)
        HBOOST_GRAPH_PULL_OPT_MEMBER(edges_size_type)
        HBOOST_GRAPH_PULL_OPT_MEMBER(degree_size_type)
#undef HBOOST_GRAPH_PULL_OPT_MEMBER
...
#define HBOOST_GRAPH_SEQUENTIAL_TRAITS_DEFINES_DISTRIBUTED_TAGS // Disable these from external versions of PBGL
    ...
    typedef hboost::forward_traversal_tag multi_pass_input_iterator_tag;

    namespace detail {
      HBOOST_MPL_HAS_XXX_TRAIT_DEF(graph_property_type)
      HBOOST_MPL_HAS_XXX_TRAIT_DEF(edge_property_type)
      HBOOST_MPL_HAS_XXX_TRAIT_DEF(vertex_property_type)
    ...
    struct graph_property_type
      : hboost::mpl::eval_if<detail::has_graph_property_type<G>,
                            detail::get_graph_property_type<G>,
                            no_property> {};
    template <typename G>
    struct edge_property_type
      : hboost::mpl::eval_if<detail::has_edge_property_type<G>,
                            detail::get_edge_property_type<G>,
                            no_property> {};
    template <typename G>
    struct vertex_property_type
      : hboost::mpl::eval_if<detail::has_vertex_property_type<G>,
