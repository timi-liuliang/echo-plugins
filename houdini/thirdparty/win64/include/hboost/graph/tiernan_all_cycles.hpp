...
#ifndef HBOOST_GRAPH_CYCLE_HPP
#define HBOOST_GRAPH_CYCLE_HPP
...
namespace hboost {
    namespace concepts {
        HBOOST_concept(CycleVisitor,(Visitor)(Path)(Graph))
        {
            HBOOST_CONCEPT_USAGE(CycleVisitor)
            {
                vis.cycle(p, g);
            }
        private:
...
struct min_max_cycle_visitor
{
    ...
    inline void cycle(const Path& p, const Graph& g)
    {
        ...
        minimum = min HBOOST_PREVENT_MACRO_SUBSTITUTION (minimum, len);
        maximum = max HBOOST_PREVENT_MACRO_SUBSTITUTION (maximum, len);
    }
    std::size_t& minimum;
    std::size_t& maximum;
};
...
namespace detail
{
                    ...
                    const ClosedMatrix& m)
    {
        ...
        HBOOST_CONCEPT_ASSERT(( VertexIndexGraphConcept<Graph> ));
    ...
    can_wrap_path(const Graph& g, const Path& p)
    {
        ...
        Vertex
            ...
            v = p.front();
        OutIterator i, end;
        for(hboost::tie(i, end) = out_edges(u, g); i != end; ++i) {
            if((target(*i, g) == v)) {
                return true;
            }
        }
                            ...
                            std::size_t maxlen)
    {
        ...
        HBOOST_CONCEPT_ASSERT(( CycleVisitorConcept<Visitor,Path,Graph> ));
