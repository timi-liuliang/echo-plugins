...
//=======================================================================
#ifndef HBOOST_GRAPH_SEQUENTIAL_VERTEX_COLORING_HPP
#define HBOOST_GRAPH_SEQUENTIAL_VERTEX_COLORING_HPP
...
#ifdef HBOOST_NO_TEMPLATED_ITERATOR_CONSTRUCTORS
#  include <iterator>
#endif
...
namespace hboost {
                             ...
                             ColorMap color)
  {
    ...
    std::vector<size_type> mark(V, 
                                std::numeric_limits<size_type>::max HBOOST_PREVENT_MACRO_SUBSTITUTION());
    ...
    for ( size_type i = 0; i < V; i++) {
      ...
      for (hboost::tie(v,vend) = adjacent_vertices(current, G); v != vend; ++v)
        mark[get(color,*v)] = i; 
