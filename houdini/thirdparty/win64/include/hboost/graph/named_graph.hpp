...
#ifndef HBOOST_GRAPH_NAMED_GRAPH_HPP
#define HBOOST_GRAPH_NAMED_GRAPH_HPP
...
namespace hboost { namespace graph {
...
public:
  ...
  {
      hboost::throw_exception(std::runtime_error("add_vertex: "
...
{
  HBOOST_STATIC_ASSERT_MSG ((hboost::is_base_of<hboost::graph_detail::stable_tag, VertexIterStability>::value), "Named graphs cannot use vecS as vertex container and remove vertices; the lack of vertex descriptor stability (which iterator stability is a proxy for) means that the name -> vertex mapping would need to be completely rebuilt after each deletion.  See https://svn.boost.org/trac/hboost/ticket/7863 for more information and a test case.");
...
#endif // HBOOST_GRAPH_NAMED_GRAPH_HPP
