...
#ifndef HBOOST_GRAPH_SLOAN_HPP
#define HBOOST_GRAPH_SLOAN_HPP
...
#include <hboost/graph/cuthill_mckee_ordering.hpp>
...
namespace hboost {
                           ...
                           DegreeMap degree)
  {
    ...
    hboost::iterator_property_map<vec_iter, VertexID, size_type, size_type&> dist_pmap(dist.begin(), get(vertex_index, G));
      ...
      *permutation++ = u;                      //Puts u to the first position in the permutation-vector
      ...
      for (hboost::tie(ei, ei_end) = out_edges(u, g); ei != ei_end; ++ei) {
        ...
        if (get(color, v) == Color::green() ) {      //tests if the vertex is inactive
          ...
          for (hboost::tie(ei2, ei2_end) = out_edges(v, g); ei2 != ei2_end; ++ei2) {
            w = target(*ei2, g);
  ...
  sloan_ordering(Graph& G, 
                 ...
                 Weight W2 )
  {
    typedef typename hboost::graph_traits<Graph>::vertex_descriptor Vertex;
