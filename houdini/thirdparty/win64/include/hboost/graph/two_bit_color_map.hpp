...
#ifndef HBOOST_TWO_BIT_COLOR_MAP_HPP
#define HBOOST_TWO_BIT_COLOR_MAP_HPP
...
struct two_bit_color_map 
{
  ...
  HBOOST_STATIC_CONSTANT(int, bits_per_char = std::numeric_limits<unsigned char>::digits);
  HBOOST_STATIC_CONSTANT(int, elements_per_char = bits_per_char / 2);
...
{
  HBOOST_STATIC_CONSTANT(int, elements_per_char = two_bit_color_map<IndexMap>::elements_per_char);
  typename property_traits<IndexMap>::value_type i = get(pm.index, key);
  HBOOST_ASSERT ((std::size_t)i < pm.n);
...
#endif // HBOOST_TWO_BIT_COLOR_MAP_HPP

#ifdef HBOOST_GRAPH_USE_MPI
#  include <hboost/graph/distributed/two_bit_color_map.hpp>
#endif
