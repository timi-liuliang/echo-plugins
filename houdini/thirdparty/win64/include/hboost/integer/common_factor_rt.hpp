...
#ifndef HBOOST_INTEGER_COMMON_FACTOR_RT_HPP
#define HBOOST_INTEGER_COMMON_FACTOR_RT_HPP

#include <hboost/integer_fwd.hpp>  // self include

#include <hboost/config.hpp>  // for HBOOST_NESTED_TEMPLATE, etc.
...
#else // HBOOST_NO_LIMITS_COMPILE_TIME_CONSTANTS
    template < typename T >
    struct gcd_optimal_evaluator
    {
        T  operator ()( T const &a, T const &b )
...
#define HBOOST_PRIVATE_GCD_UF( Ut )                  \
    template < >  struct gcd_optimal_evaluator<Ut>  \
    {  Ut  operator ()( Ut a, Ut b ) const  { return gcd_binary( a, b ); }  }

    HBOOST_PRIVATE_GCD_UF( unsigned char );
    HBOOST_PRIVATE_GCD_UF( unsigned short );
    HBOOST_PRIVATE_GCD_UF( unsigned );
    HBOOST_PRIVATE_GCD_UF( unsigned long );

#ifdef HBOOST_HAS_LONG_LONG
    HBOOST_PRIVATE_GCD_UF( hboost::ulong_long_type );
#elif defined(HBOOST_HAS_MS_INT64)
    HBOOST_PRIVATE_GCD_UF( unsigned __int64 );
#endif

#if CHAR_MIN == 0
    HBOOST_PRIVATE_GCD_UF( char ); // char is unsigned
#endif

#undef HBOOST_PRIVATE_GCD_UF

#define HBOOST_PRIVATE_GCD_SF( St, Ut )                            \
    ...
    HBOOST_PRIVATE_GCD_SF( signed char, unsigned char );
    HBOOST_PRIVATE_GCD_SF( short, unsigned short );
    HBOOST_PRIVATE_GCD_SF( int, unsigned );
    HBOOST_PRIVATE_GCD_SF( long, unsigned long );

#if CHAR_MIN < 0
    HBOOST_PRIVATE_GCD_SF( char, unsigned char ); // char is signed
#endif

#ifdef HBOOST_HAS_LONG_LONG
    HBOOST_PRIVATE_GCD_SF( hboost::long_long_type, hboost::ulong_long_type );
#elif defined(HBOOST_HAS_MS_INT64)
    HBOOST_PRIVATE_GCD_SF( __int64, unsigned __int64 );
#endif

#undef HBOOST_PRIVATE_GCD_SF
...
#endif  // HBOOST_INTEGER_COMMON_FACTOR_RT_HPP
