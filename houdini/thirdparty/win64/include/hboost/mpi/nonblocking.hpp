 ...
 */
#ifndef HBOOST_MPI_NONBLOCKING_HPP
#define HBOOST_MPI_NONBLOCKING_HPP
  ...
  while (true) {
    ...
    if (++current == last) {
      ...
      if (all_trivial_requests) {
        ...
        int index;
        status stat;
        HBOOST_MPI_CHECK_RESULT(MPI_Waitany, 
                               (n, &requests[0], &index, &stat.m_status));

        // We don't have a notion of empty requests or status objects,
        // so this is an error.
        if (index == MPI_UNDEFINED)
          hboost::throw_exception(exception("MPI_Waitany", MPI_ERR_REQUEST));
...
wait_all(ForwardIterator first, ForwardIterator last, OutputIterator out)
{
  ...
  while (num_outstanding_requests > 0) {
    ...
    if (all_trivial_requests 
      ...
      HBOOST_MPI_CHECK_RESULT(MPI_Waitall, 
...
test_all(ForwardIterator first, ForwardIterator last, OutputIterator out)
{
  ...
  HBOOST_MPI_CHECK_RESULT(MPI_Testall, (n, &requests[0], &flag, &stats[0]));
  if (flag) {
...
test_all(ForwardIterator first, ForwardIterator last)
{
  ...
  int flag = 0;
  int n = requests.size();
  HBOOST_MPI_CHECK_RESULT(MPI_Testall, 
  ...
  while (true) {
    ...
    if (++current == start_of_completed) {
      ...
      if (all_trivial_requests) {
        ...
        int num_completed;
        HBOOST_MPI_CHECK_RESULT(MPI_Waitsome, 
...
#endif // HBOOST_MPI_NONBLOCKING_HPP
