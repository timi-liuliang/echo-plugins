 ...
 */
#ifndef HBOOST_MPI_PYTHON_SERIALIZE_HPP
#define HBOOST_MPI_PYTHON_SERIALIZE_HPP

#include <hboost/mpi/python/config.hpp>
...
#define HBOOST_MPI_PYTHON_FORWARD_ONLY
#include <hboost/mpi/python.hpp>
 ...
 * HBOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE instead.
 */
#  define HBOOST_PYTHON_SERIALIZATION_ARCHIVE(IArchiver, OArchiver)        \
 ...
 * HBOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE_IMPL(IArchiver,
 * OArchiver) in one of your translation units.

DPG PICK UP HERE
 */
#define HBOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE(IArchiver, OArchiver) \
HBOOST_PYTHON_SERIALIZATION_ARCHIVE(IArchiver, OArchiver)                \
namespace hboost { namespace python { namespace detail {                 \
template<>                                                              \
HBOOST_MPI_PYTHON_DECL direct_serialization_table< IArchiver , OArchiver >& \
 ...
 * HBOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE(IArchiver, OArchiver).
 */
#define HBOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE_IMPL(IArchiver, OArchiver) \
namespace hboost { namespace python { namespace detail {                 \
template                                                                \
  class HBOOST_MPI_PYTHON_DECL direct_serialization_table< IArchiver , OArchiver >; \
                                                                        \
template<>                                                              \
 HBOOST_MPI_PYTHON_DECL                                                  \
 ...
 */
class HBOOST_MPI_PYTHON_DECL pickle {
  struct data_t;
 ...
 * HBOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE.
 */
...
namespace detail {
   ...
   */
  template<typename IArchiver, typename OArchiver>
  class HBOOST_MPI_PYTHON_DECL direct_serialization_table
  {
  public:
    typedef hboost::function3<void, OArchiver&, const object&, const unsigned int>
      saver_t;
    typedef hboost::function3<void, IArchiver&, object&, const unsigned int>
      loader_t;
   ...
   * HBOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE_IMPL to define this
...
void 
save_impl(Archiver& ar, const hboost::python::object& obj, 
          const unsigned int /*version*/,
          mpl::false_ /*has_direct_serialization*/)
{
  hboost::python::str py_string = hboost::python::pickle::dumps(obj);
  int len = hboost::python::extract<int>(py_string.attr("__len__")());
  const char* string = hboost::python::extract<const char*>(py_string);
  ar << len << hboost::serialization::make_array(string, len);
}
...
template<typename Archiver>
void 
load_impl(Archiver& ar, hboost::python::object& obj, 
          const unsigned int /*version*/, 
          mpl::false_ /*has_direct_serialization*/)
{
  int len;
  ...
  ar >> hboost::serialization::make_array(string.get(), len);
  hboost::python::str py_string(string.get(), len);
  obj = hboost::python::pickle::loads(py_string);
}
...
template<typename Archiver>
void 
save(Archiver& ar, const hboost::python::object& obj, 
     const unsigned int version)
{
  typedef Archiver OArchiver;
  typedef typename input_archiver<OArchiver>::type IArchiver;
...
template<typename Archiver>
void 
load(Archiver& ar, hboost::python::object& obj, 
     const unsigned int version)
{
  typedef Archiver IArchiver;
  typedef typename output_archiver<IArchiver>::type OArchiver;
...
inline void 
serialize(Archive& ar, hboost::python::object& obj, const unsigned int version)
{
  hboost::serialization::split_free(ar, obj, version);
}

} } // end namespace hboost::python
...
HBOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE(
  ::hboost::mpi::packed_iarchive,
  ::hboost::mpi::packed_oarchive)

namespace hboost { namespace mpi { namespace python {

template<typename T>
void
register_serialized(const T& value, PyTypeObject* type)
{
  using hboost::python::register_serialized;
  register_serialized<packed_iarchive, packed_oarchive>(value, type);
}

} } } // end namespace hboost::mpi::python

#endif // HBOOST_MPI_PYTHON_SERIALIZE_HPP
