...
#ifndef HBOOST_MPI_SCATTER_HPP
#define HBOOST_MPI_SCATTER_HPP
...
namespace detail {
               ...
               int n, int root, mpl::true_)
  {
    ...
    HBOOST_MPI_CHECK_RESULT(MPI_Scatter,
...
{
  if (comm.rank() == root)
    ::hboost::mpi::scatter<T>(comm, &in_values[0], out_value, root);
  else
    ::hboost::mpi::scatter<T>(comm, static_cast<const T*>(0), out_value, 
                             root);
}
...
{
  if (comm.rank() == root)
    ::hboost::mpi::scatter(comm, &in_values[0], out_values, n, root);
  else
    ::hboost::mpi::scatter(comm, static_cast<const T*>(0), out_values, 
                                    n, root);
}
