...
#ifndef HBOOST_MPI_SCATTERV_HPP
#define HBOOST_MPI_SCATTERV_HPP
...
namespace detail {
                ...
                const int* displs, T* out_values, int out_size, int root, mpl::true_)
  {
    ...
    HBOOST_MPI_CHECK_RESULT(MPI_Scatterv,
...
{
  if (comm.rank() == root)
    ::hboost::mpi::scatterv(comm, &in_values[0], sizes, displs,
                           out_values, out_size, root);
  else
    ::hboost::mpi::scatterv(comm, static_cast<const T*>(0), sizes, displs,
                           out_values, out_size, root);
}
         ...
         const std::vector<int>& sizes, T* out_values, int root)
{
  ...
  ::hboost::mpi::scatterv(comm, in_values, sizes, displs, out_values,
                         sizes[myrank], root);
}
...
scatterv(const communicator& comm, const std::vector<T>& in_values,
         const std::vector<int>& sizes, T* out_values, int root)
{
  ::hboost::mpi::scatterv(comm, &in_values[0], sizes, out_values, root);
}

} } // end namespace hboost::mpi

#endif // HBOOST_MPI_SCATTERV_HPP
