...
#ifndef HBOOST_MPI_BROADCAST_HPP
#define HBOOST_MPI_BROADCAST_HPP
...
namespace detail {
  ...
  broadcast_impl(const communicator& comm, T* values, int n, int root, 
                 mpl::true_)
  {
    HBOOST_MPI_CHECK_RESULT(MPI_Bcast,
                           (values, n,
                            hboost::mpi::get_mpi_datatype<T>(*values),
                            root, MPI_Comm(comm)));
  }
...
#endif // HBOOST_MPI_BROADCAST_HPP
