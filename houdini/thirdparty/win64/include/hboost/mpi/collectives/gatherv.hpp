...
#ifndef HBOOST_MPI_GATHERV_HPP
#define HBOOST_MPI_GATHERV_HPP
...
namespace detail {
               ...
               T* out_values, const int* sizes, const int* displs, int root, mpl::true_)
  {
    ...
    HBOOST_MPI_CHECK_RESULT(MPI_Gatherv,
...
gatherv(const communicator& comm, const std::vector<T>& in_values,
        ...
        int root)
{
  ::hboost::mpi::gatherv(comm, &in_values[0], in_values.size(), out_values, sizes, displs, root);
}
        ...
        T* out_values, const std::vector<int>& sizes, int root)
{
  ...
  ::hboost::mpi::gatherv(comm, in_values, in_size, out_values, sizes, displs, root);
}
...
gatherv(const communicator& comm, const std::vector<T>& in_values,
        T* out_values, const std::vector<int>& sizes, int root)
{
  ::hboost::mpi::gatherv(comm, &in_values[0], in_values.size(), out_values, sizes, root);
}

} } // end namespace hboost::mpi

#endif // HBOOST_MPI_GATHERV_HPP
