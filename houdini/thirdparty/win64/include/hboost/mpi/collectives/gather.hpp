...
#ifndef HBOOST_MPI_GATHER_HPP
#define HBOOST_MPI_GATHER_HPP
...
namespace detail {
              ...
              T* out_values, int root, mpl::true_)
  {
    ...
    HBOOST_MPI_CHECK_RESULT(MPI_Gather,
       ...
       int root)
{
  if (comm.rank() == root) {
    ...
    ::hboost::mpi::gather(comm, in_value, &out_values[0], root);
  } else {
    ::hboost::mpi::gather(comm, in_value, root);
  }
}
       ...
       std::vector<T>& out_values, int root)
{
  if (comm.rank() == root) {
    ...
    ::hboost::mpi::gather(comm, in_values, n, &out_values[0], root);
  } 
  else
    ::hboost::mpi::gather(comm, in_values, n, root);
}
