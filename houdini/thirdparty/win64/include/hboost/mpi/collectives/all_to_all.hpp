...
#ifndef HBOOST_MPI_ALL_TO_ALL_HPP
#define HBOOST_MPI_ALL_TO_ALL_HPP
...
#include <hboost/mpi/collectives_fwd.hpp>
...
namespace detail {
                  ...
                  T* out_values, mpl::true_)
  {
    ...
    HBOOST_MPI_CHECK_RESULT(MPI_Alltoall,
                  ...
                  T* out_values, mpl::false_)
  {
    ...
    HBOOST_MPI_CHECK_RESULT(MPI_Alltoallv,
    ...
    for (int src = 0; src < size; ++src) {
      if (src == rank) 
        ...
        packed_iarchive ia(comm, incoming, hboost::archive::no_header,
                           recv_disps[src]);
        for (int i = 0; i < n; ++i)
...
{
  HBOOST_ASSERT((int)in_values.size() == comm.size());
  out_values.resize(comm.size());
  ::hboost::mpi::all_to_all(comm, &in_values[0], &out_values[0]);
}
...
all_to_all(const communicator& comm, const std::vector<T>& in_values, int n,
           std::vector<T>& out_values)
{
  HBOOST_ASSERT((int)in_values.size() == comm.size() * n);
  out_values.resize(comm.size() * n);
  ::hboost::mpi::all_to_all(comm, &in_values[0], n, &out_values[0]);
}

} } // end namespace hboost::mpi

#endif // HBOOST_MPI_ALL_TO_ALL_HPP
