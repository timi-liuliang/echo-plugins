 ...
 */
#ifndef HBOOST_MPI_GROUP_HPP
#define HBOOST_MPI_GROUP_HPP
...
protected:
  ...
  struct group_free
  {
    void operator()(MPI_Group* comm) const
    {
      ...
      if (!finalized)
        HBOOST_MPI_CHECK_RESULT(MPI_Group_free, (comm));
      delete comm;
    }
  };
 ...
 */
HBOOST_MPI_DECL bool operator==(const group& g1, const group& g2);
 ...
 */
HBOOST_MPI_DECL group operator|(const group& g1, const group& g2);
                       ...
                       const group& to_group, OutputIterator out)
{
  ...
  HBOOST_MPI_CHECK_RESULT(MPI_Group_translate_ranks,
                         ((MPI_Group)*this,
                          in_array.size(),
                          &in_array[0],
                          (MPI_Group)to_group,
...
group group::include(InputIterator first, InputIterator last)
{
  ...
  MPI_Group result;
  HBOOST_MPI_CHECK_RESULT(MPI_Group_incl,
 ...
 * group.
 */
template<> HBOOST_MPI_DECL group group::include(int* first, int* last);
...
group group::exclude(InputIterator first, InputIterator last)
{
  ...
  std::vector<int> ranks(first, last);
  MPI_Group result;
  HBOOST_MPI_CHECK_RESULT(MPI_Group_excl,
                         ((MPI_Group)*this, ranks.size(), &ranks[0], &result));
  return group(result, /*adopt=*/true);
}
 ...
 * group.
 */
template<> HBOOST_MPI_DECL group group::exclude(int* first, int* last);

} } // end namespace hboost::mpi

#endif // HBOOST_MPI_GROUP_HPP
