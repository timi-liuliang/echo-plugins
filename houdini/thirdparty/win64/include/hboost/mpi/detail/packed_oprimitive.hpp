...
#ifndef HBOOST_MPI_PACKED_OPRIMITIVE_HPP
#define HBOOST_MPI_PACKED_OPRIMITIVE_HPP
...
class HBOOST_MPI_DECL packed_oprimitive
{
public:
...
private:
    ...
    void save_impl(void const * p, MPI_Datatype t, int l)
    {
      ...
      int memory_needed;
      HBOOST_MPI_CHECK_RESULT(MPI_Pack_size,(l,t,comm,&memory_needed));

      int position = buffer_.size();
      buffer_.resize(position + memory_needed);

      // pack the data into the buffer
      HBOOST_MPI_CHECK_RESULT(MPI_Pack,
      (const_cast<void*>(p), l, t, hboost::serialization::detail::get_data(buffer_), buffer_.size(), &position, comm));
      // reduce the buffer size if needed
      HBOOST_ASSERT(std::size_t(position) <= buffer_.size());
...
#endif // HBOOST_MPI_PACKED_OPRIMITIVE_HPP
