 ...
 */
#ifndef HBOOST_MPI_COMMUNICATOR_HPP
#define HBOOST_MPI_COMMUNICATOR_HPP
...
#include <hboost/mpi/skeleton_and_content_fwd.hpp>

// For (de-)serializing arrays
#include <hboost/serialization/array.hpp>

#include <hboost/mpi/detail/point_to_point.hpp>
#include <hboost/mpi/status.hpp>
#include <hboost/mpi/request.hpp>
 ...
 * in the communicator.
 */
class HBOOST_MPI_DECL communicator
{
 public:
  /**
   ...
   */
  communicator(const communicator& comm, const hboost::mpi::group& subgroup);
 ...
 protected:
  ...
  struct comm_free
  {
    ...
    {
      HBOOST_ASSERT( comm != 0 );
      HBOOST_ASSERT(*comm != MPI_COMM_NULL);
      int finalized;
      HBOOST_MPI_CHECK_RESULT(MPI_Finalized, (&finalized));
      if (!finalized)
        HBOOST_MPI_CHECK_RESULT(MPI_Comm_free, (comm));
      delete comm;
    }
  };
 ...
 */
HBOOST_MPI_DECL bool operator==(const communicator& comm1, const communicator& comm2);
...
optional<int> status::count_impl(mpl::true_) const
{
  ...
  int return_value;
  HBOOST_MPI_CHECK_RESULT(MPI_Get_count,
...
{
  HBOOST_MPI_CHECK_RESULT(MPI_Send,
...
{
  packed_oarchive oa(*this);
  oa << n << hboost::serialization::make_array(values, n);
  send(dest, tag, oa);
}
...
status communicator::recv_impl(int source, int tag, T& value, mpl::true_) const
{
  ...
  HBOOST_MPI_CHECK_RESULT(MPI_Recv,
                              ...
                              mpl::false_) const
{
  ...
  // Deserialize the data in the message
  hboost::serialization::array_wrapper<T> arr(values, count > n? n : count);
  ia >> arr;
...
{
  status stat;
  HBOOST_MPI_CHECK_RESULT(MPI_Sendrecv,
                         (const_cast<T*>(&sval), 1, 
...
{
  request req;
  HBOOST_MPI_CHECK_RESULT(MPI_Isend,
                         (const_cast<T*>(&value), 1, 
                          get_mpi_datatype<T>(value),
...
{
  shared_ptr<packed_oarchive> archive(new packed_oarchive(*this));
  *archive << n << hboost::serialization::make_array(values, n);
  request result = isend(dest, tag, *archive);
  result.m_data = archive;
  return result;
}
...
request::handle_serialized_irecv(request* self, request_action action)
{
  ...
  if (action == ra_wait) {
    ...
    if (self->m_requests[1] == MPI_REQUEST_NULL) {
      ...
      HBOOST_MPI_CHECK_RESULT(MPI_Test,
...
#ifdef HBOOST_MPI_SKELETON_AND_CONTENT_HPP
...
#endif // HBOOST_MPI_COMMUNICATOR_HPP
