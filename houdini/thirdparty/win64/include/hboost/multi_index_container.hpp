...
#ifndef HBOOST_MULTI_INDEX_HPP
#define HBOOST_MULTI_INDEX_HPP
...
#include <hboost/multi_index/detail/adl_swap.hpp>
#include <hboost/multi_index/detail/base_type.hpp>
#include <hboost/multi_index/detail/do_not_copy_elements_tag.hpp>
#include <hboost/multi_index/detail/converter.hpp>
#include <hboost/multi_index/detail/header_holder.hpp>
#include <hboost/multi_index/detail/has_tag.hpp>
...
#include <hboost/multi_index/detail/archive_constructed.hpp>
#include <hboost/multi_index/detail/serialization_version.hpp>
...
#include <hboost/multi_index/detail/invariant_assert.hpp>
#define HBOOST_MULTI_INDEX_CHECK_INVARIANT_OF(x)                              \
...
#define HBOOST_MULTI_INDEX_CHECK_INVARIANT                                    \
  HBOOST_MULTI_INDEX_CHECK_INVARIANT_OF(*this)
#else
#define HBOOST_MULTI_INDEX_CHECK_INVARIANT_OF(x)
#define HBOOST_MULTI_INDEX_CHECK_INVARIANT
#endif
...
class multi_index_container:
  private ::hboost::base_from_member<
    typename hboost::detail::allocator::rebind_to<
      Allocator,
      typename detail::multi_index_node_type<
        Value,IndexSpecifierList,Allocator>::type
    >::type>,
  HBOOST_MULTI_INDEX_PRIVATE_IF_MEMBER_TEMPLATE_FRIENDS detail::header_holder<
    typename hboost::detail::allocator::rebind_to<
      Allocator,
      typename detail::multi_index_node_type<
        Value,IndexSpecifierList,Allocator>::type
...
private:
  HBOOST_COPYABLE_AND_MOVABLE(multi_index_container)
  ...
  typedef typename
  hboost::detail::allocator::rebind_to<
    Allocator,
    typename super::node_type
  >::type                                         node_allocator;
  typedef ::hboost::base_from_member<
...
public:
  ...
  explicit multi_index_container(
    ...
    node_count(0)
  {
    HBOOST_MULTI_INDEX_CHECK_INVARIANT;
  }
  ...
  multi_index_container(HBOOST_RV_REF(multi_index_container) x):
    bfm_allocator(x.bfm_allocator::member),
    bfm_header(),
  ...
  {
    HBOOST_MULTI_INDEX_CHECK_INVARIANT;
    HBOOST_MULTI_INDEX_CHECK_INVARIANT_OF(x);
    swap_elements_(x);
  }
  ...
  multi_index_container<Value,IndexSpecifierList,Allocator>& operator=(
    HBOOST_COPY_ASSIGN_REF(multi_index_container) x)
  {
    multi_index_container y(x);
    this->swap(y);
    return *this;
  ...
  multi_index_container<Value,IndexSpecifierList,Allocator>& operator=(
    HBOOST_RV_REF(multi_index_container) x)
  {
    this->swap(x);
    return *this;
  }
  ...
  struct nth_index
  {
    HBOOST_STATIC_ASSERT(N>=0&&N<mpl::size<index_type_list>::type::value);
    typedef typename mpl::at_c<index_type_list,N>::type type;
  };

  template<int N>
  typename nth_index<N>::type& get()HBOOST_NOEXCEPT
  {
    HBOOST_STATIC_ASSERT(N>=0&&N<mpl::size<index_type_list>::type::value);
    return *this;
  }
  ...
  template<int N>
  const typename nth_index<N>::type& get()const HBOOST_NOEXCEPT
  {
    HBOOST_STATIC_ASSERT(N>=0&&N<mpl::size<index_type_list>::type::value);
    return *this;
  }
  ...
  struct index
  {
    ...
    HBOOST_STATIC_ASSERT(index_found);

    typedef typename mpl::deref<iter>::type    type;
  };

  template<typename Tag>
  typename index<Tag>::type& get()HBOOST_NOEXCEPT
  {
    return *this;
  }

  template<typename Tag>
  const typename index<Tag>::type& get()const HBOOST_NOEXCEPT
  {
    return *this;
  }
#endif
  ...
  typename nth_index_iterator<N>::type project(IteratorType it)
  {
    ...
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(it);
    HBOOST_MULTI_INDEX_CHECK_IS_OWNER(
...
HBOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS:
    ...
    HBOOST_TRY{
      ...
      HBOOST_TRY{
        ...
        if(res==x){
          ...
          hboost::detail::allocator::destroy(&x->value());
          deallocate_node(x);
          return std::pair<node_type*,bool>(res,false);
        }
      }
    ...
    HBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    ...
    HBOOST_TRY{
      detail::vartempl_placement_new(
        &x->value(),HBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
      HBOOST_TRY{
        node_type* res=super::insert_(x->value(),x,detail::emplaced_tag());
        if(res==x){
          ++node_count;
  ...
  {
    HBOOST_MULTI_INDEX_INVARIANT_ASSERT(invariant_());
  }
#endif
...
get(
  multi_index_container<Value,IndexSpecifierList,Allocator>& m)HBOOST_NOEXCEPT
{
  ...
  HBOOST_STATIC_ASSERT(N>=0&&
    N<
    mpl::size<
      HBOOST_DEDUCED_TYPENAME multi_index_type::index_type_list
    >::type::value);
...
>
typename ::hboost::multi_index::index<
  ...
  multi_index_container<Value,IndexSpecifierList,Allocator>& m)HBOOST_NOEXCEPT
{
  typedef multi_index_container<
    Value,IndexSpecifierList,Allocator>         multi_index_type;
  typedef typename ::hboost::multi_index::index<
    multi_index_container<
      Value,IndexSpecifierList,Allocator>,
    Tag
  >::type                                       index_type;
...
template<
  typename Tag,typename Value,typename IndexSpecifierList,typename Allocator
>
const typename ::hboost::multi_index::index<
  multi_index_container<Value,IndexSpecifierList,Allocator>,Tag>::type&
get(
  const multi_index_container<Value,IndexSpecifierList,Allocator>& m
)HBOOST_NOEXCEPT
  ...
  IteratorType it)
{
  ...
  HBOOST_STATIC_ASSERT((
    mpl::contains<
      HBOOST_DEDUCED_TYPENAME multi_index_type::iterator_type_list,
      IteratorType>::value));
#endif
  ...
  typedef detail::converter<
    multi_index_type,
    HBOOST_DEDUCED_TYPENAME IteratorType::container_type> converter;
  HBOOST_MULTI_INDEX_CHECK_IS_OWNER(it,converter::index(m));
#endif
  ...
  IteratorType it)
{
  ...
  HBOOST_STATIC_ASSERT((
    mpl::contains<
      ...
      IteratorType>::value||
    mpl::contains<
      HBOOST_DEDUCED_TYPENAME multi_index_type::const_iterator_type_list,
      IteratorType>::value));
#endif
 ...
 * hboost::serialization::collection_size_type.
...
template<typename Value,typename IndexSpecifierList,typename Allocator>
struct version<
  hboost::multi_index_container<Value,IndexSpecifierList,Allocator>
>
{
...
/* Associated global functions are promoted to namespace hboost, except
...
#undef HBOOST_MULTI_INDEX_CHECK_INVARIANT
#undef HBOOST_MULTI_INDEX_CHECK_INVARIANT_OF
