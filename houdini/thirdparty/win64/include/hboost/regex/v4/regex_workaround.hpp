...
#define HBOOST_REGEX_WORKAROUND_HPP
...
#ifdef HBOOST_REGEX_NO_BOOL
#  define HBOOST_REGEX_MAKE_BOOL(x) static_cast<bool>((x) ? true : false)
#else
#  define HBOOST_REGEX_MAKE_BOOL(x) static_cast<bool>(x)
#endif
...
#ifdef __cplusplus
namespace hboost{ namespace HBOOST_REGEX_DETAIL_NS{
#if HBOOST_WORKAROUND(HBOOST_MSVC,>=1400) && HBOOST_WORKAROUND(HBOOST_MSVC, <1600) && defined(_CPPLIB_VER) && defined(HBOOST_DINKUMWARE_STDLIB) && !(defined(__SGI_STL_PORT) || defined(_STLPORT_VERSION))
   //
...
#elif HBOOST_WORKAROUND(HBOOST_MSVC, > 1500)
   //
   ...
   using std::equal; 
#endif 
#if HBOOST_WORKAROUND(HBOOST_MSVC,>=1400) && defined(__STDC_WANT_SECURE_LIB__) && __STDC_WANT_SECURE_LIB__ 
