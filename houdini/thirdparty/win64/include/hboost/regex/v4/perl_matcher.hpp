...
#define HBOOST_REGEX_MATCHER_HPP

#include <hboost/regex/v4/iterator_category.hpp>
...
// error checking API:
//
HBOOST_REGEX_DECL void HBOOST_REGEX_CALL verify_options(hboost::regex_constants::syntax_option_type ef, match_flag_type mf);
//
// function can_start:
//
...
#if defined(WCHAR_MIN) && (WCHAR_MIN == 0) && !defined(HBOOST_NO_INTRINSIC_WCHAR_T)
...
iterator HBOOST_REGEX_CALL re_is_set_member(iterator next, 
                          iterator last, 
...
{   
   ...
   const ::hboost::regex_traits_wrapper<traits_type>& traits_inst = *(e.m_ptraits);
...
private:
   ...
   {
      return ::hboost::is_random_access_iterator<BidiIterator>::value ? match_dot_repeat_fast() : match_dot_repeat_slow();
   }
   bool match_backstep();
...
private:
   ...
   const ::hboost::regex_traits_wrapper<traits>& traits_inst;
...
//
#ifdef HBOOST_REGEX_RECURSIVE
#include <hboost/regex/v4/perl_matcher_recursive.hpp>
#else
#include <hboost/regex/v4/perl_matcher_non_recursive.hpp>
#endif
// this one has to be last:
#include <hboost/regex/v4/perl_matcher_common.hpp>
