...
#ifndef HBOOST_REGEX_V4_PERL_MATCHER_COMMON_HPP
#define HBOOST_REGEX_V4_PERL_MATCHER_COMMON_HPP
...
{ 
   ...
   m_stack_base = 0;
   m_backup_state = 0;
#elif defined(HBOOST_REGEX_RECURSIVE)
   ...
   match_any_mask = static_cast<unsigned char>((f & match_not_dot_newline) ? HBOOST_REGEX_DETAIL_NS::test_not_newline : HBOOST_REGEX_DETAIL_NS::test_newline);
...
void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(std::random_access_iterator_tag*)
{
   ...
   std::ptrdiff_t dist = hboost::HBOOST_REGEX_DETAIL_NS::distance(base, last);
   if(dist == 0)
      dist = 1;
   ...
   {
      max_state_count = (std::min)((std::ptrdiff_t)HBOOST_REGEX_MAX_STATE_COUNT, (std::numeric_limits<std::ptrdiff_t>::max)() - 2);
      return;
   }
   states *= dist;
   ...
   //
   if(states > HBOOST_REGEX_MAX_STATE_COUNT)
      states = HBOOST_REGEX_MAX_STATE_COUNT;
   //
...
inline void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(void*)
{
   ...
   max_state_count = HBOOST_REGEX_MAX_STATE_COUNT;
}
...
{
   ::hboost::HBOOST_REGEX_DETAIL_NS::concrete_protected_call
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_imp()
{
   ...
   used_block_count = HBOOST_REGEX_MAX_BLOCKS;
...
#if defined(HBOOST_REGEX_NON_RECURSIVE) && !defined(HBOOST_NO_EXCEPTIONS)
   }
   catch(...)
   {
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_backref()
{
   ...
   if(index >= 10000)
   {
      ...
      HBOOST_ASSERT(r.first != r.second);
      do
      {
         index = r.first->index;
         ++r.first;
...
#pragma warning(disable:4127)
   ...
   {
      std::ptrdiff_t maxlen = ::hboost::HBOOST_REGEX_DETAIL_NS::distance(backstop, position);
