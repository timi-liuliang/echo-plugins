...
#ifndef HBOOST_REGEX_V4_PERL_MATCHER_RECURSIVE_HPP
#define HBOOST_REGEX_V4_PERL_MATCHER_RECURSIVE_HPP
...
#endif
   bool greedy = (rep->greedy) && (!(m_match_flags & regex_constants::match_any) || m_independent);   
   std::size_t count = (std::min)(static_cast<std::size_t>(::hboost::HBOOST_REGEX_DETAIL_NS::distance(position, last)), static_cast<std::size_t>(greedy ? rep->max : rep->min));
   if(rep->min > count)
   {
      position = last;
   ...
   if(::hboost::is_random_access_iterator<BidiIterator>::value)
   {
      desired = 
         (std::min)(
            ...
            (std::size_t)::hboost::HBOOST_REGEX_DETAIL_NS::distance(position, last));
      count = desired;
      ++desired;
      if(icase)
      {
