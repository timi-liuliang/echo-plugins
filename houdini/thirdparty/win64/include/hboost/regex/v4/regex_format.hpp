...
#define HBOOST_REGEX_FORMAT_HPP
...
// Forward declaration:
//
   template <class BidiIterator, class Allocator = HBOOST_DEDUCED_TYPENAME std::vector<sub_match<BidiIterator> >::allocator_type >
class match_results;
...
private:
   ...
   {
      typedef typename hboost::is_convertible<ForwardIter, const char_type*>::type tag_type;
...
#endif
   inline int toi(ForwardIter& i, ForwardIter j, int base, const hboost::mpl::false_&)
   {
      if(i != j)
      {
         std::vector<char_type> v(i, j);
...
#pragma warning(pop)
#endif
   inline int toi(ForwardIter& i, ForwardIter j, int base, const hboost::mpl::true_&)
   {
   ...
   inline int toi(ForwardIter& i, ForwardIter j, int base)
   {
      ...
      // See also https://svn.boost.org/trac/hboost/ticket/6359
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_all()
{
   ...
   while(m_position != m_end)
   {
      ...
      {
      case '&':
         if(m_flags & ::hboost::regex_constants::format_sed)
         {
            ++m_position;
            put(m_results[0]);
            break;
      ...
      case '\\':
         ...
         break;
      case '(':
         if(m_flags & hboost::regex_constants::format_all)
         {
            ++m_position;
            ...
            if(m_position == m_end)
               return;
            HBOOST_ASSERT(*m_position == static_cast<char_type>(')'));
      ...
      case ')':
         ...
         break;
      case ':':
         if((m_flags & hboost::regex_constants::format_all) && m_have_conditional)
         {
            return;
         }
         put(*m_position);
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_perl()
{
   ...
   default:
      ...
      {
         std::ptrdiff_t len = ::hboost::HBOOST_REGEX_DETAIL_NS::distance(m_position, m_end);
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_escape()
{
   ...
   default:
      ...
      if((m_flags & hboost::regex_constants::format_sed) == 0)
      {
         bool breakout = false;
         switch(*m_position)
         {
      ...
      int v = this->toi(m_position, m_position+len, 10);
      if((v > 0) || ((v == 0) && (m_flags & ::hboost::regex_constants::format_sed)))
      {
         put(m_results[v]);
         break;
      }
      else if(v == 0)
      {
         ...
         --m_position;
         len = ::hboost::HBOOST_REGEX_DETAIL_NS::distance(m_position, m_end);
                         ...
                         )
{
   ...
   {
      return HBOOST_REGEX_DETAIL_NS::copy(p1, p2, out);
   }

   HBOOST_REGEX_DETAIL_NS::basic_regex_formatter<
      OutputIterator, 
...
HBOOST_MPL_HAS_XXX_TRAIT_DEF(const_iterator)
...
template<typename Formatter, bool IsFunction = hboost::is_function<Formatter>::value>
...
private:
   ...
   //
   HBOOST_STATIC_ASSERT((::hboost::is_pointer<F>::value || ::hboost::is_function<F>::value || ::hboost::is_class<F>::value));
   static formatter_wrapper<typename unwrap_reference<F>::type> f;
   static M m;
   static O out;
   static hboost::regex_constants::match_flag_type flags;
public:
   HBOOST_STATIC_CONSTANT(int, value = sizeof(check_is_formatter(f(m), f(m, out), f(m, out, flags))));
};
...
public:
   ...
   typedef typename hboost::mpl::if_<
      hboost::mpl::and_<hboost::is_pointer<F>, hboost::mpl::not_<hboost::is_function<typename hboost::remove_pointer<F>::type> > >,
      hboost::mpl::int_<0>,
      typename hboost::mpl::if_<
         has_const_iterator<F>,
         hboost::mpl::int_<1>,
         hboost::mpl::int_<format_traits_imp<F, M, O>::value>
      >::type
   >::type type;
   //
   ...
   //
   HBOOST_STATIC_ASSERT( hboost::is_class<F>::value && !has_const_iterator<F>::value ? (type::value > 1) : true);
};
...
struct format_functor3
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, hboost::regex_constants::match_flag_type f)
   {
      return hboost::unwrap_ref(func)(m, i, f);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, hboost::regex_constants::match_flag_type f, const Traits&)
   {
      return (*this)(m, i, f);
   }
private:
...
struct format_functor2
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, hboost::regex_constants::match_flag_type /*f*/)
   {
      return hboost::unwrap_ref(func)(m, i);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, hboost::regex_constants::match_flag_type f, const Traits&)
   {
...
struct format_functor1
{
   ...
   {
      return HBOOST_REGEX_DETAIL_NS::copy(s.begin(), s.end(), i);
   }
   template <class S, class OutputIter>
   ...
   OutputIter operator()(const Match& m, OutputIter i, hboost::regex_constants::match_flag_type /*f*/)
   {
      return do_format_string(hboost::unwrap_ref(func)(m), i);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, hboost::regex_constants::match_flag_type f, const Traits&)
   {
...
struct format_functor_c_string
{
   ...
   template <class OutputIter>
   OutputIter operator()(const Match& m, OutputIter i, hboost::regex_constants::match_flag_type f, const Traits& t = Traits())
   {
...
struct format_functor_container
{
   ...
   {
      //typedef typename Match::char_type char_type;
      return HBOOST_REGEX_DETAIL_NS::regex_format_imp(i, m, func.begin(), func.end(), f, t);
   }
private:
...
template <class Func, class Match, class OutputIterator, class Traits = HBOOST_REGEX_DETAIL_NS::trivial_format_traits<typename Match::char_type> >
struct compute_functor_type
{
   typedef typename format_traits<Func, Match, OutputIterator>::type tag;
   typedef typename hboost::remove_cv< typename hboost::remove_pointer<Func>::type>::type maybe_char_type;

   typedef typename mpl::if_<
      ::hboost::is_same<tag, mpl::int_<0> >, format_functor_c_string<maybe_char_type, Match, Traits>,
      typename mpl::if_<
         ::hboost::is_same<tag, mpl::int_<1> >, format_functor_container<Func, Match, Traits>,
         typename mpl::if_<
            ::hboost::is_same<tag, mpl::int_<2> >, format_functor1<Func, Match>,
            typename mpl::if_<
               ::hboost::is_same<tag, mpl::int_<3> >, format_functor2<Func, Match>, 
