...
#if !defined(HBOOST_CIRCULAR_BUFFER_DETAILS_HPP)
#define HBOOST_CIRCULAR_BUFFER_DETAILS_HPP
...
struct assign_range {
    ...
    template <class Pointer>
    void operator () (Pointer p) const {
        hboost::cb_details::uninitialized_copy(m_first, m_last, p, m_alloc);
    }
};
...
public:
    ...
    { // Check for capacity lower than min_capacity.
        HBOOST_CB_ASSERT(buffer_capacity >= min_buffer_capacity);
    }
...
#endif // #if HBOOST_CB_ENABLE_DEBUG
{
// Helper types

    //! Base iterator.
    typedef hboost::iterator<
        std::random_access_iterator_tag,
        typename Traits::value_type,
        typename Traits::difference_type,
        typename Traits::pointer,
...
// Random access iterator methods
    ...
    reference operator * () const {
        HBOOST_CB_ASSERT(is_valid(m_buff)); // check for uninitialized or invalidated iterator
        HBOOST_CB_ASSERT(m_it != 0);        // check for iterator pointing to end()
        return *m_it;
    }
    ...
    difference_type operator - (const iterator<Buff, Traits0>& it) const {
        HBOOST_CB_ASSERT(is_valid(m_buff));    // check for uninitialized or invalidated iterator
        HBOOST_CB_ASSERT(it.is_valid(m_buff)); // check for uninitialized or invalidated iterator
    ...
    iterator& operator -- () {
        HBOOST_CB_ASSERT(is_valid(m_buff));        // check for uninitialized or invalidated iterator
        HBOOST_CB_ASSERT(m_it != m_buff->m_first); // check for iterator pointing to begin()
        if (m_it == 0)
            m_it = m_buff->m_last;
    ...
    iterator& operator += (difference_type n) {
        HBOOST_CB_ASSERT(is_valid(m_buff)); // check for uninitialized or invalidated iterator
        if (n > 0) {
            HBOOST_CB_ASSERT(m_buff->end() - *this >= n); // check for too large n
    ...
    iterator& operator -= (difference_type n) {
        HBOOST_CB_ASSERT(is_valid(m_buff)); // check for uninitialized or invalidated iterator
        if (n > 0) {
            HBOOST_CB_ASSERT(*this - m_buff->begin() >= n); // check for too large n
...
inline ForwardIterator uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    ...
    HBOOST_TRY {
        for (; first != last; ++first, ++dest)
            hboost::container::allocator_traits<Alloc>::construct(a, hboost::addressof(*dest), *first);
    } HBOOST_CATCH(...) {
        for (; next != dest; ++next)
            hboost::container::allocator_traits<Alloc>::destroy(a, hboost::addressof(*next));
        HBOOST_RETHROW
    }
    HBOOST_CATCH_END
    return dest;
...
ForwardIterator uninitialized_move_if_noexcept_impl(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a,
    true_type) {
    for (; first != last; ++first, ++dest)
        hboost::container::allocator_traits<Alloc>::construct(a, hboost::addressof(*dest), hboost::move(*first));
    return dest;
}
...
template<class InputIterator, class ForwardIterator, class Alloc>
ForwardIterator uninitialized_move_if_noexcept(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    typedef typename hboost::is_nothrow_move_constructible<typename hboost::container::allocator_traits<Alloc>::value_type>::type tag_t;
...
inline void uninitialized_fill_n_with_alloc(ForwardIterator first, Diff n, const T& item, Alloc& alloc) {
    ...
    HBOOST_TRY {
        for (; n > 0; ++first, --n)
            hboost::container::allocator_traits<Alloc>::construct(alloc, hboost::addressof(*first), item);
    } HBOOST_CATCH(...) {
        for (; next != first; ++next)
            hboost::container::allocator_traits<Alloc>::destroy(alloc, hboost::addressof(*next));
        HBOOST_RETHROW
    }
    HBOOST_CATCH_END
}
...
#endif // #if !defined(HBOOST_CIRCULAR_BUFFER_DETAILS_HPP)
