...
#if !defined(HBOOST_CIRCULAR_BUFFER_SPACE_OPTIMIZED_HPP)
#define HBOOST_CIRCULAR_BUFFER_SPACE_OPTIMIZED_HPP
...
// Overridden
    ...
    /*!
        ...
        \sa <code>empty()</code>
    */
    bool full() const HBOOST_NOEXCEPT { return m_capacity_ctrl == size(); }
    ...
    */
    size_type reserve() const HBOOST_NOEXCEPT { return m_capacity_ctrl - size(); }
    ...
    */
    const capacity_type& capacity() const HBOOST_NOEXCEPT { return m_capacity_ctrl; }

#if defined(HBOOST_CB_TEST)
    ...
    */
    size_type internal_capacity() const HBOOST_NOEXCEPT { return circular_buffer<T, Alloc>::capacity(); }

#endif // #if defined(HBOOST_CB_TEST)
    ...
    /*! \brief Change the capacity (and the minimal guaranteed amount of allocated memory) of the
        ...
        \note To explicitly clear the extra allocated memory use the <b>shrink-to-fit</b> technique:<br><br>
              <code>%hboost::%circular_buffer_space_optimized\<int\> cb(1000);<br>
              ...<br>
              %hboost::%circular_buffer_space_optimized\<int\>(cb).swap(cb);</code><br><br>
    ...
    */
    explicit circular_buffer_space_optimized(const allocator_type& alloc = allocator_type()) HBOOST_NOEXCEPT
    ...
    */
    ...
    /*! \brief Move constructs a <code>circular_buffer_space_optimized</code> from <code>cb</code>, 
        ...
        \par Constant.
    */
    circular_buffer_space_optimized(circular_buffer_space_optimized<T, Alloc>&& cb) HBOOST_NOEXCEPT
    : circular_buffer<T, Alloc>()
    , m_capacity_ctrl(0) {
        cb.swap(*this);
    }
    ...
    */
    ...
    , m_capacity_ctrl(capacity_ctrl) {
        reduce_capacity(
            is_same< HBOOST_DEDUCED_TYPENAME iterator_category<InputIterator>::type, std::input_iterator_tag >());
    }

#if defined(HBOOST_CB_NEVER_DEFINED)
...
// Declared only for documentation purpose.
...
#endif // #if defined(HBOOST_CB_NEVER_DEFINED)
    ...
    */
    ...
    /*! \brief Move assigns content of <code>cb</code> to <code>*this</code>, leaving <code>cb</code> empty.
        ...
        \par Complexity
             Constant.
    */
    circular_buffer_space_optimized<T, Alloc>& operator = (circular_buffer_space_optimized<T, Alloc>&& cb) HBOOST_NOEXCEPT {
    ...
    */
    void assign(capacity_type capacity_ctrl, size_type n, param_value_type item) {
       HBOOST_CB_ASSERT(capacity_ctrl.capacity() >= n); // check for new capacity lower than n
    ...
    */
    ...
    /*!
        ...
        \par Iterator Invalidation
             ...
             this feature you have to turn the __debug_support off by defining macro HBOOST_CB_DISABLE_DEBUG,
    ...
    */
    ...
    void swap(circular_buffer_space_optimized<T, Alloc>& cb) HBOOST_NOEXCEPT {
    ...
    */
    void push_back(rvalue_type item) {
        check_low_capacity();
        circular_buffer<T, Alloc>::push_back(hboost::move(item));
    }
    ...
    */
    void push_front(rvalue_type item) {
        check_low_capacity();
        circular_buffer<T, Alloc>::push_front(hboost::move(item));
    }
    ...
    */
    iterator insert(iterator pos, rvalue_type item) {
        size_type index = pos - begin();
        check_low_capacity();
        return circular_buffer<T, Alloc>::insert(begin() + index, hboost::move(item));
    }
    ...
    */
    iterator rinsert(iterator pos, rvalue_type item) {
        size_type index = pos - begin();
        check_low_capacity();
        return circular_buffer<T, Alloc>::rinsert(begin() + index, hboost::move(item));
    }
...
// Helper methods
    ...
    */
    ...
    static size_type init_capacity(const capacity_type& capacity_ctrl, size_type n) {
        HBOOST_CB_ASSERT(capacity_ctrl.capacity() >= n); // check for capacity lower than n
    ...
    static size_type init_capacity(const capacity_type& capacity_ctrl, Iterator first, Iterator last,
        ...
        return init_capacity(
            capacity_ctrl, first, last, HBOOST_DEDUCED_TYPENAME iterator_category<Iterator>::type());
#endif
    }
...
#if !defined(HBOOST_NO_FUNCTION_TEMPLATE_ORDERING) || HBOOST_WORKAROUND(HBOOST_MSVC, HBOOST_TESTED_AT(1310))
...
inline void swap(circular_buffer_space_optimized<T, Alloc>& lhs,
    circular_buffer_space_optimized<T, Alloc>& rhs) HBOOST_NOEXCEPT {
    lhs.swap(rhs);
}

#endif // #if !defined(HBOOST_NO_FUNCTION_TEMPLATE_ORDERING) || HBOOST_WORKAROUND(HBOOST_MSVC, HBOOST_TESTED_AT(1310))

} // namespace hboost

#endif // #if !defined(HBOOST_CIRCULAR_BUFFER_SPACE_OPTIMIZED_HPP)
