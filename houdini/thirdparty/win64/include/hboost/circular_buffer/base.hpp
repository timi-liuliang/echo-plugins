...
#if !defined(HBOOST_CIRCULAR_BUFFER_BASE_HPP)
#define HBOOST_CIRCULAR_BUFFER_BASE_HPP
...
/*! \endcond */
{
  ...
  // Requirements
    //HBOOST_CLASS_REQUIRE(T, hboost, SGIAssignableConcept);


    //HBOOST_CONCEPT_ASSERT((Assignable<T>));
    //HBOOST_CONCEPT_ASSERT((CopyConstructible<T>));
    //HBOOST_CONCEPT_ASSERT((DefaultConstructible<T>));

    // Required if the circular_buffer will be compared with anther container.
    //HBOOST_CONCEPT_ASSERT((EqualityComparable<T>));
    //HBOOST_CONCEPT_ASSERT((LessThanComparable<T>));
...
// Basic types
    ...
    typedef typename hboost::container::allocator_traits<Alloc>::value_type value_type;

    //! A pointer to an element.
    typedef typename hboost::container::allocator_traits<Alloc>::pointer pointer;

    //! A const pointer to the element.
    typedef typename hboost::container::allocator_traits<Alloc>::const_pointer const_pointer;

    //! A reference to an element.
    typedef typename hboost::container::allocator_traits<Alloc>::reference reference;

    //! A const reference to an element.
    typedef typename hboost::container::allocator_traits<Alloc>::const_reference const_reference;

    //! The distance type.
    /*!
        (A signed integral type used to represent the distance between two iterators.)
    */
    typedef typename hboost::container::allocator_traits<Alloc>::difference_type difference_type;

    //! The size type.
    /*!
        (An unsigned integral type that can represent any non-negative value of the container's distance type.)
    */
    typedef typename hboost::container::allocator_traits<Alloc>::size_type size_type;
...
// Iterators
    ...
    typedef cb_details::iterator< circular_buffer<T, Alloc>, cb_details::const_traits<hboost::container::allocator_traits<Alloc> > > const_iterator;

    //! A (random access) iterator used to iterate through the <code>circular_buffer</code>.
    typedef cb_details::iterator< circular_buffer<T, Alloc>, cb_details::nonconst_traits<hboost::container::allocator_traits<Alloc> > > iterator;
...
// Helper types
    ...
    typedef HBOOST_RV_REF(value_type) rvalue_type;
...
// Allocator
    ...
    */
    allocator_type get_allocator() const HBOOST_NOEXCEPT { return m_alloc; }
    ...
    */
    allocator_type& get_allocator() HBOOST_NOEXCEPT { return m_alloc; }
...
// Element access
    ...
    */
    iterator begin() HBOOST_NOEXCEPT { return iterator(this, empty() ? 0 : m_first); }
    ...
    */
    iterator end() HBOOST_NOEXCEPT { return iterator(this, 0); }
    ...
    */
    const_iterator begin() const HBOOST_NOEXCEPT { return const_iterator(this, empty() ? 0 : m_first); }
    ...
    */
    const_iterator end() const HBOOST_NOEXCEPT { return const_iterator(this, 0); }
    ...
    */
    reverse_iterator rbegin() HBOOST_NOEXCEPT { return reverse_iterator(end()); }
    ...
    */
    reverse_iterator rend() HBOOST_NOEXCEPT { return reverse_iterator(begin()); }
    ...
    */
    const_reverse_iterator rbegin() const HBOOST_NOEXCEPT { return const_reverse_iterator(end()); }
    ...
    */
    const_reverse_iterator rend() const HBOOST_NOEXCEPT { return const_reverse_iterator(begin()); }
    ...
    */
    reference operator [] (size_type index) {
        HBOOST_CB_ASSERT(index < size()); // check for invalid index
    ...
    */
    reference front() {
        HBOOST_CB_ASSERT(!empty()); // check for empty buffer (front element not available)
        return *m_first;
    }
    ...
    */
    reference back() {
        HBOOST_CB_ASSERT(!empty()); // check for empty buffer (back element not available)
            ...
            for (pointer first = m_first; dest < src; src = first) {
                for (size_type ii = 0; src < m_end; ++src, ++dest, ++moved, ++ii) {
                    ...
                    if (is_uninitialized(dest)) {
                        hboost::container::allocator_traits<Alloc>::construct(m_alloc, hboost::addressof(*dest), hboost::move_if_noexcept(*src));
                        ++constructed;
                    } else {
                        value_type tmp = hboost::move_if_noexcept(*src); 
                        replace(src, hboost::move_if_noexcept(*dest));
                        replace(dest, hboost::move(tmp));
                    }
                }
            }
        } HBOOST_CATCH(...) {
    ...
    */
    bool is_linearized() const HBOOST_NOEXCEPT { return m_first < m_last || m_last == m_buff; }
    ...
    */
    void rotate(const_iterator new_begin) {
        HBOOST_CB_ASSERT(new_begin.is_valid(this)); // check for uninitialized or invalidated iterator
        HBOOST_CB_ASSERT(new_begin.m_it != 0);      // check for iterator pointing to end()
        if (full()) {
            ...
            if (m < n) {
                for (; m > 0; --m) {
                    push_front(hboost::move_if_noexcept(back()));
                    pop_back();
                }
            } else {
                for (; n > 0; --n) {
                    push_back(hboost::move_if_noexcept(front()));
                    pop_front();
                }
            }
        }
...
// Size and capacity
    ...
    */
    size_type size() const HBOOST_NOEXCEPT { return m_size; }
    ...
    */
    size_type max_size() const HBOOST_NOEXCEPT {
        return (std::min<size_type>)(hboost::container::allocator_traits<Alloc>::max_size(m_alloc), (std::numeric_limits<difference_type>::max)());
    }
    ...
    /*!
        ...
        \sa <code>full()</code>
    */
    bool empty() const HBOOST_NOEXCEPT { return size() == 0; }
    ...
    /*!
        ...
        \sa <code>empty()</code>
    */
    bool full() const HBOOST_NOEXCEPT { return capacity() == size(); }
    ...
    */
    size_type reserve() const HBOOST_NOEXCEPT { return capacity() - size(); }
    ...
    */
    capacity_type capacity() const HBOOST_NOEXCEPT { return m_end - m_buff; }
...
// Construction/Destruction
    ...
    /*!
        ...
        \sa <code>circular_buffer(capacity_type, const allocator_type& alloc)</code>,
            <code>set_capacity(capacity_type)</code>
    */
    explicit circular_buffer(const allocator_type& alloc = allocator_type()) HBOOST_NOEXCEPT
    ...
    */
    ...
    : m_size(n), m_alloc(alloc) {
        HBOOST_CB_ASSERT(buffer_capacity >= size()); // check for capacity lower than size
    ...
    */
    ...
    /*! \brief Move constructs a <code>circular_buffer</code> from <code>cb</code>, leaving <code>cb</code> empty.
        ...
        \par Constant.
    */
    circular_buffer(circular_buffer<T, Alloc>&& cb) HBOOST_NOEXCEPT
    ...
    */
    ...
    /*!
        ...
        \sa <code>clear()</code>
    */
    ~circular_buffer() HBOOST_NOEXCEPT {
        destroy();
#if HBOOST_CB_ENABLE_DEBUG
...
// Assign methods
    ...
    */
    ...
    /*! \brief Move assigns content of <code>cb</code> to <code>*this</code>, leaving <code>cb</code> empty.
        ...
        \par Complexity
             Constant.
    */
    circular_buffer<T, Alloc>& operator = (circular_buffer<T, Alloc>&& cb) HBOOST_NOEXCEPT {
    ...
    */
    void assign(capacity_type buffer_capacity, size_type n, param_value_type item) {
        HBOOST_CB_ASSERT(buffer_capacity >= n); // check for new capacity lower than n
    ...
    */
    void swap(circular_buffer<T, Alloc>& cb) HBOOST_NOEXCEPT {
...
private:
    ...
    void push_back_impl(ValT item) {
        if (full()) {
            ...
            hboost::container::allocator_traits<Alloc>::construct(m_alloc, hboost::addressof(*m_last), static_cast<ValT>(item));
            increment(m_last);
            ++m_size;
        }        
    }
    ...
    void push_front_impl(ValT item) {
        HBOOST_TRY {
            if (full()) {
                ...
                decrement(m_first);
                hboost::container::allocator_traits<Alloc>::construct(m_alloc, hboost::addressof(*m_first), static_cast<ValT>(item));
                ++m_size;
            }
        } HBOOST_CATCH(...) {
            increment(m_first);
...
public:
    ...
    */
    void push_back(rvalue_type item) {
        push_back_impl<rvalue_type>(hboost::move(item));
    }
    ...
    */
    void push_front(rvalue_type item) {
        push_front_impl<rvalue_type>(hboost::move(item));
    }
...
private:
    ...
    iterator insert_impl(iterator pos, ValT item) {
        HBOOST_CB_ASSERT(pos.is_valid(this)); // check for uninitialized or invalidated iterator
        iterator b = begin();
        if (full() && pos == b)
            return b;
...
// Insert
    ...
    */
    iterator insert(iterator pos, rvalue_type item) {
        return insert_impl<rvalue_type>(pos, hboost::move(item));
    }
    ...
    */
    iterator insert(iterator pos) {
        value_type temp;
        return insert(pos, hboost::move(temp));
    }
...
private:
    ...
    iterator rinsert_impl(iterator pos, ValT item) {
        ...
        if (pos == begin()) {
            ...
            HBOOST_TRY {
                while (src != pos.m_it) {
                    construct_or_replace(construct, dest, hboost::move_if_noexcept(*src));
                    increment(src);
                    increment(dest);
                    construct = false;
                }
...
public:
    ...
    */
    iterator rinsert(iterator pos, rvalue_type item) {
        return rinsert_impl<rvalue_type>(pos, hboost::move(item));
    }
    ...
    */
    iterator rinsert(iterator pos) {
        value_type temp;
        return rinsert(pos, hboost::move(temp));
    }
...
// Erase
    ...
    */
    iterator erase(iterator pos) {
        HBOOST_CB_ASSERT(pos.is_valid(this)); // check for uninitialized or invalidated iterator
        HBOOST_CB_ASSERT(pos.m_it != 0);      // check for iterator pointing to end()
        pointer next = pos.m_it;
        increment(next);
        for (pointer p = pos.m_it; next != m_last; p = next, increment(next))
            replace(p, hboost::move_if_noexcept(*next));
        decrement(m_last);
        destroy_item(m_last);
        --m_size;
#if HBOOST_CB_ENABLE_DEBUG
    ...
    */
    iterator erase(iterator first, iterator last) {
        HBOOST_CB_ASSERT(first.is_valid(this)); // check for uninitialized or invalidated iterator
        HBOOST_CB_ASSERT(last.is_valid(this));  // check for uninitialized or invalidated iterator
        HBOOST_CB_ASSERT(first <= last);        // check for wrong range
        if (first == last)
            return first;
        pointer p = first.m_it;
        while (last.m_it != 0)
            replace((first++).m_it, hboost::move_if_noexcept(*last++));
        do {
            decrement(m_last);
            destroy_item(m_last);
            --m_size;
    ...
    */
    iterator rerase(iterator pos) {
        ...
        for (decrement(prev); p != m_first; p = prev, decrement(prev))
            replace(p, hboost::move_if_noexcept(*prev));
        destroy_item(m_first);
        increment(m_first);
        --m_size;
#if HBOOST_CB_ENABLE_DEBUG
    ...
    */
    iterator rerase(iterator first, iterator last) {
        ...
        while (first.m_it != m_first) {
            ...
            decrement(p);
            replace(p, hboost::move_if_noexcept(*first.m_it));
        }
        do {
            destroy_item(m_first);
            increment(m_first);
    ...
    */
    void erase_begin(size_type n) {
        HBOOST_CB_ASSERT(n <= size()); // check for n greater than size
#if HBOOST_CB_ENABLE_DEBUG
...
// Helper methods
    ...
    bool is_uninitialized(const_pointer p) const HBOOST_NOEXCEPT {
    ...
    */
    void construct_or_replace(bool construct, pointer pos, param_value_type item) {
        if (construct)
            hboost::container::allocator_traits<Alloc>::construct(m_alloc, hboost::addressof(*pos), item);
        else
            replace(pos, item);
    }
    ...
    */
    void construct_or_replace(bool construct, pointer pos, rvalue_type item) {
        if (construct)
            hboost::container::allocator_traits<Alloc>::construct(m_alloc, hboost::addressof(*pos), hboost::move(item));
        else
            replace(pos, hboost::move(item));
    }

    //! Destroy an item.
    void destroy_item(pointer p) {
        hboost::container::allocator_traits<Alloc>::destroy(m_alloc, hboost::addressof(*p));
#if HBOOST_CB_ENABLE_DEBUG
    ...
    void initialize(Iterator first, Iterator last, const false_type&) {
        HBOOST_CB_IS_CONVERTIBLE(Iterator, value_type); // check for invalid iterator type
#if HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x581))
        initialize(first, last, iterator_category<Iterator>::type());
#else
        initialize(first, last, HBOOST_DEDUCED_TYPENAME iterator_category<Iterator>::type());
#endif
    }
    ...
    void initialize(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        HBOOST_CB_ASSERT_TEMPLATED_ITERATOR_CONSTRUCTORS // check if the STL provides templated iterator constructors
                                                        // for containers
        std::deque<value_type, allocator_type> tmp(first, last, m_alloc);
        size_type distance = tmp.size();
        initialize(distance, hboost::make_move_iterator(tmp.begin()), hboost::make_move_iterator(tmp.end()), distance);
    }

    //! Specialized initialize method.
    template <class ForwardIterator>
    void initialize(ForwardIterator first, ForwardIterator last, const std::forward_iterator_tag&) {
        HBOOST_CB_ASSERT(std::distance(first, last) >= 0); // check for wrong range
    ...
    void initialize(capacity_type buffer_capacity, IntegralType n, IntegralType item, const true_type&) {
        HBOOST_CB_ASSERT(buffer_capacity >= static_cast<size_type>(n)); // check for capacity lower than n
        m_size = static_cast<size_type>(n);
        initialize_buffer(buffer_capacity, item);
        m_first = m_buff;
...
#else
        initialize(buffer_capacity, first, last, HBOOST_DEDUCED_TYPENAME iterator_category<Iterator>::type());
#endif
    }
    ...
    void initialize(capacity_type buffer_capacity,
        ...
        while (first != last && !full()) {
            hboost::container::allocator_traits<Alloc>::construct(m_alloc, hboost::addressof(*m_last), *first++);
            increment(m_last);
            ++m_size;
        }
        while (first != last) {
...
#else
        assign(first, last, HBOOST_DEDUCED_TYPENAME iterator_category<Iterator>::type());
#endif
    }
    ...
    void assign(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        assign_n(distance, distance,
            cb_details::make_assign_range
                (hboost::make_move_iterator(tmp.begin()), hboost::make_move_iterator(tmp.end()), m_alloc));
    }
...
#else
        assign(new_capacity, first, last, HBOOST_DEDUCED_TYPENAME iterator_category<Iterator>::type());
#endif
    }
...
#else
        insert(pos, first, last, HBOOST_DEDUCED_TYPENAME iterator_category<Iterator>::type());
#endif
    }
        ...
        if (pos.m_it == 0) {
            ...
            HBOOST_TRY {
                for (; ii < construct; ++ii, increment(p))
                    hboost::container::allocator_traits<Alloc>::construct(m_alloc, hboost::addressof(*p), *wrapper());
...
#else
        rinsert(pos, first, last, HBOOST_DEDUCED_TYPENAME iterator_category<Iterator>::type());
#endif
    }
...
#if !defined(HBOOST_NO_FUNCTION_TEMPLATE_ORDERING) || defined(HBOOST_MSVC)
...
*/
template <class T, class Alloc>
inline void swap(circular_buffer<T, Alloc>& lhs, circular_buffer<T, Alloc>& rhs) HBOOST_NOEXCEPT {
    lhs.swap(rhs);
}

#endif // #if !defined(HBOOST_NO_FUNCTION_TEMPLATE_ORDERING) || defined(HBOOST_MSVC)

} // namespace hboost

#endif // #if !defined(HBOOST_CIRCULAR_BUFFER_BASE_HPP)
