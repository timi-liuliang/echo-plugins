...
//
#ifndef HBOOST_RANGE_ADAPTOR_FORMATTED_HPP_INCLUDED
#define HBOOST_RANGE_ADAPTOR_FORMATTED_HPP_INCLUDED
...
struct formatted_holder
{
    typedef typename hboost::mpl::if_<
        hboost::is_array<Sep>,
        const typename hboost::remove_extent<Sep>::type*,
        Sep
    >::type separator_t;

    typedef typename hboost::mpl::if_<
        hboost::is_array<Prefix>,
        const typename hboost::remove_extent<Prefix>::type*,
        Prefix
    >::type prefix_t;

    typedef typename hboost::mpl::if_<
        hboost::is_array<Postfix>,
        const typename hboost::remove_extent<Postfix>::type*,
        Postfix
    >::type postfix_t;
...
)
{
    ...
    return range_detail::formatted_range<iterator, Sep, Prefix, Postfix>(
        hboost::begin(rng), hboost::end(rng), holder);
}
...
template<typename SinglePassRange, typename Sep, typename Prefix,
         typename Postfix>
inline hboost::range_detail::formatted_range<
    typename hboost::range_iterator<const SinglePassRange>::type,
    Sep, Prefix, Postfix
>
format(
    const SinglePassRange& rng,
...
{
    typedef typename hboost::range_iterator<const SinglePassRange>::type
                iterator_t;

    typedef hboost::range_detail::formatted_range<
                iterator_t, Sep, Prefix, Postfix>       result_t;

    typedef hboost::range_detail::formatted_holder<Sep, Prefix, Postfix>
                holder_t;

    return result_t(hboost::begin(rng), hboost::end(rng),
...
format(const SinglePassRange& rng)
{
    ...
    namespace range
    {
        using hboost::range_detail::formatted_range;
    } // namespace range
} // namespace hboost
