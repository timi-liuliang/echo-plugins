...
#ifndef HBOOST_RANGE_ADAPTOR_TOKENIZED_HPP
#define HBOOST_RANGE_ADAPTOR_TOKENIZED_HPP
...
namespace hboost
{
    namespace range_detail
    {
        ...
        private:
            ...
            typedef HBOOST_DEDUCED_TYPENAME regex_iter::regex_type 
                regex_type;
        
            typedef hboost::iterator_range<regex_iter> 
                base;

        public:
            template< class Regex, class Submatch, class Flag >
            tokenized_range( R& r, const Regex& re, const Submatch& sub, Flag f )
              : base( regex_iter( hboost::begin(r), hboost::end(r), 
                                  regex_type(re), sub, f ),
                      regex_iter() )
            { }
        };
