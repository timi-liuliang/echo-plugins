...
#ifndef HBOOST_RANGE_ADAPTOR_ADJACENT_FILTER_IMPL_HPP
#define HBOOST_RANGE_ADAPTOR_ADJACENT_FILTER_IMPL_HPP
    ...
    namespace range_detail
    {
        ...
        class skip_iterator
          : public hboost::iterator_adaptor<
                    ...
                    Iter,
                    HBOOST_DEDUCED_TYPENAME std::iterator_traits<Iter>::value_type,
                    hboost::forward_traversal_tag,
                    HBOOST_DEDUCED_TYPENAME std::iterator_traits<Iter>::reference,
                    HBOOST_DEDUCED_TYPENAME std::iterator_traits<Iter>::difference_type
                >
          , private Pred
        {
        private:
        ...
        private:
            ...
            typedef HBOOST_DEDUCED_TYPENAME range_iterator<R>::type raw_iterator;

        public:
            adjacent_filtered_range( const P& p, R& r )
            : base_range(skip_iter(hboost::begin(r), hboost::end(r), p),
                         skip_iter(hboost::end(r), hboost::end(r), p))
            {
            }
        };
        ...
        {
            HBOOST_RANGE_CONCEPT_ASSERT((ForwardRangeConcept<ForwardRng>));
        ...
        operator|( const ForwardRng& r,
                   const adjacent_holder<BinPredicate>& f )
        {
            HBOOST_RANGE_CONCEPT_ASSERT((ForwardRangeConcept<const ForwardRng>));
