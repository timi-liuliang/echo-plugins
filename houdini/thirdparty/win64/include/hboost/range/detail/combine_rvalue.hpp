...
#define HBOOST_RANGE_combined_args(z, n, i) \
    HBOOST_PP_CAT(R, n)&& HBOOST_PP_CAT(r, n)

#define HBOOST_RANGE_combined_range_iterator(z, n, i) \
    typename range_iterator< \
        typename remove_reference<HBOOST_PP_CAT(R,n)>::type \
  >::type
...
#define HBOOST_RANGE_combine(z, n, data) \
    ...
    inline range::combined_range< \
        tuple<HBOOST_PP_ENUM(n, HBOOST_RANGE_combined_range_iterator, ~)> \
    > \
    combine(HBOOST_PP_ENUM(n, HBOOST_RANGE_combined_args, ~)) \
    { \
        typedef tuple< \
            HBOOST_PP_ENUM(n, HBOOST_RANGE_combined_range_iterator, ~) \
        > rng_tuple_t; \
