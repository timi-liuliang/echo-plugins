...
//
#ifndef HBOOST_RANGE_ALGORITHM_REMOVE_IF_HPP_INCLUDED
#define HBOOST_RANGE_ALGORITHM_REMOVE_IF_HPP_INCLUDED
...
inline HBOOST_DEDUCED_TYPENAME hboost::range_iterator<ForwardRange>::type
remove_if(ForwardRange& rng, UnaryPredicate pred)
{
    HBOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    return std::remove_if(hboost::begin(rng), hboost::end(rng), pred);
}

/// \overload
template< class ForwardRange, class UnaryPredicate >
inline HBOOST_DEDUCED_TYPENAME hboost::range_iterator<const ForwardRange>::type
...
remove_if(ForwardRange& rng, UnaryPredicate pred)
{
    ...
    return range_return<ForwardRange,re>::pack(
        std::remove_if(hboost::begin(rng), hboost::end(rng), pred),
        rng);
}
