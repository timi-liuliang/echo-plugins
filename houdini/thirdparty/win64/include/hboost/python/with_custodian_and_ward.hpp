...
#ifndef HBOOST_WITH_CUSTODIAN_AND_WARD_DWA2002131_HPP
# define HBOOST_WITH_CUSTODIAN_AND_WARD_DWA2002131_HPP

# include <hboost/python/detail/prefix.hpp>

# include <hboost/python/default_call_policies.hpp>
# include <hboost/python/object/life_support.hpp>
# include <algorithm>
...
{
    HBOOST_STATIC_ASSERT(custodian != ward);
    HBOOST_STATIC_ASSERT(custodian > 0);
    HBOOST_STATIC_ASSERT(ward > 0);
    ...
    static bool precall(ArgumentPackage const& args_)
    {
        ...
        if (custodian > arity_ || ward > arity_)
        {
            PyErr_SetString(
                PyExc_IndexError
              , "hboost::python::with_custodian_and_ward: argument index out of range"
            );
            return false;
        }
...
struct with_custodian_and_ward_postcall : BasePolicy_
{
    ...
    static PyObject* postcall(ArgumentPackage const& args_, PyObject* result)
    {
        ...
        if ( (std::max)(custodian, ward) > arity_ )
        {
            PyErr_SetString(
                PyExc_IndexError
              , "hboost::python::with_custodian_and_ward_postcall: argument index out of range"
            );
            return 0;
        }
...
#endif // HBOOST_WITH_CUSTODIAN_AND_WARD_DWA2002131_HPP
