...
#ifndef HBOOST_OBJECT_CORE_DWA2002615_HPP
# define HBOOST_OBJECT_CORE_DWA2002615_HPP

# define HBOOST_PYTHON_OBJECT_HAS_IS_NONE // added 2010-03-15 by rwgk
...
# include <hboost/python/detail/string_literal.hpp>
# include <hboost/python/detail/def_helper_fwd.hpp>
...
// attribute/item access or addition.
  ...
  //
  HBOOST_PYTHON_IS_XXX_DEF(proxy, hboost::python::api::proxy, 1)
...
# define HBOOST_PP_ITERATION_PARAMS_1 (3, (1, HBOOST_PYTHON_MAX_ARITY, <hboost/python/object_call.hpp>))
...
# endif
  ...
  PyObject* object_base_initializer(T const& x)
  {
      typedef typename is_derived<
          HBOOST_DEDUCED_TYPENAME objects::unforward_cref<T>::type
        , object
      >::type is_obj;

      return object_initializer<
          HBOOST_DEDUCED_TYPENAME unwrap_reference<T>::type
      >::get(
            x
          , is_obj()
      );
  ...
  class object : public object_base
  {
   public:
      ...
      HBOOST_PYTHON_DECL explicit object(handle<> const&);
   private:
...
# define HBOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS(derived, base)               \
    inline explicit derived(::hboost::python::detail::borrowed_reference p)     \
        : base(p) {}                                                           \
    inline explicit derived(::hboost::python::detail::new_reference p)          \
        : base(p) {}                                                           \
    inline explicit derived(::hboost::python::detail::new_non_null_reference p) \
        : base(p) {}
  ...
  struct object_initializer_impl<false, true>
  {
      ...
      get(T const& x, U)
      {
          return python::incref(get_managed_object(x, hboost::python::tag));
      }
  };
...
{ 
  ...
  return hboost::python::detail::args_proxy(x); 
} 
...
{ 
  ...
  PyObject *result = PyObject_Call(get_managed_object(self, hboost::python::tag), 
                                   args.operator object().ptr(), 
                                   0); 
  return object(hboost::python::detail::new_reference(result)); 
...
# include <hboost/python/slice_nil.hpp>

#endif // HBOOST_OBJECT_CORE_DWA2002615_HPP
