///////////////////////////////////////////////////////////////////////////////
//
// Copyright David Abrahams 2002, Joel de Guzman, 2002.
// Distributed under the Boost Software License, Version 1.0. (See
// accompanying file LICENSE_1_0.txt or copy at
// http://www.boost.org/LICENSE_1_0.txt)
//
///////////////////////////////////////////////////////////////////////////////
#if !defined(HBOOST_PP_IS_ITERATING)

# ifndef SIGNATURE_JDG20020813_HPP
#  define SIGNATURE_JDG20020813_HPP

# include <hboost/python/detail/prefix.hpp>

# include <hboost/mpl/if.hpp>
# include <hboost/type_traits/is_convertible.hpp>

#  include <hboost/python/detail/preprocessor.hpp>
#  include <hboost/preprocessor/repeat.hpp>
#  include <hboost/preprocessor/enum.hpp>
#  include <hboost/preprocessor/enum_params.hpp>
#  include <hboost/preprocessor/empty.hpp>
#  include <hboost/preprocessor/arithmetic/sub.hpp>
#  include <hboost/preprocessor/iterate.hpp>
#  include <hboost/python/detail/type_list.hpp>

#  include <hboost/preprocessor/debug/line.hpp>
#  include <hboost/preprocessor/arithmetic/sub.hpp>
#  include <hboost/preprocessor/arithmetic/inc.hpp>
#  include <hboost/preprocessor/repetition/enum_trailing_params.hpp>

# define HBOOST_PYTHON_LIST_INC(n)        \
   HBOOST_PP_CAT(mpl::vector, HBOOST_PP_INC(n))

///////////////////////////////////////////////////////////////////////////////
namespace hboost { namespace python { namespace detail {

// A metafunction returning C1 if C1 is derived from C2, and C2
// otherwise
template <class C1, class C2>
struct most_derived
{
    typedef typename mpl::if_<
        is_convertible<C1*,C2*>
      , C1
      , C2
    >::type type;
};

//  The following macros generate expansions for::
//
//      template <class RT, class T0... class TN>
//      inline mpl::vector<RT, T0...TN>
//      get_signature(RT(HBOOST_PYTHON_FN_CC *)(T0...TN), void* = 0)
//      {
//          return mpl::list<RT, T0...TN>();
//      }
//
//    where HBOOST_PYTHON_FN_CC is a calling convention keyword, can be
//
//        empty, for default calling convention
//        __cdecl (if HBOOST_PYTHON_ENABLE_CDECL is defined)
//        __stdcall (if HBOOST_PYTHON_ENABLE_STDCALL is defined)
//        __fastcall (if HBOOST_PYTHON_ENABLE_FASTCALL is defined)
//
//   And, for an appropriate assortment of cv-qualifications::
//
//      template <class RT, class ClassT, class T0... class TN>
//      inline mpl::vector<RT, ClassT&, T0...TN>
//      get_signature(RT(HBOOST_PYTHON_FN_CC ClassT::*)(T0...TN) cv))
//      {
//          return mpl::list<RT, ClassT&, T0...TN>();
//      }
//
//      template <class Target, class RT, class ClassT, class T0... class TN>
//      inline mpl::vector<
//          RT
//        , typename most_derived<Target, ClassT>::type&
//        , T0...TN
//      >
//      get_signature(RT(HBOOST_PYTHON_FN_CC ClassT::*)(T0...TN) cv), Target*)
//      {
//          return mpl::list<RT, ClassT&, T0...TN>();
//      }
//
//  There are two forms for invoking get_signature::
//
//      get_signature(f)
//
//  and ::
//
//      get_signature(f,(Target*)0)
//
//  These functions extract the return type, class (for member
//  functions) and arguments of the input signature and stuff them in
//  an mpl type sequence (the calling convention is dropped).
//  Note that cv-qualification is dropped from
//  the "hidden this" argument of member functions; that is a
//  necessary sacrifice to ensure that an lvalue from_python converter
//  is used.  A pointer is not used so that None will be rejected for
//  overload resolution.
//
//  The second form of get_signature essentially downcasts the "hidden
//  this" argument of member functions to Target, because the function
//  may actually be a member of a base class which is not wrapped, and
//  in that case conversion from python would fail.
//
// @group {

// 'default' calling convention

#  define HBOOST_PYTHON_FN_CC

#  define HBOOST_PP_ITERATION_PARAMS_1                                   \
    (3, (0, HBOOST_PYTHON_MAX_ARITY, <hboost/python/signature.hpp>))

#  include HBOOST_PP_ITERATE()

#  undef HBOOST_PYTHON_FN_CC

// __cdecl calling convention

#  if defined(HBOOST_PYTHON_ENABLE_CDECL)

#   define HBOOST_PYTHON_FN_CC __cdecl
#   define HBOOST_PYTHON_FN_CC_IS_CDECL

#   define HBOOST_PP_ITERATION_PARAMS_1                                   \
     (3, (0, HBOOST_PYTHON_MAX_ARITY, <hboost/python/signature.hpp>))

#   include HBOOST_PP_ITERATE()

#   undef HBOOST_PYTHON_FN_CC
#   undef HBOOST_PYTHON_FN_CC_IS_CDECL

#  endif // defined(HBOOST_PYTHON_ENABLE_CDECL)

// __stdcall calling convention

#  if defined(HBOOST_PYTHON_ENABLE_STDCALL)

#   define HBOOST_PYTHON_FN_CC __stdcall

#   define HBOOST_PP_ITERATION_PARAMS_1                                   \
     (3, (0, HBOOST_PYTHON_MAX_ARITY, <hboost/python/signature.hpp>))

#   include HBOOST_PP_ITERATE()

#   undef HBOOST_PYTHON_FN_CC

#  endif // defined(HBOOST_PYTHON_ENABLE_STDCALL)

// __fastcall calling convention

#  if defined(HBOOST_PYTHON_ENABLE_FASTCALL)

#   define HBOOST_PYTHON_FN_CC __fastcall

#   define HBOOST_PP_ITERATION_PARAMS_1                                   \
     (3, (0, HBOOST_PYTHON_MAX_ARITY, <hboost/python/signature.hpp>))

#   include HBOOST_PP_ITERATE()

#   undef HBOOST_PYTHON_FN_CC

#  endif // defined(HBOOST_PYTHON_ENABLE_FASTCALL)

#  undef HBOOST_PYTHON_LIST_INC

// }

}}} // namespace hboost::python::detail


# endif // SIGNATURE_JDG20020813_HPP

// For gcc 4.4 compatability, we must include the
// HBOOST_PP_ITERATION_DEPTH test inside an #else clause.
#else // HBOOST_PP_IS_ITERATING
#if HBOOST_PP_ITERATION_DEPTH() == 1 // defined(HBOOST_PP_IS_ITERATING)

# define N HBOOST_PP_ITERATION()

   // as 'get_signature(RT(*)(T0...TN), void* = 0)' is the same
   // function as 'get_signature(RT(__cdecl *)(T0...TN), void* = 0)',
   // we don't define it twice
#  if !defined(HBOOST_PYTHON_FN_CC_IS_CDECL)

template <
    class RT HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, class T)>
inline HBOOST_PYTHON_LIST_INC(N)<
    RT HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, T)>
get_signature(RT(HBOOST_PYTHON_FN_CC *)(HBOOST_PP_ENUM_PARAMS_Z(1, N, T)), void* = 0)
{
    return HBOOST_PYTHON_LIST_INC(N)<
            RT HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, T)
        >();
}

#  endif // !defined(HBOOST_PYTHON_FN_CC_IS_CDECL)

# undef N

# define HBOOST_PP_ITERATION_PARAMS_2 \
    (3, (0, 3, <hboost/python/signature.hpp>))
# include HBOOST_PP_ITERATE()

#else

# define N HBOOST_PP_RELATIVE_ITERATION(1)
# define Q HBOOST_PYTHON_CV_QUALIFIER(HBOOST_PP_ITERATION())

template <
    class RT, class ClassT HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, class T)>
inline HBOOST_PYTHON_LIST_INC(HBOOST_PP_INC(N))<
    RT, ClassT& HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, T)>
get_signature(RT(HBOOST_PYTHON_FN_CC ClassT::*)(HBOOST_PP_ENUM_PARAMS_Z(1, N, T)) Q)
{
    return HBOOST_PYTHON_LIST_INC(HBOOST_PP_INC(N))<
            RT, ClassT& HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, T)
        >();
}

template <
    class Target
  , class RT
  , class ClassT
    HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, class T)
>
inline HBOOST_PYTHON_LIST_INC(HBOOST_PP_INC(N))<
    RT
  , typename most_derived<Target, ClassT>::type&
    HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, T)
>
get_signature(
    RT(HBOOST_PYTHON_FN_CC ClassT::*)(HBOOST_PP_ENUM_PARAMS_Z(1, N, T)) Q
  , Target*
)
{
    return HBOOST_PYTHON_LIST_INC(HBOOST_PP_INC(N))<
        RT
      , HBOOST_DEDUCED_TYPENAME most_derived<Target, ClassT>::type&
        HBOOST_PP_ENUM_TRAILING_PARAMS_Z(1, N, T)
    >();
}

# undef Q
# undef N

#endif // HBOOST_PP_ITERATION_DEPTH()
#endif // !defined(HBOOST_PP_IS_ITERATING)
