...
#include <hboost/python/detail/defaults_gen.hpp>
...
#include <hboost/python/detail/scope.hpp>
...
namespace detail
{
  ...
  //      }
  //
  //  where N runs from 0 to HBOOST_PYTHON_MAX_ARITY.
  //
...
#define HBOOST_PP_ITERATION_PARAMS_1                                             \
    (3, (0, HBOOST_PYTHON_MAX_ARITY, <hboost/python/detail/defaults_def.hpp>))
...
#include HBOOST_PP_ITERATE()
      ...
      SigT const&)
  {
      ...
      typedef typename mpl::if_c<
          hboost::is_same<void, return_type>::value
          , void_return_type
...
template <>
struct define_stub_function<HBOOST_PP_ITERATION()> {
    template <class StubsT, class CallPolicies, class NameSpaceT>
    static void define(
        char const* name
        , StubsT const&
        ...
        , char const* doc)
    {
        detail::name_space_def(
            name_space
            , name
            , &StubsT::HBOOST_PP_CAT(func_, HBOOST_PP_ITERATION())
            , kw
            , policies
            , doc
            , &name_space);
