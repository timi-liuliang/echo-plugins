...
#ifndef HBOOST_OBJECT_PROTOCOL_DWA2002615_HPP
# define HBOOST_OBJECT_PROTOCOL_DWA2002615_HPP

# include <hboost/python/detail/prefix.hpp>

# include <hboost/python/object_protocol_core.hpp>
...
#  define HBOOST_PYTHON_NO_ARRAY_ARG(T)             , T (*)() = 0
# else 
#  define HBOOST_PYTHON_NO_ARRAY_ARG(T) 
# endif

template <class Target, class Key>
object getattr(Target const& target, Key const& key HBOOST_PYTHON_NO_ARRAY_ARG(Key))
{
    return getattr(object(target), object(key));
}

template <class Target, class Key, class Default>
object getattr(Target const& target, Key const& key, Default const& default_ HBOOST_PYTHON_NO_ARRAY_ARG(Key))
{
...
void setattr(object const& target, Key const& key, Value const& value HBOOST_PYTHON_NO_ARRAY_ARG(Key))
{
    setattr(target, object(key), object(value));
}

template <class Key>
void delattr(object const& target, Key const& key HBOOST_PYTHON_NO_ARRAY_ARG(Key))
{
    delattr(target, object(key));
}

template <class Target, class Key>
object getitem(Target const& target, Key const& key HBOOST_PYTHON_NO_ARRAY_ARG(Key))
{
...
template <class Key, class Value>
void setitem(object const& target, Key const& key, Value const& value HBOOST_PYTHON_NO_ARRAY_ARG(Key))
{
    setitem(target, object(key), object(value));
}

template <class Key>
void delitem(object const& target, Key const& key HBOOST_PYTHON_NO_ARRAY_ARG(Key))
{
...
#endif // HBOOST_OBJECT_PROTOCOL_DWA2002615_HPP
