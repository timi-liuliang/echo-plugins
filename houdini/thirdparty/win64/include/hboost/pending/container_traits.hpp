...
#ifndef HBOOST_GRAPH_DETAIL_CONTAINER_TRAITS_H
#define HBOOST_GRAPH_DETAIL_CONTAINER_TRAITS_H
...
#ifndef HBOOST_NO_CXX11_HDR_UNORDERED_SET
#include <unordered_set>
#endif

#ifndef HBOOST_NO_CXX11_HDR_UNORDERED_MAP
#include <unordered_map>
#endif

#ifdef HBOOST_NO_CXX11_RVALUE_REFERENCES
#define HBOOST_PENDING_FWD_TYPE(type) const type&
#define HBOOST_PENDING_FWD_VALUE(type, var) (var)
#else
#define HBOOST_PENDING_FWD_TYPE(type) type&&
#define HBOOST_PENDING_FWD_VALUE(type, var) (std::forward<type>((var)))
#endif

// The content of this file is in 'graph_detail' because otherwise
// there will be name clashes with 
// sandbox/hboost/sequence_algo/container_traits.hpp
// The 'detail' subnamespace will still cause problems.
namespace hboost { namespace graph_detail {
  ...
  // std::slist
#ifndef HBOOST_NO_SLIST
  template <class T, class Alloc>
  struct container_traits<HBOOST_STD_EXTENSION_NAMESPACE::slist<T,Alloc> > {
    typedef front_insertion_sequence_tag category;
    typedef stable_tag iterator_stability;
  };
  template <class T, class Alloc>
  front_insertion_sequence_tag container_category(
  const HBOOST_STD_EXTENSION_NAMESPACE::slist<T,Alloc>&
  )
 ...
 // hash_set, hash_map
  ...
  struct container_traits< hboost::unordered_set<Key,Eq,Hash,Alloc> > {
    typedef unordered_set_tag category;
    typedef unstable_tag iterator_stability;
  };
  template <class Key, class T, class Eq, class Hash, class Alloc>
  struct container_traits< hboost::unordered_map<Key,T,Eq,Hash,Alloc> > {
    typedef unordered_map_tag category;
    typedef unstable_tag iterator_stability;
  };
  template <class Key, class Eq, class Hash, class Alloc>
  struct container_traits< hboost::unordered_multiset<Key,Eq,Hash,Alloc> > {
    typedef unordered_multiset_tag category;
    typedef unstable_tag iterator_stability;
  };
  template <class Key, class T, class Eq, class Hash, class Alloc>
  struct container_traits< hboost::unordered_multimap<Key,T,Eq,Hash,Alloc> > {
  ...
  template <class Key, class Eq, class Hash, class Alloc>
  unordered_set_tag
  container_category(const hboost::unordered_set<Key,Eq,Hash,Alloc>&)
  { return unordered_set_tag(); }

  template <class Key, class T, class Eq, class Hash, class Alloc>
  unordered_map_tag
  container_category(const hboost::unordered_map<Key,T,Eq,Hash,Alloc>&)
  { return unordered_map_tag(); }

  template <class Key, class Eq, class Hash, class Alloc>
  unstable_tag iterator_stability(const hboost::unordered_set<Key,Eq,Hash,Alloc>&)
  { return unstable_tag(); }

  template <class Key, class T, class Eq, class Hash, class Alloc>
  unstable_tag iterator_stability(const hboost::unordered_map<Key,T,Eq,Hash,Alloc>&)
  { return unstable_tag(); }
  template <class Key, class Eq, class Hash, class Alloc>
  unordered_multiset_tag
  container_category(const hboost::unordered_multiset<Key,Eq,Hash,Alloc>&)
  { return unordered_multiset_tag(); }

  template <class Key, class T, class Eq, class Hash, class Alloc>
  unordered_multimap_tag
  container_category(const hboost::unordered_multimap<Key,T,Eq,Hash,Alloc>&)
  { return unordered_multimap_tag(); }

  template <class Key, class Eq, class Hash, class Alloc>
  unstable_tag
  iterator_stability(const hboost::unordered_multiset<Key,Eq,Hash,Alloc>&)
  { return unstable_tag(); }

  template <class Key, class T, class Eq, class Hash, class Alloc>
  unstable_tag
  iterator_stability(const hboost::unordered_multimap<Key,T,Eq,Hash,Alloc>&)
  { return unstable_tag(); }
  ...
  push_dispatch(Container& c, HBOOST_PENDING_FWD_TYPE(T) v, back_insertion_sequence_tag)
  {
    c.push_back(HBOOST_PENDING_FWD_VALUE(T, v));
    return std::make_pair(hboost::prior(c.end()), true);
  }

  template <class Container, class T>
  std::pair<typename Container::iterator, bool>
  push_dispatch(Container& c, HBOOST_PENDING_FWD_TYPE(T) v, front_insertion_sequence_tag)
  {
    c.push_front(HBOOST_PENDING_FWD_VALUE(T, v));
    return std::make_pair(c.begin(), true);
  }

  template <class AssociativeContainer, class T>
  std::pair<typename AssociativeContainer::iterator, bool>
  push_dispatch(AssociativeContainer& c, HBOOST_PENDING_FWD_TYPE(T) v, 
                unique_associative_container_tag)
  {
    return c.insert(HBOOST_PENDING_FWD_VALUE(T, v));
  }
  ...
  {
    return std::make_pair(c.insert(HBOOST_PENDING_FWD_VALUE(T, v)), true);
  }

  template <class Container, class T>
  std::pair<typename Container::iterator,bool>
  push(Container& c, HBOOST_PENDING_FWD_TYPE(T) v)
  {
    return push_dispatch(c, HBOOST_PENDING_FWD_VALUE(T, v), container_category(c));
  }
...
}} // namespace hboost::graph_detail

#undef HBOOST_PENDING_FWD_TYPE
#undef HBOOST_PENDING_FWD_VALUE

#endif // HBOOST_GRAPH_DETAIL_CONTAINER_TRAITS_H
