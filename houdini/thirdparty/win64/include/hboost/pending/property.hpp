...
#ifndef HBOOST_PROPERTY_HPP
#define HBOOST_PROPERTY_HPP
...
namespace hboost {
  ...
  namespace graph_introspect_detail {
    HBOOST_MPL_HAS_XXX_TRAIT_DEF(kind)
  ...
  struct lookup_one_property_internal {HBOOST_STATIC_CONSTANT(bool, found = false); typedef void type;};

  // Special-case properties (vertex_all, edge_all, graph_all)
#define BGL_ALL_PROP(tag) \
  template <typename T> \
  struct lookup_one_property_internal<T, tag> { \
    HBOOST_STATIC_CONSTANT(bool, found = true); \
    typedef T type; \
...
#undef BGL_ALL_PROP
  ...
  template <typename T> \
  struct lookup_one_property_internal<T, HBOOST_JOIN(kind, _bundle_t)> { \
    HBOOST_STATIC_CONSTANT(bool, found = true); \
    typedef T type; \
    static T& lookup(T& x, HBOOST_JOIN(kind, _bundle_t)) {return x;} \
    static const T& lookup(const T& x, HBOOST_JOIN(kind, _bundle_t)) {return x;} \
  }; \
 \
  template <typename Tag, typename T, typename Base> \
  struct lookup_one_property_internal<property<Tag, T, Base>, HBOOST_JOIN(kind, _bundle_t)>: lookup_one_property_internal<Base, HBOOST_JOIN(kind, _bundle_t)> { \
    private: \
    typedef lookup_one_property_internal<Base, HBOOST_JOIN(kind, _bundle_t)> base_type; \
    public: \
    template <typename BundleTag> \
    static typename lazy_enable_if_c<(base_type::found && (is_same<BundleTag, HBOOST_JOIN(kind, _bundle_t)>::value)), \
                                     add_reference<typename base_type::type> >::type \
    lookup(property<Tag, T, Base>& p, BundleTag) {return base_type::lookup(p.m_base, HBOOST_JOIN(kind, _bundle_t)());} \
    template <typename BundleTag> \
    static typename lazy_enable_if_c<(base_type::found && (is_same<BundleTag, HBOOST_JOIN(kind, _bundle_t)>::value)), \
                                     add_reference<const typename base_type::type> >::type \
    lookup(const property<Tag, T, Base>& p, BundleTag) {return base_type::lookup(p.m_base, HBOOST_JOIN(kind, _bundle_t)());} \
  }; \
...
#undef BGL_DO_ONE_BUNDLE_TYPE
  ...
  template <typename Tag, typename T, typename Base>
  struct lookup_one_property_internal<hboost::property<Tag, T, Base>, Tag> {
    HBOOST_STATIC_CONSTANT(bool, found = true);
  ...
  struct lookup_one_property_internal<hboost::property<Tag, T, Base>, PropName>: lookup_one_property_internal<Base, PropName> {
    private:
    typedef lookup_one_property_internal<Base, PropName> base_type;
    public:
    template <typename PL>
    static typename lazy_enable_if<is_same<PL, hboost::property<Tag, T, Base> >,
                                   add_reference<typename base_type::type> >::type
  ...
  struct lookup_one_property<const T, Tag> {
    HBOOST_STATIC_CONSTANT(bool, found = (lookup_one_property_internal<T, Tag>::found));
  ...
  template <class P>
  struct has_property : hboost::mpl::true_ {};
  template <>
  struct has_property<no_property> : hboost::mpl::false_ {};

} // namespace hboost

#include <hboost/pending/detail/property.hpp>
...
namespace detail {
  ...
  struct remove_first_property {
    template <typename F>
    struct result {
      typedef typename hboost::function_traits<F>::arg1_type a1;
      typedef typename hboost::remove_reference<a1>::type non_ref;
      typedef typename non_ref::next_type nx;
      typedef typename hboost::mpl::if_<hboost::is_const<non_ref>, hboost::add_const<nx>, nx>::type with_const;
      typedef typename hboost::add_reference<with_const>::type type;
    };
