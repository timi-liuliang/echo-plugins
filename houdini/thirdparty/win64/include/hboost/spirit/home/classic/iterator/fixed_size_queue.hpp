...
#define HBOOST_SPIRIT_ASSERT_FSQ_SIZE \
    HBOOST_SPIRIT_ASSERT(((m_tail + N + 1) - m_head) % (N+1) == m_size % (N+1)) \
    /**/
...
#if !defined(HBOOST_ITERATOR_ADAPTORS_VERSION) || \
    HBOOST_ITERATOR_ADAPTORS_VERSION < 0x0200
#error "Please use at least Boost V1.31.0 while compiling the fixed_size_queue class!"
#else // HBOOST_ITERATOR_ADAPTORS_VERSION < 0x0200
...
private:
        ...
        const &x) const
    {
        ...
        //  containers
        HBOOST_SPIRIT_ASSERT(p.self == p2.self);
...
#endif // HBOOST_ITERATOR_ADAPTORS_VERSION < 0x0200
...
private:
    ...
    HBOOST_STATIC_CONSTANT(std::size_t, MAX_SIZE = N);
...
fixed_size_queue<T, N>::fixed_size_queue()
    ...
    , m_size(0)
{
    HBOOST_SPIRIT_ASSERT(m_size <= N+1);
    HBOOST_SPIRIT_ASSERT_FSQ_SIZE;
    HBOOST_SPIRIT_ASSERT(m_head <= N+1);
    HBOOST_SPIRIT_ASSERT(m_tail <= N+1);
}
...
#undef HBOOST_SPIRIT_ASSERT_FSQ_SIZE
