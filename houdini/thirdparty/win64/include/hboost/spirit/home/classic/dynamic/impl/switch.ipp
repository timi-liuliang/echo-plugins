...
=============================================================================*/
#ifndef HBOOST_SPIRIT_SWITCH_IPP
#define HBOOST_SPIRIT_SWITCH_IPP
...
struct default_delegate_parse<N, true, HasDefault> {
        ...
        ScannerT const &scan, typename ScannerT::iterator_t const save)
    {
        ...
        HBOOST_STATIC_ASSERT((!hboost::is_same<DefaultT, nothing_parser>::value));
...
struct default_case {
    ...
    HBOOST_STATIC_CONSTANT(bool, is_epsilon = (
    ...
    //  case branch).
    typedef typename hboost::mpl::if_c<
...
struct default_case<CaseT, true> {
    ...
    HBOOST_STATIC_CONSTANT(bool, value = CaseT::is_default);
    HBOOST_STATIC_CONSTANT(bool, is_epsilon = (
        CaseT::is_default && CaseT::is_epsilon
    ));
...
struct case_chain {
    ...
    HBOOST_STATIC_CONSTANT(int, depth = (
...
struct case_chain<CaseT, true> {
    ...
    HBOOST_STATIC_CONSTANT(int, depth = 0);
};
        ...
        parser<compound_case_parser<LeftT, RightT, IsDefault> > >
{
    ...
    HBOOST_STATIC_CONSTANT(int, value = RightT::value);
    HBOOST_STATIC_CONSTANT(bool, is_default = IsDefault);
    HBOOST_STATIC_CONSTANT(bool, is_simple = false);
    HBOOST_STATIC_CONSTANT(bool, is_epsilon = (
        is_default &&
            hboost::is_same<typename RightT::subject_t, epsilon_parser>::value
    ));
    ...
    operator, (case_parser<N1, ParserT1, IsDefault1> const &p) const
    {
        ...
        HBOOST_STATIC_ASSERT(!default_case<self_t>::value || !IsDefault1);
        ...
        HBOOST_STATIC_ASSERT(
            case_chain<self_t>::depth < HBOOST_SPIRIT_SWITCH_CASE_LIMIT
        );
...
//
///////////////////////////////////////////////////////////////////////////////
#define HBOOST_SPIRIT_PARSE_SWITCH_TYPEDEFS(z, N, _)                         \
    typedef typename HBOOST_PP_CAT(left_t, N)::left_t                        \
        HBOOST_PP_CAT(left_t, HBOOST_PP_INC(N));                              \
    /**/

#define HBOOST_SPIRIT_PARSE_SWITCH_CASES(z, N, _)                            \
    case (long)(HBOOST_PP_CAT(left_t, N)::value):                            \
        return delegate_parse(chain_parser<N, left_t1>::right(p.left()),    \
            scan, save);                                                    \
    /**/

#define HBOOST_SPIRIT_PARSE_SWITCHES(z, N, _)                                \
    template <int Value, bool IsDefault>                                    \
    struct parse_switch<Value, HBOOST_PP_INC(N), IsDefault> {                \
                                                                            \
        template <typename ParserT, typename ScannerT>                      \
        static typename parser_result<ParserT, ScannerT>::type              \
        do_(ParserT const &p, ScannerT const &scan, long cond_value,        \
        ...
        {                                                                   \
            typedef ParserT left_t0;                                        \
            HBOOST_PP_REPEAT_FROM_TO_ ## z(0, HBOOST_PP_INC(N),               \
                HBOOST_SPIRIT_PARSE_SWITCH_TYPEDEFS, _)                      \
                                                                            \
            switch (cond_value) {                                           \
            case (long)(HBOOST_PP_CAT(left_t, HBOOST_PP_INC(N))::value):      \
                return delegate_parse(                                      \
                    chain_parser<                                           \
                        case_chain<ParserT>::depth, ParserT                 \
                    >::left(p), scan, save);                                \
                                                                            \
            HBOOST_PP_REPEAT_FROM_TO_ ## z(1, HBOOST_PP_INC(N),               \
                HBOOST_SPIRIT_PARSE_SWITCH_CASES, _)                         \
                                                                            \
...
HBOOST_PP_REPEAT(HBOOST_PP_DEC(HBOOST_SPIRIT_SWITCH_CASE_LIMIT),
    HBOOST_SPIRIT_PARSE_SWITCHES, _)

#undef HBOOST_SPIRIT_PARSE_SWITCH_TYPEDEFS
#undef HBOOST_SPIRIT_PARSE_SWITCH_CASES
#undef HBOOST_SPIRIT_PARSE_SWITCHES
///////////////////////////////////////////////////////////////////////////////
...
struct cond_actor {
    ...
    struct result
    {
        typedef typename ::hboostphoenix::actor_result<ActorT, ::hboostphoenix::tuple<> >::type
            type;
    };
...
template <typename ActorT>
struct make_cond_functor< ::hboostphoenix::actor<ActorT> > {

    static cond_actor< ::hboostphoenix::actor<ActorT> >
    do_(::hboostphoenix::actor<ActorT> const &actor)
    {
        return cond_actor< ::hboostphoenix::actor<ActorT> >(actor);
    }
};
...
#endif  // HBOOST_SPIRIT_SWITCH_IPP
