...
#ifndef HBOOST_FT_COMPONENTS_HPP_INCLUDED
#define HBOOST_FT_COMPONENTS_HPP_INCLUDED
...
#   include <hboost/function_types/detail/classifier.hpp>
#endif
...
#   elif HBOOST_FT_MAX_ARITY < 50
#     include <hboost/mpl/vector/vector50.hpp>
#   endif

#include <hboost/function_types/detail/class_transform.hpp>
...
{ 
  namespace function_types 
  {
    ...
    { 
      ...
      HBOOST_MPL_AUX_LAMBDA_SUPPORT(2,components,(T,ClassTypeTransform))
    };
...
#if !HBOOST_FT_NO_CV_FUNC_SUPPORT
...
#if !HBOOST_WORKAROUND(__BORLANDC__, <= 0x565)
#   define HBOOST_FT_variations HBOOST_FT_pointer|HBOOST_FT_member_pointer
...
#else  
#   define HBOOST_FT_variations HBOOST_FT_pointer
    ...
    struct components_impl<T C::*, L>
      : detail::retagged_if
        < detail::components_impl<typename hboost::remove_cv<T>::type *, L>
    ...
    { typedef char (& type)[1]; HBOOST_STATIC_CONSTANT(std::size_t, value = 1); };
    template<typename T> struct encode_cv<T const *>
    { typedef char (& type)[2]; HBOOST_STATIC_CONSTANT(std::size_t, value = 2); };
    template<typename T> struct encode_cv<T volatile *>
    { typedef char (& type)[3]; HBOOST_STATIC_CONSTANT(std::size_t, value = 3); };
    template<typename T> struct encode_cv<T const volatile *> 
    { typedef char (& type)[4]; HBOOST_STATIC_CONSTANT(std::size_t, value = 4); };
    ...
    template<typename T> struct cv_tag_mfp
      : detail::cv_tag_mfp_impl
        < ::hboost::function_types::detail::encode_mfp_cv<T>::value >
    { };
    ...
    template<typename T, typename D, typename L> struct bcc_class_transform 
      : bcc_class_transform_impl
        < typename decode_cv
          < T
          , ::hboost::function_types::detail::encode_mfp_cv<D>::value 
          >::type
        , L
        > 
    { };
...
#define HBOOST_FT_al_path hboost/function_types/detail/components_impl
...
#include <hboost/function_types/detail/components_as_mpl_sequence.hpp>
#include <hboost/function_types/detail/retag_default_cc.hpp>
