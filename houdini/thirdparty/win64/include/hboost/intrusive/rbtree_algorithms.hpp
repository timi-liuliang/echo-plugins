...
#ifndef HBOOST_INTRUSIVE_RBTREE_ALGORITHMS_HPP
#define HBOOST_INTRUSIVE_RBTREE_ALGORITHMS_HPP
   ...
   : public bstree_node_checker<ValueTraits, NodePtrCompare, ExtraChecker>
{
                     ...
                     return_type& check_return)
   {
      ...
      if (node_traits::get_color(p) == node_traits::red()){
         ...
         HBOOST_INTRUSIVE_INVARIANT_ASSERT(!p_left  || node_traits::get_color(p_left)  == node_traits::black());
         HBOOST_INTRUSIVE_INVARIANT_ASSERT(!p_right || node_traits::get_color(p_right) == node_traits::black());
         //Red node can't be root
         HBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_parent(node_traits::get_parent(p)) != p);
      }
      //Every path to p contains the same number of black nodes
      const std::size_t l_black_count = check_return_left.black_count_;
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(l_black_count == check_return_right.black_count_);
   ...
   #endif
{
   ...
   //! @copydoc ::hboost::intrusive::bstree_algorithms::unlink(const node_ptr&)
   ...
   static void rebalance_after_erasure(const node_ptr & header, node_ptr x, node_ptr x_parent)
   {
      while(1){
         ...
         if(x == x_parent_left){ //x is left child
            ...
            HBOOST_INTRUSIVE_INVARIANT_ASSERT(w);
...
#endif //HBOOST_INTRUSIVE_RBTREE_ALGORITHMS_HPP
