...
#ifndef HBOOST_INTRUSIVE_SLIST_HPP
#define HBOOST_INTRUSIVE_SLIST_HPP
...
#include <hboost/intrusive/slist_hook.hpp>
#include <hboost/intrusive/circular_slist_algorithms.hpp>
#include <hboost/intrusive/linear_slist_algorithms.hpp>
...
#include <hboost/intrusive/detail/uncast.hpp>
#include <hboost/intrusive/detail/mpl.hpp>
#include <hboost/intrusive/detail/iterator.hpp>
#include <hboost/intrusive/detail/slist_iterator.hpp>
#include <hboost/intrusive/detail/array_initializer.hpp>
#include <hboost/intrusive/detail/exception_disposer.hpp>
#include <hboost/intrusive/detail/equal_to_value.hpp>
...
class slist_impl
{
   ...
   //noncopyable
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(slist_impl)
   ...
   HBOOST_STATIC_ASSERT(!(linear && ((int)value_traits::link_mode == (int)auto_unlink)));
   //A list with cached last node is incompatible with auto-unlink hooks!
   HBOOST_STATIC_ASSERT(!(cache_last && ((int)value_traits::link_mode == (int)auto_unlink)));
   ...
   static node_ptr get_last_node(detail::bool_<false>)
   {
      ...
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(cache_last);
      return node_ptr();
   }
   ...
   //! <b>Effects</b>: to-do
   //!
   slist_impl(HBOOST_RV_REF(slist_impl) x)
      : data_(::hboost::move(x.priv_value_traits()))
   {
      this->priv_size_traits().set_size(size_type(0));
      node_algorithms::init_header(this->get_root_node());
      //nothrow, no need to rollback to release elements on exception
   ...
   //! <b>Effects</b>: to-do
   //!
   slist_impl& operator=(HBOOST_RV_REF(slist_impl) x)
   ...
   void push_front(reference value)
   {
      ...
      HBOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(!safemode_or_autounlink || node_algorithms::inited(to_insert));
      if(cache_last){
         if(this->empty()){
   ...
   {
      HBOOST_STATIC_ASSERT((cache_last));
      node_ptr n = priv_value_traits().to_node_ptr(value);
      HBOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(!safemode_or_autounlink || node_algorithms::inited(n));
   ...
   //! <b>Throws</b>: If cloner throws.
   template <class Cloner, class Disposer>
   void clone_from(HBOOST_RV_REF(slist_impl) src, Cloner cloner, Disposer disposer)
   {
      this->clear_and_dispose(disposer);
      detail::exception_disposer<slist_impl, Disposer>
         rollback(*this, disposer);
   ...
   {
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(node_algorithms::distance((++const_iterator(before_f)).pointed_node(), l.pointed_node()) == n);
      if(safemode_or_autounlink){
   ...
   {
      HBOOST_STATIC_ASSERT(((!cache_last)&&(!constant_time_size)&&(!stateful_value_traits)));
   ...
   {
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(node_algorithms::distance(before_f.pointed_node(), before_l.pointed_node()) == n);
      this->priv_splice_after
         (prev_pos.pointed_node(), x, before_f.pointed_node(), before_l.pointed_node());
      if(constant_time_size){
   ...
   void sort(Predicate p)
   {
      if (node_traits::get_next(node_traits::get_next(this->get_root_node()))
         ...
         while(!this->empty()){
            ...
            HBOOST_INTRUSIVE_INVARIANT_ASSERT(counter[i].empty());
   ...
   {
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(linear || !node_algorithms::inited(this->priv_value_traits().to_node_ptr(value)));
   ...
   {
      reference r =*detail::uncast(pointer_traits<const_pointer>::pointer_to(value));
      HBOOST_INTRUSIVE_INVARIANT_ASSERT (linear || !node_algorithms::inited(this->priv_value_traits().to_node_ptr(r)));
   ...
   {
      if(n){
         HBOOST_INTRUSIVE_INVARIANT_ASSERT(n > 0);
         HBOOST_INTRUSIVE_INVARIANT_ASSERT
            (size_type(hboost::intrusive::iterator_distance
   ...
   void check() const
   {
      ...
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_next(header_ptr));
      if (node_traits::get_next(header_ptr) == header_ptr)
      {
         if (constant_time_size)
            HBOOST_INTRUSIVE_INVARIANT_ASSERT(this->priv_size_traits().get_size() == 0);
         return;
      }
      size_t node_count = 0;
      ...
      while (true)
      {
         ...
         if (!linear)
         {
            HBOOST_INTRUSIVE_INVARIANT_ASSERT(next_p);
         }
         else
         {
            HBOOST_INTRUSIVE_INVARIANT_ASSERT(next_p != header_ptr);
         }
         if ((!linear && next_p == header_ptr) || (linear && !next_p))
         {
            if (cache_last)
               HBOOST_INTRUSIVE_INVARIANT_ASSERT(get_last_node() == p);
            break;
         }
         p = next_p;
         ++node_count;
      ...
      if (constant_time_size)
         HBOOST_INTRUSIVE_INVARIANT_ASSERT(this->priv_size_traits().get_size() == node_count);
   }
      ...
      >::type
{
   ...
   HBOOST_STATIC_ASSERT((detail::is_same<typename Base::value_traits::value_type, T>::value));
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(slist)
   ...
   {  return static_cast<slist &>(this->Base::operator=(HBOOST_MOVE_BASE(Base, x)));  }
   ...
   template <class Cloner, class Disposer>
   void clone_from(HBOOST_RV_REF(slist) src, Cloner cloner, Disposer disposer)
...
#endif //HBOOST_INTRUSIVE_SLIST_HPP
