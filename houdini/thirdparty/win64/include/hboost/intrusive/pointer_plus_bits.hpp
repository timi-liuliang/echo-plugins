...
#ifndef HBOOST_INTRUSIVE_POINTER_PLUS_BITS_HPP
#define HBOOST_INTRUSIVE_POINTER_PLUS_BITS_HPP

#include <hboost/intrusive/detail/config_begin.hpp>
#include <hboost/intrusive/intrusive_fwd.hpp>
#include <hboost/intrusive/detail/mpl.hpp> //ls_zeros
#include <hboost/intrusive/detail/assert.hpp> //HBOOST_INTRUSIVE_INVARIANT_ASSERT
...
struct pointer_plus_bits<T*, NumBits>
{
   ...
   HBOOST_INTRUSIVE_FORCEINLINE static pointer get_pointer(pointer n)
   {  return pointer(uintptr_t(n) & uintptr_t(~Mask));  }

   HBOOST_INTRUSIVE_FORCEINLINE static void set_pointer(pointer &n, pointer p)
   {
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(0 == (uintptr_t(p) & Mask));
      n = pointer(uintptr_t(p) | (uintptr_t(n) & Mask));
   }

   HBOOST_INTRUSIVE_FORCEINLINE static std::size_t get_bits(pointer n)
   {  return std::size_t(uintptr_t(n) & Mask);  }

   HBOOST_INTRUSIVE_FORCEINLINE static void set_bits(pointer &n, std::size_t c)
   {
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(uintptr_t(c) <= Mask);
...
#endif //HBOOST_INTRUSIVE_POINTER_PLUS_BITS_HPP
