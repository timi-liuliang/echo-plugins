...
#ifndef HBOOST_INTRUSIVE_TREE_ITERATOR_HPP
#define HBOOST_INTRUSIVE_TREE_ITERATOR_HPP
...
class tree_iterator
{
   ...
   HBOOST_INTRUSIVE_FORCEINLINE tree_iterator()
   {}

   HBOOST_INTRUSIVE_FORCEINLINE explicit tree_iterator(const node_ptr & nodeptr, const const_value_traits_ptr &traits_ptr)
      : members_(nodeptr, traits_ptr)
   {}

   HBOOST_INTRUSIVE_FORCEINLINE tree_iterator(tree_iterator<value_traits, false> const& other)
   ...
   HBOOST_INTRUSIVE_FORCEINLINE tree_iterator &operator=(const node_ptr &nodeptr)
   ...
   HBOOST_INTRUSIVE_FORCEINLINE tree_iterator&  go_left()
   {
      members_.nodeptr_ = node_traits::get_left(members_.nodeptr_);
      return static_cast<tree_iterator&> (*this);
   }

   HBOOST_INTRUSIVE_FORCEINLINE tree_iterator&  go_right()
   {
      members_.nodeptr_ = node_traits::get_right(members_.nodeptr_);
      return static_cast<tree_iterator&> (*this);
   }

   HBOOST_INTRUSIVE_FORCEINLINE tree_iterator&  go_parent()
   {
      members_.nodeptr_ = node_traits::get_parent(members_.nodeptr_);
      return static_cast<tree_iterator&> (*this);
   }

   HBOOST_INTRUSIVE_FORCEINLINE operator unspecified_bool_type() const
   {  return members_.nodeptr_ ? &tree_iterator::unspecified_bool_type_func : 0;   }

   HBOOST_INTRUSIVE_FORCEINLINE bool operator! () const
   {  return !members_.nodeptr_;   }

   HBOOST_INTRUSIVE_FORCEINLINE friend bool operator== (const tree_iterator& l, const tree_iterator& r)
...
#endif //HBOOST_INTRUSIVE_TREE_ITERATOR_HPP
