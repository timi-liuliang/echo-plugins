...
//
/////////////////////////////////////////////////////////////////////////////
#ifndef HBOOST_INTRUSIVE_BSTREE_HPP
#define HBOOST_INTRUSIVE_BSTREE_HPP
...
#include <hboost/intrusive/detail/tree_iterator.hpp>
...
#include <hboost/intrusive/detail/empty_node_checker.hpp>
#include <hboost/intrusive/detail/default_header_holder.hpp>
#include <hboost/intrusive/detail/reverse_iterator.hpp>
#include <hboost/intrusive/detail/exception_disposer.hpp>
#include <hboost/intrusive/detail/node_cloner_disposer.hpp>
#include <hboost/intrusive/detail/key_nodeptr_comp.hpp>
#include <hboost/intrusive/detail/simple_disposers.hpp>
#include <hboost/intrusive/detail/size_holder.hpp>
#include <hboost/intrusive/detail/algo_type.hpp>
#include <hboost/intrusive/detail/algorithm.hpp>
#include <hboost/intrusive/detail/tree_value_compare.hpp>
...
#include <hboost/intrusive/parent_from_member.hpp>
...
#include <hboost/intrusive/detail/minimal_less_equal_header.hpp>//less, equal_to
...
struct bstbase3
{
   ...
   typedef hboost::intrusive::reverse_iterator<iterator>                                         reverse_iterator;
   typedef hboost::intrusive::reverse_iterator<const_iterator>                                   const_reverse_iterator;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename value_traits::pointer)                               pointer;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename value_traits::const_pointer)                         const_pointer;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<pointer>::element_type)               value_type;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<pointer>::reference)                  reference;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<const_pointer>::reference)            const_reference;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<const_pointer>::difference_type)      difference_type;
   ...
   {
      HBOOST_STATIC_ASSERT(has_container_from_iterator);
   ...
   typedef typename hboost::intrusive::value_traits_pointers
   ...
   {
      HBOOST_STATIC_ASSERT((!stateful_value_traits));
...
{
   typedef ::hboost::intrusive::detail::identity<T> type;
};
   ...
   :  public bstbase<ValueTraits, VoidOrKeyOfValue, VoidOrKeyComp, ConstantTimeSize, SizeType, AlgoType, HeaderHolder>
{
   ...
   typedef HBOOST_INTRUSIVE_IMPDEF(ValueTraits)                                                  value_traits;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename value_traits::pointer)                               pointer;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename value_traits::const_pointer)                         const_pointer;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<pointer>::element_type)               value_type;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename data_type::key_type)                                 key_type;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename data_type::key_of_value)                             key_of_value;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<pointer>::reference)                  reference;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<const_pointer>::reference)            const_reference;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename pointer_traits<const_pointer>::difference_type)      difference_type;
   typedef HBOOST_INTRUSIVE_IMPDEF(SizeType)                                                     size_type;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename data_type::value_compare)                            value_compare;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename data_type::key_compare)                              key_compare;
   typedef HBOOST_INTRUSIVE_IMPDEF(iterator_type)                                                iterator;
   typedef HBOOST_INTRUSIVE_IMPDEF(const_iterator_type)                                          const_iterator;
   typedef HBOOST_INTRUSIVE_IMPDEF(hboost::intrusive::reverse_iterator<iterator>)                 reverse_iterator;
   typedef HBOOST_INTRUSIVE_IMPDEF(hboost::intrusive::reverse_iterator<const_iterator>)           const_reverse_iterator;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename value_traits::node_traits)                           node_traits;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename node_traits::node)                                   node;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename node_traits::node_ptr)                               node_ptr;
   typedef HBOOST_INTRUSIVE_IMPDEF(typename node_traits::const_node_ptr)                         const_node_ptr;
   /// @cond
   typedef typename get_algo<AlgoType, node_traits>::type                                       algo_type;
   /// @endcond
   typedef HBOOST_INTRUSIVE_IMPDEF(algo_type)                                                    node_algorithms;
   ...
   //noncopyable
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(bstree_impl)

   static const bool safemode_or_autounlink = is_safe_autounlink<value_traits::link_mode>::value;

   //Constant-time size is incompatible with auto-unlink hooks!
   HBOOST_STATIC_ASSERT(!(constant_time_size && ((int)value_traits::link_mode == (int)auto_unlink)));
   ...
   //! <b>Effects</b>: to-do
   //!
   bstree_impl(HBOOST_RV_REF(bstree_impl) x)
      : data_type(::hboost::move(x.comp()), ::hboost::move(x.get_value_traits()))
   {
      this->swap(x);
   }
   ...
   //! <b>Effects</b>: to-do
   //!
   bstree_impl& operator=(HBOOST_RV_REF(bstree_impl) x)
   ...
   {
      //This can throw
      ::hboost::adl_move_swap(this->comp(), this->comp());
      //These can't throw
   ...
   //!    move semantics.
   template <class Cloner, class Disposer>
   void clone_from(HBOOST_RV_REF(bstree_impl) src, Cloner cloner, Disposer disposer)
   {
      this->clear_and_dispose(disposer);
      if(!src.empty()){
         detail::exception_disposer<bstree_impl, Disposer>
   ...
   iterator insert_unique_commit(reference value, const insert_commit_data &commit_data)
   {
      ...
      HBOOST_ASSERT(( p == this->end()   || !this->comp()(*p, value)   ));
      HBOOST_ASSERT(( p == this->begin() || !this->comp()(value, *--p) ));
      #endif
   ...
   #else //defined(HBOOST_INTRUSIVE_DOXYGEN_INVOKED)
   ...
   {
      HBOOST_STATIC_ASSERT((!constant_time_size));
   ...
   void check(ExtraChecker extra_checker) const
   {
      ...
      if (constant_time_size)
         HBOOST_INTRUSIVE_INVARIANT_ASSERT(this->sz_traits().get_size() == checker_return.node_count);
   }
   ...
   friend bool operator==(const bstree_impl &x, const bstree_impl &y)
   {
      ...
      return hboost::intrusive::algo_equal(x.cbegin(), x.cend(), y.cbegin(), y.cend());
   }
   ...
   friend bool operator<(const bstree_impl &x, const bstree_impl &y)
   {  return ::hboost::intrusive::algo_lexicographical_compare(x.begin(), x.end(), y.begin(), y.end());  }
      ...
      >::type
{
   typedef typename make_bstree
      ...
      #endif
      >::type   Base;
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(bstree)
   ...
   bstree(HBOOST_RV_REF(bstree) x)
      :  Base(HBOOST_MOVE_BASE(Base, x))
   {}

   bstree& operator=(HBOOST_RV_REF(bstree) x)
   {  return static_cast<bstree &>(this->Base::operator=(HBOOST_MOVE_BASE(Base, x)));  }
   ...
   template <class Cloner, class Disposer>
   void clone_from(HBOOST_RV_REF(bstree) src, Cloner cloner, Disposer disposer)
...
#endif //HBOOST_INTRUSIVE_BSTREE_HPP
