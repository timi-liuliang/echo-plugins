...
#ifndef HBOOST_INTRUSIVE_LIST_HPP
#define HBOOST_INTRUSIVE_LIST_HPP
...
#include <hboost/intrusive/list_hook.hpp>
#include <hboost/intrusive/circular_list_algorithms.hpp>
...
#include <hboost/intrusive/detail/list_iterator.hpp>
...
class list_impl
{
   ...
   //noncopyable
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(list_impl)
   ...
   //! <b>Effects</b>: to-do
   //!
   list_impl(HBOOST_RV_REF(list_impl) x)
   ...
   //! <b>Effects</b>: to-do
   //!
   list_impl& operator=(HBOOST_RV_REF(list_impl) x)
   ...
   {
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(node_algorithms::distance(b.pointed_node(), e.pointed_node()) == n);
      if(safemode_or_autounlink || constant_time_size){
         return this->erase_and_dispose(b, e, detail::null_disposer());
      }
      else{
   ...
   //! <b>Throws</b>: If cloner throws. Basic guarantee.
   template <class Cloner, class Disposer>
   void clone_from(HBOOST_RV_REF(list_impl) src, Cloner cloner, Disposer disposer)
   {
      this->clear_and_dispose(disposer);
      detail::exception_disposer<list_impl, Disposer>
         rollback(*this, disposer);
   ...
   void splice(const_iterator p, list_impl&x, const_iterator f, const_iterator e, size_type n)
   {
      if(n){
         if(constant_time_size){
            HBOOST_INTRUSIVE_INVARIANT_ASSERT(n == node_algorithms::distance(f.pointed_node(), e.pointed_node()));
   ...
   static iterator s_iterator_to(reference value)
   {
      ...
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(value_traits::to_node_ptr(value)));
   ...
   static const_iterator s_iterator_to(const_reference value)
   {
      ...
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(value_traits::to_node_ptr(r)));
   ...
   {
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(this->priv_value_traits().to_node_ptr(value)));
   ...
   {
      reference r = *detail::uncast(pointer_traits<const_pointer>::pointer_to(value));
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(this->priv_value_traits().to_node_ptr(r)));
   ...
   void check() const
   {
      ...
      HBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_previous(header_ptr));
      // header's next and prev either both point to header (empty list) or neither does
      HBOOST_INTRUSIVE_INVARIANT_ASSERT((node_traits::get_next(header_ptr) == header_ptr)
      ...
      while (true)
      {
         ...
         HBOOST_INTRUSIVE_INVARIANT_ASSERT(node_traits::get_previous(next_p) == p);
         p = next_p;
   ...
   >::type
{
   ...
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(list)
   ...
   {  return static_cast<list &>(this->Base::operator=(HBOOST_MOVE_BASE(Base, x)));  }
   ...
   template <class Cloner, class Disposer>
   void clone_from(HBOOST_RV_REF(list) src, Cloner cloner, Disposer disposer)
...
#endif //HBOOST_INTRUSIVE_LIST_HPP
