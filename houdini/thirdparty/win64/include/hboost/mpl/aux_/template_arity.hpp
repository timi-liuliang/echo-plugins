
#if !defined(HBOOST_PP_IS_ITERATING)

///// header body

#ifndef HBOOST_MPL_AUX_TEMPLATE_ARITY_HPP_INCLUDED
#define HBOOST_MPL_AUX_TEMPLATE_ARITY_HPP_INCLUDED

// Copyright Aleksey Gurtovoy 2001-2004
//
// Distributed under the Boost Software License, Version 1.0. 
// (See accompanying file LICENSE_1_0.txt or copy at 
// http://www.boost.org/LICENSE_1_0.txt)
//
// See http://www.boost.org/libs/mpl for documentation.

// $Id$
// $Date$
// $Revision$

#include <hboost/mpl/aux_/config/ttp.hpp>
#include <hboost/mpl/aux_/config/lambda.hpp>

#if !defined(HBOOST_MPL_PREPROCESSING_MODE)
#   include <hboost/mpl/aux_/template_arity_fwd.hpp>
#   include <hboost/mpl/int.hpp>
#   if !defined(HBOOST_MPL_CFG_NO_FULL_LAMBDA_SUPPORT)
#   if defined(HBOOST_MPL_CFG_EXTENDED_TEMPLATE_PARAMETERS_MATCHING)
#       include <hboost/mpl/aux_/type_wrapper.hpp>
#   endif
#   else
#       include <hboost/mpl/aux_/has_rebind.hpp>
#   endif
#endif

#include <hboost/mpl/aux_/config/static_constant.hpp>
#include <hboost/mpl/aux_/config/use_preprocessed.hpp>

#if !defined(HBOOST_MPL_CFG_NO_PREPROCESSED_HEADERS) \
    && !defined(HBOOST_MPL_PREPROCESSING_MODE)

#   define HBOOST_MPL_PREPROCESSED_HEADER template_arity.hpp
#   include <hboost/mpl/aux_/include_preprocessed.hpp>

#else

#   if !defined(HBOOST_MPL_CFG_NO_FULL_LAMBDA_SUPPORT)
#   if defined(HBOOST_MPL_CFG_EXTENDED_TEMPLATE_PARAMETERS_MATCHING)

#   include <hboost/mpl/limits/arity.hpp>
#   include <hboost/mpl/aux_/preprocessor/range.hpp>
#   include <hboost/mpl/aux_/preprocessor/repeat.hpp>
#   include <hboost/mpl/aux_/preprocessor/params.hpp>
#   include <hboost/mpl/aux_/nttp_decl.hpp>

#   include <hboost/preprocessor/seq/fold_left.hpp>
#   include <hboost/preprocessor/comma_if.hpp>
#   include <hboost/preprocessor/iterate.hpp>
#   include <hboost/preprocessor/inc.hpp>
#   include <hboost/preprocessor/cat.hpp>

#   define AUX778076_ARITY HBOOST_PP_INC(HBOOST_MPL_LIMIT_METAFUNCTION_ARITY)

namespace hboost { namespace mpl { namespace aux {

template< HBOOST_MPL_AUX_NTTP_DECL(int, N) > struct arity_tag
{
    typedef char (&type)[N + 1];
};

#   define AUX778076_MAX_ARITY_OP(unused, state, i_) \
    ( HBOOST_PP_CAT(C,i_) > 0 ? HBOOST_PP_CAT(C,i_) : state ) \
/**/

template<
      HBOOST_MPL_PP_PARAMS(AUX778076_ARITY, HBOOST_MPL_AUX_NTTP_DECL(int, C))
    >
struct max_arity
{
    HBOOST_STATIC_CONSTANT(int, value = 
          HBOOST_PP_SEQ_FOLD_LEFT(
              AUX778076_MAX_ARITY_OP
            , -1
            , HBOOST_MPL_PP_RANGE(1, AUX778076_ARITY)
            )
        );
};

#   undef AUX778076_MAX_ARITY_OP

arity_tag<0>::type arity_helper(...);

#   define HBOOST_PP_ITERATION_LIMITS (1, AUX778076_ARITY)
#   define HBOOST_PP_FILENAME_1 <hboost/mpl/aux_/template_arity.hpp>
#   include HBOOST_PP_ITERATE()

template< typename F, HBOOST_MPL_AUX_NTTP_DECL(int, N) >
struct template_arity_impl
{
    HBOOST_STATIC_CONSTANT(int, value = 
          sizeof(::hboost::mpl::aux::arity_helper(type_wrapper<F>(),arity_tag<N>())) - 1
        );
};

#   define AUX778076_TEMPLATE_ARITY_IMPL_INVOCATION(unused, i_, F) \
    HBOOST_PP_COMMA_IF(i_) template_arity_impl<F,HBOOST_PP_INC(i_)>::value \
/**/

template< typename F >
struct template_arity
{
    HBOOST_STATIC_CONSTANT(int, value = (
          max_arity< HBOOST_MPL_PP_REPEAT(
              AUX778076_ARITY
            , AUX778076_TEMPLATE_ARITY_IMPL_INVOCATION
            , F
            ) >::value
        ));
        
    typedef mpl::int_<value> type;
};

#   undef AUX778076_TEMPLATE_ARITY_IMPL_INVOCATION

#   undef AUX778076_ARITY

}}}

#   endif // HBOOST_MPL_CFG_EXTENDED_TEMPLATE_PARAMETERS_MATCHING
#   else // HBOOST_MPL_CFG_NO_FULL_LAMBDA_SUPPORT

#   include <hboost/mpl/aux_/config/eti.hpp>

namespace hboost { namespace mpl { namespace aux {

template< bool >
struct template_arity_impl
{
    template< typename F > struct result_
        : mpl::int_<-1>
    {
    };
};

template<>
struct template_arity_impl<true>
{
    template< typename F > struct result_
        : F::arity
    {
    };
};

template< typename F >
struct template_arity
    : template_arity_impl< ::hboost::mpl::aux::has_rebind<F>::value >
        ::template result_<F>
{
};

#if defined(HBOOST_MPL_CFG_MSVC_ETI_BUG)
template<>
struct template_arity<int>
    : mpl::int_<-1>
{
};
#endif

}}}

#   endif // HBOOST_MPL_CFG_NO_FULL_LAMBDA_SUPPORT

#endif // HBOOST_MPL_CFG_NO_PREPROCESSED_HEADERS
#endif // HBOOST_MPL_AUX_TEMPLATE_ARITY_HPP_INCLUDED

///// iteration

#else
#define i_ HBOOST_PP_FRAME_ITERATION(1)

template<
      template< HBOOST_MPL_PP_PARAMS(i_, typename P) > class F
    , HBOOST_MPL_PP_PARAMS(i_, typename T)
    >
typename arity_tag<i_>::type
arity_helper(type_wrapper< F<HBOOST_MPL_PP_PARAMS(i_, T)> >, arity_tag<i_>);

#undef i_
#endif // HBOOST_PP_IS_ITERATING
