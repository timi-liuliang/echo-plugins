
#ifndef HBOOST_MPL_MIN_MAX_HPP_INCLUDED
#define HBOOST_MPL_MIN_MAX_HPP_INCLUDED

// Copyright Aleksey Gurtovoy 2000-2008
//
// Distributed under the Boost Software License, Version 1.0. 
// (See accompanying file LICENSE_1_0.txt or copy at 
// http://www.boost.org/LICENSE_1_0.txt)
//
// See http://www.boost.org/libs/mpl for documentation.

// $Id$
// $Date$
// $Revision$

#include <hboost/mpl/less.hpp>
#include <hboost/mpl/if.hpp>
#include <hboost/mpl/aux_/na_spec.hpp>

namespace hboost { namespace mpl {

template<
      typename HBOOST_MPL_AUX_NA_PARAM(N1)
    , typename HBOOST_MPL_AUX_NA_PARAM(N2)
    >
struct min
    : if_< less<N1,N2>,N1,N2 >
{
};

template<
      typename HBOOST_MPL_AUX_NA_PARAM(N1)
    , typename HBOOST_MPL_AUX_NA_PARAM(N2)
    >
struct max
    : if_< less<N1,N2>,N2,N1 >
{
};

HBOOST_MPL_AUX_NA_SPEC(2, min)
HBOOST_MPL_AUX_NA_SPEC(2, max)

}}

#endif // HBOOST_MPL_MIN_MAX_HPP_INCLUDED
