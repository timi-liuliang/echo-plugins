...
template<
    ...
    >
struct HBOOST_PP_CAT(vector,i_)
    : v_item<
          HBOOST_PP_CAT(T,HBOOST_PP_DEC(i_))
        , AUX778076_VECTOR_TAIL(vector,HBOOST_PP_DEC(i_),T)
        >
{
    typedef HBOOST_PP_CAT(vector,i_) type;
};
#endif
...
#   define AUX778076_VECTOR_ITEM(unused, i_, unused2) \
    typedef HBOOST_PP_CAT(T,i_) HBOOST_PP_CAT(item,i_); \
    /**/

    HBOOST_PP_REPEAT(i_, AUX778076_VECTOR_ITEM, unused)
#   undef AUX778076_VECTOR_ITEM
    typedef void_ HBOOST_PP_CAT(item,i_);
    typedef HBOOST_PP_CAT(T,HBOOST_PP_DEC(i_)) back;
...
template<>
struct push_front_impl< aux::vector_tag<HBOOST_PP_DEC(i_)> >
{
    template< typename Vector, typename T > struct apply
    {
        typedef HBOOST_PP_CAT(vector,i_)<
              T
              HBOOST_PP_COMMA_IF(HBOOST_PP_DEC(i_))
              HBOOST_PP_ENUM_PARAMS(HBOOST_PP_DEC(i_), typename Vector::item)
            > type;
    };
};
...
struct pop_front_impl< aux::vector_tag<i_> >
{
    ...
    {
        typedef HBOOST_PP_CAT(vector,HBOOST_PP_DEC(i_))<
              HBOOST_PP_ENUM_SHIFTED_PARAMS(i_, typename Vector::item)
            > type;
    };
};
...
template<>
struct push_back_impl< aux::vector_tag<HBOOST_PP_DEC(i_)> >
{
    template< typename Vector, typename T > struct apply
    {
        typedef HBOOST_PP_CAT(vector,i_)<
...
struct v_at<V,i_>
{
    typedef typename V::HBOOST_PP_CAT(item,i_) type;
};
...
template<> struct v_at_impl<i_>
{
    ...
    {
        typedef typename V_::HBOOST_PP_CAT(item,i_) type;
    };
};
}
...
struct at_impl< aux::vector_tag<i_> >
{
    ...
    {
        typedef typename aux::v_at_impl<HBOOST_MPL_AUX_VALUE_WKND(N)::value>
