...
#ifndef HBOOST_ASIO_IO_SERVICE_HPP
#define HBOOST_ASIO_IO_SERVICE_HPP
...
#include <hboost/asio/detail/wrapped_handler.hpp>
...
# include <hboost/asio/detail/winsock_init.hpp>
  ...
  || defined(__osf__)
# include <hboost/asio/detail/signal_init.hpp>
#endif
 ...
 *
 * @li hboost::asio::ip::tcp::socket
 * @li hboost::asio::ip::tcp::acceptor
 * @li hboost::asio::ip::udp::socket
 * @li hboost::asio::deadline_timer.
 *
 ...
 * creating an object of type hboost::asio::io_service::work:
 *
 * @code hboost::asio::io_service io_service;
 * hboost::asio::io_service::work work(io_service);
 * ... @endcode
 *
 ...
 *
 * @code hboost::asio::io_service io_service;
 * auto_ptr<hboost::asio::io_service::work> work(
 *     new hboost::asio::io_service::work(io_service));
 * ...
...
public:
   ...
   */
  HBOOST_ASIO_DECL explicit io_service(std::size_t concurrency_hint);
   ...
   * without blocking.
   */
  HBOOST_ASIO_DECL std::size_t run();
   ...
   * @throws hboost::system::system_error Thrown on failure.
   */
  HBOOST_ASIO_DECL std::size_t run_one();
   ...
   * @throws hboost::system::system_error Thrown on failure.
   */
  HBOOST_ASIO_DECL std::size_t poll();
   ...
   * @throws hboost::system::system_error Thrown on failure.
   */
  HBOOST_ASIO_DECL std::size_t poll_one();
  ...
  HBOOST_ASIO_INITFN_RESULT_TYPE(CompletionHandler, void ())
  dispatch(HBOOST_ASIO_MOVE_ARG(CompletionHandler) handler);
  ...
  template <typename CompletionHandler>
  HBOOST_ASIO_INITFN_RESULT_TYPE(CompletionHandler, void ())
  post(HBOOST_ASIO_MOVE_ARG(CompletionHandler) handler);
   ...
   * @code io_service.dispatch(hboost::bind(f, a1, ... an)); @endcode
   */
   ...
   *
   * @throws hboost::system::system_error Thrown on failure. If the notification
   ...
   * function:
   * @code my_io_service.notify_fork(hboost::asio::io_service::fork_prepare);
   * if (fork() == 0)
   * {
   *   // This is the child process.
   *   my_io_service.notify_fork(hboost::asio::io_service::fork_child);
   * }
   * else
   * {
   *   // This is the parent process.
   *   my_io_service.notify_fork(hboost::asio::io_service::fork_parent);
   * } @endcode
   *
   ...
   */
  HBOOST_ASIO_DECL void notify_fork(hboost::asio::io_service::fork_event event);
   ...
   *
   * @throws hboost::asio::service_already_exists Thrown if a service of the
   * given type is already present in the io_service.
   *
   * @throws hboost::asio::invalid_service_owner Thrown if the service's owning
...
private:
  ...
  hboost::asio::detail::service_registry* service_registry_;
...
public:
   ...
   */
  explicit work(hboost::asio::io_service& io_service);
...
protected:
   ...
   */
  HBOOST_ASIO_DECL service(hboost::asio::io_service& owner);

  /// Destructor.
  HBOOST_ASIO_DECL virtual ~service();
...
private:
   ...
   */
  HBOOST_ASIO_DECL virtual void fork_service(
      hboost::asio::io_service::fork_event event);

  friend class hboost::asio::detail::service_registry;
  struct key
  {
    key() : type_info_(0), id_(0) {}
    const std::type_info* type_info_;
    const hboost::asio::io_service::id* id_;
  } key_;
...
{
public:
  HBOOST_ASIO_DECL service_already_exists();
};
...
{
public:
  HBOOST_ASIO_DECL invalid_service_owner();
};
...
class service_id
  : public hboost::asio::io_service::id
{
};
...
{
public:
  static hboost::asio::detail::service_id<Type> id;

  // Constructor.
  service_base(hboost::asio::io_service& io_service)
    : hboost::asio::io_service::service(io_service)
  {
  }
};

template <typename Type>
hboost::asio::detail::service_id<Type> service_base<Type>::id;
...
#include <hboost/asio/impl/io_service.hpp>
#if defined(HBOOST_ASIO_HEADER_ONLY)
# include <hboost/asio/impl/io_service.ipp>
#endif // defined(HBOOST_ASIO_HEADER_ONLY)

#endif // HBOOST_ASIO_IO_SERVICE_HPP
