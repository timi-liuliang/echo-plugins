...
#ifndef HBOOST_ASIO_BUFFER_HPP
#define HBOOST_ASIO_BUFFER_HPP
...
#include <string>
#include <vector>
#include <hboost/asio/detail/array_fwd.hpp>

#if defined(HBOOST_ASIO_MSVC)
# if defined(_HAS_ITERATOR_DEBUGGING) && (_HAS_ITERATOR_DEBUGGING != 0)
#  if !defined(HBOOST_ASIO_DISABLE_BUFFER_DEBUGGING)
#   define HBOOST_ASIO_ENABLE_BUFFER_DEBUGGING
#  endif // !defined(HBOOST_ASIO_DISABLE_BUFFER_DEBUGGING)
...
# include <hboost/asio/detail/function.hpp>
#endif // HBOOST_ASIO_ENABLE_BUFFER_DEBUGGING
...
#  define HBOOST_ASIO_ENABLE_ARRAY_BUFFER_WORKAROUND
# endif // HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x582))
        // || HBOOST_WORKAROUND(__SUNPRO_CC, HBOOST_TESTED_AT(0x590))
#endif // defined(HBOOST_ASIO_HAS_HBOOST_WORKAROUND)

#if defined(HBOOST_ASIO_ENABLE_ARRAY_BUFFER_WORKAROUND)
# include <hboost/asio/detail/type_traits.hpp>
#endif // defined(HBOOST_ASIO_ENABLE_ARRAY_BUFFER_WORKAROUND)
 ...
 *
 * @code hboost::asio::mutable_buffer b1 = ...;
 * std::size_t s1 = hboost::asio::buffer_size(b1);
 * unsigned char* p1 = hboost::asio::buffer_cast<unsigned char*>(b1);
 * @endcode
 *
 * The hboost::asio::buffer_cast function permits violations of type safety, so
...
public:
...
#if defined(HBOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
  mutable_buffer(void* data, std::size_t size,
      hboost::asio::detail::function<void()> debug_check)
    : data_(data),
      size_(size),
  ...
  const hboost::asio::detail::function<void()>& get_debug_check() const
  {
    return debug_check_;
  }
#endif // HBOOST_ASIO_ENABLE_BUFFER_DEBUGGING
...
private:
  friend void* hboost::asio::detail::buffer_cast_helper(
      const mutable_buffer& b);
  friend std::size_t hboost::asio::detail::buffer_size_helper(
      const mutable_buffer& b);
...
#if defined(HBOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
  hboost::asio::detail::function<void()> debug_check_;
#endif // HBOOST_ASIO_ENABLE_BUFFER_DEBUGGING
};
 ...
 * and @ref buffer_cast functions:
 *
 * @code hboost::asio::const_buffer b1 = ...;
 * std::size_t s1 = hboost::asio::buffer_size(b1);
 * const unsigned char* p1 = hboost::asio::buffer_cast<const unsigned char*>(b1);
 * @endcode
 *
 * The hboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
...
public:
  ...
  const_buffer(const mutable_buffer& b)
    : data_(hboost::asio::detail::buffer_cast_helper(b)),
      size_(hboost::asio::detail::buffer_size_helper(b))
#if defined(HBOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
...
private:
  friend const void* hboost::asio::detail::buffer_cast_helper(
...
/** @defgroup buffer_size hboost::asio::buffer_size
 *
 * @brief The hboost::asio::buffer_size function determines the total number of
...
/** @defgroup buffer_cast hboost::asio::buffer_cast
 *
 * @brief The hboost::asio::buffer_cast function is used to obtain a pointer to
 ...
 * @code hboost::asio::mutable_buffer b2 = ...;
 * unsigned char* p2 = hboost::asio::buffer_cast<unsigned char*>(b2);
 * @endcode
 *
 * The hboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
...
public:
  ...
  ~buffer_debug_check()
  {
    ...
    iter_ = Iterator();
#endif // defined(HBOOST_ASIO_MSVC) && (HBOOST_ASIO_MSVC == 1400)
  }
...
/** @defgroup buffer hboost::asio::buffer
 *
 * @brief The hboost::asio::buffer function is used to create a buffer object to
 ...
 * specified size:
 *
 * @code sock.send(hboost::asio::buffer(data, size)); @endcode
 *
 * In the above example, the return value of hboost::asio::buffer meets the
 * requirements of the ConstBufferSequence concept so that it may be directly
 * passed to the socket's write function. A buffer created for modifiable
 * memory also meets the requirements of the MutableBufferSequence concept.
 *
 * An individual buffer may be created from a builtin array, std::vector,
 * std::array or hboost::array of POD elements. This helps prevent buffer
 * overruns by automatically determining the size of the buffer:
 *
 * @code char d1[128];
 * size_t bytes_transferred = sock.receive(hboost::asio::buffer(d1));
 *
 * std::vector<char> d2(128);
 * bytes_transferred = sock.receive(hboost::asio::buffer(d2));
 *
 * std::array<char, 128> d3;
 * bytes_transferred = sock.receive(hboost::asio::buffer(d3));
 *
 * hboost::array<char, 128> d4;
 * bytes_transferred = sock.receive(hboost::asio::buffer(d4)); @endcode
 *
 ...
 * unsigned char* p1 = hboost::asio::buffer_cast<unsigned char*>(b1);
 *
 * hboost::asio::const_buffer b2 = ...;
 * std::size_t s2 = hboost::asio::buffer_size(b2);
 * const void* p2 = hboost::asio::buffer_cast<const void*>(b2); @endcode
 *
 * The hboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
 *
 ...
 *
 * vector<unsigned char> data(hboost::asio::buffer_size(buffers));
 * hboost::asio::buffer_copy(hboost::asio::buffer(data), buffers); @endcode
 *
 ...
 *
 * For the hboost::asio::buffer overloads that accept an argument of type
 ...
 * follows:
 *
 * @code hboost::array<char, 6> a = { 'a', 'b', 'c', 'd', 'e' }; @endcode
 *
 * A buffer object @c b1 created using:
 *
 * @code b1 = hboost::asio::buffer(a); @endcode
 *
 * represents the entire array, <tt>{ 'a', 'b', 'c', 'd', 'e' }</tt>. An
 * optional second argument to the hboost::asio::buffer function may be used to
 * limit the size, in bytes, of the buffer:
 *
 * @code b2 = hboost::asio::buffer(a, 3); @endcode
 *
 ...
 *
 * @code b4 = hboost::asio::buffer(b1 + 1, 3); @endcode
 *
 ...
 * char d1[128];
 * std::vector<char> d2(128);
 * hboost::array<char, 128> d3;
 *
 * hboost::array<mutable_buffer, 3> bufs1 = {
 *   hboost::asio::buffer(d1),
 *   hboost::asio::buffer(d2),
 *   hboost::asio::buffer(d3) };
 * bytes_transferred = sock.receive(bufs1);
 *
 * std::vector<const_buffer> bufs2;
 * bufs2.push_back(hboost::asio::buffer(d1));
 * bufs2.push_back(hboost::asio::buffer(d2));
 * bufs2.push_back(hboost::asio::buffer(d3));
...
//
//   unspecified buffer(hboost::array<PodType, N>& array ...);
//
// and
//
//   unspecified buffer(hboost::array<const PodType, N>& array ...);
//
...
buffer(hboost::array<PodType, N>& data)
{
  typedef typename hboost::asio::detail::buffer_types<PodType>::buffer_type
    buffer_type;
  typedef typename hboost::asio::detail::buffer_types<PodType>::container_type
    container_type;
...
template <typename PodType, std::size_t N>
inline typename detail::buffer_types<PodType>::container_type
buffer(hboost::array<PodType, N>& data, std::size_t max_size_in_bytes)
{
  typedef typename hboost::asio::detail::buffer_types<PodType>::buffer_type
    buffer_type;
  typedef typename hboost::asio::detail::buffer_types<PodType>::container_type
...
#else // defined(HBOOST_ASIO_ENABLE_ARRAY_BUFFER_WORKAROUND)
 ...
 */
template <typename PodType, std::size_t N>
inline mutable_buffers_1 buffer(hboost::array<PodType, N>& data)
{
  return mutable_buffers_1(
 ...
 */
template <typename PodType, std::size_t N>
inline const_buffers_1 buffer(hboost::array<const PodType, N>& data)
{
  return const_buffers_1(
 ...
 */
template <typename PodType, std::size_t N>
inline const_buffers_1 buffer(const hboost::array<PodType, N>& data)
{
  return const_buffers_1(
      const_buffer(data.data(), data.size() * sizeof(PodType)));
}
...
#if defined(HBOOST_ASIO_HAS_STD_ARRAY) || defined(GENERATING_DOCUMENTATION)
...
#endif // defined(HBOOST_ASIO_HAS_STD_ARRAY) || defined(GENERATING_DOCUMENTATION)
...
/** @defgroup buffer_copy hboost::asio::buffer_copy
 *
 * @brief The hboost::asio::buffer_copy function is used to copy bytes from a
