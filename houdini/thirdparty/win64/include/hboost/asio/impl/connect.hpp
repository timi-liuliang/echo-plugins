...
#ifndef HBOOST_ASIO_IMPL_CONNECT_HPP
#define HBOOST_ASIO_IMPL_CONNECT_HPP
...
namespace detail
{
  struct default_connect_condition
  {
    ...
    Iterator operator()(const hboost::system::error_code&, Iterator next)
    {
      return next;
    }
  };
    ...
    hboost::system::error_code& ec)
{
  ...
  if (!ec)
    ec = hboost::asio::error::not_found;
...
namespace detail
{
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const hboost::system::error_code& ec,
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const hboost::system::error_code&, Iterator&, Iterator&)
    {
    }
  };
  ...
  public:
    connect_op(basic_socket<Protocol, SocketService>& sock,
      ...
      : base_from_connect_condition<ConnectCondition>(connect_condition),
        ...
        end_(end),
        start_(0),
        handler_(HBOOST_ASIO_MOVE_CAST(ComposedConnectHandler)(handler))
    {
    }
    ...
    connect_op(connect_op&& other)
      : base_from_connect_condition<ConnectCondition>(other),
        ...
        end_(other.end_),
        start_(other.start_),
        handler_(HBOOST_ASIO_MOVE_CAST(ComposedConnectHandler)(other.handler_))
    {
    }
#endif // defined(HBOOST_ASIO_HAS_MOVE)

    void operator()(hboost::system::error_code ec, int start = 0)
    {
      switch (start_ = start)
      {
        case 1:
        for (;;)
        {
          ...
          if (iter_ != end_)
          {
            ...
            socket_.async_connect(*iter_,
                HBOOST_ASIO_MOVE_CAST(connect_op)(*this));
            return;
          }
        ...
        handler_(static_cast<const hboost::system::error_code&>(ec),
...
inline HBOOST_ASIO_INITFN_RESULT_TYPE(ComposedConnectHandler,
    ...
    Iterator begin, HBOOST_ASIO_MOVE_ARG(ComposedConnectHandler) handler)
{
  ...
  HBOOST_ASIO_COMPOSED_CONNECT_HANDLER_CHECK(
      ComposedConnectHandler, handler, Iterator) type_check;

  detail::async_result_init<ComposedConnectHandler,
    void (hboost::system::error_code, Iterator)> init(
      HBOOST_ASIO_MOVE_CAST(ComposedConnectHandler)(handler));

  detail::connect_op<Protocol, SocketService, Iterator,
    detail::default_connect_condition, HBOOST_ASIO_HANDLER_TYPE(
      ComposedConnectHandler, void (hboost::system::error_code, Iterator))>(s,
        begin, Iterator(), detail::default_connect_condition(), init.handler)(
          hboost::system::error_code(), 1);
    ...
    HBOOST_ASIO_MOVE_ARG(ComposedConnectHandler) handler)
{
  ...
  detail::connect_op<Protocol, SocketService, Iterator,
    ConnectCondition, HBOOST_ASIO_HANDLER_TYPE(
      ComposedConnectHandler, void (hboost::system::error_code, Iterator))>(s,
...
#endif // HBOOST_ASIO_IMPL_CONNECT_HPP
