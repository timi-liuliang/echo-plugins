...
#ifndef HBOOST_ASIO_IMPL_READ_UNTIL_HPP
#define HBOOST_ASIO_IMPL_READ_UNTIL_HPP
...
inline std::size_t read_until(SyncReadStream& s,
    hboost::asio::basic_streambuf<Allocator>& b, char delim)
{
  hboost::system::error_code ec;
  std::size_t bytes_transferred = read_until(s, b, delim, ec);
  hboost::asio::detail::throw_error(ec, "read_until");
    ...
    hboost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    typedef typename hboost::asio::basic_streambuf<
      Allocator>::const_buffers_type const_buffers_type;
    typedef hboost::asio::buffers_iterator<const_buffers_type> iterator;
...
template <typename SyncReadStream, typename Allocator>
inline std::size_t read_until(SyncReadStream& s,
    hboost::asio::basic_streambuf<Allocator>& b, const std::string& delim)
{
  hboost::system::error_code ec;
  std::size_t bytes_transferred = read_until(s, b, delim, ec);
  hboost::asio::detail::throw_error(ec, "read_until");
...
template <typename SyncReadStream, typename Allocator>
inline std::size_t read_until(SyncReadStream& s,
    hboost::asio::basic_streambuf<Allocator>& b, const hboost::regex& expr)
{
  hboost::system::error_code ec;
    ...
    hboost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    // Look for a match.
    hboost::match_results<iterator,
      typename std::vector<hboost::sub_match<iterator> >::allocator_type>
        match_results;
    if (regex_search(start_pos, end, match_results, expr,
          hboost::match_default | hboost::match_partial))
    {
...
std::size_t read_until(SyncReadStream& s,
    ...
    MatchCondition match_condition, hboost::system::error_code& ec,
...
template <typename SyncReadStream, typename Allocator, typename MatchCondition>
inline std::size_t read_until(SyncReadStream& s,
    hboost::asio::basic_streambuf<Allocator>& b, MatchCondition match_condition,
    typename enable_if<is_match_condition<MatchCondition>::value>::type*)
{
  hboost::system::error_code ec;
      ...
      case 1:
        for (;;)
        {
          ...
          stream_.async_read_some(streambuf_.prepare(bytes_to_read),
              HBOOST_ASIO_MOVE_CAST(read_until_delim_op)(*this));
          return; default:
    ...
    HBOOST_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  detail::read_until_delim_op<AsyncReadStream,
    Allocator, HBOOST_ASIO_HANDLER_TYPE(ReadHandler,
      void (hboost::system::error_code, std::size_t))>(
...
namespace detail
{
  ...
  public:
    ...
    read_until_delim_string_op(read_until_delim_string_op&& other)
      : stream_(other.stream_),
        streambuf_(other.streambuf_),
        delim_(HBOOST_ASIO_MOVE_CAST(std::string)(other.delim_)),
        start_(other.start_),
        search_position_(other.search_position_),
        handler_(HBOOST_ASIO_MOVE_CAST(ReadHandler)(other.handler_))
    {
        ...
        std::size_t bytes_transferred, int start = 0)
    {
      ...
      case 1:
        for (;;)
        {
          ...
          // Start a new asynchronous read operation to obtain more data.
          stream_.async_read_some(streambuf_.prepare(bytes_to_read),
              HBOOST_ASIO_MOVE_CAST(read_until_delim_string_op)(*this));
          return; default:
          streambuf_.commit(bytes_transferred);
          if (ec || bytes_transferred == 0)
            break;
...
namespace detail
{
  ...
  public:
    read_until_expr_op(AsyncReadStream& stream,
        hboost::asio::basic_streambuf<Allocator>& streambuf,
        const hboost::regex& expr, ReadHandler& handler)
      : stream_(stream),
        streambuf_(streambuf),
        expr_(expr),
        start_(0),
        ...
        std::size_t bytes_transferred, int start = 0)
    {
      ...
      case 1:
        for (;;)
        {
          ...
          // Start a new asynchronous read operation to obtain more data.
          stream_.async_read_some(streambuf_.prepare(bytes_to_read),
              HBOOST_ASIO_MOVE_CAST(read_until_expr_op)(*this));
          return; default:
          streambuf_.commit(bytes_transferred);
          if (ec || bytes_transferred == 0)
            break;
    ...
    HBOOST_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  detail::read_until_expr_op<AsyncReadStream, Allocator,
    hboost::regex, HBOOST_ASIO_HANDLER_TYPE(ReadHandler,
      void (hboost::system::error_code, std::size_t))>(
        ...
        std::size_t bytes_transferred, int start = 0)
    {
      ...
      case 1:
        for (;;)
        {
          ...
          // Start a new asynchronous read operation to obtain more data.
          stream_.async_read_some(streambuf_.prepare(bytes_to_read),
              HBOOST_ASIO_MOVE_CAST(read_until_match_op)(*this));
          return; default:
          streambuf_.commit(bytes_transferred);
          if (ec || bytes_transferred == 0)
            break;
...
async_read_until(AsyncReadStream& s,
    hboost::asio::basic_streambuf<Allocator>& b,
    MatchCondition match_condition, HBOOST_ASIO_MOVE_ARG(ReadHandler) handler,
    typename enable_if<is_match_condition<MatchCondition>::value>::type*)
{
  // If you get an error on the following line it means that your handler does
  // not meet the documented type requirements for a ReadHandler.
  ...
  detail::read_until_match_op<AsyncReadStream, Allocator,
    MatchCondition, HBOOST_ASIO_HANDLER_TYPE(ReadHandler,
      void (hboost::system::error_code, std::size_t))>(
...
#endif // HBOOST_ASIO_IMPL_READ_UNTIL_HPP
