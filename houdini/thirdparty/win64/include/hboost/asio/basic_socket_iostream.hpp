...
#ifndef HBOOST_ASIO_BASIC_SOCKET_IOSTREAM_HPP
#define HBOOST_ASIO_BASIC_SOCKET_IOSTREAM_HPP
...
# define HBOOST_ASIO_PRIVATE_CTR_DEF(n) \
  template <HBOOST_ASIO_VARIADIC_TPARAMS(n)> \
  explicit basic_socket_iostream(HBOOST_ASIO_VARIADIC_PARAMS(n)) \
    : std::basic_iostream<char>( \
        &this->detail::socket_iostream_base< \
          Protocol, StreamSocketService, Time, \
          TimeTraits, TimerService>::streambuf_) \
  { \
    ...
    if (rdbuf()->connect(HBOOST_ASIO_VARIADIC_ARGS(n)) == 0) \
      this->setstate(std::ios_base::failbit); \
  } \
  /**/
...
# define HBOOST_ASIO_PRIVATE_CONNECT_DEF(n) \
  template <HBOOST_ASIO_VARIADIC_TPARAMS(n)> \
  void connect(HBOOST_ASIO_VARIADIC_PARAMS(n)) \
  { \
    if (rdbuf()->connect(HBOOST_ASIO_VARIADIC_ARGS(n)) == 0) \
      this->setstate(std::ios_base::failbit); \
  } \
...
public:
...
#else
  HBOOST_ASIO_VARIADIC_GENERATE(HBOOST_ASIO_PRIVATE_CTR_DEF)
#endif
   ...
   * } @endcode
   */
  const hboost::system::error_code& error() const
  {
...
#if !defined(HBOOST_ASIO_HAS_VARIADIC_TEMPLATES)
# undef HBOOST_ASIO_PRIVATE_CTR_DEF
# undef HBOOST_ASIO_PRIVATE_CONNECT_DEF
#endif // !defined(HBOOST_ASIO_HAS_VARIADIC_TEMPLATES)

#endif // !defined(HBOOST_ASIO_NO_IOSTREAM)

#endif // HBOOST_ASIO_BASIC_SOCKET_IOSTREAM_HPP
