...
#ifndef HBOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
#define HBOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
...
#else
# include <hboost/asio/steady_timer.hpp>
#endif

#if !defined(HBOOST_ASIO_HAS_VARIADIC_TEMPLATES)

# include <hboost/asio/detail/variadic_templates.hpp>
...
# define HBOOST_ASIO_PRIVATE_CONNECT_DEF(n) \
  template <HBOOST_ASIO_VARIADIC_TPARAMS(n)> \
  basic_socket_streambuf<Protocol, StreamSocketService, \
    Time, TimeTraits, TimerService>* connect(HBOOST_ASIO_VARIADIC_PARAMS(n)) \
  { \
    init_buffers(); \
    this->basic_socket<Protocol, StreamSocketService>::close(ec_); \
    typedef typename Protocol::resolver resolver_type; \
    typedef typename resolver_type::query resolver_query; \
    resolver_query query(HBOOST_ASIO_VARIADIC_ARGS(n)); \
    resolve_and_connect(query); \
    return !ec_ ? this : 0; \
  } \
  /**/
...
template <typename Protocol,
  ...
  || defined(GENERATING_DOCUMENTATION)
    typename Time = hboost::posix_time::ptime,
    typename TimeTraits = hboost::asio::time_traits<Time>,
...
public:
      ...
      const endpoint_type& endpoint)
  {
    ...
    {
      ec_ = hboost::asio::error::operation_aborted;
      return 0;
    }
  ...
  basic_socket_streambuf<Protocol, StreamSocketService>* connect(
      T1 t1, ..., TN tn);
#elif defined(HBOOST_ASIO_HAS_VARIADIC_TEMPLATES)
  template <typename... T>
  basic_socket_streambuf<Protocol, StreamSocketService,
...
#else
  HBOOST_ASIO_VARIADIC_GENERATE(HBOOST_ASIO_PRIVATE_CONNECT_DEF)
#endif
   ...
   * buffer.
   */
  const hboost::system::error_code& puberror() const
  {
    return error();
  }
   ...
   * hboost::asio::error::operation_aborted.
   *
  ...
  void expires_at(const time_type& expiry_time)
  {
    ...
    hboost::asio::detail::throw_error(ec, "expires_at");
  ...
  void expires_from_now(const duration_type& expiry_time)
  {
    ...
    hboost::asio::detail::throw_error(ec, "expires_from_now");
...
protected:
  int_type underflow()
  {
    if (gptr() == egptr())
    {
      ...
      this->get_service().async_receive(this->get_implementation(),
          hboost::asio::buffer(hboost::asio::buffer(get_buffer_) + putback_max),
          0, handler);
  ...
  int_type overflow(int_type c)
  {
    if (unbuffered_)
    {
      ...
      else
      {
        ...
        this->get_service().async_send(this->get_implementation(),
            hboost::asio::buffer(&ch, sizeof(char_type)), 0, handler);
    ...
    else
    {
      ...
      hboost::asio::const_buffer buffer =
        hboost::asio::buffer(pbase(), pptr() - pbase());
      while (hboost::asio::buffer_size(buffer) > 0)
      {
        if (timer_state_ == timer_has_expired)
        {
          ec_ = hboost::asio::error::operation_aborted;
        ...
        io_handler handler = { this };
        this->get_service().async_send(this->get_implementation(),
            hboost::asio::buffer(buffer), 0, handler);
   ...
   * buffer.
   */
  virtual const hboost::system::error_code& error() const
  {
    return ec_;
  }
...
private:
  ...
  void resolve_and_connect(const ResolverQuery& query)
  {
    ...
    {
      iterator_type end;
      ec_ = hboost::asio::error::host_not_found;
      while (ec_ && i != end)
      {
        this->basic_socket<Protocol, StreamSocketService>::close(ec_);
  ...
  struct timer_handler
  {
    ...
    void operator()(const hboost::system::error_code&)
    {
  ...
  void start_timer()
  {
    if (timer_state_ != timer_is_pending)
    {
      ...
      handler(hboost::system::error_code());
    }
  }

  enum { putback_max = 8 };
  enum { buffer_size = 512 };
  hboost::asio::detail::array<char, buffer_size> get_buffer_;
  hboost::asio::detail::array<char, buffer_size> put_buffer_;
  bool unbuffered_;
...
#if !defined(HBOOST_ASIO_HAS_VARIADIC_TEMPLATES)
# undef HBOOST_ASIO_PRIVATE_CONNECT_DEF
#endif // !defined(HBOOST_ASIO_HAS_VARIADIC_TEMPLATES)

#endif // !defined(HBOOST_ASIO_NO_IOSTREAM)

#endif // HBOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
