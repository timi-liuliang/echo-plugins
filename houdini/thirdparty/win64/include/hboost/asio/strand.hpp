...
#ifndef HBOOST_ASIO_STRAND_HPP
#define HBOOST_ASIO_STRAND_HPP
...
#include <hboost/asio/detail/strand_service.hpp>
...
public:
   ...
   */
  explicit strand(hboost::asio::io_service& io_service)
    : service_(hboost::asio::use_service<
        hboost::asio::detail::strand_service>(io_service))
  {
  ...
  dispatch(HBOOST_ASIO_MOVE_ARG(CompletionHandler) handler)
  {
    ...
    HBOOST_ASIO_COMPLETION_HANDLER_CHECK(CompletionHandler, handler) type_check;

    detail::async_result_init<
      CompletionHandler, void ()> init(
        HBOOST_ASIO_MOVE_CAST(CompletionHandler)(handler));
   ...
   * @code strand.dispatch(hboost::bind(f, a1, ... an)); @endcode
   */
...
private:
  hboost::asio::detail::strand_service& service_;
  hboost::asio::detail::strand_service::implementation_type impl_;
};

/// (Deprecated: Use hboost::asio::io_service::strand.) Typedef for backwards
/// compatibility.
typedef hboost::asio::io_service::strand strand;
