...
#ifndef HBOOST_ASIO_DETAIL_WIN_IOCP_IO_SERVICE_HPP
#define HBOOST_ASIO_DETAIL_WIN_IOCP_IO_SERVICE_HPP
...
class win_iocp_io_service
  : public hboost::asio::detail::service_base<win_iocp_io_service>
{
public:

  // Constructor. Specifies a concurrency hint that is passed through to the
  // underlying I/O completion port.
  HBOOST_ASIO_DECL win_iocp_io_service(hboost::asio::io_service& io_service,
  ...
  HBOOST_ASIO_DECL hboost::system::error_code register_handle(
      HANDLE handle, hboost::system::error_code& ec);

  // Run the event loop until stopped or no more work.
  HBOOST_ASIO_DECL size_t run(hboost::system::error_code& ec);

  // Run until stopped or one operation is performed.
  HBOOST_ASIO_DECL size_t run_one(hboost::system::error_code& ec);

  // Poll for operations without blocking.
  HBOOST_ASIO_DECL size_t poll(hboost::system::error_code& ec);

  // Poll for one operation without blocking.
  HBOOST_ASIO_DECL size_t poll_one(hboost::system::error_code& ec);
  ...
  HBOOST_ASIO_DECL void post_deferred_completion(win_iocp_operation* op);

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() was previously called for the operations.
  HBOOST_ASIO_DECL void post_deferred_completions(
  ...
  HBOOST_ASIO_DECL void on_pending(win_iocp_operation* op);

  // Called after starting an overlapped I/O operation that completed
  // immediately. The caller must have already called work_started() prior to
  // starting the operation.
  HBOOST_ASIO_DECL void on_completion(win_iocp_operation* op,
  ...
  // starting the operation.
  HBOOST_ASIO_DECL void on_completion(win_iocp_operation* op,
      const hboost::system::error_code& ec, DWORD bytes_transferred = 0);
...
private:
  ...
  // either 0 or 1).
  HBOOST_ASIO_DECL size_t do_one(bool block, hboost::system::error_code& ec);

  // Helper to calculate the GetQueuedCompletionStatus timeout.
  HBOOST_ASIO_DECL static DWORD get_gqcs_timeout();
  ...
  HBOOST_ASIO_DECL void update_timeout();
...
#include <hboost/asio/detail/impl/win_iocp_io_service.hpp>
#if defined(HBOOST_ASIO_HEADER_ONLY)
# include <hboost/asio/detail/impl/win_iocp_io_service.ipp>
#endif // defined(HBOOST_ASIO_HEADER_ONLY)

#endif // defined(HBOOST_ASIO_HAS_IOCP)

#endif // HBOOST_ASIO_DETAIL_WIN_IOCP_IO_SERVICE_HPP
