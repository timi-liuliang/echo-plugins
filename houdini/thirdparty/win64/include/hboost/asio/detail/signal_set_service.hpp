...
#ifndef HBOOST_ASIO_DETAIL_SIGNAL_SET_SERVICE_HPP
#define HBOOST_ASIO_DETAIL_SIGNAL_SET_SERVICE_HPP
...
#include <hboost/asio/detail/signal_handler.hpp>
...
extern HBOOST_ASIO_DECL struct signal_state* get_signal_state();

extern "C" HBOOST_ASIO_DECL void hboost_asio_signal_handler(int signal_number);
...
public:
  ...
  // Constructor.
  HBOOST_ASIO_DECL signal_set_service(hboost::asio::io_service& io_service);

  // Destructor.
  HBOOST_ASIO_DECL ~signal_set_service();
  ...
  HBOOST_ASIO_DECL hboost::system::error_code add(implementation_type& impl,
      int signal_number, hboost::system::error_code& ec);

  // Remove a signal to a signal_set.
  HBOOST_ASIO_DECL hboost::system::error_code remove(implementation_type& impl,
      int signal_number, hboost::system::error_code& ec);

  // Remove all signals from a signal_set.
  HBOOST_ASIO_DECL hboost::system::error_code clear(implementation_type& impl,
  ...
  void async_wait(implementation_type& impl, Handler& handler)
  {
    ...
    HBOOST_ASIO_HANDLER_CREATION((p.p, "signal_set", &impl, "async_wait"));
  ...
  HBOOST_ASIO_DECL static void deliver_signal(int signal_number);

private:
  // Helper function to add a service to the global signal state.
  HBOOST_ASIO_DECL static void add_service(signal_set_service* service);

  // Helper function to remove a service from the global signal state.
  HBOOST_ASIO_DECL static void remove_service(signal_set_service* service);

  // Helper function to create the pipe descriptors.
  HBOOST_ASIO_DECL static void open_descriptors();

  // Helper function to close the pipe descriptors.
  HBOOST_ASIO_DECL static void close_descriptors();

  // Helper function to start a wait operation.
  HBOOST_ASIO_DECL void start_wait_op(implementation_type& impl, signal_op* op);
...
# include <hboost/asio/detail/impl/signal_set_service.ipp>
#endif // defined(HBOOST_ASIO_HEADER_ONLY)

#endif // HBOOST_ASIO_DETAIL_SIGNAL_SET_SERVICE_HPP
