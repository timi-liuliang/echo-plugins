...
#ifndef HBOOST_ASIO_DETAIL_EPOLL_REACTOR_HPP
#define HBOOST_ASIO_DETAIL_EPOLL_REACTOR_HPP
...
#include <hboost/asio/detail/object_pool.hpp>
...
class epoll_reactor
  : public hboost::asio::detail::service_base<epoll_reactor>
{
public:
  ...
  class descriptor_state : operation
  {
    ...
    HBOOST_ASIO_DECL descriptor_state();
    void set_ready_events(uint32_t events) { task_result_ = events; }
    HBOOST_ASIO_DECL operation* perform_io(uint32_t events);
    HBOOST_ASIO_DECL static void do_complete(
  ...
  // Constructor.
  HBOOST_ASIO_DECL epoll_reactor(hboost::asio::io_service& io_service);

  // Destructor.
  HBOOST_ASIO_DECL ~epoll_reactor();
  ...
  // code on failure.
  HBOOST_ASIO_DECL int register_descriptor(socket_type descriptor,
  ...
  HBOOST_ASIO_DECL void cancel_ops(socket_type descriptor,
      per_descriptor_data& descriptor_data);
...
private:
  ...
  // cannot be created.
  HBOOST_ASIO_DECL static int do_epoll_create();

  // Create the timerfd file descriptor. Does not throw.
  HBOOST_ASIO_DECL static int do_timerfd_create();

  // Allocate a new descriptor state object.
  HBOOST_ASIO_DECL descriptor_state* allocate_descriptor_state();

  // Free an existing descriptor state object.
  HBOOST_ASIO_DECL void free_descriptor_state(descriptor_state* s);
  ...
  HBOOST_ASIO_DECL int get_timeout(itimerspec& ts);
...
#include <hboost/asio/detail/impl/epoll_reactor.hpp>
#if defined(HBOOST_ASIO_HEADER_ONLY)
# include <hboost/asio/detail/impl/epoll_reactor.ipp>
#endif // defined(HBOOST_ASIO_HEADER_ONLY)

#endif // defined(HBOOST_ASIO_HAS_EPOLL)

#endif // HBOOST_ASIO_DETAIL_EPOLL_REACTOR_HPP
