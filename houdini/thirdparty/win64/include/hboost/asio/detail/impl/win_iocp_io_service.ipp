...
#ifndef HBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_IO_SERVICE_IPP
#define HBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_IO_SERVICE_IPP
...
win_iocp_io_service::win_iocp_io_service(
    hboost::asio::io_service& io_service, size_t concurrency_hint)
  : hboost::asio::detail::service_base<win_iocp_io_service>(io_service),
    iocp_(),
    outstanding_work_(0),
    stopped_(0),
...
{
  HBOOST_ASIO_HANDLER_TRACKING_INIT;
  ...
  if (!iocp_.handle)
  {
    ...
    hboost::asio::detail::throw_error(ec, "iocp");
  }
}
...
hboost::system::error_code win_iocp_io_service::register_handle(
...
size_t win_iocp_io_service::run(hboost::system::error_code& ec)
{
...
size_t win_iocp_io_service::run_one(hboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_service::poll(hboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_service::poll_one(hboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
  ...
  if (::InterlockedExchange(&stopped_, 1) == 0)
  {
    if (::InterlockedExchange(&stop_event_posted_, 1) == 0)
    {
      if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
      {
        ...
        hboost::system::error_code ec(last_error,
            hboost::asio::error::get_system_category());
        hboost::asio::detail::throw_error(ec, "pqcs");
      }
    }
  }
}
...
void win_iocp_io_service::on_completion(win_iocp_operation* op,
    const hboost::system::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
...
size_t win_iocp_io_service::do_one(bool block, hboost::system::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      hboost::system::error_code result_ec(last_error,
          hboost::asio::error::get_system_category());
      ...
      {
        result_ec = hboost::system::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<hboost::system::error_category*>(op->Internal));
...
void win_iocp_io_service::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      hboost::system::error_code ec(last_error,
          hboost::asio::error::get_system_category());
      hboost::asio::detail::throw_error(ec, "timer");
    }
...
#endif // HBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_IO_SERVICE_IPP
