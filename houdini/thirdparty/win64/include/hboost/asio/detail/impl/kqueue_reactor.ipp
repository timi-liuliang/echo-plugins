...
#ifndef HBOOST_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
#define HBOOST_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
...
# define HBOOST_ASIO_KQUEUE_EV_SET(ev, ident, filt, flags, fflags, data, udata) \
...
kqueue_reactor::kqueue_reactor(hboost::asio::io_service& io_service)
  : hboost::asio::detail::service_base<kqueue_reactor>(io_service),
    ...
    shutdown_(false)
{
  ...
  HBOOST_ASIO_KQUEUE_EV_SET(&events[0], interrupter_.read_descriptor(),
      EVFILT_READ, EV_ADD, 0, 0, &interrupter_);
  if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
  {
    hboost::system::error_code error(errno,
        hboost::asio::error::get_system_category());
    hboost::asio::detail::throw_error(error);
  }
}
...
void kqueue_reactor::fork_service(hboost::asio::io_service::fork_event fork_ev)
{
  if (fork_ev == hboost::asio::io_service::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      hboost::system::error_code ec(errno,
          hboost::asio::error::get_system_category());
      hboost::asio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      ...
      {
        HBOOST_ASIO_KQUEUE_EV_SET(&events[0], state->descriptor_,
            EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, state);
        HBOOST_ASIO_KQUEUE_EV_SET(&events[1], state->descriptor_,
            EVFILT_WRITE, EV_ADD | EV_CLEAR, 0, 0, state);
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          hboost::system::error_code ec(errno,
              hboost::asio::error::get_system_category());
          hboost::asio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, reactor_op* op)
{
  ...
  struct kevent events[1];
  HBOOST_ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        HBOOST_ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        HBOOST_ASIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        ...
        {
          op->ec_ = hboost::system::error_code(errno,
              hboost::asio::error::get_system_category());
          io_service_.post_immediate_completion(op, is_continuation);
          return;
        }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, bool closing)
{
  ...
  if (!descriptor_data->shutdown_)
  {
    ...
    {
      struct kevent events[2];
      HBOOST_ASIO_KQUEUE_EV_SET(&events[0], descriptor,
          EVFILT_READ, EV_DELETE, 0, 0, 0);
      HBOOST_ASIO_KQUEUE_EV_SET(&events[1], descriptor,
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    else
    {
          ...
          && descriptor_data->op_queue_[write_op].empty())
      {
        ...
        HBOOST_ASIO_KQUEUE_EV_SET(&delete_events[0],
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    hboost::system::error_code ec(errno,
        hboost::asio::error::get_system_category());
    hboost::asio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
...
#undef HBOOST_ASIO_KQUEUE_EV_SET

#include <hboost/asio/detail/pop_options.hpp>

#endif // defined(HBOOST_ASIO_HAS_KQUEUE)

#endif // HBOOST_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
