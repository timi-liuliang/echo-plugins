...
#ifndef HBOOST_ASIO_DETAIL_IMPL_SERVICE_REGISTRY_IPP
#define HBOOST_ASIO_DETAIL_IMPL_SERVICE_REGISTRY_IPP
...
service_registry::~service_registry()
{
  ...
  hboost::asio::io_service::service* service = first_service_;
  while (service)
  {
  ...
  {
    hboost::asio::io_service::service* next_service = first_service_->next_;
    destroy(first_service_);
    first_service_ = next_service;
  }
}

void service_registry::notify_fork(hboost::asio::io_service::fork_event fork_ev)
{
  // Make a copy of all of the services while holding the lock. We don't want
  // to hold the lock while calling into each service, as it may try to call
  // back into this class.
  std::vector<hboost::asio::io_service::service*> services;
  {
  ...
  if (fork_ev == hboost::asio::io_service::fork_prepare)
...
void service_registry::destroy(hboost::asio::io_service::service* service)
{
  delete service;
}

hboost::asio::io_service::service* service_registry::do_use_service(
    ...
    hboost::asio::io_service::service* new_service)
{
  if (&owner_ != &new_service->get_io_service())
    hboost::asio::detail::throw_exception(invalid_service_owner());
  ...
  {
    if (keys_match(service->key_, key))
      hboost::asio::detail::throw_exception(service_already_exists());
    service = service->next_;
  }
...
#endif // HBOOST_ASIO_DETAIL_IMPL_SERVICE_REGISTRY_IPP
