...
#ifndef HBOOST_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
#define HBOOST_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
...
#include <cstring>
#include <hboost/asio/detail/winrt_ssocket_service_base.hpp>
...
void winrt_ssocket_service_base::shutdown_service()
{
  ...
  hboost::asio::detail::mutex::scoped_lock lock(mutex_);
    ...
    winrt_ssocket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    hboost::asio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
hboost::system::error_code winrt_ssocket_service_base::close(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    void* addr, std::size_t addr_len, hboost::system::error_code& ec) const
{
  ...
  try
  {
    ...
    case HBOOST_ASIO_OS_DEF(AF_INET):
      ...
      {
        socket_ops::inet_pton(HBOOST_ASIO_OS_DEF(AF_INET), addr_string.c_str(),
    ...
    case HBOOST_ASIO_OS_DEF(AF_INET6):
      ...
      {
        socket_ops::inet_pton(HBOOST_ASIO_OS_DEF(AF_INET6), addr_string.c_str(),
...
hboost::system::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    std::size_t optlen, hboost::system::error_code& ec)
{
  ...
  try
  {
    if (level == HBOOST_ASIO_OS_DEF(SOL_SOCKET)
        && optname == HBOOST_ASIO_OS_DEF(SO_KEEPALIVE))
    {
    ...
    else if (level == HBOOST_ASIO_OS_DEF(IPPROTO_TCP)
        && optname == HBOOST_ASIO_OS_DEF(TCP_NODELAY))
    {
      if (optlen == sizeof(int))
      {
        int value = 0;
...
hboost::system::error_code winrt_ssocket_service_base::do_connect(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const void* addr, hboost::system::error_code& ec)
{
  ...
  {
  case HBOOST_ASIO_OS_DEF(AF_INET):
    socket_ops::inet_ntop(HBOOST_ASIO_OS_DEF(AF_INET),
    ...
    break;
  case HBOOST_ASIO_OS_DEF(AF_INET6):
    socket_ops::inet_ntop(HBOOST_ASIO_OS_DEF(AF_INET6),
    ...
    const void* addr, winrt_async_op<void>* op, bool is_continuation)
{
  if (!is_open(impl))
  {
    op->ec_ = hboost::asio::error::bad_descriptor;
  ...
  case HBOOST_ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    op->ec_ = hboost::asio::error::address_family_not_supported;
    break;
  }
  ...
  {
    op->ec_ = hboost::system::error_code(
...
std::size_t winrt_ssocket_service_base::do_send(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const hboost::asio::const_buffer& data,
    socket_base::message_flags flags, hboost::system::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<hboost::asio::const_buffer,
      hboost::asio::const_buffers_1> bufs(hboost::asio::buffer(data));
      ...
      winrt_async_op<unsigned int>* op, bool is_continuation)
{
  if (flags)
  {
    op->ec_ = hboost::asio::error::operation_not_supported;
    io_service_.post_immediate_completion(op, is_continuation);
    return;
  }
  ...
  {
    op->ec_ = hboost::system::error_code(e->HResult,
        hboost::system::system_category());
    io_service_.post_immediate_completion(op, is_continuation);
  }
}
...
std::size_t winrt_ssocket_service_base::do_receive(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const hboost::asio::mutable_buffer& data,
    socket_base::message_flags flags, hboost::system::error_code& ec)
{
  if (flags)
  {
  ...
  try
  {
    buffer_sequence_adapter<hboost::asio::mutable_buffer,
        hboost::asio::mutable_buffers_1> bufs(hboost::asio::buffer(data));
...
#endif // HBOOST_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
