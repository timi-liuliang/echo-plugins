...
#ifndef HBOOST_ASIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
#define HBOOST_ASIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
...
#include <hboost/asio/detail/signal_blocker.hpp>
#include <hboost/asio/detail/signal_set_service.hpp>
...
signal_state* get_signal_state()
{
  static signal_state state = {
    HBOOST_ASIO_STATIC_MUTEX_INIT, -1, -1, false, 0, { 0 } };
  return &state;
}

void hboost_asio_signal_handler(int signal_number)
{
...
#if defined(HBOOST_ASIO_HAS_SIGNAL) && !defined(HBOOST_ASIO_HAS_SIGACTION)
  ::signal(signal_number, hboost_asio_signal_handler);
#endif // defined(HBOOST_ASIO_HAS_SIGNAL) && !defined(HBOOST_ASIO_HAS_SIGACTION)
}
...
signal_set_service::signal_set_service(
  ...
  : io_service_(hboost::asio::use_service<io_service_impl>(io_service)),
#if !defined(HBOOST_ASIO_WINDOWS) \
  && !defined(HBOOST_ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
    reactor_(hboost::asio::use_service<reactor>(io_service)),
#endif // !defined(HBOOST_ASIO_WINDOWS)
       //   && !defined(HBOOST_ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
    next_(0),
    ...
    hboost::asio::io_service::fork_event fork_ev)
{
  ...
  switch (fork_ev)
  {
  case hboost::asio::io_service::fork_prepare:
    {
    ...
    break;
  case hboost::asio::io_service::fork_parent:
    ...
    break;
  case hboost::asio::io_service::fork_child:
    if (state->fork_prepared_)
    {
      hboost::asio::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
hboost::system::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl,
    int signal_number, hboost::system::error_code& ec)
{
...
# if defined(HBOOST_ASIO_HAS_SIGACTION)
      ...
      sa.sa_handler = hboost_asio_signal_handler;
      sigfillset(&sa.sa_mask);
      if (::sigaction(signal_number, &sa, 0) == -1)
# else // defined(HBOOST_ASIO_HAS_SIGACTION)
      if (::signal(signal_number, hboost_asio_signal_handler) == SIG_ERR)
...
hboost::system::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
    int signal_number, hboost::system::error_code& ec)
{
  // Check that the signal number is valid.
...
hboost::system::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
...
hboost::system::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    hboost::system::error_code& ec)
{
  HBOOST_ASIO_HANDLER_OPERATION(("signal_set", &impl, "cancel"));
  ...
  op_queue<operation> ops;
  {
    ...
    {
      op->ec_ = hboost::asio::error::operation_aborted;
      impl.queue_.pop();
      ops.push(op);
    }
  }
...
void signal_set_service::open_descriptors()
{
  ...
  {
    hboost::system::error_code ec(errno,
        hboost::asio::error::get_system_category());
    hboost::asio::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(HBOOST_ASIO_WINDOWS)
       //   && !defined(HBOOST_ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
...
#endif // HBOOST_ASIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
