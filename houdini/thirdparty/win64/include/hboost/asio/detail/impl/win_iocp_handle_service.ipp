...
#ifndef HBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
#define HBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
...
{
public:
  explicit overlapped_wrapper(hboost::system::error_code& ec)
  {
    Internal = 0;
    InternalHigh = 0;
    Offset = 0;
...
win_iocp_handle_service::win_iocp_handle_service(
  ...
  : iocp_service_(hboost::asio::use_service<win_iocp_io_service>(io_service)),
    mutex_(),
    impl_list_(0)
{
}
...
hboost::system::error_code win_iocp_handle_service::assign(
    win_iocp_handle_service::implementation_type& impl,
...
hboost::system::error_code win_iocp_handle_service::close(
    win_iocp_handle_service::implementation_type& impl,
    hboost::system::error_code& ec)
{
  if (is_open(impl))
  {
    HBOOST_ASIO_HANDLER_OPERATION(("handle", &impl, "close"));
...
hboost::system::error_code win_iocp_handle_service::cancel(
    win_iocp_handle_service::implementation_type& impl,
    hboost::system::error_code& ec)
{
  if (!is_open(impl))
  ...
  HBOOST_ASIO_HANDLER_OPERATION(("handle", &impl, "cancel"));
...
size_t win_iocp_handle_service::do_write(
    ...
    const hboost::asio::const_buffer& buffer, hboost::system::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = hboost::asio::error::bad_descriptor;
  ...
  BOOL ok = ::WriteFile(impl.handle_,
      hboost::asio::buffer_cast<LPCVOID>(buffer),
      static_cast<DWORD>(hboost::asio::buffer_size(buffer)), 0, &overlapped);
  if (!ok) 
  {
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_write_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const hboost::asio::const_buffer& buffer, operation* op)
{
  ...
  else if (hboost::asio::buffer_size(buffer) == 0)
  {
    // A request to write 0 bytes on a handle is a no-op.
  ...
  else
  {
    ...
    BOOL ok = ::WriteFile(impl.handle_,
        hboost::asio::buffer_cast<LPCVOID>(buffer),
        static_cast<DWORD>(hboost::asio::buffer_size(buffer)),
...
size_t win_iocp_handle_service::do_read(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const hboost::asio::mutable_buffer& buffer, hboost::system::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = hboost::asio::error::bad_descriptor;
  ...
  BOOL ok = ::ReadFile(impl.handle_,
      hboost::asio::buffer_cast<LPVOID>(buffer),
      static_cast<DWORD>(hboost::asio::buffer_size(buffer)), 0, &overlapped);
  if (!ok) 
  {
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_read_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const hboost::asio::mutable_buffer& buffer, operation* op)
{
  update_cancellation_thread_id(impl);
  iocp_service_.work_started();
...
#endif // HBOOST_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
