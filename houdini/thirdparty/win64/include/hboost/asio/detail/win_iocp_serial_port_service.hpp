...
#ifndef HBOOST_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
#define HBOOST_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
...
#if defined(HBOOST_ASIO_HAS_IOCP) && defined(HBOOST_ASIO_HAS_SERIAL_PORT)
...
public:
  ...
  // Constructor.
  HBOOST_ASIO_DECL win_iocp_serial_port_service(
  ...
  HBOOST_ASIO_DECL hboost::system::error_code open(implementation_type& impl,
      const std::string& device, hboost::system::error_code& ec);
  ...
  hboost::system::error_code set_option(implementation_type& impl,
      const SettableSerialPortOption& option, hboost::system::error_code& ec)
  {
  ...
  hboost::system::error_code get_option(const implementation_type& impl,
      GettableSerialPortOption& option, hboost::system::error_code& ec) const
  {
  ...
  hboost::system::error_code send_break(implementation_type&,
      hboost::system::error_code& ec)
  {
...
private:
  ...
  typedef hboost::system::error_code (*store_function_type)(
      const void*, ::DCB&, hboost::system::error_code&);

  // Helper function template to store a serial port option.
  template <typename SettableSerialPortOption>
  static hboost::system::error_code store_option(const void* option,
      ::DCB& storage, hboost::system::error_code& ec)
  {
  ...
  HBOOST_ASIO_DECL hboost::system::error_code do_set_option(
      ...
      const void* option, hboost::system::error_code& ec);

  // Function pointer type for loading a serial port option.
  typedef hboost::system::error_code (*load_function_type)(
      void*, const ::DCB&, hboost::system::error_code&);

  // Helper function template to load a serial port option.
  template <typename GettableSerialPortOption>
  static hboost::system::error_code load_option(void* option,
      const ::DCB& storage, hboost::system::error_code& ec)
  {
    return static_cast<GettableSerialPortOption*>(option)->load(storage, ec);
  }

  // Helper function to get a serial port option.
  HBOOST_ASIO_DECL hboost::system::error_code do_get_option(
      const implementation_type& impl, load_function_type load,
      void* option, hboost::system::error_code& ec) const;
...
# include <hboost/asio/detail/impl/win_iocp_serial_port_service.ipp>
#endif // defined(HBOOST_ASIO_HEADER_ONLY)

#endif // defined(HBOOST_ASIO_HAS_IOCP) && defined(HBOOST_ASIO_HAS_SERIAL_PORT)

#endif // HBOOST_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
