...
#ifndef HBOOST_ASIO_IP_BASIC_RESOLVER_QUERY_HPP
#define HBOOST_ASIO_IP_BASIC_RESOLVER_QUERY_HPP
...
/**
 * The hboost::asio::ip::basic_resolver_query class template describes a query
...
public:
      ...
      service_name_(service)
  {
    typename InternetProtocol::endpoint endpoint;
    hints_.ai_flags = static_cast<int>(resolve_flags);
    hints_.ai_family = HBOOST_ASIO_OS_DEF(AF_UNSPEC);
    hints_.ai_socktype = endpoint.protocol().type();
    hints_.ai_protocol = endpoint.protocol().protocol();
    hints_.ai_addrlen = 0;
    hints_.ai_canonname = 0;
  ...
  const hboost::asio::detail::addrinfo_type& hints() const
  {
    return hints_;
  }
...
private:
  hboost::asio::detail::addrinfo_type hints_;
...
#endif // HBOOST_ASIO_IP_BASIC_RESOLVER_QUERY_HPP
