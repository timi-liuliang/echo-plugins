...
#ifndef HBOOST_ASIO_IP_IMPL_ADDRESS_V6_IPP
#define HBOOST_ASIO_IP_IMPL_ADDRESS_V6_IPP
...
std::string address_v6::to_string(hboost::system::error_code& ec) const
{
  char addr_str[hboost::asio::detail::max_addr_v6_str_len];
  const char* addr =
    hboost::asio::detail::socket_ops::inet_ntop(
        HBOOST_ASIO_OS_DEF(AF_INET6), &addr_, addr_str,
        hboost::asio::detail::max_addr_v6_str_len, scope_id_, ec);
  if (addr == 0)
    return std::string();
  return addr;
}
    ...
    const char* str, hboost::system::error_code& ec)
{
  ...
  if (hboost::asio::detail::socket_ops::inet_pton(
        HBOOST_ASIO_OS_DEF(AF_INET6), str, &tmp.addr_, &tmp.scope_id_, ec) <= 0)
    return address_v6();
  return tmp;
}
...
bool operator==(const address_v6& a1, const address_v6& a2)
{
  ...
  return memcmp(&a1.addr_, &a2.addr_,
      sizeof(hboost::asio::detail::in6_addr_type)) == 0
...
bool operator<(const address_v6& a1, const address_v6& a2)
{
  ...
  int memcmp_result = memcmp(&a1.addr_, &a2.addr_,
      sizeof(hboost::asio::detail::in6_addr_type));
  if (memcmp_result < 0)
    return true;
  if (memcmp_result > 0)
    return false;
...
#endif // HBOOST_ASIO_IP_IMPL_ADDRESS_V6_IPP
