...
#ifndef HBOOST_ASIO_BUFFERED_READ_STREAM_HPP
#define HBOOST_ASIO_BUFFERED_READ_STREAM_HPP
...
#include <hboost/asio/detail/buffer_resize_guard.hpp>
...
public:
      ...
      HBOOST_ASIO_MOVE_ARG(WriteHandler) handler)
  {
    ...
    next_layer_.async_write_some(buffers,
        HBOOST_ASIO_MOVE_CAST(HBOOST_ASIO_HANDLER_TYPE(WriteHandler,
  ...
  std::size_t fill(hboost::system::error_code& ec);

  /// Start an asynchronous fill.
  template <typename ReadHandler>
  HBOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler,
      void (hboost::system::error_code, std::size_t))
  async_fill(HBOOST_ASIO_MOVE_ARG(ReadHandler) handler);
...
private:
  ...
  {
    std::size_t bytes_copied = hboost::asio::buffer_copy(
  ...
  {
    return hboost::asio::buffer_copy(buffers, storage_.data(), storage_.size());
  }
...
#include <hboost/asio/impl/buffered_read_stream.hpp>

#endif // HBOOST_ASIO_BUFFERED_READ_STREAM_HPP
