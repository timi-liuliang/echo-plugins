...
#ifndef HBOOST_XPRESSIVE_REGEX_COMPILER_HPP_EAN_10_04_2005
#define HBOOST_XPRESSIVE_REGEX_COMPILER_HPP_EAN_10_04_2005
...
#include <hboost/xpressive/detail/dynamic/parser.hpp>
#include <hboost/xpressive/detail/dynamic/parse_charset.hpp>
#include <hboost/xpressive/detail/dynamic/parser_enum.hpp>
#include <hboost/xpressive/detail/dynamic/parser_traits.hpp>
#include <hboost/xpressive/detail/core/linker.hpp>
#include <hboost/xpressive/detail/core/optimize.hpp>
...
struct regex_compiler
{
    ...
    {
        return this->compile(hboost::begin(pat), hboost::end(pat), flags);
    }
...
private:
    ...
    {
        HBOOST_MPL_ASSERT((is_same<char_type, typename iterator_value<FwdIter>::type>));
        ...
        if(token_group_begin == this->traits_.get_token(tmp, end) &&
           HBOOST_XPR_ENSURE_(tmp != end, error_paren, "mismatched parenthesis") &&
        ...
        HBOOST_XPR_ENSURE_(begin == end, error_paren, "mismatched parenthesis");
    ...
    basic_regex<BidiIter> compile_(InputIter begin, InputIter end, flag_type flags, std::input_iterator_tag)
    {
        ...
        return this->compile_(hboost::begin(pat), hboost::end(pat), flags, std::forward_iterator_tag());
    }
    ...
    detail::sequence<BidiIter> parse_group(FwdIter &begin, FwdIter end)
    {
        ...
        case token_comment:
            while(HBOOST_XPR_ENSURE_(begin != end, error_paren, "mismatched parenthesis"))
            {
                ...
                case token_escape:
                    HBOOST_XPR_ENSURE_(begin != end, error_escape, "incomplete escape sequence");
                    HBOOST_FALLTHROUGH;
                case token_literal:
                    ++begin;
                    break;
        ...
        case token_named_mark:
            ...
            {
                HBOOST_XPR_ENSURE_(this->self_->named_marks_[i].name_ != name, error_badmark, "named mark already exists");
            }
    ...
    detail::sequence<BidiIter> parse_atom(FwdIter &begin, FwdIter end)
    {
        ...
        case token_invalid_quantifier:
            HBOOST_THROW_EXCEPTION(regex_error(error_badrepeat, "quantifier not expected"));
            break;
    ...
    detail::sequence<BidiIter> parse_quant(FwdIter &begin, FwdIter end)
    {
        ...
        if(!seq.empty() && begin != end && detail::quant_none != seq.quant())
        {
            ...
            {
                HBOOST_ASSERT(spec.min_ <= spec.max_);
    ...
    string_type parse_literal(FwdIter &begin, FwdIter end)
    {
        using namespace regex_constants;
        HBOOST_ASSERT(begin != end);
        HBOOST_ASSERT(token_literal == this->traits_.get_token(begin, end));
        escape_value esc = { 0, 0, 0, detail::escape_char };
        ...
        for(FwdIter prev = begin, tmp = ++begin; begin != end; prev = begin, begin = tmp)
        {
            ...
            if(this->traits_.get_quant_spec(tmp, end, spec))
            {
                ...
                {
                    begin = prev;
                    literal.erase(hboost::prior(literal.end()));
                }
                return literal;
            }
    ...
    {
        HBOOST_XPR_ENSURE_(begin != end, regex_constants::error_escape, "incomplete escape sequence");
