...
#ifndef HBOOST_XPRESSIVE_MATCH_RESULTS_HPP_EAN_10_04_2005
#define HBOOST_XPRESSIVE_MATCH_RESULTS_HPP_EAN_10_04_2005
...
#include <hboost/iterator_adaptors.hpp>
...
#if HBOOST_ITERATOR_ADAPTORS_VERSION >= 0x0200
...
#include <hboost/xpressive/detail/core/sub_match_vector.hpp>
#include <hboost/xpressive/detail/utility/sequence_stack.hpp>
#include <hboost/xpressive/detail/core/results_cache.hpp>
#include <hboost/xpressive/detail/utility/literals.hpp>
#include <hboost/xpressive/detail/utility/algorithm.hpp>
#include <hboost/xpressive/detail/utility/counted_base.hpp>
// Doxygen can't handle proto :-(
#ifndef HBOOST_XPRESSIVE_DOXYGEN_INVOKED
  ...
  : std::iterator<std::output_iterator_tag, Char, void, void, case_converting_iterator<OutputIterator, Char> >
{
    ...
    {
        HBOOST_ASSERT(scope == scope_next || scope == scope_rest);
        if(scope == scope_next)
            iter.next_ = trans;
        else
            iter.rest_ = trans;
...
#if HBOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ == 0)
...
public:
    ...
    {
        return this->format_(out, hboost::as_literal(fmt), flags, mpl::size_t<0>());
    }
    ...
    match_results<BidiIter> &let(Arg const &arg)
    {
        ...
        HBOOST_MPL_ASSERT((proto::matches<Arg, detail::ActionArgBinding>));
        HBOOST_MPL_ASSERT((is_same<typename arg_left_type::type, arg_right_type>));
...
private:
    ...
    {
        return std::copy(hboost::begin(result), hboost::end(result), out);
    }
    ...
    ) const
    {
        ...
        iterator cur = hboost::begin(format), end = hboost::end(format);
    ...
    OutputIterator format_perl_(ForwardIterator cur, ForwardIterator end, OutputIterator out) const
    {
        ...
        while(cur != end)
        {
            ...
            case HBOOST_XPR_CHAR_(char_type, '\\'):
                if(++cur != end && HBOOST_XPR_CHAR_(char_type, 'g') == *cur)
                {
    ...
    OutputIterator format_all_impl_(ForwardIterator &cur, ForwardIterator end, OutputIterator out, bool metacolon = false) const
    {
        ...
        while(cur != end)
        {
            ...
            case HBOOST_XPR_CHAR_(char_type, '('):
                ...
                HBOOST_XPR_ENSURE_(HBOOST_XPR_CHAR_(char_type, ')') == *(cur-1)
                  , regex_constants::error_paren, "unbalanced parentheses in format string");
                break;
            ...
            case HBOOST_XPR_CHAR_(char_type, '?'):
                ...
                HBOOST_XPR_ENSURE_(0 != sub, regex_constants::error_subreg, "invalid back-reference");
                if(this->sub_matches_[ sub ].matched)
                {
                    out = this->format_all_impl_(cur, end, out, true);
                    if(HBOOST_XPR_CHAR_(char_type, ':') == *(cur-1))
    ...
    ) const
    {
        if(cur == end)
        {
            *out++ = HBOOST_XPR_CHAR_(char_type, '$');
        }
        else if(HBOOST_XPR_CHAR_(char_type, '$') == *cur)
        {
            *out++ = *cur++;
        }
        else if(HBOOST_XPR_CHAR_(char_type, '&') == *cur) // whole match
        {
            ++cur;
            out = std::copy(this->sub_matches_[ 0 ].first, this->sub_matches_[ 0 ].second, out);
        }
        else if(HBOOST_XPR_CHAR_(char_type, '`') == *cur) // prefix
        {
            ++cur;
            out = std::copy(this->prefix().first, this->prefix().second, out);
        }
        else if(HBOOST_XPR_CHAR_(char_type, '\'') == *cur) // suffix
        {
            ++cur;
    ...
    ) const
    {
        ...
        case HBOOST_XPR_CHAR_(char_type, 'a'):
            *out++ = HBOOST_XPR_CHAR_(char_type, '\a');
            break;
        ...
        case HBOOST_XPR_CHAR_(char_type, 'f'):
            *out++ = HBOOST_XPR_CHAR_(char_type, '\f');
            break;
        ...
        case HBOOST_XPR_CHAR_(char_type, 'r'):
            *out++ = HBOOST_XPR_CHAR_(char_type, '\r');
            break;
        ...
        case HBOOST_XPR_CHAR_(char_type, 'v'):
            *out++ = HBOOST_XPR_CHAR_(char_type, '\v');
            break;

        case HBOOST_XPR_CHAR_(char_type, 'x'):
            HBOOST_XPR_ENSURE_(cur != end, error_escape, "unexpected end of format found");
            if(HBOOST_XPR_CHAR_(char_type, '{') == *cur)
            {
                HBOOST_XPR_ENSURE_(++cur != end, error_escape, "unexpected end of format found");
                tmp = cur;
                *out++ = converter(detail::toi(cur, end, *this->traits_, 16, 0xffff));
                HBOOST_XPR_ENSURE_(4 == std::distance(tmp, cur) && cur != end && HBOOST_XPR_CHAR_(char_type, '}') == *cur++
            ...
            else
            {
                ...
                *out++ = converter(detail::toi(cur, end, *this->traits_, 16, 0xff));
                HBOOST_XPR_ENSURE_(2 == std::distance(tmp, cur), error_escape
        ...
        case HBOOST_XPR_CHAR_(char_type, 'c'):
            ...
            HBOOST_XPR_ENSURE_
            (
                this->traits_->in_range(HBOOST_XPR_CHAR_(char_type, 'a'), HBOOST_XPR_CHAR_(char_type, 'z'), *cur)
             || this->traits_->in_range(HBOOST_XPR_CHAR_(char_type, 'A'), HBOOST_XPR_CHAR_(char_type, 'Z'), *cur)
              , error_escape
        ...
        case HBOOST_XPR_CHAR_(char_type, 'l'):
            if(!set_transform(out, detail::op_lower, detail::scope_next))
            {
                *out++ = HBOOST_XPR_CHAR_(char_type, 'l');
            }
            break;
        ...
        case HBOOST_XPR_CHAR_(char_type, 'u'):
            ...
            {
                *out++ = HBOOST_XPR_CHAR_(char_type, 'u');
            }
            break;
        ...
        case HBOOST_XPR_CHAR_(char_type, 'E'):
            ...
            {
                *out++ = HBOOST_XPR_CHAR_(char_type, 'E');
            }
            break;
    ...
    {
        using namespace regex_constants;
        HBOOST_XPR_ENSURE_(cur != end && HBOOST_XPR_CHAR_(char_type, '<') == *cur++
            , error_badmark, "invalid named back-reference");
        ForwardIterator begin = cur;
        for(; cur != end && HBOOST_XPR_CHAR_(char_type, '>') != *cur; ++cur)
        {}
        HBOOST_XPR_ENSURE_(cur != begin && cur != end && HBOOST_XPR_CHAR_(char_type, '>') == *cur
            , error_badmark, "invalid named back-reference");
        ...
        HBOOST_THROW_EXCEPTION(regex_error(error_badmark, "invalid named back-reference"));
    ...
    hboost::optional<BidiIter> base_;
    hboost::optional<sub_match<BidiIter> > prefix_;
    hboost::optional<sub_match<BidiIter> > suffix_;
...
namespace std
{
    ...
    concept_map OutputIterator<
        hboost::xpressive::detail::noop_output_iterator<Char_>
      , Char_
    >
    {};
}
