...
#ifndef HBOOST_XPRESSIVE_DETAIL_SYMBOLS_HPP_DRJ_06_11_2007
#define HBOOST_XPRESSIVE_DETAIL_SYMBOLS_HPP_DRJ_06_11_2007
...
namespace hboost { namespace xpressive { namespace detail
{
    ...
    struct symbols
    {
        ...
        {
            iterator begin = hboost::begin(map);
            iterator end = hboost::end(map);
            node* root_p = this->root.get();
            for(; begin != end; ++begin)
            {
                key_iterator kbegin = hboost::begin(begin->first);
                key_iterator kend = hboost::end(begin->first);
