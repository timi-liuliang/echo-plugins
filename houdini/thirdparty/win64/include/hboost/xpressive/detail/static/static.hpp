...
#ifndef HBOOST_XPRESSIVE_DETAIL_STATIC_STATIC_HPP_EAN_10_04_2005
#define HBOOST_XPRESSIVE_DETAIL_STATIC_STATIC_HPP_EAN_10_04_2005
...
#include <hboost/xpressive/detail/core/peeker.hpp>
#include <hboost/xpressive/detail/static/placeholders.hpp>
  ...
  : Next
{
    ...
    bool match(match_state<BidiIter> &state) const
    {
        return static_cast<Next const *>(this)->
            HBOOST_NESTED_TEMPLATE push_match<Top>(state);
    }
...
//protected:
    ...
    {
        return that.HBOOST_NESTED_TEMPLATE push_match<Top>(state);
    }
};
...
inline stacked_xpression<Top, Next> const &stacked_xpression_cast(Next const &next)
{
    ...
    HBOOST_MPL_ASSERT_RELATION(sizeof(stacked_xpression<Top, Next>), ==, sizeof(Next));
  ...
  : Matcher
{
    ...
    HBOOST_STATIC_CONSTANT(bool, pure = Matcher::pure && Next::pure);
...
struct no_next
{
    HBOOST_STATIC_CONSTANT(std::size_t, width = 0);
    HBOOST_STATIC_CONSTANT(bool, pure = true);
