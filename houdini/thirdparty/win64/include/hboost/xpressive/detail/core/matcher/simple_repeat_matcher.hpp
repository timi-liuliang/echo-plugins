...
#ifndef HBOOST_XPRESSIVE_DETAIL_CORE_MATCHER_SIMPLE_REPEAT_MATCHER_HPP_EAN_10_04_2005
#define HBOOST_XPRESSIVE_DETAIL_CORE_MATCHER_SIMPLE_REPEAT_MATCHER_HPP_EAN_10_04_2005
...
#include <hboost/xpressive/detail/static/type_traits.hpp>
...
namespace hboost { namespace xpressive { namespace detail
{
      ...
      : quant_style_variable_width
    {
          ...
          , leading_(false)
        {
            ...
            HBOOST_ASSERT(min <= max);
            HBOOST_ASSERT(0 != max);
            HBOOST_ASSERT(0 != width && unknown_width() != width);
            HBOOST_ASSERT(Xpr::width == unknown_width() || Xpr::width == width);
        }
        ...
        bool match_(match_state<BidiIter> &state, Next const &next, greedy_slow_tag) const
        {
            ...
            if(this->leading_)
            {
                state.next_search_ = (matches && matches < this->max_)
                                   ? state.cur_
                                   : (tmp == state.end_) ? tmp : hboost::next(tmp);
            }
        ...
        bool match_(match_state<BidiIter> &state, Next const &next, greedy_fast_tag) const
        {
            ...
            if(this->min_ > diff_to_end)
            {
                if(this->leading_)
                {
                    state.next_search_ = (tmp == state.end_) ? tmp : hboost::next(tmp);
                }
                return false;
            }
