...
#ifndef HBOOST_XPRESSIVE_DETAIL_CORE_MATCHER_OPTIONAL_MATCHER_HPP_EAN_10_04_2005
#define HBOOST_XPRESSIVE_DETAIL_CORE_MATCHER_OPTIONAL_MATCHER_HPP_EAN_10_04_2005
...
namespace hboost { namespace xpressive { namespace detail
{
    ...
    private:
        ...
        {
            return this->xpr_.HBOOST_NESTED_TEMPLATE push_match<Next>(state)
                || next.match(state);
        }
        ...
        bool match_(match_state<BidiIter> &state, Next const &next, mpl::false_) const // Non-greedy
        {
            return next.match(state)
                || this->xpr_.HBOOST_NESTED_TEMPLATE push_match<Next>(state);
        }
