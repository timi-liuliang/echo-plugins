...
#ifndef HBOOST_XPRESSIVE_ALGORITHMS_HPP_EAN_10_04_2005
#define HBOOST_XPRESSIVE_ALGORITHMS_HPP_EAN_10_04_2005
...
#include <hboost/xpressive/detail/utility/save_restore.hpp>

/// INTERNAL ONLY
///
#define HBOOST_XPR_NONDEDUCED_TYPE_(x) typename mpl::identity<x>::type
...
namespace detail
{
    ...
    (
        HBOOST_XPR_NONDEDUCED_TYPE_(BidiIter) begin
      , HBOOST_XPR_NONDEDUCED_TYPE_(BidiIter) end
...
inline bool regex_match
(
    HBOOST_XPR_NONDEDUCED_TYPE_(Char) *begin
...
)
{
    ...
    // to BidiIter here.
    BidiIter begin = hboost::begin(rng), end = hboost::end(rng);
    return detail::regex_match_impl(begin, end, what, re, flags);
}
        ...
        else if(impl.finder_ && (!partial_ok || impl.finder_->ok_for_partial_matches()))
        {
            ...
            if(find(state))
            {
                ...
                do
                {
                    ...
                    HBOOST_ASSERT(state.cur_ == sub0begin);
                    not_null.restore();
                }
...
)
{
    ...
    // Note that the result iterator of the range must be convertible
    // to BidiIter here.
    BidiIter begin = hboost::begin(str), end = hboost::end(str);
...
(
    HBOOST_XPR_NONDEDUCED_TYPE_(Char) *str
  , basic_regex<Char *> const &re
  , Formatter const &format
  , regex_constants::match_flag_type flags = regex_constants::match_default
  , typename disable_if<detail::is_char_ptr<Formatter> >::type * = 0
