    ...
    #include <hboost/proto/transform/detail/preprocessed/when.hpp>
    ...
    #define HBOOST_PP_ITERATION_PARAMS_1                                                             \
        (3, (0, HBOOST_PROTO_MAX_ARITY, <hboost/proto/transform/detail/when.hpp>))
    ...
    /// ObjectTransforms.
    template<typename Grammar, typename R HBOOST_PP_ENUM_TRAILING_PARAMS(N, typename A)>
    struct when<Grammar, R(HBOOST_PP_ENUM_PARAMS(N, A))>
      : detail::when_impl<Grammar, R, R(HBOOST_PP_ENUM_PARAMS(N, A))>
    {};
