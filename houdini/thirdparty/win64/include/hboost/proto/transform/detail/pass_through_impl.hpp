    ...
    #include <hboost/proto/transform/detail/preprocessed/pass_through_impl.hpp>

#elif !defined(HBOOST_PP_IS_ITERATING)

    #define HBOOST_PROTO_DEFINE_TRANSFORM_TYPE(Z, N, DATA)                                   \
        typename Grammar::HBOOST_PP_CAT(proto_child, N)::template impl<                      \
    ...
    #define HBOOST_PROTO_DEFINE_TRANSFORM(Z, N, DATA)                                        \
        typename Grammar::HBOOST_PP_CAT(proto_child, N)::template impl<                      \
            typename result_of::child_c<Expr, N>::type                                      \
          , State                                                                           \
          , Data                                                                            \
        >()(                                                                                \
            e.proto_base().HBOOST_PP_CAT(child, N), s, d                                     \
        )                                                                                   \
        /**/
    ...
    #define HBOOST_PP_ITERATION_PARAMS_1 (3, (1, HBOOST_PROTO_MAX_ARITY, <hboost/proto/transform/detail/pass_through_impl.hpp>))
    ...
    #undef HBOOST_PROTO_DEFINE_TRANSFORM
    #undef HBOOST_PROTO_DEFINE_TRANSFORM_TYPE
      ...
      : transform_impl<Expr, State, Data>
    {
        ...
        typedef
            typename base_expr<
              ...
              , HBOOST_PP_CAT(list, N)<
                    HBOOST_PP_ENUM(N, HBOOST_PROTO_DEFINE_TRANSFORM_TYPE, ~)
                >
            >::type
        expr_type;

        typedef typename result_domain::proto_generator proto_generator;
        typedef typename HBOOST_PROTO_RESULT_OF<proto_generator(expr_type)>::type result_type;

        HBOOST_FORCEINLINE
        HBOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(result_type, result_type const)
        operator ()(
        ...
        ) const
        {
            expr_type const that = {
                HBOOST_PP_ENUM(N, HBOOST_PROTO_DEFINE_TRANSFORM, ~)
            };
