...
#ifndef HBOOST_PROTO_TRANSFORM_ARG_HPP_EAN_11_01_2007
#define HBOOST_PROTO_TRANSFORM_ARG_HPP_EAN_11_01_2007
...
#include <hboost/proto/transform/env.hpp>
...
namespace hboost { namespace proto
{
    ...
    /// assert( hboost::addressof(i) == hboost::addressof(j) );
    /// \endcode
    ...
    struct _state : transform<_state>
    {
        ...
        struct impl : transform_impl<Expr, State, Data>
        {
            ...
            /// \return \c s
            /// \throw nothrow
            HBOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(result_type, typename impl::state_param)
            operator ()(
    ...
    struct _child_c : transform<_child_c<N> >
    {
        ...
        struct impl : transform_impl<Expr, State, Data>
        {
            ...
            /// \throw nothrow
            #ifdef HBOOST_PROTO_STRICT_RESULT_OF
            result_type
            #else
    ...
    /// in a \c hboost::reference_wrapper\<\>.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// hboost::reference_wrapper<proto::terminal<int>::type> j
    ///     = proto::when<_, proto::_byref(_)>()(i);
    /// assert( hboost::addressof(i) == hboost::addressof(j.get()) );
    /// \endcode
    struct _byref : callable
    {
        ...
        {
            typedef hboost::reference_wrapper<T const> const type;
        };

        template<typename This, typename T>
        struct result<This(T &)>
        {
            typedef hboost::reference_wrapper<T> const type;
        };

        /// Wrap the parameter \c t in a \c hboost::reference_wrapper\<\>
        /// \param t The object to wrap
        /// \return <tt>hboost::ref(t)</tt>
        /// \throw nothrow
        template<typename T>
        hboost::reference_wrapper<T> const operator ()(T &t) const
        {
            return hboost::reference_wrapper<T>(t);
        }

        /// \overload
        ///
        template<typename T>
        hboost::reference_wrapper<T const> const operator ()(T const &t) const
        {
            return hboost::reference_wrapper<T const>(t);
        }
    };

    /// \brief A unary CallableTransform that strips references
    /// and \c hboost::reference_wrapper\<\> from its argument.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// int j = 67;
    /// int k = proto::when<_, proto::_byval(proto::_state)>()(i, hboost::ref(j));
    /// assert( 67 == k );
    /// \endcode
    struct _byval : callable
    {
        ...
        template<typename This, typename T>
        struct result<This(hboost::reference_wrapper<T>)>
          : result<This(T)>
        {};
        ...
        ///
        template<typename T>
        T operator ()(hboost::reference_wrapper<T> const &t) const
        {
            return t;
        }
    };
