...
#ifndef HBOOST_PROTO_TRANSFORM_ENV_HPP_EAN_18_07_2012
#define HBOOST_PROTO_TRANSFORM_ENV_HPP_EAN_18_07_2012
...
#include <hboost/proto/detail/poly_function.hpp>
#include <hboost/proto/detail/is_noncopyable.hpp>
...
namespace hboost
{
    ...
    #define HBOOST_PROTO_DEFINE_ENV_VAR(TAG, NAME)                                                   \
        struct TAG                                                                                  \
        {                                                                                           \
            template<typename Value>                                                                \
            hboost::proto::env<TAG, Value &> const                                                   \
                operator =(hboost::reference_wrapper<Value> &value) const                            \
            {                                                                                       \
                return hboost::proto::env<TAG, Value &>(value.get());                                \
            }                                                                                       \
            template<typename Value>                                                                \
            hboost::proto::env<TAG, Value &> const                                                   \
                operator =(hboost::reference_wrapper<Value> const &value) const                      \
            {                                                                                       \
                return hboost::proto::env<TAG, Value &>(value.get());                                \
            }                                                                                       \
            template<typename Value>                                                                \
            typename hboost::disable_if_c<                                                           \
                hboost::is_const<Value>::value                                                       \
              , hboost::proto::env<TAG, typename hboost::proto::detail::value_type<Value>::type>      \
            >::type const operator =(Value &value) const                                            \
            {                                                                                       \
                return hboost::proto::env<TAG, typename hboost::proto::detail::value_type<Value>::type>(value); \
            }                                                                                       \
            template<typename Value>                                                                \
            hboost::proto::env<TAG, typename hboost::proto::detail::value_type<Value const>::type> const \
                operator =(Value const &value) const                                                \
            {                                                                                       \
                return hboost::proto::env<TAG, typename hboost::proto::detail::value_type<Value const>::type>(value); \
            }                                                                                       \
        };                                                                                          \
                                                                                                    \
        TAG const NAME = {}                                                                         \
        ...
        namespace envns_
        {
            ...
            public:
                ...
                #if HBOOST_WORKAROUND(__GNUC__, == 3) || (HBOOST_WORKAROUND(__GNUC__, == 4) && __GNUC_MINOR__ <= 2)
                /// INTERNAL ONLY
                struct found
                {
                    typedef Value type;
            ...
            HBOOST_PROTO_DEFINE_ENV_VAR(data_type, data);
        }
        ...
        namespace functional
        {
            ...
            struct as_env
            {
                ...
                typename impl<typename detail::normalize_arg<T &>::type>::result_type const
                    operator()(T &t HBOOST_PROTO_DISABLE_IF_IS_CONST(T)) const
                {
        ...
        namespace result_of
        {
            template<typename T>
            struct as_env
              : HBOOST_PROTO_RESULT_OF<functional::as_env(T)>
            {};

            template<typename Env, typename Key>
            struct has_env_var
              : HBOOST_PROTO_RESULT_OF<functional::has_env_var<Key>(Env)>::type
            {};

            template<typename Env, typename Key>
            struct env_var
              : HBOOST_PROTO_RESULT_OF<functional::env_var<Key>(Env)>
            {};
        }
        ...
        // as_env
        template<typename T>
        typename proto::result_of::as_env<T &>::type const as_env(T &t HBOOST_PROTO_DISABLE_IF_IS_CONST(T))
        {
        ...
        typename proto::result_of::has_env_var<Env &, Key>::type has_env_var(Env &e HBOOST_PROTO_DISABLE_IF_IS_CONST(Env))
        {
        ...
        // env_var
        template<typename Key, typename Env>
        typename proto::result_of::env_var<Env &, Key>::type env_var(Env &e HBOOST_PROTO_DISABLE_IF_IS_CONST(Env))
        {
        ...
        namespace envns_
        {
            ...
            inline typename disable_if_c<
                is_const<T>::value
              , env<T1, V1, HBOOST_PROTO_UNCVREF(typename result_of::as_env<T &>::type)>
            >::type const operator,(T &t, env<T1, V1> const &head)
            {
                return env<T1, V1, HBOOST_PROTO_UNCVREF(typename result_of::as_env<T &>::type)>(
                    head[T1()]
                  , proto::as_env(t)
                );
            }
            ...
            template<typename T, typename T1, typename V1>
            inline env<T1, V1, HBOOST_PROTO_UNCVREF(typename result_of::as_env<T const &>::type)> const
                operator,(T const &t, env<T1, V1> const &head)
            {
                return env<T1, V1, HBOOST_PROTO_UNCVREF(typename result_of::as_env<T const &>::type)>(
                    head[T1()]
                  , proto::as_env(t)
                );
            }
          ...
          : proto::transform<_env_var<Key> >
        {
              ...
              : transform_impl<Expr, State, Data>
            {
                ...
                HBOOST_MPL_ASSERT_NOT((is_same<result_type, key_not_found>)); // lookup failed

                HBOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(result_type, typename impl::data::template lookup<Key>::const_reference)
                operator ()(
          ...
          : transform<_env>
        {
              ...
              : transform_impl<Expr, State, Data>
            {
                ...
                HBOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(result_type, typename impl::data_param)
                operator ()(
                    typename impl::expr_param
                  , typename impl::state_param
                  , typename impl::data_param d
