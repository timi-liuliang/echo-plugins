...
#ifndef HBOOST_PROTO_MATCHES_HPP_EAN_11_03_2006
#define HBOOST_PROTO_MATCHES_HPP_EAN_11_03_2006
...
#include <hboost/proto/transform/when.hpp>
...
#define HBOOST_PROTO_LOGICAL_typename_G  HBOOST_PP_ENUM_PARAMS(HBOOST_PROTO_MAX_LOGICAL_ARITY, typename G)
#define HBOOST_PROTO_LOGICAL_G           HBOOST_PP_ENUM_PARAMS(HBOOST_PROTO_MAX_LOGICAL_ARITY, G)
...
namespace hboost { namespace proto
{
    ...
    namespace detail
    {
        ...
        template<typename T, typename U
            HBOOST_PROTO_TEMPLATE_ARITY_PARAM(long Arity = detail::template_arity<U>::value)
        >
        ...
        template<typename T>
        struct lambda_matches<T, proto::_ HBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : mpl::true_
        {};

        template<typename T>
        struct lambda_matches<T, T HBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : mpl::true_
        {};

        template<typename T, std::size_t M, typename U>
        struct lambda_matches<T[M], U HBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : array_matches<T[M], U>
        {};

        template<typename T, std::size_t M>
        struct lambda_matches<T[M], _ HBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : mpl::true_
        {};

        template<typename T, std::size_t M>
        struct lambda_matches<T[M], T[M] HBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : mpl::true_
        {};

        template<template<typename> class T, typename Expr0, typename Grammar0>
        struct lambda_matches<T<Expr0>, T<Grammar0> HBOOST_PROTO_TEMPLATE_ARITY_PARAM(1) >
        ...
        template<
            typename T
          , typename U
          , bool B = is_array<HBOOST_PROTO_UNCVREF(T)>::value
        >
        ...
        struct terminal_matches
          : mpl::or_<
                mpl::and_<
                  ...
                  , lambda_matches<
                        HBOOST_PROTO_UNCVREF(T)
                      , HBOOST_PROTO_UNCVREF(U)
                    >
                >
        ...
        #include <hboost/proto/detail/and_n.hpp>
        #include <hboost/proto/detail/or_n.hpp>
        #include <hboost/proto/detail/matches_.hpp>
        #include <hboost/proto/detail/vararg_matches_impl.hpp>
        #include <hboost/proto/detail/lambda_matches.hpp>
    ...
    ///     <tt>hboost::result_of\<when\<_,T\>(E,int,int)\>::type::value</tt>
    ...
    ///     \c E matches <tt>C::case_\<hboost::result_of\<T(E)\>::type\></tt>.
    ...
    /// <tt>hboost::result_of\<when\<_,If\>(E,int,int)\>::type::value</tt>
    /// is \c true and \c E matches \c U; or, if
    /// <tt>hboost::result_of\<when\<_,If\>(E,int,int)\>::type::value</tt>
    ...
    /// if <tt>hboost::result_of\<when\<_,If\>(E,S,V)\>::type::value</tt>
    ...
    template<HBOOST_PROTO_LOGICAL_typename_G>
    struct or_ : transform<or_<HBOOST_PROTO_LOGICAL_G> >
    {
    ...
    template<HBOOST_PROTO_LOGICAL_typename_G>
    struct and_ : transform<and_<HBOOST_PROTO_LOGICAL_G> >
    {
    ...
    /// matches <tt>C::case_\<hboost::result_of\<T(E)\>::type\></tt>.
    ///
    ...
    /// <tt>C::case_\<hboost::result_of\<T(E,S,D)\>::type\>()(e, s, d)</tt>.
    ...
    ///
    template<HBOOST_PROTO_LOGICAL_typename_G>
    struct is_callable<or_<HBOOST_PROTO_LOGICAL_G> >
      : mpl::true_
    {};
    ...
    ///
    template<HBOOST_PROTO_LOGICAL_typename_G>
    struct is_callable<and_<HBOOST_PROTO_LOGICAL_G> >
      : mpl::true_
    {};
...
#undef HBOOST_PROTO_LOGICAL_typename_G
#undef HBOOST_PROTO_LOGICAL_G
