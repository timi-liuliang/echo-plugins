    ...
    #include <hboost/proto/context/detail/preprocessed/callable_eval.hpp>

#elif !defined(HBOOST_PP_IS_ITERATING)

    #define HBOOST_PROTO_CHILD_N_TYPE(Z, N, Expr)                                                    \
        typedef typename proto::result_of::child_c<Expr const &, N>::type HBOOST_PP_CAT(child, N);   \
        /**/

    #define HBOOST_PROTO_CHILD_N(Z, N, expr)                                                         \
    ...
    #define HBOOST_PP_ITERATION_PARAMS_1                                                             \
        (3, (1, HBOOST_PROTO_MAX_ARITY, <hboost/proto/context/detail/callable_eval.hpp>))
            ...
            static const bool value =
                ...
                sizeof(
                    detail::check_is_expr_handled(
                        (sctx_(
                            stag_
                            HBOOST_PP_ENUM_TRAILING(N, HBOOST_PROTO_CHILD_N, sexpr_)
                        ), 0)
                    )
                );
    ...
    namespace context
    {
        ...
        {
            HBOOST_PP_REPEAT(N, HBOOST_PROTO_CHILD_N_TYPE, Expr)

            typedef
                typename HBOOST_PROTO_RESULT_OF<
                    Context(
                        typename Expr::proto_tag
                        HBOOST_PP_ENUM_TRAILING_PARAMS(N, child)
                    )
                >::type
            result_type;
            ...
            result_type operator ()(Expr &expr, Context &context) const
            {
                return context(
                    typename Expr::proto_tag()
                    HBOOST_PP_ENUM_TRAILING(N, HBOOST_PROTO_CHILD_N, expr)
                );
            }
        };
    }
