...
#ifndef HBOOST_IOSTREAMS_GZIP_HPP_INCLUDED
#define HBOOST_IOSTREAMS_GZIP_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif

#include <hboost/config.hpp> // STATIC_CONSTANT, STDC_NAMESPACE, 
...
#include <hboost/config.hpp>               // Put size_t in std.
#include <hboost/detail/workaround.hpp>
#include <hboost/cstdint.hpp>              // uint8_t, uint32_t.
...
#include <hboost/iostreams/device/back_inserter.hpp>
#include <hboost/iostreams/filter/zlib.hpp>
#include <hboost/iostreams/pipeline.hpp>     
#include <hboost/iostreams/putback.hpp>
...
using namespace hboost::iostreams::zlib;
...
//
class gzip_error : public HBOOST_IOSTREAMS_FAILURE {
public:
    explicit gzip_error(int error)
        : HBOOST_IOSTREAMS_FAILURE("gzip error"),
...
public:
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        if (!(flags_ & f_header_done)) {
            ...
            offset_ += hboost::iostreams::write(snk, header_.data() + offset_, amt);
    ...
    template<typename Sink>
    void close(Sink& snk, HBOOST_IOS::openmode m)
    {
        try {
...
private:
    ...
    template<typename Sink>
    static void write_long(long n, Sink& next, hboost::mpl::true_)
    {
        hboost::iostreams::put(next, static_cast<char>(0xFF & n));
        hboost::iostreams::put(next, static_cast<char>(0xFF & (n >> 8)));
        hboost::iostreams::put(next, static_cast<char>(0xFF & (n >> 16)));
        hboost::iostreams::put(next, static_cast<char>(0xFF & (n >> 24)));
    }
    template<typename Sink>
    static void write_long(long n, Sink& next, hboost::mpl::false_)
    {
    }
    template<typename Sink>
    ...
    void close_impl()
    {
        #if HBOOST_WORKAROUND(__GNUC__, == 2) && defined(__STL_CONFIG_H) || \
            HBOOST_WORKAROUND(HBOOST_DINKUMWARE_STDLIB, == 1) \
            /**/
...
HBOOST_IOSTREAMS_PIPABLE(basic_gzip_compressor, 1)
...
class HBOOST_IOSTREAMS_DECL gzip_header {
public:
    gzip_header() { reset(); }
...
class HBOOST_IOSTREAMS_DECL gzip_footer {
public:
    gzip_footer() { reset(); }
...
} // End namespace hboost::iostreams::detail.
...
public:
        ...
        while(result < n) {
            ...
            if (state_ == s_header) {
                ...
                try {
                    ...
                    hboost::throw_exception(gzip_error(e));
                }
            } else { // state_ == s_footer
                if (footer_.done()) {
                    if (footer_.crc() != this->crc())
                        hboost::throw_exception(gzip_error(gzip::bad_crc));

                    base_type::close(snk, HBOOST_IOS::out);
                    state_ = s_start;
                } else {
                    int c = s[result++];
                    footer_.process(c);
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        while (result < n && state_ != s_done) {
            ...
            if (state_ == s_header) {
                int c = hboost::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    hboost::throw_exception(gzip_error(gzip::bad_header));
                ...
                int c = hboost::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    hboost::throw_exception(gzip_error(gzip::bad_footer));
                } else if (traits_type::would_block(c)) {
                    break;
                }
                footer_.process(c);
                if (footer_.done()) {
                    ...
                    if (traits_type::is_eof(c)) {
                        ...
                        peek.putback(c);
                        base_type::close(peek, HBOOST_IOS::in);
                        state_ = s_start;
                        header_.reset();
                        footer_.reset();
                    }
    ...
    template<typename Source>
    void close(Source& src, HBOOST_IOS::openmode m)
    {
        try {
    ...
    struct peekable_source {
        ...
        std::streamsize read(char* s, std::streamsize n)
        {
            ...
            if (offset_ < pbsize) {
                ...
                HBOOST_IOSTREAMS_CHAR_TRAITS(char)::copy(
            ...
            std::streamsize amt = 
                hboost::iostreams::read(src_, s + result, n - result);
            return amt != -1 ? 
                result + amt : 
                result ? result : -1;
        }
        bool putback(char c)
        {
            if (offset_) {
                ...
                hboost::throw_exception(
                    hboost::iostreams::detail::bad_putback());
            }
            return true;
        }
...
HBOOST_IOSTREAMS_PIPABLE(basic_gzip_decompressor, 1)
...
{
    hboost::iostreams::back_insert_device<std::string> out(footer_);
...
#endif // #ifndef HBOOST_IOSTREAMS_GZIP_HPP_INCLUDED
