...
#ifndef HBOOST_IOSTREAMS_AGGREGATE_FILTER_HPP_INCLUDED
#define HBOOST_IOSTREAMS_AGGREGATE_FILTER_HPP_INCLUDED
...
#include <vector>
#include <hboost/iostreams/constants.hpp>      // default_device_buffer_size 
...
#include <hboost/iostreams/read.hpp>           // check_eof 
...
public:
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        if (amt) {
            HBOOST_IOSTREAMS_CHAR_TRAITS(char_type)::copy(s, &data_[ptr_], amt);
            ptr_ += amt;
        }
    ...
    template<typename Sink>
    void close(Sink& sink, HBOOST_IOS::openmode which)
    {
        if ((state_ & f_read) != 0 && which == HBOOST_IOS::in)
            close_impl();
        if ((state_ & f_write) != 0 && which == HBOOST_IOS::out) {
            try {
...
private:
    ...
    void do_read(Source& src)
    {
        ...
        while (true) {
            ...
            Ch                     buf[size];
            std::streamsize        amt;
            if ((amt = hboost::iostreams::read(src, buf, size)) == -1)
                break;
...
HBOOST_IOSTREAMS_PIPABLE(aggregate_filter, 1)

} } // End namespaces iostreams, boost.

#include <hboost/iostreams/detail/config/enable_warnings.hpp>  // MSVC.

#endif // #ifndef HBOOST_IOSTREAMS_AGGREGATE_FILTER_HPP_INCLUDED
