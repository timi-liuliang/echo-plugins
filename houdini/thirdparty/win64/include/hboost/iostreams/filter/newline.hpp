...
#ifndef HBOOST_IOSTREAMS_NEWLINE_FILTER_HPP_INCLUDED
#define HBOOST_IOSTREAMS_NEWLINE_FILTER_HPP_INCLUDED
...
#include <hboost/iostreams/detail/ios.hpp>  // HBOOST_IOSTREAMS_FAILURE 
#include <hboost/iostreams/read.hpp>        // get 
#include <hboost/iostreams/write.hpp>       // put 
...
#define HBOOST_IOSTREAMS_ASSERT_UNREACHABLE(val) \
    (HBOOST_ASSERT("unreachable code" == 0), val) \
    /**/
...
class newline_error
    : public HBOOST_IOSTREAMS_FAILURE, public detail::newline_base
{
private:
    friend class newline_checker;
    newline_error(int flags)
        : HBOOST_IOSTREAMS_FAILURE("bad line endings"),
...
public:
    ...
    explicit newline_filter(int target) : flags_(target)
    {
        ...
        {
            hboost::throw_exception(std::logic_error("bad flags"));
        }
    }
    ...
    int get(Source& src)
    {
        ...
        HBOOST_ASSERT((flags_ & f_write) == 0);
        flags_ |= f_read;
    ...
    template<typename Sink>
    void close(Sink& dest, HBOOST_IOS::openmode)
    {
...
private:
    ...
    int newline()
    {
        ...
        return HBOOST_IOSTREAMS_ASSERT_UNREACHABLE(0);
    }
    ...
    bool newline(Sink& dest)
    {
        ...
        switch (flags_ & iostreams::newline::platform_mask) {
        case iostreams::newline::posix:
            success = hboost::iostreams::put(dest, LF);
            break;
        case iostreams::newline::mac:
            success = hboost::iostreams::put(dest, CR);
            break;
        case iostreams::newline::dos:
            if ((flags_ & f_has_LF) != 0) {
                if ((success = hboost::iostreams::put(dest, LF)))
                    flags_ &= ~f_has_LF;
            } else if (hboost::iostreams::put(dest, CR)) {
                if (!(success = hboost::iostreams::put(dest, LF)))
                    flags_ |= f_has_LF;
            }
            break;
...
HBOOST_IOSTREAMS_PIPABLE(newline_filter, 0)
...
public:
    ...
    template<typename Sink>
    void close(Sink&, HBOOST_IOS::openmode)
    {
...
private:
    void fail() { hboost::throw_exception(newline_error(source())); }
...
HBOOST_IOSTREAMS_PIPABLE(newline_checker, 0)

} } // End namespaces iostreams, boost.

#include <hboost/iostreams/detail/config/enable_warnings.hpp>

#endif // #ifndef HBOOST_IOSTREAMS_NEWLINE_FILTER_HPP_INCLUDED
