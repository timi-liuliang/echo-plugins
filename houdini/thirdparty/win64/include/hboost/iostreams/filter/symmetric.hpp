...
#ifndef HBOOST_IOSTREAMS_SYMMETRIC_FILTER_HPP_INCLUDED
#define HBOOST_IOSTREAMS_SYMMETRIC_FILTER_HPP_INCLUDED
...
#include <hboost/iostreams/operations.hpp>       // read, write.
...
template< typename SymmetricFilter,
          typename Alloc =
              std::allocator<
                  HBOOST_DEDUCED_TYPENAME char_type_of<SymmetricFilter>::type
              > >
class symmetric_filter {
public:
    typedef typename char_type_of<SymmetricFilter>::type      char_type;
    typedef HBOOST_IOSTREAMS_CHAR_TRAITS(char_type)            traits_type;
    ...
    #define HBOOST_PP_LOCAL_MACRO(n) \
        HBOOST_IOSTREAMS_TEMPLATE_PARAMS(n, T) \
        explicit symmetric_filter( \
              int buffer_size HBOOST_PP_COMMA_IF(n) \
              HBOOST_PP_ENUM_BINARY_PARAMS(n, const T, &t) ) \
            : pimpl_(new impl(buffer_size HBOOST_PP_COMMA_IF(n) \
                     HBOOST_PP_ENUM_PARAMS(n, t))) \
            { HBOOST_ASSERT(buffer_size > 0); } \
        /**/
    #define HBOOST_PP_LOCAL_LIMITS (0, HBOOST_IOSTREAMS_MAX_FORWARDING_ARITY)
    ...
    template<typename Sink>
    void close(Sink& snk, HBOOST_IOS::openmode mode)
    {
        if (mode == HBOOST_IOS::out) {
...
private:
    ...
    bool flush(Sink& snk, mpl::true_)
    {
        ...
        std::streamsize result =
            hboost::iostreams::write(snk, buf().data(), amt);
    ...
    #define HBOOST_PP_LOCAL_MACRO(n) \
        HBOOST_IOSTREAMS_TEMPLATE_PARAMS(n, T) \
        impl( int buffer_size HBOOST_PP_COMMA_IF(n) \
              HBOOST_PP_ENUM_BINARY_PARAMS(n, const T, &t) ) \
            : SymmetricFilter(HBOOST_PP_ENUM_PARAMS(n, t)), \
...
HBOOST_IOSTREAMS_PIPABLE(symmetric_filter, 2)
...
#endif // #ifndef HBOOST_IOSTREAMS_SYMMETRIC_FILTER_HPP_INCLUDED
