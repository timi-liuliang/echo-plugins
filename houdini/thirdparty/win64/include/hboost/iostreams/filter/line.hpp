...
#ifndef HBOOST_IOSTREAMS_LINE_FILTER_HPP_INCLUDED
#define HBOOST_IOSTREAMS_LINE_FILTER_HPP_INCLUDED
...
#include <hboost/iostreams/checked_operations.hpp>
...
// Must come last.
#include <hboost/iostreams/detail/config/disable_warnings.hpp> // VC7.1 C4244.
...
public:
    ...
    {
        if ((flags_ & f_read) && which == HBOOST_IOS::in)
            close_impl();

        if ((flags_ & f_write) && which == HBOOST_IOS::out) {
            try {
                if (!cur_line_.empty())
                    write_line(snk);
            } catch (...) {
...
HBOOST_IOSTREAMS_PIPABLE(basic_line_filter, 2)
...
#endif // #ifndef HBOOST_IOSTREAMS_LINE_FILTER_HPP_INCLUDED
