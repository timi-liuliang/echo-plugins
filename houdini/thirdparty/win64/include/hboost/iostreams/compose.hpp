...
#ifndef HBOOST_IOSTREAMS_COMPOSE_HPP_INCLUDED
#define HBOOST_IOSTREAMS_COMPOSE_HPP_INCLUDED
...
template< typename First, 
          typename Second,
          typename FirstMode = 
              HBOOST_DEDUCED_TYPENAME mode_of<First>::type,
          typename SecondMode = 
              HBOOST_DEDUCED_TYPENAME mode_of<Second>::type >
...
template< typename Filter,
          typename Device,
          typename Mode =
              HBOOST_DEDUCED_TYPENAME composite_mode<Filter, Device>::type >
...
private:
    ...
    HBOOST_STATIC_ASSERT(is_device<Device>::value);
public:
...
template< typename Filter1, 
          typename Filter2,
          typename Mode =
              HBOOST_DEDUCED_TYPENAME composite_mode<Filter1, Filter2>::type >
...
private:
    ...
    HBOOST_STATIC_ASSERT(is_filter<Filter1>::value);
    HBOOST_STATIC_ASSERT(is_filter<Filter2>::value);
public:
    ...
    {
        composite_device<filter_ref, Source> cmp(hboost::ref(filter2_), src);
    ...
    {
        composite_device<filter_ref, Sink> cmp(hboost::ref(filter2_), snk);
    ...
    std::streampos seek( Device& dev, stream_offset off, HBOOST_IOS::seekdir way,
                         HBOOST_IOS::openmode which =
                             HBOOST_IOS::in | HBOOST_IOS::out )
    {
        composite_device<filter_ref, Device> cmp(hboost::ref(filter2_), dev);
    ...
    void close(Device& dev)
    {
        ...
        HBOOST_STATIC_ASSERT((!is_convertible<category, dual_use>::value));
        ...
        if (!is_convertible<first_mode, dual_use>::value) {
            detail::execute_all(
                detail::call_close(filter2_, dev, HBOOST_IOS::in),
                detail::call_close(filter1_, cmp, HBOOST_IOS::in),
                detail::call_close(filter1_, cmp, HBOOST_IOS::out),
                detail::call_close(filter2_, dev, HBOOST_IOS::out)
            );
...
compose( const Filter& filter, const FilterOrDevice& fod
         HBOOST_IOSTREAMS_DISABLE_IF_STREAM(FilterOrDevice) )
...
void composite_device<Filter, Device, Mode>::close()
{
    ...
    if (!is_convertible<filter_mode, dual_use>::value) {
        detail::execute_all(
            detail::call_close(device_, HBOOST_IOS::in),
            detail::call_close(filter_, device_, HBOOST_IOS::in),
            detail::call_close(filter_, device_, HBOOST_IOS::out),
            detail::call_close(device_, HBOOST_IOS::out)
        );
...
template<typename Filter, typename Device, typename Mode>
void composite_device<Filter, Device, Mode>::close(HBOOST_IOS::openmode which)
{
    HBOOST_STATIC_ASSERT((is_convertible<Mode, two_sequence>::value));
    HBOOST_STATIC_ASSERT(!(is_convertible<filter_mode, dual_use>::value));
...
#endif // #ifndef HBOOST_IOSTREAMS_COMPOSE_HPP_INCLUDED
