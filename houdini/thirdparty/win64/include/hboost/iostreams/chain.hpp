...
#ifndef HBOOST_IOSTREAMS_DETAIL_CHAIN_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_CHAIN_HPP_INCLUDED
...
#include <hboost/detail/workaround.hpp>          // HBOOST_NESTED_TEMPLATE 
...
#include <hboost/iostreams/device/null.hpp>
#include <hboost/iostreams/positioning.hpp>
#include <hboost/iostreams/traits.hpp>           // is_filter.
...
#include <hboost/iostreams/detail/execute.hpp>   // VC6.5 requires this
#if HBOOST_WORKAROUND(HBOOST_MSVC, < 1310)        // #include order
...
// Boost.Python and Boost.Function.
    ...
    /**/
# include <cstring>
# define HBOOST_IOSTREAMS_COMPARE_TYPE_ID(X,Y) \
     (std::strcmp((X).name(),(Y).name()) == 0)
#else
# define HBOOST_IOSTREAMS_COMPARE_TYPE_ID(X,Y) ((X)==(Y))
#endif

// Deprecated
#define HBOOST_IOSTREAMS_COMPONENT_TYPE(chain, index) \
    chain.component_type( index ) \
    /**/

#if !HBOOST_WORKAROUND(HBOOST_MSVC, < 1310)
# define HBOOST_IOSTREAMS_COMPONENT(chain, index, target) \
    chain.component< target >( index ) \
    /**/
#else
# define HBOOST_IOSTREAMS_COMPONENT(chain, index, target) \
    chain.component( index, ::hboost::type< target >() ) \
    /**/
#endif
...
public:
    ...
    const std::type_info& component_type(int n) const
    {
        if (static_cast<size_type>(n) >= size())
            hboost::throw_exception(std::out_of_range("bad chain offset"));
        return (*hboost::next(list().begin(), n))->component_type();
    }
    ...
    template<typename T>
    T* component(int n) const { return component(n, hboost::type<T>()); }
...
#endif
    template<typename T>
    T* component(int n, hboost::type<T>) const
    {
        if (static_cast<size_type>(n) >= size())
            hboost::throw_exception(std::out_of_range("bad chain offset"));
        streambuf_type* link = *hboost::next(list().begin(), n);
        if (HBOOST_IOSTREAMS_COMPARE_TYPE_ID(link->component_type(), typeid(T)))
...
public:
    ...
    HBOOST_IOSTREAMS_DEFINE_PUSH(push, mode, char_type, push_impl)
    void pop();
    ...
    bool sync() { return front().HBOOST_IOSTREAMS_PUBSYNC() != -1; }
    bool strict_sync();
private:
    template<typename T>
                   ...
                   std::streamsize pback_size = -1)
    {
        ...
        typedef stream_buffer<
                    component_type,
                    HBOOST_IOSTREAMS_CHAR_TRAITS(char_type),
                    Alloc, Mode
                >                                         streambuf_t;
        typedef typename list_type::iterator              iterator;
        HBOOST_STATIC_ASSERT((is_convertible<category, Mode>::value));
        if (is_complete())
            hboost::throw_exception(std::logic_error("chain complete"));
    ...
    static void close(streambuf_type* b, HBOOST_IOS::openmode m)
    {
        if (m == HBOOST_IOS::out && is_convertible<Mode, output>::value)
            b->HBOOST_IOSTREAMS_PUBSYNC();
        b->close(m);
    }
    ...
    struct closer  : public std::unary_function<streambuf_type*, void>  {
        closer(HBOOST_IOS::openmode m) : mode_(m) { }
        void operator() (streambuf_type* b)
        {
            close(b, mode_);
        }
        HBOOST_IOS::openmode mode_;
    };
        ...
        void close()
            {
                if ((flags_ & f_open) != 0) {
                    ...
                    links_.front()->HBOOST_IOSTREAMS_PUBSYNC();
                    try {
                        hboost::iostreams::detail::execute_foreach(
                            links_.rbegin(), links_.rend(), 
                            closer(HBOOST_IOS::in)
                        );
                    } catch (...) {
                        try {
                            hboost::iostreams::detail::execute_foreach(
                                links_.begin(), links_.end(), 
                                closer(HBOOST_IOS::out)
                            );
                        } catch (...) { }
                        throw;
                    }
...
//
// Macro: HBOOST_IOSTREAMS_DECL_CHAIN(name, category)
...
//
#define HBOOST_IOSTREAMS_DECL_CHAIN(name_, default_char_) \
    template< typename Mode, typename Ch = default_char_, \
              typename Tr = HBOOST_IOSTREAMS_CHAR_TRAITS(Ch), \
              typename Alloc = std::allocator<Ch> > \
    class name_ : public hboost::iostreams::detail::chain_base< \
    ...
    public: \
        ...
        typedef Mode                                   mode; \
    private: \
        typedef hboost::iostreams::detail::chain_base< \
                    name_<Mode, Ch, Tr, Alloc>, \
                    Ch, Tr, Alloc, Mode \
                >                                      base_type; \
    public: \
    ...
    /**/
HBOOST_IOSTREAMS_DECL_CHAIN(chain, char)
HBOOST_IOSTREAMS_DECL_CHAIN(wchain, wchar_t)
#undef HBOOST_IOSTREAMS_DECL_CHAIN
...
public:
    ...
    { return chain_->HBOOST_NESTED_TEMPLATE component_type<N>(); }

    template<typename T>
    T* component(int n) const
    { return chain_->HBOOST_NESTED_TEMPLATE component<T>(n); }

    // Deprecated.
    template<int N, typename T>
    T* component() const
    { return chain_->HBOOST_NESTED_TEMPLATE component<N, T>(); }
#else
    template<typename T>
    T* component(int n, hboost::type<T> t) const
...
protected:
    ...
    { chain_->push(t HBOOST_IOSTREAMS_PUSH_ARGS()); }
...
#endif // #ifndef HBOOST_IOSTREAMS_DETAIL_CHAIN_HPP_INCLUDED
