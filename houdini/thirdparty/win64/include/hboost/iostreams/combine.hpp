...
#ifndef HBOOST_IOSTREAMS_COMBINE_HPP_INCLUDED
#define HBOOST_IOSTREAMS_COMBINE_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif              

#include <hboost/config.hpp> // NO_STD_LOCALE, DEDUCED_TYPENAME.
...
public:
    ...
    struct category
        : bidirectional, 
          ...
          localizable_tag
        { };
    HBOOST_STATIC_ASSERT(is_device<Source>::value);
    HBOOST_STATIC_ASSERT(is_device<Sink>::value);
    HBOOST_STATIC_ASSERT((is_convertible<in_category, input>::value));
    HBOOST_STATIC_ASSERT((is_convertible<out_category, output>::value));
    HBOOST_STATIC_ASSERT((is_same<char_type, sink_char_type>::value));
...
public:
    ...
    struct category 
        : multichar_bidirectional_filter_tag,
          ...
          localizable_tag
        { };
    HBOOST_STATIC_ASSERT(is_filter<InputFilter>::value);
    HBOOST_STATIC_ASSERT(is_filter<OutputFilter>::value);
    HBOOST_STATIC_ASSERT((is_convertible<in_category, input>::value));
    HBOOST_STATIC_ASSERT((is_convertible<out_category, output>::value));
    HBOOST_STATIC_ASSERT((is_same<char_type, output_char_type>::value));
    combined_filter(const InputFilter& in, const OutputFilter& out);

    template<typename Source>
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    { return hboost::iostreams::read(in_, src, s, n); }

    template<typename Sink>
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    { return hboost::iostreams::write(out_, snk, s, n); }

    template<typename Sink>
    void close(Sink& snk, HBOOST_IOS::openmode which)
    {
        if (which == HBOOST_IOS::in) {
            if (is_convertible<in_category, dual_use>::value) {
                iostreams::close(in_, snk, HBOOST_IOS::in);
            } else {
        ...
        if (which == HBOOST_IOS::out) {
            if (is_convertible<out_category, dual_use>::value) {
                iostreams::close(out_, snk, HBOOST_IOS::out);
            } else {
...
struct combine_traits {
    typedef combination<
                HBOOST_DEDUCED_TYPENAME detail::unwrapped_type<In>::type, 
                HBOOST_DEDUCED_TYPENAME detail::unwrapped_type<Out>::type
            > type;
};
...
template<typename Source, typename Sink>
inline void
combined_device<Source, Sink>::close(HBOOST_IOS::openmode which)
{ 
    if (which == HBOOST_IOS::in)
...
#endif // #ifndef HBOOST_IOSTREAMS_COMBINE_HPP_INCLUDED
