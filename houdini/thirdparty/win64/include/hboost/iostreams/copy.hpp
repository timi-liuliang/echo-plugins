...
#ifndef HBOOST_IOSTREAMS_COPY_HPP_INCLUDED
#define HBOOST_IOSTREAMS_COPY_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif              

#include <hboost/config.hpp>                 // Make sure ptrdiff_t is in std.
...
#include <hboost/iostreams/operations.hpp>  // read, write, close.
...
{ 
    ...
    // hboost::iostreams::detail::execute() in the primary 
...
std::streamsize copy_impl(Source src, Sink snk, std::streamsize buffer_size)
{
    ...
    HBOOST_STATIC_ASSERT((is_same<src_char, snk_char>::value));
...
copy( const Source& src, const Sink& snk,
      ...
      HBOOST_IOSTREAMS_DISABLE_IF_STREAM(Source)
      HBOOST_IOSTREAMS_DISABLE_IF_STREAM(Sink) )
{ 
    typedef typename char_type_of<Source>::type char_type;
...
copy( Source& src, const Sink& snk,
      std::streamsize buffer_size = default_device_buffer_size
      HBOOST_IOSTREAMS_ENABLE_IF_STREAM(Source)
      HBOOST_IOSTREAMS_DISABLE_IF_STREAM(Sink) ) 
{ 
    typedef typename char_type_of<Source>::type char_type;
...
copy( const Source& src, Sink& snk,
      std::streamsize buffer_size = default_device_buffer_size
      HBOOST_IOSTREAMS_DISABLE_IF_STREAM(Source)
      HBOOST_IOSTREAMS_ENABLE_IF_STREAM(Sink) ) 
{ 
    typedef typename char_type_of<Source>::type char_type;
    return detail::copy_impl( detail::resolve<input, char_type>(src), 
...
#endif // #ifndef HBOOST_IOSTREAMS_COPY_HPP_INCLUDED
