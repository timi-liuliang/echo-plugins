...
#ifndef HBOOST_IOSTREAMS_TEE_HPP_INCLUDED
#define HBOOST_IOSTREAMS_TEE_HPP_INCLUDED
...
#include <hboost/config.hpp>  // HBOOST_DEDUCE_TYPENAME.
...
#include <hboost/iostreams/detail/functional.hpp>  // call_close_all 
...
public:
    ...
    HBOOST_STATIC_ASSERT((
        is_convertible< // Using mode_of causes failures on VC6-7.0.
            HBOOST_DEDUCED_TYPENAME iostreams::category_of<Device>::type, output
        >::value
    ));
    ...
    void close(Next&, HBOOST_IOS::openmode)
    { 
...
HBOOST_IOSTREAMS_PIPABLE(tee_filter, 1)
...
public:
    ...
    HBOOST_STATIC_ASSERT((
        is_same<
            char_type, 
            HBOOST_DEDUCED_TYPENAME char_type_of<Sink>::type
        >::value
    ));
    HBOOST_STATIC_ASSERT((
        is_convertible<
            HBOOST_DEDUCED_TYPENAME iostreams::category_of<Sink>::type, 
            output
        >::value
    ));
    struct category
    ...
    std::streamsize read(char_type* s, std::streamsize n)
    {
        HBOOST_STATIC_ASSERT((
            is_convertible<
                HBOOST_DEDUCED_TYPENAME iostreams::category_of<Device>::type, input
            >::value
        ));
    ...
    std::streamsize write(const char_type* s, std::streamsize n)
    {
        ...
        (void) result1; // Suppress 'unused variable' warning.
        (void) result2;
        HBOOST_ASSERT(result1 == n && result2 == n);
        return n;
    }
    void close()
    {
...
#endif // #ifndef HBOOST_IOSTREAMS_TEE_HPP_INCLUDED
