...
#ifndef HBOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED

#include <algorithm>                             // min, max.
#include <cassert>
#include <exception>
#include <typeinfo>
#include <hboost/config.hpp>                      // Member template friends.
...
// Must come last.
#include <hboost/iostreams/detail/config/disable_warnings.hpp>  // MSVC, BCC 5.x
...
public:
    ...
    void open(const T& t HBOOST_IOSTREAMS_PUSH_PARAMS());
    bool is_open() const;
    void close();
...
void indirect_streambuf<T, Tr, Alloc, Mode>::close()
{
    ...
    base_type* self = this;
    detail::execute_all(
        detail::call_member_close(*self, HBOOST_IOS::in),
...
{
    if ( gptr() != 0 && way == HBOOST_IOS::cur && which == HBOOST_IOS::in && 
         eback() - gptr() <= off && off <= egptr() - gptr() ) 
    {   // Small seek optimization
        gbump(off);
        return obj().seek(0, HBOOST_IOS::cur, HBOOST_IOS::in, next_) -
               static_cast<off_type>(egptr() - gptr());
    }
    if (pptr() != 0) 
        this->HBOOST_IOSTREAMS_PUBSYNC(); // sync() confuses VisualAge 6.
    if (way == HBOOST_IOS::cur && gptr())
...
inline void indirect_streambuf<T, Tr, Alloc, Mode>::close_impl
    (HBOOST_IOS::openmode which)
{
    if (which == HBOOST_IOS::in && is_convertible<Mode, input>::value) {
        setg(0, 0, 0);
    }
    if (which == HBOOST_IOS::out && is_convertible<Mode, output>::value) {
        sync();
        setp(0, 0);
    }
    if ( !is_convertible<category, dual_use>::value ||
         is_convertible<Mode, input>::value == (which == HBOOST_IOS::in) )
    {
...
#include <hboost/iostreams/detail/config/enable_warnings.hpp> // MSVC, BCC 5.x

#endif // #ifndef HBOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED
