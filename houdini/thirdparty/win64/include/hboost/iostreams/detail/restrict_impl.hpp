 ...
 * File:        hboost/iostreams/detail/restrict_impl.hpp
 * Date:        Sun Jan 06 12:57:30 MST 2008
 * Copyright:   2007-2008 CodeRage, LLC
 * Author:      Jonathan Turkanis
 * Contact:     turkanis at coderage dot com
 *
 * If included with the macro HBOOST_IOSTREAMS_RESTRICT undefined, defines the 
 * class template hboost::iostreams::restriction. If included with the macro
 * HBOOST_IOSTREAMS_RESTRICT defined as an identifier, defines the overloaded
 * function template hboost::iostreams::HBOOST_IOSTREAMS_RESTRICT, and object 
 * generator for hboost::iostreams::restriction.
 *
 * This design allows <hboost/iostreams/restrict.hpp> and 
 * <hboost/iostreams/slice.hpp> to share an implementation.
 */

#if !defined(HBOOST_IOSTREAMS_RESTRICT_IMPL_HPP_INCLUDED) && \
    !defined(HBOOST_IOSTREAMS_RESTRICT)
# define HBOOST_IOSTREAMS_RESTRICT_IMPL_HPP_INCLUDED
                    
//------------------Implementation of restriction-----------------------------//

# include <algorithm>          // min.
# include <utility>            // pair.
# include <hboost/cstdint.hpp>  // intmax_t.
...
# include <hboost/iostreams/detail/adapter/device_adapter.hpp>
# include <hboost/iostreams/detail/adapter/filter_adapter.hpp>
...
# include <hboost/iostreams/skip.hpp>
...
public:
    ...
    HBOOST_STATIC_ASSERT(!(is_convertible<mode, detail::two_sequence>::value));
    struct category
        : mode,
          device_tag,
          closable_tag,
    ...
    std::streampos seek(stream_offset off, HBOOST_IOS::seekdir way);
private:
...
public:
    ...
    template<typename Device>
    std::streampos seek(Device& dev, stream_offset off, HBOOST_IOS::seekdir way)
    {
        stream_offset next;
        if (way == HBOOST_IOS::beg) {
            ...
            pos_ = this->component().seek(dev, off, HBOOST_IOS::end);
            if (pos_ < beg_)
        ...
        if (next < beg_ || (end_ != -1 && next >= end_))
            hboost::throw_exception(bad_seek());
        pos_ = this->component().seek(dev, next, HBOOST_IOS::cur);
        return offset_to_position(pos_ - beg_);
    }
    ...
    template<typename Device>
    void close(Device& dev, HBOOST_IOS::openmode which) 
    { 
        open_ = false;
...
private:
    template<typename Device>
    void open(Device& dev, HBOOST_IOS::openmode which)
    {
        typedef typename is_convertible<mode, dual_use>::type is_dual_use;
        open_ = true;
        which = is_dual_use() ? which : (HBOOST_IOS::in | HBOOST_IOS::out);
...
{
    if (len < -1 || off < 0)
        hboost::throw_exception(HBOOST_IOSTREAMS_FAILURE("bad offset"));
    ...
    (stream_offset off, HBOOST_IOS::seekdir way)
{
    ...
    if (way == HBOOST_IOS::beg) {
        ...
        // Restriction is half-open; seek relative to the actual end.
        pos_ = iostreams::seek(this->component(), off, HBOOST_IOS::end);
        if (pos_ < beg_)
            hboost::throw_exception(bad_seek());
        return offset_to_position(pos_ - beg_);
    }
    if (next < beg_ || (end_ != -1 && next > end_))
        hboost::throw_exception(bad_seek());
    pos_ = iostreams::seek(this->component(), next - pos_, HBOOST_IOS::cur);
    return offset_to_position(pos_ - beg_);
}
...
{
    HBOOST_STATIC_ASSERT((is_convertible<category, input>::value));
...
{
    HBOOST_STATIC_ASSERT((is_convertible<category, output>::value));
    return std::make_pair(beg_, end_);
}
...
#elif defined(HBOOST_IOSTREAMS_RESTRICT)
...
# ifndef HBOOST_IOSTREAMS_BROKEN_OVERLOAD_RESOLUTION //------------------------//
#  ifndef HBOOST_IOSTREAMS_NO_STREAM_TEMPLATES //------------------------------//

template<typename T>
restriction<T> 
HBOOST_IOSTREAMS_RESTRICT( const T& t, stream_offset off, stream_offset len = -1
                          HBOOST_IOSTREAMS_DISABLE_IF_STREAM(T) )
{ return restriction<T>(t, off, len); }

template<typename Ch, typename Tr>
restriction< std::basic_streambuf<Ch, Tr> >
HBOOST_IOSTREAMS_RESTRICT( std::basic_streambuf<Ch, Tr>& sb, stream_offset off, 
                          stream_offset len = -1 )
{ return restriction< std::basic_streambuf<Ch, Tr> >(sb, off, len); }

template<typename Ch, typename Tr>
restriction< std::basic_istream<Ch, Tr> >
HBOOST_IOSTREAMS_RESTRICT
...
# else // #ifndef HBOOST_IOSTREAMS_BROKEN_OVERLOAD_RESOLUTION //---------------//
...
HBOOST_IOSTREAMS_RESTRICT( const T& t, stream_offset off, stream_offset len = -1
                          HBOOST_IOSTREAMS_DISABLE_IF_STREAM(T) )
{ return HBOOST_IOSTREAMS_RESTRICT(t, off, len, is_std_io<T>()); }
...
template<typename T>
restriction<T>
HBOOST_IOSTREAMS_RESTRICT(T& t, stream_offset off, stream_offset len = -1)
{ return restriction<T>(t, off, len); }

#  endif // Borland 5.x, VC6-7.0 or GCC 2.9x //-------------------------------//
# endif // #ifndef HBOOST_IOSTREAMS_BROKEN_OVERLOAD_RESOLUTION //--------------//

} } // End namespaces iostreams, boost.

#endif // #if !defined(HBOOST_IOSTREAMS_RESTRICT_IMPL_HPP_INCLUDED) ...
