 ...
 * hboost::iostreams::detail::execute_all() and the function template 
...
#ifndef HBOOST_IOSTREAMS_DETAIL_EXECUTE_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_EXECUTE_HPP_INCLUDED
...
#include <hboost/iostreams/detail/config/limits.hpp>   // MAX_EXECUTE_ARITY
...
template< typename Op, 
          typename Result = // VC6.5 workaround.
              #if !defined(HBOOST_NO_RESULT_OF) && \
                  !HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x592))
                  typename hboost::result_of<Op()>::type
              #else
                  HBOOST_DEDUCED_TYPENAME Op::result_type
              #endif
          >
...
#define HBOOST_PP_LOCAL_MACRO(n) \
   template<typename Op, HBOOST_PP_ENUM_PARAMS(n, typename C)> \
   typename execute_traits<Op>::result_type \
   execute_all(Op op, HBOOST_PP_ENUM_BINARY_PARAMS(n, C, c)) \
   { \
       typename execute_traits<Op>::result_type r; \
       try { \
           r = hboost::iostreams::detail::execute_all( \
                   op HBOOST_PP_COMMA_IF(HBOOST_PP_DEC(n)) \
                   HBOOST_PP_ENUM_PARAMS(HBOOST_PP_DEC(n), c) \
               ); \
       } catch (...) { \
           try { \
               HBOOST_PP_CAT(c, HBOOST_PP_DEC(n))(); \
           } catch (...) { } \
           throw; \
       } \
       HBOOST_PP_CAT(c, HBOOST_PP_DEC(n))(); \
...
#define HBOOST_PP_LOCAL_LIMITS (1, HBOOST_IOSTREAMS_MAX_EXECUTE_ARITY)
...
Op execute_foreach(InIt first, InIt last, Op op)
{
    ...
    try {
        ...
        try {
            ++first;
            hboost::iostreams::detail::execute_foreach(first, last, op);
        } catch (...) { }
        throw;
    }
    ++first;
    return hboost::iostreams::detail::execute_foreach(first, last, op);
}

} } } // End namespaces detail, iostreams, boost.

#endif // #ifndef HBOOST_IOSTREAMS_DETAIL_EXECUTE_HPP_INCLUDED
