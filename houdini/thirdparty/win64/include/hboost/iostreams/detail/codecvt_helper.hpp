...
//
// // In global namespace:
// HBOOST_IOSTREAMS_CODECVT_SPEC(mystate)
//
// // In user namespace:
...
#ifndef HBOOST_IOSTREAMS_DETAIL_CODECVT_HELPER_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_CODECVT_HELPER_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif

#include <hboost/config.hpp>  // Put size_t in std, HBOOST_MSVC, Dinkum.
#include <hboost/detail/workaround.hpp>
#include <algorithm>         // min.
#include <cstddef>           // size_t.
#include <locale>            // locale, codecvt_base, codecvt.
#include <hboost/iostreams/detail/config/codecvt.hpp>
...
#else // #if !HBOOST_WORKAROUND(HBOOST_DINKUMWARE_STDLIB, == 1) //--------------//
...
#endif // #if !HBOOST_WORKAROUND(HBOOST_DINKUMWARE_STDLIB, == 1) //-------------//
...
#if defined(HBOOST_IOSTREAMS_NO_PRIMARY_CODECVT_DEFINITION) || \
    defined(HBOOST_IOSTREAMS_EMPTY_PRIMARY_CODECVT_DEFINITION) || \
...
public:
    ...
    int length( HBOOST_IOSTREAMS_CODECVT_CV_QUALIFIER State& state, 
...
protected:
    ...
    virtual int do_length( HBOOST_IOSTREAMS_CODECVT_CV_QUALIFIER State&, 
                           const Extern* first1, const Extern* last1,
                           std::size_t len2 ) const throw()
    {
...
//------------------Definition of HBOOST_IOSTREAMS_CODECVT_SPEC----------------//

#if defined(HBOOST_IOSTREAMS_NO_PRIMARY_CODECVT_DEFINITION) || \
    defined(HBOOST_IOSTREAMS_EMPTY_PRIMARY_CODECVT_DEFINITION) \
    /**/
# ifndef HBOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
#  define HBOOST_IOSTREAMS_CODECVT_SPEC(state) \
    namespace std { \
        template<typename Intern, typename Extern> \
        class codecvt<Intern, Extern, state> \
            : public ::hboost::iostreams::detail::codecvt_impl< \
                         Intern, Extern, state \
                     > \
        { \
        public: \
            codecvt(std::size_t refs = 0) \
                : ::hboost::iostreams::detail::codecvt_impl< \
                      Intern, Extern, state \
                  >(refs) \
                { } \
...
struct codecvt_helper : std::codecvt<Intern, Extern, State> { 
    ...
    #if !defined(HBOOST_IOSTREAMS_NO_CODECVT_CTOR_FROM_SIZE_T)
        : std::codecvt<Intern, Extern, State>(refs)
    #else
        : std::codecvt<Intern, Extern, State>()
    #endif
        { }
#ifdef HBOOST_IOSTREAMS_NO_CODECVT_MAX_LENGTH
    int max_length() const throw() { return do_max_length(); }
protected:
    virtual int do_max_length() const throw() { return 1; }
#endif
...
#endif // #ifndef HBOOST_IOSTREAMS_DETAIL_CODECVT_HELPER_HPP_INCLUDED
