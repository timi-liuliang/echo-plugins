 ...
 * File:        hboost/iostreams/detail/functional.hpp
...
#ifndef HBOOST_IOSTREAMS_DETAIL_FUNCTIONAL_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_FUNCTIONAL_HPP_INCLUDED
...
namespace hboost { namespace iostreams { namespace detail {
    ...
    // hboost::iostreams::close

template<typename T>
class device_close_operation {
public:
    typedef void result_type;
    device_close_operation(T& t, HBOOST_IOS::openmode which) 
        : t_(t), which_(which) 
        { }
    void operator()() const { hboost::iostreams::close(t_, which_); }
private:
    device_close_operation& operator=(const device_close_operation&);
    T&                   t_;
    HBOOST_IOS::openmode  which_;
};
...
public:
    typedef void result_type;
    filter_close_operation(T& t, Sink& snk, HBOOST_IOS::openmode which)
        : t_(t), snk_(snk), which_(which)
        { }
    void operator()() const { hboost::iostreams::close(t_, snk_, which_); }
private:
...
call_close(T& t, HBOOST_IOS::openmode which) 
{ return device_close_operation<T>(t, which); }

template<typename T, typename Sink>
filter_close_operation<T, Sink> 
call_close(T& t, Sink& snk, HBOOST_IOS::openmode which) 
{ return filter_close_operation<T, Sink>(t, snk, which); }

    // Function objects and object generators for invoking
    // hboost::iostreams::detail::close_all
...
public:
    typedef void result_type;
    member_close_operation(T& t, HBOOST_IOS::openmode which) 
        : t_(t), which_(which) 
        { }
...
template<typename T>
member_close_operation<T> call_member_close(T& t, HBOOST_IOS::openmode which) 
...
#endif // #ifndef HBOOST_IOSTREAMS_DETAIL_FUNCTIONAL_HPP_INCLUDED
