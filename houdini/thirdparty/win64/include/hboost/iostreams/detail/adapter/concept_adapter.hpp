...
#ifndef HBOOST_IOSTREAMS_DETAIL_CONCEPT_ADAPTER_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_CONCEPT_ADAPTER_HPP_INCLUDED

#include <hboost/config.hpp>                             // SFINAE.
...
#include <hboost/iostreams/detail/config/unreachable_return.hpp>
...
public:
    ...
    { HBOOST_STATIC_ASSERT(is_std_io<T>::value); }
    ...
    std::streampos seek( stream_offset off, HBOOST_IOS::seekdir way,
                         HBOOST_IOS::openmode which, Device* dev )
    ...
    template<typename Device>
    void close(HBOOST_IOS::openmode which, Device* dev)
    ...
    bool flush( Device* dev )
    {
        ...
        if (dev && dev->HBOOST_IOSTREAMS_PUBSYNC() == -1)
            result = false;
        return result;
    }
...
struct device_wrapper_impl<any_tag> {
    ...
    seek( Device& dev, Dummy*, stream_offset off, 
          HBOOST_IOS::seekdir way, HBOOST_IOS::openmode which )
    { 
    ...
    template<typename Device>
    static std::streampos 
    seek( Device&, stream_offset, HBOOST_IOS::seekdir, 
          HBOOST_IOS::openmode, any_tag )
    { 
        hboost::throw_exception(cant_seek());
        HBOOST_IOSTREAMS_UNREACHABLE_RETURN(0)
    }
    ...
    template<typename Device, typename Dummy>
    static void close(Device& dev, Dummy*, HBOOST_IOS::openmode which)
...
struct device_wrapper_impl<input> : device_wrapper_impl<any_tag>  {
    ...
    write( Device&, Dummy*, const typename char_type_of<Device>::type*,
           std::streamsize )
    { hboost::throw_exception(cant_write());
      HBOOST_IOSTREAMS_UNREACHABLE_RETURN(0) }
};
...
struct flt_wrapper_impl<any_tag> {
    ...
    seek( Filter&, Device*, stream_offset,
          HBOOST_IOS::seekdir, HBOOST_IOS::openmode, any_tag )
    { hboost::throw_exception(cant_seek());
      HBOOST_IOSTREAMS_UNREACHABLE_RETURN(0) }
    ...
    static std::streampos
    seek( Filter& f, Device* dev, stream_offset off,
          HBOOST_IOS::seekdir way, HBOOST_IOS::openmode,
    ...
    template<typename Filter, typename Device>
    static void close(Filter& f, Device* dev, HBOOST_IOS::openmode which)
...
#endif // #ifndef HBOOST_IOSTREAMS_DETAIL_CONCEPT_ADAPTER_HPP_INCLUDED
