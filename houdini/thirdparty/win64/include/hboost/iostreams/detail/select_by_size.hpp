...
// HBOOST_SELECT_BY_SIZE_MAX_CASE defined to be the largest N such that case_<N>
...
// Example usage:
//
//    #define HBOOST_SELECT_BY_SIZE_MAX_CASE 7   // (Needed when default was 2)
//    #include <hboost/utility/select_by_size.hpp>
//
//    using namespace hboost::utility;
//
//    case_<0> helper(bool);
//    case_<1> helper(int);
...
//
//    struct test {
//        HBOOST_SELECT_BY_SIZE(int, value, helper(9876UL));
//    };
//
...
#ifndef HBOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED
...
namespace hboost { namespace utility {
...
struct select_by_size {
    HBOOST_STATIC_CONSTANT(int, value = 
...
#define SELECT_BY_SIZE_SPEC(n) \
    namespace hboost { namespace iostreams { namespace detail { \
      static const int HBOOST_PP_CAT(sizeof_case_, n) = sizeof(case_<n - 1>); \
      template<> \
      struct select_by_size< HBOOST_PP_CAT(sizeof_case_, n) > { \
          struct type { HBOOST_STATIC_CONSTANT(int, value = n - 1); }; \
...
#define HBOOST_PP_LOCAL_MACRO(n) SELECT_BY_SIZE_SPEC(n)
#define HBOOST_PP_LOCAL_LIMITS (0, 20)
#include HBOOST_PP_LOCAL_ITERATE()
#undef HBOOST_PP_LOCAL_MACRO

//--------------Definition of SELECT_BY_SIZE----------------------------------//

#define HBOOST_SELECT_BY_SIZE(type_, name, expr) \
    HBOOST_STATIC_CONSTANT( \
        unsigned, \
        HBOOST_PP_CAT(hboost_select_by_size_temp_, name) = sizeof(expr) \
    ); \
    HBOOST_STATIC_CONSTANT( \
        type_, \
        name = \
            ( ::hboost::iostreams::detail::select_by_size< \
                HBOOST_PP_CAT(hboost_select_by_size_temp_, name) \
              >::value ) \
    ) \
    /**/

#endif // #ifndef HBOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED

//----------Specializations of SELECT_BY_SIZE (outside main inclued guards)---//

#if defined(HBOOST_SELECT_BY_SIZE_MAX_CASE) && \
    HBOOST_SELECT_BY_SIZE_MAX_CASE > SELECT_BY_SIZE_MAX_SPECIALIZED

#define HBOOST_PP_LOCAL_MACRO(n) SELECT_BY_SIZE_SPEC(n)
#define HBOOST_PP_LOCAL_LIMITS \
    (SELECT_BY_SIZE_MAX_SPECIALIZED, HBOOST_SELECT_BY_SIZE_MAX_CASE) \
    /**/
#include HBOOST_PP_LOCAL_ITERATE()
#undef HBOOST_PP_LOCAL_MACRO
#undef SELECT_BY_SIZE_MAX_SPECIALIZED
#define SELECT_BY_SIZE_MAX_SPECIALIZED HBOOST_SELECT_BY_SIZE_MAX_CASE
