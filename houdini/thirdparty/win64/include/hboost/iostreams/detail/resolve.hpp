...
#ifndef HBOOST_IOSTREAMS_DETAIL_RESOLVE_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_RESOLVE_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif              

#include <hboost/config.hpp> // partial spec, put size_t in std.
#include <cstddef>          // std::size_t.
#include <hboost/detail/is_incrementable.hpp>
#include <hboost/detail/workaround.hpp>
#include <hboost/iostreams/detail/adapter/mode_adapter.hpp>
#include <hboost/iostreams/detail/adapter/output_iterator_adapter.hpp>
...
#include <hboost/iostreams/detail/is_dereferenceable.hpp>
...
#include <hboost/iostreams/device/array.hpp>
#include <hboost/iostreams/traits.hpp>
#include <hboost/mpl/and.hpp>
#include <hboost/mpl/bool.hpp> // true_.
...
// Must come last.
#include <hboost/iostreams/detail/config/disable_warnings.hpp> // VC7.1 C4224.
...
struct resolve_traits {
    typedef typename 
            mpl::if_<
                hboost::detail::is_incrementable<T>,
...
resolve( const T& t 
         ...
         #if HBOOST_WORKAROUND(HBOOST_INTEL_CXX_VERSION, HBOOST_TESTED_AT(810)) ||\
             HBOOST_WORKAROUND(__MWERKS__, HBOOST_TESTED_AT(0x3205)) || \
             HBOOST_WORKAROUND(HBOOST_IOSTREAMS_GCC, HBOOST_TESTED_AT(400)) ||\
             HBOOST_WORKAROUND(__IBMCPP__, HBOOST_TESTED_AT(1110))
             /**/
...
#  if !HBOOST_WORKAROUND(HBOOST_MSVC, <= 1300)
    ...
    range_adapter< Mode, hboost::iterator_range<Iter> > 
    resolve(const hboost::iterator_range<Iter>& rng)
    { return range_adapter< Mode, hboost::iterator_range<Iter> >(rng); }
#  endif // #if HBOOST_WORKAROUND(HBOOST_MSVC, <= 1300)
...
template<typename Mode, typename Ch, typename T>
typename resolve_traits<Mode, Ch, T>::type 
resolve(const T& t HBOOST_IOSTREAMS_DISABLE_IF_STREAM(T))
...
template<typename Mode, typename Ch, typename T>
typename resolve_traits<Mode, Ch, T>::type 
resolve(T& t HBOOST_IOSTREAMS_ENABLE_IF_STREAM(T))
{ return resolve<Mode, Ch>(t, is_std_io<T>()); }
...
#include <hboost/iostreams/detail/config/enable_warnings.hpp> // VC7.1 4224.

#endif // HBOOST_IOSTREAMS_DETAIL_RESOLVE_HPP_INCLUDED
