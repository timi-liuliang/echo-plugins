...
#ifndef HBOOST_IOSTREAMS_DETAIL_BUFFERS_HPP_INCLUDED
#define HBOOST_IOSTREAMS_DETAIL_BUFFERS_HPP_INCLUDED
...
#include <hboost/config.hpp>                    // member templates.
...
#include <hboost/iostreams/traits.hpp>          // int_type_of.
...
inline basic_buffer<Ch, Alloc>::~basic_buffer()
{
    if (buf_) {
        allocator_type().deallocate(buf_,
            static_cast<HBOOST_DEDUCED_TYPENAME Alloc::size_type>(size_));
    }
}
