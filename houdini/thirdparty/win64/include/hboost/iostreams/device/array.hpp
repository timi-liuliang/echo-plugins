...
#ifndef HBOOST_IOSTREAMS_ARRAY_HPP_INCLUDED
#define HBOOST_IOSTREAMS_ARRAY_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif

#include <hboost/config.hpp>         // HBOOST_MSVC, make sure size_t is in std.
...
#if !HBOOST_WORKAROUND(HBOOST_MSVC, <= 1300)
# define HBOOST_IOSTREAMS_ARRAY_CTOR(name, ch) \
    template<int N> \
    HBOOST_PP_CAT(basic_, name)(ch (&ar)[N]) \
        : base_type(ar) { } \
    /**/
#else
# define HBOOST_IOSTREAMS_ARRAY_CTOR(name, ch)
#endif
#define HBOOST_IOSTREAMS_ARRAY(name, mode) \
    template<typename Ch> \
    struct HBOOST_PP_CAT(basic_, name) : detail::array_adapter<mode, Ch> { \
    private: \
        typedef detail::array_adapter<mode, Ch>  base_type; \
    public: \
        typedef typename base_type::char_type    char_type; \
        typedef typename base_type::category     category; \
        HBOOST_PP_CAT(basic_, name)(char_type* begin, char_type* end) \
            : base_type(begin, end) { } \
        HBOOST_PP_CAT(basic_, name)(char_type* begin, std::size_t length) \
            : base_type(begin, length) { } \
        HBOOST_PP_CAT(basic_, name)(const char_type* begin, const char_type* end) \
            : base_type(begin, end) { } \
        HBOOST_PP_CAT(basic_, name)(const char_type* begin, std::size_t length) \
            : base_type(begin, length) { } \
        HBOOST_IOSTREAMS_ARRAY_CTOR(name, Ch) \
    }; \
    typedef HBOOST_PP_CAT(basic_, name)<char>     name; \
    typedef HBOOST_PP_CAT(basic_, name)<wchar_t>  HBOOST_PP_CAT(w, name); \
    /**/
HBOOST_IOSTREAMS_ARRAY(array_source, input_seekable)
HBOOST_IOSTREAMS_ARRAY(array_sink, output_seekable)
HBOOST_IOSTREAMS_ARRAY(array, seekable)
#undef HBOOST_IOSTREAMS_ARRAY_CTOR
#undef HBOOST_IOSTREAMS_ARRAY
...
#endif // #ifndef HBOOST_IOSTREAMS_ARRAY_HPP_INCLUDED
