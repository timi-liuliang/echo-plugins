...
#ifndef HBOOST_IOSTREAMS_FILE_HPP_INCLUDED
#define HBOOST_IOSTREAMS_FILE_HPP_INCLUDED
...
#include <hboost/iostreams/detail/fstream.hpp>
#include <hboost/iostreams/operations.hpp>       // seek.
...
private:
    struct impl {
        impl(const std::string& path, HBOOST_IOS::openmode mode)
            { file_.open(path.c_str(), mode); }
        ~impl() { if (file_.is_open()) file_.close(); }
        HBOOST_IOSTREAMS_BASIC_FILEBUF(Ch) file_;
    };
...
struct basic_file_source : private basic_file<Ch> {
    ...
    basic_file_source( const std::string& path,
                       HBOOST_IOS::openmode mode = 
                           HBOOST_IOS::in )
        : basic_file<Ch>(path, mode & ~HBOOST_IOS::out, HBOOST_IOS::in)
        { }
    void open( const std::string& path,
               HBOOST_IOS::openmode mode = HBOOST_IOS::in )
    {
        basic_file<Ch>::open(path, mode & ~HBOOST_IOS::out, HBOOST_IOS::in);
    }
};
...
struct basic_file_sink : private basic_file<Ch> {
    ...
    basic_file_sink( const std::string& path,
        ...
        : basic_file<Ch>(path, mode & ~HBOOST_IOS::in, HBOOST_IOS::out)
        { }
    void open( const std::string& path,
               HBOOST_IOS::openmode mode = HBOOST_IOS::out )
    {
        basic_file<Ch>::open(path, mode & ~HBOOST_IOS::in, HBOOST_IOS::out);
    }
};
...
basic_file<Ch>::basic_file
    ( const std::string& path, HBOOST_IOS::openmode mode, 
      HBOOST_IOS::openmode base_mode )
{ 
...
{ return pimpl_->file_.HBOOST_IOSTREAMS_PUBSYNC() == 0; }
...
#endif // #ifndef HBOOST_IOSTREAMS_FILE_HPP_INCLUDED
