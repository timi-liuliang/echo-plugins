...
// (C) Copyright Craig Henderson 2002.   'hboost/memmap.hpp' from sandbox
// Distributed under the Boost Software License, Version 1.0. (See accompanying
// file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt.)

#ifndef HBOOST_IOSTREAMS_MAPPED_FILE_HPP_INCLUDED
#define HBOOST_IOSTREAMS_MAPPED_FILE_HPP_INCLUDED
...
#include <hboost/iostreams/close.hpp>
#include <hboost/iostreams/concepts.hpp>
...
#include <hboost/iostreams/detail/ios.hpp>     // openmode, failure
#include <hboost/iostreams/detail/path.hpp>
...
public:
    ...
    HBOOST_IOS::openmode         mode;  // Deprecated
...
// hboost::filesystem::path, hboost::filesystem::wpath, 
// and hboost::iostreams::detail::path (used to store either a std::string or a
// std::wstring).
    ...
    : detail::mapped_file_params_base 
{
    ...
    // with hboost::filesystem::wpath
#ifndef HBOOST_IOSTREAMS_NO_WIDE_STREAMS
    HBOOST_STATIC_ASSERT((!is_same<Path, std::wstring>::value));
#endif
...
class HBOOST_IOSTREAMS_DECL mapped_file_source : public mapped_file_base {
private:
    ...
    friend struct hboost::iostreams::operations<mapped_file_source>;
public:
    ...
    HBOOST_STATIC_CONSTANT(size_type, max_length = static_cast<size_type>(-1));
...
private:
    ...
    hboost::shared_ptr<impl_type> pimpl_;
};

//------------------Definition of mapped_file---------------------------------//

class HBOOST_IOSTREAMS_DECL mapped_file : public mapped_file_base {
private:
    typedef mapped_file_source                      delegate_type;
    typedef delegate_type::safe_bool                safe_bool;
    typedef basic_mapped_file_params<detail::path>  param_type;
    friend struct hboost::iostreams::operations<mapped_file >;
...
public:
    ...
    HBOOST_STATIC_CONSTANT(size_type, max_length = delegate_type::max_length);
...
class HBOOST_IOSTREAMS_DECL mapped_file_sink : private mapped_file {
public:
    friend struct hboost::iostreams::operations<mapped_file_sink>;
...
mapped_file_source::mapped_file_source( 
    const Path& path, size_type length, hboost::intmax_t offset)
...
void mapped_file_source::open(const basic_mapped_file_params<Path>& p)
{
    ...
    if (params.flags) {
        if (params.flags != mapped_file::readonly)
            hboost::throw_exception(HBOOST_IOSTREAMS_FAILURE("invalid flags"));
    } else {
        if (params.mode & HBOOST_IOS::out)
            hboost::throw_exception(HBOOST_IOSTREAMS_FAILURE("invalid mode"));
        params.mode |= HBOOST_IOS::in;
    }
    open_impl(params);
}
...
template<typename Path>
mapped_file::mapped_file( 
    const Path& path, HBOOST_IOS::openmode mode, 
    size_type length, stream_offset offset )
...
mapped_file_sink::mapped_file_sink(
    ...
    hboost::intmax_t offset, mapmode flags )
...
void mapped_file_sink::open(const basic_mapped_file_params<Path>& p)
{
    ...
    if (params.flags) {
        ...
        if (params.mode & HBOOST_IOS::in)
            hboost::throw_exception(HBOOST_IOSTREAMS_FAILURE("invalid mode"));
        params.mode |= HBOOST_IOS::out;
    }
...
struct operations<mapped_file_source>
    : hboost::iostreams::detail::close_impl<closable_tag>
{
...
#endif // #ifndef HBOOST_IOSTREAMS_MAPPED_FILE_HPP_INCLUDED
