...
#ifndef HBOOST_IOSTREAMS_FILE_DESCRIPTOR_HPP_INCLUDED
#define HBOOST_IOSTREAMS_FILE_DESCRIPTOR_HPP_INCLUDED
...
#include <hboost/iostreams/categories.hpp>  // tags.
...
#include <hboost/iostreams/detail/file_handle.hpp>
...
class HBOOST_IOSTREAMS_DECL file_descriptor {
public:
...
#if defined(HBOOST_IOSTREAMS_USE_DEPRECATED)
    // Constructors taking file desciptors
...
private:
    ...
    void open( const detail::path& path, 
               HBOOST_IOS::openmode, 
               HBOOST_IOS::openmode = HBOOST_IOS::openmode(0) );

    typedef detail::file_descriptor_impl impl_type;
    shared_ptr<impl_type> pimpl_;
};

class HBOOST_IOSTREAMS_DECL file_descriptor_source : private file_descriptor {
public:
#ifdef HBOOST_IOSTREAMS_WINDOWS
    ...
    explicit file_descriptor_source( const std::string& path,
                                     HBOOST_IOS::openmode mode = HBOOST_IOS::in );
    ...
    // open overload taking a std::string
    void open(const std::string& path, HBOOST_IOS::openmode mode = HBOOST_IOS::in);

    // open overload taking C-style string
    void open(const char* path, HBOOST_IOS::openmode mode = HBOOST_IOS::in);

    // open overload taking a Boost.Filesystem path
    template<typename Path>
    void open(const Path& path, HBOOST_IOS::openmode mode = HBOOST_IOS::in);
private:

    // open overload taking a detail::path
    void open(const detail::path& path, HBOOST_IOS::openmode);
};

class HBOOST_IOSTREAMS_DECL file_descriptor_sink : private file_descriptor {
public:
#ifdef HBOOST_IOSTREAMS_WINDOWS
    typedef void*  handle_type;  // A.k.a HANDLE
#else
    ...
    explicit file_descriptor_sink( const std::string& path,
                                   HBOOST_IOS::openmode mode = HBOOST_IOS::out );
...
#endif // #ifndef HBOOST_IOSTREAMS_FILE_DESCRIPTOR_HPP_INCLUDED
