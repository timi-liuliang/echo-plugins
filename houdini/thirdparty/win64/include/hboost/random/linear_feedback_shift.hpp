...
#ifndef HBOOST_RANDOM_LINEAR_FEEDBACK_SHIFT_HPP
#define HBOOST_RANDOM_LINEAR_FEEDBACK_SHIFT_HPP
...
public:
    ...
    HBOOST_STATIC_CONSTANT(int, exponent1 = k);
    HBOOST_STATIC_CONSTANT(int, exponent2 = q);
    HBOOST_STATIC_CONSTANT(int, step_size = s);
    HBOOST_STATIC_CONSTANT(UIntType, default_seed = 341);
    ...
    HBOOST_STATIC_ASSERT(w > 0);
    HBOOST_STATIC_ASSERT(q > 0);
    HBOOST_STATIC_ASSERT(k < w);
    HBOOST_STATIC_ASSERT(0 < 2*q && 2*q < k);
    HBOOST_STATIC_ASSERT(0 < s && s <= k-q);
    ...
    HBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(linear_feedback_shift_engine,
        UIntType, s0)
    { seed(s0); }
    
    /** Constructs a @c linear_feedback_shift_engine, seeding it with seq. */
    HBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(linear_feedback_shift_engine,
        SeedSeq, seq)
    { seed(seq); }
    ...
    HBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(linear_feedback_shift_engine,
        UIntType, s0)
    {
        value = s0 & wordmask();
        if(value < (1 << (w-k))) {
     ...
     */
    HBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(linear_feedback_shift_engine,
        SeedSeq, seq)
     ...
     */
    HBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, linear_feedback_shift_engine, x)
    {
        os << x.value;
        return os;
    }
     ...
     */
    HBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, linear_feedback_shift_engine, x)
    {
        is >> x.value;
        return is;
    }
     ...
     */
    HBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(linear_feedback_shift_engine, x, y)
     ...
     * sequences of outputs.
     */
    HBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(linear_feedback_shift_engine)

private:
    /// \cond show_private
    static UIntType wordmask() { return hboost::low_bits_mask_t<w>::sig_bits; }
    /// \endcond
    UIntType value;
};
...
public:
    ...
    HBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(linear_feedback_shift,
        SeedSeq, seq)
    { seed(seq); }
    HBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(linear_feedback_shift,
        UIntType, val)
    { seed(val); }
    template<class It>
...
#endif // HBOOST_RANDOM_LINEAR_FEEDBACK_SHIFT_HPP
