...
#ifndef HBOOST_RANDOM_DISCARD_BLOCK_HPP
#define HBOOST_RANDOM_DISCARD_BLOCK_HPP
...
public:
    ...
    HBOOST_STATIC_CONSTANT(std::size_t, block_size = p);
    HBOOST_STATIC_CONSTANT(std::size_t, used_block = r);

    HBOOST_STATIC_CONSTANT(bool, has_fixed_range = false);
    HBOOST_STATIC_CONSTANT(std::size_t, total_block = p);
    HBOOST_STATIC_CONSTANT(std::size_t, returned_block = r);

    HBOOST_STATIC_ASSERT(total_block >= returned_block);
     ...
     */
    HBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(discard_block_engine,
                                               seed_type, value)
     ...
     */
    HBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(discard_block_engine, SeedSeq, seq)
    { _rng.seed(seq); _n = 0; }
    ...
    HBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(discard_block_engine, seed_type, s)
    { _rng.seed(s); _n = 0; }
    /** Seeds the underlying generator with seq. */
    HBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(discard_block_engine, SeedSeq, seq)
    { _rng.seed(seq); _n = 0; }
...
public:
    ...
    result_type min HBOOST_PREVENT_MACRO_SUBSTITUTION ()
    { return (this->base().min)(); }
    result_type max HBOOST_PREVENT_MACRO_SUBSTITUTION ()
...
#endif // HBOOST_RANDOM_DISCARD_BLOCK_HPP
