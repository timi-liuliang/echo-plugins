...
#ifndef HBOOST_RANDOM_RANDOM_GENERATOR_HPP
#define HBOOST_RANDOM_RANDOM_GENERATOR_HPP
...
{
private:
    typedef hboost::random::detail::ptr_helper<Engine> helper_type;
public:
     ...
     */
    result_type min HBOOST_PREVENT_MACRO_SUBSTITUTION () const { return (distribution().min)(); }
    /**
     * Precondition: distribution().max() is well-formed
     *
     * Returns: distribution().max()
     */
    result_type max HBOOST_PREVENT_MACRO_SUBSTITUTION () const { return (distribution().max)(); }
...
#endif // HBOOST_RANDOM_RANDOM_GENERATOR_HPP
