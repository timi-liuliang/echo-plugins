...
#ifndef HBOOST_RANDOM_NON_CENTRAL_CHI_SQUARED_DISTRIBUTION_HPP
#define HBOOST_RANDOM_NON_CENTRAL_CHI_SQUARED_DISTRIBUTION_HPP
...
public:
    ...
    public:
        ...
        {
            HBOOST_ASSERT(k_arg > RealType(0));
            HBOOST_ASSERT(lambda_arg > RealType(0));
        }
    ...
    RealType operator()(URNG& eng) 
    {
        using std::sqrt;
        if (_param.k() > 1) {
            hboost::random::normal_distribution<RealType> n_dist;
            hboost::random::chi_squared_distribution<RealType> c_dist(_param.k() - RealType(1));
            ...
            hboost::random::poisson_distribution<> p_dist(_param.lambda()/RealType(2));
            hboost::random::poisson_distribution<>::result_type _p = p_dist(eng);
            hboost::random::chi_squared_distribution<RealType> c_dist(_param.k() + RealType(2)*_p);
            return c_dist(eng);
        }
    }
    ...
    RealType min HBOOST_PREVENT_MACRO_SUBSTITUTION() const
    { return RealType(0); }
    
    /** Returns the largest value that the distribution can produce. */
    RealType max HBOOST_PREVENT_MACRO_SUBSTITUTION() const
    { return (std::numeric_limits<RealType>::infinity)(); }

    /** Writes the parameters of the distribution to a @c std::ostream. */
    HBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, non_central_chi_squared_distribution, dist)
    {
        os << dist.param();
        return os;
    }
    ...
    /** reads the parameters of the distribution from a @c std::istream. */
    HBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, non_central_chi_squared_distribution, dist)
    {
        param_type parm;
        if(is >> parm) {
            dist.param(parm);
    ...
    HBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(non_central_chi_squared_distribution, lhs, rhs)
    ...
    HBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(non_central_chi_squared_distribution)
