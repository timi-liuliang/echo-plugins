...
#ifndef HBOOST_RANDOM_GEOMETRIC_DISTRIBUTION_HPP
#define HBOOST_RANDOM_GEOMETRIC_DISTRIBUTION_HPP

#include <hboost/config/no_tr1/cmath.hpp>          // std::log
#include <iosfwd>
#include <ios>
 ...
 * hboost::geometric_distribution.  A backwards compatible
 * wrapper is provided in namespace hboost.
 * @endxmlwarning
 */
...
public:
    ...
    public:
        ...
        explicit param_type(RealType p_arg = RealType(0.5))
          : _p(p_arg)
        {
            HBOOST_ASSERT(RealType(0) < _p && _p < RealType(1));
        }
    ...
    IntType min HBOOST_PREVENT_MACRO_SUBSTITUTION () const { return IntType(0); }

    /** Returns the largest value that the distribution can produce. */
    IntType max HBOOST_PREVENT_MACRO_SUBSTITUTION () const
    ...
    result_type operator()(Engine& eng) const
    {
        using std::log;
        using std::floor;
        RealType x = RealType(1) - hboost::uniform_01<RealType>()(eng);
    ...
    HBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, geometric_distribution, gd)
    {
        os << gd._p;
        return os;
    }
    ...
    HBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, geometric_distribution, gd)
    {
        param_type parm;
        if(is >> parm) {
            gd.param(parm);
     ...
     */
    HBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(geometric_distribution, lhs, rhs)
    { return lhs._p == rhs._p; }
     ...
     * sequences of values given equal generators.
     */
    HBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(geometric_distribution)
...
#endif // HBOOST_RANDOM_GEOMETRIC_DISTRIBUTION_HPP
