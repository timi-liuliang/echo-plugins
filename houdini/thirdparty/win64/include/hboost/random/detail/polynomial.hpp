...
#ifndef HBOOST_RANDOM_DETAIL_POLYNOMIAL_HPP
#define HBOOST_RANDOM_DETAIL_POLYNOMIAL_HPP
...
public:
    ...
    {
        const std::size_t bits = std::numeric_limits<digit_t>::digits;
        HBOOST_ASSERT(shift > 0);
        HBOOST_ASSERT(shift < bits);
        digit_t prev = 0;
        for(std::size_t i = 0; i < size; ++i) {
            digit_t tmp = val[i];
    ...
    struct sparse_mod {
        sparse_mod(const digit_t * divisor, std::size_t divisor_bits)
        {
            ...
            HBOOST_ASSERT(_bit_indices.back() == divisor_bits - 1);
    ...
    static void mod_pow_x(hboost::uintmax_t exponent, const digit_t * mod, std::size_t mod_bits, digit_t * out)
    {
        const std::size_t bits = std::numeric_limits<digit_t>::digits;
        ...
        hboost::uintmax_t i = std::numeric_limits<hboost::uintmax_t>::digits - 1;
        while(((hboost::uintmax_t(1) << i) & exponent) == 0) {
            --i;
        }
        out[0] = 2;
        std::fill_n(out + 1, n - 1, digit_t(0));
        ...
        while(i--) {
            sqr(out, n);
            m(out, 2 * mod_bits - 1);
            if((hboost::uintmax_t(1) << i) & exponent) {
                shift_left(out, n, 1);
...
public:
    ...
    friend polynomial mod_pow_x(hboost::uintmax_t exponent, polynomial mod);
private:
...
inline polynomial mod_pow_x(hboost::uintmax_t exponent, polynomial mod)
{
    polynomial result;
    mod.normalize();
...
#endif // HBOOST_RANDOM_DETAIL_POLYNOMIAL_HPP
