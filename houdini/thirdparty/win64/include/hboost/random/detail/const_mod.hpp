...
#ifndef HBOOST_RANDOM_CONST_MOD_HPP
#define HBOOST_RANDOM_CONST_MOD_HPP

#include <hboost/assert.hpp>
#include <hboost/static_assert.hpp>
#include <hboost/integer_traits.hpp>
#include <hboost/type_traits/make_unsigned.hpp>
#include <hboost/random/detail/large_arithmetic.hpp>
...
public:
  static IntType apply(IntType x)
  {
    if(((unsigned_m() - 1) & unsigned_m()) == 0)
      ...
      HBOOST_ASSERT(suppress_warnings == 0);
  ...
  static IntType pow(IntType a, hboost::uintmax_t exponent)
  {
      IntType result = 1;
      while(exponent != 0) {
          if(exponent % 2 == 1) {
...
private:
  ...
  static IntType mult_schrage(IntType a, IntType value)
  {
    ...
    HBOOST_ASSERT(r < q);        // check that overflow cannot happen
  ...
  static IntType mult_general(IntType a, IntType b)
  {
    ...
    HBOOST_ASSERT(modulus == m);
    if(::hboost::uintmax_t(modulus) <=
        (::std::numeric_limits< ::hboost::uintmax_t>::max)() / modulus)
    {
      return static_cast<IntType>(hboost::uintmax_t(a) * b % modulus);
    } else {
  ...
  static IntType invert_euclidian0(IntType c)
  {
    ...
    IntType q = max / n;
    HBOOST_ASSERT(max % n != n - 1 && "c must be relatively prime to m.");
    l1 += q * l2;
    p = max - q * n + 1;
    for(;;) {
      if(p == 0)
...
#endif // HBOOST_RANDOM_CONST_MOD_HPP
