...
#ifndef HBOOST_RANDOM_UNIFORM_01_HPP
#define HBOOST_RANDOM_UNIFORM_01_HPP
 ...
 * (to-be-invented) @c hboost::bigfloat class with random bits efficiently.
...
public:
  ...
  result_type min HBOOST_PREVENT_MACRO_SUBSTITUTION () const;
  result_type max HBOOST_PREVENT_MACRO_SUBSTITUTION () const;
  void reset();
...
public:
  ...
  result_type min HBOOST_PREVENT_MACRO_SUBSTITUTION () const { return result_type(0); }
  result_type max HBOOST_PREVENT_MACRO_SUBSTITUTION () const { return result_type(1); }
  void reset() { }
...
{
  typedef hboost::random::detail::ptr_helper<UniformRandomNumberGenerator> traits;
public:
  ...
  HBOOST_STATIC_ASSERT(!std::numeric_limits<RealType>::is_integer);
#endif
...
#if !defined(HBOOST_NO_OPERATORS_IN_NAMESPACE) && !defined(HBOOST_NO_MEMBER_TEMPLATE_FRIENDS)
  template<class CharT, class Traits>
  friend std::basic_ostream<CharT,Traits>&
...
class uniform_01
  : public detail::select_uniform_01<UniformRandomNumberGenerator>::HBOOST_NESTED_TEMPLATE apply<RealType>::type
{
  typedef typename detail::select_uniform_01<UniformRandomNumberGenerator>::HBOOST_NESTED_TEMPLATE apply<RealType>::type impl_type;
  typedef hboost::random::detail::ptr_helper<UniformRandomNumberGenerator> traits;
public:
...
#endif // HBOOST_RANDOM_UNIFORM_01_HPP
