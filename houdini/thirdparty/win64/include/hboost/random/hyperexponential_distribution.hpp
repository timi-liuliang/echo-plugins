 ...
 *
 * Much of the code here taken by hboost::math::hyperexponential_distribution.
...
#ifndef HBOOST_RANDOM_HYPEREXPONENTIAL_DISTRIBUTION_HPP
#define HBOOST_RANDOM_HYPEREXPONENTIAL_DISTRIBUTION_HPP


#include <hboost/config.hpp>
#include <hboost/math/special_functions/fpclassify.hpp>
#include <hboost/random/detail/operators.hpp>
#include <hboost/random/detail/vector_io.hpp>
...
#include <cstddef>
#include <iterator>
#ifndef HBOOST_NO_CXX11_HDR_INITIALIZER_LIST
# include <initializer_list>
#endif // HBOOST_NO_CXX11_HDR_INITIALIZER_LIST
#include <iostream>
#include <limits>
#include <numeric>
#include <vector>
...
namespace hboost { namespace random {
...
bool check_probabilities(std::vector<RealT> const& probabilities)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        if (probabilities[i] < 0
            || probabilities[i] > 1
            || !(hboost::math::isfinite)(probabilities[i]))
        {
            return false;
        }
        sum += probabilities[i];
...
class hyperexponential_distribution
{
    ...
    public: class param_type
    {
        ...
        public: template <typename ProbRangeT, typename RateRangeT>
                param_type(ProbRangeT const& prob_range,
                           ...
                           typename hboost::disable_if_c<hboost::has_pre_increment<ProbRangeT>::value || hboost::has_pre_increment<RateRangeT>::value>::type* = 0)
        : probs_(hboost::begin(prob_range), hboost::end(prob_range)),
          rates_(hboost::begin(rate_range), hboost::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
        ...
        public: template <typename RateIterT>
                param_type(RateIterT rate_first, 
                           RateIterT rate_last,  
                           typename hboost::enable_if_c<hboost::has_pre_increment<RateIterT>::value>::type* = 0)
        ...
        : probs_(hboost::size(rate_range), 1), // Will be normalized below
          rates_(hboost::begin(rate_range), hboost::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
        ...
        public: HBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, param_type, param)
        {
        ...
        public: HBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, param_type, param)
        {
        ...
        public: HBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(param_type, lhs, rhs)
        {
        ...
        public: HBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(param_type)
    ...
    : dd_(std::vector<RealT>(hboost::size(rate_range), 1)),
      rates_(hboost::begin(rate_range), hboost::end(rate_range))
    {
        assert( hyperexp_detail::check_params(dd_.probabilities(), rates_) );
    }
            ...
            RealT operator()(URNG& urng) const
    {
        ...
        return hboost::random::exponential_distribution<RealT>(rates_[i])(urng);
    }
    ...
    public: RealT min HBOOST_PREVENT_MACRO_SUBSTITUTION () const
    {
        return 0;
    }

    /** Returns the largest value that the distribution can produce. */
    public: RealT max HBOOST_PREVENT_MACRO_SUBSTITUTION () const
    {
    ...
    {
        dd_.param(typename hboost::random::discrete_distribution<int,RealT>::param_type(param.probabilities()));
        rates_ = param.rates();
    }
    ...
    public: HBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, hyperexponential_distribution, hd)
    {
        os << hd.param();
        return os;
    }
    ...
    public: HBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, hyperexponential_distribution, hd)
    {
        param_type param;
        if(is >> param)
        {
     ...
     */
    public: HBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(hyperexponential_distribution, lhs, rhs)
    {
     ...
     */
    public: HBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(hyperexponential_distribution)


    private: hboost::random::discrete_distribution<int,RealT> dd_; ///< The \c discrete_distribution used to sample the phase probability and choose the rate
    private: std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
}; // hyperexponential_distribution

}} // namespace hboost::random


#endif // HBOOST_RANDOM_HYPEREXPONENTIAL_DISTRIBUTION_HPP
