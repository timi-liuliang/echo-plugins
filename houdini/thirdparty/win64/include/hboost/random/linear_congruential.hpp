...
#ifndef HBOOST_RANDOM_LINEAR_CONGRUENTIAL_HPP
#define HBOOST_RANDOM_LINEAR_CONGRUENTIAL_HPP
...
public:
    ...
    HBOOST_STATIC_CONSTANT(IntType, multiplier = a);
    HBOOST_STATIC_CONSTANT(IntType, increment = c);
    HBOOST_STATIC_CONSTANT(IntType, modulus = m);
    HBOOST_STATIC_CONSTANT(IntType, default_seed = 1);
    
    HBOOST_STATIC_ASSERT(std::numeric_limits<IntType>::is_integer);
    HBOOST_STATIC_ASSERT(m == 0 || a < m);
    HBOOST_STATIC_ASSERT(m == 0 || c < m);
     ...
     */
    HBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(linear_congruential_engine,
                                               IntType, x0)
    { seed(x0); }
     ...
     */
    HBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(linear_congruential_engine,
                                             SeedSeq, seq)
    { seed(seq); }
     ...
     */
    HBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(linear_congruential_engine, IntType, x0)
    {
        ...
        HBOOST_ASSERT(_x >= (min)());
        HBOOST_ASSERT(_x <= (max)());
    }

    /**
     * Seeds a @c linear_congruential_engine using values from a SeedSeq.
     */
    HBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(linear_congruential_engine, SeedSeq, seq)
...
#if !defined(HBOOST_RANDOM_NO_STREAM_OPERATORS)
...
{
public:
    typedef hboost::uint32_t result_type;

    HBOOST_STATIC_CONSTANT(bool, has_fixed_range = false);
    /**
     * Returns the smallest value that the generator can produce
     */
    static uint32_t min HBOOST_PREVENT_MACRO_SUBSTITUTION () { return 0; }
    /**
     * Returns the largest value that the generator can produce
     */
    static uint32_t max HBOOST_PREVENT_MACRO_SUBSTITUTION ()
    { return 0x7FFFFFFF; }
     ...
     */
    HBOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(rand48, result_type, x0)
    { seed(x0); }
    /**
     * Seeds the generator with values produced by @c seq.generate().
     */
    HBOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(rand48, SeedSeq, seq)
    { seed(seq); }
    /**
     ...
     */
    HBOOST_RANDOM_DETAIL_ARITHMETIC_SEED(rand48, result_type, x0)
    { lcf.seed(cnv(x0)); }
    /**
     * Seeds the generator using values from an iterator range,
     * and updates first to point one past the last value consumed.
     ...
     * Seeds the generator with values produced by @c seq.generate().
     */
    HBOOST_RANDOM_DETAIL_SEED_SEQ_SEED(rand48, SeedSeq, seq)
    { lcf.seed(seq); }
    ...
    /** Advances the state of the generator by @c z. */
    void discard(hboost::uintmax_t z) { lcf.discard(z); }
...
private:
    ...
    static hboost::uint64_t cnv(hboost::uint32_t x)
    { return (static_cast<uint64_t>(x) << 16) | 0x330e; }
    /// \endcond
};
#endif /* !HBOOST_NO_INT64_T && !HBOOST_NO_INTEGRAL_INT64_T */
...
#endif // HBOOST_RANDOM_LINEAR_CONGRUENTIAL_HPP
