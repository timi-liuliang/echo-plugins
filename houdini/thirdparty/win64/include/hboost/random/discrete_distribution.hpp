...
#ifndef HBOOST_RANDOM_DISCRETE_DISTRIBUTION_HPP_INCLUDED
#define HBOOST_RANDOM_DISCRETE_DISTRIBUTION_HPP_INCLUDED
...
public:
    ...
    public:
        ...
        explicit param_type(const Range& range)
          : _probabilities(hboost::begin(range), hboost::end(range))
        {
            normalize();
        }
    ...
    {
        init(hboost::begin(range), hboost::end(range));
    }
    /**
    ...
    {
        HBOOST_ASSERT(!_impl._alias_table.empty());
        IntType result;
        WeightType test;
        do {
    ...
    result_type min HBOOST_PREVENT_MACRO_SUBSTITUTION () const { return 0; }
    ...
    HBOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, discrete_distribution, dd)
    {
        os << dd.param();
        return os;
    }
    ...
    HBOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, discrete_distribution, dd)
    {
        param_type parm;
        if(is >> parm) {
            dd.param(parm);
     ...
     */
    HBOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(discrete_distribution, lhs, rhs)
    {
     ...
     */
    HBOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(discrete_distribution)
...
private:
    ...
    typedef typename detail::select_alias_table<
        (::hboost::is_integral<WeightType>::value)
