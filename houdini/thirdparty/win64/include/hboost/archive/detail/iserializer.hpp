#ifndef HBOOST_ARCHIVE_DETAIL_ISERIALIZER_HPP
#define HBOOST_ARCHIVE_DETAIL_ISERIALIZER_HPP
...
#include <hboost/serialization/assume_abstract.hpp>
    ...
    #define DONT_USE_HAS_NEW_OPERATOR (                    \
           HBOOST_WORKAROUND(__IBMCPP__, < 1210)            \
...
#include <hboost/archive/detail/basic_iserializer.hpp>
#include <hboost/archive/detail/basic_pointer_iserializer.hpp>
...
private:
    virtual void destroy(/*const*/ void *address) const {
        hboost::serialization::access::destroy(static_cast<T *>(address));
    }
protected:
    ...
    {}
public:
    virtual HBOOST_DLLEXPORT void load_object_data(
        basic_iarchive & ar,
        void *x, 
    ...
    virtual bool tracking(const unsigned int /* flags */) const {
        return hboost::serialization::tracking_level< T >::value 
                == hboost::serialization::track_always
            || ( hboost::serialization::tracking_level< T >::value 
                == hboost::serialization::track_selectively
...
template<class Archive, class T>
HBOOST_DLLEXPORT void iserializer<Archive, T>::load_object_data(
    basic_iarchive & ar,
    void *x, 
    const unsigned int file_version
) const {
    ...
    if(file_version > static_cast<const unsigned int>(version()))
        hboost::serialization::throw_exception(
            archive::archive_exception(
                hboost::archive::archive_exception::unsupported_class_version,
                get_debug_info()
            )
        );
    #endif
...
struct heap_allocation {
    // hboost::has_new_operator< T > doesn't work on these compilers
    #if DONT_USE_HAS_NEW_OPERATOR
...
private:
    ...
    HBOOST_DLLEXPORT virtual void load_object_ptr(
        basic_iarchive & ar, 
        void * x,
        const unsigned int file_version
    ) const HBOOST_USED;
...
// note: HBOOST_DLLEXPORT is so that code for polymorphic class
// serialized only through base class won't get optimized out
template<class Archive, class T>
HBOOST_DLLEXPORT void pointer_iserializer<Archive, T>::load_object_ptr(
    basic_iarchive & ar, 
    void * t,
    const unsigned int file_version
) const
{
    ...
    HBOOST_TRY {
        ...
        hboost::serialization::load_construct_data_adl<Archive, T>(
            ar_impl,
            static_cast<T *>(t),
            file_version
        );
    ...
    ar_impl >> hboost::serialization::make_nvp(NULL, * static_cast<T *>(t));
}
...
struct load_pointer_type {
    ...
    static const basic_pointer_iserializer * register_type(Archive &ar, const T & /*t*/){
        ...
        typedef typename
            mpl::eval_if<
                hboost::serialization::is_abstract<const T>,
                hboost::mpl::identity<abstract>,
                hboost::mpl::identity<non_abstract>  
            >::type typex;
    ...
    static T * pointer_tweak(
        ...
        void * upcast = const_cast<void *>(
            hboost::serialization::void_upcast(
                eti,
                hboost::serialization::singleton<
                    typename 
                    hboost::serialization::type_info_implementation< T >::type
    ...
    find(const hboost::serialization::extended_type_info & type){
...
struct load_array_type {
    ...
    static void invoke(Archive &ar, T &t){
        ...
        hboost::serialization::collection_size_type count;
        ar >> HBOOST_SERIALIZATION_NVP(count);
        if(static_cast<std::size_t>(count) > current_count)
            hboost::serialization::throw_exception(
                archive::archive_exception(
                    hboost::archive::archive_exception::array_size_too_short
                )
            );
...
#endif // HBOOST_ARCHIVE_DETAIL_ISERIALIZER_HPP
