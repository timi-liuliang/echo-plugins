#ifndef HBOOST_ARCHIVE_DETAIL_HELPER_COLLECTION_HPP
#define HBOOST_ARCHIVE_DETAIL_HELPER_COLLECTION_HPP
...
class helper_collection
{
    ...
    typedef std::pair<
        const void *,
        hboost::shared_ptr<void>
    > helper_value_type;
    template<class T>
    hboost::shared_ptr<void> make_helper_ptr(){
        // use hboost::shared_ptr rather than std::shared_ptr to maintain
        // c++03 compatibility
        return hboost::make_shared<T>();
    }
    ...
    struct predicate {
        HBOOST_DELETED_FUNCTION(predicate & operator=(const predicate & rhs))
    public:
...
#endif // HBOOST_ARCHIVE_DETAIL_HELPER_COLLECTION_HPP
