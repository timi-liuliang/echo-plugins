#ifndef HBOOST_ARCHIVE_ITERATORS_WCHAR_FROM_MB_HPP
#define HBOOST_ARCHIVE_ITERATORS_WCHAR_FROM_MB_HPP
...
#include <hboost/archive/iterators/dataflow_exception.hpp>
...
template<class Base>
class wchar_from_mb 
    : public hboost::iterator_adaptor<
        wchar_from_mb<Base>, 
        Base, 
        wchar_t,
    ...
    >
{
    ...
    typedef typename hboost::iterator_adaptor<
        wchar_from_mb<Base>, 
        Base, 
        wchar_t,
        single_pass_traversal_tag,
    ...
    hboost::archive::detail::utf8_codecvt_facet m_codecvt_facet;
    std::mbstate_t m_mbs;

    template<typename T>
    struct sliding_buffer {
        hboost::array<T, 32> m_buffer;
        typename hboost::array<T, 32>::const_iterator m_next_available;
        typename hboost::array<T, 32>::iterator m_next;
        bool m_done;
        // default ctor
        sliding_buffer() :
...
public:
    ...
    wchar_from_mb(T start) : 
        ...
        m_mbs(std::mbstate_t())
    {
        HBOOST_ASSERT(std::mbsinit(&m_mbs));
        drain();
    }
...
void wchar_from_mb<Base>::drain(){
    HBOOST_ASSERT(! m_input.m_done);
    for(;;){
        typename hboost::iterators::iterator_reference<Base>::type c = *(this->base_reference());
    ...
    const typename hboost::iterators::iterator_value<Base>::type * input_new_start;
    ...
    HBOOST_ASSERT(std::codecvt_base::ok == r);
...
#endif // HBOOST_ARCHIVE_ITERATORS_WCHAR_FROM_MB_HPP
