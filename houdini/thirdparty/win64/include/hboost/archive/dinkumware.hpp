#ifndef HBOOST_ARCHIVE_DINKUMWARE_HPP
#define HBOOST_ARCHIVE_DINKUMWARE_HPP
...
operator<<(basic_ostream<CharType> & os, hboost::uint64_t t){
...
template<class CharType>
basic_ostream<CharType> & 
operator<<(basic_ostream<CharType> &os, hboost::int64_t t){
    if(0 <= t){
        os << static_cast<hboost::uint64_t>(t);
    }
    else{
        os.put('-');
        os << -t;
...
operator>>(basic_istream<CharType> &is, hboost::int64_t & t){
    CharType d;
    do{
        d = is.get();
    }
...
template<class CharType>
basic_istream<CharType> & 
operator>>(basic_istream<CharType> &is, hboost::uint64_t & t){
    hboost::int64_t it;
    is >> it;
    t = it;
    return is;
...
#endif //HBOOST_ARCHIVE_DINKUMWARE_HPP
