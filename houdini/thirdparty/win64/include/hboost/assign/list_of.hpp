...
#ifndef HBOOST_ASSIGN_LIST_OF_HPP
#define HBOOST_ASSIGN_LIST_OF_HPP

#if defined(_MSC_VER)
# pragma once
#endif

#include <hboost/assign/assignment_exception.hpp>
...
// this here is necessary to avoid compiler error in <hboost/array.hpp>
#if !HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x564))
...
namespace assign_detail
{
    ...
    struct assign_decay
    {
        ...
        //
        typedef HBOOST_DEDUCED_TYPENAME mpl::eval_if<
            ::hboost::is_array<T>,
            ::hboost::decay<const T>,
            ::hboost::decay<T> >::type type;
    };
    ...
    public:
        ...
        Container convert_to_container() const
        {
            ...
            HBOOST_STATIC_CONSTANT( bool, is_array_flag = sizeof( assign_detail::assign_is_array( c ) ) 
                                   == sizeof( type_traits::yes_type ) );

            typedef HBOOST_DEDUCED_TYPENAME mpl::if_c< is_array_flag,
                                                      array_type_tag,
...
// old Dinkumware doesn't support iterator type as template
        ...
        {
            typedef HBOOST_DEDUCED_TYPENAME Array::value_type value_type;

#if HBOOST_WORKAROUND(HBOOST_INTEL, <= 910 ) || HBOOST_WORKAROUND(__SUNPRO_CC, <= 0x580 )
            HBOOST_DEDUCED_TYPENAME remove_const<Array>::type ar;
#else
            Array ar;
#endif            
    ...
    {
        return ::hboost::iterator_range_detail::equal( l, r );
    }
    ...
    inline bool operator<( const converter<T,I>& l, const Range& r )
    {
        return ::hboost::iterator_range_detail::less_than( l, r );
    }
    ...
    {
        return Os << ::hboost::make_iterator_range( r.begin(), r.end() );
    }
    ...
    template< class T > 
    class generic_list : 
        public converter< generic_list< HBOOST_DEDUCED_TYPENAME assign_decay<T>::type >,
                          HBOOST_DEDUCED_TYPENAME std::deque<HBOOST_DEDUCED_TYPENAME 
                                                            assign_decay<T>::type>::iterator >
    {
        typedef HBOOST_DEDUCED_TYPENAME assign_decay<T>::type Ty;
        typedef std::deque<Ty>  impl_type;
        mutable impl_type       values_;
        
    public:
        typedef HBOOST_DEDUCED_TYPENAME impl_type::iterator         iterator;
        typedef iterator                                           const_iterator;
        typedef HBOOST_DEDUCED_TYPENAME impl_type::value_type       value_type;
        typedef HBOOST_DEDUCED_TYPENAME impl_type::size_type        size_type;
        typedef HBOOST_DEDUCED_TYPENAME impl_type::difference_type  difference_type;
...
#define HBOOST_ASSIGN_PARAMS1(n) HBOOST_PP_ENUM_PARAMS(n, class U)
#define HBOOST_ASSIGN_PARAMS2(n) HBOOST_PP_ENUM_BINARY_PARAMS(n, U, const& u)
#define HBOOST_ASSIGN_PARAMS3(n) HBOOST_PP_ENUM_PARAMS(n, u)
#define HBOOST_ASSIGN_PARAMS4(n) HBOOST_PP_ENUM_PARAMS(n, U)
#define HBOOST_ASSIGN_PARAMS2_NO_REF(n) HBOOST_PP_ENUM_BINARY_PARAMS(n, U, u)

#define HBOOST_PP_LOCAL_LIMITS (1, HBOOST_ASSIGN_MAX_PARAMETERS)
#define HBOOST_PP_LOCAL_MACRO(n) \
    template< class U, HBOOST_ASSIGN_PARAMS1(n) > \
    generic_list& operator()(U const& u, HBOOST_ASSIGN_PARAMS2(n) ) \
    { \
        this->push_back( Ty(u, HBOOST_ASSIGN_PARAMS3(n))); \
        return *this; \
    } \
    /**/
...
#include HBOOST_PP_LOCAL_ITERATE()
        ...
        {
            return this-> HBOOST_NESTED_TEMPLATE convert_to_container<Container>();
        }
    };
...
namespace assign
{
    ...
    inline assign_detail::static_generic_list< HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<T>::type,N>
    ref_list_of( T& t )
    {
        return assign_detail::static_generic_list<HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<T>::type,N>( t );
    }
    
    template< int N, class T >
    inline assign_detail::static_generic_list<const HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<T>::type,N>
    cref_list_of( const T& t )
    {
        return assign_detail::static_generic_list<const HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<T>::type,N>( t );
    }

#define HBOOST_PP_LOCAL_LIMITS (1, HBOOST_ASSIGN_MAX_PARAMETERS)
#define HBOOST_PP_LOCAL_MACRO(n) \
    template< class T, class U, HBOOST_ASSIGN_PARAMS1(n) > \
    inline assign_detail::generic_list<T> \
    list_of(U const& u, HBOOST_ASSIGN_PARAMS2(n) ) \
    { \
        return assign_detail::generic_list<T>()(u, HBOOST_ASSIGN_PARAMS3(n)); \
    } \
    /**/
...
#define HBOOST_PP_LOCAL_MACRO(n) \
    template< class U, HBOOST_ASSIGN_PARAMS1(n) > \
    inline assign_detail::generic_list< tuple<U, HBOOST_ASSIGN_PARAMS4(n)> > \
    tuple_list_of(U u, HBOOST_ASSIGN_PARAMS2_NO_REF(n) ) \
    { \
        return assign_detail::generic_list< tuple<U, HBOOST_ASSIGN_PARAMS4(n)> >()( tuple<U,HBOOST_ASSIGN_PARAMS4(n)>( u, HBOOST_ASSIGN_PARAMS3(n) )); \
    } \
    /**/
...
#include HBOOST_PP_LOCAL_ITERATE()
    ...
    inline assign_detail::generic_list< std::pair
        < 
            HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<Key>::type, 
            HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<T>::type
        > >
    map_list_of( const Key& k, const T& t )
    {
        typedef HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<Key>::type k_type;
        typedef HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<T>::type   t_type;
    ...
    inline assign_detail::generic_list< std::pair
        < 
            HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<F>::type, 
            HBOOST_DEDUCED_TYPENAME assign_detail::assign_decay<S>::type
        > >
...
#undef HBOOST_ASSIGN_PARAMS4
#undef HBOOST_ASSIGN_PARAMS2_NO_REF
