...
//      great as hboost::intmax_t can cause some differences to be
...
// Revision History
// 11 Feb 2001 - Use HBOOST_STATIC_CONSTANT (David Abrahams)
...
// 06 Feb 2001 - Factored if_true out into hboost/detail/select_type.hpp
//               (David Abrahams)
...
#ifndef HBOOST_NUMERIC_TRAITS_HPP_DWA20001901
# define HBOOST_NUMERIC_TRAITS_HPP_DWA20001901
...
namespace hboost { namespace detail {
  ...
  struct is_signed
  {
    ...
    HBOOST_STATIC_CONSTANT(bool, value = (Number(-1) < Number(0)));
#else
    HBOOST_STATIC_CONSTANT(bool, value = std::numeric_limits<Number>::is_signed);
#endif
  };
...
# ifndef HBOOST_NO_LIMITS_COMPILE_TIME_CONSTANTS
  ...
  template <> struct digit_traits_select<true>
  {
      ...
      {
          HBOOST_STATIC_CONSTANT(int, digits = std::numeric_limits<T>::digits);
      };
  };
  ...
  template <> struct digit_traits_select<false>
  {
      template <class T> struct traits
      {
          HBOOST_STATIC_CONSTANT(int, digits = (
  ...
  template <class T> struct digit_traits
  {
      ...
      HBOOST_STATIC_CONSTANT(int, digits = traits::digits);
  };
#endif

  // Template class integer_traits<Integer> -- traits of various integer types
  // This should probably be rolled into hboost::integer_traits one day, but I
      ...
      if_true<(int(x::is_signed)
              && (!int(x::is_bounded)
                 // digits is the number of no-sign bits
                  || (int(x::digits) + 1 >= digit_traits<hboost::intmax_t>::digits)))>::template then<
        Integer,
...
#else
      HBOOST_STATIC_ASSERT(hboost::is_integral<Integer>::value);
...
#endif // HBOOST_NUMERIC_TRAITS_HPP_DWA20001901
