...
#ifndef HBOOST_INTERPROCESS_FILE_MAPPING_HPP
#define HBOOST_INTERPROCESS_FILE_MAPPING_HPP
...
#if !defined(HBOOST_INTERPROCESS_MAPPED_FILES)
...
class file_mapping
{
   ...
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(file_mapping)
   ...
   //!Does not throw
   file_mapping(HBOOST_RV_REF(file_mapping) moved)
   ...
   //!Does not throw
   file_mapping &operator=(HBOOST_RV_REF(file_mapping) moved)
   {
      file_mapping tmp(hboost::move(moved));
      this->swap(tmp);
      return *this;
   }
...
#endif   //HBOOST_INTERPROCESS_FILE_MAPPING_HPP
