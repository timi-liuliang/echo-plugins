...
#ifndef HBOOST_INTERPROCESS_MAPPED_REGION_HPP
#define HBOOST_INTERPROCESS_MAPPED_REGION_HPP
...
#define HBOOST_INTERPROCESS_MADVISE_USES_CADDR_T
#include <sys/types.h>
#endif
...
#define HBOOST_INTERPROCESS_MADV_DONTNEED_HAS_NONDESTRUCTIVE_SEMANTICS
#endif
...
class mapped_region
{
   #if !defined(HBOOST_INTERPROCESS_DOXYGEN_INVOKED)
   //Non-copyable
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(mapped_region)
   #endif   //#ifndef HBOOST_INTERPROCESS_DOXYGEN_INVOKED
   ...
   mapped_region(HBOOST_RV_REF(mapped_region) other)
   #if defined (HBOOST_INTERPROCESS_WINDOWS)
   :  m_base(0), m_size(0)
   ,  m_page_offset(0)
   ,  m_mode(read_only)
   ...
   mapped_region &operator=(HBOOST_RV_REF(mapped_region) other)
   {
      mapped_region tmp(hboost::move(other));
      this->swap(tmp);
      return *this;
   }
   ...
   #if defined(HBOOST_INTERPROCESS_WINDOWS) && !defined(HBOOST_INTERPROCESS_FORCE_GENERIC_EMULATION)
   template<int Dummy>
   ...
   (std::size_t bytes, bool from_back, void *&shrink_page_start, std::size_t &shrink_page_bytes)
{
   ...
   if(m_base == 0 || bytes > m_size){
      ...
      if(from_back){
         ...
         HBOOST_ASSERT(shrink_page_bytes%page_size == 0);
      }
      return true;
   }
}
   ...
   (offset_t mapping_size, offset_t offset, offset_t page_offset, std::size_t &size)
{
   ...
   if(mapping_size < offset ||
      hboost::uintmax_t(mapping_size - (offset - page_offset)) >
         hboost::uintmax_t(std::size_t(-1))){
...
#else    //#if defined (HBOOST_INTERPROCESS_WINDOWS)
   ...
   : m_base(0), m_size(0), m_page_offset(0), m_mode(mode), m_is_xsi(false)
{
   ...
   #ifdef HBOOST_INTERPROCESS_XSI_SHARED_MEMORY_OBJECTS
   if(map_hnd.is_xsi){
      //Get the size
      ::shmid_ds xsi_ds;
   ...
   #endif   //ifdef HBOOST_INTERPROCESS_XSI_SHARED_MEMORY_OBJECTS
   ...
   #ifdef MAP_NOSYNC
      #define HBOOST_INTERPROCESS_MAP_NOSYNC MAP_NOSYNC
   #else
      #define HBOOST_INTERPROCESS_MAP_NOSYNC 0
   #endif   //MAP_NOSYNC

   //Create new mapping
   int prot    = 0;
   int flags   = map_options == default_map_options ? HBOOST_INTERPROCESS_MAP_NOSYNC : map_options;

   #undef HBOOST_INTERPROCESS_MAP_NOSYNC
...
inline bool mapped_region::advise(advice_types advice)
{
   ...
   switch(advice){
      ...
      case advice_dontneed:
         ...
         mode = mode_padv;
         #elif defined(MADV_DONTNEED) && defined(HBOOST_INTERPROCESS_MADV_DONTNEED_HAS_NONDESTRUCTIVE_SEMANTICS)
   ...
   switch(mode){
         ...
         case mode_madv:
         return 0 == madvise(
            #if defined(HBOOST_INTERPROCESS_MADVISE_USES_CADDR_T)
            (caddr_t)
            #endif
...
{
   ::hboost::adl_move_swap(this->m_base, other.m_base);
   ::hboost::adl_move_swap(this->m_size, other.m_size);
   ::hboost::adl_move_swap(this->m_page_offset, other.m_page_offset);
   ::hboost::adl_move_swap(this->m_mode,  other.m_mode);
   #if defined (HBOOST_INTERPROCESS_WINDOWS)
   ::hboost::adl_move_swap(this->m_file_or_mapping_hnd, other.m_file_or_mapping_hnd);
   #else
   ::hboost::adl_move_swap(this->m_is_xsi, other.m_is_xsi);
   #endif
}
...
#endif   //HBOOST_INTERPROCESS_MAPPED_REGION_HPP

#if !defined(HBOOST_INTERPROCESS_DOXYGEN_INVOKED)

#ifndef HBOOST_INTERPROCESS_MAPPED_REGION_EXT_HPP
#define HBOOST_INTERPROCESS_MAPPED_REGION_EXT_HPP
...
#endif   //defined(HBOOST_INTERPROCESS_WINDOWS) && !defined(HBOOST_INTERPROCESS_FORCE_GENERIC_EMULATION)

#endif   //#ifdef HBOOST_INTERPROCESS_MAPPED_REGION_EXT_HPP
