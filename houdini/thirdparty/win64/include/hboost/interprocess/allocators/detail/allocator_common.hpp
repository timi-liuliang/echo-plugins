...
#ifndef HBOOST_INTERPROCESS_ALLOCATOR_DETAIL_ALLOCATOR_COMMON_HPP
#define HBOOST_INTERPROCESS_ALLOCATOR_DETAIL_ALLOCATOR_COMMON_HPP
...
#include <hboost/interprocess/detail/utilities.hpp> //to_raw_pointer
#include <hboost/utility/addressof.hpp> //hboost::addressof
#include <hboost/assert.hpp>   //HBOOST_ASSERT
#include <hboost/interprocess/exceptions.hpp> //bad_alloc
#include <hboost/interprocess/sync/scoped_lock.hpp> //scoped_lock
#include <hboost/interprocess/containers/allocation_type.hpp> //hboost::interprocess::allocation_type
...
struct get_or_create_node_pool_func
{
   ...
   //!Can throw hboost::interprocess::bad_alloc
   void operator()()
   {
      //Find or create the node_pool_t
      mp_node_pool =    mp_segment_manager->template find_or_construct
                        <NodePool>(hboost::interprocess::unique_instance)(mp_segment_manager);
...
struct destroy_if_last_link_func
{
   ...
   void operator()()
   {
      ...
      mp_node_pool->get_segment_manager()->template destroy<NodePool>(hboost::interprocess::unique_instance);
   }
...
class cache_impl
{
   ...
   {
      ::hboost::adl_move_swap(mp_node_pool, other.mp_node_pool);
      ::hboost::adl_move_swap(m_cached_nodes, other.m_cached_nodes);
      ::hboost::adl_move_swap(m_max_cached_nodes, other.m_max_cached_nodes);
   }
};
...
class array_allocation_impl
{
   ...
   typedef hboost::container::container_detail::transform_multiallocation_chain
   ...
   pointer allocation_command(hboost::interprocess::allocation_type command,
   ...
   {  return pointer(hboost::addressof(value));  }

   //!Returns address of non mutable object.
   //!Never throws
   const_pointer address(const_reference value) const
   {  return const_pointer(hboost::addressof(value));  }

   //!Constructs an object
   //!Throws if T's constructor throws
   //!For backwards compatibility with libraries using C++03 allocators
   template<class P>
   void construct(const pointer &ptr, HBOOST_FWD_REF(P) p)
   {  ::new((void*)ipcdetail::to_raw_pointer(ptr), hboost_container_new_t()) value_type(::hboost::forward<P>(p));  }

   //!Destroys object. Throws if object's
   //!destructor throws
   void destroy(const pointer &ptr)
   {  HBOOST_ASSERT(ptr != 0); (*ptr).~value_type();  }
};
      ...
      , SegmentManager>
{
   ...
   //!Throws hboost::interprocess::bad_alloc if there is no enough memory
         ...
         <cached_allocator_impl<T, NodePool, Version>, T, typename NodePool::segment_manager>
{
   ...
   {  ::hboost::adl_move_swap(alloc1.m_cache, alloc2.m_cache);   }
   ...
   #endif   //!defined(HBOOST_INTERPROCESS_DOXYGEN_INVOKED)
};
   ...
   : public private_node_allocator_t
{
 ...
 public:
   ...
   //!Allocates array of count elements. Can throw hboost::interprocess::bad_alloc
   void *allocate_node()
   {
      //-----------------------
   ...
   size_type dec_ref_count()
   {
      ...
      hboost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
      HBOOST_ASSERT(m_header.m_usecount > 0);
      return --m_header.m_usecount;
   }
...
#endif   //#ifndef HBOOST_INTERPROCESS_ALLOCATOR_DETAIL_ALLOCATOR_COMMON_HPP
