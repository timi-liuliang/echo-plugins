...
#ifndef HBOOST_INTERPROCESS_DETAIL_SPIN_RECURSIVE_MUTEX_HPP
#define HBOOST_INTERPROCESS_DETAIL_SPIN_RECURSIVE_MUTEX_HPP
...
inline void spin_recursive_mutex::lock()
{
   ...
   if(ipcdetail::equal_systemwide_thread_id(thr_id , old_id)){
      if((unsigned int)(m_nLockCount+1) == 0){
         ...
         throw interprocess_exception("hboost::interprocess::spin_recursive_mutex recursive lock overflow");
      }
      ++m_nLockCount;
   }
   else{
...
inline bool spin_recursive_mutex::timed_lock(const hboost::posix_time::ptime &abs_time)
{
   typedef ipcdetail::OS_systemwide_thread_id_t handle_t;
   const handle_t thr_id(ipcdetail::get_current_systemwide_thread_id());
   handle_t old_id;
...
inline void spin_recursive_mutex::unlock()
{
   ...
   (void)old_id;
   (void)thr_id;
   HBOOST_ASSERT(ipcdetail::equal_systemwide_thread_id(thr_id, old_id));
   --m_nLockCount;
   if(!m_nLockCount){
...
#endif   //HBOOST_INTERPROCESS_DETAIL_SPIN_RECURSIVE_MUTEX_HPP
