...
#ifndef HBOOST_INTERPROCESS_DETAIL_MANAGED_MEMORY_IMPL_HPP
#define HBOOST_INTERPROCESS_DETAIL_MANAGED_MEMORY_IMPL_HPP
...
#include <hboost/interprocess/segment_manager.hpp>
...
class basic_managed_memory_impl
{
   ...
   bool  create_impl   (void *addr, size_type size)
   {
      ...
      HBOOST_TRY{
         ...
         mp_header       = ::new(addr, hboost_container_new_t()) segment_manager(size);
      }
      HBOOST_CATCH(...){
         return false;
      }
   ...
   //!memory is available throws a hboost::interprocess::bad_alloc exception
   ...
   //!
   //!-> Throws hboost::interprocess::bad_alloc if there is no available memory
   //!
...
#endif   //HBOOST_INTERPROCESS_DETAIL_MANAGED_MEMORY_IMPL_HPP
