...
#ifndef HBOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
#define HBOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
...
#endif   //HBOOST_INTERPROCESS_XSI_SHARED_MEMORY_OBJECTS
...
{
   //Non-copyable
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(managed_open_or_create_impl)
   ...
   static const std::size_t
      ManagedOpenOrCreateUserOffset =
         ct_rounded_size
            < sizeof(hboost::uint32_t)
            , MemAlignment ? (MemAlignment) :
               (::hboost::container::container_detail::alignment_of
   ...
   managed_open_or_create_impl(HBOOST_RV_REF(managed_open_or_create_impl) moved)
   {  this->swap(moved);   }

   managed_open_or_create_impl &operator=(HBOOST_RV_REF(managed_open_or_create_impl) moved)
   {
      managed_open_or_create_impl tmp(hboost::move(moved));
      this->swap(tmp);
      return *this;
   }
       ...
       ConstructFunc construct_func)
   {
      ...
      if(created){
         try{
            ...
            hboost::uint32_t *patomic_word = 0;  //avoid gcc warning
            patomic_word = static_cast<hboost::uint32_t*>(region.get_address());
            hboost::uint32_t previous = atomic_cas32(patomic_word, InitializingSegment, UninitializedSegment);
         ...
         hboost::uint32_t *patomic_word = static_cast<hboost::uint32_t*>(region.get_address());
         hboost::uint32_t value = atomic_read32(patomic_word);
      ...
      if(StoreDevice){
         this->DevHolder::get_device() = hboost::move(dev);
      }
   }
...
#endif   //#ifndef HBOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
