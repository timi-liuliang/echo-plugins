...
#ifndef HBOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
#define HBOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
...
class file_wrapper
{
   ...
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(file_wrapper)
   ...
   //!Does not throw
   file_wrapper(HBOOST_RV_REF(file_wrapper) moved)
   ...
   //!After the call, "moved" does not represent any file.
   //!Does not throw
   file_wrapper &operator=(HBOOST_RV_REF(file_wrapper) moved)
   {
      file_wrapper tmp(hboost::move(moved));
      this->swap(tmp);
      return *this;
   }
...
#endif   //HBOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
