...
#ifndef HBOOST_INTERPROCESS_WIN32_API_HPP
#define HBOOST_INTERPROCESS_WIN32_API_HPP
...
#include <hboost/date_time/filetime_functions.hpp>
#include <cstddef>
#include <cstring>
#include <cstdlib>
#include <cstdio>
...
#  if defined(HBOOST_INTERPROCESS_BOOTSTAMP_IS_LASTBOOTUPTIME)
#  include <wbemidl.h>
#  include <objbase.h>
#  endif
...
//////////////////////////////////////////////////////////////////////////////
//
// Declaration of Windows structures or typedefs if HBOOST_USE_WINDOWS_H is used
//
//////////////////////////////////////////////////////////////////////////////

//Ignore -pedantic errors here (anonymous structs, etc.)
#if defined(HBOOST_GCC) && (HBOOST_GCC >= 40600)
...
#else    //#ifndef HBOOST_USE_WINDOWS_H
...
union system_timeofday_information
{
   struct data_t
   {
      ...
      ::hboost::ulong_long_type ullBootTimeBias;
      ::hboost::ulong_long_type ullSleepTimeBias;
   } data;
...
//it can define HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL to one of these:
//
// COINIT_APARTMENTTHREADED_BIPC
// COINIT_MULTITHREADED_BIPC
// COINIT_DISABLE_OLE1DDE_BIPC
// COINIT_SPEED_OVER_MEMORY_BIPC
#if !defined(HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL)
   #define HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL COINIT_APARTMENTTHREADED_BIPC
#elif (HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL != COINIT_APARTMENTTHREADED_BIPC) &&\
      (HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL != COINIT_MULTITHREADED_BIPC)     &&\
      (HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL != COINIT_DISABLE_OLE1DDE_BIPC)   &&\
      (HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL != COINIT_SPEED_OVER_MEMORY_BIPC)
   #error "Wrong value for HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL macro"
#endif
...
inline void * create_file_mapping (void * handle, unsigned long access, ::hboost::ulong_long_type file_offset, const char * name, interprocess_security_attributes *psec)
{
   const unsigned long high_size(file_offset >> 32), low_size((hboost::uint32_t)file_offset);
...
inline void *map_view_of_file_ex(void *handle, unsigned long file_access, ::hboost::ulong_long_type offset, std::size_t numbytes, void *base_addr)
{
   const unsigned long offset_low  = (unsigned long)(offset & ((::hboost::ulong_long_type)0xFFFFFFFF));
...
{  return HBOOST_INTERLOCKED_INCREMENT(const_cast<long*>(addr));  }

inline long interlocked_decrement(long volatile *addr)
{  return HBOOST_INTERLOCKED_DECREMENT(const_cast<long*>(addr));  }

inline long interlocked_compare_exchange(long volatile *addr, long val1, long val2)
{  return HBOOST_INTERLOCKED_COMPARE_EXCHANGE(const_cast<long*>(addr), val1, val2);  }

inline long interlocked_exchange_add(long volatile* addend, long value)
{  return HBOOST_INTERLOCKED_EXCHANGE_ADD(const_cast<long*>(addend), value);  }

inline long interlocked_exchange(long volatile* addend, long value)
{  return HBOOST_INTERLOCKED_EXCHANGE(const_cast<long*>(addend), value);  }
...
struct function_address_holder
{
   ...
   {
      HBOOST_ASSERT(id < (unsigned int)NumModule);
   ...
   {
      HBOOST_ASSERT(id < (unsigned int)NumFunction);
...
class nt_query_mem_deleter
{
   ...
   static const std::size_t rename_suffix =
      (SystemTimeOfDayInfoLength + sizeof(unsigned long) + sizeof(hboost::uint32_t))*2;
...
inline bool unlink_file(const char *filename)
{
   ...
   try{
      ...
      handle_closer h_closer(fh);
      {
         ...
         static volatile hboost::uint32_t u32_count = 0;
         interlocked_decrement(reinterpret_cast<volatile long*>(&u32_count));
         buffer_to_wide_str(const_cast<const hboost::uint32_t *>(&u32_count), sizeof(hboost::uint32_t), &pfri->FileName[filename_string_length]);
         filename_string_length += sizeof(hboost::uint32_t)*2;
...
inline bool get_wmi_class_attribute( std::wstring& strValue, const wchar_t *wmi_class, const wchar_t *wmi_class_var)
{
   ...
   //
   //See HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL definition if you need to change the
   //default value of this macro in your application
   long co_init_ret = CoInitializeEx(0, HBOOST_INTERPROCESS_WINDOWS_COINIT_MODEL);
...
#endif //#ifdef HBOOST_INTERPROCESS_WIN32_API_HPP
