...
#ifndef HBOOST_INTERPROCESS_WINDOWS_INTERMODULE_SINGLETON_HPP
#define HBOOST_INTERPROCESS_WINDOWS_INTERMODULE_SINGLETON_HPP
...
#include <hboost/interprocess/detail/intermodule_singleton_common.hpp>
...
{
   typedef hboost::container::map<hboost::container::string, ref_count_ptr> map_type;

   public:
   windows_semaphore_based_map()
   {
      map_type *m = new map_type;
      hboost::uint32_t initial_count = 0;
      hboost::uint32_t max_count = 0;
      ...
      if(sizeof(void*) == sizeof(hboost::uint32_t)){
         ...
         {
            void *addr;
            hboost::uint32_t addr_uint32;
         } caster;
         caster.addr = m;
         //memory is at least 4 byte aligned in windows
         HBOOST_ASSERT((caster.addr_uint32 & hboost::uint32_t(3)) == 0);
         max_count = caster.addr_uint32 >> 2;
      }
      else if(sizeof(void*) == sizeof(hboost::uint64_t)){
         //Relying in UB with a cast through union, but all known windows compilers
         //accept this (C11 accepts this).
         union caster_union
         {
            void *addr;
            hboost::uint64_t addr_uint64;
         } caster;
         caster.addr = m;
         ...
         caster.addr = m;
         HBOOST_ASSERT((caster.addr_uint64 & hboost::uint64_t(3)) == 0);
         max_count = hboost::uint32_t(caster.addr_uint64 >> 32);
         initial_count = hboost::uint32_t(caster.addr_uint64);
         initial_count = initial_count/4;
         //Make sure top two bits are zero
         HBOOST_ASSERT((max_count & hboost::uint32_t(0xC0000000)) == 0);
         //Set quasi-top bit
         max_count |= hboost::uint32_t(0x40000000);
      }
      bool created = false;
      ...
      scoped_lock<winapi_mutex_wrapper> lck(m_mtx_lock);
      {
         ...
         if(!created){
            ...
            HBOOST_ASSERT(&get_map_unlocked() == m);
         }
         m_sem_count.post();
      }
   }
   ...
   map_type &get_map_unlocked()
   {
      if(sizeof(void*) == sizeof(hboost::uint32_t)){
         ...
         hboost::uint32_t max_count(m_sem_map.limit()), initial_count(m_sem_map.value());
         //Clear quasi-top bit
         max_count &= hboost::uint32_t(0xBFFFFFFF);
         caster.addr_uint64 = max_count;
         caster.addr_uint64 =  caster.addr_uint64 << 32;
         caster.addr_uint64 |= hboost::uint64_t(initial_count) << 2;
         return *static_cast<map_type*>(caster.addr);
      }
   }
   ...
   ref_count_ptr *find(const char *name)
   {
      ...
      map_type::iterator it = map.find(hboost::container::string(name));
      if(it != map.end()){
         return &it->second;
      }
      else{
   ...
   ref_count_ptr * insert(const char *name, const ref_count_ptr &ref)
   {
      scoped_lock<winapi_mutex_wrapper> lck(m_mtx_lock);
      map_type &map = this->get_map_unlocked();
      map_type::iterator it = map.insert(map_type::value_type(hboost::container::string(name), ref)).first;
      return &it->second;
   }
   ...
   bool erase(const char *name)
   {
      scoped_lock<winapi_mutex_wrapper> lck(m_mtx_lock);
      map_type &map = this->get_map_unlocked();
      return map.erase(hboost::container::string(name)) != 0;
   }
...
#endif   //#ifndef HBOOST_INTERPROCESS_WINDOWS_INTERMODULE_SINGLETON_HPP
