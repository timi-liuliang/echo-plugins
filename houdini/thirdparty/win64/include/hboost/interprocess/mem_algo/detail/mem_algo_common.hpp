...
#ifndef HBOOST_INTERPROCESS_DETAIL_MEM_ALGO_COMMON_HPP
#define HBOOST_INTERPROCESS_DETAIL_MEM_ALGO_COMMON_HPP
...
class basic_multiallocation_chain
   : public hboost::container::container_detail::
      basic_multiallocation_chain<VoidPointer>
{
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(basic_multiallocation_chain)
   ...
   basic_multiallocation_chain(HBOOST_RV_REF(basic_multiallocation_chain) other)
      :  base_t(::hboost::move(static_cast<base_t&>(other)))
   {}

   basic_multiallocation_chain& operator=(HBOOST_RV_REF(basic_multiallocation_chain) other)
   {
      this->base_t::operator=(::hboost::move(static_cast<base_t&>(other)));
      return *this;
   }

   void *pop_front()
   {
      return hboost::interprocess::ipcdetail::to_raw_pointer(this->base_t::pop_front());
   }
};
...
class memory_algorithm_common
{
   ...
   {
      (void)uint_ptr;
      HBOOST_ASSERT(uint_ptr % Alignment == 0);
   }
      ...
      size_type &lcm_out, size_type &needs_backwards_lcmed_out)
   {
      ...
      if((backwards_multiple & (backwards_multiple-1)) == 0){
         ...
         HBOOST_ASSERT((needs_backwards % backwards_multiple) == 0);
      ...
      //Check if it's multiple of alignment
         ...
         HBOOST_ASSERT((needs_backwards_lcmed & (Alignment - 1u)) == 0);
         lcm_out = lcm_val;
         needs_backwards_lcmed_out = needs_backwards_lcmed;
         return true;
      }
      //Check if it's multiple of the half of the alignmment
         ...
         HBOOST_ASSERT((needs_backwards_lcmed % lcm_val) == 0);
         lcm_out = lcm_val;
         needs_backwards_lcmed_out = needs_backwards_lcmed;
         return true;
      }
      ...
      (MemoryAlgorithm *memory_algo, size_type nbytes, size_type alignment)
   {
      ...
      if ((alignment & (alignment - size_type(1u))) != 0){
         ...
         HBOOST_ASSERT((alignment & (alignment - size_type(1u))) == 0);
         return 0;
      }
      ...
      if(alignment <= Alignment){
         ...
         return memory_algo->priv_allocate
            (hboost::interprocess::allocate_new, nbytes, real_size, ignore_reuse);
      }
      ...
      real_size = request;
      void *ignore_reuse = 0;
      void *buffer = memory_algo->priv_allocate(hboost::interprocess::allocate_new, request, real_size, ignore_reuse);
      if(!buffer){
         return 0;
      }
         ...
         if(old_size >= (first_min_units + MinBlockUnits)){
            ...
            HBOOST_ASSERT(second->m_size >= MinBlockUnits);
      ...
      HBOOST_ASSERT(pos <= (reinterpret_cast<char*>(first) + first->m_size*Alignment));
      HBOOST_ASSERT(first->m_size >= 2*MinBlockUnits);
      HBOOST_ASSERT((pos + MinBlockUnits*Alignment - AllocatedCtrlBytes + nbytes*Alignment/Alignment) <=
      ...
      if((old_size - first->m_size) >= (second_min_units + MinBlockUnits)){
         ...
         HBOOST_ASSERT(third->m_size >= MinBlockUnits);
         memory_algo->priv_mark_new_allocated_block(second);
      ...
      ,const size_type max_size, size_type &received_size)
   {
      ...
      HBOOST_ASSERT(memory_algo->priv_is_allocated_block(block));
      ...
      if(total_request_units && !multiplication_overflows(total_request_units, Alignment)){
         ...
         while(low_idx < n_elements){
            ...
            void *ret = memory_algo->priv_allocate
               (hboost::interprocess::allocate_new, min_allocation, received_size, ignore_reuse);
            if(!ret){
               break;
            }
            ...
            while(total_used_units < received_units){
               ...
               : max_value(memory_algo->priv_get_total_units(elem_sizes[low_idx+1]*sizeof_element), ptr_size_units))
                  ...
                  if((received_units - total_used_units) >= (elem_units + MemoryAlgorithm::BlockCtrlUnits)){
                     ...
                     HBOOST_ASSERT(shrink_ok);
                     //Some sanity checks
                     HBOOST_ASSERT(shrunk_request == shrunk_received);
                     HBOOST_ASSERT(elem_units == ((shrunk_request-UsableByPreviousChunk)/Alignment + AllocatedCtrlUnits));
                     //"new_block->m_size" must have been reduced to elem_units by "shrink"
                     HBOOST_ASSERT(new_block->m_size == elem_units);
               ...
               HBOOST_ASSERT((new_block->m_size*Alignment - AllocatedCtrlUnits) >= sizeof(void_pointer));
               void_pointer p = ::new(memory_algo->priv_get_user_buffer(new_block), hboost_container_new_t())void_pointer(0);
               chain.push_back(p);
               ++low_idx;
            }
            //Sanity check
            HBOOST_ASSERT(total_used_units == received_units);
         }
...
#endif   //#ifndef HBOOST_INTERPROCESS_DETAIL_MEM_ALGO_COMMON_HPP
