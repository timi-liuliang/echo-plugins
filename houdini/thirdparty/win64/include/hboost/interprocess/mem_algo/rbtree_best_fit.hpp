...
#ifndef HBOOST_INTERPROCESS_MEM_ALGO_RBTREE_BEST_FIT_HPP
#define HBOOST_INTERPROCESS_MEM_ALGO_RBTREE_BEST_FIT_HPP
...
// interprocess
#include <hboost/interprocess/containers/allocation_type.hpp>
#include <hboost/interprocess/exceptions.hpp>
#include <hboost/interprocess/interprocess_fwd.hpp>
#include <hboost/interprocess/mem_algo/detail/mem_algo_common.hpp>
#include <hboost/interprocess/offset_ptr.hpp>
#include <hboost/interprocess/sync/scoped_lock.hpp>
// interprocess/detail
#include <hboost/interprocess/detail/min_max.hpp>
#include <hboost/interprocess/detail/math_functions.hpp>
...
// move/detail
#include <hboost/move/detail/type_traits.hpp> //make_unsigned, alignment_of
// intrusive
...
//#define HBOOST_INTERPROCESS_RBTREE_BEST_FIT_ABI_V1_HPP
...
class rbtree_best_fit
{
   ...
   typedef typename hboost::intrusive::pointer_traits<char_ptr>::difference_type difference_type;
   typedef typename hboost::container::container_detail::make_unsigned<difference_type>::type     size_type;
   ...
   {
      //-----------------------
      hboost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
   ...
   template<class T>
   T * allocation_command  (hboost::interprocess::allocation_type command, size_type limit_size,
                           size_type &prefer_in_recvd_out_size, T *&reuse);

   void * raw_allocation_command (hboost::interprocess::allocation_type command,   size_type limit_object,
   ...
   void* priv_allocation_command(hboost::interprocess::allocation_type command,   size_type limit_size,
                        size_type &prefer_in_recvd_out_size, void *&reuse_ptr, size_type sizeof_object);


   //!Real allocation algorithm with min allocation option
   void * priv_allocate( hboost::interprocess::allocation_type command
   ...
   void* priv_expand_both_sides(hboost::interprocess::allocation_type command
                               ,size_type min_size
                               ,size_type &prefer_in_recvd_out_size
                               ,void *reuse_ptr
   ...
   static const size_type Alignment = !MemAlignment
      ? size_type(::hboost::container::container_detail::alignment_of
                  < ::hboost::container::container_detail::max_align_t>::value)
      : size_type(MemAlignment)
      ;

   private:
   //Due to embedded bits in size, Alignment must be at least 4
   HBOOST_STATIC_ASSERT((Alignment >= 4));
   //Due to rbtree size optimizations, Alignment must have at least pointer alignment
   HBOOST_STATIC_ASSERT((Alignment >= ::hboost::container::container_detail::alignment_of<void_pointer>::value));
   ...
   HBOOST_STATIC_ASSERT((0 == (Alignment & (Alignment - size_type(1u)))));
   #endif   //#ifndef HBOOST_INTERPROCESS_DOXYGEN_INVOKED
   public:
   ...
   priv_add_segment(void *addr, size_type segment_size)
{
   ...
   //Check size
   HBOOST_ASSERT(segment_size >= (BlockCtrlBytes + EndCtrlBlockBytes));

   //Initialize the first big block and the "end" node
   block_ctrl *first_big_block = ::new(addr, hboost_container_new_t())block_ctrl;
   first_big_block->m_size = segment_size/Alignment - EndCtrlBlockUnits;
   HBOOST_ASSERT(first_big_block->m_size >= BlockCtrlUnits);
   ...
   #ifdef HBOOST_INTERPROCESS_RBTREE_BEST_FIT_ABI_V1_HPP
   ...
   HBOOST_ASSERT(priv_next_block(first_big_block) == end_block);
   HBOOST_ASSERT(priv_prev_block(end_block) == first_big_block);
   HBOOST_ASSERT(priv_first_block() == first_big_block);
   HBOOST_ASSERT(priv_end_block() == end_block);
   ...
   HBOOST_ASSERT(static_cast<void*>(static_cast<SizeHolder*>(first_big_block))
   ...
   rbtree_best_fit(size_type segment_size, size_type extra_hdr_bytes)
{
   ...
   HBOOST_ASSERT(get_min_size(extra_hdr_bytes) <= segment_size);
...
//   HBOOST_ASSERT(m_header.m_allocated == 0);
//   HBOOST_ASSERT(m_header.m_root.m_next->m_next == block_ctrl_ptr(&m_header.m_root));
}
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::grow(size_type extra_size)
{
   ...
   HBOOST_ASSERT(new_end_block == priv_end_block());
   ...
   HBOOST_ASSERT(new_block->m_size >= BlockCtrlUnits);
   priv_mark_as_allocated_block(new_block);
   HBOOST_ASSERT(priv_next_block(new_block) == new_end_block);
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::shrink_to_fit()
{
   ...
   if(priv_next_block(first_block) == old_end_block){
      ...
      void *ignore_reuse = 0;
      unique_buffer = priv_allocate(hboost::interprocess::allocate_new, 0, ignore_recvd, ignore_reuse);
      ...
      HBOOST_ASSERT(priv_is_allocated_block(unique_block));
      algo_impl_t::assert_alignment(unique_block);
      last_block = priv_next_block(unique_block);
      HBOOST_ASSERT(!priv_is_allocated_block(last_block));
   ...
   HBOOST_ASSERT(new_end_block->m_size == (old_end_block_size - last_block_size));

   //Update managed buffer's size
   m_header.m_size = shrunk_border_offset;
   HBOOST_ASSERT(priv_end_block() == new_end_block);
   if(unique_buffer)
   ...
   allocate(size_type nbytes)
{
   ...
   void *ignore_reuse = 0;
   return priv_allocate(hboost::interprocess::allocate_new, nbytes, ignore_recvd, ignore_reuse);
}
...
inline T* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   allocation_command  (hboost::interprocess::allocation_type command,   size_type limit_size,
                        size_type &prefer_in_recvd_out_size, T *&reuse)
{
   ...
   HBOOST_ASSERT(0 == ((std::size_t)ret % ::hboost::container::container_detail::alignment_of<T>::value));
   return static_cast<T*>(ret);
}

template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   raw_allocation_command  (hboost::interprocess::allocation_type command,   size_type limit_objects,
                        size_type &prefer_in_recvd_out_objects, void *&reuse_ptr, size_type sizeof_object)
{
   ...
   if(command & hboost::interprocess::try_shrink_in_place){
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_allocation_command (hboost::interprocess::allocation_type command,   size_type limit_size,
                       size_type &prefer_in_recvd_out_size,
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_expand_both_sides(hboost::interprocess::allocation_type command
                         ,size_type min_size
                         ,size_type &prefer_in_recvd_out_size
                         ,void *reuse_ptr
                         ,bool only_preferred_backwards
                         ,size_type backwards_multiple)
{
   ...
   if(command & hboost::interprocess::expand_fwd){
   ...
   if(backwards_multiple){
      HBOOST_ASSERT(0 == (min_size       % backwards_multiple));
      HBOOST_ASSERT(0 == (preferred_size % backwards_multiple));
   }

   if(command & hboost::interprocess::expand_bwd){
      //Obtain the real size of the block
      ...
      HBOOST_ASSERT(!priv_is_allocated_block(prev_block));

      //Some sanity checks
      HBOOST_ASSERT(prev_block->m_size == reuse->m_prev_size);
      ...
      if(size_type(prev_block->m_size*Alignment) >= needs_backwards_aligned){
         ...
         if(command & hboost::interprocess::expand_fwd){
            ...
            HBOOST_ASSERT(prefer_in_recvd_out_size == received_size2);
         }
         ...
         if(prev_block->m_size >= (needs_backwards_aligned/Alignment + BlockCtrlUnits)){
            ...
            HBOOST_ASSERT(prev_block->m_size >= BlockCtrlUnits);
            ...
            HBOOST_ASSERT((static_cast<char*>(reuse_ptr) - static_cast<char*>(user_ptr)) % backwards_multiple == 0);
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
void * rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_allocate(hboost::interprocess::allocation_type command
...
{
   size_type const preferred_size = prefer_in_recvd_out_size;
   if(command & hboost::interprocess::shrink_in_place){
      if(!reuse_ptr)  return static_cast<void*>(0);
      bool success =
   ...
   if(reuse_ptr && (command & (hboost::interprocess::expand_fwd | hboost::interprocess::expand_bwd))){
   ...
   if(command & hboost::interprocess::allocate_new){
   ...
   priv_expand (void *ptr, const size_type min_size, size_type &prefer_in_recvd_out_size)
{
   ...
   HBOOST_ASSERT(priv_is_allocated_block(block));
   ...
   HBOOST_ASSERT(min_user_units <= preferred_user_units);
   ...
   if((merged_units - intended_units) >=  BlockCtrlUnits){
      ...
      HBOOST_ASSERT(next_block->m_size == priv_next_block(next_block)->m_prev_size);
      ...
      block_ctrl *rem_block = ::new(reinterpret_cast<block_ctrl*>
                     (reinterpret_cast<char*>(block) + intended_units*Alignment), hboost_container_new_t())block_ctrl;
      rem_block->m_size  = rem_units;
      algo_impl_t::assert_alignment(rem_block);
      HBOOST_ASSERT(rem_block->m_size >= BlockCtrlUnits);
      ...
      HBOOST_ASSERT(block->m_size >= BlockCtrlUnits);
...
{
   HBOOST_ASSERT(!ptr->m_prev_allocated);
      ...
      (typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *first_segment_block)
{
   ...
   HBOOST_ASSERT(first_segment_block->m_prev_allocated);
   block_ctrl *end_block = reinterpret_cast<block_ctrl *>
      (reinterpret_cast<char*>(first_segment_block) + first_segment_block->m_prev_size*Alignment);
   (void)end_block;
   HBOOST_ASSERT(end_block->m_allocated == 1);
   HBOOST_ASSERT(end_block->m_size == first_segment_block->m_prev_size);
   HBOOST_ASSERT(end_block > first_segment_block);
   return end_block;
}
      ...
      (typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *end_segment_block)
{
   //The first block's logic is different from the rest of blocks: stores in m_prev_size the absolute
   //distance with the end block
   HBOOST_ASSERT(end_segment_block->m_allocated);
   ...
   (void)first_block;
   HBOOST_ASSERT(first_block->m_prev_allocated == 1);
   HBOOST_ASSERT(first_block->m_prev_size == end_segment_block->m_size);
   HBOOST_ASSERT(end_segment_block > first_block);
   return first_block;
}
      ...
      (typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *block)
{
   ...
   if(block != priv_end_block()){
      ...
      HBOOST_ASSERT(allocated == next_block_prev_allocated);
   }
   #endif
   return allocated;
}
      ...
      (typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *block)
{
   if(block->m_prev_allocated){
      ...
      if(block != priv_first_block()){
         ...
         (void)prev;
         HBOOST_ASSERT(!prev->m_allocated);
         HBOOST_ASSERT(prev->m_size == block->m_prev_size);
      }
      #endif
      return false;
   }
   ...
   ,size_type &received_size)
{
   ...
   if (block->m_size >= upper_nunits){
      ...
      //This is the remaining block
      block_ctrl *rem_block = ::new(reinterpret_cast<block_ctrl*>
                     (reinterpret_cast<char*>(block) + Alignment*nunits), hboost_container_new_t())block_ctrl;
      algo_impl_t::assert_alignment(rem_block);
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_deallocate(void* addr)
{
   ...
   HBOOST_ASSERT(m_header.m_allocated >= block_old_size);
   ...
   if(merge_with_prev || merge_with_next){
      ...
      if(merge_with_prev){
         ...
         HBOOST_ASSERT(block_to_insert->m_size >= BlockCtrlUnits);
      }
...
#endif   //#ifndef HBOOST_INTERPROCESS_MEM_ALGO_RBTREE_BEST_FIT_HPP
