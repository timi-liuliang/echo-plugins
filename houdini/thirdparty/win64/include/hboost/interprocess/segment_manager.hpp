...
#ifndef HBOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
#define HBOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
...
#include <hboost/interprocess/detail/transform_iterator.hpp>

#include <hboost/interprocess/detail/mpl.hpp>
#include <hboost/interprocess/detail/nothrow.hpp>
#include <hboost/interprocess/detail/segment_manager_helper.hpp>
#include <hboost/interprocess/detail/named_proxy.hpp>
#include <hboost/interprocess/detail/utilities.hpp>
#include <hboost/interprocess/offset_ptr.hpp>
#include <hboost/interprocess/indexes/iset_index.hpp>
#include <hboost/interprocess/exceptions.hpp>
#include <hboost/interprocess/allocators/allocator.hpp>
#include <hboost/interprocess/smart_ptr/deleter.hpp>
   ...
   :  private MemoryAlgorithm
{
   ...
   {
      HBOOST_ASSERT((sizeof(segment_manager_base<MemoryAlgorithm>) == sizeof(MemoryAlgorithm)));
   }
   ...
   //!Allocates nbytes bytes. Throws hboost::interprocess::bad_alloc
   //!on failure
                           ...
                           size_type &prefer_in_recvd_out_size, T *&reuse)
   {
      T *ret = MemoryAlgorithm::allocation_command
         (command | hboost::interprocess::nothrow_allocation, limit_size, prefer_in_recvd_out_size, reuse);
      if(!(command & hboost::interprocess::nothrow_allocation) && !ret)
         throw bad_alloc();
      return ret;
   }

   void *raw_allocation_command  (hboost::interprocess::allocation_type command,   size_type limit_objects,
                           size_type &prefer_in_recvd_out_size, void *&reuse, size_type sizeof_object = 1)
   {
      void *ret = MemoryAlgorithm::raw_allocation_command
         ( command | hboost::interprocess::nothrow_allocation, limit_objects,
      ...
      (size_type num, bool dothrow, ipcdetail::in_place_interface &table)
   {
      ...
      block_header_t * hdr = ::new(ptr_struct, hboost_container_new_t()) block_header_t(block_info);
   ...
   :  public segment_manager_base<MemoryAlgorithm>
{
      ...
      ,  m_header(static_cast<segment_manager_base_t*>(get_this_pointer()))
   {
      ...
      HBOOST_ASSERT( this_addr == segm_addr);
   }
   ...
   {
      HBOOST_ASSERT(!name.is_anonymous());
   ...
   //!Can throw hboost::interprocess::bad_alloc if there is no enough memory.
   ...
   struct allocator
   {
      typedef hboost::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef hboost::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      hboost::interprocess::allocation_type type = ctrl_data->alloc_type();
      if(type == anonymous_type){
         HBOOST_ASSERT((type == anonymous_type && ctrl_data->m_num_char == 0) ||
      ...
      //Sanity checks
      HBOOST_ASSERT(ctrl_data->sizeof_char() == sizeof(CharType));
      HBOOST_ASSERT(ctrl_data->m_num_char == std::char_traits<CharType>::length(name));
      return name;
   }
   ...
   {
      //Get header
      HBOOST_ASSERT((ctrl_data->value_bytes() %sizeofvalue) == 0);
   ...
   {
      //Get header
      HBOOST_ASSERT((instance_type)ctrl_data->alloc_type() < max_allocation_type);
       ...
       size_type &length, ipcdetail::true_ is_intrusive, bool use_lock)
   {
      ...
      if(it != index.end()){
         ...
         //Sanity check
         HBOOST_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
         HBOOST_ASSERT(ctrl_data->sizeof_char() == sizeof(CharT));
                                   ...
                                   ipcdetail::in_place_interface &table, ipcdetail::true_ is_intrusive_index)
   {
      ...
      //Sanity check
      HBOOST_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
      HBOOST_ASSERT(sizeof(CharT) == ctrl_data->sizeof_char());
      ...
      ipcdetail::in_place_interface &table)
   {
      ...
      //Sanity check
      HBOOST_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
      HBOOST_ASSERT(static_cast<void*>(stored_name) == static_cast<void*>(ctrl_data->template name<CharT>()));
      HBOOST_ASSERT(sizeof(CharT) == ctrl_data->sizeof_char());
      ...
      IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index, ipcdetail::true_ is_intrusive)
   {
     ...
     std::size_t namelen  = std::char_traits<CharT>::length(name);
      ...
      intrusive_value_type * intrusive_hdr = ::new(buffer_ptr, hboost_container_new_t()) intrusive_value_type();
      block_header_t * hdr = ::new(intrusive_hdr->get_block_header(), hboost_container_new_t())block_header_t(block_info);
      void *ptr = 0; //avoid gcc warning
      ptr = hdr->value();
      ...
      IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index, ipcdetail::false_ is_intrusive)
   {
      ...
      if(is_node_index_t::value){
         ...
         index_it *idr = ::new(buffer_ptr, hboost_container_new_t()) index_it(it);
      ...
      hdr = ::new(hdr, hboost_container_new_t())block_header_t(block_info);
      void *ptr = 0; //avoid gcc warning
      ptr = hdr->value();
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(hboost::move(local));
   }
...
#endif //#ifndef HBOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
