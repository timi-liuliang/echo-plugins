...
//  22 Jun 00  Change _MSC_VER to HBOOST_MSVC so other compilers are not
...
#ifndef HBOOST_RATIONAL_HPP
#define HBOOST_RATIONAL_HPP

#include <hboost/config.hpp>      // for HBOOST_NO_STDC_NAMESPACE, HBOOST_MSVC, etc
...
#include <hboost/operators.hpp>   // for hboost::addable etc
#include <cstdlib>               // for std::abs
#include <hboost/call_traits.hpp> // for hboost::call_traits
#include <hboost/detail/workaround.hpp> // for HBOOST_WORKAROUND
#include <hboost/assert.hpp>      // for HBOOST_ASSERT
#include <hboost/integer/common_factor_rt.hpp> // for hboost::integer::gcd, lcm
#include <limits>                // for std::numeric_limits
#include <hboost/static_assert.hpp>  // for HBOOST_STATIC_ASSERT
#include <hboost/throw_exception.hpp>

// Control whether depreciated GCD and LCM functions are included (default: yes)
#ifndef HBOOST_CONTROL_RATIONAL_HAS_GCD
#define HBOOST_CONTROL_RATIONAL_HAS_GCD  1
#endif

namespace hboost {

#if HBOOST_CONTROL_RATIONAL_HAS_GCD
...
#endif  // HBOOST_CONTROL_RATIONAL_HAS_GCD
    ...
    > > > > > > > > > > > > > > > >
{
    ...
    HBOOST_STATIC_ASSERT( ::std::numeric_limits<IntType>::is_specialized );

    // Helper types
    typedef typename hboost::call_traits<IntType>::param_type param_type;
...
#ifndef HBOOST_NO_MEMBER_TEMPLATES
    template < typename NewType >
    HBOOST_CONSTEXPR explicit
       ...
       (HBOOST_THROW_EXCEPTION(bad_rational("bad rational: denormalized conversion")), 0)){}
#endif
...
rational<IntType>& rational<IntType>::operator/= (const rational<IntType>& r)
{
    ...
    if (r_num == zero)
        HBOOST_THROW_EXCEPTION(bad_rational());
    if (num == zero)
        return *this;
...
rational<IntType>::operator/= (param_type i)
{
    ...
    if(i == zero) HBOOST_THROW_EXCEPTION(bad_rational());
...
bool rational<IntType>::operator< (const rational<IntType>& r) const
{
    ...
    HBOOST_ASSERT( this->den > zero );
    HBOOST_ASSERT( r.den > zero );
...
void rational<IntType>::normalize()
{
    ...
    if (den < zero)
       HBOOST_THROW_EXCEPTION(bad_rational("bad rational: non-zero singular denominator"));

    HBOOST_ASSERT( this->test_invariant() );
}
