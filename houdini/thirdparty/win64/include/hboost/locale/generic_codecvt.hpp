...
//
#ifndef HBOOST_LOCALE_GENERIC_CODECVT_HPP
#define HBOOST_LOCALE_GENERIC_CODECVT_HPP

#include <hboost/locale/utf.hpp>
...
#ifndef HBOOST_LOCALE_DOXYGEN
//
// Make sure that mbstate can keep 16 bit of UTF-16 sequence
//
HBOOST_STATIC_ASSERT(sizeof(std::mbstate_t)>=2);
#endif
...
#define HBOOST_LOCALE_DO_LENGTH_MBSTATE_CONST
#endif
...
/// class latin1_codecvt :hboost::locale::generic_codecvt<CharType,latin1_codecvt<CharType> > 
/// {
/// public:
///    
///     /* Standard codecvt constructor */ 
///     latin1_codecvt(size_t refs = 0) : hboost::locale::generic_codecvt<CharType,latin1_codecvt<CharType> >(refs) 
///     {
///     }
///
...
///     }
///
///     hboost::locale::utf::code_point to_unicode(state_type &,char const *&begin,char const *end) const
///     {
///        if(begin == end)
///           return hboost::locale::utf::incomplete;
///        return *begin++; 
///     }
///
///     hboost::locale::utf::code_point from_unicode(state_type &,hboost::locale::utf::code_point u,char *begin,char const *end) const
///     {
///        if(u >= 256)
///           return hboost::locale::utf::illegal;
///        if(begin == end)
///           return hboost::locale::utf::incomplete;
///        *begin = u;
///        return 1; 
...
/// \code
/// template<typename CharType>
/// class icu_codecvt :hboost::locale::generic_codecvt<CharType,icu_codecvt<CharType> > 
/// {
/// public:
///    
///     /* Standard codecvt constructor */ 
///     icu_codecvt(std::string const &name,refs = 0) : 
///         hboost::locale::generic_codecvt<CharType,latin1_codecvt<CharType> >(refs)
///     { ... }
///
///     /* State is unused but required by generic_codecvt */
...
///     }
///     
///     hboost::locale::utf::code_point to_unicode(state_type &ptr,char const *&begin,char const *end) const
///     {
///         UErrorCode err = U_ZERO_ERROR;
///         hboost::locale::utf::code_point cp = ucnv_getNextUChar(ptr.get(),&begin,end,&err);
///         ...
///     }
///     ...
/// };
...
protected:
    ...
    {
        hboost::uint16_t &state = *reinterpret_cast<hboost::uint16_t *>(&s);
#ifdef DEBUG_CODECVT            
    ...
    do_length(  std::mbstate_t 
    #ifdef HBOOST_LOCALE_DO_LENGTH_MBSTATE_CONST
            const   
    #endif
            &std_state,
            char const *from,
            ...
            size_t max) const
    {
        #ifndef HBOOST_LOCALE_DO_LENGTH_MBSTATE_CONST
        char const *save_from = from;
        hboost::uint16_t &state = *reinterpret_cast<hboost::uint16_t *>(&std_state);
        #else
        size_t save_max = max;
        hboost::uint16_t state = *reinterpret_cast<hboost::uint16_t const *>(&std_state);
        #endif
        ...
        while(max > 0 && from < from_end){
            ...
            hboost::uint32_t ch=implementation().to_unicode(cvt_state,from,from_end);
            if(ch==hboost::locale::utf::incomplete || ch==hboost::locale::utf::illegal) {
                from = prev_from;
                break;
            }
            max --;
            ...
            uchar *&to_next) const
    {
        ...
        while(to < to_end && from < from_end)
        {
            ...
            if(ch==hboost::locale::utf::illegal) {
                from = from_saved;
                r=std::codecvt_base::error;
                break;
            }
            if(ch==hboost::locale::utf::incomplete) {
                from = from_saved;
            ...
            if(ch <= 0xFFFF) {
                *to++=ch;
                ...
                ch-=0x10000;
                hboost::uint16_t vh = ch >> 10;
                hboost::uint16_t vl = ch & 0x3FF;
                hboost::uint16_t w1 = vh + 0xD800;
                hboost::uint16_t w2 = vl + 0xDC00;
                if(state == 0) {
                    from = from_saved;
                    *to++ = w1;
                    state = 1;
            ...
            char *&to_next) const
    {
        ...
        std::cout << "Left in " << std::dec << from_end - from << " out " << to_end -to << std::endl;
            ...
            if(!hboost::locale::utf::is_valid_codepoint(ch)) {
                r=std::codecvt_base::error;
                break;
            }
            hboost::uint32_t len = implementation().from_unicode(cvt_state,ch,to,to_end);
            if(len == hboost::locale::utf::incomplete) {
                r=std::codecvt_base::partial;
                break;
            }
            else if(len == hboost::locale::utf::illegal) {
                r=std::codecvt_base::error;
                break;
            }
            else
