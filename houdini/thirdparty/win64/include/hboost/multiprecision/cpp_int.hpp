...
#ifndef HBOOST_MP_CPP_INT_HPP
#define HBOOST_MP_CPP_INT_HPP

#include <iostream>
#include <iomanip>
#include <hboost/cstdint.hpp>
#include <hboost/multiprecision/number.hpp>
#include <hboost/multiprecision/detail/integer_ops.hpp>
...
#include <hboost/multiprecision/rational_adaptor.hpp>
#include <hboost/multiprecision/traits/is_byte_container.hpp>
#include <hboost/detail/endian.hpp>
#include <hboost/integer/static_min_max.hpp>
#include <hboost/type_traits/common_type.hpp>
#include <hboost/type_traits/make_signed.hpp>
#include <hboost/multiprecision/cpp_int/checked.hpp>
#ifdef HBOOST_MP_USER_DEFINED_LITERALS
#include <hboost/multiprecision/cpp_int/value_pack.hpp>
#endif
...
template <unsigned MinBits = 0, unsigned MaxBits = 0, hboost::multiprecision::cpp_integer_type SignType = signed_magnitude, cpp_int_check_type Checked = unchecked, class Allocator = typename mpl::if_c<MinBits && (MinBits == MaxBits), void, std::allocator<limb_type> >::type >
...
namespace detail {
   ...
   template <unsigned MinBits, unsigned MaxBits, hboost::multiprecision::cpp_integer_type SignType, cpp_int_check_type Checked, class Allocator>
   struct is_byte_container<backends::cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator> > : public hboost::false_type {};
...
inline void verify_new_size(unsigned new_size, unsigned min_size, const mpl::int_<checked>&)
{
   if(new_size < min_size)
      HBOOST_THROW_EXCEPTION(std::overflow_error("Unable to allocate sufficient storage for the value of the result: value overflows the maximum allowable magnitude."));
}
...
inline void verify_limb_mask(bool b, U limb, U mask, const mpl::int_<checked>&)
{
   ...
   if(b && (limb & ~mask))
      HBOOST_THROW_EXCEPTION(std::overflow_error("Overflow in cpp_int arithmetic: there is insufficient precision in the target type to hold all of the bits of the result."));
}
template <class U>
...
struct cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false> : private Allocator::template rebind<limb_type>::other
{
   ...
   //
   HBOOST_STATIC_ASSERT(!is_void<Allocator>::value);
...
public:
   HBOOST_STATIC_CONSTANT(unsigned, limb_bits = sizeof(limb_type) * CHAR_BIT);
   HBOOST_STATIC_CONSTANT(limb_type, max_limb_value = ~static_cast<limb_type>(0u));
   HBOOST_STATIC_CONSTANT(limb_type, sign_bit_mask = static_cast<limb_type>(1u) << (limb_bits - 1));
   HBOOST_STATIC_CONSTANT(unsigned, internal_limb_count =
      MinBits
         ? (MinBits / limb_bits + ((MinBits % limb_bits) ? 1 : 0))
         : (sizeof(limb_data) / sizeof(limb_type)));
   HBOOST_STATIC_CONSTANT(bool, variable = true);
...
private:
   union data_type
   {
      ...
      HBOOST_CONSTEXPR data_type() : first(0) {}
      HBOOST_CONSTEXPR data_type(limb_type i) : first(i) {}
      HBOOST_CONSTEXPR data_type(signed_limb_type i) : first(i < 0 ? static_cast<limb_type>(hboost::multiprecision::detail::unsigned_abs(i)) : i) {}
#ifdef HBOOST_LITTLE_ENDIAN
      HBOOST_CONSTEXPR data_type(double_limb_type i) : double_first(i) {}
      HBOOST_CONSTEXPR data_type(signed_double_limb_type i) : double_first(i < 0 ? static_cast<double_limb_type>(hboost::multiprecision::detail::unsigned_abs(i)) : i) {}
#endif
   };
...
public:
   ...
   // Direct construction:
   //
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(limb_type i)HBOOST_NOEXCEPT
      : m_data(i), m_limbs(1), m_sign(false), m_internal(true) { }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(signed_limb_type i)HBOOST_NOEXCEPT
      : m_data(i), m_limbs(1), m_sign(i < 0), m_internal(true) { }
#if defined(HBOOST_LITTLE_ENDIAN)
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(double_limb_type i)HBOOST_NOEXCEPT
      : m_data(i), m_limbs(i > max_limb_value ? 2 : 1), m_sign(false), m_internal(true) { }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(signed_double_limb_type i)HBOOST_NOEXCEPT
      : m_data(i), m_limbs(i < 0 ? (static_cast<double_limb_type>(hboost::multiprecision::detail::unsigned_abs(i)) > static_cast<double_limb_type>(max_limb_value) ? 2 : 1) : (i > max_limb_value ? 2 : 1)),
        m_sign(i < 0), m_internal(true) { }
#endif
   //
   // Helper functions for getting at our internal data, and manipulating storage:
   //
   HBOOST_MP_FORCEINLINE allocator_type& allocator() HBOOST_NOEXCEPT { return *this; }
   HBOOST_MP_FORCEINLINE const allocator_type& allocator()const HBOOST_NOEXCEPT { return *this; }
   HBOOST_MP_FORCEINLINE unsigned size()const  HBOOST_NOEXCEPT { return m_limbs; }
   HBOOST_MP_FORCEINLINE limb_pointer limbs()  HBOOST_NOEXCEPT { return m_internal ? m_data.la : m_data.ld.data; }
   HBOOST_MP_FORCEINLINE const_limb_pointer limbs()const  HBOOST_NOEXCEPT { return m_internal ? m_data.la : m_data.ld.data; }
   HBOOST_MP_FORCEINLINE unsigned capacity()const  HBOOST_NOEXCEPT { return m_internal ? internal_limb_count : m_data.ld.capacity; }
   HBOOST_MP_FORCEINLINE bool sign()const  HBOOST_NOEXCEPT { return m_sign; }
   void sign(bool b)  HBOOST_NOEXCEPT
   {
      m_sign = b;
   ...
   HBOOST_MP_FORCEINLINE void normalize() HBOOST_NOEXCEPT
   {
      limb_pointer p = limbs();
      while((m_limbs-1) && !p[m_limbs - 1])--m_limbs;
   }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base() HBOOST_NOEXCEPT : m_data(), m_limbs(1), m_sign(false), m_internal(true) {}
   HBOOST_MP_FORCEINLINE cpp_int_base(const cpp_int_base& o) : allocator_type(o), m_limbs(0), m_internal(true)
   {
   ...
   cpp_int_base& operator = (cpp_int_base&& o) HBOOST_NOEXCEPT
   {
      if(!m_internal)
...
#endif
   HBOOST_MP_FORCEINLINE ~cpp_int_base() HBOOST_NOEXCEPT
   {
      if(!m_internal)
         allocator().deallocate(limbs(), capacity());
   }
   ...
   HBOOST_MP_FORCEINLINE void negate() HBOOST_NOEXCEPT
   {
      m_sign = !m_sign;
      // Check for zero value:
      if(m_sign && (m_limbs == 1))
   ...
   HBOOST_MP_FORCEINLINE bool isneg()const HBOOST_NOEXCEPT
   {
      return m_sign;
   }
   HBOOST_MP_FORCEINLINE void do_swap(cpp_int_base& o) HBOOST_NOEXCEPT
   {
...
protected:
   template <class A>
   void check_in_range(const A&) HBOOST_NOEXCEPT {}
};
...
struct cpp_int_base<MinBits, MaxBits, unsigned_magnitude, Checked, Allocator, false> : private Allocator::template rebind<limb_type>::other
{
   ...
   //
   HBOOST_STATIC_ASSERT_MSG(((sizeof(Allocator) == 0) && !is_void<Allocator>::value), "There is curently no support for unsigned arbitrary precision integers.");
};
//
...
struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false>
{
   ...
   // Interface invariants:
   //
   HBOOST_STATIC_ASSERT_MSG(MinBits > sizeof(double_limb_type) * CHAR_BIT, "Template parameter MinBits is inconsistent with the parameter trivial - did you mistakingly try to override the trivial parameter?");

public:
   HBOOST_STATIC_CONSTANT(unsigned, limb_bits = sizeof(limb_type) * CHAR_BIT);
   HBOOST_STATIC_CONSTANT(limb_type, max_limb_value = ~static_cast<limb_type>(0u));
   HBOOST_STATIC_CONSTANT(limb_type, sign_bit_mask = static_cast<limb_type>(1u) << (limb_bits - 1));
   HBOOST_STATIC_CONSTANT(unsigned, internal_limb_count = MinBits / limb_bits + ((MinBits % limb_bits) ? 1 : 0));
   HBOOST_STATIC_CONSTANT(bool, variable = false);
   HBOOST_STATIC_CONSTANT(limb_type, upper_limb_mask = (MinBits % limb_bits) ? (limb_type(1) << (MinBits % limb_bits)) -1 : (~limb_type(0)));
   HBOOST_STATIC_ASSERT_MSG(internal_limb_count >= 2, "A fixed precision integer type must have at least 2 limbs");
...
private:
   union data_type{
      ...
      HBOOST_CONSTEXPR data_type() : m_first_limb(0) {}
      HBOOST_CONSTEXPR data_type(limb_type i) : m_first_limb(i) {}
      HBOOST_CONSTEXPR data_type(double_limb_type i) : m_double_first_limb(i) {}
#if defined(HBOOST_MP_USER_DEFINED_LITERALS)
      template <limb_type...VALUES>
      HBOOST_CONSTEXPR data_type(literals::detail::value_pack<VALUES...>) : m_data{ VALUES... } {}
#endif
   } m_wrapper;
   hboost::uint16_t    m_limbs;
   bool               m_sign;
...
public:
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(signed_double_limb_type i)HBOOST_NOEXCEPT
      : m_wrapper(double_limb_type(i < 0 ? static_cast<double_limb_type>(hboost::multiprecision::detail::unsigned_abs(i)) : i)),
        m_limbs(i < 0 ? (static_cast<double_limb_type>(hboost::multiprecision::detail::unsigned_abs(i)) > max_limb_value ? 2 : 1) : (i > max_limb_value ? 2 : 1)),
        m_sign(i < 0) {}
#endif
#if defined(HBOOST_MP_USER_DEFINED_LITERALS)
      template <limb_type...VALUES>
      HBOOST_CONSTEXPR cpp_int_base(literals::detail::value_pack<VALUES...> i)
         : m_wrapper(i), m_limbs(sizeof...(VALUES)), m_sign(false) {}
      HBOOST_CONSTEXPR cpp_int_base(literals::detail::value_pack<> i)
         : m_wrapper(i), m_limbs(1), m_sign(false) {}
      HBOOST_CONSTEXPR cpp_int_base(const cpp_int_base& a, const literals::detail::negate_tag&)
   ...
   //
   HBOOST_MP_FORCEINLINE unsigned size()const HBOOST_NOEXCEPT { return m_limbs; }
   HBOOST_MP_FORCEINLINE limb_pointer limbs() HBOOST_NOEXCEPT { return m_wrapper.m_data; }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const_limb_pointer limbs()const HBOOST_NOEXCEPT { return m_wrapper.m_data; }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool sign()const HBOOST_NOEXCEPT { return m_sign; }
   HBOOST_MP_FORCEINLINE void sign(bool b) HBOOST_NOEXCEPT
   {
      m_sign = b;
      // Check for zero value:
      if(m_sign && (m_limbs == 1))
   ...
   HBOOST_MP_FORCEINLINE void resize(unsigned new_size, unsigned min_size) HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   {
      m_limbs = static_cast<hboost::uint16_t>((std::min)(new_size, internal_limb_count));
      detail::verify_new_size(m_limbs, min_size, checked_type());
   }
   HBOOST_MP_FORCEINLINE void normalize() HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   {
      limb_pointer p = limbs();
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base()HBOOST_NOEXCEPT : m_wrapper(limb_type(0u)), m_limbs(1), m_sign(false) {}
   // Not defaulted, it breaks constexpr support in the Intel compiler for some reason:
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(const cpp_int_base& o)HBOOST_NOEXCEPT
      : m_wrapper(o.m_wrapper), m_limbs(o.m_limbs), m_sign(o.m_sign) {}
   // Defaulted functions:
   //~cpp_int_base() HBOOST_NOEXCEPT {}

   void assign(const cpp_int_base& o) HBOOST_NOEXCEPT
   {
      if(this != &o)
      {
         m_limbs = o.m_limbs;
...
public:
   ...
   HBOOST_MP_FORCEINLINE cpp_int_base(signed_limb_type i)HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   ...
   HBOOST_MP_FORCEINLINE cpp_int_base(signed_double_limb_type i)HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
      : m_wrapper(double_limb_type(i < 0 ? static_cast<double_limb_type>(hboost::multiprecision::detail::unsigned_abs(i)) : i)), 
      m_limbs(i < 0 ? (static_cast<double_limb_type>(hboost::multiprecision::detail::unsigned_abs(i)) > max_limb_value ? 2 : 1) : (i > max_limb_value ? 2 : 1)) 
   {
      if (i < 0) negate();
      ...
      HBOOST_CONSTEXPR cpp_int_base(literals::detail::value_pack<>)
   ...
   HBOOST_MP_FORCEINLINE limb_pointer limbs() HBOOST_NOEXCEPT { return m_wrapper.m_data; }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const_limb_pointer limbs()const HBOOST_NOEXCEPT { return m_wrapper.m_data; }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool sign()const HBOOST_NOEXCEPT { return false; }
   HBOOST_MP_FORCEINLINE void sign(bool b) HBOOST_MP_NOEXCEPT_IF((Checked == unchecked)) {  if(b) negate(); }
   HBOOST_MP_FORCEINLINE void resize(unsigned new_size, unsigned min_size) HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   {
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base() HBOOST_NOEXCEPT
   ...
   HBOOST_MP_FORCEINLINE void assign(const cpp_int_base& o) HBOOST_NOEXCEPT
   {
      if(this != &o)
      {
         m_limbs = o.m_limbs;
...
private:
   ...
   {
      HBOOST_THROW_EXCEPTION(std::range_error("Attempt to negate an unsigned number."));
   }
   void check_negate(const mpl::int_<unchecked>&){}
public:
   void negate() HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   {
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool isneg()const HBOOST_NOEXCEPT
   {
      return false;
   }
   HBOOST_MP_FORCEINLINE void do_swap(cpp_int_base& o) HBOOST_NOEXCEPT
...
#endif
//
// Traits classes to figure out a native type with N bits, these vary from hboost::uint_t<N> only
// because some platforms have native integer types longer than hboost::long_long_type, "really hboost::long_long_type" anyone??
//
...
{
   typedef typename hboost::uint_t<N>::least type;
};

template <unsigned N>
struct trivial_limb_type : public trivial_limb_type_imp<N, N <= sizeof(hboost::long_long_type) * CHAR_BIT> {};
//
...
struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, true>
{
   ...
   typedef mpl::int_<Checked>                         checked_type;
protected:
   HBOOST_STATIC_CONSTANT(unsigned, limb_bits = sizeof(local_limb_type) * CHAR_BIT);
   HBOOST_STATIC_CONSTANT(local_limb_type, limb_mask = (MinBits < limb_bits) ? local_limb_type((local_limb_type(~local_limb_type(0))) >> (limb_bits - MinBits)) : local_limb_type(~local_limb_type(0)));
private:
...
protected:
   template <class T>
   typename hboost::disable_if_c<!hboost::is_integral<T>::value || (std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= (int)MinBits))>::type
      check_in_range(T val, const mpl::int_<checked>&)
   {
      ...
      if(static_cast<common_type>(hboost::multiprecision::detail::unsigned_abs(val)) > static_cast<common_type>(limb_mask))
         HBOOST_THROW_EXCEPTION(std::range_error("The argument to a cpp_int constructor exceeded the largest value it can represent."));
   }
   template <class T>
   typename hboost::disable_if_c<hboost::is_integral<T>::value || (std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= (int)MinBits))>::type
      check_in_range(T val, const mpl::int_<checked>&)
   ...
   void check_in_range(T, const mpl::int_<C>&) HBOOST_NOEXCEPT {}

   template <class T>
   void check_in_range(T val) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<T>(), checked_type())))
   {
...
public:
   ...
   //
   template <class SI>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(SI i, typename hboost::enable_if_c<is_signed<SI>::value && (Checked == unchecked) >::type const* = 0) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
      : m_data(i < 0 ? static_cast<local_limb_type>(static_cast<typename make_unsigned<SI>::type>(hboost::multiprecision::detail::unsigned_abs(i)) & limb_mask) : static_cast<local_limb_type>(i)& limb_mask), m_sign(i < 0) {}
   template <class SI>
   HBOOST_MP_FORCEINLINE cpp_int_base(SI i, typename hboost::enable_if_c<is_signed<SI>::value && (Checked == checked) >::type const* = 0) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
      : m_data(i < 0 ? (static_cast<local_limb_type>(static_cast<typename make_unsigned<SI>::type>(hboost::multiprecision::detail::unsigned_abs(i)) & limb_mask)) : static_cast<local_limb_type>(i)& limb_mask), m_sign(i < 0) 
   { check_in_range(i); }
   template <class UI>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(UI i, typename hboost::enable_if_c<is_unsigned<UI>::value && (Checked == unchecked)>::type const* = 0) HBOOST_NOEXCEPT
      : m_data(static_cast<local_limb_type>(i) & limb_mask), m_sign(false) {}
   template <class UI>
   HBOOST_MP_FORCEINLINE cpp_int_base(UI i, typename hboost::enable_if_c<is_unsigned<UI>::value && (Checked == checked)>::type const* = 0) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<UI>())))
      : m_data(static_cast<local_limb_type>(i) & limb_mask), m_sign(false) { check_in_range(i); }
   template <class F>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(F i, typename hboost::enable_if_c<is_floating_point<F>::value && (Checked == unchecked)>::type const* = 0) HBOOST_NOEXCEPT
      : m_data(static_cast<local_limb_type>(std::fabs(i)) & limb_mask), m_sign(i < 0) {}
   template <class F>
   HBOOST_MP_FORCEINLINE cpp_int_base(F i, typename hboost::enable_if_c<is_floating_point<F>::value && (Checked == checked)>::type const* = 0)
      : m_data(static_cast<local_limb_type>(std::fabs(i)) & limb_mask), m_sign(i < 0) { check_in_range(i); }
#if defined(HBOOST_MP_USER_DEFINED_LITERALS)
      HBOOST_CONSTEXPR cpp_int_base(literals::detail::value_pack<>) HBOOST_NOEXCEPT
         : m_data(static_cast<local_limb_type>(0u)), m_sign(false) {}
      template <limb_type a>
      HBOOST_CONSTEXPR cpp_int_base(literals::detail::value_pack<a>)HBOOST_NOEXCEPT
         : m_data(static_cast<local_limb_type>(a)), m_sign(false) {}
      template <limb_type a, limb_type b>
      HBOOST_CONSTEXPR cpp_int_base(literals::detail::value_pack<a, b>)HBOOST_NOEXCEPT
         : m_data(static_cast<local_limb_type>(a) | (static_cast<local_limb_type>(b) << bits_per_limb)), m_sign(false) {}
      HBOOST_CONSTEXPR cpp_int_base(const cpp_int_base& a, const literals::detail::negate_tag&)HBOOST_NOEXCEPT
         : m_data(a.m_data), m_sign(a.m_data ? !a.m_sign : false) {}
#endif
   //
   // Helper functions for getting at our internal data, and manipulating storage:
   //
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR unsigned size()const HBOOST_NOEXCEPT { return 1; }
   HBOOST_MP_FORCEINLINE limb_pointer limbs() HBOOST_NOEXCEPT { return &m_data; }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const_limb_pointer limbs()const HBOOST_NOEXCEPT { return &m_data; }
   HBOOST_MP_FORCEINLINE bool sign()const HBOOST_NOEXCEPT { return m_sign; }
   HBOOST_MP_FORCEINLINE void sign(bool b) HBOOST_NOEXCEPT
   {
      m_sign = b;
   ...
   HBOOST_MP_FORCEINLINE void resize(unsigned new_size, unsigned min_size)
   {
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base() HBOOST_NOEXCEPT : m_data(0), m_sign(false) {}
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(const cpp_int_base& o) HBOOST_NOEXCEPT
...
private:
   HBOOST_STATIC_CONSTANT(unsigned, limb_bits = sizeof(local_limb_type) * CHAR_BIT);
   HBOOST_STATIC_CONSTANT(local_limb_type, limb_mask = limb_bits != MinBits ? 
...
protected:
   template <class T>
   typename hboost::disable_if_c<std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= (int)MinBits)>::type
      check_in_range(T val, const mpl::int_<checked>&, const hboost::false_type&)
   {
      typedef typename common_type<T, local_limb_type>::type common_type;
   ...
   template <class T>
   void check_in_range(T val, const mpl::int_<checked>&, const hboost::true_type&)
   {
      typedef typename common_type<T, local_limb_type>::type common_type;
      ...
      if(static_cast<common_type>(val) > limb_mask)
         HBOOST_THROW_EXCEPTION(std::range_error("The argument to a cpp_int constructor exceeded the largest value it can represent."));
      if(val < 0)
         HBOOST_THROW_EXCEPTION(std::range_error("The argument to an unsigned cpp_int constructor was negative."));
   }
   template <class T, int C, bool B>
   HBOOST_MP_FORCEINLINE void check_in_range(T, const mpl::int_<C>&, const hboost::integral_constant<bool, B>&) HBOOST_NOEXCEPT {}

   template <class T>
   HBOOST_MP_FORCEINLINE void check_in_range(T val) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<T>(), checked_type(), is_signed<T>())))
   {
...
public:
   ...
   //
   template <class SI>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(SI i, typename hboost::enable_if_c<is_signed<SI>::value && (Checked == unchecked) >::type const* = 0) HBOOST_NOEXCEPT
   ...
   template <class F>
   HBOOST_MP_FORCEINLINE cpp_int_base(F i, typename hboost::enable_if<is_floating_point<F> >::type const* = 0) HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const_limb_pointer limbs()const HBOOST_NOEXCEPT { return &m_data; }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool sign()const HBOOST_NOEXCEPT { return false; }
   HBOOST_MP_FORCEINLINE void sign(bool b) HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   {
      if(b)
         negate();
   }
   HBOOST_MP_FORCEINLINE void resize(unsigned, unsigned min_size)
   {
      detail::verify_new_size(2, min_size, checked_type());
   }
   HBOOST_MP_FORCEINLINE void normalize() HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base() HBOOST_NOEXCEPT : m_data(0) {}
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_base(const cpp_int_base& o) HBOOST_NOEXCEPT
      : m_data(o.m_data) {}
   //~cpp_int_base() HBOOST_NOEXCEPT {}
   HBOOST_MP_FORCEINLINE void assign(const cpp_int_base& o) HBOOST_NOEXCEPT
   ...
   HBOOST_MP_FORCEINLINE void negate() HBOOST_MP_NOEXCEPT_IF((Checked == unchecked))
   {
      if(Checked == checked)
      {
         HBOOST_THROW_EXCEPTION(std::range_error("Attempt to negate an unsigned type."));
      }
      m_data = ~m_data;
      ++m_data;
   }
...
public:
   typedef typename mpl::if_<
      ...
      hboost::long_long_type, signed_double_limb_type>,
      mpl::list<signed_limb_type, signed_double_limb_type>
   >::type                                                           signed_types;
   typedef typename mpl::if_<
      trivial_tag,
      mpl::list<unsigned char, unsigned short, unsigned,
      unsigned long, hboost::ulong_long_type, double_limb_type>,
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_backend() HBOOST_NOEXCEPT{}
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_backend(const cpp_int_backend& o) HBOOST_MP_NOEXCEPT_IF(hboost::is_void<Allocator>::value) : base_type(o) {}
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_backend(cpp_int_backend&& o) HBOOST_NOEXCEPT
   ...
   //
   template <class Arg>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR cpp_int_backend(Arg i, typename hboost::enable_if_c<is_allowed_cpp_int_base_conversion<Arg, base_type>::value >::type const* = 0)HBOOST_MP_NOEXCEPT_IF(noexcept(base_type(std::declval<Arg>())))
      : base_type(i) {}
...
private:
   ...
   void do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other, mpl::true_ const&, mpl::false_ const &)
   {
      ...
      if(other.size() > 1)
      {
         ...
         {
            HBOOST_THROW_EXCEPTION(std::range_error("Assignment of a cpp_int that is out of range for the target type."));
         }
      }
      *this = v;
      this->sign(other.sign());
   ...
   void do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other, mpl::false_ const&, mpl::true_ const &)
   {
       ...
       *this = static_cast<
            typename hboost::multiprecision::detail::canonical<
...
public:
   ...
   cpp_int_backend(
      ...
      typename hboost::enable_if_c<is_implicit_cpp_int_conversion<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>, self_type>::value>::type* = 0)
      : base_type()
   {
      do_assign(
         other,
   ...
   explicit cpp_int_backend(
      const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other,
      typename hboost::disable_if_c<is_implicit_cpp_int_conversion<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>, self_type>::value>::type* = 0)
      : base_type()
   {
      do_assign(
         other,
...
#ifdef HBOOST_MP_USER_DEFINED_LITERALS
   HBOOST_CONSTEXPR cpp_int_backend(const cpp_int_backend& a, const literals::detail::negate_tag& tag)
      : base_type(static_cast<const base_type&>(a), tag){}
#endif

   HBOOST_MP_FORCEINLINE cpp_int_backend& operator = (const cpp_int_backend& o) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().assign(std::declval<const cpp_int_backend&>())))
   {
      this->assign(o);
      return *this;
   }
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
   HBOOST_MP_FORCEINLINE cpp_int_backend& operator = (cpp_int_backend&& o) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<base_type&>() = std::declval<base_type>()))
   {
...
private:
   template <class A>
   typename hboost::enable_if<is_unsigned<A> >::type do_assign_arithmetic(A val, const mpl::true_&) 
         HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().check_in_range(std::declval<A>())))
   {
   ...
   template <class A>
   typename hboost::disable_if_c<is_unsigned<A>::value || !is_integral<A>::value >::type do_assign_arithmetic(A val, const mpl::true_&) 
         HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().check_in_range(std::declval<A>())) && noexcept(std::declval<cpp_int_backend>().sign(true)))
   {
      this->check_in_range(val);
      *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(hboost::multiprecision::detail::unsigned_abs(val)) : static_cast<typename self_type::local_limb_type>(val);
      this->sign(val < 0);
      this->normalize();
   }
   template <class A>
   typename hboost::enable_if_c< !is_integral<A>::value>::type do_assign_arithmetic(A val, const mpl::true_&)
   {
      this->check_in_range(val);
      *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(hboost::multiprecision::detail::abs(val)) : static_cast<typename self_type::local_limb_type>(val);
      this->sign(val < 0);
      this->normalize();
   }
   HBOOST_MP_FORCEINLINE void do_assign_arithmetic(limb_type i, const mpl::false_&) HBOOST_NOEXCEPT
   {
      this->resize(1, 1);
      *this->limbs() = i;
      this->sign(false);
   }
   HBOOST_MP_FORCEINLINE void do_assign_arithmetic(signed_limb_type i, const mpl::false_&) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().sign(true)))
   {
      this->resize(1, 1);
      *this->limbs() = static_cast<limb_type>(hboost::multiprecision::detail::unsigned_abs(i));
      this->sign(i < 0);
   }
   void do_assign_arithmetic(double_limb_type i, const mpl::false_&) HBOOST_NOEXCEPT
   {
      HBOOST_STATIC_ASSERT(sizeof(i) == 2 * sizeof(limb_type));
      HBOOST_STATIC_ASSERT(base_type::internal_limb_count >= 2);
   ...
   void do_assign_arithmetic(signed_double_limb_type i, const mpl::false_&) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().sign(true)))
   {
      HBOOST_STATIC_ASSERT(sizeof(i) == 2 * sizeof(limb_type));
      HBOOST_STATIC_ASSERT(base_type::internal_limb_count >= 2);
      bool s = false;
      ...
      if(i < 0)
         s = true;
      ui = static_cast<double_limb_type>(hboost::multiprecision::detail::unsigned_abs(i));
      typename base_type::limb_pointer p = this->limbs();
   ...
   void do_assign_arithmetic(long double a, const mpl::false_&)
   {
      ...
      HBOOST_ASSERT(!(hboost::math::isinf)(a));
      HBOOST_ASSERT(!(hboost::math::isnan)(a));
...
public:
   ...
   HBOOST_MP_FORCEINLINE typename hboost::enable_if_c<!hboost::multiprecision::detail::is_byte_container<Arithmetic>::value, cpp_int_backend&>::type operator = (Arithmetic val) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().do_assign_arithmetic(std::declval<Arithmetic>(), trivial_tag())))
   {
      ...
      *this = 0;
      ...
      if(n)
      {
         ...
         while(*s)
         {
            ...
            if(val >= radix)
            {
               HBOOST_THROW_EXCEPTION(std::runtime_error("Unexpected content found while parsing character string."));
            }
            ...
            while(*s)
            {
               ...
               for(unsigned i = 0; i < digits_per_block_10; ++i)
               {
                  ...
                  if(*s >= '0' && *s <= '9')
                     ...
                     HBOOST_THROW_EXCEPTION(std::runtime_error("Unexpected character encountered in input."));
                  block *= 10;
                  block += val;
                  if(!*++s)
                  {
...
public:
   ...
   HBOOST_MP_FORCEINLINE void swap(cpp_int_backend& o) HBOOST_NOEXCEPT
   {
      this->do_swap(o);
   }
private:
   std::string do_get_trivial_string(std::ios_base::fmtflags f, const mpl::false_&)const
   {
      ...
      if(this->sign() && (((f & std::ios_base::hex) == std::ios_base::hex) || ((f & std::ios_base::oct) == std::ios_base::oct)))
         HBOOST_THROW_EXCEPTION(std::runtime_error("Base 8 or 16 printing of negative numbers is not supported."));
      std::stringstream ss;
   ...
   {
#ifdef HBOOST_MP_NO_DOUBLE_LIMB_TYPE_IO
   ...
   std::string do_get_string(std::ios_base::fmtflags f, const mpl::false_&)const
   {
      ...
      else
      {
         ...
         if(this->size() == 1)
         {
            result = hboost::lexical_cast<std::string>(t.limbs()[0]);
         }
         else
         {
...
public:
   template <class Container>
   cpp_int_backend(const Container& c, typename hboost::enable_if_c<hboost::multiprecision::detail::is_byte_container<Container>::value>::type const* = 0)
   {
      //
      // We assume that c is a sequence of (unsigned) bytes with the most significant byte first:
      //
   ...
   template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2, cpp_int_check_type Checked2, class Allocator2>
   int compare_imp(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& o, const mpl::false_&, const mpl::false_&)const HBOOST_NOEXCEPT
   {
      if(this->sign() != o.sign())
   ...
   template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2, cpp_int_check_type Checked2, class Allocator2>
   int compare_imp(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& o, const mpl::true_&, const mpl::true_&)const HBOOST_NOEXCEPT
   {
      if(this->sign())
      {
         if(o.sign())
   ...
   template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2, cpp_int_check_type Checked2, class Allocator2>
   int compare(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& o)const HBOOST_NOEXCEPT
   {
   ...
   template <unsigned MinBits2, unsigned MaxBits2, cpp_integer_type SignType2, cpp_int_check_type Checked2, class Allocator2>
   int compare_unsigned(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& o)const HBOOST_NOEXCEPT
   {
      if(this->size() != o.size())
      {
   ...
   template <class Arithmetic>
   HBOOST_MP_FORCEINLINE typename hboost::enable_if<is_arithmetic<Arithmetic>, int>::type compare(Arithmetic i)const
   {
      // braindead version:
      cpp_int_backend t;
      t = i;
...
using hboost::multiprecision::backends::cpp_int_backend;
...
//
#include <hboost/multiprecision/cpp_int/comparison.hpp>
#include <hboost/multiprecision/cpp_int/add.hpp>
#include <hboost/multiprecision/cpp_int/multiply.hpp>
#include <hboost/multiprecision/cpp_int/divide.hpp>
#include <hboost/multiprecision/cpp_int/bitwise.hpp>
#include <hboost/multiprecision/cpp_int/misc.hpp>
#include <hboost/multiprecision/cpp_int/limits.hpp>
#ifdef HBOOST_MP_USER_DEFINED_LITERALS
#include <hboost/multiprecision/cpp_int/literals.hpp>
#endif
#include <hboost/multiprecision/cpp_int/serialize.hpp>
#include <hboost/multiprecision/cpp_int/import_export.hpp>
