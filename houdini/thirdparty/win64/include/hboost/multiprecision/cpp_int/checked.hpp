...
#ifndef HBOOST_MP_CPP_INT_CHECKED_HPP
#define HBOOST_MP_CPP_INT_CHECKED_HPP

namespace hboost{ namespace multiprecision{ namespace backends{ namespace detail{

//
// Simple routines for performing checked arithmetic with a builtin arithmetic type.
// Note that this is not a complete header, it must be included as part of hboost/multiprecision/cpp_int.hpp.
//

inline void raise_overflow(std::string op)
{
   HBOOST_THROW_EXCEPTION(std::overflow_error("overflow in " + op));
}
...
{
   HBOOST_THROW_EXCEPTION(std::range_error("Subtraction resulted in a negative value, but the type is unsigned"));
}
...
{
   return checked_add_imp(a, b, hboost::is_signed<A>());
}
template <class A>
...
{
   return checked_subtract_imp(a, b, hboost::is_signed<A>());
}
template <class A>
...
template <class A>
inline A checked_left_shift(A a, hboost::ulong_long_type shift, const mpl::int_<checked>&)
{
   if(a && shift)
   {
      if((shift > sizeof(A) * CHAR_BIT) || (a >> (sizeof(A) * CHAR_BIT - shift)))
         HBOOST_THROW_EXCEPTION(std::overflow_error("Shift out of range"));
   }
   return a << shift;
}
template <class A>
inline A checked_left_shift(A a, hboost::ulong_long_type shift, const mpl::int_<unchecked>&)
{
