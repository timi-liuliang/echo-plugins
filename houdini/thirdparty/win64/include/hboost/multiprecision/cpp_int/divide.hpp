...
//
#ifndef HBOOST_MP_CPP_INT_DIV_HPP
#define HBOOST_MP_CPP_INT_DIV_HPP
   ...
   CppInt1& r)
{
   ...
   do
   {
      ...
      HBOOST_ASSERT(guess); // If the guess ever gets to zero we go on forever....
      //
      // Update result:
      //
   ...
   HBOOST_ASSERT(r.compare_unsigned(y) < 0); // remainder must be less than the divisor or our code has failed
}
   ...
   CppInt1& r)
{
   ...
   if(y == 0)
   {
      HBOOST_THROW_EXCEPTION(std::overflow_error("Integer Division by zero."));
   }
   //
   ...
   HBOOST_ASSERT(r.compare(y) < 0); // remainder must be less than the divisor or our code has failed
}
      ...
      signed_limb_type& b)
{
   cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> r;
   bool s = a.sign() != (b < 0);
   divide_unsigned_helper(&result, a, static_cast<limb_type>(hboost::multiprecision::detail::unsigned_abs(b)), r);
   result.sign(s);
}
...
{
   bool s = a.sign();
   divide_unsigned_helper(static_cast<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>* >(0), a, static_cast<limb_type>(hboost::multiprecision::detail::unsigned_abs(b)), result);
   result.sign(s);
}
...
{
   if(!*o.limbs())
      HBOOST_THROW_EXCEPTION(std::overflow_error("Division by zero."));
