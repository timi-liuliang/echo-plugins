...
#ifndef HBOOST_MATH_EXTENDED_REAL_HPP
#define HBOOST_MATH_EXTENDED_REAL_HPP

#include <hboost/cstdint.hpp>
#include <hboost/mpl/max.hpp>
...
#include <hboost/multiprecision/detail/generic_interconvert.hpp>
#include <hboost/multiprecision/detail/number_compare.hpp>
#include <hboost/multiprecision/traits/is_restricted_conversion.hpp>
...
public:
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR number() HBOOST_MP_NOEXCEPT_IF(noexcept(Backend())) {}
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR number(const number& e) HBOOST_MP_NOEXCEPT_IF(noexcept(Backend(std::declval<Backend const&>()))) : m_backend(e.m_backend){}
   template <class V>
   HBOOST_MP_FORCEINLINE number(const V& v, typename hboost::enable_if_c<
            (hboost::is_arithmetic<V>::value || is_same<std::string, V>::value || is_convertible<V, const char*>::value)
   ...
   template <class V>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR number(const V& v, typename hboost::enable_if_c<
            is_convertible<typename detail::canonical<V, Backend>::type, Backend>::value
            && !detail::is_restricted_conversion<typename detail::canonical<V, Backend>::type, Backend>::value
   >::type* = 0) 
#ifndef HBOOST_INTEL
          HBOOST_MP_NOEXCEPT_IF(noexcept(Backend(std::declval<typename detail::canonical<V, Backend>::type const&>())))
#endif
      : m_backend(canonical_value(v)) {}
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR number(const number& e, unsigned digits10) 
      HBOOST_MP_NOEXCEPT_IF(noexcept(Backend(std::declval<Backend const&>(), std::declval<unsigned>())))
      : m_backend(e.m_backend, digits10){}
   template <class V>
   explicit HBOOST_MP_FORCEINLINE number(const V& v, typename hboost::enable_if_c<
            (hboost::is_arithmetic<V>::value || is_same<std::string, V>::value || is_convertible<V, const char*>::value)
            && !detail::is_explicitly_convertible<typename detail::canonical<V, Backend>::type, Backend>::value
            && detail::is_restricted_conversion<typename detail::canonical<V, Backend>::type, Backend>::value
         >::type* = 0) 
         HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<Backend&>() = std::declval<typename detail::canonical<V, Backend>::type const&>()))
   {
      m_backend = canonical_value(v);
   }
   template <class V>
   explicit HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR number(const V& v, typename hboost::enable_if_c<
            detail::is_explicitly_convertible<typename detail::canonical<V, Backend>::type, Backend>::value
            && (detail::is_restricted_conversion<typename detail::canonical<V, Backend>::type, Backend>::value
                || !is_convertible<typename detail::canonical<V, Backend>::type, Backend>::value)
         >::type* = 0)
   ...
   //
   template <class V>
   number(V v, unsigned digits10, typename hboost::enable_if<mpl::or_<hboost::is_arithmetic<V>, is_same<std::string, V>, is_convertible<V, const char*> > >::type* dummy1 = 0)
   {
   ...
   */
   ...
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR number(const number<Backend, ET>& val)
      HBOOST_MP_NOEXCEPT_IF(noexcept(Backend(std::declval<Backend const&>()))) : m_backend(val.backend()) {}

   template <class Other, expression_template_option ET>
   HBOOST_MP_FORCEINLINE number(const number<Other, ET>& val,
         typename hboost::enable_if_c<(hboost::is_convertible<Other, Backend>::value && !detail::is_restricted_conversion<Other, Backend>::value)>::type* = 0)
      HBOOST_MP_NOEXCEPT_IF(noexcept(Backend(std::declval<Other const&>())))
      : m_backend(val.backend()) {}

   template <class Other, expression_template_option ET>
   explicit number(const number<Other, ET>& val, typename hboost::enable_if_c<
   ...
   template <class Other, expression_template_option ET>
   explicit HBOOST_MP_FORCEINLINE number(const number<Other, ET>& val, typename hboost::enable_if_c<
         (detail::is_explicitly_convertible<Other, Backend>::value
            && (detail::is_restricted_conversion<Other, Backend>::value || !hboost::is_convertible<Other, Backend>::value))
         >::type* = 0) HBOOST_MP_NOEXCEPT_IF(noexcept(Backend(std::declval<Other const&>())))
      : m_backend(val.backend()) {}

   template <class V>
   HBOOST_MP_FORCEINLINE number(V v1, V v2, typename hboost::enable_if<mpl::or_<hboost::is_arithmetic<V>, is_same<std::string, V>, is_convertible<V, const char*> > >::type* = 0)
   {
      using default_ops::assign_components;
      assign_components(m_backend, canonical_value(v1), canonical_value(v2));
   }
   template <class Other, expression_template_option ET>
   HBOOST_MP_FORCEINLINE number(const number<Other, ET>& v1, const number<Other, ET>& v2, typename hboost::enable_if<hboost::is_convertible<Other, Backend> >::type* = 0)
   {
      using default_ops::assign_components;
   ...
   typename hboost::enable_if<is_convertible<typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type, self_type>, number&>::type operator=(const detail::expression<tag, Arg1, Arg2, Arg3, Arg4>& e)
   {
   ...
   HBOOST_MP_FORCEINLINE number& operator=(const number& e)
      HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<Backend&>() = std::declval<Backend const&>()))
   {
   ...
   template <class V>
   HBOOST_MP_FORCEINLINE typename hboost::enable_if<is_convertible<V, self_type>, number<Backend, ExpressionTemplates>& >::type
      operator=(const V& v)
      HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<Backend&>() = std::declval<const typename detail::canonical<V, Backend>::type&>()))
   {
      m_backend = canonical_value(v);
      return *this;
   }
   template <class V>
   HBOOST_MP_FORCEINLINE number<Backend, ExpressionTemplates>& assign(const V& v)
      HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<Backend&>() = std::declval<const typename detail::canonical<V, Backend>::type&>()))
   {
      m_backend = canonical_value(v);
      return *this;
   ...
   template <class Other, expression_template_option ET>
   typename hboost::disable_if<hboost::multiprecision::detail::is_explicitly_convertible<Other, Backend>, number<Backend, ExpressionTemplates>& >::type
   ...
   template <class tag, class Arg1, class Arg2, class Arg3, class Arg4>
   number(const detail::expression<tag, Arg1, Arg2, Arg3, Arg4>& e, typename hboost::enable_if_c<is_convertible<typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type, self_type>::value>::type* = 0)
   {
      *this = e;
   }
   template <class tag, class Arg1, class Arg2, class Arg3, class Arg4>
   explicit number(const detail::expression<tag, Arg1, Arg2, Arg3, Arg4>& e,
      typename hboost::enable_if_c<!is_convertible<typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type, self_type>::value
      && hboost::multiprecision::detail::is_explicitly_convertible<typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type, self_type>::value>::type* = 0)
   {
      assign(e);
   }

#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR number(number&& r)
      HBOOST_MP_NOEXCEPT_IF(noexcept(Backend(std::declval<Backend>())))
      : m_backend(static_cast<Backend&&>(r.m_backend)){}
   HBOOST_MP_FORCEINLINE number& operator=(number&& r) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<Backend&>() = std::declval<Backend>()))
   {
      m_backend = static_cast<Backend&&>(r.m_backend);
      return *this;
   }
   ...
   template <class V>
   typename hboost::enable_if<hboost::is_convertible<V, self_type>, number<Backend, ExpressionTemplates>& >::type
      operator+=(const V& v)
   {
   ...
   number& operator%=(const self_type& e)
   {
      HBOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The modulus operation is only valid for integer types");
   ...
   //
   HBOOST_MP_FORCEINLINE number& operator++()
   {
   ...
   template <class V>
   HBOOST_MP_FORCEINLINE typename hboost::enable_if<is_integral<V>, number&>::type operator <<= (V val)
   {
      HBOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The left-shift operation is only valid for integer types");
   ...
   HBOOST_MP_FORCEINLINE typename hboost::enable_if<is_integral<V>, number&>::type operator >>= (V val)
   {
      HBOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The right-shift operation is only valid for integer types");
      detail::check_shift_range(val, mpl::bool_<(sizeof(V) > sizeof(std::size_t))>(), is_signed<V>());
      eval_right_shift(m_backend, static_cast<std::size_t>(canonical_value(val)));
      return *this;
   }

   HBOOST_MP_FORCEINLINE number& operator /= (const self_type& e)
   {
   ...
   template <class V>
   HBOOST_MP_FORCEINLINE typename hboost::enable_if<hboost::is_convertible<V, self_type>, number<Backend, ExpressionTemplates>& >::type
      operator/=(const V& v)
   {
   ...
   HBOOST_MP_FORCEINLINE number& operator&=(const self_type& e)
   {
      HBOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The bitwise & operation is only valid for integer types");
   ...
   // swap:
   //
   HBOOST_MP_FORCEINLINE void swap(self_type& other) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<Backend>().swap(std::declval<Backend&>())))
   {
   ...
   // Zero and sign:
   //
   HBOOST_MP_FORCEINLINE bool is_zero()const
   {
      using default_ops::eval_is_zero;
      return eval_is_zero(m_backend);
   }
   HBOOST_MP_FORCEINLINE int sign()const
   {
...
public:
   ...
   //
#ifndef HBOOST_MP_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS
#  if (defined(__GNUC__) && (__GNUC__ == 4) && (__GNUC_MINOR__ < 7)) || (defined(HBOOST_INTEL) && (HBOOST_INTEL <= 1500))
   //
   ...
   // an arithmetic type:
   //
   template <class T, typename hboost::enable_if<is_same<T, bool>, int>::type = 0>
   explicit operator T ()const
   {
      using default_ops::eval_is_zero;
      return !eval_is_zero(backend());
   }
   template <class T, typename hboost::disable_if_c<is_same<T, bool>::value || is_void<T>::value, int>::type = 0>
   explicit operator T ()const
   {
...
#  else
   ...
   HBOOST_MP_FORCEINLINE explicit operator bool()const
   {
      return !is_zero();
   }
...
#  endif
   ...
   HBOOST_MP_FORCEINLINE operator unmentionable_type()const
   {
   ...
   // Default precision:
   //
   static unsigned default_precision() HBOOST_NOEXCEPT
   {
   ...
   unsigned precision()const HBOOST_NOEXCEPT
   {
   ...
   // Comparison:
   //
   HBOOST_MP_FORCEINLINE int compare(const number<Backend, ExpressionTemplates>& o)const
      HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<Backend>().compare(std::declval<Backend>())))
   {
      return m_backend.compare(o.m_backend);
   }
   template <class V>
   HBOOST_MP_FORCEINLINE typename hboost::enable_if<is_arithmetic<V>, int>::type compare(const V& o)const
   {
      using default_ops::eval_get_sign;
      if(o == 0)
         return eval_get_sign(m_backend);
   ...
   HBOOST_MP_FORCEINLINE Backend& backend() HBOOST_NOEXCEPT
   {
      return m_backend;
   }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const Backend& backend()const HBOOST_NOEXCEPT
   {
      return m_backend;
   }
private:
   ...
   void do_assign(const Exp& e, const detail::bitwise_and&)
   {
      ...
      // This operation is only valid for integer backends:
      //
      HBOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "Bitwise operations are only valid for integer types");
   ...
   void do_assign(const Exp& e, const detail::shift_left&)
   {
      ...
      HBOOST_STATIC_ASSERT_MSG(right_arity::value == 0, "The left shift operator requires an integer value for the shift operand.");
      typedef typename right_type::result_type right_value_type;
      HBOOST_STATIC_ASSERT_MSG(is_integral<right_value_type>::value, "The left shift operator requires an integer value for the shift operand.");
   ...
   void do_assign_function_2(const F& f, const Exp1& val1, const Exp2& val2, const Tag1&, const detail::terminal&)
   {
      ...
      f(m_backend, HBOOST_MP_MOVE(temp1.backend()), function_arg_value(val2));
   }
   ...
   void do_assign_function_2(const F& f, const Exp1& val1, const Exp2& val2, const detail::terminal&, const Tag2&)
   {
      ...
      f(m_backend, function_arg_value(val1), HBOOST_MP_MOVE(temp2.backend()));
   }
   ...
   void do_assign_function_2(const F& f, const Exp1& val1, const Exp2& val2, const Tag1&, const Tag2&)
   {
      self_type temp1(val1);
      self_type temp2(val2);
      f(m_backend, HBOOST_MP_MOVE(temp1.backend()), HBOOST_MP_MOVE(temp2.backend()));
   }
   ...
   {
      number t(val1);
      do_assign_function_3b(f, HBOOST_MP_MOVE(t), val2, val3, t2, t3);
   }
   ...
   {
      number t(val2);
      do_assign_function_3c(f, val1, HBOOST_MP_MOVE(t), val3, t3);
   }
   ...
   {
      number t(val3);
      do_assign_function_3c(f, val1, val2, HBOOST_MP_MOVE(t), detail::terminal());
   }
   ...
   //
   template <class Exp>
   typename hboost::disable_if_c<hboost::multiprecision::number_category<self_type>::value == hboost::multiprecision::number_kind_integer || sizeof(Exp) == 1>::type 
   ...
   template <class Exp>
   HBOOST_MP_FORCEINLINE bool contains_self(const Exp& e)const HBOOST_NOEXCEPT
   {
      return contains_self(e, typename Exp::arity());
   }
   template <class Exp>
   HBOOST_MP_FORCEINLINE bool contains_self(const Exp& e, mpl::int_<0> const&)const HBOOST_NOEXCEPT
   {
      return is_realy_self(e.value());
   }
   template <class Exp>
   HBOOST_MP_FORCEINLINE bool contains_self(const Exp& e, mpl::int_<1> const&)const HBOOST_NOEXCEPT
   {
      typedef typename Exp::left_type child_type;
      return contains_self(e.left(), typename child_type::arity());
   }
   template <class Exp>
   HBOOST_MP_FORCEINLINE bool contains_self(const Exp& e, mpl::int_<2> const&)const HBOOST_NOEXCEPT
   {
   ...
   template <class Exp>
   HBOOST_MP_FORCEINLINE bool contains_self(const Exp& e, mpl::int_<3> const&)const HBOOST_NOEXCEPT
   {
      typedef typename Exp::left_type child0_type;
   ...
   template <class Exp>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool is_self(const Exp& e)const HBOOST_NOEXCEPT
   {
      return is_self(e, typename Exp::arity());
   }
   template <class Exp>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool is_self(const Exp& e, mpl::int_<0> const&)const HBOOST_NOEXCEPT
   {
      return is_realy_self(e.value());
   }
   template <class Exp, int v>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool is_self(const Exp&, mpl::int_<v> const&)const HBOOST_NOEXCEPT
   {
      return false;
   }

   template <class Val>
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool is_realy_self(const Val&)const HBOOST_NOEXCEPT{ return false; }
   HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR bool is_realy_self(const self_type& v)const HBOOST_NOEXCEPT{ return &v == this; }

   static HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const Backend& function_arg_value(const self_type& v) HBOOST_NOEXCEPT {  return v.backend();  }
   template <class V>
   static HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const V& function_arg_value(const V& v) HBOOST_NOEXCEPT {  return v;  }
   template <class A1, class A2, class A3, class A4>
   static HBOOST_MP_FORCEINLINE const A1& function_arg_value(const detail::expression<detail::terminal, A1, A2, A3, A4>& exp) HBOOST_NOEXCEPT { return exp.value(); }
   template <class A2, class A3, class A4>
   static HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const Backend& function_arg_value(const detail::expression<detail::terminal, number<Backend>, A2, A3, A4>& exp) HBOOST_NOEXCEPT { return exp.value().backend(); }
   Backend m_backend;
...
public:
   ...
   //
   static HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const Backend& canonical_value(const self_type& v) HBOOST_NOEXCEPT {  return v.m_backend;  }
   template <class B2, expression_template_option ET>
   static HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR const B2& canonical_value(const number<B2, ET>& v) HBOOST_NOEXCEPT {  return v.backend();  }
   template <class V>
   static HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR typename hboost::disable_if<is_same<typename detail::canonical<V, Backend>::type, V>, typename detail::canonical<V, Backend>::type>::type
      canonical_value(const V& v) HBOOST_NOEXCEPT {  return static_cast<typename detail::canonical<V, Backend>::type>(v);  }
   template <class V>
   static HBOOST_MP_FORCEINLINE HBOOST_CONSTEXPR typename hboost::enable_if<is_same<typename detail::canonical<V, Backend>::type, V>, const V&>::type
      canonical_value(const V& v) HBOOST_NOEXCEPT {  return v;  }
   static HBOOST_MP_FORCEINLINE typename detail::canonical<std::string, Backend>::type canonical_value(const std::string& v) HBOOST_NOEXCEPT {  return v.c_str();  }
...
inline std::istream& operator >> (std::istream& is, number<Backend, ExpressionTemplates>& r)
{
   ...
   std::string s;
   switch(hboost::multiprecision::number_category<number<Backend, ExpressionTemplates> >::value)
   {
   case hboost::multiprecision::number_kind_integer:
      if(oct_format)
      ...
      break;
   case hboost::multiprecision::number_kind_floating_point:
...
template <class Backend, expression_template_option ExpressionTemplates>
HBOOST_MP_FORCEINLINE void swap(number<Backend, ExpressionTemplates>& a, number<Backend, ExpressionTemplates>& b) 
   HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<number<Backend, ExpressionTemplates>&>() = std::declval<number<Backend, ExpressionTemplates>&>()))
{
   a.swap(b);
}
...
typename hboost::enable_if<hboost::is_arithmetic<Arithmetic>, bool>::type operator == (const rational<multiprecision::number<T, ExpressionTemplates> >& a, const Arithmetic& b)
{
   return a == multiprecision::number<T, ExpressionTemplates>(b);
}

template <class T, multiprecision::expression_template_option ExpressionTemplates, class Arithmetic>
typename hboost::enable_if<hboost::is_arithmetic<Arithmetic>, bool>::type operator == (const Arithmetic& b, const rational<multiprecision::number<T, ExpressionTemplates> >& a)
{
   return a == multiprecision::number<T, ExpressionTemplates>(b);
}
...
template <class I>
struct component_type<hboost::rational<I> >
{
   typedef I type;
};
...
#include <hboost/multiprecision/detail/ublas_interop.hpp>
