...
#ifndef HBOOST_MATH_RATIONAL_ADAPTER_HPP
#define HBOOST_MATH_RATIONAL_ADAPTER_HPP
...
struct rational_adaptor
{
   ...
   typedef hboost::rational<integer_type>        rational_type;
   ...
   rational_adaptor() HBOOST_MP_NOEXCEPT_IF(noexcept(rational_type())) {}
   rational_adaptor(const rational_adaptor& o) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<rational_type&>() = std::declval<const rational_type&>()))
   {
      m_value = o.m_value;
   }
   rational_adaptor(const IntBackend& o) HBOOST_MP_NOEXCEPT_IF(noexcept(rational_type(std::declval<const IntBackend&>()))) : m_value(o) {}
   ...
   explicit rational_adaptor(const U& u, 
      typename enable_if_c<
         hboost::multiprecision::detail::is_explicitly_convertible<U, IntBackend>::value && !is_convertible<U, IntBackend>::value
      >::type* = 0) 
      : m_value(IntBackend(u)){}
   template <class U>
   typename enable_if_c<(hboost::multiprecision::detail::is_explicitly_convertible<U, IntBackend>::value && !is_arithmetic<U>::value), rational_adaptor&>::type operator = (const U& u) 
   {
      m_value = IntBackend(u);
   }

#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
   rational_adaptor(rational_adaptor&& o) HBOOST_MP_NOEXCEPT_IF(noexcept(rational_type(std::declval<rational_type>()))) : m_value(static_cast<rational_type&&>(o.m_value)) {}
   rational_adaptor(IntBackend&& o) HBOOST_MP_NOEXCEPT_IF(noexcept(rational_type(std::declval<IntBackend>()))) : m_value(static_cast<IntBackend&&>(o)) {}
   rational_adaptor& operator = (rational_adaptor&& o) HBOOST_MP_NOEXCEPT_IF(noexcept(std::declval<rational_type&>() = std::declval<rational_type>()))
   {
   ...
   rational_adaptor& operator = (const char* s)
   {
      ...
      if(*s)
      {
         HBOOST_THROW_EXCEPTION(std::runtime_error(std::string("Could not parse the string \"") + p + std::string("\" as a valid rational number.")));
      }
      data().assign(v1, v2);
      return *this;
   }
...
inline void eval_divide(rational_adaptor<IntBackend>& result, const rational_adaptor<IntBackend>& o)
{
   ...
   if(eval_is_zero(o))
   {
      HBOOST_THROW_EXCEPTION(std::overflow_error("Divide by zero."));
   }
   result.data() /= o.data();
}
...
inline typename enable_if_c<number_category<R>::value == number_kind_floating_point>::type eval_convert_to(R* result, const rational_adaptor<IntBackend>& backend)
{
   ...
   //
   ::hboost::multiprecision::detail::generic_convert_rational_to_float(*result, backend);
}
...
using hboost::multiprecision::backends::rational_adaptor;
...
template <class IntBackend, hboost::multiprecision::expression_template_option ExpressionTemplates>
class numeric_limits<hboost::multiprecision::number<hboost::multiprecision::rational_adaptor<IntBackend>, ExpressionTemplates> > : public std::numeric_limits<hboost::multiprecision::number<IntBackend, ExpressionTemplates> >
{
   typedef std::numeric_limits<hboost::multiprecision::number<IntBackend> > base_type;
   typedef hboost::multiprecision::number<hboost::multiprecision::rational_adaptor<IntBackend> > number_type;
public:
   HBOOST_STATIC_CONSTEXPR bool is_integer = false;
   HBOOST_STATIC_CONSTEXPR bool is_exact = true;
   HBOOST_STATIC_CONSTEXPR number_type (min)() { return (base_type::min)(); }
   HBOOST_STATIC_CONSTEXPR number_type (max)() { return (base_type::max)(); }
   HBOOST_STATIC_CONSTEXPR number_type lowest() { return -(max)(); }
   HBOOST_STATIC_CONSTEXPR number_type epsilon() { return base_type::epsilon(); }
   HBOOST_STATIC_CONSTEXPR number_type round_error() { return epsilon() / 2; }
   HBOOST_STATIC_CONSTEXPR number_type infinity() { return base_type::infinity(); }
   HBOOST_STATIC_CONSTEXPR number_type quiet_NaN() { return base_type::quiet_NaN(); }
   HBOOST_STATIC_CONSTEXPR number_type signaling_NaN() { return base_type::signaling_NaN(); }
   HBOOST_STATIC_CONSTEXPR number_type denorm_min() { return base_type::denorm_min(); }
};

#ifndef HBOOST_NO_INCLASS_MEMBER_INITIALIZATION

template <class IntBackend, hboost::multiprecision::expression_template_option ExpressionTemplates>
HBOOST_CONSTEXPR_OR_CONST bool numeric_limits<hboost::multiprecision::number<hboost::multiprecision::rational_adaptor<IntBackend>, ExpressionTemplates> >::is_integer;
template <class IntBackend, hboost::multiprecision::expression_template_option ExpressionTemplates>
HBOOST_CONSTEXPR_OR_CONST bool numeric_limits<hboost::multiprecision::number<hboost::multiprecision::rational_adaptor<IntBackend>, ExpressionTemplates> >::is_exact;
