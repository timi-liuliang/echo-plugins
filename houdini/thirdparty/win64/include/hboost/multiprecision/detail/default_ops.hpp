...
#ifndef HBOOST_MATH_BIG_NUM_DEF_OPS
#define HBOOST_MATH_BIG_NUM_DEF_OPS
...
#ifndef HBOOST_MP_INSTRUMENT
#define INSTRUMENT_BACKEND(x)
#else
#define INSTRUMENT_BACKEND(x)\
   std::cout << HBOOST_STRINGIZE(x) << " = " << x.str(0, std::ios_base::scientific) << std::endl;
#endif
#endif


namespace hboost{ namespace multiprecision{ namespace default_ops{
...
// we just use et_on everywhere.  We could use a HBOOST_WORKAROUND but that just obfuscates the
// code even more....
//
...
inline void eval_convert_to(terminal<R>* result, const B& backend)
{
   ...
   //
   result->value = hboost::lexical_cast<R>(backend.str(0, std::ios_base::fmtflags(0)));
}
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The fabs function is only valid for floating point types.");
   typedef typename T::signed_types type_list;
   typedef typename mpl::front<type_list>::type front;
   result = arg;
   if(arg.compare(front(0)) < 0)
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_floating_point, "The fpclassify function is only valid for floating point types.");
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The fmod function is only valid for floating point types.");
...
{
   typedef typename hboost::multiprecision::detail::canonical<A, T>::type canonical_type;
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The trunc function is only valid for floating point types.");
   int c = eval_fpclassify(a);
   if(c == (int)FP_NAN || c == (int)FP_INFINITE)
   {
      result = hboost::math::policies::raise_rounding_error("hboost::multiprecision::trunc<%1%>(%1%)", 0, number<T>(a), number<T>(a), hboost::math::policies::policy<>()).backend();
      return;
   }
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The round function is only valid for floating point types.");
   typedef typename hboost::multiprecision::detail::canonical<float, T>::type fp_type;
   int c = eval_fpclassify(a);
   if((c == (int)FP_NAN) || (c == (int)FP_INFINITE))
   {
      result = hboost::math::policies::raise_rounding_error("hboost::multiprecision::round<%1%>(%1%)", 0, number<T>(a), number<T>(a), hboost::math::policies::policy<>()).backend();
      return;
   }
   if(eval_get_sign(a) < 0)
   {
...
{
   typedef typename hboost::multiprecision::detail::canonical<Arithmetic, T>::type si_type;
...
{
   typedef typename hboost::multiprecision::detail::canonical<unsigned, T>::type ui_type;
   int c = eval_get_sign(val);
   if(c == 0)
   {
      HBOOST_THROW_EXCEPTION(std::range_error("No bits were set in the operand."));
   }
   if(c < 0)
   {
      HBOOST_THROW_EXCEPTION(std::range_error("Testing individual bits in negative values is not supported - results are undefined."));
   }
   unsigned result = 0;
   T mask, t;
   mask = ui_type(1);
...
void eval_integer_sqrt(B& s, B& r, const B& x)
{
   ...
   // at some point.
   //
   typedef typename hboost::multiprecision::detail::canonical<unsigned char, B>::type ui_type;
...
{
   HBOOST_STATIC_ASSERT_MSG(!std::numeric_limits<number<B> >::is_specialized || (std::numeric_limits<number<B> >::radix == 2), "The default implementation of ilogb requires a base 2 number type");
...
{
   typedef typename hboost::mpl::if_c<hboost::is_same<hboost::intmax_t, long>::value, hboost::long_long_type, hboost::intmax_t>::type max_t;
   result = static_cast<max_t>(eval_ilogb(val));
}
template <class B, class A>
inline void eval_scalbn(B& result, const B& val, A e)
{
   HBOOST_STATIC_ASSERT_MSG(!std::numeric_limits<number<B> >::is_specialized || (std::numeric_limits<number<B> >::radix == 2), "The default implementation of scalbn requires a base 2 number type");
...
//
#include <hboost/multiprecision/detail/functions/constants.hpp>
#include <hboost/multiprecision/detail/functions/pow.hpp>
#include <hboost/multiprecision/detail/functions/trig.hpp>
...
inline int fpclassify HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::number<Backend, ExpressionTemplates>& arg)
{
   using multiprecision::default_ops::eval_fpclassify;
   return eval_fpclassify(arg.backend());
}
template <class tag, class A1, class A2, class A3, class A4>
inline int fpclassify HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::detail::expression<tag, A1, A2, A3, A4>& arg)
{
   typedef typename multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type value_type;
   return (fpclassify)(value_type(arg));
}
template <class Backend, multiprecision::expression_template_option ExpressionTemplates>
inline bool isfinite HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::number<Backend, ExpressionTemplates>& arg)
{
   int v = (fpclassify)(arg);
   return (v != (int)FP_INFINITE) && (v != (int)FP_NAN);
}
template <class tag, class A1, class A2, class A3, class A4>
inline bool isfinite HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::detail::expression<tag, A1, A2, A3, A4>& arg)
{
   typedef typename multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type value_type;
   return (isfinite)(value_type(arg));
}
template <class Backend, multiprecision::expression_template_option ExpressionTemplates>
inline bool isnan HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::number<Backend, ExpressionTemplates>& arg)
{
   return (fpclassify)(arg) == (int)FP_NAN;
}
template <class tag, class A1, class A2, class A3, class A4>
inline bool isnan HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::detail::expression<tag, A1, A2, A3, A4>& arg)
{
   typedef typename multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type value_type;
   return (isnan)(value_type(arg));
}
template <class Backend, multiprecision::expression_template_option ExpressionTemplates>
inline bool isinf HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::number<Backend, ExpressionTemplates>& arg)
{
   return (fpclassify)(arg) == (int)FP_INFINITE;
}
template <class tag, class A1, class A2, class A3, class A4>
inline bool isinf HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::detail::expression<tag, A1, A2, A3, A4>& arg)
{
   typedef typename multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type value_type;
   return (isinf)(value_type(arg));
}
template <class Backend, multiprecision::expression_template_option ExpressionTemplates>
inline bool isnormal HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::number<Backend, ExpressionTemplates>& arg)
{
   return (fpclassify)(arg) == (int)FP_NORMAL;
}
template <class tag, class A1, class A2, class A3, class A4>
inline bool isnormal HBOOST_PREVENT_MACRO_SUBSTITUTION(const multiprecision::detail::expression<tag, A1, A2, A3, A4>& arg)
{
   typedef typename multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type value_type;
...
{
   HBOOST_STATIC_ASSERT_MSG((is_convertible<B2, B1>::value), "No conversion to the target of a mixed precision addition exists");
   HBOOST_STATIC_ASSERT_MSG((is_convertible<B3, B1>::value), "No conversion to the target of a mixed precision addition exists");
...
inline typename detail::expression<tag, A1, A2, A3, A4>::result_type trunc(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   ...
   return HBOOST_MP_MOVE(trunc(number_type(v), pol));
}
...
inline int itrunc(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<int>::max)()) || r < (std::numeric_limits<int>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::itrunc<%1%>(%1%)", 0, number_type(v), 0, pol);
   return r.template convert_to<int>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline int itrunc(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return itrunc(v, hboost::math::policies::policy<>());
}
template <class Backend, expression_template_option ExpressionTemplates, class Policy>
inline int itrunc(const number<Backend, ExpressionTemplates>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<int>::max)()) || r < (std::numeric_limits<int>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::itrunc<%1%>(%1%)", 0, v, 0, pol);
   return r.template convert_to<int>();
}
...
inline long ltrunc(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   typedef typename detail::expression<tag, A1, A2, A3, A4>::result_type number_type;
   number_type r = trunc(v, pol);
   if((r > (std::numeric_limits<long>::max)()) || r < (std::numeric_limits<long>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::ltrunc<%1%>(%1%)", 0, number_type(v), 0L, pol);
   return r.template convert_to<long>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline long ltrunc(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return ltrunc(v, hboost::math::policies::policy<>());
}
...
inline long ltrunc(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<long>::max)()) || r < (std::numeric_limits<long>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::ltrunc<%1%>(%1%)", 0, v, 0L, pol);
   return r.template convert_to<long>();
}
...
template <class tag, class A1, class A2, class A3, class A4, class Policy>
inline hboost::long_long_type lltrunc(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   typedef typename detail::expression<tag, A1, A2, A3, A4>::result_type number_type;
   number_type r = trunc(v, pol);
   if((r > (std::numeric_limits<hboost::long_long_type>::max)()) || r < (std::numeric_limits<hboost::long_long_type>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::lltrunc<%1%>(%1%)", 0, number_type(v), 0LL, pol);
   return r.template convert_to<hboost::long_long_type>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline hboost::long_long_type lltrunc(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return lltrunc(v, hboost::math::policies::policy<>());
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
inline hboost::long_long_type lltrunc(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   number<T, ExpressionTemplates> r = trunc(v, pol);
   if((r > (std::numeric_limits<hboost::long_long_type>::max)()) || r < (std::numeric_limits<hboost::long_long_type>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::lltrunc<%1%>(%1%)", 0, v, 0LL, pol);
   return r.template convert_to<hboost::long_long_type>();
}
template <class T, expression_template_option ExpressionTemplates>
inline hboost::long_long_type lltrunc(const number<T, ExpressionTemplates>& v)
{
   return lltrunc(v, hboost::math::policies::policy<>());
}
#endif
...
{
   typedef typename detail::expression<tag, A1, A2, A3, A4>::result_type number_type;
   return HBOOST_MP_MOVE(round(static_cast<number_type>(v), pol));
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
...
inline int iround(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<int>::max)()) || r < (std::numeric_limits<int>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::iround<%1%>(%1%)", 0, number_type(v), 0, pol);
   return r.template convert_to<int>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline int iround(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return iround(v, hboost::math::policies::policy<>());
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
inline int iround(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<int>::max)()) || r < (std::numeric_limits<int>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::iround<%1%>(%1%)", 0, v, 0, pol);
   return r.template convert_to<int>();
}
template <class T, expression_template_option ExpressionTemplates>
...
inline long lround(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   ...
   number_type r = round(v, pol);
   if((r > (std::numeric_limits<long>::max)()) || r < (std::numeric_limits<long>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::lround<%1%>(%1%)", 0, number_type(v), 0L, pol);
   return r.template convert_to<long>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline long lround(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return lround(v, hboost::math::policies::policy<>());
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
inline long lround(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   number<T, ExpressionTemplates> r = round(v, pol);
   if((r > (std::numeric_limits<long>::max)()) || r < (std::numeric_limits<long>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::lround<%1%>(%1%)", 0, v, 0L, pol);
   return r.template convert_to<long>();
}
template <class T, expression_template_option ExpressionTemplates>
...
#ifndef HBOOST_NO_LONG_LONG
template <class tag, class A1, class A2, class A3, class A4, class Policy>
inline hboost::long_long_type llround(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   typedef typename detail::expression<tag, A1, A2, A3, A4>::result_type number_type;
   number_type r = round(v, pol);
   if((r > (std::numeric_limits<hboost::long_long_type>::max)()) || r < (std::numeric_limits<hboost::long_long_type>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::iround<%1%>(%1%)", 0, number_type(v), 0LL, pol);
   return r.template convert_to<hboost::long_long_type>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline hboost::long_long_type llround(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return llround(v, hboost::math::policies::policy<>());
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
inline hboost::long_long_type llround(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   number<T, ExpressionTemplates> r = round(v, pol);
   if((r > (std::numeric_limits<hboost::long_long_type>::max)()) || r < (std::numeric_limits<hboost::long_long_type>::min)() || !(hboost::math::isfinite)(v))
      return hboost::math::policies::raise_rounding_error("hboost::multiprecision::iround<%1%>(%1%)", 0, v, 0LL, pol);
   return r.template convert_to<hboost::long_long_type>();
}
template <class T, expression_template_option ExpressionTemplates>
inline hboost::long_long_type llround(const number<T, ExpressionTemplates>& v)
{
   return llround(v, hboost::math::policies::policy<>());
}
#endif
...
{
   typedef typename detail::expression<tag, A1, A2, A3, A4>::result_type number_type;
   return HBOOST_MP_MOVE(frexp(static_cast<number_type>(v), pint));
}
template <class T, expression_template_option ExpressionTemplates>
...
template <class T, expression_template_option ExpressionTemplates>
inline typename enable_if_c<number_category<T>::value == number_kind_floating_point, number<T, ExpressionTemplates> >::type frexp(const number<T, ExpressionTemplates>& v, hboost::long_long_type* pint)
{
   using default_ops::eval_frexp;
   number<T, ExpressionTemplates> result;
   eval_frexp(result.backend(), v.backend(), pint);
...
template <class tag, class A1, class A2, class A3, class A4>
inline typename enable_if_c<number_category<typename detail::expression<tag, A1, A2, A3, A4>::result_type>::value == number_kind_floating_point, typename detail::expression<tag, A1, A2, A3, A4>::result_type>::type
frexp(const detail::expression<tag, A1, A2, A3, A4>& v, hboost::long_long_type* pint)
{
   typedef typename detail::expression<tag, A1, A2, A3, A4>::result_type number_type;
   return HBOOST_MP_MOVE(frexp(static_cast<number_type>(v), pint));
}
...
namespace detail{\
template <class Backend> \
struct HBOOST_JOIN(func, _funct)\
{\
   void operator()(Backend& result, const Backend& arg)const\
   {\
      using default_ops::HBOOST_JOIN(eval_,func);\
      HBOOST_JOIN(eval_,func)(result, arg);\
   }\
};\
\
}\
...
inline typename enable_if_c<number_category<detail::expression<tag, A1, A2, A3, A4> >::value == category,\
   detail::expression<\
    detail::function\
  , detail::HBOOST_JOIN(func, _funct)<typename detail::backend_type<detail::expression<tag, A1, A2, A3, A4> >::type> \
...
inline typename enable_if_c<number_category<Backend>::value == category,\
   detail::expression<\
    detail::function\
  , detail::HBOOST_JOIN(func, _funct)<Backend> \
...
template <class Backend> \
inline typename hboost::enable_if_c<\
   hboost::multiprecision::number_category<Backend>::value == category,\
   number<Backend, et_off> >::type \
func(const number<Backend, et_off>& arg)\
{\
   number<Backend, et_off> result;\
   using default_ops::HBOOST_JOIN(eval_,func);\
   HBOOST_JOIN(eval_,func)(result.backend(), arg.backend());\
   return HBOOST_MP_MOVE(result);\
}
...
{\
   ...
   {\
      using default_ops:: HBOOST_JOIN(eval_,func);\
      HBOOST_JOIN(eval_,func)(result, arg, a);\
   }\
...
{\
   number<Backend, et_off> result;\
   using default_ops:: HBOOST_JOIN(eval_,func);\
   HBOOST_JOIN(eval_,func)(result.backend(), arg.backend(), a.backend());\
   return HBOOST_MP_MOVE(result);\
}\
template <class Backend, class Arithmetic> \
inline typename enable_if_c<\
...
{\
   ...
   number<Backend, et_off> result;\
   using default_ops:: HBOOST_JOIN(eval_,func);\
   HBOOST_JOIN(eval_,func)(result.backend(), arg.backend(), static_cast<canonical_type>(a));\
   return HBOOST_MP_MOVE(result);\
}\
template <class Backend, class Arithmetic> \
inline typename enable_if_c<\
...
{\
   ...
   number<Backend, et_off> result;\
   using default_ops:: HBOOST_JOIN(eval_,func);\
   HBOOST_JOIN(eval_,func)(result.backend(), static_cast<canonical_type>(a), arg.backend());\
   return HBOOST_MP_MOVE(result);\
}\
...
{\
   number<Backend, et_off> result;\
   using default_ops:: HBOOST_JOIN(eval_,func);\
   HBOOST_JOIN(eval_,func)(result.backend(), arg.backend(), a);\
   return HBOOST_MP_MOVE(result);\
}\
...
HETERO_BINARY_OP_FUNCTOR_B(ldexp, hboost::long_long_type, number_kind_floating_point)
//HETERO_BINARY_OP_FUNCTOR_B(frexp, hboost::long_long_type*, number_kind_floating_point)
...
HETERO_BINARY_OP_FUNCTOR_B(scalbn, hboost::long_long_type, number_kind_floating_point)
...
{
   return HBOOST_MP_MOVE(detail::sinc_pi_imp(x));
}
...
{
   return HBOOST_MP_MOVE(detail::sinhc_pi_imp(x));
}
...
{
   return HBOOST_MP_MOVE(hboost::math::sinhc_pi(x));
}
...
//
#include <hboost/multiprecision/detail/no_et_ops.hpp>
#include <hboost/multiprecision/detail/et_ops.hpp>
//
// min/max overloads:
//
#include <hboost/multiprecision/detail/min_max.hpp>
