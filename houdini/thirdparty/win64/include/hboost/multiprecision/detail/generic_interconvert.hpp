...
#ifndef HBOOST_MP_GENERIC_INTERCONVERT_HPP
#define HBOOST_MP_GENERIC_INTERCONVERT_HPP

#include <hboost/multiprecision/detail/default_ops.hpp>
...
template <class To, class B, ::hboost::multiprecision::expression_template_option et>
...
#pragma warning(disable:4127)
   ...
   static const int shift = std::numeric_limits<hboost::intmax_t>::digits - 1;
   ...
   while(!eval_is_zero(f))
   {
      ...
      typename hboost::multiprecision::detail::canonical<hboost::intmax_t, To>::type ll;
   ...
   generic_convert_rational_to_float_imp(To& result, Integer& num, Integer& denom, const mpl::false_&)
{
   ...
   if(shift > 0)
      ...
      denom <<= hboost::multiprecision::detail::unsigned_abs(shift);
   Integer q, r;
   ...
   {
      HBOOST_ASSERT(q_bits == 1 + std::numeric_limits<To>::digits);
      //
...
void generic_interconvert_float2rational(To& to, const From& from, const mpl::int_<2>& /*radix*/)
{
   ...
   static const int shift = std::numeric_limits<hboost::long_long_type>::digits;
   typename From::exponent_type e;
   ...
   while(val)
   {
      val = ldexp(val, shift);
      e -= shift;
      hboost::long_long_type ll = hboost::math::lltrunc(val);
      val -= ll;
      num <<= shift;
      num += ll;
   }
...
#endif  // HBOOST_MP_GENERIC_INTERCONVERT_HPP
