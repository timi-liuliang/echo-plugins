...
#ifndef HBOOST_MATH_BIG_NUM_BASE_HPP
#define HBOOST_MATH_BIG_NUM_BASE_HPP
...
#  define HBOOST_MP_FORCEINLINE HBOOST_FORCEINLINE
#else
#  define HBOOST_MP_FORCEINLINE inline
#endif

#if (defined(HBOOST_GCC) && (HBOOST_GCC <= 40700)) || defined(__SUNPRO_CC)
#  define HBOOST_MP_NOEXCEPT_IF(x)
#else
#  define HBOOST_MP_NOEXCEPT_IF(x) HBOOST_NOEXCEPT_IF(x)
#endif

#if defined(HBOOST_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS) || defined(__SUNPRO_CC)
#define HBOOST_MP_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS
#endif
...
namespace detail{
//
// Workaround for missing abs(hboost::long_long_type) and abs(__int128) on some compilers:
//
template <class T>
HBOOST_CONSTEXPR typename enable_if_c<(is_signed<T>::value || is_floating_point<T>::value), T>::type abs(T t) HBOOST_NOEXCEPT
{
   // This strange expression avoids a hardware trap in the corner case
   // that val is the most negative value permitted in hboost::long_long_type.
   // See https://svn.boost.org/trac/hboost/ticket/9740.
   return t < 0 ? T(1u) + T(-(t + 1)) : t;
}
template <class T>
HBOOST_CONSTEXPR typename enable_if_c<(is_unsigned<T>::value), T>::type abs(T t) HBOOST_NOEXCEPT
{
   return t;
}

#define HBOOST_MP_USING_ABS using hboost::multiprecision::detail::abs;

template <class T>
HBOOST_CONSTEXPR typename enable_if_c<(is_signed<T>::value || is_floating_point<T>::value), typename make_unsigned<T>::type>::type unsigned_abs(T t) HBOOST_NOEXCEPT
{
   // This strange expression avoids a hardware trap in the corner case
   // that val is the most negative value permitted in hboost::long_long_type.
   // See https://svn.boost.org/trac/hboost/ticket/9740.
...
template <class T>
HBOOST_CONSTEXPR typename enable_if_c<(is_unsigned<T>::value), T>::type unsigned_abs(T t) HBOOST_NOEXCEPT
{
   return t;
}
...
#  define HBOOST_MP_MOVE(x) std::move(x)
#else
#  define HBOOST_MP_MOVE(x) x
#endif

template <class T>
struct bits_of
{
   HBOOST_STATIC_ASSERT(is_integral<T>::value || is_enum<T>::value || std::numeric_limits<T>::is_specialized);
...
struct canonical_imp<Val, Backend, mpl::int_<0> >
{
   ...
   typedef typename mpl::eval_if<hboost::is_same<iter_type, end_type>, mpl::identity<Val>, mpl::deref<iter_type> >::type type;
};
template <class Val, class Backend>
...
struct expression<tag, Arg1, void, void, void>
{
   ...
   const Arg1& left_ref()const HBOOST_NOEXCEPT { return arg; }

   static const unsigned depth = left_type::depth + 1;
#ifndef HBOOST_MP_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS
#  if (defined(__GNUC__) && (__GNUC__ == 4) && (__GNUC_MINOR__ < 7) && !defined(__clang__)) || (defined(HBOOST_INTEL) && (HBOOST_INTEL <= 1500))
   //
   ...
   template <class T, typename hboost::disable_if_c<is_same<T, bool>::value || is_void<T>::value || is_number<T>::value, int>::type = 0>
   explicit operator T ()const
   {
      return static_cast<T>(static_cast<result_type>(*this));
   }
#  else
   template <class T, typename hboost::disable_if_c<is_number<T>::value || is_constructible<T const&, result_type>::value, int>::type = 0>
   explicit operator T()const
   {
      return static_cast<T>(static_cast<result_type>(*this));
   }
...
struct expression<terminal, Arg1, void, void, void>
{
   ...
   const Arg1& value()const HBOOST_NOEXCEPT { return arg; }
...
struct expression<tag, Arg1, Arg2, void, void>
{
   ...
   const Arg1& left_ref()const HBOOST_NOEXCEPT { return arg1; }
   const Arg2& right_ref()const HBOOST_NOEXCEPT { return arg2; }
...
struct expression<tag, Arg1, Arg2, Arg3, void>
{
   ...
   const Arg1& left_ref()const HBOOST_NOEXCEPT { return arg1; }
   const Arg2& middle_ref()const HBOOST_NOEXCEPT { return arg2; }
   const Arg3& right_ref()const HBOOST_NOEXCEPT { return arg3; }
...
struct expression
{
   ...
   const Arg1& left_ref()const HBOOST_NOEXCEPT { return arg1; }
   const Arg2& left_middle_ref()const HBOOST_NOEXCEPT { return arg2; }
   const Arg3& right_middle_ref()const HBOOST_NOEXCEPT { return arg3; }
   const Arg4& right_ref()const HBOOST_NOEXCEPT { return arg4; }
...
struct digits2
{
   ...
   HBOOST_STATIC_ASSERT(LONG_MAX / 1000 > (std::numeric_limits<T>::digits + 1));
...
#ifndef HBOOST_MP_MIN_EXPONENT_DIGITS
#ifdef _MSC_VER
#  define HBOOST_MP_MIN_EXPONENT_DIGITS 2
#else
#  define HBOOST_MP_MIN_EXPONENT_DIGITS 2
#endif
#endif
...
template <class S>
void format_float_string(S& str, hboost::intmax_t my_exp, hboost::intmax_t digits, std::ios_base::fmtflags f, bool iszero)
{
   ...
   else if(!fixed || (my_exp >= 0))
   {
      ...
      //
      hboost::intmax_t chars = str.size();
      chars = digits - chars;
      if(scientific)
         ++chars;
      if(chars > 0)
   ...
   {
      if(1 + my_exp > static_cast<hboost::intmax_t>(str.size()))
      {
      ...
      else if(my_exp + 1 < static_cast<hboost::intmax_t>(str.size()))
      {
         if(my_exp < 0)
         {
   ...
   else
   {
      ...
      S e = hboost::lexical_cast<S>(abs(my_exp));
      if(e.size() < HBOOST_MP_MIN_EXPONENT_DIGITS)
         e.insert(static_cast<std::string::size_type>(0), HBOOST_MP_MIN_EXPONENT_DIGITS - e.size(), '0');
      if(my_exp < 0)
...
void check_shift_range(V val, const mpl::true_&, const mpl::true_&)
{
   if(val > (std::numeric_limits<std::size_t>::max)())
      HBOOST_THROW_EXCEPTION(std::out_of_range("Can not shift by a value greater than std::numeric_limits<std::size_t>::max()."));
   if(val < 0)
      HBOOST_THROW_EXCEPTION(std::out_of_range("Can not shift by a negative value."));
}
template <class V>
...
template <class V>
void check_shift_range(V, const mpl::false_&, const mpl::false_&) HBOOST_NOEXCEPT{}
...
template <class tag, class A1, class A2, class A3, class A4>
struct promote_arg<hboost::multiprecision::detail::expression<tag, A1, A2, A3, A4> >
{
   typedef typename hboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type type;
};

template <class R, class B, hboost::multiprecision::expression_template_option ET>
inline R real_cast(const hboost::multiprecision::number<B, ET>& val)
{
   return val.template convert_to<R>();
}

template <class R, class tag, class A1, class A2, class A3, class A4>
inline R real_cast(const hboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>& val)
{
   typedef typename hboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type val_type;
...
namespace constants{
   ...
   template <class B, hboost::multiprecision::expression_template_option ET>
   struct is_explicitly_convertible_from_string<hboost::multiprecision::number<B, ET> >
   {
...
#endif // HBOOST_MATH_BIG_NUM_BASE_HPP
