...
{
   typedef typename hboost::multiprecision::detail::canonical<hboost::int32_t, T>::type si_type;
   typedef typename hboost::multiprecision::detail::canonical<hboost::uint32_t, T>::type ui_type;
   ...
   T tol;
   tol = ui_type(1);
   eval_ldexp(tol, tol, 1 - hboost::multiprecision::detail::digits2<number<T, et_on> >::value);
   ...
   static const int series_limit = 
      hboost::multiprecision::detail::digits2<number<T, et_on> >::value < 100
      ? 100 : hboost::multiprecision::detail::digits2<number<T, et_on> >::value;
   ...
   if(n >= series_limit)
      HBOOST_THROW_EXCEPTION(std::runtime_error("H0F1 Failed to Converge"));
}
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The sin function is only valid for floating point types.");
   if(&result == &x)
   {
      T temp;
      eval_sin(temp, x);
   ...
   case FP_NAN:
      if(std::numeric_limits<number<T, et_on> >::has_quiet_NaN)
         ...
         HBOOST_THROW_EXCEPTION(std::domain_error("Result is undefined or complex and there is no NaN for this number type."));
      return;
   case FP_ZERO:
      result = ui_type(0);
      return;
   ...
   if(xx.compare(get_constant_pi<T>()) > 0)
   {
      ...
      HBOOST_MATH_INSTRUMENT_CODE(xx.str(0, std::ios_base::scientific));
      HBOOST_MATH_INSTRUMENT_CODE(n_pi.str(0, std::ios_base::scientific));
   ...
   else if(b_near_zero)
   {
      ...
      t2 = fp_type(1.5);
      hyp0F1(result, t2, t);
      HBOOST_MATH_INSTRUMENT_CODE(result.str(0, std::ios_base::scientific));
   ...
   else
   {
      ...
      for(hboost::int32_t k = static_cast<hboost::int32_t>(0); k < n_scale; k++)
      {
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The cos function is only valid for floating point types.");
   if(&result == &x)
   {
      T temp;
      eval_cos(temp, x);
   ...
   if(xx.compare(get_constant_pi<T>()) > 0)
   {
      ...
      HBOOST_MATH_INSTRUMENT_CODE(t.str(0, std::ios_base::scientific));
      eval_subtract(xx, t);
      HBOOST_MATH_INSTRUMENT_CODE(xx.str(0, std::ios_base::scientific));
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The tan function is only valid for floating point types.");
   if(&result == &x)
   {
      T temp;
      eval_tan(temp, x);
...
void hyp2F1(T& result, const T& a, const T& b, const T& c, const T& x)
{
  ...
  // There are no checks on input range or parameter boundaries.
   ...
   T lim;
   eval_ldexp(lim, result, 1 - hboost::multiprecision::detail::digits2<number<T, et_on> >::value);
   ...
   if(n > series_limit)
      HBOOST_THROW_EXCEPTION(std::runtime_error("H2F1 failed to converge."));
}

template <class T>
void eval_asin(T& result, const T& x)
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The asin function is only valid for floating point types.");
   typedef typename hboost::multiprecision::detail::canonical<hboost::uint32_t, T>::type ui_type;
   typedef typename mpl::front<typename T::float_types>::type fp_type;
   ...
   typedef typename hboost::multiprecision::detail::canonical<long double, T>::type guess_type;
#else
   ...
   unsigned target_precision = hboost::multiprecision::detail::digits2<number<T, et_on> >::value;
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The acos function is only valid for floating point types.");
   typedef typename hboost::multiprecision::detail::canonical<hboost::uint32_t, T>::type ui_type;
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The atan function is only valid for floating point types.");
   typedef typename hboost::multiprecision::detail::canonical<hboost::int32_t, T>::type si_type;
   typedef typename hboost::multiprecision::detail::canonical<hboost::uint32_t, T>::type ui_type;
   typedef typename mpl::front<typename T::float_types>::type fp_type;
   ...
   // Newton-Raphson iteration
   static const hboost::int32_t double_digits10_minus_a_few = std::numeric_limits<double>::digits10 - 3;

   T s, c, t;
   for(hboost::int32_t digits = double_digits10_minus_a_few; digits <= std::numeric_limits<number<T, et_on> >::digits10; digits *= 2)
   {
      eval_sin(s, result);
      eval_cos(c, result);
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The atan2 function is only valid for floating point types.");
   if(&result == &y)
   {
      T temp(y);
