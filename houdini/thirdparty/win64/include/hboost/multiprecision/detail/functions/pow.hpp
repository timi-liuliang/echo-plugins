...
inline void pow_imp(T& result, const T& t, const U& p, const mpl::false_&)
{
   ...
   typedef typename hboost::multiprecision::detail::canonical<U, T>::type int_type;
...
{
   detail::pow_imp(result, t, p, hboost::is_signed<U>());
}
...
void hyp0F0(T& H0F0, const T& x)
{
   ...
   HBOOST_ASSERT(&H0F0 != &x);
   long tol = hboost::multiprecision::detail::digits2<number<T, et_on> >::value;
   T t;
   ...
   if(n >= series_limit)
      HBOOST_THROW_EXCEPTION(std::runtime_error("H0F0 failed to converge"));
}
...
void hyp1F0(T& H1F0, const T& a, const T& x)
{
   ...
   typedef typename hboost::multiprecision::detail::canonical<int, T>::type si_type;

   HBOOST_ASSERT(&H1F0 != &x);
   HBOOST_ASSERT(&H1F0 != &a);
   ...
   T lim;
   eval_ldexp(lim, H1F0, 1 - hboost::multiprecision::detail::digits2<number<T, et_on> >::value);
   if(eval_get_sign(lim) < 0)
      lim.negate();
   ...
   if(n >= series_limit)
      HBOOST_THROW_EXCEPTION(std::runtime_error("H1F0 failed to converge"));
}

template <class T>
void eval_exp(T& result, const T& x)
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The exp function is only valid for floating point types.");
   if(&x == &result)
   {
      T temp;
      eval_exp(temp, x);
   ...
   typedef typename hboost::multiprecision::detail::canonical<exp_type, T>::type canonical_exp_type;
   ...
   typename hboost::multiprecision::detail::canonical<hboost::intmax_t, T>::type ll;
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The log function is only valid for floating point types.");
   //
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The log10 function is only valid for floating point types.");
   eval_log(result, arg);
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The pow function is only valid for floating point types.");
   typedef typename hboost::multiprecision::detail::canonical<int, T>::type si_type;
   typedef typename mpl::front<typename T::float_types>::type fp_type;
   ...
   typename hboost::multiprecision::detail::canonical<hboost::intmax_t, T>::type an;
   T fa;
   ...
   catch(const std::exception&)
   {
      ...
      an = (std::numeric_limits<hboost::intmax_t>::max)();
   }
#endif
   if((eval_get_sign(x) < 0))
   {
      typename hboost::multiprecision::detail::canonical<hboost::uintmax_t, T>::type aun;
#ifndef HBOOST_NO_EXCEPTIONS
      try
      {
#endif
      ...
      {
         HBOOST_THROW_EXCEPTION(std::domain_error("Result of pow is undefined or non-real and there is no NaN for this number type."));
      }
      return;
   }
...
namespace detail{
   ...
   void small_sinh_series(T x, T& result)
   {
      ...
      T lim(x);
      eval_ldexp(lim, lim, 1 - hboost::multiprecision::detail::digits2<number<T, et_on> >::value);
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The sinh function is only valid for floating point types.");
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The cosh function is only valid for floating point types.");
...
{
   HBOOST_STATIC_ASSERT_MSG(number_category<T>::value == number_kind_floating_point, "The tanh function is only valid for floating point types.");
  T c;
