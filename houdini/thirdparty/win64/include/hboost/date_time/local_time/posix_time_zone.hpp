#ifndef HBOOST__DATE_TIME_POSIX_TIME_ZONE__
#define HBOOST__DATE_TIME_POSIX_TIME_ZONE__
...
#include <hboost/date_time/gregorian/gregorian.hpp>
#include <hboost/date_time/time_zone_names.hpp>
#include <hboost/date_time/time_zone_base.hpp>
#include <hboost/date_time/local_time/dst_transition_day_rules.hpp>
#include <hboost/date_time/posix_time/posix_time.hpp>
#include <hboost/date_time/string_convert.hpp>
#include <hboost/date_time/time_parsing.hpp>
...
namespace local_time{
  ...
  typedef hboost::date_time::dst_adjustment_offsets<hboost::posix_time::time_duration> dst_adjustment_offsets;
   ...
   * A hboost::local_time::bad_offset exception will be thrown for:<br>
   ...
   * A hboost::local_time::bad_adjustment exception will be thrown for:<br>
  ...
  public:
    typedef hboost::posix_time::time_duration time_duration_type;
    ...
    typedef hboost::char_separator<char_type, std::char_traits<char_type> > char_separator_type;
    typedef hboost::tokenizer<char_separator_type,
      ...
      dst_calc_rules_()
    {
      ...
      if (it == end)
        HBOOST_THROW_EXCEPTION(std::invalid_argument("Could not parse time zone name"));
      calc_zone(*it++);
      if(has_dst_)
      {
        if (it == end)
          HBOOST_THROW_EXCEPTION(std::invalid_argument("Could not parse DST begin time"));
        string_type dst_begin = *it++;

        if (it == end)
          HBOOST_THROW_EXCEPTION(std::invalid_argument("Could not parse DST end time"));
    ...
    virtual string_type to_posix_string() const
    {
      ...
      ss.fill('0');
      hboost::shared_ptr<dst_calc_rule> no_rules;
      // std
      ss << std_zone_abbrev();
      // offset
  ...
  private:
    ...
    hboost::shared_ptr<dst_calc_rule> dst_calc_rules_;
    ...
    void calc_zone(const string_type& obj){
      ...
      if(sit != obj_end){
        ...
        {
          hboost::throw_exception(bad_offset(posix_time::to_simple_string(base_utc_offset_)));
        }
      }
      ...
      if(sit != obj_end){
        ...
        {
          hboost::throw_exception(bad_adjustment(posix_time::to_simple_string(dst_offsets_.dst_adjust_)));
        }
      }
    ...
    void calc_rules(const string_type& start, const string_type& end){
      ...
      {
        hboost::throw_exception(bad_offset(posix_time::to_simple_string(dst_offsets_.dst_start_offset_)));
      }
      ...
      {
        hboost::throw_exception(bad_offset(posix_time::to_simple_string(dst_offsets_.dst_end_offset_)));
      }
    }
...
} } // namespace hboost::local_time


#endif // HBOOST__DATE_TIME_POSIX_TIME_ZONE__
