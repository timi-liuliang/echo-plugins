#ifndef HBOOST_DATE_TIME_C_TIME_HPP___
#define HBOOST_DATE_TIME_C_TIME_HPP___
...
namespace date_time {
  ...
  struct c_time {
    public:
#if defined(HBOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS)
	...
	#endif
        if (!result)
          hboost::throw_exception(std::runtime_error("could not convert calendar time to local time"));
        return result;
      }
      //! requires a pointer to a user created std::tm struct
      inline
	...
	#endif
        if (!result)
          hboost::throw_exception(std::runtime_error("could not convert calendar time to UTC time"));
        return result;
      }
#else // HBOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS
...
#endif // HBOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS
  };
}} // namespaces

#endif // HBOOST_DATE_TIME_C_TIME_HPP___
