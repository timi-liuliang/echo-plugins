#ifndef HBOOST_POSIX_TIME_CONVERSION_HPP___
#define HBOOST_POSIX_TIME_CONVERSION_HPP___
...
#include <hboost/date_time/c_time.hpp>
#include <hboost/date_time/time_resolution_traits.hpp> // absolute_value
#include <hboost/date_time/gregorian/conversion.hpp>
...
namespace posix_time {
  ...
  inline
  std::tm to_tm(const hboost::posix_time::ptime& t) {
    std::tm timetm = hboost::gregorian::to_tm(t.date());
    hboost::posix_time::time_duration td = t.time_of_day();
  ...
  inline
  std::tm to_tm(const hboost::posix_time::time_duration& td) {
    std::tm timetm;
  ...
  ptime ptime_from_tm(const std::tm& timetm) {
    hboost::gregorian::date d = hboost::gregorian::date_from_tm(timetm);
    return ptime(d, time_duration(timetm.tm_hour, timetm.tm_min, timetm.tm_sec));
  }


#if defined(HBOOST_HAS_FTIME)
  ...
  {
    return hboost::date_time::time_from_ftime<TimeT>(ft);
  }

#endif // HBOOST_HAS_FTIME

} } //namespace hboost::posix_time
