...
#include "hboost/date_time/gregorian/greg_serialize.hpp"
...
HBOOST_SERIALIZATION_SPLIT_FREE(hboost::posix_time::ptime)
HBOOST_SERIALIZATION_SPLIT_FREE(hboost::posix_time::time_duration)
HBOOST_SERIALIZATION_SPLIT_FREE(hboost::posix_time::time_period)
          ...
          unsigned int /*version*/)
{
  ...
  if(!d.is_special()) {
    ar & make_nvp("ptime_time_duration", td);
    pt = hboost::posix_time::ptime(d,td);
  }
  else {
    pt = hboost::posix_time::ptime(d.as_special());
  }
                                ...
                                const unsigned int /*file_version*/)
{
  ...
  new(pt) hboost::posix_time::ptime(hboost::posix_time::not_a_date_time);
}
...
template<class Archive>
void load(Archive & ar, 
          hboost::posix_time::time_period & tp, 
          unsigned int /*version*/)
{
  ...
  ar & make_nvp("time_period_begin", beg);
  ar & make_nvp("time_period_end", end);
  tp = hboost::posix_time::time_period(beg, end);
}
                                ...
                                const unsigned int /*file_version*/)
{
  ...
  posix_time::ptime beg(d,td);
  posix_time::ptime end(d,td);
  new(tp) hboost::posix_time::time_period(beg,end);
}
