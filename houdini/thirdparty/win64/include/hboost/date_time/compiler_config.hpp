#ifndef HBOOST_DATE_TIME_COMPILER_CONFIG_HPP___
#define HBOOST_DATE_TIME_COMPILER_CONFIG_HPP___
...
//     (defined(__BORLANDC__) && (__BORLANDC__ <= 0x0581) ) )   &&
    ...
    || (HBOOST_WORKAROUND( _MSC_VER, <= 1300) )                  \
    )                                                           \
...
#include <hboost/date_time/locale_config.hpp> //set up locale configurations

//Set up a configuration parameter for platforms that have 
//GetTimeOfDay
#if defined(HBOOST_HAS_GETTIMEOFDAY) || defined(HBOOST_HAS_FTIME)
#define HBOOST_DATE_TIME_HAS_HIGH_PRECISION_CLOCK
#endif
...
#define HBOOST_DATE_TIME_OPTIONAL_GREGORIAN_TYPES
// these extensions are known to cause problems with gcc295
#if defined(__GNUC__) && (__GNUC__ < 3)
#undef HBOOST_DATE_TIME_OPTIONAL_GREGORIAN_TYPES
#endif

#if (defined(HBOOST_NO_INCLASS_MEMBER_INITIALIZATION) || HBOOST_WORKAROUND( __BORLANDC__,  HBOOST_TESTED_AT(0x581) ) )
#define HBOOST_DATE_TIME_NO_MEMBER_INIT
#endif
...
#if defined(__BORLANDC__) && defined(HBOOST_BCB_WITH_RW_LIB)
#define HBOOST_DATE_TIME_NO_WISTREAM_ITERATOR
#endif


// Borland v5.64 does not have the following in std namespace; v5.5.1 does
#if defined(__BORLANDC__) && defined(HBOOST_BCB_WITH_STLPORT)
#include <locale>
namespace std {
...
#define HBOOST_DATE_TIME_INCLUDE_LIMITED_HEADERS
#endif

// The macro marks up places where compiler complains for missing return statement or
// uninitialized variables after calling to hboost::throw_exception.
// HBOOST_UNREACHABLE_RETURN doesn't work since even compilers that support
// unreachable statements detection emit such warnings.
#if defined(_MSC_VER)
// Use special MSVC extension to markup unreachable code
#  define HBOOST_DATE_TIME_UNREACHABLE_EXPRESSION(x) __assume(false)
#elif !defined(HBOOST_NO_UNREACHABLE_RETURN_DETECTION)
// Call to a non-returning function should suppress the warning
#  if defined(HBOOST_NO_STDC_NAMESPACE)
namespace std {
    using ::abort;
}
#  endif // defined(HBOOST_NO_STDC_NAMESPACE)
#  define HBOOST_DATE_TIME_UNREACHABLE_EXPRESSION(x) std::abort()
#else
// For other poor compilers the specified expression is compiled. Usually, this would be a return statement.
#  define HBOOST_DATE_TIME_UNREACHABLE_EXPRESSION(x) x
#endif
 ...
 * it must be properly prefixed with HBOOST_DATE_TIME_DECL.
 * The corresponding .cpp file must have HBOOST_DATE_TIME_SOURCE
...
// libraries to be dynamically linked, or HBOOST_DATE_TIME_DYN_LINK
// if they want just this one to be dynamically liked:
#if defined(HBOOST_ALL_DYN_LINK) || defined(HBOOST_DATE_TIME_DYN_LINK)
    // export if this is our own source, otherwise import:
#   ifdef HBOOST_DATE_TIME_SOURCE
#     define HBOOST_DATE_TIME_DECL HBOOST_SYMBOL_EXPORT
#   else
#     define HBOOST_DATE_TIME_DECL HBOOST_SYMBOL_IMPORT
#   endif  // HBOOST_DATE_TIME_SOURCE
#endif  // DYN_LINK
//
// if HBOOST_WHATEVER_DECL isn't defined yet define it now:
#ifndef HBOOST_DATE_TIME_DECL
#  define HBOOST_DATE_TIME_DECL
#endif

//
// Automatically link to the correct build variant where possible. 
// 
#if !defined(HBOOST_ALL_NO_LIB) && !defined(HBOOST_DATE_TIME_NO_LIB) && !defined(HBOOST_DATE_TIME_SOURCE)
//
// Set the name of our library, this will get undef'ed by auto_link.hpp
// once it's done with it:
//
#define HBOOST_LIB_NAME hboost_date_time
//
...
#   define HBOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS
#  endif
#endif
