...
#ifndef HBOOST_MOVE_DEFAULT_DELETE_HPP_INCLUDED
#define HBOOST_MOVE_DEFAULT_DELETE_HPP_INCLUDED
...
namespace bmupmu = ::hboost::move_upmu;
...
#if !defined(HBOOST_NO_CXX11_NULLPTR) && !defined(HBOOST_NO_CXX11_DECLTYPE)
   typedef decltype(nullptr) nullptr_type;
#elif !defined(HBOOST_NO_CXX11_NULLPTR)
...
namespace bmupd = hboost::move_upd;
namespace bmupmu = ::hboost::move_upmu;
...
struct default_delete
{
   //! Default constructor.
   //!
   HBOOST_CONSTEXPR default_delete()
   //Avoid "defaulted on its first declaration must not have an exception-specification" error for GCC 4.6
   #if !defined(HBOOST_GCC) || (HBOOST_GCC < 40600 && HBOOST_GCC >= 40700) || defined(HBOOST_MOVE_DOXYGEN_INVOKED)
   HBOOST_NOEXCEPT
   #endif
   #if !defined(HBOOST_NO_CXX11_DEFAULTED_FUNCTIONS) || defined(HBOOST_MOVE_DOXYGEN_INVOKED)
   = default;
   #else
   {};
   #endif
   ...
   //!
   default_delete(const default_delete&) HBOOST_NOEXCEPT = default;
   //! Trivial assignment
   //!
   default_delete &operator=(const default_delete&) HBOOST_NOEXCEPT = default;
   #else
   ...
   default_delete(const default_delete<U>&
      HBOOST_MOVE_DOCIGN(HBOOST_MOVE_I typename bmupd::enable_def_del<U HBOOST_MOVE_I T>::type* =0)
      ) HBOOST_NOEXCEPT
   {
      //If T is not an array type, U derives from T
      //and T has no virtual destructor, then you have a problem
      HBOOST_STATIC_ASSERT(( !::hboost::move_upmu::missing_virtual_destructor<default_delete, U>::value ));
   }
   ...
   //!   - If T is an array type and U* is a more CV qualified pointer to remove_extent<T>::type.
   template <class U>
   HBOOST_MOVE_DOC1ST(default_delete&, 
      typename bmupd::enable_def_del<U HBOOST_MOVE_I T HBOOST_MOVE_I default_delete &>::type)
      operator=(const default_delete<U>&) HBOOST_NOEXCEPT
   {
      //If T is not an array type, U derives from T
      //and T has no virtual destructor, then you have a problem
      HBOOST_STATIC_ASSERT(( !::hboost::move_upmu::missing_virtual_destructor<default_delete, U>::value ));
   ...
   template <class U>
   HBOOST_MOVE_DOC1ST(void, typename bmupd::enable_defdel_call<U HBOOST_MOVE_I T HBOOST_MOVE_I void>::type)
      operator()(U* ptr) const HBOOST_NOEXCEPT
   {
      //U must be a complete type
      HBOOST_STATIC_ASSERT(sizeof(U) > 0);
      //If T is not an array type, U derives from T
      //and T has no virtual destructor, then you have a problem
      HBOOST_STATIC_ASSERT(( !::hboost::move_upmu::missing_virtual_destructor<default_delete, U>::value ));
   ...
   //!
   void operator()(HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type)) const HBOOST_NOEXCEPT
   {  HBOOST_STATIC_ASSERT(sizeof(element_type) > 0);  }
};
...
#endif   //#ifndef HBOOST_MOVE_DEFAULT_DELETE_HPP_INCLUDED
