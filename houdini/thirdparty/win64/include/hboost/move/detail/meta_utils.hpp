...
#ifndef HBOOST_MOVE_DETAIL_META_UTILS_HPP
#define HBOOST_MOVE_DETAIL_META_UTILS_HPP
...
{
   T & v_;
   HBOOST_MOVE_FORCEINLINE addr_impl_ref( T & v ): v_( v ) {}
   HBOOST_MOVE_FORCEINLINE operator T& () const { return v_; }
...
{
   HBOOST_MOVE_FORCEINLINE static T * f( T & v, long )
   {
      return reinterpret_cast<T*>(
         &const_cast<char&>(reinterpret_cast<const volatile char &>(v)));
   }

   HBOOST_MOVE_FORCEINLINE static T * f( T * v, int )
   {  return v;  }
};

template<class T>
HBOOST_MOVE_FORCEINLINE T * addressof( T & v )
{
   return ::hboost::move_detail::addressof_impl<T>::f
      ( ::hboost::move_detail::addr_impl_ref<T>( v ), 0 );
}
...
#if defined(HBOOST_GCC) && (HBOOST_GCC <= 40000)
#define HBOOST_MOVE_HELPERS_RETURN_SFINAE_BROKEN
#endif
...
template<class T>
struct has_move_emulation_enabled_impl< ::hboost::rv<T> >
{  static const bool value = false;  };
...
#else // #ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES

template< class T >
struct is_rvalue_reference< hboost::rv<T>& >
{  static const bool value = true;  };

template< class T >
struct is_rvalue_reference< const hboost::rv<T>& >
{  static const bool value = true;  };

#endif // #ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
...
namespace detail_add_rvalue_reference
{
   ...
   struct add_rvalue_reference_impl< T, true, rv > { typedef ::hboost::rv<T>& type; };
} // namespace detail_add_rvalue_reference
...
#endif //#ifndef HBOOST_MOVE_DETAIL_META_UTILS_HPP
