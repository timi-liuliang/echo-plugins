...
#ifndef HBOOST_MOVE_ADL_MOVE_SWAP_HPP
#define HBOOST_MOVE_ADL_MOVE_SWAP_HPP
...
//Try to avoid including <algorithm>, as it's quite big
   ...
   #if !defined(HBOOST_GCC) || ((__GNUC__ < 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ < 3)))
...
#include <hboost/move/utility_core.hpp> //for hboost::move

#if !defined(HBOOST_MOVE_DOXYGEN_INVOKED)

#if defined(HBOOST_NO_CXX11_RVALUE_REFERENCES)
namespace hboost_move_member_swap {
...
namespace hboost_move_adl_swap{
...
template<class T>
HBOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y, typename hboost::move_detail::enable_if_c<!hboost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
template<class T>
HBOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename hboost::move_detail::enable_if< and_op_not_impl<hboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , hboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::hboost::move(x)); x = ::hboost::move(y); y = ::hboost::move(t);  }

template<class T>
HBOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename hboost::move_detail::enable_if< and_op_impl< hboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , hboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
#endif   //#if defined(HBOOST_NO_CXX11_RVALUE_REFERENCES)
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::hboost_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
//!      <code>T t(::hboost::move(x)); x = ::hboost::move(y); y = ::hboost::move(t);</code>.
template<class T>
HBOOST_MOVE_FORCEINLINE void adl_move_swap(T& x, T& y)
{
   ::hboost_move_adl_swap::swap_proxy(x, y);
}

//! Exchanges elements between range [first1, last1) and another range starting at first2
//! using hboost::adl_move_swap.
//! 
//! Parameters:
...
{
    while (first1 != last1) {
      ::hboost::adl_move_swap(*first1, *first2);
      ++first1;
      ++first2;
    }
   return first2;
...
{
   while (first1 != last1) {
      ::hboost::adl_move_swap(*(--last1), *(--last2));
   }
   return last2;
}
...
#endif   //#ifndef HBOOST_MOVE_ADL_MOVE_SWAP_HPP
