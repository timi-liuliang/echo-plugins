...
#ifndef HBOOST_MOVE_UNIQUE_PTR_HPP_INCLUDED
#define HBOOST_MOVE_UNIQUE_PTR_HPP_INCLUDED
...
#include <hboost/move/default_delete.hpp>
...
class is_noncopyable
{
   ...
   template<class U> static true_t  dispatch(typename U::hboost_move_no_copy_constructor_or_assign*);
   public:
...
struct deleter_types
{
   ...
   typedef ::hboost::rv<D> &                                       deleter_arg_type2;
   #endif
};
...
struct unique_ptr_data
{
   ...
   HBOOST_MOVE_FORCEINLINE unique_ptr_data() HBOOST_NOEXCEPT
      : m_p(), d()
   {}

   HBOOST_MOVE_FORCEINLINE explicit unique_ptr_data(P p) HBOOST_NOEXCEPT
      : m_p(p), d()
   {}

   HBOOST_MOVE_FORCEINLINE unique_ptr_data(P p, deleter_arg_type1 d1) HBOOST_NOEXCEPT
      : m_p(p), d(d1)
   {}

   template <class U>
   HBOOST_MOVE_FORCEINLINE unique_ptr_data(P p, HBOOST_FWD_REF(U) d1) HBOOST_NOEXCEPT
      : m_p(p), d(::hboost::forward<U>(d1))
   {}

   HBOOST_MOVE_FORCEINLINE del_ref deleter()       { return d; }
   HBOOST_MOVE_FORCEINLINE del_cref deleter() const{ return d; }
   ...
   : private D
{
   ...
   template <class U>
   HBOOST_MOVE_FORCEINLINE unique_ptr_data(P p, HBOOST_FWD_REF(U) d) HBOOST_NOEXCEPT
      : D(::hboost::forward<U>(d)), m_p(p)
   {}

   HBOOST_MOVE_FORCEINLINE del_ref deleter()        HBOOST_NOEXCEPT   {  return static_cast<del_ref>(*this);   }
   HBOOST_MOVE_FORCEINLINE del_cref deleter() const HBOOST_NOEXCEPT   {  return static_cast<del_cref>(*this);  }
...
class is_rvalue_convertible
{
   ...
   #else
   typedef typename bmupmu::if_c
      < ::hboost::has_move_emulation_enabled<T>::value && !bmupmu::is_reference<T>::value
      , ::hboost::rv<T>&
...
class unique_ptr
{
   ...
   private:
   #else
   HBOOST_MOVABLE_BUT_NOT_COPYABLE(unique_ptr)
   ...
   //! synonym for T*.
   typedef typename HBOOST_MOVE_SEEDOC(pointer_type_obtainer::type) pointer;
   //! If T is an array type, then element_type is equal to T. Otherwise, if T is a type
   //! in the form U[], element_type is equal to U.
   typedef typename HBOOST_MOVE_SEEDOC(bmupmu::remove_extent<T>::type) element_type;
   ...
   HBOOST_MOVE_FORCEINLINE HBOOST_CONSTEXPR unique_ptr() HBOOST_NOEXCEPT
      : m_data()
   {
      //If this constructor is instantiated with a pointer type or reference type
      //for the template argument D, the program is ill-formed.
      HBOOST_STATIC_ASSERT(!bmupmu::is_pointer<D>::value);
      HBOOST_STATIC_ASSERT(!bmupmu::is_reference<D>::value);
   }

   //! <b>Effects</b>: Same as <tt>unique_ptr()</tt> (default constructor).
   //! 
   HBOOST_MOVE_FORCEINLINE HBOOST_CONSTEXPR unique_ptr(HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type)) HBOOST_NOEXCEPT
      : m_data()
   {
      //If this constructor is instantiated with a pointer type or reference type
      //for the template argument D, the program is ill-formed.
   ...
   HBOOST_MOVE_FORCEINLINE explicit unique_ptr(Pointer p
      HBOOST_MOVE_DOCIGN(HBOOST_MOVE_I typename bmupd::enable_up_ptr<T HBOOST_MOVE_I Pointer HBOOST_MOVE_I pointer>::type* =0)
                 ) HBOOST_NOEXCEPT
      : m_data(p)
   {
      ...
      HBOOST_STATIC_ASSERT(( !::hboost::move_upmu::missing_virtual_destructor
   ...
   //!      - If T is an array type and Pointer is a more CV qualified pointer to element_type.
   template<class Pointer>
   HBOOST_MOVE_FORCEINLINE unique_ptr(Pointer p, HBOOST_MOVE_SEEDOC(deleter_arg_type1) d1
      HBOOST_MOVE_DOCIGN(HBOOST_MOVE_I typename bmupd::enable_up_ptr<T HBOOST_MOVE_I Pointer HBOOST_MOVE_I pointer>::type* =0)
              ) HBOOST_NOEXCEPT
      : m_data(p, d1)
   {
   ...
   HBOOST_MOVE_FORCEINLINE unique_ptr(HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type), HBOOST_MOVE_SEEDOC(deleter_arg_type1) d1) HBOOST_NOEXCEPT
      : m_data(pointer(), d1)
   {}
   ...
   //!      - If T is an array type and Pointer is a more CV qualified pointer to element_type.
   template<class Pointer>
   HBOOST_MOVE_FORCEINLINE unique_ptr(Pointer p, HBOOST_MOVE_SEEDOC(deleter_arg_type2) d2
      HBOOST_MOVE_DOCIGN(HBOOST_MOVE_I typename bmupd::enable_up_ptr<T HBOOST_MOVE_I Pointer HBOOST_MOVE_I pointer>::type* =0)
             ) HBOOST_NOEXCEPT
      : m_data(p, ::hboost::move(d2))
   {
      //If T is not an array type, element_type_t<Pointer> derives from T
      //it uses the default deleter and T has no virtual destructor, then you have a problem
      HBOOST_STATIC_ASSERT(( !::hboost::move_upmu::missing_virtual_destructor
   ...
   //!   and additionally <tt>get() == nullptr</tt>
   HBOOST_MOVE_FORCEINLINE unique_ptr(HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type), HBOOST_MOVE_SEEDOC(deleter_arg_type2) d2) HBOOST_NOEXCEPT
      : m_data(pointer(), ::hboost::move(d2))
   {}
   ...
   HBOOST_MOVE_FORCEINLINE unique_ptr(HBOOST_RV_REF(unique_ptr) u) HBOOST_NOEXCEPT
      : m_data(u.release(), ::hboost::move_if_not_lvalue_reference<D>(u.get_deleter()))
   {}
   ...
   HBOOST_MOVE_FORCEINLINE unique_ptr( HBOOST_RV_REF_BEG_IF_CXX11 unique_ptr<U, E> HBOOST_RV_REF_END_IF_CXX11 u
      HBOOST_MOVE_DOCIGN(HBOOST_MOVE_I typename bmupd::enable_up_moveconv_constr<T HBOOST_MOVE_I D HBOOST_MOVE_I U HBOOST_MOVE_I E>::type* =0)
      ) HBOOST_NOEXCEPT
      : m_data(u.release(), ::hboost::move_if_not_lvalue_reference<E>(u.get_deleter()))
   {
   ...
   //!
   //! <b>Returns</b>: *this.
   unique_ptr& operator=(HBOOST_RV_REF(unique_ptr) u) HBOOST_NOEXCEPT
   {
      this->reset(u.release());
      m_data.deleter() = ::hboost::move_if_not_lvalue_reference<D>(u.get_deleter());
      return *this;
   }
   ...
   //! <b>Returns</b>: *this.
   template <class U, class E>
   HBOOST_MOVE_DOC1ST(unique_ptr&, typename bmupd::enable_up_moveconv_assign
         <T HBOOST_MOVE_I D HBOOST_MOVE_I U HBOOST_MOVE_I E HBOOST_MOVE_I unique_ptr &>::type)
      operator=(HBOOST_RV_REF_BEG unique_ptr<U, E> HBOOST_RV_REF_END u) HBOOST_NOEXCEPT
   {
      this->reset(u.release());
      m_data.deleter() = ::hboost::move_if_not_lvalue_reference<E>(u.get_deleter());
      return *this;
   }
   ...
   //!
   //! <b>Returns</b>: *this.
   unique_ptr& operator=(HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type)) HBOOST_NOEXCEPT
   ...
   HBOOST_MOVE_DOC1ST(element_type&, typename bmupmu::add_lvalue_reference<element_type>::type)
      operator*() const HBOOST_NOEXCEPT
   {
      HBOOST_STATIC_ASSERT((!bmupmu::is_array<T>::value));
      return *m_data.m_p;
   }
   ...
   HBOOST_MOVE_FORCEINLINE HBOOST_MOVE_DOC1ST(element_type&, typename bmupmu::add_lvalue_reference<element_type>::type)
      operator[](std::size_t i) const HBOOST_NOEXCEPT
   {
      HBOOST_ASSERT( bmupmu::extent<T>::value == 0 || i < bmupmu::extent<T>::value );
      HBOOST_ASSERT(m_data.m_p);
      return m_data.m_p[i];
   }
   ...
   //!
   //! <b>Remarks</b: If T is an array type, the program is ill-formed.
   HBOOST_MOVE_FORCEINLINE pointer operator->() const HBOOST_NOEXCEPT
   {
      HBOOST_STATIC_ASSERT((!bmupmu::is_array<T>::value));
      HBOOST_ASSERT(m_data.m_p);
      return m_data.m_p;
   ...
   //!
   HBOOST_MOVE_FORCEINLINE pointer get() const HBOOST_NOEXCEPT
   {  return m_data.m_p;  }

   //! <b>Returns</b>: A reference to the stored deleter.
   //!
   HBOOST_MOVE_FORCEINLINE HBOOST_MOVE_DOC1ST(D&, typename bmupmu::add_lvalue_reference<D>::type)
      get_deleter() HBOOST_NOEXCEPT
   {  return m_data.deleter();  }   

   //! <b>Returns</b>: A reference to the stored deleter.
   //!
   HBOOST_MOVE_FORCEINLINE HBOOST_MOVE_DOC1ST(const D&, typename bmupmu::add_const_lvalue_reference<D>::type)
      get_deleter() const HBOOST_NOEXCEPT
   {  return m_data.deleter();  }

   #ifdef HBOOST_MOVE_DOXYGEN_INVOKED
   //! <b>Returns</b>: Returns: get() != nullptr.
   //!
   HBOOST_MOVE_FORCEINLINE explicit operator bool
   #else
   HBOOST_MOVE_FORCEINLINE operator bmupd::explicit_bool_arg
   #endif
   ...
   HBOOST_MOVE_FORCEINLINE pointer release() HBOOST_NOEXCEPT
   {
   ...
   //!      - If T is an array type and Pointer is a more CV qualified pointer to element_type.
   template<class Pointer>
   HBOOST_MOVE_DOC1ST(void, typename bmupd::enable_up_ptr<T HBOOST_MOVE_I Pointer HBOOST_MOVE_I pointer HBOOST_MOVE_I void>::type)
      reset(Pointer p) HBOOST_NOEXCEPT
   {
      //If T is not an array type, element_type_t<Pointer> derives from T
      //it uses the default deleter and T has no virtual destructor, then you have a problem
      HBOOST_STATIC_ASSERT(( !::hboost::move_upmu::missing_virtual_destructor
   ...
   //! <b>Postconditions</b>: <tt>get() == p</tt>. Note: The postcondition does not hold if the call to <tt>get_deleter()</tt>
   //!   destroys *this since <tt>this->get()</tt> is no longer a valid expression.
   void reset() HBOOST_NOEXCEPT
   {  this->reset(pointer());  }

   //! <b>Effects</b>: Same as <tt>reset()</tt>
   //! 
   void reset(HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type)) HBOOST_NOEXCEPT
   {  this->reset(); }
   ...
   void swap(unique_ptr& u) HBOOST_NOEXCEPT
   {
      ::hboost::adl_move_swap(m_data.m_p, u.m_data.m_p);
      ::hboost::adl_move_swap(m_data.deleter(), u.m_data.deleter());
   }
};
...
HBOOST_MOVE_FORCEINLINE void swap(unique_ptr<T, D> &x, unique_ptr<T, D> &y) HBOOST_NOEXCEPT
{  x.swap(y); }

//! <b>Returns</b>: <tt>x.get() == y.get()</tt>.
//!
template <class T1, class D1, class T2, class D2>
HBOOST_MOVE_FORCEINLINE bool operator==(const unique_ptr<T1, D1> &x, const unique_ptr<T2, D2> &y)
{  return x.get() == y.get(); }
...
//!
template <class T, class D>
HBOOST_MOVE_FORCEINLINE bool operator==(const unique_ptr<T, D> &x, HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type)) HBOOST_NOEXCEPT
{  return !x;  }

//! <b>Returns</b>:!x.
//!
template <class T, class D>
HBOOST_MOVE_FORCEINLINE bool operator==(HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type), const unique_ptr<T, D> &x) HBOOST_NOEXCEPT
{  return !x;  }
...
template <class T, class D>
HBOOST_MOVE_FORCEINLINE bool operator<(const unique_ptr<T, D> &x, HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type))
...
template <class T, class D>
HBOOST_MOVE_FORCEINLINE bool operator<(HBOOST_MOVE_DOC0PTR(bmupd::nullptr_type), const unique_ptr<T, D> &x)
...
#endif   //#ifndef HBOOST_MOVE_UNIQUE_PTR_HPP_INCLUDED
