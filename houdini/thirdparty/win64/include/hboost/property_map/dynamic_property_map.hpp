...
namespace detail {
  ...
  { return hboost::lexical_cast<Value>(value); }
...
public:
  ...
  virtual hboost::any get(const any& key) = 0;
...
typename hboost::property_traits<PMap>::reference
...
class dynamic_property_map_adaptor : public dynamic_property_map
{
  ...
  {
    HBOOST_THROW_EXCEPTION(dynamic_const_put_error());
  }
...
public:
  ...
  virtual hboost::any get(const any& key_)
  {
    return get_wrapper_xxx(property_map_, any_cast<typename hboost::property_traits<PropertyMap>::key_type>(key_));
  }

  virtual std::string get_string(const any& key_)
  {
    std::ostringstream out;
    out << get_wrapper_xxx(property_map_, any_cast<typename hboost::property_traits<PropertyMap>::key_type>(key_));
    return out.str();
  }
...
{
  typedef std::multimap<std::string, hboost::shared_ptr<dynamic_property_map> >
    property_maps_type;
  typedef hboost::function3<hboost::shared_ptr<dynamic_property_map>,
                           const std::string&,
                           const hboost::any&,
                           const hboost::any&> generate_fn_type;
public:
  ...
  {
    hboost::shared_ptr<dynamic_property_map> pm(
      hboost::static_pointer_cast<dynamic_property_map>(
        hboost::make_shared<detail::dynamic_property_map_adaptor<PropertyMap> >(property_map_)));
  ...
  void
  insert(const std::string& name, hboost::shared_ptr<dynamic_property_map> pm)
  {
    property_maps.insert(property_maps_type::value_type(name, pm));
  }

  template<typename Key, typename Value>
  hboost::shared_ptr<dynamic_property_map>
  generate(const std::string& name, const Key& key, const Value& value)
  {
    if(!generate_fn) {
      HBOOST_THROW_EXCEPTION(property_not_found(name));
    } else {
    ...
    const Value& value)
{
  ...
  hboost::shared_ptr<dynamic_property_map> new_map = dp.generate(name, key, value);
  if (new_map.get()) {
...
get(const std::string& name, const dynamic_properties& dp, const Key& key)
{
  ...
  HBOOST_THROW_EXCEPTION(dynamic_get_failure(name));
}
...
inline
hboost::shared_ptr<hboost::dynamic_property_map>
ignore_other_properties(const std::string&,
                        const hboost::any&,
                        const hboost::any&) {
  return hboost::shared_ptr<hboost::dynamic_property_map>();
}
