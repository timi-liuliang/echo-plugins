...
#ifndef HBOOST_PARALLEL_LOCAL_PROPERTY_MAP_HPP
#define HBOOST_PARALLEL_LOCAL_PROPERTY_MAP_HPP
...
namespace hboost {
  ...
  public:
    ...
    reference operator[](const key_type& key)
    {
      ...
      HBOOST_ASSERT(p.first == process_id(process_group_));
  ...
  {
    typename property_traits<GlobalMap>::value_type p = get(pm.global(), key);
    HBOOST_ASSERT(p.first == process_id(pm.process_group()));
    put(pm.base(), p.second, v);
  }
} // end namespace hboost
#endif // HBOOST_PARALLEL_LOCAL_PROPERTY_MAP_HPP
