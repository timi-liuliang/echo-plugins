...
#include <hboost/property_map/parallel/simple_trigger.hpp>
...
void PBGL_DISTRIB_PMAP::prune_ghost_cells() const
{
  ...
  while (data->ghost_cells->size() > data->max_ghost_cells) {
    ...
    if (data->model & cm_flush) {
      ...
      hboost::parallel::detail::untracked_pair<key_type, value_type> const& victim
...
handle_put(int /*source*/, int /*tag*/, 
           const hboost::parallel::detail::untracked_pair<key_type, value_type>& req, trigger_receive_context)
{
  using hboost::get;
  ...
  HBOOST_ASSERT(p.first == process_id(data->process_group));
                ...
                trigger_receive_context)
{
  ...
  typedef hboost::parallel::detail::untracked_pair<key_type, value_type> key_value;
...
PBGL_DISTRIB_PMAP::set_max_ghost_cells(std::size_t max_ghost_cells)
{
  if ((data->model & cm_backward) && max_ghost_cells > 0)
      hboost::throw_exception(std::runtime_error("distributed_property_map::set_max_ghost_cells: "
