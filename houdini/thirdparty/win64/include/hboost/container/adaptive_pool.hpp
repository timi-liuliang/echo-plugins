...
#ifndef HBOOST_CONTAINER_ADAPTIVE_POOL_HPP
#define HBOOST_CONTAINER_ADAPTIVE_POOL_HPP
...
#include <hboost/container/detail/adaptive_node_pool.hpp>
...
template < class T
         , std::size_t NodesPerBlock   HBOOST_CONTAINER_DOCONLY(= ADP_nodes_per_block)
         , std::size_t MaxFreeBlocks   HBOOST_CONTAINER_DOCONLY(= ADP_max_free_blocks)
         , std::size_t OverheadPercent HBOOST_CONTAINER_DOCONLY(= ADP_overhead_percent)
         HBOOST_CONTAINER_DOCIGN(HBOOST_MOVE_I unsigned Version)
         >
class adaptive_pool
{
   ...
   typedef adaptive_pool
      <T, NodesPerBlock, MaxFreeBlocks, OverheadPercent
         HBOOST_CONTAINER_DOCIGN(HBOOST_MOVE_I Version)
         >   self_t;
   ...
   HBOOST_CONTAINER_DOCIGN(HBOOST_STATIC_ASSERT((Version <=2)));
   ...
   public:
   //!Default constructor
   adaptive_pool() HBOOST_NOEXCEPT_OR_NOTHROW
   {}

   //!Copy constructor from other adaptive_pool.
   adaptive_pool(const adaptive_pool &) HBOOST_NOEXCEPT_OR_NOTHROW
   {}
   ...
   adaptive_pool
      (const adaptive_pool<T2, NodesPerBlock, MaxFreeBlocks, OverheadPercent
            HBOOST_CONTAINER_DOCIGN(HBOOST_MOVE_I Version)> &) HBOOST_NOEXCEPT_OR_NOTHROW
   {}
   ...
   void allocate_many(size_type elem_size, std::size_t n_elements, multiallocation_chain &chain)
   {
      ...
      chain.incorporate_after(chain.before_begin()
                             ,(T*)HBOOST_CONTAINER_MEMCHAIN_FIRSTMEM(&ch)
                             ,(T*)HBOOST_CONTAINER_MEMCHAIN_LASTMEM(&ch)
                             ,HBOOST_CONTAINER_MEMCHAIN_SIZE(&ch) );*/
      if(HBOOST_UNLIKELY(!dlmalloc_multialloc_nodes
   ...
   void allocate_many(const size_type *elem_sizes, size_type n_elements, multiallocation_chain &chain)
   {
      ...
      dlmalloc_memchain ch;
      HBOOST_CONTAINER_MEMCHAIN_INIT(&ch);
      if(HBOOST_UNLIKELY(!dlmalloc_multialloc_arrays(n_elements, elem_sizes, sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, &ch))){
         hboost::container::throw_bad_alloc();
      }
      chain.incorporate_after(chain.before_begin()
                             ,(T*)HBOOST_CONTAINER_MEMCHAIN_FIRSTMEM(&ch)
                             ,(T*)HBOOST_CONTAINER_MEMCHAIN_LASTMEM(&ch)
                             ,HBOOST_CONTAINER_MEMCHAIN_SIZE(&ch) );*/
      if(HBOOST_UNLIKELY(!dlmalloc_multialloc_arrays
   ...
   {/*
      ...
      HBOOST_CONTAINER_MEMCHAIN_INIT_FROM(&ch, beg, last, size);
   ...
   friend void swap(adaptive_pool &, adaptive_pool &) HBOOST_NOEXCEPT_OR_NOTHROW
   {}

   //!An allocator always compares to true, as memory allocated with one
   //!instance can be deallocated by another instance
   friend bool operator==(const adaptive_pool &, const adaptive_pool &) HBOOST_NOEXCEPT_OR_NOTHROW
   {  return true;   }
      ...
      ,size_type &prefer_in_recvd_out_size, pointer &reuse_ptr)
   {
      ...
      if(HBOOST_UNLIKELY(limit_size > this->max_size() || preferred_size > this->max_size())){
         return pointer();
      }
...
#endif   //#ifndef HBOOST_CONTAINER_ADAPTIVE_POOL_HPP
