...
#ifndef HBOOST_CONTAINER_DETAIL_NODE_ALLOC_HPP_
#define HBOOST_CONTAINER_DETAIL_NODE_ALLOC_HPP_
...
#include <hboost/intrusive/options.hpp>
// move
...
HBOOST_INTRUSIVE_INSTANTIATE_DEFAULT_TYPE_TMPLT(value_compare)
HBOOST_INTRUSIVE_INSTANTIATE_DEFAULT_TYPE_TMPLT(predicate_type)
...
struct node_alloc_holder
{
   ...
   typedef HBOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT
      ( hboost::container::container_detail::
   ...
   private:
   HBOOST_COPYABLE_AND_MOVABLE(node_alloc_holder)
   ...
   explicit node_alloc_holder(HBOOST_RV_REF(node_alloc_holder) x)
      : members_(hboost::move(x.node_alloc()))
   ...
   explicit node_alloc_holder(HBOOST_RV_REF(node_alloc_holder) x, const value_compare &c)
      : members_(hboost::move(x.node_alloc()), c)
   {  this->icont().swap(x.icont());  }
   ...
   NodePtr create_node(Args &&...args)
   {
      ...
      allocator_traits<NodeAlloc>::construct
         ( this->node_alloc()
         , container_detail::addressof(p->m_data), hboost::forward<Args>(args)...);
      node_deallocator.release();
      //This does not throw
      typedef typename Node::hook_type hook_type;
      ::new(static_cast<hook_type*>(container_detail::to_raw_pointer(p)), hboost_container_new_t()) hook_type;
      return (p);
   }

   #else //defined(HBOOST_NO_CXX11_VARIADIC_TEMPLATES)

   #define HBOOST_CONTAINER_NODE_ALLOC_HOLDER_CONSTRUCT_IMPL(N) \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   NodePtr create_node(HBOOST_MOVE_UREF##N)\
   {\
      NodePtr p = this->allocate_one();\
      Deallocator node_deallocator(p, this->node_alloc());\
      allocator_traits<NodeAlloc>::construct\
         ...
         , container_detail::addressof(p->m_data)\
          HBOOST_MOVE_I##N HBOOST_MOVE_FWD##N);\
      node_deallocator.release();\
      typedef typename Node::hook_type hook_type;\
      ::new(static_cast<hook_type*>(container_detail::to_raw_pointer(p)), hboost_container_new_t()) hook_type;\
      return (p);\
   ...
   //
   HBOOST_MOVE_ITERATE_0TO9(HBOOST_CONTAINER_NODE_ALLOC_HOLDER_CONSTRUCT_IMPL)
   #undef HBOOST_CONTAINER_NODE_ALLOC_HOLDER_CONSTRUCT_IMPL
   ...
   NodePtr create_node_from_it(const It &it)
   {
      NodePtr p = this->allocate_one();
      Deallocator node_deallocator(p, this->node_alloc());
      ::hboost::container::construct_in_place(this->node_alloc(), container_detail::addressof(p->m_data), it);
      node_deallocator.release();
      //This does not throw
      typedef typename Node::hook_type hook_type;
      ::new(static_cast<hook_type*>(container_detail::to_raw_pointer(p)), hboost_container_new_t()) hook_type;
   ...
   NodePtr create_node_from_key(HBOOST_FWD_REF(KeyConvertible) key)
   {
      NodePtr p = this->allocate_one();
      NodeAlloc &na = this->node_alloc();
      Deallocator node_deallocator(p, this->node_alloc());
      node_allocator_traits_type::construct
         (na, container_detail::addressof(p->m_data.first), hboost::forward<KeyConvertible>(key));
      HBOOST_TRY{
      ...
      if(n){
         ...
         HBOOST_TRY{
            ...
            while(n--){
               ...
               ++itbeg;
               //This can throw
               hboost::container::construct_in_place(nalloc, container_detail::addressof(p->m_data), beg);
               sdestructor.set(p);
               ++beg;
               //This does not throw
               typedef typename Node::hook_type hook_type;
               ::new(static_cast<hook_type*>(p), hboost_container_new_t()) hook_type;
   ...
   void clear(version_2)
   {
      ...
      //HBOOST_STATIC_ASSERT((::hboost::has_move_emulation_enabled<typename NodeAlloc::multiallocation_chain>::value == true));
      if(!chain.empty())
   ...
   struct move_cloner
   {
      ...
      {  //Use m_data instead of get_data to allow moving const key in [multi]map
         return m_holder.create_node(::hboost::move(other.m_data));
      }
      ...
      :  public NodeAlloc
   {
      ...
      explicit members_holder(HBOOST_FWD_REF(ConvertibleToAlloc) c2alloc)
         :  NodeAlloc(hboost::forward<ConvertibleToAlloc>(c2alloc))
         , m_icont()
      {}

      template<class ConvertibleToAlloc>
      members_holder(HBOOST_FWD_REF(ConvertibleToAlloc) c2alloc, const value_compare &c)
         :  NodeAlloc(hboost::forward<ConvertibleToAlloc>(c2alloc))
...
#endif // HBOOST_CONTAINER_DETAIL_NODE_ALLOC_HPP_
