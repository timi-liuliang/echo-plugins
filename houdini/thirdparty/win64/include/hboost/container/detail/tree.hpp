...
#ifndef HBOOST_CONTAINER_TREE_HPP
#define HBOOST_CONTAINER_TREE_HPP
...
#include <hboost/container/options.hpp>

// container/detail
#include <hboost/container/detail/algorithm.hpp> //algo_equal(), algo_lexicographical_compare
#include <hboost/container/detail/compare_functors.hpp>
...
#include <hboost/container/detail/node_alloc_holder.hpp>
...
#include <hboost/intrusive/avltree.hpp>
#include <hboost/intrusive/splaytree.hpp>
#include <hboost/intrusive/sgtree.hpp>
// intrusive/detail
#include <hboost/intrusive/detail/minimal_pair_header.hpp>   //pair
#include <hboost/intrusive/detail/tree_value_compare.hpp>    //tree_value_compare
// move
...
using hboost::intrusive::tree_value_compare;

template<class VoidPointer, hboost::container::tree_type_enum tree_type_value, bool OptimizeSize>
struct intrusive_tree_hook;

template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, hboost::container::red_black_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, hboost::container::avl_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, hboost::container::scapegoat_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, hboost::container::splay_tree, OptimizeSize>
{
   typedef typename container_detail::bi::make_bs_set_base_hook
      < container_detail::bi::void_pointer<VoidPointer>
      , container_detail::bi::link_mode<container_detail::bi::normal_link>
...
{
   typedef pair<typename hboost::move_detail::remove_const<T1>::type, T2> type;
};

//The node to be store in the tree
template <class T, class VoidPointer, hboost::container::tree_type_enum tree_type_value, bool OptimizeSize>
struct tree_node
   :  public intrusive_tree_hook<VoidPointer, tree_type_value, OptimizeSize>::type
{
   private:
   //HBOOST_COPYABLE_AND_MOVABLE(tree_node)
   tree_node();
   ...
   HBOOST_CONTAINER_FORCEINLINE T &get_data()
   {
      T* ptr = reinterpret_cast<T*>(&this->m_data);
      return *ptr;
   }

   HBOOST_CONTAINER_FORCEINLINE const T &get_data() const
   {
   ...
   template<class T1, class T2>
   HBOOST_CONTAINER_FORCEINLINE void do_assign(const std::pair<const T1, T2> &p)
   {
   ...
   template<class T1, class T2>
   HBOOST_CONTAINER_FORCEINLINE void do_assign(const pair<const T1, T2> &p)
   {
      const_cast<T1&>(m_data.first) = p.first;
      m_data.second  = p.second;
   }
   ...
   template<class V>
   HBOOST_CONTAINER_FORCEINLINE void do_assign(const V &v)
   {  m_data = v; }

   template<class T1, class T2>
   HBOOST_CONTAINER_FORCEINLINE void do_move_assign(std::pair<const T1, T2> &p)
   {
      const_cast<T1&>(m_data.first) = ::hboost::move(p.first);
      m_data.second = ::hboost::move(p.second);
   }

   template<class T1, class T2>
   HBOOST_CONTAINER_FORCEINLINE void do_move_assign(pair<const T1, T2> &p)
   {
      const_cast<T1&>(m_data.first) = ::hboost::move(p.first);
      m_data.second  = ::hboost::move(p.second);
   }
   ...
   template<class V>
   HBOOST_CONTAINER_FORCEINLINE void do_move_assign(V &v)
   {  m_data = ::hboost::move(v); }
};
...
template< class NodeType, class NodeCompareType
        ...
        , hboost::container::tree_type_enum tree_type_value>
struct intrusive_tree_dispatch;

template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, hboost::container::red_black_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, hboost::container::avl_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, hboost::container::scapegoat_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, hboost::container::splay_tree>
{
...
template<class Allocator, class ValueCompare, hboost::container::tree_type_enum tree_type_value, bool OptimizeSize>
...
template<hboost::container::tree_type_enum tree_type_value>
      ...
      >
{
   ...
   HBOOST_COPYABLE_AND_MOVABLE(tree)
   ...
   typedef hboost::container::reverse_iterator<iterator>                 reverse_iterator;
   typedef hboost::container::reverse_iterator<const_iterator>           const_reverse_iterator;
      ...
      : AllocHolder(value_compare(comp), a)
   {
      if(unique_insertion){
         ...
         this->allocate_many_and_construct
            ( first, hboost::container::iterator_distance(first, last)
   ...
   tree(HBOOST_RV_REF(tree) x)
      :  AllocHolder(HBOOST_MOVE_BASE(AllocHolder, x), x.value_comp())
   {}
   ...
   tree(HBOOST_RV_REF(tree) x, const allocator_type &a)
      :  AllocHolder(x.value_comp(), a)
   {
      if(this->node_alloc() == x.node_alloc()){
         ...
         this->icont().clone_from
            (hboost::move(x.icont()), typename AllocHolder::move_cloner(*this), Destroyer(this->node_alloc()));
      }
   }
   ...
   tree& operator=(HBOOST_COPY_ASSIGN_REF(tree) x)
   {
      if (&x != this){
         ...
         //automatically
         Icont other_tree(::hboost::move(this->icont()));
   ...
   tree& operator=(HBOOST_RV_REF(tree) x)
                                 ...
                                 && hboost::container::container_detail::is_nothrow_move_assignable<Compare>::value )
   {
      ...
      if(propagate_alloc || allocators_equal){
         ...
         //Obtain resources
         this->icont() = hboost::move(x.icont());
      }
   ...
   {
      HBOOST_ASSERT((priv_is_linked)(hint));
      std::pair<iiterator, bool> ret =
   ...
   iterator insert_unique_commit
      (HBOOST_FWD_REF(MovableConvertible) v, insert_commit_data &data)
   {
      NodePtr tmp = AllocHolder::create_node(hboost::forward<MovableConvertible>(v));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
      iterator ret(this->icont().insert_unique_commit(*tmp, data));
      destroy_deallocator.release();
      return ret;
   ...
   template<class MovableConvertible>
   std::pair<iterator,bool> insert_unique(HBOOST_FWD_REF(MovableConvertible) v)
   {
      insert_commit_data data;
      std::pair<iterator,bool> ret =
         this->insert_unique_check(KeyOfValue()(v), data);
      if(ret.second){
         ret.first = this->insert_unique_commit(hboost::forward<MovableConvertible>(v), data);
      }
      return ret;
   }
   ...
   template<class MovableConvertible>
   void push_back_impl(HBOOST_FWD_REF(MovableConvertible) v)
   {
      NodePtr tmp(AllocHolder::create_node(hboost::forward<MovableConvertible>(v)));
   ...
   template <class... Args>
   std::pair<iterator, bool> emplace_unique(HBOOST_FWD_REF(Args)... args)
   {  return this->emplace_unique_impl(AllocHolder::create_node(hboost::forward<Args>(args)...));   }

   template <class... Args>
   iterator emplace_hint_unique(const_iterator hint, HBOOST_FWD_REF(Args)... args)
   {  return this->emplace_unique_hint_impl(hint, AllocHolder::create_node(hboost::forward<Args>(args)...));   }

   template <class... Args>
   iterator emplace_equal(HBOOST_FWD_REF(Args)... args)
   {
      NodePtr tmp(AllocHolder::create_node(hboost::forward<Args>(args)...));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
   ...
   template <class... Args>
   iterator emplace_hint_equal(const_iterator hint, HBOOST_FWD_REF(Args)... args)
   {
      HBOOST_ASSERT((priv_is_linked)(hint));
      NodePtr tmp(AllocHolder::create_node(hboost::forward<Args>(args)...));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
   ...
   #define HBOOST_CONTAINER_TREE_EMPLACE_CODE(N) \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   std::pair<iterator, bool> emplace_unique(HBOOST_MOVE_UREF##N)\
   {  return this->emplace_unique_impl(AllocHolder::create_node(HBOOST_MOVE_FWD##N));  }\
   \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   iterator emplace_hint_unique(const_iterator hint HBOOST_MOVE_I##N HBOOST_MOVE_UREF##N)\
   {  return this->emplace_unique_hint_impl(hint, AllocHolder::create_node(HBOOST_MOVE_FWD##N)); }\
   \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   iterator emplace_equal(HBOOST_MOVE_UREF##N)\
   {\
      NodePtr tmp(AllocHolder::create_node(HBOOST_MOVE_FWD##N));\
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());\
      iterator ret(this->icont().insert_equal(this->icont().end(), *tmp));\
      destroy_deallocator.release();\
      return ret;\
   ...
   \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   iterator emplace_hint_equal(const_iterator hint HBOOST_MOVE_I##N HBOOST_MOVE_UREF##N)\
   {\
      HBOOST_ASSERT((priv_is_linked)(hint));\
      NodePtr tmp(AllocHolder::create_node(HBOOST_MOVE_FWD##N));\
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());\
   ...
   //
   HBOOST_MOVE_ITERATE_0TO9(HBOOST_CONTAINER_TREE_EMPLACE_CODE)
   #undef HBOOST_CONTAINER_TREE_EMPLACE_CODE

   #endif   // !defined(HBOOST_NO_CXX11_VARIADIC_TEMPLATES)

   template<class MovableConvertible>
   iterator insert_unique_convertible(const_iterator hint, HBOOST_FWD_REF(MovableConvertible) v)
   {
      HBOOST_ASSERT((priv_is_linked)(hint));
      insert_commit_data data;
      std::pair<iterator,bool> ret =
      ...
      if(!ret.second)
         return ret.first;
      return this->insert_unique_commit(hboost::forward<MovableConvertible>(v), data);
   }

   HBOOST_MOVE_CONVERSION_AWARE_CATCH_1ARG(insert_unique, value_type, iterator, this->insert_unique_convertible, const_iterator, const_iterator)
   ...
   template<class MovableConvertible>
   iterator insert_equal(HBOOST_FWD_REF(MovableConvertible) v)
   {
      NodePtr tmp(AllocHolder::create_node(hboost::forward<MovableConvertible>(v)));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
      iterator ret(this->icont().insert_equal(this->icont().end(), *tmp));
   ...
   template<class MovableConvertible>
   iterator insert_equal_convertible(const_iterator hint, HBOOST_FWD_REF(MovableConvertible) v)
   {
      HBOOST_ASSERT((priv_is_linked)(hint));
      NodePtr tmp(AllocHolder::create_node(hboost::forward<MovableConvertible>(v)));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
   ...
   HBOOST_MOVE_CONVERSION_AWARE_CATCH_1ARG(insert_equal, value_type, iterator, this->insert_equal_convertible, const_iterator, const_iterator)
   ...
   iterator insert_from_key(HBOOST_FWD_REF(KeyConvertible) key)
   {
      insert_commit_data data;
      ...
      return ret.second
               ? this->insert_unique_key_commit(hboost::forward<KeyConvertible>(key), data)
               : iterator(ret.first);
   }

   template<class KeyConvertible>
   iterator insert_unique_key_commit
      (HBOOST_FWD_REF(KeyConvertible) key, insert_commit_data &data)
   {
      NodePtr tmp = AllocHolder::create_node_from_key(hboost::forward<KeyConvertible>(key));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
      iterator ret(this->icont().insert_unique_commit(*tmp, data));
      destroy_deallocator.release();
      return ret;
   ...
   {
      HBOOST_ASSERT(position != this->cend() && (priv_is_linked)(position));
   ...
   {
      HBOOST_ASSERT(first == last || (first != this->cend() && (priv_is_linked)(first)));
      HBOOST_ASSERT(first == last || (priv_is_linked)(last));
...
struct has_trivial_destructor_after_move
   < 
      ::hboost::container::container_detail::tree
...
#endif //HBOOST_CONTAINER_TREE_HPP
