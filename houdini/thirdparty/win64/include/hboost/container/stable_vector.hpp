...
#ifndef HBOOST_CONTAINER_STABLE_VECTOR_HPP
#define HBOOST_CONTAINER_STABLE_VECTOR_HPP
...
#include <hboost/container/detail/allocator_version_traits.hpp>
#include <hboost/container/detail/construct_in_place.hpp>
      ...
      >
{
   ...
   public:
   typename ::hboost::intrusive::pointer_traits<Pointer>::element_type value;
};

template<class VoidPtr, class VoidAllocator>
struct index_traits
{
   typedef hboost::intrusive::
      pointer_traits
   ...
   typedef ::hboost::container::vector
...
{
   typedef hboost::intrusive::pointer_traits<Pointer>                                non_const_ptr_traits;
   public:
   ...
   typedef typename ::hboost::container::container_detail::if_c
      < IsConst
      , typename non_const_ptr_traits::template
         rebind_pointer<const value_type>::type
      , Pointer
      >::type                                                                       pointer;
   typedef hboost::intrusive::pointer_traits<pointer>                                ptr_traits;
   ...
   explicit stable_vector_iterator(node_base_ptr p) HBOOST_NOEXCEPT_OR_NOTHROW
      : m_pn(p)
   {}

   stable_vector_iterator() HBOOST_NOEXCEPT_OR_NOTHROW
      : m_pn() //Value initialization to achieve "null iterators" (N3644)
   {}

   stable_vector_iterator(stable_vector_iterator<Pointer, false> const& other) HBOOST_NOEXCEPT_OR_NOTHROW
      :  m_pn(other.node_pointer())
   {}

   node_ptr node_pointer() const HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //Increment / Decrement
   stable_vector_iterator& operator++() HBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   stable_vector_iterator operator++(int) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   reference operator[](difference_type off) const HBOOST_NOEXCEPT_OR_NOTHROW
   {  return node_ptr_traits::static_cast_from(this->m_pn->up[off])->value;  }

   stable_vector_iterator& operator+=(difference_type off) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      if(off) this->m_pn = this->m_pn->up[off];
      return *this;
   }

   friend stable_vector_iterator operator+(const stable_vector_iterator &left, difference_type off) HBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   friend stable_vector_iterator operator+(difference_type off, const stable_vector_iterator& right) HBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   friend difference_type operator-(const stable_vector_iterator &left, const stable_vector_iterator &right) HBOOST_NOEXCEPT_OR_NOTHROW
   {  return left.m_pn->up - right.m_pn->up;  }

   //Comparison operators
   friend bool operator==   (const stable_vector_iterator& l, const stable_vector_iterator& r) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   {  return l.m_pn->up >= r.m_pn->up;  }
      ...
      #define STABLE_VECTOR_CHECK_INVARIANT \
               invariant_checker HBOOST_JOIN(check_invariant_,__LINE__)(*this); \
...
class stable_vector
{
   ...
   typedef ::hboost::container::container_detail::integral_constant
      <unsigned, hboost::container::container_detail::
   ...
   #ifndef HBOOST_CONTAINER_DOXYGEN_INVOKED
   private:
   HBOOST_COPYABLE_AND_MOVABLE(stable_vector)
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit stable_vector(const allocator_type& al) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //!
   //! <b>Complexity</b>: Constant.
   stable_vector(HBOOST_RV_REF(stable_vector) x)
      : internal_data(hboost::move(x.priv_node_alloc())), index(hboost::move(x.index))
   {
   ...
   stable_vector(HBOOST_RV_REF(stable_vector) x, const allocator_type &a)
      : internal_data(a), index(a)
   {
      if(this->priv_node_alloc() == x.priv_node_alloc()){
         ...
         stable_vector_detail::clear_on_destroy<stable_vector> cod(*this);
         this->insert(this->cend(), hboost::make_move_iterator(x.begin()), hboost::make_move_iterator(x.end()));
         STABLE_VECTOR_CHECK_INVARIANT;
         cod.release();
      }
   }
   ...
   stable_vector& operator=(HBOOST_COPY_ASSIGN_REF(stable_vector) x)
   {
      STABLE_VECTOR_CHECK_INVARIANT;
      if (&x != this){
   ...
   stable_vector& operator=(HBOOST_RV_REF(stable_vector) x)
   ...
   //!
   //! <b>Complexity</b>: Constant.
   size_type capacity() const HBOOST_NOEXCEPT_OR_NOTHROW
   {
      const size_type index_size             = this->index.size();
      HBOOST_ASSERT(!index_size || index_size >= ExtraPointers);
      ...
      HBOOST_ASSERT(node_extra_capacity <= (this->index.capacity()- index_size));
   ...
   void emplace_back(Args &&...args)
   {
      ...
      EmplaceFunctor &&ef = EmplaceFunctor(hboost::forward<Args>(args)...);
   ...
   #define HBOOST_CONTAINER_STABLE_VECTOR_EMPLACE_CODE(N) \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   void emplace_back(HBOOST_MOVE_UREF##N)\
   {\
      typedef emplace_functor##N\
         HBOOST_MOVE_LT##N HBOOST_MOVE_TARG##N HBOOST_MOVE_GT##N EmplaceFunctor;\
      typedef emplace_iterator<value_type, EmplaceFunctor, difference_type>  EmplaceIterator;\
      EmplaceFunctor ef HBOOST_MOVE_LP##N HBOOST_MOVE_FWD##N HBOOST_MOVE_RP##N;\
      this->insert(this->cend() , EmplaceIterator(ef), EmplaceIterator());\
   }\
   \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   ...
   //
   HBOOST_MOVE_ITERATE_0TO9(HBOOST_CONTAINER_STABLE_VECTOR_EMPLACE_CODE)
   #undef HBOOST_CONTAINER_STABLE_VECTOR_EMPLACE_CODE
   ...
   {
      HBOOST_ASSERT(this->priv_in_range_or_end(p));
      const size_type num_new = static_cast<size_type>(hboost::container::iterator_distance(first, last));
   ...
   {
      HBOOST_ASSERT(this->priv_in_range(p));
      STABLE_VECTOR_CHECK_INVARIANT;
   ...
   void swap(stable_vector & x)
      HBOOST_NOEXCEPT_IF( allocator_traits_type::propagate_on_container_swap::value
                                || allocator_traits_type::is_always_equal::value)
   {
      HBOOST_ASSERT(allocator_traits_type::propagate_on_container_swap::value ||
                   allocator_traits_type::is_always_equal::value ||
   ...
   {
      //Check range
      HBOOST_ASSERT(this->index.empty() || (this->index.data() <= p->up));
      HBOOST_ASSERT(this->index.empty() || p->up <= (this->index.data() + this->index.size()));
   ...
   template <class U>
   void priv_push_back(HBOOST_MOVE_CATCH_FWD(U) x)
   {
      if(HBOOST_LIKELY(this->priv_capacity_bigger_than_size())){
         ...
         HBOOST_ASSERT(!!p);
         {
            ...
            //This might throw
            this->priv_build_node_from_convertible(p, ::hboost::forward<U>(x));
            rollback.release();
         }
         ...
         this->insert(this->cend(), ::hboost::forward<U>(x));
      }
   }
   ...
   iterator priv_insert(const_iterator p, HBOOST_RV_REF(T) x)
   {
      HBOOST_ASSERT(this->priv_in_range_or_end(p));
      typedef repeat_iterator<T, difference_type>  repeat_it;
      typedef hboost::move_iterator<repeat_it>      repeat_move_it;
   ...
   node_ptr priv_get_from_pool()
   {
      ...
      HBOOST_ASSERT(!this->index.empty());
      node_base_ptr &pool_first_ref = *(this->index.end() - (ExtraPointers-1));
      node_base_ptr &pool_last_ref  = this->index.back();
      multiallocation_chain holder;
      holder.incorporate_after( holder.before_begin()
   ...
   {
      //This can throw
      hboost::container::construct_in_place
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , it);
      //This does not throw
      ::new(static_cast<node_base_type*>(container_detail::to_raw_pointer(p)), hboost_container_new_t())
         node_base_type(index_traits_type::ptr_to_node_base_ptr(*up_index));
   }

   template<class ValueConvertible>
   void priv_build_node_from_convertible(const node_ptr &p, HBOOST_FWD_REF(ValueConvertible) value_convertible)
   {
      //This can throw
      hboost::container::allocator_traits<node_allocator_type>::construct
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , ::hboost::forward<ValueConvertible>(value_convertible));
      //This does not throw
      ::new(static_cast<node_base_type*>(container_detail::to_raw_pointer(p)), hboost_container_new_t()) node_base_type;
   }

   void priv_swap_members(stable_vector &x)
   {
      hboost::adl_move_swap(this->internal_data.pool_size, x.internal_data.pool_size);
   ...
   class invariant_checker
   {
      ...
      ~invariant_checker(){HBOOST_ASSERT(p->priv_invariant());}
      void touch(){}
   };
   #endif
   ...
   {
      private:
      HBOOST_MOVABLE_BUT_NOT_COPYABLE(ebo_holder)

      public:
      template<class AllocatorRLValue>
      explicit ebo_holder(HBOOST_FWD_REF(AllocatorRLValue) a)
         : node_allocator_type(hboost::forward<AllocatorRLValue>(a))
         , pool_size(0)
         , end_node()
      {}
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<hboost::container::stable_vector<T, Allocator> >
{
   typedef typename ::hboost::container::allocator_traits<Allocator>::pointer pointer;
...
#endif   //HBOOST_CONTAINER_STABLE_VECTOR_HPP
