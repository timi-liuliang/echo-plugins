...
//
//////////////////////////////////////////////////////////////////////////////
#ifndef HBOOST_CONTAINER_LIST_HPP
#define HBOOST_CONTAINER_LIST_HPP
...
// container/detail
#include <hboost/container/detail/algorithm.hpp>
...
#include <hboost/intrusive/list.hpp>
// other
      ...
      <Allocator, typename container_detail::intrusive_list_type<Allocator>::type>
{
   ...
   HBOOST_COPYABLE_AND_MOVABLE(list)
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit list(const allocator_type &a) HBOOST_NOEXCEPT_OR_NOTHROW
      : AllocHolder(a)
   {}
   ...
   list(HBOOST_RV_REF(list) x, const allocator_type &a)
      : AllocHolder(a)
   {
      if(this->node_alloc() == x.node_alloc()){
         ...
         this->insert(this->cbegin(), hboost::make_move_iterator(x.begin()), hboost::make_move_iterator(x.end()));
      }
   }
   ...
   ~list() HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   list& operator=(HBOOST_COPY_ASSIGN_REF(list) x)
   {
      if (&x != this){
   ...
   list& operator=(HBOOST_RV_REF(list) x)
   ...
   {  this->emplace(this->cend(), hboost::forward<Args>(args)...); }
   ...
   {  this->emplace(this->cbegin(), hboost::forward<Args>(args)...);  }
   ...
   {
      HBOOST_ASSERT((priv_is_linked)(position));
   ...
   #define HBOOST_CONTAINER_LIST_EMPLACE_CODE(N) \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   void emplace_back(HBOOST_MOVE_UREF##N)\
   {  this->emplace(this->cend() HBOOST_MOVE_I##N HBOOST_MOVE_FWD##N);  }\
   \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   void emplace_front(HBOOST_MOVE_UREF##N)\
   {  this->emplace(this->cbegin() HBOOST_MOVE_I##N HBOOST_MOVE_FWD##N);}\
   \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   iterator emplace(const_iterator position HBOOST_MOVE_I##N HBOOST_MOVE_UREF##N)\
   {\
      HBOOST_ASSERT(position == this->cend() || (--(++position) == position) );\
      NodePtr pnode (AllocHolder::create_node(HBOOST_MOVE_FWD##N));\
      return iterator(this->icont().insert(position.get(), *pnode));\
   }\
   //
   HBOOST_MOVE_ITERATE_0TO9(HBOOST_CONTAINER_LIST_EMPLACE_CODE)
   #undef HBOOST_CONTAINER_LIST_EMPLACE_CODE
   ...
   {
      HBOOST_ASSERT((priv_is_linked)(p));
   ...
   {
      HBOOST_ASSERT(p != this->cend() && (priv_is_linked)(p));
   ...
   void splice(const_iterator p, list& x) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT((priv_is_linked)(p));
      HBOOST_ASSERT(this != &x);
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of
   //!    this list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, HBOOST_RV_REF(list) x) HBOOST_NOEXCEPT_OR_NOTHROW
   {
   ...
   void splice(const_iterator p, list &x, const_iterator i) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT((priv_is_linked)(p));
      HBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, HBOOST_RV_REF(list) x, const_iterator i) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT(this != &x);
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, list &x, const_iterator first, const_iterator last) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT((priv_is_linked)(p));
      HBOOST_ASSERT(first == last || (first != x.cend() && x.priv_is_linked(first)));
      HBOOST_ASSERT(first == last || x.priv_is_linked(last));
      HBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, HBOOST_RV_REF(list) x, const_iterator first, const_iterator last) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT(this != &x);
      //Additional checks done in splice()
   ...
   //!
   //! <b>Note</b>: Non-standard extension
   void splice(const_iterator p, list &x, const_iterator first, const_iterator last, size_type n) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //!
   //! <b>Note</b>: Non-standard extension
   void splice(const_iterator p, HBOOST_RV_REF(list) x, const_iterator first, const_iterator last, size_type n) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Complexity</b>: This function is linear time: it performs at most
   //!   size() + x.size() - 1 comparisons.
   void merge(HBOOST_RV_REF(list) x)
   ...
   //! <b>Note</b>: Iterators and references to *this are not invalidated.
   template <class StrictWeakOrdering>
   void merge(HBOOST_RV_REF(list) x, StrictWeakOrdering comp)
   ...
   {
      HBOOST_ASSERT((priv_is_linked)(p));
      NodePtr tmp = AllocHolder::create_node(hboost::move(x));
      return iterator(this->icont().insert(p.get(), *tmp));
   }
   ...
   void priv_push_back (HBOOST_RV_REF(T) x)
   {  this->insert(this->cend(), hboost::move(x));    }

   void priv_push_front (const T &x)
   {  this->insert(this->cbegin(), x);  }

   void priv_push_front (HBOOST_RV_REF(T) x)
   {  this->insert(this->cbegin(), hboost::move(x));  }
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<hboost::container::list<T, Allocator> >
{
   typedef typename ::hboost::container::allocator_traits<Allocator>::pointer pointer;
...
#endif // HBOOST_CONTAINER_LIST_HPP
