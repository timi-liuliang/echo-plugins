...
#ifndef HBOOST_CONTAINER_SLIST_HPP
#define HBOOST_CONTAINER_SLIST_HPP
      ...
      <Allocator, typename container_detail::intrusive_slist_type<Allocator>::type>
{
   ...
   typedef hboost::container::equal_to_value<Allocator>      equal_to_value_type;

   HBOOST_COPYABLE_AND_MOVABLE(slist)
   ...
   typedef HBOOST_CONTAINER_IMPDEF(NodeAlloc)                                  stored_allocator_type;
   ...
   //!
   //! <b>Complexity</b>: Constant.
   explicit slist(const allocator_type& a) HBOOST_NOEXCEPT_OR_NOTHROW
      :  AllocHolder(a)
   {}
   ...
   //!
   //! <b>Complexity</b>: Constant.
   slist(HBOOST_RV_REF(slist) x)
      : AllocHolder(HBOOST_MOVE_BASE(AllocHolder, x))
   {}
   ...
   slist(HBOOST_RV_REF(slist) x, const allocator_type &a)
      : AllocHolder(a)
   {
      if(this->node_alloc() == x.node_alloc()){
         ...
         this->insert_after(this->cbefore_begin(), hboost::make_move_iterator(x.begin()), hboost::make_move_iterator(x.end()));
      }
   }
   ...
   ~slist() HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   slist& operator= (HBOOST_COPY_ASSIGN_REF(slist) x)
   {
      if (&x != this){
   ...
   slist& operator= (HBOOST_RV_REF(slist) x)
   ...
   //!
   //! <b>Complexity</b>: Constant.
   iterator before_begin() HBOOST_NOEXCEPT_OR_NOTHROW
   {  return iterator(end());  }
   ...
   //!
   //! <b>Complexity</b>: Constant.
   const_iterator before_begin() const HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //!
   //! <b>Complexity</b>: Constant.
   const_iterator cbefore_begin() const HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //!
   //! <b>Note</b>: Non-standard extension.
   iterator previous(iterator p) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   {  this->emplace_after(this->cbefore_begin(), hboost::forward<Args>(args)...); }
   ...
   //! <b>Complexity</b>: Constant
   template <class... Args>
   iterator emplace_after(const_iterator prev, HBOOST_FWD_REF(Args)... args)
   {
      NodePtr pnode(AllocHolder::create_node(hboost::forward<Args>(args)...));
      return iterator(this->icont().insert_after(prev.get(), *pnode));
   }

   #else // !defined(HBOOST_NO_CXX11_VARIADIC_TEMPLATES)

   #define HBOOST_CONTAINER_SLIST_EMPLACE_CODE(N) \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   void emplace_front(HBOOST_MOVE_UREF##N)\
   {  this->emplace_after(this->cbefore_begin() HBOOST_MOVE_I##N HBOOST_MOVE_FWD##N);}\
   \
   HBOOST_MOVE_TMPL_LT##N HBOOST_MOVE_CLASS##N HBOOST_MOVE_GT##N \
   iterator emplace_after(const_iterator p HBOOST_MOVE_I##N HBOOST_MOVE_UREF##N)\
   {\
      NodePtr pnode (AllocHolder::create_node(HBOOST_MOVE_FWD##N));\
      return iterator(this->icont().insert_after(p.get(), *pnode));\
   }\
   //
   HBOOST_MOVE_ITERATE_0TO9(HBOOST_CONTAINER_SLIST_EMPLACE_CODE)
   #undef HBOOST_CONTAINER_SLIST_EMPLACE_CODE
   ...
   #else
   HBOOST_MOVE_CONVERSION_AWARE_CATCH_1ARG(insert_after, T, iterator, priv_insert_after, const_iterator, const_iterator)
   #endif
      ...
      )
   {
      ...
      this->allocate_many_and_construct(first, hboost::container::iterator_distance(first, last), func);
   ...
   void splice_after(const_iterator prev_p, slist& x) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT(this != &x);
      HBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of
   //!    this list. Iterators of this list and all the references are not invalidated.
   void splice_after(const_iterator prev_p, HBOOST_RV_REF(slist) x) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void splice_after(const_iterator prev_p, slist& x, const_iterator prev) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice_after(const_iterator prev_p, HBOOST_RV_REF(slist) x, const_iterator prev) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void splice_after(const_iterator prev_p,      slist& x,
      const_iterator before_first,  const_iterator before_last) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT(this->node_alloc() == x.node_alloc());
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice_after(const_iterator prev_p,      HBOOST_RV_REF(slist) x,
      const_iterator before_first,  const_iterator before_last) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void splice_after(const_iterator prev_p,      slist& x,
                     ...
                     size_type n) HBOOST_NOEXCEPT_OR_NOTHROW
   {
      HBOOST_ASSERT(this->node_alloc() == x.node_alloc());
      this->icont().splice_after
   ...
   //! <b>Complexity</b>: This function is linear time: it performs at most
   //!   size() + x.size() - 1 comparisons.
   void merge(HBOOST_RV_REF(slist) x)
   ...
   //! <b>Note</b>: Iterators and references to *this are not invalidated.
   template <class StrictWeakOrdering>
   void merge(HBOOST_RV_REF(slist) x, StrictWeakOrdering comp)
   ...
   void reverse() HBOOST_NOEXCEPT_OR_NOTHROW
   {  this->icont().reverse();  }
   ...
   {  return this->emplace_after(this->previous(p), hboost::forward<Args>(args)...);  }
   ...
   {\
      return this->emplace_after(this->previous(p) HBOOST_MOVE_I##N HBOOST_MOVE_FWD##N);\
   }\
   //
   HBOOST_MOVE_ITERATE_0TO9(HBOOST_CONTAINER_SLIST_EMPLACE_CODE)
   #undef HBOOST_CONTAINER_SLIST_EMPLACE_CODE
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of
   //!    this list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, slist& x) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of
   //!    this list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, HBOOST_RV_REF(slist) x) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, slist& x, const_iterator i) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, HBOOST_RV_REF(slist) x, const_iterator i) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, slist& x, const_iterator first, const_iterator last) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator p, HBOOST_RV_REF(slist) x, const_iterator first, const_iterator last) HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   void priv_push_front (HBOOST_RV_REF(T) x)
   {  this->insert_after(this->cbefore_begin(), ::hboost::move(x));  }
   ...
   {  return this->insert_after(previous(p), ::hboost::forward<U>(x)); }

   template<class U>
   iterator priv_insert_after(const_iterator prev_p, HBOOST_FWD_REF(U) x)
   {  return iterator(this->icont().insert_after(prev_p.get(), *this->create_node(::hboost::forward<U>(x)))); }
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<hboost::container::slist<T, Allocator> >
{
   typedef typename ::hboost::container::allocator_traits<Allocator>::pointer pointer;
...
HBOOST_CONTAINER_DOC1ST(namespace std {, HBOOST_MOVE_STD_NS_BEG)

template <class T, class Allocator>
class insert_iterator<hboost::container::slist<T, Allocator> >
{
 protected:
   typedef hboost::container::slist<T, Allocator> Container;
   Container* container;
