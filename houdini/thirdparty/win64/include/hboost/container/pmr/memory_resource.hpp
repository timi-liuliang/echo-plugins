...
#ifndef HBOOST_CONTAINER_PMR_MEMORY_RESOURCE_HPP
#define HBOOST_CONTAINER_PMR_MEMORY_RESOURCE_HPP
...
class memory_resource
{
   public:
   // For exposition only
   static HBOOST_CONSTEXPR_OR_CONST std::size_t max_align =
      hboost::move_detail::alignment_of<hboost::move_detail::max_align_t>::value;
   ...
   bool is_equal(const memory_resource& other) const HBOOST_NOEXCEPT
   {  return this->do_is_equal(other);  }

   //! <b>Returns</b>:
   //!   `&a == &b || a.is_equal(b)`.
   friend bool operator==(const memory_resource& a, const memory_resource& b) HBOOST_NOEXCEPT
   ...
   virtual bool do_is_equal(const memory_resource& other) const HBOOST_NOEXCEPT = 0;
};
...
#endif   //HBOOST_CONTAINER_PMR_MEMORY_RESOURCE_HPP
