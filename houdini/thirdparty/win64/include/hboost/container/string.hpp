...
#ifndef HBOOST_CONTAINER_STRING_HPP
#define HBOOST_CONTAINER_STRING_HPP
...
#include <hboost/container/detail/minimal_char_traits_header.hpp>
...
class basic_string_base
{
 ...
 public:
   ...
   typedef ::hboost::intrusive::pointer_traits<pointer> pointer_traits;
   ...
   explicit basic_string_base(HBOOST_RV_REF(allocator_type) a)
      :  members_(hboost::move(a))
   {  this->init();  }
      ...
      :  public Allocator
   {
      ...
      explicit members_holder(HBOOST_FWD_REF(AllocatorConvertible) a)
         :  Allocator(hboost::forward<AllocatorConvertible>(a))
      {}
   ...
   typedef container_detail::integral_constant<unsigned,
      hboost::container::container_detail::version<Allocator>::value> alloc_version;
   ...
   void swap_data(basic_string_base& other)
   {
      if(this->is_short()){
         ...
         if(other.is_short()){
            ...
            hboost::adl_move_swap(this->members_.m_repr.long_repr(), other.members_.m_repr.long_repr());
         }
      }
   }
};
   ...
   :  private container_detail::basic_string_base<Allocator>
{
   ...
   private:
   typedef allocator_traits<Allocator> allocator_traits_type;
   HBOOST_COPYABLE_AND_MOVABLE(basic_string)
   ...
   //!
   //! <b>Throws</b>: Nothing
   explicit basic_string(const allocator_type& a) HBOOST_NOEXCEPT_OR_NOTHROW
      : base_t(a)
   { this->priv_terminate_string(); }
   ...
   basic_string(HBOOST_RV_REF(basic_string) s) HBOOST_NOEXCEPT_OR_NOTHROW
      : base_t(hboost::move(s.alloc()))
   {
   ...
   basic_string(HBOOST_RV_REF(basic_string) s, const allocator_type &a)
      : base_t(a)
   {
      this->priv_terminate_string();
      if(a == this->alloc()){
   ...
   //!
   //! <b>Complexity</b>: Constant.
   ~basic_string() HBOOST_NOEXCEPT_OR_NOTHROW
   {}
   ...
   basic_string& operator=(HBOOST_COPY_ASSIGN_REF(basic_string) x)
   {
      if (&x != this){
   ...
   basic_string& operator=(HBOOST_RV_REF(basic_string) x)
   ...
   //!
   //! <b>Complexity</b>: Constant.
   size_type length() const HBOOST_NOEXCEPT_OR_NOTHROW
   { return this->size(); }
   ...
   //!
   //! <b>Returns</b>: *this
   basic_string& assign(HBOOST_RV_REF(basic_string) ms) HBOOST_NOEXCEPT_OR_NOTHROW
      ...
      )
   {
      const size_type n_pos = p - this->cbegin();
      if (first != last) {
         const size_type n = hboost::container::iterator_distance(first, last);
         const size_type old_size = this->priv_size();
         ...
         if(enough_capacity){
            ...
            if (elems_after >= n) {
               ...
               hboost::container::iterator_advance(mid, elems_after + 1);
   ...
   {
      difference_type n = hboost::container::iterator_distance(j1, j2);
      ...
      if (len >= n) {
         ...
         ForwardIter m = j1;
         hboost::container::iterator_advance(m, len);
   ...
   //!
   //! <b>Complexity</b>: constant time.
   const CharT* c_str() const HBOOST_NOEXCEPT_OR_NOTHROW
   ...
   //!
   //! <b>Complexity</b>: constant time.
   const CharT* data()  const HBOOST_NOEXCEPT_OR_NOTHROW
   {  return container_detail::to_raw_pointer(this->priv_addr()); }
   ...
   {
      typedef typename hboost::container::iterator_traits<InputIter>::iterator_category Category;
...
template <class CharT, class Traits, class Allocator> inline
   basic_string<CharT, Traits, Allocator> operator+
      ( HBOOST_RV_REF_BEG basic_string<CharT, Traits, Allocator> HBOOST_RV_REF_END x
      , HBOOST_RV_REF_BEG basic_string<CharT, Traits, Allocator> HBOOST_RV_REF_END y)
{
   x += y;
...
struct has_trivial_destructor_after_move<hboost::container::basic_string<C, T, Allocator> >
{
   typedef typename ::hboost::container::allocator_traits<Allocator>::pointer pointer;
...
#endif // HBOOST_CONTAINER_STRING_HPP
