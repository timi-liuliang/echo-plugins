...
#ifndef HBOOST_CONTAINER_ALLOCATOR_HPP
#define HBOOST_CONTAINER_ALLOCATOR_HPP
...
template< class T
        , unsigned Version HBOOST_CONTAINER_DOCONLY(=2)
        , unsigned int AllocationDisableMask HBOOST_CONTAINER_DOCONLY(=0)>
class allocator
{
   ...
   static const unsigned int ForbiddenMask =
      HBOOST_CONTAINER_ALLOCATE_NEW | HBOOST_CONTAINER_EXPAND_BWD | HBOOST_CONTAINER_EXPAND_FWD ;

   //The mask can't disable all the allocation types
   HBOOST_STATIC_ASSERT((  (AllocationDisableMask & ForbiddenMask) != ForbiddenMask  ));

   //The mask is only valid for version 2 allocators
   HBOOST_STATIC_ASSERT((  Version != 1 || (AllocationDisableMask == 0)  ));
   ...
   //!Default constructor
   //!Never throws
   allocator() HBOOST_NOEXCEPT_OR_NOTHROW
   {}

   //!Constructor from other allocator.
   //!Never throws
   allocator(const allocator &) HBOOST_NOEXCEPT_OR_NOTHROW
   {}
   ...
   //!Never throws
   void deallocate(pointer ptr, size_type) HBOOST_NOEXCEPT_OR_NOTHROW
   {  dlmalloc_free(ptr);  }
   ...
   friend bool operator==(const allocator &, const allocator &) HBOOST_NOEXCEPT_OR_NOTHROW
   {  return true;   }
                         ...
                         pointer &reuse)
   {
      ...
      if(!ret && !(command & HBOOST_CONTAINER_NOTHROW_ALLOCATION))
         hboost::container::throw_bad_alloc();
      return ret;
   }
...
#endif   //HBOOST_CONTAINER_ALLOCATOR_HPP
