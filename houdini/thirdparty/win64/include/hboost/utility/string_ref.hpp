...
#ifndef HBOOST_STRING_REF_HPP
#define HBOOST_STRING_REF_HPP

#include <hboost/config.hpp>
#include <hboost/detail/workaround.hpp>
#include <hboost/utility/string_ref_fwd.hpp>
...
namespace hboost {
    ...
    public:
        ...
        // construct/copy
        HBOOST_CONSTEXPR basic_string_ref ()
            : ptr_(NULL), len_(0) {}

        HBOOST_CONSTEXPR basic_string_ref (const basic_string_ref &rhs)
        ...
        HBOOST_CONSTEXPR basic_string_ref(const charT* str, size_type len)
            : ptr_(str), len_(len) {}
        ...
        // iterators
        HBOOST_CONSTEXPR const_iterator   begin() const { return ptr_; }
        HBOOST_CONSTEXPR const_iterator  cbegin() const { return ptr_; }
        HBOOST_CONSTEXPR const_iterator     end() const { return ptr_ + len_; }
        HBOOST_CONSTEXPR const_iterator    cend() const { return ptr_ + len_; }
        ...
        // capacity
        HBOOST_CONSTEXPR size_type size()     const { return len_; }
        HBOOST_CONSTEXPR size_type length()   const { return len_; }
        HBOOST_CONSTEXPR size_type max_size() const { return len_; }
        HBOOST_CONSTEXPR bool empty()         const { return len_ == 0; }

        // element access
        HBOOST_CONSTEXPR const charT& operator[](size_type pos) const { return ptr_[pos]; }

        const charT& at(size_t pos) const {
            if ( pos >= len_ )
                HBOOST_THROW_EXCEPTION( std::out_of_range ( "hboost::string_ref::at" ) );
            return ptr_[pos];
            }

        HBOOST_CONSTEXPR const charT& front() const { return ptr_[0]; }
        HBOOST_CONSTEXPR const charT& back()  const { return ptr_[len_-1]; }
        HBOOST_CONSTEXPR const charT* data()  const { return ptr_; }
        ...
        basic_string_ref substr(size_type pos, size_type n=npos) const {
            if ( pos > size())
                HBOOST_THROW_EXCEPTION( std::out_of_range ( "string_ref::substr" ) );
...
namespace std {
    // Hashing
    template<> struct hash<hboost::string_ref>;
    template<> struct hash<hboost::u16string_ref>;
    template<> struct hash<hboost::u32string_ref>;
    template<> struct hash<hboost::wstring_ref>;
}
#endif
