//  Boost string_algo library string_algo.hpp header file  ---------------------------//

//  Copyright Pavol Droba 2002-2004.
//
// Distributed under the Boost Software License, Version 1.0.
//    (See accompanying file LICENSE_1_0.txt or copy at
//          http://www.boost.org/LICENSE_1_0.txt)

//  See http://www.boost.org/ for updates, documentation, and revision history.

#ifndef HBOOST_STRING_ALGO_HPP
#define HBOOST_STRING_ALGO_HPP

/*! \file
    Cumulative include for string_algo library
*/

#include <hboost/algorithm/string/std_containers_traits.hpp>
#include <hboost/algorithm/string/trim.hpp>
#include <hboost/algorithm/string/case_conv.hpp>
#include <hboost/algorithm/string/predicate.hpp>
#include <hboost/algorithm/string/find.hpp>
#include <hboost/algorithm/string/split.hpp>
#include <hboost/algorithm/string/join.hpp>
#include <hboost/algorithm/string/replace.hpp>
#include <hboost/algorithm/string/erase.hpp>
#include <hboost/algorithm/string/classification.hpp>
#include <hboost/algorithm/string/find_iterator.hpp>


#endif  // HBOOST_STRING_ALGO_HPP
