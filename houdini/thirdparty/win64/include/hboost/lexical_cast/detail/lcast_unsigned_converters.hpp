...
#ifndef HBOOST_LEXICAL_CAST_DETAIL_LCAST_UNSIGNED_CONVERTERS_HPP
#define HBOOST_LEXICAL_CAST_DETAIL_LCAST_UNSIGNED_CONVERTERS_HPP
...
namespace hboost 
{
    namespace detail // lcast_to_unsigned
    {
        template<class T>
        inline
        HBOOST_DEDUCED_TYPENAME hboost::make_unsigned<T>::type lcast_to_unsigned(const T value) HBOOST_NOEXCEPT {
            typedef HBOOST_DEDUCED_TYPENAME hboost::make_unsigned<T>::type result_type;
            return value < 0 
    ...
    namespace detail // lcast_put_unsigned
    {
        ...
        class lcast_put_unsigned: hboost::noncopyable {
            typedef HBOOST_DEDUCED_TYPENAME Traits::int_type int_type;
            HBOOST_DEDUCED_TYPENAME hboost::mpl::if_c<
        ...
        public:
            lcast_put_unsigned(const T n_param, CharT* finish) HBOOST_NOEXCEPT 
                ...
                , m_czero(lcast_char_constants<CharT>::zero), m_zero(Traits::to_int_type(m_czero))
            {
                ...
                HBOOST_STATIC_ASSERT(!std::numeric_limits<T>::is_signed);
#endif
            }
            ...
            CharT* convert() {
                ...
                numpunct const& np = HBOOST_USE_FACET(numpunct, loc);
                ...
                HBOOST_STATIC_ASSERT(std::numeric_limits<T>::digits10 < CHAR_MAX);
#endif
        ...
        private:
            inline bool main_convert_iteration() HBOOST_NOEXCEPT {
                --m_finish;
            ...
            inline CharT* main_convert_loop() HBOOST_NOEXCEPT {
                while (main_convert_iteration());
                return m_finish;
            }
        };
    ...
    {
        template <class Traits, class T, class CharT>
        class lcast_ret_unsigned: hboost::noncopyable {
        ...
        public:
            lcast_ret_unsigned(T& value, const CharT* const begin, const CharT* end) HBOOST_NOEXCEPT
                : m_multiplier_overflowed(false), m_multiplier(1), m_value(value), m_begin(begin), m_end(end)
            {
                ...
                HBOOST_STATIC_ASSERT_MSG(std::numeric_limits<T>::is_specialized,
                    "std::numeric_limits are not specialized for integral type passed to hboost::lexical_cast"
                );
#endif
            }
...
#ifdef HBOOST_LEXICAL_CAST_ASSUME_C_LOCALE
                return main_convert_loop();
#else
                std::locale loc;
                if (loc == std::locale::classic()) {
        ...
        private:
            ...
            bool main_convert_loop() HBOOST_NOEXCEPT {
                for ( ; m_end >= m_begin; --m_end) {
                    if (!main_convert_iteration()) {
                        return false;
                    }
...
#endif // HBOOST_LEXICAL_CAST_DETAIL_LCAST_UNSIGNED_CONVERTERS_HPP
