...
#ifndef HBOOST_TYPE_INDEX_TYPE_INDEX_FACADE_HPP
#define HBOOST_TYPE_INDEX_TYPE_INDEX_FACADE_HPP
...
// Forward declaration from #include <hboost/functional/hash_fwd.hpp>
namespace hboost {
...
private:
    /// @cond
    const Derived & derived() const HBOOST_NOEXCEPT {
...
public:
    ...
    inline bool equal(const Derived& rhs) const HBOOST_NOEXCEPT {
    ...
    inline bool before(const Derived& rhs) const HBOOST_NOEXCEPT {
        const char* const left = derived().raw_name();
        const char* const right = rhs.raw_name();
    ...
    /// \note <hboost/functional/hash.hpp> has to be included if this function is used.
    inline std::size_t hash_code() const HBOOST_NOEXCEPT {
        const char* const name_raw = derived().raw_name();
        return hboost::hash_range(name_raw, name_raw + std::strlen(name_raw));
    }
...
protected:
    ...
    /// hboost::typeindex::type_id() will call this method, if Derived has same type as hboost::typeindex::type_index.
    ///
    ...
    template <class T>
    static Derived type_id() HBOOST_NOEXCEPT;

    /// This is a factory method that is used to create instances of Derived classes.
    /// hboost::typeindex::type_id_with_cvr() will call this method, if Derived has same type as hboost::typeindex::type_index.
    ///
    /// \b Override: This function \b may be redefined and made public in Derived class. Overrides \b must not throw. 
    ...
    /// \return type_index for type T.
    template <class T>
    static Derived type_id_with_cvr() HBOOST_NOEXCEPT;

    /// This is a factory method that is used to create instances of Derived classes.
    /// hboost::typeindex::type_id_runtime(const T&) will call this method, if Derived has same type as hboost::typeindex::type_index.
    ///
    ...
    template <class T>
    static Derived type_id_runtime(const T& variable) HBOOST_NOEXCEPT;
...
/// @cond
template <class Derived, class TypeInfo>
inline bool operator == (const type_index_facade<Derived, TypeInfo>& lhs, const type_index_facade<Derived, TypeInfo>& rhs) HBOOST_NOEXCEPT {
...
template <class Derived, class TypeInfo>
inline bool operator == (const TypeInfo& lhs, const type_index_facade<Derived, TypeInfo>& rhs) HBOOST_NOEXCEPT {
...
template <class Derived, class TypeInfo>
inline bool operator == (const type_index_facade<Derived, TypeInfo>& lhs, const TypeInfo& rhs) HBOOST_NOEXCEPT {
...
#ifdef HBOOST_NO_TEMPLATED_IOSTREAMS
/// @cond
...
#endif // HBOOST_NO_TEMPLATED_IOSTREAMS
#endif // HBOOST_NO_IOSTREAM

/// This free function is used by Boost's unordered containers.
/// \note <hboost/functional/hash.hpp> has to be included if this function is used.
template <class Derived, class TypeInfo>
inline std::size_t hash_value(const type_index_facade<Derived, TypeInfo>& lhs) HBOOST_NOEXCEPT {
    return static_cast<Derived const&>(lhs).hash_code();
}

}} // namespace hboost::typeindex

#endif // HBOOST_TYPE_INDEX_TYPE_INDEX_FACADE_HPP
