...
#ifndef HBOOST_TYPE_INDEX_CTTI_TYPE_INDEX_HPP
#define HBOOST_TYPE_INDEX_CTTI_TYPE_INDEX_HPP

/// \file ctti_type_index.hpp
/// \brief Contains hboost::typeindex::ctti_type_index class.
///
/// hboost::typeindex::ctti_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
/// It is used in situations when typeid() method is not available or 
/// HBOOST_TYPE_INDEX_FORCE_NO_RTTI_COMPATIBILITY macro is defined.

#include <hboost/type_index/type_index_facade.hpp>
#include <hboost/type_index/detail/compile_time_type_info.hpp>
...
template <class T>
inline const detail::ctti_data& ctti_construct() HBOOST_NOEXCEPT {
    ...
    return *reinterpret_cast<const detail::ctti_data*>(hboost::detail::ctti<T>::n());
}
...
class ctti_type_index: public type_index_facade<ctti_type_index, detail::ctti_data> {
    ...
    inline std::size_t get_raw_name_length() const HBOOST_NOEXCEPT;

public:
    typedef detail::ctti_data type_info_t;

    inline ctti_type_index() HBOOST_NOEXCEPT
        : data_(&ctti_construct<void>())
    {}

    inline ctti_type_index(const type_info_t& data) HBOOST_NOEXCEPT
        : data_(&data)
    {}
    ...
    template <class T>
    inline static ctti_type_index type_id() HBOOST_NOEXCEPT;

    template <class T>
    inline static ctti_type_index type_id_with_cvr() HBOOST_NOEXCEPT;

    template <class T>
    inline static ctti_type_index type_id_runtime(const T& variable) HBOOST_NOEXCEPT;
};


inline const ctti_type_index::type_info_t& ctti_type_index::type_info() const HBOOST_NOEXCEPT {
    return *data_;
}


template <class T>
inline ctti_type_index ctti_type_index::type_id() HBOOST_NOEXCEPT {
    typedef HBOOST_DEDUCED_TYPENAME hboost::remove_reference<T>::type no_ref_t;
    typedef HBOOST_DEDUCED_TYPENAME hboost::remove_cv<no_ref_t>::type no_cvr_t;
...
template <class T>
inline ctti_type_index ctti_type_index::type_id_with_cvr() HBOOST_NOEXCEPT {
    return ctti_construct<T>();
}


template <class T>
inline ctti_type_index ctti_type_index::type_id_runtime(const T& variable) HBOOST_NOEXCEPT {
    return variable.hboost_type_index_type_id_runtime_();
}


inline const char* ctti_type_index::raw_name() const HBOOST_NOEXCEPT {
    return reinterpret_cast<const char*>(data_);
}

inline std::size_t ctti_type_index::get_raw_name_length() const HBOOST_NOEXCEPT {
...
inline std::size_t ctti_type_index::hash_code() const HBOOST_NOEXCEPT {
    return hboost::hash_range(raw_name(), raw_name() + get_raw_name_length());
}


}} // namespace hboost::typeindex

#endif // HBOOST_TYPE_INDEX_CTTI_TYPE_INDEX_HPP
