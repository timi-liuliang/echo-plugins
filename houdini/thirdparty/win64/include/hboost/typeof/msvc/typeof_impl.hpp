...
# if HBOOST_WORKAROUND(HBOOST_MSVC,>=1300) && defined(_MSC_EXTENSIONS)
        ...
        struct encode_counter
        {
            ...
            {
                HBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 257>::count));
            }
            __if_not_exists(the_counter<N + 256>)
            {
                __if_exists(the_counter<N + 64>)
                {
                    HBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 65>::count));
                }
                __if_not_exists(the_counter<N + 64>)
                {
                    __if_exists(the_counter<N + 16>)
                    {
                        HBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 17>::count));
                    }
                    __if_not_exists(the_counter<N + 16>)
                    {
                        __if_exists(the_counter<N + 4>)
                        {
                            HBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 5>::count));
                        }
                        __if_not_exists(the_counter<N + 4>)
                        {
                            __if_exists(the_counter<N>)
                            {
                                HBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 1>::count));
                            }
...
# define HBOOST_TYPEOF_INDEX(T) (encode_counter<T>::count)
# define HBOOST_TYPEOF_NEXT_INDEX(next)
# else
...
# define HBOOST_TYPEOF_INDEX(T) (sizeof(*hboost::type_of::encode_index((hboost::type_of::encode_counter<1005>*)0)))
...
# if 0 //HBOOST_WORKAROUND(HBOOST_MSVC,==1310)
...
# define HBOOST_TYPEOF(expr) \
    hboost::type_of::msvc_typeid_wrapper<typeid(hboost::type_of::encode_start(expr))>::type

# define HBOOST_TYPEOF_TPL(expr) typename HBOOST_TYPEOF(expr)

# define HBOOST_TYPEOF_NESTED_TYPEDEF_TPL(name,expr) \
struct name {\
    enum {_typeof_register_value=sizeof(typeid(hboost::type_of::typeof_register_type<name>(expr)))};\
