...
#ifndef HBOOST_SIGNALS_SLOT_HEADER
#define HBOOST_SIGNALS_SLOT_HEADER

#include <hboost/signals/detail/signals_common.hpp>
#include <hboost/signals/connection.hpp>
#include <hboost/signals/trackable.hpp>
...
namespace hboost {
  namespace HBOOST_SIGNALS_NAMESPACE {
    namespace detail {
      class HBOOST_SIGNALS_DECL slot_base {
    ...
    get_invocable_slot(const F& f, HBOOST_SIGNALS_NAMESPACE::detail::signal_tag)
      { return reference_wrapper<const F>(f); }

    template<typename F>
    const F&
    get_invocable_slot(const F& f, HBOOST_SIGNALS_NAMESPACE::detail::reference_tag)
      { return f; }

    template<typename F>
    const F&
    get_invocable_slot(const F& f, HBOOST_SIGNALS_NAMESPACE::detail::value_tag)
      { return f; }

    // Get the slot so that it can be inspected for trackable objects
    template<typename F>
    const F&
    get_inspectable_slot(const F& f, HBOOST_SIGNALS_NAMESPACE::detail::signal_tag)
      { return f; }

    template<typename F>
    const F&
    get_inspectable_slot(const reference_wrapper<F>& f, HBOOST_SIGNALS_NAMESPACE::detail::reference_tag)
      { return f.get(); }

    template<typename F>
    const F&
    get_inspectable_slot(const F& f, HBOOST_SIGNALS_NAMESPACE::detail::value_tag)
      { return f; }

    // Determines the type of the slot - is it a signal, a reference to a
    // slot or just a normal slot.
    template<typename F>
    typename HBOOST_SIGNALS_NAMESPACE::detail::get_slot_tag<F>::type
    tag_type(const F&)
    {
      typedef typename HBOOST_SIGNALS_NAMESPACE::detail::get_slot_tag<F>::type
        the_tag_type;
  ...
  class slot : public HBOOST_SIGNALS_NAMESPACE::detail::slot_base {
    typedef HBOOST_SIGNALS_NAMESPACE::detail::slot_base inherited;
    typedef typename inherited::data_t data_t;

  public:
    template<typename F>
    slot(const F& f) : slot_function(HBOOST_SIGNALS_NAMESPACE::get_invocable_slot(f, HBOOST_SIGNALS_NAMESPACE::tag_type(f)))
    {
      ...
      // have been made.
      HBOOST_SIGNALS_NAMESPACE::detail::bound_objects_visitor 
        do_bind(this->data->bound_objects);
      visit_each(do_bind, 
                 HBOOST_SIGNALS_NAMESPACE::get_inspectable_slot
                   (f, HBOOST_SIGNALS_NAMESPACE::tag_type(f)));
      create_connection();
    }
...
#endif // HBOOST_SIGNALS_SLOT_HEADER
