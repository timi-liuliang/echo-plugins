...
// ----------------------------------------------------------------------------
#ifndef HBOOST_PROPERTY_TREE_DETAIL_PTREE_IMPLEMENTATION_HPP_INCLUDED
#define HBOOST_PROPERTY_TREE_DETAIL_PTREE_IMPLEMENTATION_HPP_INCLUDED

#include <hboost/iterator/iterator_adaptor.hpp>
#include <hboost/iterator/reverse_iterator.hpp>
#include <hboost/assert.hpp>
#include <hboost/utility/swap.hpp>
#include <memory>

#if (defined(HBOOST_MSVC) && \
     (_MSC_FULL_VER >= 160000000 && _MSC_FULL_VER < 170000000)) || \
    (defined(HBOOST_INTEL_WIN) && \
     defined(HBOOST_DINKUMWARE_STDLIB))
#define HBOOST_PROPERTY_TREE_PAIR_BUG
#endif

namespace hboost { namespace property_tree
{
    ...
    struct basic_ptree<K, D, C>::subs
    {
...
#if defined(HBOOST_PROPERTY_TREE_PAIR_BUG)
        ...
        static by_name_index& assoc(self_type *s) {
            return ch(s).HBOOST_NESTED_TEMPLATE get<by_name>();
        }
    ...
    template <class K, class D, class C>
    class basic_ptree<K, D, C>::const_iterator : public hboost::iterator_adaptor<
    ...
    class basic_ptree<K, D, C>::reverse_iterator
        : public hboost::reverse_iterator<iterator>
    {
    public:
        reverse_iterator() {}
        explicit reverse_iterator(iterator b)
            : hboost::reverse_iterator<iterator>(b)
        {}
    };
    template <class K, class D, class C>
    class basic_ptree<K, D, C>::const_reverse_iterator
        : public hboost::reverse_iterator<const_iterator>
    {
    public:
        const_reverse_iterator() {}
        explicit const_reverse_iterator(const_iterator b)
            : hboost::reverse_iterator<const_iterator>(b)
        {}
        const_reverse_iterator(
    ...
    class basic_ptree<K, D, C>::assoc_iterator
        : public hboost::iterator_adaptor<assoc_iterator,
                                         typename subs::by_name_index::iterator,
                                         value_type>
    {
        friend class hboost::iterator_core_access;
        typedef hboost::iterator_adaptor<assoc_iterator,
                                         typename subs::by_name_index::iterator,
                                         value_type>
            baset;
    public:
    ...
    class basic_ptree<K, D, C>::const_assoc_iterator
        : public hboost::iterator_adaptor<const_assoc_iterator,
        ...
        basic_ptree<K, D, C>::to_iterator(assoc_iterator ai)
    {
        return iterator(subs::ch(this).
            HBOOST_NESTED_TEMPLATE project<0>(ai.base()));
    }
        ...
        basic_ptree<K, D, C>::get_child(const path_type &path)
    {
        ...
        if (!n) {
            HBOOST_PROPERTY_TREE_THROW(ptree_bad_path("No such node", path));
        }
        return *n;
    }
    ...
    typename hboost::enable_if<detail::is_translator<Translator>, Type>::type
    basic_ptree<K, D, C>::get_value(Translator tr) const
    {
        if(hboost::optional<Type> o = get_value_optional<Type>(tr)) {
            return *o;
        }
        HBOOST_PROPERTY_TREE_THROW(ptree_bad_data(
    ...
    template<class K, class D, class C>
    template<class Type> inline
    typename hboost::disable_if<detail::is_translator<Type>, Type>::type
    ...
    basic_ptree<K, D, C>::get(const path_type &path,
                              Translator tr) const
    {
        return get_child(path).HBOOST_NESTED_TEMPLATE get_value<Type>(tr);
    }
    ...
    {
        return get_child(path).HBOOST_NESTED_TEMPLATE get_value<Type>();
    }
                                                         ...
                                                         Translator tr) const
    {
        if (optional<const self_type&> child = get_child_optional(path))
            return child.get().
                HBOOST_NESTED_TEMPLATE get_value_optional<Type>(tr);
        else
    ...
    {
        if (optional<const self_type&> child = get_child_optional(path))
            return child.get().HBOOST_NESTED_TEMPLATE get_value_optional<Type>();
        else
            return optional<Type>();
    }
    ...
    {
        HBOOST_ASSERT(!p.empty() && "Empty path not allowed for put_child.");
        if(p.single()) {
...
#if defined(HBOOST_PROPERTY_TREE_PAIR_BUG)
#undef HBOOST_PROPERTY_TREE_PAIR_BUG
#endif
