...
// ----------------------------------------------------------------------------
#ifndef HBOOST_PROPERTY_TREE_DETAIL_XML_PARSER_READ_RAPIDXML_HPP_INCLUDED
#define HBOOST_PROPERTY_TREE_DETAIL_XML_PARSER_READ_RAPIDXML_HPP_INCLUDED

#include <hboost/property_tree/ptree.hpp>
#include <hboost/property_tree/detail/xml_parser_error.hpp>
#include <hboost/property_tree/detail/xml_parser_flags.hpp>
#include <hboost/property_tree/detail/xml_parser_utils.hpp>
#include <hboost/property_tree/detail/rapidxml.hpp>
#include <vector>

namespace hboost { namespace property_tree { namespace xml_parser
{

    template<class Ptree, class Ch>
    void read_xml_node(detail::hboostrapidxml::xml_node<Ch> *node,
                       Ptree &pt, int flags)
    {
        using namespace detail::hboostrapidxml;
        switch (node->type())
        {
            // Element nodes
            case node_element: 
        ...
        try {
            ...
            if (flags & no_comments) {
                if (flags & trim_whitespace)
                    doc.HBOOST_NESTED_TEMPLATE parse<f_tws>(&v.front());
                else
                    doc.HBOOST_NESTED_TEMPLATE parse<0>(&v.front());
            } else {
                if (flags & trim_whitespace)
                    doc.HBOOST_NESTED_TEMPLATE parse<f_tws_c>(&v.front());
                else
                    doc.HBOOST_NESTED_TEMPLATE parse<f_c>(&v.front());
            }
