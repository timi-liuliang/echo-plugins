...
// ----------------------------------------------------------------------------
#ifndef HBOOST_PROPERTY_TREE_DETAIL_XML_PARSER_UTILS_HPP_INCLUDED
#define HBOOST_PROPERTY_TREE_DETAIL_XML_PARSER_UTILS_HPP_INCLUDED
    ...
    Str decode_char_entities(const Str &s)
    {
        ...
        for (typename Str::const_iterator it = s.begin(); it != end; ++it)
        {
            if (*it == Ch('&'))
            {
                ...
                if (semicolon == end)
                    HBOOST_PROPERTY_TREE_THROW(xml_parser_error("invalid character entity", "", 0));
