...
#ifndef HBOOST_PROPERTY_TREE_STREAM_TRANSLATOR_HPP_INCLUDED
#define HBOOST_PROPERTY_TREE_STREAM_TRANSLATOR_HPP_INCLUDED

#include <hboost/property_tree/ptree_fwd.hpp>

#include <hboost/optional.hpp>
#include <hboost/optional/optional_io.hpp>
...
namespace hboost { namespace property_tree
{
    ...
    namespace detail
    {
        ...
        struct is_inexact_impl
        {
            ...
            struct test
            {
                typedef hboost::false_type type;
            };
        };
        template <>
        struct is_inexact_impl<true>
        {
            ...
            struct test
            {
              typedef hboost::integral_constant<bool,
        ...
        struct is_inexact
        {
            typedef typename hboost::decay<F>::type decayed;
            typedef typename is_inexact_impl<
                std::numeric_limits<decayed>::is_specialized
            >::HBOOST_NESTED_TEMPLATE test<decayed>::type type;
    ...
    struct customize_stream<Ch, Traits, F,
        typename hboost::enable_if< detail::is_inexact<F> >::type
    >
    {
    ...
    public:
        ...
        hboost::optional<E> get_value(const internal_type &v) {
            std::basic_istringstream<Ch, Traits, Alloc> iss(v);
            iss.imbue(m_loc);
            E e;
            customized::extract(iss, e);
            if(iss.fail() || iss.bad() || iss.get() != Traits::eof()) {
                return hboost::optional<E>();
            }
            return e;
        }
        hboost::optional<internal_type> put_value(const E &v) {
            ...
            return hboost::optional<internal_type>();
        }
