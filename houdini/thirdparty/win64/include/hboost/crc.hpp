...
#ifndef HBOOST_CRC_HPP
#define HBOOST_CRC_HPP

#include <hboost/config.hpp>   // for HBOOST_STATIC_CONSTANT, etc.
...
#if !(defined(HBOOST_NO_DEPENDENT_TYPES_IN_TEMPLATE_VALUE_PARAMETERS) || (defined(HBOOST_MSVC) && (HBOOST_MSVC <= 1300)))
#define HBOOST_CRC_PARM_TYPE  typename ::hboost::uint_t<Bits>::fast
#else
#define HBOOST_CRC_PARM_TYPE  unsigned long
#endif
...
#if defined(__GNUC__) || !defined(HBOOST_NO_EXPLICIT_FUNCTION_TEMPLATE_ARGUMENTS)
#define HBOOST_CRC_DUMMY_PARM_TYPE
#define HBOOST_CRC_DUMMY_INIT
#define HBOOST_ACRC_DUMMY_PARM_TYPE
#define HBOOST_ACRC_DUMMY_INIT
#else
namespace hboost { namespace detail {
    template < std::size_t Bits, HBOOST_CRC_PARM_TYPE TruncPoly,
     HBOOST_CRC_PARM_TYPE InitRem, HBOOST_CRC_PARM_TYPE FinalXor,
     bool ReflectIn, bool ReflectRem >
    struct dummy_crc_argument  { };
} }
#define HBOOST_CRC_DUMMY_PARM_TYPE   , detail::dummy_crc_argument<Bits, \
 TruncPoly, InitRem, FinalXor, ReflectIn, ReflectRem> *p_
#define HBOOST_CRC_DUMMY_INIT        HBOOST_CRC_DUMMY_PARM_TYPE = 0
#define HBOOST_ACRC_DUMMY_PARM_TYPE  , detail::dummy_crc_argument<Bits, \
 TruncPoly, 0, 0, false, false> *p_
#define HBOOST_ACRC_DUMMY_INIT       HBOOST_ACRC_DUMMY_PARM_TYPE = 0
#endif
...
template < std::size_t Bits, HBOOST_CRC_PARM_TYPE TruncPoly = 0u,
           HBOOST_CRC_PARM_TYPE InitRem = 0u,
           HBOOST_CRC_PARM_TYPE FinalXor = 0u, bool ReflectIn = false,
...
namespace detail
{
    ...
    template < std::size_t Bits, HBOOST_CRC_PARM_TYPE TruncPoly, bool Reflect >
        struct crc_table_t;

    template < std::size_t Bits, bool DoReflect >
        class crc_helper;

    #ifndef HBOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
    template < std::size_t Bits >
...
public:
    ...
    HBOOST_STATIC_CONSTANT( std::size_t, bit_count = Bits );
    HBOOST_STATIC_CONSTANT( value_type, truncated_polynominal = TruncPoly );
    HBOOST_STATIC_CONSTANT( value_type, initial_remainder = InitRem );
    HBOOST_STATIC_CONSTANT( value_type, final_xor_value = FinalXor );
    HBOOST_STATIC_CONSTANT( bool, reflect_input = ReflectIn );
    HBOOST_STATIC_CONSTANT( bool, reflect_remainder = ReflectRem );
...
private:
    ...
    HBOOST_STATIC_CONSTANT( bool, reflect_output = (ReflectRem != ReflectIn) );

    #ifndef __BORLANDC__
    #define HBOOST_CRC_REF_OUT_VAL  reflect_output
    #else
    typedef crc_optimal  self_type;
    #define HBOOST_CRC_REF_OUT_VAL  (self_type::reflect_output)
    #endif
    ...
    typedef detail::crc_helper<Bits, HBOOST_CRC_REF_OUT_VAL>  reflect_out_type;

    #undef HBOOST_CRC_REF_OUT_VAL
...
namespace detail
{
    ...
    struct high_uint_t
        : hboost::uint_t< Bits >
    {
        typedef hboost::uint_t<Bits>        base_type;
...
#else
        HBOOST_STATIC_CONSTANT( least, high_bit = (least( 1u ) << ( Bits
         - 1u )) );
        HBOOST_STATIC_CONSTANT( fast, high_bit_fast = (fast( 1u ) << ( Bits
         - 1u )) );
#endif

    };  // hboost::detail::high_uint_t
    ...
    struct reflector
    {
        typedef typename hboost::uint_t<Bits>::fast  value_type;

        static  value_type  reflect( value_type x );

    };  // hboost::detail::reflector
        ...
        : high_uint_t< Bits >
    {
        ...
        #else
        HBOOST_STATIC_CONSTANT( least, high_bit = base_type::high_bit );
        HBOOST_STATIC_CONSTANT( fast, high_bit_fast = base_type::high_bit_fast );
        #endif
        ...
        HBOOST_STATIC_CONSTANT( fast, sig_bits_fast = static_cast<fast>(sig_bits) );
#else
        HBOOST_STATIC_CONSTANT( fast, sig_bits_fast = fast(sig_bits) );
#endif
    };  // hboost::detail::mask_uint_t
    ...
    struct crc_table_t
    {
        HBOOST_STATIC_CONSTANT( std::size_t, byte_combos = (1ul << CHAR_BIT) );
...
#define HBOOST_CRC_OPTIMAL_NAME  crc_optimal<Bits, TruncPoly, InitRem, \
...
template < std::size_t Bits, HBOOST_CRC_PARM_TYPE TruncPoly,
           ...
           bool ReflectIn, bool ReflectRem >
inline
HBOOST_CRC_OPTIMAL_NAME::crc_optimal
(
    typename HBOOST_CRC_OPTIMAL_NAME::value_type  init_rem  // = InitRem
)
...
template < std::size_t Bits, HBOOST_CRC_PARM_TYPE TruncPoly,
           ...
           bool ReflectIn, bool ReflectRem >
inline
typename HBOOST_CRC_OPTIMAL_NAME::value_type
HBOOST_CRC_OPTIMAL_NAME::get_truncated_polynominal
(
) const
{
    return TruncPoly;
...
inline
typename HBOOST_CRC_OPTIMAL_NAME::value_type
HBOOST_CRC_OPTIMAL_NAME::get_initial_remainder
(
) const
{
    return InitRem;
...
inline
typename HBOOST_CRC_OPTIMAL_NAME::value_type
HBOOST_CRC_OPTIMAL_NAME::get_final_xor_value
(
) const
{
    return FinalXor;
...
inline
bool
HBOOST_CRC_OPTIMAL_NAME::get_reflect_input
(
) const
{
    return ReflectIn;
...
inline
bool
HBOOST_CRC_OPTIMAL_NAME::get_reflect_remainder
(
) const
{
    return ReflectRem;
...
inline
typename HBOOST_CRC_OPTIMAL_NAME::value_type
HBOOST_CRC_OPTIMAL_NAME::get_interim_remainder
(
) const
{
...
inline
void
HBOOST_CRC_OPTIMAL_NAME::reset
(
    typename HBOOST_CRC_OPTIMAL_NAME::value_type  new_rem  // = InitRem
)
{
...
inline
void
HBOOST_CRC_OPTIMAL_NAME::process_byte
(
    unsigned char  byte
)
{
...
template < std::size_t Bits, HBOOST_CRC_PARM_TYPE TruncPoly,
           ...
           bool ReflectIn, bool ReflectRem >
void
HBOOST_CRC_OPTIMAL_NAME::process_block
(
    void const *  bytes_begin,
    void const *  bytes_end
)
...
inline
void
HBOOST_CRC_OPTIMAL_NAME::process_bytes
(
    void const *   buffer,
    std::size_t  byte_count
)
...
inline
typename HBOOST_CRC_OPTIMAL_NAME::value_type
HBOOST_CRC_OPTIMAL_NAME::checksum
(
) const
{
...
inline
void
HBOOST_CRC_OPTIMAL_NAME::operator ()
(
    unsigned char  byte
)
{
...
(
    void const *  buffer,
    std::size_t   byte_count
    HBOOST_CRC_DUMMY_INIT
)
{
    HBOOST_CRC_OPTIMAL_NAME  computer;
...
)
{
   ...
   // HBOOST_ACRC_DUMMY_INIT added a third argument (for a workaround), the "0"
...
#undef HBOOST_CRC_OPTIMAL_NAME
#undef HBOOST_ACRC_DUMMY_INIT
#undef HBOOST_ACRC_DUMMY_PARM_TYPE
#undef HBOOST_CRC_DUMMY_INIT
#undef HBOOST_CRC_DUMMY_PARM_TYPE
#undef HBOOST_CRC_PARM_TYPE


#endif  // HBOOST_CRC_HPP
