...
#ifndef HBOOST_THREAD_COMPLETION_LATCH_HPP
#define HBOOST_THREAD_COMPLETION_LATCH_HPP

#include <hboost/thread/detail/config.hpp>
#include <hboost/thread/detail/delete.hpp>
#include <hboost/thread/detail/counter.hpp>
...
namespace hboost
{
  ...
  private:
    ...
    struct around_wait
    {
      ...
      hboost::unique_lock<hboost::mutex> &lk_;
      around_wait(completion_latch &that, hboost::unique_lock<hboost::mutex> &lk)
      : that_(that), lk_(lk)
      {
  ...
  public:
    HBOOST_THREAD_NO_COPYABLE( completion_latch )
    ...
    template <typename F>
    completion_latch(std::size_t count, HBOOST_THREAD_RV_REF(F) funct) :
    count_(count),
    funct_(hboost::move(funct)),
    waiters_(0),
    leavers_(0)
    {
    }
    ...
    void wait()
    {
      hboost::unique_lock<hboost::mutex> lk(mutex_);
    ...
    {
      hboost::lock_guard<hboost::mutex> lk(mutex_);
      //HBOOST_ASSERT(count_ == 0);
      count_ = count;
    }
...
#ifdef HBOOST_NO_CXX11_HDR_FUNCTIONAL
    template <typename F>
    completion_function then(HBOOST_THREAD_RV_REF(F) funct)
    {
      hboost::lock_guard<hboost::mutex> lk(mutex_);
