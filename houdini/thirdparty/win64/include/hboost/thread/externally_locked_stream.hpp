...
#ifndef HBOOST_THREAD_EXTERNALLY_LOCKED_STREAM_HPP
#define HBOOST_THREAD_EXTERNALLY_LOCKED_STREAM_HPP

#include <hboost/thread/detail/config.hpp>
#include <hboost/thread/detail/move.hpp>
#include <hboost/thread/detail/delete.hpp>

#include <hboost/thread/externally_locked.hpp>
...
namespace hboost
{
  ...
  public:
    ...
    HBOOST_THREAD_MOVABLE_ONLY( stream_guard)
    ...
    stream_guard(HBOOST_THREAD_RV_REF(stream_guard) rhs) HBOOST_NOEXCEPT
    : mtx_(rhs.mtx_)
    {
      rhs.mtx_= 0;
    }
    ...
    Stream& get() const
    {
      HBOOST_THREAD_ASSERT_PRECONDITION(  mtx_, lock_error() );
  ...
  public:
    HBOOST_THREAD_NO_COPYABLE( externally_locked_stream)

    /**
     * Effects: Constructs an externally locked object storing the cloaked reference object.
     */
    externally_locked_stream(Stream& stream, RecursiveMutex& mtx) HBOOST_NOEXCEPT :
      base_type(stream, mtx)
    {
    }

    stream_guard<Stream, RecursiveMutex> hold() HBOOST_NOEXCEPT
    {
