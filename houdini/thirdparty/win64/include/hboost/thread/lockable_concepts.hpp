...
#ifndef HBOOST_THREAD_LOCKABLE_CONCEPTS_HPP
#define HBOOST_THREAD_LOCKABLE_CONCEPTS_HPP
...
namespace hboost
{
  ...
  struct BasicLockable
  {
    ...
    HBOOST_CONCEPT_USAGE(BasicLockable)
    {
      l.lock();
      l.unlock();
    }
  ...
  struct Lockable
  {
    HBOOST_CONCEPT_ASSERT(( BasicLockable<Mutex> ));

    HBOOST_CONCEPT_USAGE(Lockable)
    {
  ...
  {
    HBOOST_CONCEPT_ASSERT(( Lockable<Mutex> ));

    HBOOST_CONCEPT_USAGE(TimedLockable)
    {
  ...
  {
    HBOOST_CONCEPT_ASSERT(( TimedLockable<Mutex> ));

    HBOOST_CONCEPT_USAGE(SharedLockable)
    {
      l.lock_shared();
      l.unlock_shared();
  ...
  {
    HBOOST_CONCEPT_ASSERT(( SharedLockable<Mutex> ));

    HBOOST_CONCEPT_USAGE(UpgradeLockable)
    {
      l.lock_upgrade();
      l.unlock_upgrade();
