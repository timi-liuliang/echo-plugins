...
#ifndef HBOOST_THREAD_SYNC_TIMED_QUEUE_HPP
#define HBOOST_THREAD_SYNC_TIMED_QUEUE_HPP

#include <hboost/thread/detail/config.hpp>

#include <hboost/thread/concurrent_queues/sync_priority_queue.hpp>
#include <hboost/chrono/duration.hpp>
#include <hboost/chrono/time_point.hpp>
#include <hboost/chrono/system_clocks.hpp>
#include <hboost/chrono/chrono_io.hpp>
...
namespace detail
{
  ...
  struct scheduled_type
  {
    ...
    HBOOST_THREAD_COPYABLE_AND_MOVABLE(scheduled_type)

    scheduled_type(T const& pdata, time_point tp) : data(pdata), time(tp) {}
    scheduled_type(HBOOST_THREAD_RV_REF(T) pdata, time_point tp) : data(hboost::move(pdata)), time(tp) {}

    scheduled_type(scheduled_type const& other) : data(other.data), time(other.time) {}
    scheduled_type& operator=(HBOOST_THREAD_COPY_ASSIGN_REF(scheduled_type) other) {
      data = other.data;
      time = other.time;
      return *this;
    }

    scheduled_type(HBOOST_THREAD_RV_REF(scheduled_type) other) : data(hboost::move(other.data)), time(other.time) {}
    scheduled_type& operator=(HBOOST_THREAD_RV_REF(scheduled_type) other) {
      data = hboost::move(other.data);
      time = other.time;
      return *this;
    }
  ...
  public:
    ...
    template <class Duration>
    void push(HBOOST_THREAD_RV_REF(T) elem, chrono::time_point<clock,Duration> const& tp);
    template <class Rep, class Period>
    void push(HBOOST_THREAD_RV_REF(T) elem, chrono::duration<Rep,Period> const& dura);
    ...
    template <class Duration>
    queue_op_status try_push(HBOOST_THREAD_RV_REF(T) elem, chrono::time_point<clock,Duration> const& tp);
    template <class Rep, class Period>
    queue_op_status try_push(HBOOST_THREAD_RV_REF(T) elem, chrono::duration<Rep,Period> const& dura);
  ...
  private:
    ...
    sync_timed_queue(HBOOST_THREAD_RV_REF(sync_timed_queue));
    sync_timed_queue& operator=(HBOOST_THREAD_RV_REF(sync_timed_queue));
  }; //end class
  ...
  template <class T, class Clock>
  template <class Duration>
  void sync_timed_queue<T, Clock>::push(HBOOST_THREAD_RV_REF(T) elem, chrono::time_point<clock,Duration> const& tp)
  {
    super::push(stype(hboost::move(elem),tp));
  }

  template <class T, class Clock>
  template <class Rep, class Period>
  void sync_timed_queue<T, Clock>::push(HBOOST_THREAD_RV_REF(T) elem, chrono::duration<Rep,Period> const& dura)
  {
    push(hboost::move(elem), clock::now() + dura);
  }
  ...
  template <class T, class Clock>
  template <class Duration>
  queue_op_status sync_timed_queue<T, Clock>::try_push(HBOOST_THREAD_RV_REF(T) elem, chrono::time_point<clock,Duration> const& tp)
  {
    return super::try_push(stype(hboost::move(elem), tp));
  }

  template <class T, class Clock>
  template <class Rep, class Period>
  queue_op_status sync_timed_queue<T, Clock>::try_push(HBOOST_THREAD_RV_REF(T) elem, chrono::duration<Rep,Period> const& dura)
  {
    return try_push(hboost::move(elem), clock::now() + dura);
  }
  ...
  T sync_timed_queue<T, Clock>::pull(unique_lock<mutex>&)
  {
    ...
    return hboost::move(super::data_.pull().data);
#else
  ...
  {
#if ! defined  HBOOST_NO_CXX11_RVALUE_REFERENCES
    elem = hboost::move(super::data_.pull().data);
#else
