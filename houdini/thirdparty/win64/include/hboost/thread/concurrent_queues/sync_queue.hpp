#ifndef HBOOST_THREAD_CONCURRENT_QUEUES_SYNC_QUEUE_HPP
#define HBOOST_THREAD_CONCURRENT_QUEUES_SYNC_QUEUE_HPP
...
namespace concurrent
{
  ...
  public:
    ...
    HBOOST_THREAD_NO_COPYABLE(sync_queue)
    inline sync_queue();
    ...
    inline queue_op_status try_push(HBOOST_THREAD_RV_REF(value_type) x);
    inline queue_op_status nonblocking_push(HBOOST_THREAD_RV_REF(value_type) x);
    inline queue_op_status wait_push(HBOOST_THREAD_RV_REF(value_type) x);
  ...
  private:
    ...
    inline queue_op_status try_push(HBOOST_THREAD_RV_REF(value_type) x, unique_lock<mutex>& lk);
    inline queue_op_status wait_push(HBOOST_THREAD_RV_REF(value_type) x, unique_lock<mutex>& lk);
    ...
    inline void push(HBOOST_THREAD_RV_REF(value_type) elem, unique_lock<mutex>& lk)
    {
...
//  }
  ...
  template <class ValueType, class Container>
  queue_op_status sync_queue<ValueType, Container>::try_push(HBOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    if (super::closed(lk)) return queue_op_status::closed;
    push(hboost::move(elem), lk);
    return queue_op_status::success;
  }

  template <class ValueType, class Container>
  queue_op_status sync_queue<ValueType, Container>::try_push(HBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(super::mtx_);
  ...
  template <class ValueType, class Container>
  queue_op_status sync_queue<ValueType, Container>::wait_push(HBOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    if (super::closed(lk)) return queue_op_status::closed;
    push(hboost::move(elem), lk);
    return queue_op_status::success;
  ...
  template <class ValueType, class Container>
  queue_op_status sync_queue<ValueType, Container>::wait_push(HBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(super::mtx_);
    return wait_push(hboost::move(elem), lk);
  }

  template <class ValueType, class Container>
  queue_op_status sync_queue<ValueType, Container>::nonblocking_push(HBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(super::mtx_, try_to_lock);
    if (!lk.owns_lock())
    {
  ...
  template <class ValueType, class Container>
  void sync_queue<ValueType, Container>::push(HBOOST_THREAD_RV_REF(ValueType) elem)
  {
      unique_lock<mutex> lk(super::mtx_);
      super::throw_if_closed(lk);
      push(hboost::move(elem), lk);
  ...
  template <class ValueType, class Container>
  sync_queue<ValueType, Container>& operator<<(sync_queue<ValueType, Container>& sbq, HBOOST_THREAD_RV_REF(ValueType) elem)
  {
    sbq.push(hboost::move(elem));
    return sbq;
  }
