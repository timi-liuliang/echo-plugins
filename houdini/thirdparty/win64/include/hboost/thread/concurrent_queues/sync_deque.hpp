#ifndef HBOOST_THREAD_CONCURRENT_QUEUES_SYNC_DEQUE_HPP
#define HBOOST_THREAD_CONCURRENT_QUEUES_SYNC_DEQUE_HPP
...
#include <hboost/thread/concurrent_queues/detail/sync_queue_base.hpp>
#include <hboost/thread/concurrent_queues/queue_op_status.hpp>
#include <hboost/thread/condition_variable.hpp>
#include <hboost/thread/csbl/devector.hpp>
...
namespace concurrent
{
  ...
  public:
    ...
    HBOOST_THREAD_NO_COPYABLE(sync_deque)
    inline sync_deque();
  ...
  private:
    ...
    {
      elem = hboost::move(super::data_.front());
      super::data_.pop_front();
    }
    inline value_type pull_front(unique_lock<mutex>& )
    {
      value_type e = hboost::move(super::data_.front());
      super::data_.pop_front();
      return hboost::move(e);
    }
    ...
    inline void push_back(HBOOST_THREAD_RV_REF(value_type) elem, unique_lock<mutex>& lk)
    {
      super::data_.push_back(hboost::move(elem));
      super::notify_not_empty_if_needed(lk);
    }
  };
...
//  }
  ...
  template <class ValueType, class Container>
  queue_op_status sync_deque<ValueType, Container>::try_push_back(HBOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    if (super::closed(lk)) return queue_op_status::closed;
    push_back(hboost::move(elem), lk);
    return queue_op_status::success;
  }

  template <class ValueType, class Container>
  queue_op_status sync_deque<ValueType, Container>::try_push_back(HBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(super::mtx_);
  ...
  template <class ValueType, class Container>
  queue_op_status sync_deque<ValueType, Container>::wait_push_back(HBOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    if (super::closed(lk)) return queue_op_status::closed;
    push_back(hboost::move(elem), lk);
    return queue_op_status::success;
  ...
  template <class ValueType, class Container>
  queue_op_status sync_deque<ValueType, Container>::wait_push_back(HBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(super::mtx_);
    return wait_push_back(hboost::move(elem), lk);
  }

  template <class ValueType, class Container>
  queue_op_status sync_deque<ValueType, Container>::nonblocking_push_back(HBOOST_THREAD_RV_REF(ValueType) elem)
  {
    unique_lock<mutex> lk(super::mtx_, try_to_lock);
    if (!lk.owns_lock())
    {
  ...
  template <class ValueType, class Container>
  void sync_deque<ValueType, Container>::push_back(HBOOST_THREAD_RV_REF(ValueType) elem)
  {
      unique_lock<mutex> lk(super::mtx_);
      super::throw_if_closed(lk);
      push_back(hboost::move(elem), lk);
  ...
  template <class ValueType, class Container>
  sync_deque<ValueType, Container>& operator<<(sync_deque<ValueType, Container>& sbq, HBOOST_THREAD_RV_REF(ValueType) elem)
  {
