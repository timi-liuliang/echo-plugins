#ifndef HBOOST_THREAD_CONCURRENT_QUEUES_DETAIL_SYNC_DEQUE_BASE_HPP
#define HBOOST_THREAD_CONCURRENT_QUEUES_DETAIL_SYNC_DEQUE_BASE_HPP
...
namespace detail
{
  ...
  public:
    ...
    HBOOST_THREAD_NO_COPYABLE(sync_deque_base)
  ...
  protected:
    ...
    inline size_type size(lock_guard<mutex>& ) const HBOOST_NOEXCEPT
    {
      return data_.size();
    }
  ...
  void sync_deque_base<ValueType, Queue>::throw_if_closed(unique_lock<mutex>& lk)
  {
    if (closed(lk))
    {
      HBOOST_THROW_EXCEPTION( sync_deque_is_closed() );
    }
  }
  template <class ValueType, class Queue>
