#ifndef HBOOST_THREAD_QUEUE_VIEWS_HPP
#define HBOOST_THREAD_QUEUE_VIEWS_HPP
...
#include <hboost/thread/concurrent_queues/deque_base.hpp>
...
namespace concurrent
{
  ...
  public:
    ...
    deque_back_view(Queue& q) HBOOST_NOEXCEPT : queue(&q) {}
    ...
    void push(HBOOST_THREAD_RV_REF(value_type) x) { queue->push_back(hboost::move(x)); }
    queue_op_status try_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->try_push_back(hboost::move(x)); }
    queue_op_status nonblocking_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->nonblocking_push_back(hboost::move(x)); }
    queue_op_status wait_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->wait_push_back(hboost::move(x)); }
  };
  ...
  public:
    ...
    // Constructors/Assignment/Destructors
    deque_front_view(Queue& q) HBOOST_NOEXCEPT : queue(&q) {}
    ...
    void push(HBOOST_THREAD_RV_REF(value_type) x) { queue->push_front(forward<value_type>(x)); }
    queue_op_status try_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->try_push_front(forward<value_type>(x)); }
    queue_op_status nonblocking_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->nonblocking_push_front(forward<value_type>(x)); }
    queue_op_status wait_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->wait_push_front(forward<value_type>(x)); }
  ...
  struct deque_back : deque_back_view<deque_base<T> >
  {
    ...
    deque_back(deque_base<T>& q) HBOOST_NOEXCEPT : base_type(q) {}
  };
  template <class T>
  struct deque_front : deque_front_view<deque_base<T> >
  {
    typedef deque_front_view<deque_base<T> > base_type;
    deque_front(deque_base<T>& q) HBOOST_NOEXCEPT : base_type(q) {}
