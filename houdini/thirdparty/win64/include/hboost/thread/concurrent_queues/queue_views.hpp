...
#include <hboost/thread/concurrent_queues/queue_base.hpp>
...
namespace concurrent
{
  ...
  public:
    ...
    queue_back_view(Queue& q) HBOOST_NOEXCEPT : queue(&q) {}
    ...
    void push(HBOOST_THREAD_RV_REF(value_type) x) { queue->push(hboost::move(x)); }
    queue_op_status try_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->try_push(hboost::move(x)); }
    queue_op_status nonblocking_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->nonblocking_push(hboost::move(x)); }
    queue_op_status wait_push(HBOOST_THREAD_RV_REF(value_type) x) { return queue->wait_push(hboost::move(x)); }
  };
  ...
  public:
    ...
    // Constructors/Assignment/Destructors
    queue_front_view(Queue& q) HBOOST_NOEXCEPT : queue(&q) {}
  ...
  struct queue_back : queue_back_view<queue_base<T> >
  {
    ...
    queue_back(queue_base<T>& q) HBOOST_NOEXCEPT : base_type(q) {}
  };
  template <class T>
  struct queue_front : queue_front_view<queue_base<T> >
  {
    typedef queue_front_view<queue_base<T> > base_type;
    queue_front(queue_base<T>& q) HBOOST_NOEXCEPT : base_type(q) {}
