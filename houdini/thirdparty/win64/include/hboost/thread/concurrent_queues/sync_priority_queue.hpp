...
#ifndef HBOOST_THREAD_SYNC_PRIORITY_QUEUE
#define HBOOST_THREAD_SYNC_PRIORITY_QUEUE
...
namespace detail {
  ...
  public:
      ...
      void push(HBOOST_RV_REF(Type) element)
      {
          _elements.push_back(hboost::move(element));
          std::push_heap(_elements.begin(), _elements.end(), _compare);
      }
      ...
      Type pull()
      {
          Type result = hboost::move(_elements.front());
          pop();
          return hboost::move(result);
      }
...
namespace concurrent
{
  ...
  class sync_priority_queue
    : public detail::sync_queue_base<ValueType, hboost::detail::priority_queue<ValueType,Container,Compare> >
  {
    typedef detail::sync_queue_base<ValueType, hboost::detail::priority_queue<ValueType,Container,Compare> >  super;
  ...
  public:
    ...
    void push(HBOOST_THREAD_RV_REF(ValueType) elem);

    queue_op_status try_push(const ValueType& elem);
    queue_op_status try_push(HBOOST_THREAD_RV_REF(ValueType) elem);
  ...
  private:
    ...
    void push(unique_lock<mutex>&, HBOOST_THREAD_RV_REF(ValueType) elem);
    void push(lock_guard<mutex>&, HBOOST_THREAD_RV_REF(ValueType) elem);

    queue_op_status try_push(unique_lock<mutex>&, const ValueType& elem);
    queue_op_status try_push(unique_lock<mutex>&, HBOOST_THREAD_RV_REF(ValueType) elem);
    ...
    sync_priority_queue(HBOOST_THREAD_RV_REF(sync_priority_queue));
    sync_priority_queue& operator= (HBOOST_THREAD_RV_REF(sync_priority_queue));
  }; //end class
  ...
  //////////////////////
  template <class T, class Container,class Cmp>
  void sync_priority_queue<T,Container,Cmp>::push(unique_lock<mutex>& lk, HBOOST_THREAD_RV_REF(T) elem)
  {
    super::throw_if_closed(lk);
    super::data_.push(hboost::move(elem));
    super::notify_not_empty_if_needed(lk);
  }
  template <class T, class Container,class Cmp>
  void sync_priority_queue<T,Container,Cmp>::push(lock_guard<mutex>& lk, HBOOST_THREAD_RV_REF(T) elem)
  {
    super::throw_if_closed(lk);
    super::data_.push(hboost::move(elem));
    super::notify_not_empty_if_needed(lk);
  ...
  template <class T, class Container,class Cmp>
  void sync_priority_queue<T,Container,Cmp>::push(HBOOST_THREAD_RV_REF(T) elem)
  {
    lock_guard<mutex> lk(super::mtx_);
    push(lk, hboost::move(elem));
  }
  ...
  //////////////////////
  template <class T, class Container,class Cmp>
  queue_op_status sync_priority_queue<T,Container,Cmp>::try_push(HBOOST_THREAD_RV_REF(T) elem)
  {
    lock_guard<mutex> lk(super::mtx_);
    if (super::closed(lk)) return queue_op_status::closed;
    push(lk, hboost::move(elem));
