...
#ifndef HBOOST_THREAD_EXECUTORS_BASIC_THREAD_POOL_HPP
#define HBOOST_THREAD_EXECUTORS_BASIC_THREAD_POOL_HPP
...
#include <hboost/thread/concurrent_queues/sync_queue.hpp>
...
namespace executors
{
  ...
  private:
    ...
    template <class AtThreadEntry>
    void worker_thread3(HBOOST_THREAD_FWD_REF(AtThreadEntry) at_thread_entry)
    {
      at_thread_entry(*this);
      worker_thread();
    }
  ...
  public:
    ...
    HBOOST_THREAD_NO_COPYABLE(basic_thread_pool)
    ...
    basic_thread_pool(unsigned const thread_count = thread::hardware_concurrency()+1)
    {
      try
      {
        ...
        for (unsigned i = 0; i < thread_count; ++i)
        {
          ...
          threads.push_back(thread_t(hboost::move(th)));
#else
    ...
    template <class AtThreadEntry>
    basic_thread_pool( unsigned const thread_count, HBOOST_THREAD_FWD_REF(AtThreadEntry) at_thread_entry)
    {
      try
      {
        threads.reserve(thread_count);
        for (unsigned i = 0; i < thread_count; ++i)
        {
          thread th (&basic_thread_pool::worker_thread3<AtThreadEntry>, this, hboost::forward<AtThreadEntry>(at_thread_entry));
          threads.push_back(thread_t(hboost::move(th)));
          //threads.push_back(thread_t(&basic_thread_pool::worker_thread, this)); // do not compile
        }
      }
     ...
     */
    void submit(HBOOST_THREAD_RV_REF(work) closure)  {
      work_queue.push(hboost::move(closure));
    }
    ...
    template <typename Closure>
    void submit(HBOOST_THREAD_FWD_REF(Closure) closure)
    {
      //submit(work(hboost::forward<Closure>(closure)));
      work w((hboost::forward<Closure>(closure)));
      submit(hboost::move(w));
    }
