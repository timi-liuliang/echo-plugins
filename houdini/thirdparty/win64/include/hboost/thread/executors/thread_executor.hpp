...
#ifndef HBOOST_THREAD_THREAD_EXECUTOR_HPP
#define HBOOST_THREAD_THREAD_EXECUTOR_HPP
...
namespace executors
{
  ...
  public:
    ...
    HBOOST_THREAD_NO_COPYABLE(thread_executor)
    ...
    void submit(Closure & closure)
    {
      ...
      thread th(closure);
      threads_.push_back(thread_t(hboost::move(th)));
    }
#endif
    ...
    void submit(HBOOST_THREAD_FWD_REF(Closure) closure)
    {
      ...
      if (closed(lk))  HBOOST_THROW_EXCEPTION( sync_queue_is_closed() );
      threads_.reserve(threads_.size() + 1);
      thread th(hboost::forward<Closure>(closure));
      threads_.push_back(thread_t(hboost::move(th)));
    }
