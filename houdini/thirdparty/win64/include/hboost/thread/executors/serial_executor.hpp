...
#ifndef HBOOST_THREAD_SERIAL_EXECUTOR_HPP
#define HBOOST_THREAD_SERIAL_EXECUTOR_HPP
...
#include <hboost/thread/scoped_thread.hpp>
...
namespace executors
{
  ...
  private:
    ...
    struct try_executing_one_task {
      work& task;
      hboost::promise<void> &p;
      try_executing_one_task(work& task, hboost::promise<void> &p)
      : task(task), p(p) {}
      void operator()() {
        try {
          task();
  ...
  public:
     ...
     */
    generic_executor_ref& underlying_executor() HBOOST_NOEXCEPT { return ex; }
  ...
  public:
    ...
    HBOOST_THREAD_NO_COPYABLE(serial_executor)
