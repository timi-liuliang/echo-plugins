...
#ifndef HBOOST_THREAD_EXECUTORS_EXECUTOR_ADAPTOR_HPP
#define HBOOST_THREAD_EXECUTORS_EXECUTOR_ADAPTOR_HPP

#include <hboost/thread/detail/config.hpp>

#include <hboost/thread/executors/executor.hpp>
...
namespace executors
{
  ...
  public:
    ...
    HBOOST_THREAD_NO_COPYABLE(executor_adaptor)
    ...
    executor_adaptor(HBOOST_THREAD_RV_REF(Args) ... args) : ex(hboost::forward<Args>(args)...) {}
#else
    /**
     * executor_adaptor constructor
     */
    ...
    executor_adaptor(
        HBOOST_THREAD_FWD_REF(A1) a1,
        HBOOST_THREAD_FWD_REF(A2) a2,
        HBOOST_THREAD_FWD_REF(A3) a3
        ) :
      ex(
          hboost::forward<A1>(a1),
          hboost::forward<A2>(a2),
          hboost::forward<A3>(a3)
          ) {}
#endif
    ...
    void submit(HBOOST_THREAD_RV_REF(work) closure)  {
      return ex.submit(hboost::move(closure));
    }
