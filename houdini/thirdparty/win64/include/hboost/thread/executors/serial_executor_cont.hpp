...
#ifndef HBOOST_THREAD_SERIAL_EXECUTOR_CONT_HPP
#define HBOOST_THREAD_SERIAL_EXECUTOR_CONT_HPP
...
namespace executors
{
  ...
  private:
    ...
    struct continuation {
      ...
      continuation(HBOOST_THREAD_RV_REF(work) tsk)
      : task(hboost::move(tsk)) {}
  ...
  public:
     ...
     */
    generic_executor_ref& underlying_executor() HBOOST_NOEXCEPT { return ex_; }

    /// serial_executor_cont is not copyable.
    HBOOST_THREAD_NO_COPYABLE(serial_executor_cont)
    ...
    {
      lock_guard<mutex> lk(mtx_);
      if (closed(lk))       HBOOST_THROW_EXCEPTION( sync_queue_is_closed() );
    ...
    void submit(HBOOST_THREAD_FWD_REF(Closure) closure)
    {
      lock_guard<mutex> lk(mtx_);
      if (closed(lk))       HBOOST_THROW_EXCEPTION( sync_queue_is_closed() );
      fut_ = fut_.then(ex_, continuation(work(hboost::forward<Closure>(closure))));
    }
