...
#ifndef HBOOST_THREAD_EXECUTORS_SCHEDULER_HPP
#define HBOOST_THREAD_EXECUTORS_SCHEDULER_HPP

#include <hboost/thread/detail/config.hpp>
#include <hboost/thread/executors/detail/scheduled_executor_base.hpp>
...
namespace hboost
{
  namespace executors
  {
    ...
    resubmit(Executor& ex, HBOOST_THREAD_FWD_REF(Function) funct) {
      return resubmitter<Executor, typename decay<Function>::type >(ex, hboost::move(funct));
    }
    ...
    public:
      ...
      template <class Work>
      void submit(HBOOST_THREAD_FWD_REF(Work) w)
      {
        if (closed())
        {
          HBOOST_THROW_EXCEPTION( sync_queue_is_closed() );
        }
        sch.submit_at(resubmit(ex,hboost::forward<Work>(w)), tp);
      }
    ...
    public:
      ...
      void submit(HBOOST_THREAD_FWD_REF(Work) w)
      {
        ...
        sch.submit_at(hboost::forward<Work>(w), tp);
      }
