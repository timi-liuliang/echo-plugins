...
#ifndef HBOOST_THREAD_EXECUTORS_DETAIL_SCHEDULED_EXECUTOR_BASE_HPP
#define HBOOST_THREAD_EXECUTORS_DETAIL_SCHEDULED_EXECUTOR_BASE_HPP

#include <hboost/thread/concurrent_queues/sync_timed_queue.hpp>
#include <hboost/thread/executors/detail/priority_executor_base.hpp>
...
namespace detail
{
  ...
  public:
    ...
    {
      this->_workq.push(hboost::move(w), tp);
    }

    void submit_after(work w, const duration& dura)
    {
      this->_workq.push(hboost::move(w), dura+clock::now());
    }
