...
#ifndef HBOOST_THREAD_STRICT_LOCK_HPP
#define HBOOST_THREAD_STRICT_LOCK_HPP
...
namespace hboost
{
  ...
  class strict_lock
  {
    ...
    HBOOST_CONCEPT_ASSERT(( BasicLockable<Lockable> ));
  public:
    typedef Lockable mutex_type;

    // construct/copy/destroy:

    HBOOST_THREAD_NO_COPYABLE( strict_lock)
     ...
     */
    bool owns_lock(const mutex_type* l) const HBOOST_NOEXCEPT
    {
      return l == mutex();
    } /*< strict locks specific function >*/

    //HBOOST_ADRESS_OF_DELETE(strict_lock) /*< disable aliasing >*/
    //HBOOST_HEAP_ALLOCATION_DELETE(strict_lock) /*< disable heap allocation >*/
  ...
  {
    HBOOST_CONCEPT_ASSERT(( BasicLock<Lock> )); /*< The Lock must be a movable lock >*/
  public:
    typedef typename Lock::mutex_type mutex_type; /*< Name the lockable type locked by Lock >*/

    HBOOST_THREAD_NO_COPYABLE( nested_strict_lock)
     ...
     * __Throws:
     *
     * - lock_error when HBOOST_THREAD_THROW_IF_PRECONDITION_NOT_SATISFIED is defined and lk.mutex() == null_ptr
     *
    ...
    {
      /*< Define HBOOST_THREAD_DONT_CHECK_PRECONDITIONS if you don't want to check lk ownership >*/
      HBOOST_THREAD_ASSERT_PRECONDITION(  lk.mutex() != 0,
          lock_error()
      );
    ...
    {
      /*< Define HBOOST_THREAD_DONT_CHECK_PRECONDITIONS if you don't want to check lk ownership >*/
      HBOOST_THREAD_ASSERT_PRECONDITION(  lk_.mutex() != 0,
          lock_error()
      );
     ...
     */
    ~nested_strict_lock()HBOOST_NOEXCEPT
    {
     ...
     */
    bool owns_lock(mutex_type const* l) const HBOOST_NOEXCEPT
    {
      return l == mutex();
    }

    //HBOOST_ADRESS_OF_DELETE(nested_strict_lock)
    //HBOOST_HEAP_ALLOCATEION_DELETE(nested_strict_lock)
...
#if ! defined HBOOST_THREAD_NO_MAKE_STRICT_LOCK
  template <typename Lockable>
