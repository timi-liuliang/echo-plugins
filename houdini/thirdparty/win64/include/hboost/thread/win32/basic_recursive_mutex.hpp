#ifndef HBOOST_BASIC_RECURSIVE_MUTEX_WIN32_HPP
#define HBOOST_BASIC_RECURSIVE_MUTEX_WIN32_HPP
    ...
    namespace detail
    {
        ...
        struct basic_recursive_mutex_impl
        {
            ...
            void lock()
            {
                ...
                {
                    mutex.lock();
                    HBOOST_INTERLOCKED_EXCHANGE(&locking_thread_id,current_thread_id);
                    recursion_count=1;
                }
            }
#if defined HBOOST_THREAD_USES_DATETIME
            bool timed_lock(::hboost::system_time const& target)
            {
                long const current_thread_id=win32::GetCurrentThreadId();
        ...
        bool try_lock_until(const chrono::time_point<Clock, Duration>& t)
        {
            ...
            void unlock()
            {
                if(!--recursion_count)
                {
                    HBOOST_INTERLOCKED_EXCHANGE(&locking_thread_id,0);
                    mutex.unlock();
                }
            }

        private:
            bool try_recursive_lock(long current_thread_id) HBOOST_NOEXCEPT
            {
                if(::hboost::detail::interlocked_read_acquire(&locking_thread_id)==current_thread_id)
                {
                    ++recursion_count;
                    return true;
                }
            ...
            bool try_basic_lock(long current_thread_id) HBOOST_NOEXCEPT
            {
                if(mutex.try_lock())
                {
                    HBOOST_INTERLOCKED_EXCHANGE(&locking_thread_id,current_thread_id);
...
#if defined HBOOST_THREAD_USES_DATETIME
            bool try_timed_lock(long current_thread_id,::hboost::system_time const& target)
            {
...
#define HBOOST_BASIC_RECURSIVE_MUTEX_INITIALIZER {0}
