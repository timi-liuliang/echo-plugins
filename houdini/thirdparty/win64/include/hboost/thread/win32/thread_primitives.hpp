#ifndef HBOOST_WIN32_THREAD_PRIMITIVES_HPP
#define HBOOST_WIN32_THREAD_PRIMITIVES_HPP
...
//#include <hboost/detail/winapi/synchronization.hpp>
#include <algorithm>
...
# if HBOOST_USE_WINAPI_VERSION < HBOOST_WINAPI_VERSION_VISTA
            using ::CreateMutexW;
            using ::CreateEventW;
...
# ifdef HBOOST_NO_ANSI_APIS
# if defined(HBOOST_USE_WINAPI_VERSION) && ( HBOOST_USE_WINAPI_VERSION < HBOOST_WINAPI_VERSION_VISTA )
...
#define _InterlockedCompareExchange InterlockedCompareExchange
...
#elif defined(HBOOST_USE_WINAPI_VERSION) && ( HBOOST_USE_WINAPI_VERSION < HBOOST_WINAPI_VERSION_VISTA )
            ...
            {
                HBOOST_VERIFY(ReleaseSemaphore(semaphore,count,0)!=0);
            }
...
#else
                    ::hboost::detail::win32::Sleep(0);
#endif
                }
                else
                {
#if HBOOST_PLAT_WINDOWS_RUNTIME
                    ::hboost::detail::win32::WaitForSingleObjectEx(::hboost::detail::win32::GetCurrentThread(), milliseconds, 0);
#else
                    ::hboost::detail::win32::Sleep(milliseconds);
#endif
                }
            }

#if HBOOST_PLAT_WINDOWS_RUNTIME
            class HBOOST_THREAD_DECL scoped_winrt_thread
            {
            public:
                ...
                ~scoped_winrt_thread()
                {
                    if (m_completionHandle != ::hboost::detail::win32::invalid_handle_value)
                    {
                ...
                {
                    HBOOST_ASSERT(m_completionHandle != ::hboost::detail::win32::invalid_handle_value);
...
#endif
            class HBOOST_THREAD_DECL handle_manager
            {
            private:
                ...
                void cleanup()
                {
                    ...
                    {
                        HBOOST_VERIFY(CloseHandle(handle_to_manage));
                    }
                }
...
#if defined(HBOOST_MSVC) && (_MSC_VER>=1400)  && !defined(UNDER_CE)
...
#define HBOOST_THREAD_BTS_DEFINED
#elif (defined(HBOOST_MSVC) || defined(HBOOST_INTEL_WIN)) && defined(_M_IX86)
namespace hboost
{
    namespace detail
    {
...
#ifndef HBOOST_THREAD_BTS_DEFINED
    ...
    namespace detail
    {
        namespace win32
        {
            inline bool interlocked_bit_test_and_set(long* x,long bit)
            {
                ...
                do
                {
                    long const current=HBOOST_INTERLOCKED_COMPARE_EXCHANGE(x,old|value,old);
                    if(current==old)
                    {
                        break;
                    }
