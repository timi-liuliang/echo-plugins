...
#ifndef HBOOST_THREAD_USER_SCHEDULER_HPP
#define HBOOST_THREAD_USER_SCHEDULER_HPP
...
namespace hboost
{
  ...
  public:
    ...
    HBOOST_THREAD_NO_COPYABLE(user_scheduler)
    ...
    template <typename Closure>
    void submit(HBOOST_THREAD_RV_REF(Closure) closure)
    {
      work w =hboost::move(closure);
      work_queue.push(hboost::move(w));
      //work_queue.push(work(hboost::move(closure))); // todo check why this doesn't work
    }
