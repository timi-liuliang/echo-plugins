...
#ifndef HBOOST_THREAD_LOCK_CONCEPTS_HPP
#define HBOOST_THREAD_LOCK_CONCEPTS_HPP
...
namespace hboost
{
  ...
  struct BasicLock
  {
    ...
    HBOOST_CONCEPT_ASSERT(( BasicLockable<mutex_type> ));

    HBOOST_CONCEPT_USAGE(BasicLock)
    {
      const Lk l1(mtx);
      Lk l2(mtx, defer_lock);
      Lk l3(mtx, adopt_lock);
      Lk l4(( Lk()));
      Lk l5(( hboost::move(l2)));
  ...
  struct Lock
  {
    HBOOST_CONCEPT_ASSERT(( BasicLock<Lk> ));
    typedef typename Lk::mutex_type mutex_type;
    HBOOST_CONCEPT_ASSERT(( Lockable<mutex_type> ));

    HBOOST_CONCEPT_USAGE(Lock)
    {
  ...
  struct TimedLock
  {
    HBOOST_CONCEPT_ASSERT(( Lock<Lk> ));
    typedef typename Lk::mutex_type mutex_type;
    HBOOST_CONCEPT_ASSERT(( TimedLockable<mutex_type> ));

    HBOOST_CONCEPT_USAGE(TimedLock)
    {
      const Lk l1(mtx, t);
      Lk l2(mtx, d);
  ...
  private:
    ...
    mutex_type& mtx;
    hboost::chrono::system_clock::time_point t;
    hboost::chrono::system_clock::duration d;
  };

  template <typename Lk>
  struct UniqueLock
  {
    HBOOST_CONCEPT_ASSERT(( TimedLock<Lk> ));
    typedef typename Lk::mutex_type mutex_type;

    HBOOST_CONCEPT_USAGE(UniqueLock)
    {
  ...
  struct SharedLock
  {
    ...
    HBOOST_CONCEPT_USAGE(SharedLock)
    {
    }
    SharedLock() :
      mtx(*static_cast<mutex_type*>(0))
  ...
  struct UpgradeLock
  {
    HBOOST_CONCEPT_ASSERT(( SharedLock<Lk> ));
    typedef typename Lk::mutex_type mutex_type;

    HBOOST_CONCEPT_USAGE(UpgradeLock)
    {
    }
    UpgradeLock() :
      mtx(*static_cast<mutex_type*>(0))
  ...
  struct StrictLock
  {
    typedef typename Lk::mutex_type mutex_type;
    HBOOST_CONCEPT_ASSERT(( BasicLockable<mutex_type> ));
    HBOOST_STATIC_ASSERT(( is_strict_lock<Lk>::value ));

    HBOOST_CONCEPT_USAGE( StrictLock)
    {
