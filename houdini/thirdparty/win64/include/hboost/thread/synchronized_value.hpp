...
#ifndef HBOOST_THREAD_SYNCHRONIZED_VALUE_HPP
#define HBOOST_THREAD_SYNCHRONIZED_VALUE_HPP
...
#include <hboost/thread/lock_factories.hpp>
...
//#include <hboost/thread/detail/is_nothrow_default_constructible.hpp>
//#if ! defined HBOOST_NO_CXX11_HDR_TYPE_TRAITS
//#include <type_traits>
//#endif

#if ! defined(HBOOST_THREAD_NO_SYNCHRONIZE)
#include <tuple> // todo change to <hboost/tuple.hpp> once Boost.Tuple or Boost.Fusion provides Move semantics on C++98 compilers.
#include <functional>
#endif
...
namespace hboost
{
  ...
  protected:
    ...
    hboost::unique_lock<mutex_type> lk_;
    T const& value_;

  public:
    HBOOST_THREAD_MOVABLE_ONLY( const_strict_lock_ptr )
    ...
    const_strict_lock_ptr(T const& val, Lockable & mtx, adopt_lock_t tag) HBOOST_NOEXCEPT :
     ...
     */
    const_strict_lock_ptr(HBOOST_THREAD_RV_REF(const_strict_lock_ptr) other) HBOOST_NOEXCEPT
    : lk_(hboost::move(HBOOST_THREAD_RV(other).lk_)),value_(HBOOST_THREAD_RV(other).value_)
    {
    }
  ...
  public:
    HBOOST_THREAD_MOVABLE_ONLY( strict_lock_ptr )
     ...
     * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
     */
    strict_lock_ptr(HBOOST_THREAD_RV_REF(strict_lock_ptr) other)
    : base_type(hboost::move(static_cast<base_type&>(other)))
    {
    }
  ...
  public:
    HBOOST_THREAD_MOVABLE_ONLY(const_unique_lock_ptr)
     ...
     */
    const_unique_lock_ptr(T const& val, Lockable & mtx, adopt_lock_t) HBOOST_NOEXCEPT
     ...
     */
    const_unique_lock_ptr(T const& val, Lockable & mtx, defer_lock_t) HBOOST_NOEXCEPT
     ...
     */
    const_unique_lock_ptr(T const& val, Lockable & mtx, try_to_lock_t) HBOOST_NOEXCEPT
     ...
     * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
     */
    const_unique_lock_ptr(HBOOST_THREAD_RV_REF(const_unique_lock_ptr) other) HBOOST_NOEXCEPT
    : base_type(hboost::move(static_cast<base_type&>(other))), value_(HBOOST_THREAD_RV(other).value_)
    {
    }
  ...
  public:
    ...
    HBOOST_THREAD_MOVABLE_ONLY(unique_lock_ptr)
     ...
     * @effects stores a reference to it and to the value type @c value taking ownership.
     */
    unique_lock_ptr(T & value, Lockable & mtx, adopt_lock_t) HBOOST_NOEXCEPT
     ...
     * @effects stores a reference to it and to the value type @c value c.
     */
    unique_lock_ptr(T & value, Lockable & mtx, defer_lock_t) HBOOST_NOEXCEPT
     ...
     * @effects try to lock the mutex @c mtx, stores a reference to it and to the value type @c value.
     */
    unique_lock_ptr(T & value, Lockable & mtx, try_to_lock_t) HBOOST_NOEXCEPT
     ...
     * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
     */
    unique_lock_ptr(HBOOST_THREAD_RV_REF(unique_lock_ptr) other) HBOOST_NOEXCEPT
    : base_type(hboost::move(static_cast<base_type&>(other)))
    {
    }
  ...
  public:
     ...
     */
    synchronized_value()
    //HBOOST_NOEXCEPT_IF(is_nothrow_default_constructible<T>::value)
    : value_()
    {
    }
    ...
    //HBOOST_NOEXCEPT_IF(is_nothrow_copy_constructible<T>::value)
    : value_(other)
    {
    }
     ...
     */
    synchronized_value(HBOOST_THREAD_RV_REF(T) other)
    //HBOOST_NOEXCEPT_IF(is_nothrow_move_constructible<T>::value)
    : value_(hboost::move(other))
    {
    }
     ...
     *
     */
    synchronized_value(HBOOST_THREAD_RV_REF(synchronized_value) other)
    {
      strict_lock<mutex_type> lk(HBOOST_THREAD_RV(other).mtx_);
      value_= hboost::move(HBOOST_THREAD_RV(other).value_);
    }
    ...
    {
      return HBOOST_THREAD_MAKE_RV_REF((strict_lock_ptr<T,Lockable>(value_, mtx_)));
    }
    /**
    ...
    {
      return HBOOST_THREAD_MAKE_RV_REF((const_strict_lock_ptr<T,Lockable>(value_, mtx_)));
    }
    ...
    template <typename F>
    inline
    typename hboost::result_of<F(value_type&)>::type
    operator()(HBOOST_THREAD_RV_REF(F) fct)
    {
      strict_lock<mutex_type> lk(mtx_);
      return fct(value_);
    }
    template <typename F>
    inline
    typename hboost::result_of<F(value_type const&)>::type
    operator()(HBOOST_THREAD_RV_REF(F) fct) const
    {
      strict_lock<mutex_type> lk(mtx_);
      return fct(value_);
    }
    ...
    {
      return HBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_)));
    }
    const_unique_lock_ptr<T,Lockable> unique_synchronize() const
    {
      return HBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_)));
    }
    unique_lock_ptr<T,Lockable> unique_synchronize(defer_lock_t tag)
    {
      return HBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_, tag)));
    }
    const_unique_lock_ptr<T,Lockable> unique_synchronize(defer_lock_t tag) const
    {
      return HBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_, tag)));
    }
    unique_lock_ptr<T,Lockable> defer_synchronize() HBOOST_NOEXCEPT
    {
      return HBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_, defer_lock)));
    }
    const_unique_lock_ptr<T,Lockable> defer_synchronize() const HBOOST_NOEXCEPT
    {
      return HBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_, defer_lock)));
    }
    unique_lock_ptr<T,Lockable> try_to_synchronize() HBOOST_NOEXCEPT
    {
      return HBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_, try_to_lock)));
    }
    const_unique_lock_ptr<T,Lockable> try_to_synchronize() const HBOOST_NOEXCEPT
    {
      return HBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_, try_to_lock)));
    }
    unique_lock_ptr<T,Lockable> adopt_synchronize() HBOOST_NOEXCEPT
    {
      return HBOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T,Lockable>(value_, mtx_, adopt_lock)));
    }
    const_unique_lock_ptr<T,Lockable> adopt_synchronize() const HBOOST_NOEXCEPT
    {
      return HBOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T,Lockable>(value_, mtx_, adopt_lock)));
    }
    ...
    public:
      HBOOST_THREAD_MOVABLE_ONLY(deref_value)

      deref_value(HBOOST_THREAD_RV_REF(deref_value) other):
      lk_(hboost::move(HBOOST_THREAD_RV(other).lk_)),value_(HBOOST_THREAD_RV(other).value_)
      {}
      operator T&()
      {
    ...
    public:
      HBOOST_THREAD_MOVABLE_ONLY(const_deref_value)

      const_deref_value(HBOOST_THREAD_RV_REF(const_deref_value) other):
      lk_(hboost::move(HBOOST_THREAD_RV(other).lk_)), value_(HBOOST_THREAD_RV(other).value_)
      {}
  ...
  public:
    ...
    {
      return HBOOST_THREAD_MAKE_RV_REF(deref_value(*this));
    }

    const_deref_value operator*() const
    {
      return HBOOST_THREAD_MAKE_RV_REF(const_deref_value(*this));
    }
