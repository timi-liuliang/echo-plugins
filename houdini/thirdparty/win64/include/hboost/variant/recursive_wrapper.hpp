...
#ifndef HBOOST_VARIANT_RECURSIVE_WRAPPER_HPP
#define HBOOST_VARIANT_RECURSIVE_WRAPPER_HPP
...
public: // modifiers
    ...
    void swap(recursive_wrapper& operand) HBOOST_NOEXCEPT
    {
        T* temp = operand.p_;
        operand.p_ = p_;
        p_ = temp;
...
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES 
    recursive_wrapper& operator=(recursive_wrapper&& rhs) HBOOST_NOEXCEPT
    {
        swap(rhs);
        return *this;
    }
...
//
template <typename T>
inline void swap(recursive_wrapper<T>& lhs, recursive_wrapper<T>& rhs) HBOOST_NOEXCEPT
{
    lhs.swap(rhs);
}
...
#endif // HBOOST_VARIANT_RECURSIVE_WRAPPER_HPP
