//-----------------------------------------------------------------------------
// boost variant/recursive_variant.hpp header file
// See http://www.boost.org for updates, documentation, and revision history.
//-----------------------------------------------------------------------------
//
// Copyright (c) 2003 Eric Friedman
// Copyright (c) 2013 Antony Polukhin
//
// Distributed under the Boost Software License, Version 1.0. (See
// accompanying file LICENSE_1_0.txt or copy at
// http://www.boost.org/LICENSE_1_0.txt)

#ifndef HBOOST_VARIANT_RECURSIVE_VARIANT_HPP
#define HBOOST_VARIANT_RECURSIVE_VARIANT_HPP

#include "hboost/variant/variant_fwd.hpp"
#include "hboost/variant/detail/enable_recursive.hpp"
#include "hboost/variant/detail/substitute_fwd.hpp"
#include "hboost/variant/detail/make_variant_list.hpp"
#include "hboost/variant/detail/over_sequence.hpp"

#include "hboost/mpl/aux_/lambda_arity_param.hpp"

#include "hboost/mpl/equal.hpp"
#include "hboost/mpl/eval_if.hpp"
#include "hboost/mpl/identity.hpp"
#include "hboost/mpl/if.hpp"
#include "hboost/mpl/protect.hpp"
#include "hboost/mpl/transform.hpp"
#include "hboost/type_traits/is_same.hpp"
#include "hboost/preprocessor/cat.hpp"
#include "hboost/preprocessor/repeat.hpp"

#include "hboost/mpl/bool.hpp"
#include "hboost/mpl/is_sequence.hpp"
#include "hboost/variant/variant.hpp"

namespace hboost {

namespace detail { namespace variant {

///////////////////////////////////////////////////////////////////////////////
// (detail) metafunction specialization substitute
//
// Handles embedded variant types when substituting for recursive_variant_.
//

#if !defined(HBOOST_VARIANT_DETAIL_NO_SUBSTITUTE)

template <
      HBOOST_VARIANT_ENUM_PARAMS(typename T)
    , typename RecursiveVariant
      HBOOST_MPL_AUX_LAMBDA_ARITY_PARAM(typename Arity)
    >
struct substitute<
      ::hboost::variant<
          recursive_flag< T0 >
        , HBOOST_VARIANT_ENUM_SHIFTED_PARAMS(T)
        >
    , RecursiveVariant
    , ::hboost::recursive_variant_
      HBOOST_MPL_AUX_LAMBDA_ARITY_PARAM(Arity)
    >
{
    typedef ::hboost::variant<
          recursive_flag< T0 >
        , HBOOST_VARIANT_ENUM_SHIFTED_PARAMS(T)
        > type;
};

template <
      HBOOST_VARIANT_ENUM_PARAMS(typename T)
    , typename RecursiveVariant
      HBOOST_MPL_AUX_LAMBDA_ARITY_PARAM(typename Arity)
    >
struct substitute<
      ::hboost::variant<
          ::hboost::detail::variant::over_sequence< T0 >
        , HBOOST_VARIANT_ENUM_SHIFTED_PARAMS(T)
        >
    , RecursiveVariant
    , ::hboost::recursive_variant_
      HBOOST_MPL_AUX_LAMBDA_ARITY_PARAM(Arity)
    >
{
private:

    typedef T0 initial_types;

    typedef typename mpl::transform<
          initial_types
        , mpl::protect< quoted_enable_recursive<RecursiveVariant,mpl::true_> >
        >::type types;

public:

    typedef typename mpl::if_<
          mpl::equal<initial_types, types, ::hboost::is_same<mpl::_1, mpl::_2> >
        , ::hboost::variant<
              ::hboost::detail::variant::over_sequence< T0 >
            , HBOOST_VARIANT_ENUM_SHIFTED_PARAMS(T)
            >
        , ::hboost::variant< over_sequence<types> >
        >::type type;
};

template <
      HBOOST_VARIANT_ENUM_PARAMS(typename T)
    , typename RecursiveVariant
      HBOOST_MPL_AUX_LAMBDA_ARITY_PARAM(typename Arity)
    >
struct substitute<
      ::hboost::variant< HBOOST_VARIANT_ENUM_PARAMS(T) >
    , RecursiveVariant
    , ::hboost::recursive_variant_
      HBOOST_MPL_AUX_LAMBDA_ARITY_PARAM(Arity)
    >
{
#if !defined(HBOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES)
    
    typedef ::hboost::variant<
        typename enable_recursive<   
              T0              
            , RecursiveVariant               
            , mpl::true_                     
        >::type,
        typename enable_recursive<   
              TN              
            , RecursiveVariant               
            , mpl::true_                     
        >::type...  
    > type;

#else // defined(HBOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES)

private: // helpers, for metafunction result (below)

    #define HBOOST_VARIANT_AUX_ENABLE_RECURSIVE_TYPEDEFS(z,N,_)  \
        typedef typename enable_recursive<   \
              HBOOST_PP_CAT(T,N)              \
            , RecursiveVariant               \
            , mpl::true_                     \
            >::type HBOOST_PP_CAT(wknd_T,N);  \
        /**/

    HBOOST_PP_REPEAT(
          HBOOST_VARIANT_LIMIT_TYPES
        , HBOOST_VARIANT_AUX_ENABLE_RECURSIVE_TYPEDEFS
        , _
        )

    #undef HBOOST_VARIANT_AUX_ENABLE_RECURSIVE_TYPEDEFS

public: // metafunction result

    typedef ::hboost::variant< HBOOST_VARIANT_ENUM_PARAMS(wknd_T) > type;
#endif // HBOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES workaround
};

#else // defined(HBOOST_VARIANT_DETAIL_NO_SUBSTITUTE)

//
// no specializations: embedded variants unsupported on these compilers!
//

#endif // !defined(HBOOST_VARIANT_DETAIL_NO_SUBSTITUTE)

}} // namespace detail::variant

///////////////////////////////////////////////////////////////////////////////
// metafunction make_recursive_variant
//
// See docs and hboost/variant/variant_fwd.hpp for more information.
//
template < HBOOST_VARIANT_ENUM_PARAMS(typename T) >
struct make_recursive_variant
{
public: // metafunction result

    typedef hboost::variant<
          detail::variant::recursive_flag< T0 >
        , HBOOST_VARIANT_ENUM_SHIFTED_PARAMS(T)
        > type;

};

///////////////////////////////////////////////////////////////////////////////
// metafunction make_recursive_variant_over
//
// See docs and hboost/variant/variant_fwd.hpp for more information.
//
template <typename Types>
struct make_recursive_variant_over
{
private: // precondition assertions

    HBOOST_STATIC_ASSERT(( ::hboost::mpl::is_sequence<Types>::value ));

public: // metafunction result

    typedef typename make_recursive_variant<
          detail::variant::over_sequence< Types >
        >::type type;

};

} // namespace hboost

#endif // HBOOST_VARIANT_RECURSIVE_VARIANT_HPP
