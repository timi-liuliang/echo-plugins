...
#ifndef HBOOST_VARIANT_VARIANT_HPP
#define HBOOST_VARIANT_VARIANT_HPP
...
#include "hboost/variant/detail/config.hpp"
...
#include "hboost/variant/detail/initializer.hpp"
#include "hboost/variant/detail/make_variant_list.hpp"
#include "hboost/variant/detail/over_sequence.hpp"
#include "hboost/variant/detail/visitation_impl.hpp"
#include "hboost/variant/detail/hash_variant.hpp"

#include "hboost/variant/detail/generic_result_type.hpp"
#include "hboost/variant/detail/move.hpp"

#include "hboost/detail/no_exceptions_support.hpp"
#include "hboost/detail/reference_content.hpp"
#include "hboost/aligned_storage.hpp"
#include "hboost/blank.hpp"
#include "hboost/math/common_factor_ct.hpp"
...
#include "hboost/mpl/max_element.hpp"
...
//   Defined in hboost/variant/detail/visitation_impl.hpp.
//
// HBOOST_VARIANT_MINIMIZE_SIZE
...
#if defined(HBOOST_VARIANT_MINIMIZE_SIZE)
...
struct add_alignment
{
    ...
    struct apply
        : mpl::size_t<
              ::hboost::math::static_lcm<
                  HBOOST_MPL_AUX_VALUE_WKND(State)::value
                , ::hboost::alignment_of<Item>::value
                >::value
            >
    {};
};
...
//
// This implementation is designed to prefer hboost::blank over other potential
...
private: // helpers, for metafunction result (below)
    ...
    // [...now search the rest of the sequence for hboost::blank...]

    typedef typename mpl::iter_fold_if<
          mpl::iterator_range< first_result_it,end_it >
        , first_result_index, mpl::protect< mpl::next<> >
        , mpl::protect< mpl::not_same_as<hboost::blank> >
        >::type second_result_;
...
struct is_variant_move_noexcept_constructible {
    typedef typename hboost::mpl::find_if<
        Types, mpl::not_<hboost::is_nothrow_move_constructible<hboost::mpl::_1> >
    >::type iterator_t;

    typedef typename hboost::mpl::end<Types>::type end_t;
    typedef typename hboost::is_same<
        iterator_t, end_t
    >::type type;
};
...
struct is_variant_move_noexcept_assignable {
    typedef typename hboost::mpl::find_if<
        Types, mpl::not_<hboost::is_nothrow_move_assignable<hboost::mpl::_1> >
    >::type iterator_t;
...
#endif // HBOOST_NO_CXX11_NOEXCEPT
...
public: // metafunction result
    ...
    typedef ::hboost::aligned_storage<
          HBOOST_MPL_AUX_VALUE_WKND(max_size)::value
        , HBOOST_MPL_AUX_VALUE_WKND(max_alignment)::value
        > type;
};
...
public: // visitor interfaces
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(T& operand, int) const HBOOST_NOEXCEPT
    {
        operand.~T(); // must be noexcept

#if HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x0551)) || \
    HBOOST_WORKAROUND(HBOOST_MSVC, HBOOST_TESTED_AT(1600))
        operand; // suppresses warnings
#endif

        HBOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // visitor interface
    ...
    T& operator()(T& operand) const HBOOST_NOEXCEPT
    {
        return operand;
    }
    ...
    T& operator()(U&) const
    {
        ...
        return ::hboost::detail::variant::forced_return< T& >();
    }
};
...
public: // structors
    ...
    explicit copy_into(void* storage) HBOOST_NOEXCEPT
        : storage_(storage)
    {
    }
...
public: // internal visitor interface
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(hboost::detail::variant::backup_holder<T>& operand, long) const
    {
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(const hboost::detail::variant::backup_holder<T>& operand, long) const
    {
        new(storage_) T( operand.get() );
        HBOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // structors
    ...
    explicit move_into(void* storage) HBOOST_NOEXCEPT
        : storage_(storage)
    {
    }
...
public: // internal visitor interface
    ...
    internal_visit(hboost::detail::variant::backup_holder<T>& operand, long) const
    {
        new(storage_) T( ::hboost::detail::variant::move(operand.get()) );
        HBOOST_VARIANT_AUX_RETURN_VOID;
    }

    template <typename T>
        HBOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(T& operand, int) const HBOOST_NOEXCEPT_IF(HBOOST_NOEXCEPT_EXPR(T(hboost::declval<T>())))
    {
        new(storage_) T(::hboost::detail::variant::move(operand));
        HBOOST_VARIANT_AUX_RETURN_VOID;
    }
};
#endif
...
public: // structors
    ...
    explicit assign_storage(const void* rhs_storage) HBOOST_NOEXCEPT
...
public: // structors
    ...
    explicit move_storage(void* rhs_storage) HBOOST_NOEXCEPT
        : rhs_storage_(rhs_storage)
    {
    }
...
public: // internal visitor interfaces
    ...
    {
        lhs_content.get()
            = ::hboost::detail::variant::move(static_cast<backup_holder<T>* >(rhs_storage_)->get());
        HBOOST_VARIANT_AUX_RETURN_VOID;
    }
    ...
    internal_visit(T& lhs_content, int) const
    {
        ...
        // Hint: Are any of the bounded types const-qualified or references?
        //
        lhs_content = ::hboost::detail::variant::move(*static_cast<T* >(rhs_storage_));
        HBOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // structors
    ...
    explicit direct_assigner(const T& rhs) HBOOST_NOEXCEPT
        : rhs_(rhs)
    {
    }
...
public: // visitor interface
    ...
    template <typename U>
    bool operator()(U&) HBOOST_NOEXCEPT
    {
        return false;
    }

#if HBOOST_WORKAROUND(HBOOST_MSVC, HBOOST_TESTED_AT(1600))
private:
...
public: // structors
    ...
    explicit direct_mover(T& rhs) HBOOST_NOEXCEPT
        : rhs_(rhs)
    {
    }
...
public: // visitor interface
    ...
    bool operator()(T& lhs)
    {
        lhs = ::hboost::detail::variant::move(rhs_);
        return true;
    }
...
private: // helpers, for visitor interface (below)
        ...
        )
    {
        ...
        LhsT backup_lhs_content(
              ::hboost::detail::variant::move(lhs_content)
            ); // nothrow
        ...
        HBOOST_CATCH (...)
        {
            ...
            new(lhs_.storage_.address())
                LhsT(
                      ::hboost::detail::variant::move(backup_lhs_content)
                    ); // nothrow
...
public: // structors
    ...
    explicit swap_with(Variant& toswap) HBOOST_NOEXCEPT
        : toswap_(toswap)
    {
    }
...
public: // internal visitor interfaces
    ...
    void operator()(T& operand) const
    {
        ...
        // ...and swap:
        ::hboost::detail::variant::move_swap( operand, other );
    }
...
class reflect
    : public static_visitor<const hboost::typeindex::type_info&>
{
public: // visitor interfaces

    template <typename T>
    const hboost::typeindex::type_info& operator()(const T&) const HBOOST_NOEXCEPT
    {
        return hboost::typeindex::type_id<T>().type_info();
    }
...
public: // structors
    ...
    explicit comparer(const Variant& lhs) HBOOST_NOEXCEPT
        : lhs_(lhs)
    {
    }
...
public: // structors
    ...
    explicit invoke_visitor(Visitor& visitor) HBOOST_NOEXCEPT
        : visitor_(visitor)
    {
    }
...
#   if HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x0564))
    template <typename T>
...
public: // internal visitor interfaces, cont.
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(hboost::recursive_wrapper<T>& operand, long)
    {
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const hboost::recursive_wrapper<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(hboost::detail::reference_content<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const hboost::detail::reference_content<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(hboost::detail::variant::backup_holder<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        HBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const hboost::detail::variant::backup_holder<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
...
//
// See docs and hboost/variant/variant_fwd.hpp for more information.
//
template <
      typename T0_
    , HBOOST_VARIANT_ENUM_SHIFTED_PARAMS(typename T)
    >
class variant
{
...
#if !defined(HBOOST_VARIANT_NO_TYPE_SEQUENCE_SUPPORT)
...
private: // helpers, for typedefs (below)
    ...
    typedef typename mpl::eval_if<
        ...
        , detail::variant::make_variant_list<
              unwrapped_T0_
            , HBOOST_VARIANT_ENUM_SHIFTED_PARAMS(T)
            >
        >::type specified_types;

    HBOOST_STATIC_ASSERT((
          ::hboost::mpl::not_< mpl::empty<specified_types> >::value
        ));
...
#else // defined(HBOOST_VARIANT_NO_TYPE_SEQUENCE_SUPPORT)

private: // helpers, for typedefs (below)

    typedef unwrapped_T0_ T0;

    #define HBOOST_VARIANT_AUX_ENABLE_RECURSIVE_TYPEDEFS(z,N,_) \
        typedef typename mpl::eval_if< \
              is_recursive_ \
            , detail::variant::enable_recursive< \
                ...
                , wknd_self_t \
                > \
            , mpl::identity< HBOOST_PP_CAT(T,N) > \
            >::type HBOOST_PP_CAT(recursive_enabled_T,N); \
        /**/

    HBOOST_PP_REPEAT(
          HBOOST_VARIANT_LIMIT_TYPES
        , HBOOST_VARIANT_AUX_ENABLE_RECURSIVE_TYPEDEFS
        , _
        )

    #undef HBOOST_VARIANT_AUX_ENABLE_RECURSIVE_TYPEDEFS

    #define HBOOST_VARIANT_AUX_UNWRAP_RECURSIVE_TYPEDEFS(z,N,_) \
        typedef typename unwrap_recursive< \
              HBOOST_PP_CAT(recursive_enabled_T,N) \
            >::type HBOOST_PP_CAT(public_T,N); \
        /**/

    HBOOST_PP_REPEAT(
          HBOOST_VARIANT_LIMIT_TYPES
        , HBOOST_VARIANT_AUX_UNWRAP_RECURSIVE_TYPEDEFS
        , _
        )

    #undef HBOOST_VARIANT_AUX_UNWRAP_RECURSIVE_TYPEDEFS

public: // public typedefs

    typedef typename detail::variant::make_variant_list<
          HBOOST_VARIANT_ENUM_PARAMS(public_T)
        >::type types;

private: // helpers, for internal typedefs (below)

    #define HBOOST_VARIANT_AUX_MAKE_REFERENCE_CONTENT_TYPEDEFS(z,N,_) \
        typedef detail::make_reference_content< \
              HBOOST_PP_CAT(recursive_enabled_T,N) \
            >::type HBOOST_PP_CAT(internal_T,N); \
        /**/

    HBOOST_PP_REPEAT(
          HBOOST_VARIANT_LIMIT_TYPES
        , HBOOST_VARIANT_AUX_MAKE_REFERENCE_CONTENT_TYPEDEFS
        , _
        )

    #undef HBOOST_VARIANT_AUX_MAKE_REFERENCE_CONTENT_TYPEDEFS

private: // internal typedefs

    typedef typename detail::variant::make_variant_list<
          HBOOST_VARIANT_ENUM_PARAMS(internal_T)
        >::type internal_types;
...
private: // static precondition assertions
    ...
    //
    HBOOST_MPL_ASSERT_NOT(( is_sequence_based_ ));

#endif // HBOOST_VARIANT_NO_TYPE_SEQUENCE_SUPPORT workaround
...
#else // defined(HBOOST_VARIANT_MINIMIZE_SIZE)
...
#endif // HBOOST_VARIANT_MINIMIZE_SIZE switch
...
// representation -- private when possible
    ...
    void indicate_which(int which_arg) HBOOST_NOEXCEPT
    {
        which_ = static_cast<which_t>( which_arg );
    }

    void indicate_backup_which(int which_arg) HBOOST_NOEXCEPT
    {
...
private: // helpers, for queries (below)
    ...
    bool using_backup() const HBOOST_NOEXCEPT
    {
        return which_ < 0;
    }
...
public: // queries
    ...
    int which() const HBOOST_NOEXCEPT
    {
...
private: // helpers, for structors (below)
    ...
    struct initializer
        : HBOOST_VARIANT_AUX_INITIALIZER_T(
              recursive_enabled_types, recursive_enabled_T
            )
    {
    };

    void destroy_content() HBOOST_NOEXCEPT
    {
...
public: // structors
    ...
    variant() 
              ...
              HBOOST_NOEXCEPT_IF(hboost::has_nothrow_constructor<internal_T0>::value)
#endif
    {
#ifdef _MSC_VER
#pragma warning( push )
...
private: // helpers, for structors, cont. (below)
    ...
    public: // structors
        ...
        explicit convert_copy_into(void* storage) HBOOST_NOEXCEPT
            : storage_(storage)
        {
        }
...
#   endif
        ...
        template <typename T>
        int internal_visit(hboost::detail::reference_content<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const hboost::detail::reference_content<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(hboost::detail::variant::backup_holder<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const hboost::detail::variant::backup_holder<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(hboost::recursive_wrapper<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const hboost::recursive_wrapper<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }
    ...
    public: // structors
        ...
        explicit convert_move_into(void* storage) HBOOST_NOEXCEPT
            : storage_(storage)
        {
        }
...
private: // helpers, for structors, below 
...
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename T>
    typename hboost::enable_if<hboost::is_rvalue_reference<T&&> >::type convert_construct(
          T&& operand
        , int
        , mpl::false_ = mpl::false_() // is_foreign_variant
        )
...
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename Variant>
    typename hboost::enable_if<hboost::is_rvalue_reference<Variant&&> >::type convert_construct(
          Variant&& operand
        , long
        , mpl::true_// is_foreign_variant
        )
...
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename Variant>
    typename hboost::enable_if<hboost::is_rvalue_reference<Variant&&> >::type convert_construct_variant(Variant&& operand)
    {
        // [Determine if the given variant is itself a bounded type, or if its
        //  content needs to be converted (i.e., it is a 'foreign' variant):]
        //
    ...
    template <HBOOST_VARIANT_ENUM_PARAMS(typename U)>
    void convert_construct(
          hboost::variant<HBOOST_VARIANT_ENUM_PARAMS(U)>& operand
        , long
        )
    {
    ...
    template <HBOOST_VARIANT_ENUM_PARAMS(typename U)>
    void convert_construct(
          const hboost::variant<HBOOST_VARIANT_ENUM_PARAMS(U)>& operand
        , long
        )
    {
        convert_construct_variant(operand);    
...
#if !defined(HBOOST_VARIANT_AUX_BROKEN_CONSTRUCTOR_TEMPLATE_ORDERING)
...
#elif defined(HBOOST_VARIANT_AUX_HAS_CONSTRUCTOR_TEMPLATE_ORDERING_SFINAE_WKND)
...
#else // !defined(HBOOST_VARIANT_AUX_HAS_CONSTRUCTOR_TEMPLATE_ORDERING_SFINAE_WKND)
...
#endif // HBOOST_VARIANT_AUX_BROKEN_CONSTRUCTOR_TEMPLATE_ORDERING workarounds
    
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
    template <class T>
    variant(T&& operand, typename hboost::enable_if<hboost::is_rvalue_reference<T&&> >::type* = 0, 
...
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
    variant(variant&& operand) HBOOST_NOEXCEPT_IF(variant_move_noexcept_constructible::type::value)
    {
...
#   endif
    ...
    public: // structors
        ...
        assigner(variant& lhs, int rhs_which) HBOOST_NOEXCEPT
            : lhs_(lhs)
            , rhs_which_(rhs_which)
        {
        }
    ...
    protected: // helpers, for internal visitor interface (below)
        ...
        template <typename RhsT, typename B1, typename _B2>
        void assign_impl(
              const RhsT& rhs_content
            , mpl::true_ // has_nothrow_copy
            , B1 // is_nothrow_move_constructible
            , _B2 // has_fallback_type
            ) const HBOOST_NOEXCEPT
        {
        ...
        void construct_fallback() const HBOOST_NOEXCEPT {
            ...
            lhs_.indicate_which(
                  HBOOST_MPL_AUX_VALUE_WKND(fallback_type_index_)::value
                ); // nothrow
        }
    ...
    public: // structors
        ...
        move_assigner(variant& lhs, int rhs_which) HBOOST_NOEXCEPT
    ...
    private: // helpers, for internal visitor interface (below)
        ...
        template <typename RhsT, typename _B2>
        void assign_impl(
              RhsT& rhs_content
            , mpl::true_ // has_nothrow_copy
            , mpl::false_ // is_nothrow_move_constructible
        ...
        {
            assigner::assign_impl(rhs_content, mpl::true_(), mpl::false_(), _B2());
        }

        template <typename RhsT, typename B, typename _B2>
        void assign_impl(
              RhsT& rhs_content
            , B // has_nothrow_copy
            , mpl::true_ // is_nothrow_move_constructible
...
public: // modifiers
...
#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
    template <class T>
    typename hboost::enable_if_c<hboost::is_rvalue_reference<T&&>::value && !hboost::is_const<T>::value, variant& >::type 
        operator=(T&& rhs) 
    {
    ...
    variant& operator=(variant&& rhs) 
        ...
        HBOOST_NOEXCEPT_IF(variant_move_noexcept_constructible::type::value && variant_move_noexcept_assignable::type::value)
#endif
    {
...
public: // queries
    ...
    const hboost::typeindex::type_info& type() const
    {
...
// Obsolete. Remove.
#   define HBOOST_VARIANT_AUX_FAIL_COMPARISON_RETURN_TYPE \
    void

    template <typename U>
    void operator==(const U&) const
    {
        HBOOST_STATIC_ASSERT( false && sizeof(U) );
    }
...
#else// defined(HBOOST_NO_MEMBER_TEMPLATE_FRIENDS)

public:

#endif// !defined(HBOOST_NO_MEMBER_TEMPLATE_FRIENDS)
...
private: // precondition assertions
    ...
    HBOOST_STATIC_ASSERT(( ::hboost::mpl::is_sequence<Types>::value ));
...
inline void swap(
      variant< HBOOST_VARIANT_ENUM_PARAMS(T) >& lhs
    , variant< HBOOST_VARIANT_ENUM_PARAMS(T) >& rhs
    )
{
    lhs.swap(rhs);
}
...
#include "hboost/variant/detail/variant_io.hpp"
#endif // HBOOST_NO_IOSTREAM

#endif // HBOOST_VARIANT_VARIANT_HPP
