...
#ifndef HBOOST_PTR_CONTAINER_DETAIL_ASSOCIATIVE_PTR_CONTAINER_HPP
#define HBOOST_PTR_CONTAINER_DETAIL_ASSOCIATIVE_PTR_CONTAINER_HPP
...
namespace ptr_container_detail
{
        ...
        public reversible_ptr_container<Config,CloneAllocator>
    {
        ...
        typedef HBOOST_DEDUCED_TYPENAME base_type::scoped_deleter
                                scoped_deleter;

        typedef HBOOST_DEDUCED_TYPENAME Config::container_type
                                container_type;
    public: // typedefs
        typedef HBOOST_DEDUCED_TYPENAME Config::key_type
                                key_type;
        typedef HBOOST_DEDUCED_TYPENAME Config::key_compare
                                key_compare;
        typedef HBOOST_DEDUCED_TYPENAME Config::value_compare
                                value_compare;
        typedef HBOOST_DEDUCED_TYPENAME Config::hasher
                                hasher;
        typedef HBOOST_DEDUCED_TYPENAME Config::key_equal
                                key_equal;
        typedef HBOOST_DEDUCED_TYPENAME Config::iterator
                                iterator;
        typedef HBOOST_DEDUCED_TYPENAME Config::const_iterator
                                const_iterator;
        typedef HBOOST_DEDUCED_TYPENAME Config::local_iterator
                                local_iterator;
        typedef HBOOST_DEDUCED_TYPENAME Config::const_local_iterator
    ...
    public: // associative container interface
        ...
        HBOOST_DEDUCED_TYPENAME hboost::disable_if< hboost::is_convertible<Range&,key_type&>, 
                                                  iterator >::type
        erase( const Range& r )
        {
    ...
    protected:
        ...
        void multi_transfer( HBOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator object,
                             AssociatePtrCont& from ) // strong
        {
            HBOOST_ASSERT( (void*)&from != (void*)this );
            HBOOST_ASSERT( !from.empty() && "Cannot transfer from empty container" );
        ...
        template< class AssociatePtrCont >
        size_type multi_transfer( HBOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator first,
                                  HBOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator last,
                                  AssociatePtrCont& from ) // basic
        {
            ...
            for( ; first != last; )
            {
                HBOOST_ASSERT( first != from.end() );
                this->base().insert( *first.base() );     // strong
                HBOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator 
                    to_delete( first );
                ++first;
        ...
        template< class AssociatePtrCont >
        bool single_transfer( HBOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator object,
                              AssociatePtrCont& from ) // strong
        {
            HBOOST_ASSERT( (void*)&from != (void*)this );
            HBOOST_ASSERT( !from.empty() && "Cannot transfer from empty container" );

            std::pair<HBOOST_DEDUCED_TYPENAME base_type::ptr_iterator,bool> p =
                this->base().insert( *object.base() );     // strong
            if( p.second )
                from.base().erase( object.base() );        // nothrow
        ...
        template< class AssociatePtrCont >
        size_type single_transfer( HBOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator first,
                                   HBOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator last,
                                   AssociatePtrCont& from ) // basic
        {
            HBOOST_ASSERT( (void*)&from != (void*)this );
        ...
        {
            HBOOST_ASSERT( !this->empty() );
            HBOOST_ASSERT( *this->begin().base() != 0 );
            return *this->begin(); 
        }
    ...
    public:
#if HBOOST_WORKAROUND(__DECCXX_VER, HBOOST_TESTED_AT(70190006))
        iterator begin()
        {
