...
#ifndef HBOOST_PTR_CONTAINER_DETAIL_REVERSIBLE_PTR_CONTAINER_HPP
#define HBOOST_PTR_CONTAINER_DETAIL_REVERSIBLE_PTR_CONTAINER_HPP

#if defined(_MSC_VER) && (_MSC_VER >= 1200)
# pragma once
#endif

#include <hboost/ptr_container/detail/throw_exception.hpp>
#include <hboost/ptr_container/detail/scoped_deleter.hpp>
#include <hboost/ptr_container/detail/static_move_ptr.hpp>
#include <hboost/ptr_container/exception.hpp>
#include <hboost/ptr_container/clone_allocator.hpp>
#include <hboost/ptr_container/nullable.hpp>

#ifdef HBOOST_NO_SFINAE
#else
#include <hboost/range/functions.hpp>
#endif
...
namespace ptr_container_detail
{
    ...
    {
        HBOOST_STATIC_CONSTANT(bool, value = is_pointer<T>::value || is_integral<T>::value );
    };
    ...
    {
    private:
        HBOOST_STATIC_CONSTANT( bool, allow_null = Config::allow_null );
        
        typedef HBOOST_DEDUCED_TYPENAME Config::value_type Ty_;
        ...
        struct null_clone_allocator
        {
            ...
            static Ty_* allocate_clone( const Ty_* x )
            {
                ...
                {
                    HBOOST_ASSERT( x != 0 && "Cannot insert clone of null!" );
                }

                Ty_* res = CloneAllocator::allocate_clone( *x );
                HBOOST_ASSERT( typeid(*res) == typeid(*x) &&
        ...
        typedef HBOOST_DEDUCED_TYPENAME Config::void_container_type  Cont;
    ...
    public: // typedefs
        ...
        typedef  HBOOST_DEDUCED_TYPENAME Config::iterator 
                                   iterator;
        typedef  HBOOST_DEDUCED_TYPENAME Config::const_iterator
                                   const_iterator;
        typedef  hboost::reverse_iterator< iterator > 
                                   reverse_iterator;  
        typedef  hboost::reverse_iterator< const_iterator >     
                                   const_reverse_iterator;
        typedef  HBOOST_DEDUCED_TYPENAME Cont::difference_type
                                   difference_type; 
        typedef  HBOOST_DEDUCED_TYPENAME Cont::size_type
                                   size_type;
        typedef  HBOOST_DEDUCED_TYPENAME Config::allocator_type
                                   allocator_type;
    ...
    protected: 
        ...
        typedef ptr_container_detail::scoped_deleter<Ty_,null_cloner_type>
                                   scoped_deleter;
        typedef HBOOST_DEDUCED_TYPENAME Cont::iterator
                                   ptr_iterator; 
        typedef HBOOST_DEDUCED_TYPENAME Cont::const_iterator
                                   ptr_const_iterator; 
    private:
        ...
        {
            HBOOST_ASSERT( size_type( std::distance( sd.begin(), sd.end() ) ) == c_.size() );
    ...
    protected:
        ...
        static void enforce_null_policy( const Ty_* x, const char* msg )
        {
            if( !allow_null )
            {
                HBOOST_PTR_CONTAINER_THROW_EXCEPTION( 0 == x && "null not allowed", 
                                                     bad_pointer, msg );
            }
        }
    ...
    public: // modifiers
        ...
        {
            return erase( hboost::begin(r), hboost::end(r) );
        }
    ...
    public: // access interface
        ...
        { 
            HBOOST_ASSERT( where != end() );
            
            HBOOST_PTR_CONTAINER_THROW_EXCEPTION( empty(), bad_ptr_container_operation,
                                                 "'release()' on empty container" ); 
            
            auto_type ptr( Config::get_pointer( where ) );  // nothrow
            c_.erase( where.base() );                       // nothrow
            return hboost::ptr_container_detail::move( ptr ); 
        }
        ...
        { 
            ...
            return hboost::ptr_container_detail::move( old );
        }
        ...
        auto_type replace( size_type idx, Ty_* x ) // strong
        {
            ...
            HBOOST_PTR_CONTAINER_THROW_EXCEPTION( idx >= size(), bad_index, 
...
#if HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x564))    
#define HBOOST_PTR_CONTAINER_DEFINE_RELEASE( base_type ) \
    typename base_type::auto_type                   \
    release( typename base_type::iterator i )       \
    {                                               \
        return hboost::ptr_container_detail::move(base_type::release(i)); \
    }                                               
#else
#define HBOOST_PTR_CONTAINER_DEFINE_RELEASE( base_type ) \
    ...
    //
#define HBOOST_PTR_CONTAINER_DEFINE_RELEASE_AND_CLONE( PC, base_type, this_type ) \
    ...
    HBOOST_PTR_CONTAINER_DEFINE_RELEASE( base_type ) \
                                                    \
...
#define HBOOST_PTR_CONTAINER_DEFINE_COPY_CONSTRUCTORS( PC, base_type ) \
                                                                      \
    template< class U >                                               \
...
#define HBOOST_PTR_CONTAINER_DEFINE_CONSTRUCTORS( PC, base_type )                       \
    typedef HBOOST_DEDUCED_TYPENAME base_type::iterator        iterator;                \
    typedef HBOOST_DEDUCED_TYPENAME base_type::size_type       size_type;               \
    typedef HBOOST_DEDUCED_TYPENAME base_type::const_reference const_reference;         \
    typedef HBOOST_DEDUCED_TYPENAME base_type::allocator_type  allocator_type;          \
    PC() {}                                                                            \
...
#define HBOOST_PTR_CONTAINER_DEFINE_NON_INHERITED_MEMBERS( PC, base_type, this_type )           \
   HBOOST_PTR_CONTAINER_DEFINE_CONSTRUCTORS( PC, base_type )                                    \
   HBOOST_PTR_CONTAINER_DEFINE_RELEASE_AND_CLONE( PC, base_type, this_type )

#define HBOOST_PTR_CONTAINER_DEFINE_SEQEUENCE_MEMBERS( PC, base_type, this_type )  \
    HBOOST_PTR_CONTAINER_DEFINE_NON_INHERITED_MEMBERS( PC, base_type, this_type )  \
    HBOOST_PTR_CONTAINER_DEFINE_COPY_CONSTRUCTORS( PC, base_type )
