...
#ifndef HBOOST_PTR_CONTAINER_PTR_SEQUENCE_ADAPTER_HPP
#define HBOOST_PTR_CONTAINER_PTR_SEQUENCE_ADAPTER_HPP
...
#include <hboost/ptr_container/detail/reversible_ptr_container.hpp>
#include <hboost/ptr_container/indirect_fun.hpp>
#include <hboost/ptr_container/detail/void_ptr_iterator.hpp>
...
{    
    ...
    {
        typedef HBOOST_DEDUCED_TYPENAME remove_nullable<T>::type
                    U;
        typedef VoidPtrSeq
                    void_container_type;

        typedef HBOOST_DEDUCED_TYPENAME VoidPtrSeq::allocator_type
                    allocator_type;
        
        typedef U   value_type;

        typedef void_ptr_iterator<
                        HBOOST_DEDUCED_TYPENAME VoidPtrSeq::iterator, U > 
                    iterator;
       
        typedef void_ptr_iterator<
                        HBOOST_DEDUCED_TYPENAME VoidPtrSeq::const_iterator, const U >
                    const_iterator;

#if defined(HBOOST_NO_SFINAE) || defined(HBOOST_NO_FUNCTION_TEMPLATE_ORDERING)
...
#if defined(HBOOST_NO_SFINAE) && !HBOOST_WORKAROUND(__MWERKS__, <= 0x3003)
        ...
        HBOOST_STATIC_CONSTANT(bool, allow_null = hboost::is_nullable<T>::value );
    };
    ...
    protected:
        typedef HBOOST_DEDUCED_TYPENAME base_type::scoped_deleter scoped_deleter;
         
    public:
        typedef HBOOST_DEDUCED_TYPENAME base_type::value_type  value_type; 
        typedef HBOOST_DEDUCED_TYPENAME base_type::reference   reference; 
        typedef HBOOST_DEDUCED_TYPENAME base_type::const_reference 
                                                              const_reference;
        typedef HBOOST_DEDUCED_TYPENAME base_type::auto_type   auto_type;
        typedef HBOOST_DEDUCED_TYPENAME base_type::clone_allocator_type
                                                              clone_allocator_type;
        typedef HBOOST_DEDUCED_TYPENAME base_type::iterator    iterator;          
        typedef HBOOST_DEDUCED_TYPENAME base_type::size_type   size_type;  
        typedef HBOOST_DEDUCED_TYPENAME base_type::allocator_type  
                                                              allocator_type;
        ...
        { 
            ...
            HBOOST_ASSERT( !::hboost::is_null( this->begin() ) );
            return *this->begin(); 
        }
        ...
        reference back()
        {
            ...
            HBOOST_ASSERT( !::hboost::is_null( --this->end() ) );
            return *--this->end(); 
        }
    ...
    public: // deque/vector inerface
        ...
        {
            HBOOST_ASSERT( n < this->size() );
            HBOOST_ASSERT( !this->is_null( n ) );
        ...
        {
            HBOOST_PTR_CONTAINER_THROW_EXCEPTION( n >= this->size(), bad_index, 
                                                 "'at()' out of bounds" );
            HBOOST_ASSERT( !this->is_null( n ) );
            return (*this)[n];
        }
    ...
    public: // assign, insert, transfer
        ...
        {
            assign( hboost::begin(r), hboost::end(r ) );
        }
    ...
    public:
        ...
        {
            insert_impl( before, first, last, HBOOST_DEDUCED_TYPENAME
        ...
        hboost::disable_if< ptr_container_detail::is_pointer_or_integral<Range> >::type
        insert( iterator before, const Range& r )
        {
            insert( before, hboost::begin(r), hboost::end(r) );
        }
        ...
        void transfer( iterator before, 
                       HBOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator first, 
                       HBOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator last, 
                       PtrSeqAdapter& from ) // strong
        {
            HBOOST_ASSERT( (void*)&from != (void*)this );
            if( from.empty() )
                return;
            this->base().
        ...
        template< class PtrSeqAdapter >
        void transfer( iterator before, 
                       HBOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator object, 
                       PtrSeqAdapter& from ) // strong
        {
            HBOOST_ASSERT( (void*)&from != (void*)this );
            if( from.empty() )
        ...
        HBOOST_DEDUCED_TYPENAME hboost::disable_if< hboost::is_same< Range,
                      HBOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator > >::type
        transfer( iterator before, const Range& r, PtrSeqAdapter& from ) // strong
        {
            transfer( before, hboost::begin(r), hboost::end(r), from );
        }
    ...
    public: // C-array support
        ...
        {
            HBOOST_ASSERT( from != 0 );
            if( delete_from )
            {
                HBOOST_DEDUCED_TYPENAME base_type::scoped_deleter 
    ...
    public: // resize
        ...
        void resize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( hboost::next( this->begin(), size ), this->end() );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
                    this->push_back( new HBOOST_DEDUCED_TYPENAME 
                                     hboost::remove_pointer<value_type>::type() ); 
            }

            HBOOST_ASSERT( this->size() == size );
        }
        ...
        void rresize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( this->begin(), 
                             hboost::next( this->begin(), old_size - size ) );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
                    this->push_front( new HBOOST_DEDUCED_TYPENAME 
                                      hboost::remove_pointer<value_type>::type() ); 
            }
    ...
    public: // algorithms
        ...
        {
            HBOOST_ASSERT( first <= last && "out of range sort()" );
            HBOOST_ASSERT( this->begin() <= first && "out of range sort()" );
            HBOOST_ASSERT( last <= this->end() && "out of range sort()" ); 
    ...
    private:
        ...
        {
            HBOOST_ASSERT( first <= last && "out of range unique()/erase_if()" );
            HBOOST_ASSERT( this->begin() <= first && "out of range unique()/erase_if()" );
            HBOOST_ASSERT( last <= this->end() && "out of range unique()/erase_if)(" );             
        }
        
        void range_check( iterator first, iterator last )
        {
            range_check_impl( first, last, 
                              HBOOST_DEDUCED_TYPENAME iterator_category<iterator>::type() );
        }
    ...
    public:
        ...
        void unique( iterator first, iterator last, Compare comp )
        {
            ...
            for( ; next != last; ++next )
            {
                HBOOST_ASSERT( !::hboost::is_null(prev) );
                HBOOST_ASSERT( !::hboost::is_null(next) );
                if( comp( *prev, *next ) )
                {
