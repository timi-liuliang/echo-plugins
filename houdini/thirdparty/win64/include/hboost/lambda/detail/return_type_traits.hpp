...
#ifndef HBOOST_LAMBDA_RETURN_TYPE_TRAITS_HPP
#define HBOOST_LAMBDA_RETURN_TYPE_TRAITS_HPP
...
namespace detail {
  ...
  class protect_conversion {
      typedef typename hboost::remove_reference<T>::type non_ref_T;
    public:

  // add const to rvalues, so that all rvalues are stored as const in 
  // the args tuple
    typedef typename detail::IF_type<
      hboost::is_reference<T>::value && !hboost::is_const<non_ref_T>::value,
...
// experimental feature
  ...
  typedef typename hboost::remove_reference<A>::type non_ref_A;
  typedef typename hboost::remove_reference<B>::type non_ref_B;
