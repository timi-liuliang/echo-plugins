...
#ifndef HBOOST_LAMBDA_ARITY_CODE_HPP
#define HBOOST_LAMBDA_ARITY_CODE_HPP
...
template <class T> struct get_arity {
  ...
  HBOOST_STATIC_CONSTANT(int, value = detail::get_arity_<typename hboost::remove_cv<typename hboost::remove_reference<T>::type>::type>::value);
...
struct get_arity_<lambda_functor<T> > {
  HBOOST_STATIC_CONSTANT(int, value = get_arity<T>::value);
};

template<class Action, class Args>
struct get_arity_<lambda_functor_base<Action, Args> > {
  HBOOST_STATIC_CONSTANT(int, value = get_tuple_arity<Args>::value);
};

template<int I>
struct get_arity_<placeholder<I> > {
  HBOOST_STATIC_CONSTANT(int, value = I);
};
...
template<class T>
struct get_tuple_arity {
  HBOOST_STATIC_CONSTANT(int, value = get_arity<typename T::head_type>::value | get_tuple_arity<typename T::tail_type>::value);
};
...
struct has_placeholder {
  HBOOST_STATIC_CONSTANT(bool, value = (get_arity<T>::value & I) != 0);
}; 

template<int I, int J>
struct includes_placeholder {
  HBOOST_STATIC_CONSTANT(bool, value = (J & I) != 0);
};
