...
#ifndef HBOOST_LAMBDA_LAMBDA_FUNCTORS_HPP
#define HBOOST_LAMBDA_LAMBDA_FUNCTORS_HPP
...
#define HBOOST_LAMBDA_DISABLE_IF_ARRAY1(A1, R1)\
  typename lazy_disable_if<is_array<A1>, typename R1 >::type
#define HBOOST_LAMBDA_DISABLE_IF_ARRAY2(A1, A2, R1, R2) \
  typename lazy_disable_if<mpl::or_<is_array<A1>, is_array<A2> >, typename R1, R2 >::type
#define HBOOST_LAMBDA_DISABLE_IF_ARRAY3(A1, A2, A3, R1, R2, R3) \
  typename lazy_disable_if<mpl::or_<is_array<A1>, is_array<A2>, is_array<A3> >, typename R1, R2, R3 >::type

#else

#define HBOOST_LAMBDA_DISABLE_IF_ARRAY1(A1, R1) typename R1::type
#define HBOOST_LAMBDA_DISABLE_IF_ARRAY2(A1, A2, R1, R2) typename R1, R2::type
#define HBOOST_LAMBDA_DISABLE_IF_ARRAY3(A1, A2, A3, R1, R2, R3) typename R1, R2, R3::type
...
template<> struct placeholder<FIRST> {
  ...
  RET call(CALL_FORMAL_ARGS) const { 
    HBOOST_STATIC_ASSERT(hboost::is_reference<RET>::value); 
...
HBOOST_STATIC_CONSTANT(int, arity_bits = get_arity<T>::value);
...
public:
  ...
  template<class A>
  HBOOST_LAMBDA_DISABLE_IF_ARRAY1(A, inherited::template sig<tuple<A const&> >)
  ...
  template<class A, class B>
  HBOOST_LAMBDA_DISABLE_IF_ARRAY2(A, B, inherited::template sig<tuple<A const&, B&> >)
  ...
  template<class A, class B>
  HBOOST_LAMBDA_DISABLE_IF_ARRAY2(A, B, inherited::template sig<tuple<A&, B const&> >)
  ...
  template<class A, class B>
  HBOOST_LAMBDA_DISABLE_IF_ARRAY2(A, B, inherited::template sig<tuple<A const&, B const&> >)
  ...
  template<class A, class B, class C>
  HBOOST_LAMBDA_DISABLE_IF_ARRAY3(A, B, C, inherited::template sig<tuple<A const&, B const&, C const&> >)
...
#if !defined(HBOOST_RESULT_OF_USE_DECLTYPE) || defined(HBOOST_NO_CXX11_DECLTYPE)

template<class T>
struct result_of<hboost::lambda::lambda_functor<T>()>
{
    typedef typename hboost::lambda::lambda_functor<T>::nullary_return_type type;
};

template<class T>
struct result_of<const hboost::lambda::lambda_functor<T>()>
{
    typedef typename hboost::lambda::lambda_functor<T>::nullary_return_type type;
};
...
template<class T>
struct tr1_result_of<hboost::lambda::lambda_functor<T>()>
{
    typedef typename hboost::lambda::lambda_functor<T>::nullary_return_type type;
};

template<class T>
struct tr1_result_of<const hboost::lambda::lambda_functor<T>()>
{
    typedef typename hboost::lambda::lambda_functor<T>::nullary_return_type type;
};
