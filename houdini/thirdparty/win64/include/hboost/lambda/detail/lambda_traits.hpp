...
#ifndef HBOOST_LAMBDA_LAMBDA_TRAITS_HPP
#define HBOOST_LAMBDA_LAMBDA_TRAITS_HPP
...
struct IF_value_
{
  HBOOST_STATIC_CONSTANT(int, value = T::value);
};
...
struct IF_value
{
  HBOOST_STATIC_CONSTANT(int, value = (IF_value_<typename IF<C, T, E>::RET>::value));
};
...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename hboost::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    hboost::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<int N, class T> struct tuple_element_as_reference {   
  typedef typename
     hboost::tuples::access_traits<
       typename hboost::tuples::element<N, T>::type
     >::non_const_type type;
};
...
struct parameter_traits_<hboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const hboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<volatile hboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const volatile hboost::reference_wrapper<T>, Any >{
  typedef T& type;
};
...
struct const_copy_argument {
  typedef typename 
    detail::parameter_traits_<
      T,
      typename detail::IF<hboost::is_function<T>::value, T&, const T>::RET
    >::type type;
};
...
class bind_type_generator {
  ...
  HBOOST_STATIC_CONSTANT(int, nof_elems = hboost::tuples::length<args_t>::value);
...
#endif // HBOOST_LAMBDA_TRAITS_HPP
