...
#ifndef HBOOST_LAMBDA_FUNCTION_ADAPTORS_HPP
#define HBOOST_LAMBDA_FUNCTION_ADAPTORS_HPP
...
HBOOST_MPL_HAS_XXX_TEMPLATE_DEF(sig)

template<class Tuple>
struct remove_references_from_elements {
  typedef typename hboost::tuples::cons<
    typename hboost::remove_reference<typename Tuple::head_type>::type,
    typename remove_references_from_elements<typename Tuple::tail_type>::type
  > type;
};

template<>
struct remove_references_from_elements<hboost::tuples::null_type> {
...
template <class Func> struct function_adaptor {
  ...
  // Support functors that use the hboost::result_of return type convention.
...
struct function_adaptor<T Object::*> {
  ...
  template<class Args> class sig { 
    typedef typename hboost::tuples::element<1, Args>::type argument_type;
    typedef typename hboost::remove_reference<
      argument_type
    >::type unref_type;

    typedef typename detail::IF<hboost::is_const<unref_type>::value,
      typename hboost::add_const<T>::type,
      T
    >::RET properly_consted_return_type;

    typedef typename detail::IF<hboost::is_volatile<unref_type>::value,
      typename hboost::add_volatile<properly_consted_return_type>::type,
      properly_consted_return_type
    >::RET properly_cvd_return_type;


  public:
    typedef typename detail::IF<hboost::is_reference<argument_type>::value,
      typename hboost::add_reference<properly_cvd_return_type>::type,
