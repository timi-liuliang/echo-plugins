...
#if !defined(HBOOST_LAMBDA_MEMBER_PTR_HPP)
#define HBOOST_LAMBDA_MEMBER_PTR_HPP
...
struct member_pointer {
  ...
  HBOOST_STATIC_CONSTANT(bool, is_data_member = false);
  HBOOST_STATIC_CONSTANT(bool, is_function_member = false);
};
...
struct member_pointer<T U::*> {
  ...
  HBOOST_STATIC_CONSTANT(bool, is_data_member = true);
  HBOOST_STATIC_CONSTANT(bool, is_function_member = false);
};

template<class T, class U>
struct member_pointer<const T U::*> {
  typedef typename hboost::add_reference<const T>::type type;
  typedef U class_type;
...
struct member_pointer<volatile T U::*> {
  typedef typename hboost::add_reference<volatile T>::type type;
  typedef U class_type;
...
struct member_pointer<const volatile T U::*> {
  typedef typename hboost::add_reference<const volatile T>::type type;
  typedef U class_type;
...
struct member_pointer<T (U::*)()> {
  ...
  typedef U qualified_class_type;
  HBOOST_STATIC_CONSTANT(bool, is_data_member = false);
  HBOOST_STATIC_CONSTANT(bool, is_function_member = true);
};
...
public:
  ...
  private:
    ...
    typedef typename hboost::remove_reference<type0>::type type1;
    ...
    typedef typename ::hboost::remove_pointer<non_ref_A>::type non_pointer_A; 
  ...
  public:
    ...
    typedef typename detail::IF<
      ::hboost::is_const<non_pointer_A>::value, 
      typename ::hboost::add_const<type1>::type,
      type1
    >::RET type2;
    typedef typename detail::IF<
      ::hboost::is_volatile<non_pointer_A>::value, 
      typename ::hboost::add_volatile<type2>::type,
      type2
    >::RET type3;
    // add reference back
    typedef typename ::hboost::add_reference<type3>::type type;
  };
};
...
struct member_pointer_action_helper<false, true> {
  ...
  template<class RET, class A, class B>
  static RET apply(A& a, B& b) { 
    typedef typename ::hboost::remove_cv<B>::type plainB;
    typedef typename detail::member_pointer<plainB>::type ret_t; 
    typedef typename ::hboost::remove_cv<A>::type plainA;
...
public:
  ...
  static RET apply(A& a, B& b) {
    typedef typename 
      ::hboost::remove_cv<B>::type plainB;

    return detail::member_pointer_action_helper<
        hboost::is_pointer<A>::value && 
...
struct return_type_N<other_action<member_pointer_action>, Args> {
  typedef typename hboost::tuples::element<0, Args>::type A;
  typedef typename hboost::tuples::element<1, Args>::type B;
  typedef typename 
    return_type_2<other_action<member_pointer_action>, 
                  typename hboost::remove_reference<A>::type, 
                  typename hboost::remove_reference<B>::type
                 >::type type;
};
