...
#ifndef HBOOST_LAMBDA_OPERATOR_RETURN_TYPE_TRAITS_HPP
#define HBOOST_LAMBDA_OPERATOR_RETURN_TYPE_TRAITS_HPP
...
#include "hboost/detail/container_fwd.hpp"
...
struct return_type_1<other_action<addressof_action>, A> { 
  ...
  typedef typename detail::IF<
    hboost::is_same<type1, detail::unspecified>::value, 
...
template <class A> struct contentsof_type {
  typedef typename hboost::indirect_reference<A>::type type; 
};
...
struct pointer_arithmetic_traits<plus_action, A, B> { 
  ...
  typedef typename 
    array_to_pointer<typename hboost::remove_reference<A>::type>::type AP;
  typedef typename 
    array_to_pointer<typename hboost::remove_reference<B>::type>::type BP;

  static const bool is_pointer_A = hboost::is_pointer<AP>::value;
  static const bool is_pointer_B = hboost::is_pointer<BP>::value;  
    ...
    hboost::is_same<
      typename hboost::remove_const<
        typename hboost::remove_pointer<
          typename hboost::remove_const<AP>::type
        >::type
      >::type,
      typename hboost::remove_const<
        typename hboost::remove_pointer<
          typename hboost::remove_const<BP>::type
        >::type
      >::type
    >::value;
...
struct return_type_2<bitwise_action<Act>, A, B>
{
  ...
  // the apply function
  HBOOST_STATIC_ASSERT(!(hboost::is_float<plain_A>::value && hboost::is_float<plain_B>::value));
...
private:
  typedef typename hboost::remove_reference<A>::type plainA;
public:
...
struct return_type_2<arithmetic_assignment_action<Act>, A, B> { 
  ...
  typedef typename 
    detail::IF<
      hboost::is_same<type1, detail::unspecified>::value, 
      typename hboost::add_reference<A>::type,
      type1
    >::RET type;
};
...
struct return_type_2<other_action<subscript_action>, A, B> {
  ...
  typedef typename hboost::remove_reference<A>::type nonref_A;
  typedef typename hboost::remove_reference<B>::type nonref_B;
