...
#if !defined(HBOOST_LAMBDA_LOOPS_HPP)
#define HBOOST_LAMBDA_LOOPS_HPP
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    for(detail::select(hboost::tuples::get<0>(args), CALL_ACTUAL_ARGS); 
        detail::select(hboost::tuples::get<1>(args), CALL_ACTUAL_ARGS); 
        detail::select(hboost::tuples::get<2>(args), CALL_ACTUAL_ARGS))
      
      detail::select(hboost::tuples::get<3>(args), CALL_ACTUAL_ARGS);
  }
};
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
    while(detail::select(hboost::tuples::get<0>(args), CALL_ACTUAL_ARGS))
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
          do {} while (detail::select(hboost::tuples::get<0>(args), CALL_ACTUAL_ARGS) );
  }
};
