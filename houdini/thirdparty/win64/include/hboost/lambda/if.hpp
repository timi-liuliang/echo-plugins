...
#if !defined(HBOOST_LAMBDA_IF_HPP)
#define HBOOST_LAMBDA_IF_HPP

#include "hboost/lambda/core.hpp"

// Arithmetic type promotion needed for if_then_else_return
#include "hboost/lambda/detail/operator_actions.hpp"
#include "hboost/lambda/detail/operator_return_type_traits.hpp"
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(hboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(hboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
      ...
      detail::select(hboost::tuples::get<2>(args), CALL_ACTUAL_ARGS);
  }
};
...
struct return_type_2_ifthenelsereturn<1, false, false, false, A, B> {
  ...
  typedef const typename hboost::remove_reference<A>::type plainA; 
  typedef const typename hboost::remove_reference<B>::type plainB; 
  // TODO: Add support for volatile ?

  typedef typename
       return_type_2_ifthenelsereturn<
         2,
         hboost::is_convertible<plainA,plainB>::value, 
         hboost::is_convertible<plainB,plainA>::value,
         hboost::is_same<plainA,plainB>::value,
         plainA, 
         plainB>::type type;
};
...
struct return_type_2<other_action<ifthenelsereturn_action>, A, B> { 
  ...
  typedef typename 
    hboost::add_const<typename detail::same_or_not<A1, B1>::type>::type type;
};
