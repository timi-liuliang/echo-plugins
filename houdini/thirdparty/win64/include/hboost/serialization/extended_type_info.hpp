#ifndef HBOOST_SERIALIZATION_EXTENDED_TYPE_INFO_HPP
#define HBOOST_SERIALIZATION_EXTENDED_TYPE_INFO_HPP
...
#define HBOOST_SERIALIZATION_MAX_KEY_SIZE 128
...
class HBOOST_SYMBOL_VISIBLE extended_type_info :
    private hboost::noncopyable
{
private:
    friend class hboost::serialization::void_cast_detail::void_caster;
...
protected:
    HBOOST_SERIALIZATION_DECL void key_unregister() const;
    HBOOST_SERIALIZATION_DECL void key_register() const;
    // this class can't be used as is. It's just the 
    // common functionality for all type_info replacement
    // systems.  Hence, make these protected
    HBOOST_SERIALIZATION_DECL extended_type_info(
        const unsigned int type_info_key,
        const char * key
    );
    virtual HBOOST_SERIALIZATION_DECL ~extended_type_info();
public:
    const char * get_key() const {
        return m_key;
    }
    virtual const char * get_debug_info() const = 0;
    HBOOST_SERIALIZATION_DECL bool operator<(const extended_type_info &rhs) const;
    HBOOST_SERIALIZATION_DECL bool operator==(const extended_type_info &rhs) const;
    ...
    // gcc 4.5 mingw error
    static HBOOST_SERIALIZATION_DECL const extended_type_info *
    find(const char *key);
    // for plugins
...
template<class T>
struct guid_defined : hboost::mpl::false_ {};
...
#endif // HBOOST_SERIALIZATION_EXTENDED_TYPE_INFO_HPP
