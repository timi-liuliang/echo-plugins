// hboost/uuid/sha1.hpp header file  ----------------------------------------------//
...
#ifndef HBOOST_UUID_SHA1_H
#define HBOOST_UUID_SHA1_H
...
HBOOST_STATIC_ASSERT(sizeof(unsigned char)*8 == 8);
HBOOST_STATIC_ASSERT(sizeof(unsigned int)*8 == 32);
...
inline void sha1::process_byte(unsigned char byte)
{
    ...
    if (bit_count_low < 0xFFFFFFF8) {
        ...
        if (bit_count_high <= 0xFFFFFFFE) {
            ...
            HBOOST_THROW_EXCEPTION(std::runtime_error("sha1 too many bytes"));
        }
    }
}
