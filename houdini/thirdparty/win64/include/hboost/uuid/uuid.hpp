...
//  25 Feb 2008 - moved to namespace hboost::uuids
//  19 Mar 2009 - changed to a POD, reorganized files
//  28 Nov 2009 - disabled deprecated warnings for MSVC
//  30 Nov 2009 - used HBOOST_STATIC_CONSTANT
//  02 Dec 2009 - removed HBOOST_STATIC_CONSTANT - not all compilers like it
//  29 Apr 2013 - added support for noexcept and constexpr, added optimizations for SSE/AVX

#ifndef HBOOST_UUID_HPP
#define HBOOST_UUID_HPP

#include <cstddef>
#include <hboost/cstdint.hpp>
#include <hboost/uuid/detail/config.hpp>
#ifndef HBOOST_UUID_NO_TYPE_TRAITS
...
public:
    ...
    // a cpp file
    //HBOOST_STATIC_CONSTANT(size_type, static_size = 16);
    static HBOOST_CONSTEXPR size_type static_size() HBOOST_NOEXCEPT { return 16; }

public:
    iterator begin() HBOOST_NOEXCEPT { return data; }
    const_iterator begin() const HBOOST_NOEXCEPT { return data; }
    iterator end() HBOOST_NOEXCEPT { return data+size(); }
    const_iterator end() const HBOOST_NOEXCEPT { return data+size(); }

    HBOOST_CONSTEXPR size_type size() const HBOOST_NOEXCEPT { return static_size(); }

    bool is_nil() const HBOOST_NOEXCEPT;
    ...
    variant_type variant() const HBOOST_NOEXCEPT
    {
    ...
    version_type version() const HBOOST_NOEXCEPT
    {
    ...
    void swap(uuid& rhs) HBOOST_NOEXCEPT;
...
bool operator== (uuid const& lhs, uuid const& rhs) HBOOST_NOEXCEPT;
bool operator< (uuid const& lhs, uuid const& rhs) HBOOST_NOEXCEPT;

inline bool operator!=(uuid const& lhs, uuid const& rhs) HBOOST_NOEXCEPT
{
    return !(lhs == rhs);
}
...
inline void swap(uuid& lhs, uuid& rhs) HBOOST_NOEXCEPT
{
    lhs.swap(rhs);
}

// This is equivalent to hboost::hash_range(u.begin(), u.end());
inline std::size_t hash_value(uuid const& u) HBOOST_NOEXCEPT
{
    std::size_t seed = 0;
...
#include <hboost/uuid/detail/uuid_x86.hpp>
#else
#include <hboost/uuid/detail/uuid_generic.hpp>
#endif
