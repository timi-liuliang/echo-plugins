...
//  25 Feb 2008 - moved to namespace hboost::uuids::detail
...
#ifndef HBOOST_UUID_SEED_RNG_HPP
#define HBOOST_UUID_SEED_RNG_HPP
...
#include <hboost/core/noncopyable.hpp>
#include <hboost/uuid/sha1.hpp>
//#include <hboost/nondet_random.hpp> //forward declare hboost::random::random_device

// can't use hboost::generator_iterator since hboost::random number seed(Iter&, Iter)
// functions need a last iterator
//#include <hboost/generator_iterator.hpp>
...
#   include <hboost/detail/winapi/crypt.hpp> // for CryptAcquireContextA, CryptGenRandom, CryptReleaseContext
#   include <hboost/detail/winapi/timers.hpp>
#   include <hboost/detail/winapi/process.hpp>
#   include <hboost/detail/winapi/thread.hpp>
#else 
...
class seed_rng: private hboost::noncopyable
{
public:
...
public:
    ...
    seed_rng() HBOOST_NOEXCEPT
        : rd_index_(5)
        , random_(NULL)
    {
#if defined(HBOOST_WINDOWS)
        if (!hboost::detail::winapi::CryptAcquireContextA(
                    &random_,
                    NULL,
                    NULL,
                    hboost::detail::winapi::PROV_RSA_FULL_,
                    hboost::detail::winapi::CRYPT_VERIFYCONTEXT_ | hboost::detail::winapi::CRYPT_SILENT_))
        {
            random_ = NULL;
        }
#else
    ...
    ~seed_rng() HBOOST_NOEXCEPT
    {
        if (random_) {
#if defined(HBOOST_WINDOWS)
            hboost::detail::winapi::CryptReleaseContext(random_, 0);
#else
            std::fclose(random_);
#endif
        }
    ...
    result_type min HBOOST_PREVENT_MACRO_SUBSTITUTION () const HBOOST_NOEXCEPT
    {
        return (std::numeric_limits<result_type>::min)();
    }
    result_type max HBOOST_PREVENT_MACRO_SUBSTITUTION () const HBOOST_NOEXCEPT
    {
...
private:
    HBOOST_STATIC_CONSTANT(std::size_t, internal_state_size = 5);
...
#if defined(HBOOST_WINDOWS)
            hboost::detail::winapi::CryptGenRandom(random_, sizeof(state), state);
#else
...
#if defined(HBOOST_WINDOWS)
            hboost::detail::winapi::DWORD_ procid = hboost::detail::winapi::GetCurrentProcessId();
            sha.process_bytes( (unsigned char const*)&procid, sizeof( procid ) );

            hboost::detail::winapi::DWORD_ threadid = hboost::detail::winapi::GetCurrentThreadId();
            sha.process_bytes( (unsigned char const*)&threadid, sizeof( threadid ) );

            hboost::detail::winapi::LARGE_INTEGER_ ts;
            ts.QuadPart = 0;
            hboost::detail::winapi::QueryPerformanceCounter( &ts );
...
#if defined(HBOOST_WINDOWS)
    hboost::detail::winapi::HCRYPTPROV_ random_;
#else
    std::FILE * random_;
#endif
};

// almost a copy of hboost::generator_iterator
...
template <>
inline void seed<hboost::random::random_device>(hboost::random::random_device&) {}

// random_device does not / can not be seeded
template <>
inline void seed<seed_rng>(seed_rng&) {}

}}} //namespace hboost::uuids::detail
