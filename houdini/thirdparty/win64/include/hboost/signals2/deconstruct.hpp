#ifndef HBOOST_SIGNALS2_DECONSTRUCT_HPP
#define HBOOST_SIGNALS2_DECONSTRUCT_HPP
...
public:
    ...
    const shared_ptr<T>& postconstruct() const
    {
        if(!_postconstructed)
        {
            adl_postconstruct(_sp, const_cast<typename hboost::remove_const<T>::type *>(_sp.get()));
...
private:
    friend class hboost::signals2::deconstruct_access;
...
private:
    ...
    void destroy()
    {
        if( initialized_ )
        {
            ...
            using hboost::signals2::detail::adl_predestruct;
            adl_predestruct(const_cast<typename hboost::remove_const<T>::type *>(p));
            p->~T();
            initialized_ = false;
        }
    }
...
public:
    ...
    {
        hboost::shared_ptr< T > pt( static_cast< T* >( 0 ), detail::deconstruct_deleter< T >() );

        detail::deconstruct_deleter< T > * pd = hboost::get_deleter< detail::deconstruct_deleter< T > >( pt );
        ...
        hboost::shared_ptr< T > retval( pt, static_cast< T* >( pv ) );
        hboost::detail::sp_enable_shared_from_this(&retval, retval.get(), retval.get());
        return retval;
...
#endif // #ifndef HBOOST_SIGNALS2_DECONSTRUCT_HPP
