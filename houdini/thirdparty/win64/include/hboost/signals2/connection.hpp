/*
  hboost::signals2::connection provides a handle to a signal/slot connection.
...
#ifndef HBOOST_SIGNALS2_CONNECTION_HPP
#define HBOOST_SIGNALS2_CONNECTION_HPP

#include <hboost/function.hpp>
#include <hboost/mpl/bool.hpp>
#include <hboost/noncopyable.hpp>
#include <hboost/shared_ptr.hpp>
#include <hboost/signals2/detail/auto_buffer.hpp>
#include <hboost/signals2/detail/null_output_iterator.hpp>
  ...
  namespace signals2
  {
    ...
    namespace detail
    {
      ...
      public:
        ...
        {
          HBOOST_ASSERT(m_slot_refcount != 0);
          ++m_slot_refcount;
        }
      ...
      public:
        ...
        connection_body(const SlotType &slot_in, const hboost::shared_ptr<mutex_type> &signal_mutex):
      ...
      private:
        mutable hboost::shared_ptr<SlotType> m_slot;
    ...
    public:
      ...
      {}
      connection(const hboost::weak_ptr<detail::connection_body_base> &connectionBody):
      ...
      {
        hboost::shared_ptr<detail::connection_body_base> connectionBody(_weak_connection_body.lock());
      ...
      {
        hboost::shared_ptr<detail::connection_body_base> connectionBody(_weak_connection_body.lock());
        hboost::shared_ptr<detail::connection_body_base> otherConnectionBody(other._weak_connection_body.lock());
    ...
    protected:
      ...
      hboost::weak_ptr<detail::connection_body_base> _weak_connection_body;
    };
...
#endif  // HBOOST_SIGNALS2_CONNECTION_HPP
