...
#ifndef HBOOST_SIGNALS2_SLOT_GROUPS_HPP
#define HBOOST_SIGNALS2_SLOT_GROUPS_HPP
...
namespace hboost {
  namespace signals2 {
    namespace detail {
      ...
      struct group_key
      {
        typedef std::pair<enum slot_meta_group, hboost::optional<Group> > type;
      };
      ...
      public:
          ...
          _group_map(other._group_map), _group_key_compare(other._group_key_compare)
        {
          ...
          {
            HBOOST_ASSERT(this_map_it != _group_map.end());
        ...
        {
          HBOOST_ASSERT(it != _list.end());
          map_iterator map_it = _group_map.lower_bound(key);
          HBOOST_ASSERT(map_it != _group_map.end());
          HBOOST_ASSERT(weakly_equivalent(map_it->first, key));
          if(map_it->second == it)
          {
            iterator next = it;
            ++next;
...
#endif // HBOOST_SIGNALS2_SLOT_GROUPS_HPP
