...
#ifndef HBOOST_SIGNALS2_DETAIL_AUTO_BUFFER_HPP_25_02_2009
#define HBOOST_SIGNALS2_DETAIL_AUTO_BUFFER_HPP_25_02_2009
...
namespace detail
{
    ...
    {
        HBOOST_STATIC_CONSTANT( unsigned, value = N );
    };
    ...
    namespace auto_buffer_detail
    {
        ...
        {
            HBOOST_STATIC_CONSTANT( unsigned, value = Policy::value * sizeof(T) );
        };
        ...
        {
            HBOOST_STATIC_CONSTANT( unsigned, value = Policy::value );
        };

        template< unsigned N, class T >
        struct compute_buffer_objects< store_n_bytes<N>, T >
        {
            HBOOST_STATIC_CONSTANT( unsigned, value = N / sizeof(T) );
        };
    }
    ...
    private:
        ...
        HBOOST_STATIC_CONSTANT( bool, is_stack_buffer_empty = N == 0u );
    ...
    public:
        ...
        typedef typename hboost::mpl::if_c< hboost::has_trivial_assign<T>::value
    ...
    private:
        ...
        {
            copy_rai( begin, end, where, hboost::has_trivial_assign<T>() );
        }

        static void copy_rai( const T* begin, const T* end,
                              pointer where, const hboost::true_type& )
        {
        ...
        static void copy_rai( I begin, I end,
                              pointer where, const hboost::integral_constant<bool, b>& )
        {
        ...
        {
            assign_impl( begin, end, where, hboost::has_trivial_assign<T>() );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const hboost::true_type& )
        {
            std::memcpy( where, begin, sizeof(T) * std::distance(begin,end) );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const hboost::false_type& )
        {
            for( ; begin != end; ++begin, ++where )
                *where = *begin;
        }

        void unchecked_push_back_n( size_type n, const hboost::true_type& )
        {
            std::uninitialized_fill( end(), end() + n, T() );
            size_ += n;
        }

        void unchecked_push_back_n( size_type n, const hboost::false_type& )
        {
            for( size_type i = 0u; i < n; ++i )
                unchecked_push_back();
        }

        void auto_buffer_destroy( pointer where, const hboost::false_type& )
        {
            (*where).~T();
        }

        void auto_buffer_destroy( pointer, const hboost::true_type& )
        { }

        void auto_buffer_destroy( pointer where )
        {
            auto_buffer_destroy( where, hboost::has_trivial_destructor<T>() );
        }

        void destroy_back_n( size_type n, const hboost::false_type& )
        {
            HBOOST_ASSERT( n > 0 );
        ...
        void destroy_back_n( size_type, const hboost::true_type& )
        { }

        void destroy_back_n( size_type n )
        {
            destroy_back_n( n, hboost::has_trivial_destructor<T>() );
        }

        void auto_buffer_destroy( const hboost::false_type& x )
        {
            if( size_ )
        ...
        void auto_buffer_destroy( const hboost::true_type& )
        {
            deallocate( buffer_, members_.capacity_ );
        }

        pointer move_to_new_buffer( size_type new_capacity, const hboost::false_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
            hboost::multi_index::detail::scope_guard guard =
                hboost::multi_index::detail::make_obj_guard( *this,
        ...
        pointer move_to_new_buffer( size_type new_capacity, const hboost::true_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
        ...
        void reserve_impl( size_type new_capacity )
        {
            pointer new_buffer = move_to_new_buffer( new_capacity,
                                                 hboost::has_nothrow_copy<T>() );
            (*this).~auto_buffer();
            buffer_   = new_buffer;
            members_.capacity_ = new_capacity;
            HBOOST_ASSERT( size_ <= members_.capacity_ );
        }

        size_type new_capacity_impl( size_type n )
        {
            HBOOST_ASSERT( n > members_.capacity_ );
        ...
        static void swap_helper( auto_buffer& l, auto_buffer& r,
                                 const hboost::true_type& )
        {
            HBOOST_ASSERT( l.is_on_stack() && r.is_on_stack() );

            auto_buffer temp( l.begin(), l.end() );
            assign_impl( r.begin(), r.end(), l.begin() );
            assign_impl( temp.begin(), temp.end(), r.begin() );
            hboost::swap( l.size_, r.size_ );
            hboost::swap( l.members_.capacity_, r.members_.capacity_ );
        }

        static void swap_helper( auto_buffer& l, auto_buffer& r,
                                 const hboost::false_type& )
        {
            HBOOST_ASSERT( l.is_on_stack() && r.is_on_stack() );
            ...
            size_type i = 0u;
            for(  ; i < min_size; ++i )
                hboost::swap( (*smallest)[i], (*largest)[i] );
        ...
        void grow_back( size_type n, const hboost::true_type& )
        {
            HBOOST_ASSERT( size_ + n <= members_.capacity_ );
            size_ += n;
        }

        void grow_back( size_type n, const hboost::false_type& )
        {
        ...
        {
            grow_back( n, hboost::has_trivial_constructor<T>() );
        }

        void grow_back_one( const hboost::true_type& )
        {
            HBOOST_ASSERT( size_ + 1 <= members_.capacity_ );
            size_ += 1;
        }

        void grow_back_one( const hboost::false_type& )
        {
            unchecked_push_back();
        }
        ...
        void grow_back_one()
        {
            grow_back_one( hboost::has_trivial_constructor<T>() );
        }
    ...
    public:
        ...
        ~auto_buffer()
        {
            ...
            if( buffer_ ) // do we need this check? Yes, but only
                ...
                auto_buffer_destroy( hboost::has_trivial_destructor<T>() );
        }
    ...
    public:
        ...
        {
            HBOOST_ASSERT( size_ + n <= members_.capacity_ );
            unchecked_push_back_n( n, hboost::has_trivial_assign<T>() );
        }
        ...
        {
            HBOOST_ASSERT( size_ + std::distance(begin_arg, end_arg) <= members_.capacity_ );
        ...
        {
            HBOOST_ASSERT( members_.capacity_  >= N );
        ...
        {
            HBOOST_ASSERT( !empty() );
            auto_buffer_destroy( buffer_ + size_ - 1, hboost::has_trivial_destructor<T>() );
            --size_;
        }
        ...
        {
            HBOOST_ASSERT( !empty() );
            HBOOST_ASSERT( cbegin() <= where );
            HBOOST_ASSERT( cend() > where );
            ...
            HBOOST_ASSERT( result <= end() );
            return result;
        }

        iterator erase( const_iterator from, const_iterator to )
        {
            HBOOST_ASSERT( !(std::distance(from,to)>0) ||
                          !empty() );
            HBOOST_ASSERT( cbegin() <= from );
            HBOOST_ASSERT( cend() >= to );

            unsigned elements = std::distance(to,cend());

            if( elements > 0u )
            {
                HBOOST_ASSERT( elements > 0u );
        ...
        void shrink_to_fit()
        {
            ...
            HBOOST_ASSERT( is_on_stack() || size_ == members_.capacity_ );
            HBOOST_ASSERT( !is_on_stack() || size_ <= members_.capacity_ );
        }
        ...
        void swap( auto_buffer& r )
        {
            ...
            if( both_on_heap )
            {
                hboost::swap( get_allocator(), r.get_allocator() );
                hboost::swap( members_.capacity_, r.members_.capacity_ );
                hboost::swap( buffer_, r.buffer_ );
                hboost::swap( size_, r.size_ );
                HBOOST_ASSERT( is_valid() );
                HBOOST_ASSERT( r.is_valid() );
                return;
            }

            HBOOST_ASSERT( on_stack || r_on_stack );
            ...
            if( exactly_one_on_stack )
            {
                ...
                hboost::swap( get_allocator(), r.get_allocator() );  // assume nothrow
                hboost::swap( members_.capacity_, r.members_.capacity_ );
                hboost::swap( size_, r.size_ );
                one_on_stack->buffer_ = other->buffer_;
                other->buffer_        = new_buffer;
                HBOOST_ASSERT( other->is_on_stack() );
                HBOOST_ASSERT( !one_on_stack->is_on_stack() );
                HBOOST_ASSERT( is_valid() );
                HBOOST_ASSERT( r.is_valid() );
                return;
            }
            ...
            HBOOST_ASSERT( on_stack && r_on_stack );
            swap_helper( *this, r, hboost::has_trivial_assign<T>() );
            HBOOST_ASSERT( is_valid() );
            HBOOST_ASSERT( r.is_valid() );
        }

    private:
        typedef hboost::aligned_storage< N * sizeof(T),
                                        hboost::alignment_of<T>::value >
                               storage;
