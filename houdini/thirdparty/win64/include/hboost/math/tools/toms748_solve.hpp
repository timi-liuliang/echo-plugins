...
#ifndef HBOOST_MATH_TOOLS_SOLVE_ROOT_HPP
#define HBOOST_MATH_TOOLS_SOLVE_ROOT_HPP
...
#ifdef HBOOST_MATH_LOG_ROOT_ITERATIONS
#  define HBOOST_MATH_LOGGER_INCLUDE <hboost/math/tools/iteration_logger.hpp>
#  include HBOOST_MATH_LOGGER_INCLUDE
#  undef HBOOST_MATH_LOGGER_INCLUDE
#else
#  define HBOOST_MATH_LOG_COUNT(count)
#endif
...
void bracket(F f, T& a, T& b, T c, T& fa, T& fb, T& d, T& fd)
{
   ...
   // to the root.
   //
   HBOOST_MATH_STD_USING  // For ADL of std math functions
   ...
   //
   if(hboost::math::sign(fa) * hboost::math::sign(fc) < 0)
   {
      d = b;
      fd = fb;
      b = c;
                           ...
                           unsigned count)
{
   ...
   //
   T c;
   if(hboost::math::sign(A) * hboost::math::sign(fa) > 0)
   {
      c = a;
   }
   else
                    ...
                    const T& fd, const T& fe)
{
   ...
   //
   HBOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b
      << " d = " << d << " e = " << e << " fa = " << fa << " fb = " << fb 
      << " fd = " << fd << " fe = " << fe);
   T q11 = (d - e) * fd / (fe - fd);
   T q21 = (b - d) * fb / (fd - fb);
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, hboost::uintmax_t& max_iter, const Policy& pol)
{
   //
   ...
   static const char* function = "hboost::math::tools::toms748_solve<%1%>";
   ...
   b = bx;
   if(a >= b)
      return hboost::math::detail::pair_from_single(policies::raise_domain_error(
         function, 
   ...
   if(hboost::math::sign(fa) * hboost::math::sign(fb) > 0)
      return hboost::math::detail::pair_from_single(policies::raise_domain_error(
         function, 
   ...
   while(count && (fa != 0) && !tol(a, b))
   {
      ...
      {
         c = detail::quadratic_interpolate(a, b, d, fa, fb, fd, 2);
         HBOOST_MATH_INSTRUMENT_CODE("Can't take cubic step!!!!");
      }
      else
      {
      ...
      detail::bracket(f, a, b, c, fa, fb, d, fd);
      HBOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b);
      HBOOST_MATH_INSTRUMENT_CODE(" tol = " << T((fabs(a) - fabs(b)) / fabs(a)));
      if((0 == --count) || (fa == 0) || tol(a, b))
         break;
      //
      ...
      --count;
      HBOOST_MATH_INSTRUMENT_CODE("Not converging: Taking a bisection!!!!");
      HBOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b);
   } // while loop
   ...
   HBOOST_MATH_LOG_COUNT(max_iter)
   return std::make_pair(a, b);
}

template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, hboost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, fax, fbx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, hboost::uintmax_t& max_iter, const Policy& pol)
{
   max_iter -= 2;
...
template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, hboost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
std::pair<T, T> bracket_and_solve_root(F f, const T& guess, T factor, bool rising, Tol tol, hboost::uintmax_t& max_iter, const Policy& pol)
{
   HBOOST_MATH_STD_USING
   static const char* function = "hboost::math::tools::bracket_and_solve_root<%1%>";
   //
   ...
   //
   hboost::uintmax_t count = max_iter - 1;
   ...
   if((fa < 0) == (guess < 0 ? !rising : rising))
   {
      ...
      // until we find it:
      //
      while((hboost::math::sign)(fb) == (hboost::math::sign)(fa))
      {
         if(count == 0)
            return hboost::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", b, pol));
         //
         ...
         fb = f(b);
         --count;
         HBOOST_MATH_INSTRUMENT_CODE("a = " << a << " b = " << b << " fa = " << fa << " fb = " << fb << " count = " << count);
      }
   }
   else
   {
      ...
      while((hboost::math::sign)(fb) == (hboost::math::sign)(fa))
      {
         ...
         if(count == 0)
            return hboost::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", a, pol));
         //
         // Heuristic: normally it's best not to increase the step sizes as we'll just end up
         // with a really wide range to search for the root.  However, if the initial guess was *really*
         // bad then we need to speed up the search otherwise we'll take forever if we're orders of
   ...
   std::pair<T, T> r = toms748_solve(
      ...
      pol);
   max_iter += count;
   HBOOST_MATH_INSTRUMENT_CODE("max_iter = " << max_iter << " count = " << count);
   HBOOST_MATH_LOG_COUNT(max_iter)
   return r;
}

template <class F, class T, class Tol>
inline std::pair<T, T> bracket_and_solve_root(F f, const T& guess, const T& factor, bool rising, Tol tol, hboost::uintmax_t& max_iter)
{
...
#endif // HBOOST_MATH_TOOLS_SOLVE_ROOT_HPP
