...
#ifndef HBOOST_MATH_SF_CBRT_HPP
#define HBOOST_MATH_SF_CBRT_HPP
...
#include <hboost/mpl/divides.hpp>
...
struct big_int_type
{
   operator hboost::uintmax_t()const;
};
...
struct largest_cbrt_int_type
{
   typedef typename mpl::if_<
      hboost::is_convertible<big_int_type, T>,
      hboost::uintmax_t,
      unsigned int
   >::type type;
};
...
T cbrt_imp(T z, const Policy& pol)
{
   ...
   if(!(hboost::math::isfinite)(z))
   {
      return policies::raise_domain_error("hboost::math::cbrt<%1%>(%1%)", "Argument to function must be finite but got %1%.", z, pol);
   }
   ...
   HBOOST_STATIC_ASSERT( ::std::numeric_limits<shift_type>::radix == 2);
