...
#ifndef HBOOST_MATH_BERNOULLI_DETAIL_HPP
#define HBOOST_MATH_BERNOULLI_DETAIL_HPP

#include <hboost/config.hpp>
#include <hboost/detail/lightweight_mutex.hpp>
...
#  include <atomic>
#  define HBOOST_MATH_ATOMIC_NS std
...
typedef hboost::atomic<int> atomic_counter_type;
typedef int atomic_integer_type;
#elif HBOOST_ATOMIC_SHORT_LOCK_FREE == 2
typedef hboost::atomic<short> atomic_counter_type;
typedef short atomic_integer_type;
#elif HBOOST_ATOMIC_LONG_LOCK_FREE == 2
typedef hboost::atomic<long> atomic_counter_type;
typedef long atomic_integer_type;
#elif HBOOST_ATOMIC_LLONG_LOCK_FREE == 2
typedef hboost::atomic<long long> atomic_counter_type;
typedef long long atomic_integer_type;
#else
#  define HBOOST_MATH_NO_ATOMIC_INT
#endif
...
#endif  // HBOOST_NO_CXX11_HDR_ATOMIC

#endif // HBOOST_HAS_THREADS
...
T b2n_asymptotic(int n)
{
   ...
   const T approximate_log_of_bernoulli_bn = 
        ((hboost::math::constants::half<T>() + nx) * log(nx))
        + ((hboost::math::constants::half<T>() - nx) * log(hboost::math::constants::pi<T>()))
        + (((T(3) / 2) - nx) * hboost::math::constants::ln_two<T>())
        + ((nx * (T(2) - (nx2 * 7) * (1 + ((nx2 * 30) * ((nx2 * 12) - 1))))) / (((nx2 * nx2) * nx2) * 2520));
   return ((n / 2) & 1 ? 1 : -1) * (approximate_log_of_bernoulli_bn > tools::log_max_value<T>() 
      ? policies::raise_overflow_error<T>("hboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, nx, Policy())
...
T t2n_asymptotic(int n)
{
   ...
   if(tools::max_value<T>() / p2 < t2n)
      return policies::raise_overflow_error<T>("hboost::math::tangent_t2n<%1%>(std::size_t)", 0, T(n), Policy());
   t2n *= p2;
   p2 -= 1;
   if(tools::max_value<T>() / p2 < t2n)
      return policies::raise_overflow_error<T>("hboost::math::tangent_t2n<%1%>(std::size_t)", 0, Policy());
   t2n *= p2;
   return t2n;
}
//
...
struct max_bernoulli_root_functor
{
   ...
   double operator()(double n)
   {
      ...
      const double approximate_log_of_bernoulli_bn
         =   ((hboost::math::constants::half<double>() + n) * log(n))
           + ((hboost::math::constants::half<double>() - n) * log(hboost::math::constants::pi<double>()))
           + (((double(3) / 2) - n) * hboost::math::constants::ln_two<double>())
...
{
   long long t = lltrunc(hboost::math::tools::log_max_value<T>());
   max_bernoulli_root_functor fun(t);
   hboost::math::tools::equal_floor tol;
   hboost::uintmax_t max_iter = hboost::math::policies::get_max_root_iterations<Policy>();
   return static_cast<std::size_t>(hboost::math::tools::toms748_solve(fun, sqrt(double(t)), double(t), tol, max_iter).first) / 2;
}
...
struct bernoulli_initializer
{
   struct init
   {
      init()
      {
         ...
         //
         hboost::math::bernoulli_b2n<T>(2, Policy());
#ifndef HBOOST_NO_EXCEPTIONS
         try{
#endif
            hboost::math::bernoulli_b2n<T>(max_bernoulli_b2n<T>::value + 1, Policy());
#ifndef HBOOST_NO_EXCEPTIONS
         } catch(const std::overflow_error&){}
#endif
         hboost::math::tangent_t2n<T>(2, Policy());
      }
...
// hboost::container::static_vector here, but that allocates on the stack, which may well
...
struct fixed_vector : private std::allocator<T>
{
   ...
   T& operator[](unsigned n) { HBOOST_ASSERT(n < m_used); return m_data[n]; }
   const T& operator[](unsigned n)const { HBOOST_ASSERT(n < m_used); return m_data[n]; }
   ...
   void resize(unsigned n, const T& val)
   {
      if(n > m_capacity)
      {
         HBOOST_THROW_EXCEPTION(std::runtime_error("Exhausted storage for Bernoulli numbers."));
      }
...
public:
...
#if defined(HBOOST_HAS_THREADS) && !defined(HBOOST_MATH_NO_ATOMIC_INT)
      , m_counter(0)
#endif
   {}
   ...
   void tangent(std::size_t m)
   {
      ...
      HBOOST_MATH_INSTRUMENT_VARIABLE(min_overflow_index);
      ...
      if(prev_size == 0)
      {
         ...
         HBOOST_MATH_INSTRUMENT_VARIABLE(tn[0]);
         HBOOST_MATH_INSTRUMENT_VARIABLE(tn[1]);
      }

      for(std::size_t i = std::max<size_t>(2, prev_size); i < m; i++)
      {
         bool overflow_check = false;
         if(i >= min_overflow_index && (hboost::math::tools::max_value<T>() / (i-1) < m_intermediates[1]) )
         {
            std::fill(tn.begin() + i, tn.end(), hboost::math::tools::max_value<T>());
            break;
         }
         ...
         for(std::size_t j = 2; j <= i; j++)
         {
            overflow_check =
                  ...
                  (hboost::math::tools::max_value<T>() / (i - j) < m_intermediates[j])
                  || (hboost::math::tools::max_value<T>() / (i - j + 2) < m_intermediates[j-1])
                  || (hboost::math::tools::max_value<T>() - m_intermediates[j] * (i - j) < m_intermediates[j-1] * (i - j + 2))
                  || ((hboost::math::isinf)(m_intermediates[j]))
                );
         ...
         HBOOST_MATH_INSTRUMENT_VARIABLE(i);
         HBOOST_MATH_INSTRUMENT_VARIABLE(tn[static_cast<typename container_type::size_type>(i)]);
      }
   }
   ...
   OutputIterator copy_bernoulli_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      //
      // 1) There are no threads (HBOOST_HAS_THREADS is not defined).
      ...
      if(start + n > bn.capacity())
      {
         ...
         for(; n; ++start, --n)
         {
            *out = policies::raise_overflow_error<T>("hboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(start), pol);
            ++out;
         }
         return out;
      }
      ...
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("hboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[i];
         ++out;
      }
   #elif defined(HBOOST_MATH_NO_ATOMIC_INT)
      //
      // We need to grab a mutex every time we get here, for both readers and writers:
      //
      hboost::detail::lightweight_mutex::scoped_lock l(m_mutex);
      if(start + n >= bn.size())
      {
         std::size_t new_size = (std::min)((std::max)((std::max)(std::size_t(start + n), std::size_t(bn.size() + 20)), std::size_t(50)), std::size_t(bn.capacity()));
         tangent_numbers_series(new_size);
      ...
      //
      if(static_cast<std::size_t>(m_counter.load(HBOOST_MATH_ATOMIC_NS::memory_order_consume)) < start + n)
      {
         hboost::detail::lightweight_mutex::scoped_lock l(m_mutex);
         ...
         if(static_cast<std::size_t>(m_counter.load(HBOOST_MATH_ATOMIC_NS::memory_order_consume)) < start + n)
         {
            ...
            m_counter.store(static_cast<atomic_integer_type>(bn.size()), HBOOST_MATH_ATOMIC_NS::memory_order_release);
         }
      }

      for(std::size_t i = (std::max)(static_cast<std::size_t>(max_bernoulli_b2n<T>::value + 1), start); i < start + n; ++i)
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("hboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[static_cast<typename container_type::size_type>(i)];
         ++out;
      }
   ...
   OutputIterator copy_tangent_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      for(std::size_t i = start; i < start + n; ++i)
      {
         if(i >= m_overflow_limit)
            *out = policies::raise_overflow_error<T>("hboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol);
         else
         {
...
#if !defined(HBOOST_HAS_THREADS)
#elif defined(HBOOST_MATH_NO_ATOMIC_INT)
   hboost::detail::lightweight_mutex m_mutex;
#else
   hboost::detail::lightweight_mutex m_mutex;
...
#endif // HBOOST_MATH_BERNOULLI_DETAIL_HPP
