...
#ifndef HBOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
#define HBOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
...
inline T didonato_FN(T p, T a, T x, unsigned N, T tolerance, const Policy& pol)
{
   ...
   //
   HBOOST_MATH_STD_USING
   T u = log(p) + hboost::math::lgamma(a + 1, pol);
...
T find_inverse_gamma(T a, T p, T q, const Policy& pol, bool* p_has_10_digits)
{
   ...
   *p_has_10_digits = false;
   ...
   {
      T g = hboost::math::tgamma(a, pol);
      T b = q * g;
      HBOOST_MATH_INSTRUMENT_VARIABLE(g);
      HBOOST_MATH_INSTRUMENT_VARIABLE(b);
      if((b > 0.6) || ((b >= 0.45) && (a >= 0.3)))
      {
         ...
         {
            u = pow(p * g * a, 1 / a);
            HBOOST_MATH_INSTRUMENT_VARIABLE(u);
         }
         else
         {
   ...
   else
   {
      ...
      HBOOST_MATH_INSTRUMENT_VARIABLE(s);
      ...
      HBOOST_MATH_INSTRUMENT_VARIABLE(ra);
      ...
      HBOOST_MATH_INSTRUMENT_VARIABLE(w);
      ...
      else
      {
         ...
         if(w < 0.15f * ap1)
         {
            ...
            T v = log(p) + hboost::math::lgamma(ap1, pol);
            z = exp((v + w) / a);
            s = hboost::math::log1p(z / ap1 * (1 + z / ap2), pol);
            z = exp((v + z - s) / a);
            s = hboost::math::log1p(z / ap1 * (1 + z / ap2), pol);
            z = exp((v + z - s) / a);
            s = hboost::math::log1p(z / ap1 * (1 + z / ap2 * (1 + z / (a + 3))), pol);
            z = exp((v + z - s) / a);
...
struct gamma_p_inverse_func
{
   ...
   hboost::math::tuple<T, T, T> operator()(const T& x)const
   {
      ...
      HBOOST_MATH_STD_USING  // For ADL of std functions.

      T f, f1;
      value_type ft;
      f = static_cast<T>(hboost::math::detail::gamma_incomplete_imp(
      ...
      return hboost::math::make_tuple(static_cast<T>(f - p), f1, f2);
   }
private:
   T a, p;
   bool invert;
...
T gamma_p_inv_imp(T a, T p, const Policy& pol)
{
   ...
   static const char* function = "hboost::math::gamma_p_inv<%1%>(%1%, %1%)";

   HBOOST_MATH_INSTRUMENT_VARIABLE(a);
   HBOOST_MATH_INSTRUMENT_VARIABLE(p);
   ...
   HBOOST_MATH_INSTRUMENT_VARIABLE(guess);
   //
...
T gamma_q_inv_imp(T a, T q, const Policy& pol)
{
   ...
   static const char* function = "hboost::math::gamma_q_inv<%1%>(%1%, %1%)";
...
#endif // HBOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
