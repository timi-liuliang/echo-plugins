...
namespace hboost { namespace program_options { 
    ...
    extern HBOOST_PROGRAM_OPTIONS_DECL std::string arg;
    ...
    template<class T, class charT>
    void 
    typed_value<T, charT>::notify(const hboost::any& value_store) const
    {
        const T* value = hboost::any_cast<T>(&value_store);
        if (m_store_to) {
            *m_store_to = *value;
        }
        if (m_notifier) {
    ...
    namespace validators {
            ...
            bool allow_empty = false)
        {
            ...
            if (v.size() > 1)
                hboost::throw_exception(validation_error(validation_error::multiple_values_not_allowed));
            else if (v.size() == 1)
                return v.front();
            else if (!allow_empty)
                hboost::throw_exception(validation_error(validation_error::at_least_one_value_required));
            return empty;
        }

        /* Throws multiple_occurrences if 'value' is not empty. */
        HBOOST_PROGRAM_OPTIONS_DECL void 
        check_first_occurrence(const hboost::any& value);
    }
    ...
    */
                  ...
                  T*, long)
    {
        ...
        try {
            ...
            hboost::throw_exception(invalid_option_value(s));
        }
    }

    HBOOST_PROGRAM_OPTIONS_DECL void validate(hboost::any& v, 
    ...
    {
        if (v.empty()) {
            v = hboost::any(std::vector<T>());
        }
        std::vector<T>* tv = hboost::any_cast< std::vector<T> >(&v);
        assert(NULL != tv);
        for (unsigned i = 0; i < s.size(); ++i)
        {
            try {
                ...
                cv.push_back(s[i]);
                validate(a, cv, (T*)0, 0);                
                tv->push_back(hboost::any_cast<T>(a));
            }
            catch(const bad_lexical_cast& /*e*/) {
                hboost::throw_exception(invalid_option_value(s[i]));
            }
        }
    }
    ...
    value()
    {
        ...
        return hboost::program_options::value<T>(0);
    }
