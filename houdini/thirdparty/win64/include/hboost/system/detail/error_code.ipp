...
#include <hboost/config/warning_disable.hpp>
...
# if defined( HBOOST_WINDOWS_API )
#   include <windows.h>
#   if !HBOOST_PLAT_WINDOWS_RUNTIME
#     include <hboost/system/detail/local_free_on_destruction.hpp>
#   endif
...
namespace
{
  ...
  public:
    ...
    const char *   name() const HBOOST_SYSTEM_NOEXCEPT;
  ...
  public:
    ...
    const char *        name() const HBOOST_SYSTEM_NOEXCEPT;
    std::string         message( int ev ) const;
    error_condition     default_error_condition( int ev ) const HBOOST_SYSTEM_NOEXCEPT;
  };

  //  generic_error_category implementation  ---------------------------------//

  const char * generic_error_category::name() const HBOOST_SYSTEM_NOEXCEPT
  {
    return "generic";
  }
  ...
  {
    using namespace hboost::system::errc;
#if defined(__PGI)
      using hboost::system::errc::invalid_argument;
#endif
  ...
  //      thread safe.
  # if defined(HBOOST_WINDOWS_API) || defined(__hpux) || defined(__sun)\
     || (defined(__linux) && (!defined(__USE_XOPEN2K) || defined(HBOOST_SYSTEM_USE_STRERROR)))\
...
#   endif
  ...
  const char * system_error_category::name() const HBOOST_SYSTEM_NOEXCEPT
  {
    return "system";
  }

  error_condition system_error_category::default_error_condition( int ev ) const HBOOST_SYSTEM_NOEXCEPT
  {
    using namespace hboost::system::errc;
#if defined(__PGI)
      using hboost::system::errc::invalid_argument;
...
# else
  ...
  std::string system_error_category::message( int ev ) const
  {
#if defined(UNDER_CE) || HBOOST_PLAT_WINDOWS_RUNTIME || defined(HBOOST_NO_ANSI_APIS)
...
# ifndef HBOOST_SYSTEM_NO_DEPRECATED
    HBOOST_SYSTEM_DECL error_code throws; // "throw on error" special error_code;
...
#   define HBOOST_SYSTEM_LINKAGE inline
# else
#   define HBOOST_SYSTEM_LINKAGE HBOOST_SYSTEM_DECL
# endif

    HBOOST_SYSTEM_LINKAGE const error_category & system_category() HBOOST_SYSTEM_NOEXCEPT
    {
      static const system_error_category  system_category_const;
      return system_category_const;
    }

    HBOOST_SYSTEM_LINKAGE const error_category & generic_category() HBOOST_SYSTEM_NOEXCEPT
    {
