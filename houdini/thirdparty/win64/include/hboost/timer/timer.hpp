...
#ifndef HBOOST_TIMER_TIMER_HPP                  
#define HBOOST_TIMER_TIMER_HPP

#include <hboost/config/warning_disable.hpp>

#include <hboost/timer/config.hpp>
...
namespace timer
{
  ...
  typedef hboost::int_least64_t nanosecond_type;
...
//  cpu_timer  -------------------------------------------------------------------------//
  ...
  class HBOOST_TIMER_DECL cpu_timer
  {
  public:

    //  constructor
    cpu_timer() HBOOST_NOEXCEPT                                   { start(); }

    //  observers
    bool          is_stopped() const HBOOST_NOEXCEPT              { return m_is_stopped; }
    cpu_times     elapsed() const HBOOST_NOEXCEPT;  // does not stop()
    std::string   format(short places, const std::string& format) const
                        { return ::hboost::timer::format(elapsed(), places, format); }
    std::string   format(short places = default_places) const
                        { return ::hboost::timer::format(elapsed(), places); }
    //  actions
    void          start() HBOOST_NOEXCEPT;
    void          stop() HBOOST_NOEXCEPT;
    void          resume() HBOOST_NOEXCEPT; 
...
//  auto_cpu_timer  --------------------------------------------------------------------//
  ...
  class HBOOST_TIMER_DECL auto_cpu_timer : public cpu_timer
  {
  public:
