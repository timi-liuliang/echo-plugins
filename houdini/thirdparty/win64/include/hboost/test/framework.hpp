...
#ifndef HBOOST_TEST_FRAMEWORK_HPP_020805GER
#define HBOOST_TEST_FRAMEWORK_HPP_020805GER
...
/// Different depending on whether HBOOST_TEST_ALTERNATIVE_INIT_API is defined or not
...
HBOOST_TEST_DECL void                init( init_unit_test_func init_func, int argc, char* argv[] );
...
HBOOST_TEST_DECL void                finalize_setup_phase( test_unit_id tu = INV_TEST_UNIT_ID);

/// This function returns true when testing is in progress (setup is finished).
HBOOST_TEST_DECL bool                test_in_progress();
...
HBOOST_TEST_DECL void    shutdown();
/// @}
...
HBOOST_TEST_DECL test_suite&         current_auto_test_suite( test_suite* ts = 0, bool push_or_pop = true );
...
HBOOST_TEST_DECL void                register_test_unit( test_case* tc );
...
HBOOST_TEST_DECL void                register_test_unit( test_suite* ts );
...
HBOOST_TEST_DECL void                deregister_test_unit( test_unit* tu );
...
HBOOST_TEST_DECL void                register_observer( test_observer& to );

/// Excldes the observer object form the framework's list of test observers
/// @param[in]  to  test observer object to exclude
HBOOST_TEST_DECL void                deregister_observer( test_observer& to );
...
/// @{
/// Context accessor
struct HBOOST_TEST_DECL context_generator {
...
HBOOST_TEST_DECL int                 add_context( lazy_ostream const& context_descr, bool sticky );
...
HBOOST_TEST_DECL void                clear_context( int context_id = -1 );
/// Produces an instance of small "delegate" object, which facilitates access to collected context.
HBOOST_TEST_DECL context_generator   get_context();
/// @}
...
HBOOST_TEST_DECL master_test_suite_t& master_test_suite();
...
HBOOST_TEST_DECL test_case const&    current_test_case();
...
HBOOST_TEST_DECL test_unit_id        current_test_case_id(); /* safe version of above */
...
HBOOST_TEST_DECL test_unit&          get( test_unit_id tu_id, test_unit_type tu_type );
...
HBOOST_TEST_DECL void                run( test_unit_id tu = INV_TEST_UNIT_ID, bool continue_test = true );
/// Initiates test execution. Same as other overload
HBOOST_TEST_DECL void                run( test_unit const* tu, bool continue_test = true );
/// @}

/// @name Test events dispatchers
/// @{
/// Reports results of assertion to all test observers
HBOOST_TEST_DECL void                assertion_result( unit_test::assertion_result ar );
/// Reports uncaught exception to all test observers
HBOOST_TEST_DECL void                exception_caught( execution_exception const& );
/// Reports aborted test unit to all test observers
HBOOST_TEST_DECL void                test_unit_aborted( test_unit const& );
/// @}

namespace impl {
// exclusively for self test
HBOOST_TEST_DECL void                setup_for_execution( test_unit const& );
} // namespace impl
...
struct HBOOST_TEST_DECL internal_error : public std::runtime_error {
...
struct HBOOST_TEST_DECL setup_error : public std::runtime_error {
    setup_error( const_string m ) : std::runtime_error( std::string( m.begin(), m.size() ) ) {}
};

#define HBOOST_TEST_SETUP_ASSERT( cond, msg ) HBOOST_TEST_I_ASSRT( cond, unit_test::framework::setup_error( msg ) )
...
#endif // HBOOST_TEST_FRAMEWORK_HPP_020805GER
