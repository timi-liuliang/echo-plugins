...
#ifndef HBOOST_TEST_UTILS_RUNTIME_CLA_PARSER_HPP
#define HBOOST_TEST_UTILS_RUNTIME_CLA_PARSER_HPP

// Boost.Test Runtime parameters
#include <hboost/test/utils/runtime/argument.hpp>
#include <hboost/test/utils/runtime/modifier.hpp>
#include <hboost/test/utils/runtime/parameter.hpp>

#include <hboost/test/utils/runtime/cla/argv_traverser.hpp>

// Boost.Test
#include <hboost/test/utils/foreach.hpp>
#include <hboost/test/utils/algorithm.hpp>
#include <hboost/test/detail/throw_exception.hpp>

#include <hboost/algorithm/cxx11/all_of.hpp> // !! ?? unnecessary after cxx11
...
typedef std::vector<hboost::reference_wrapper<parameter_cla_id const> > param_cla_id_list;
...
struct parameter_trie {
    ...
    {
        HBOOST_TEST_I_ASSRT( !m_has_final_candidate && (!final || m_id_candidates.empty()),
...
public:
...
#endif
    {
        ...
        HBOOST_TEST_I_ASSRT( algorithm::all_of( m_end_of_param_indicator.begin(),
        ...
        HBOOST_TEST_I_ASSRT( algorithm::all_of( m_negation_prefix.begin(),
    ...
    parse( int argc, char** argv, runtime::arguments_store& res )
    {
        ...
        while( !tr.eoi() ) {
            ...
            if( negative_form ) {
                HBOOST_TEST_I_ASSRT( found_id.m_negatable,
            ...
            if( !value_separator.is_empty() || !found_param->p_has_optional_value ) {
                ...
                HBOOST_TEST_I_ASSRT( found_id.m_value_separator == value_separator,
                                    format_error( found_param->p_name ) 
                ...
                HBOOST_TEST_I_ASSRT( !value.is_empty(),
                                    format_error( found_param->p_name )
            ...
            HBOOST_TEST_I_ASSRT( !res.has( found_param->p_name ) || found_param->p_repeatable,
    ...
    help( std::ostream& ostr, parameters_store const& parameters, cstring param_name )
    {
        ...
        HBOOST_TEST_FOREACH( parameters_store::storage_type::value_type const&, v, parameters.all() ) {
...
private:
    ...
    build_trie( parameters_store const& parameters )
    {
        ...
        HBOOST_TEST_FOREACH( parameters_store::storage_type::value_type const&, v, parameters.all() ) {
            ...
            HBOOST_TEST_FOREACH( parameter_cla_id const&, id, param->cla_ids() ) {
    ...
    validate_token_format( cstring token, cstring& prefix, cstring& name, cstring& separator, bool& negative_form )
    {
        ...
        if( name.empty() ) {
            ...
            HBOOST_TEST_I_THROW( format_error() << "Invalid format for an actual argument " << token );
        }
    ...
    locate_parameter( trie_ptr curr_trie, cstring name, cstring token )
    {
        ...
        HBOOST_TEST_FOREACH( char, c, name ) {
            if( curr_trie ) {
                ...
                if( next_trie )
                    ...
                    HBOOST_TEST_FOREACH( trie_per_char::value_type const&, typo_cand, curr_trie->m_subtrie ) {
                        // 'wrong char' typo
                ...
                HBOOST_TEST_FOREACH( trie_ptr, typo_cand, typo_candidates ) {
...
// !! ??            unique_typo_candidate.reserve( typo_candidates.size() );
            ...
            HBOOST_TEST_FOREACH( trie_ptr, trie_cand, typo_candidates ) {
                // avoid ambiguos candidate trie
                if( trie_cand->m_id_candidates.size() > 1 )
                    continue;

                HBOOST_TEST_FOREACH( parameter_cla_id const&, param_cand, trie_cand->m_id_candidates ) {
            ...
            HBOOST_TEST_I_THROW( unrecognized_param( std::move(typo_candidate_names) )
                                << "An unrecognized parameter in the argument "
                                << token );
#else
            HBOOST_TEST_I_THROW( unrecognized_param( typo_candidate_names )
                                << "An unrecognized parameter in the argument "
                                << token );
#endif
        }
        ...
        if( curr_trie->m_id_candidates.size() > 1 ) {
            ...
            HBOOST_TEST_FOREACH( parameter_cla_id const&, param_id, curr_trie->m_id_candidates )
                amb_names.push_back( param_id.m_tag );

#ifndef HBOOST_NO_CXX11_RVALUE_REFERENCES
            HBOOST_TEST_I_THROW( ambiguous_param( std::move( amb_names ) )
                                << "An ambiguous parameter name in the argument " << token );
#else
            HBOOST_TEST_I_THROW( ambiguous_param( amb_names )
                                << "An ambiguous parameter name in the argument " << token );
#endif
        }
...
#endif // HBOOST_TEST_UTILS_RUNTIME_CLA_PARSER_HPP
