...
#ifndef HBOOST_TEST_UTILS_RUNTIME_PARAMETER_HPP
#define HBOOST_TEST_UTILS_RUNTIME_PARAMETER_HPP

// Boost.Test Runtime parameters
#include <hboost/test/utils/runtime/fwd.hpp>
#include <hboost/test/utils/runtime/modifier.hpp>
#include <hboost/test/utils/runtime/argument.hpp>
#include <hboost/test/utils/runtime/argument_factory.hpp>
...
#include <hboost/algorithm/cxx11/all_of.hpp>
...
struct parameter_cla_id {
    ...
    , m_negatable( negatable )
    {
        ...
        HBOOST_TEST_I_ASSRT( algorithm::all_of( m_prefix.begin(), m_prefix.end(), valid_prefix_char ),
                            invalid_cla_id() << "Parameter " << m_tag
                                             << " has invalid characters in prefix." );

        HBOOST_TEST_I_ASSRT( algorithm::all_of( m_tag.begin(), m_tag.end(), valid_name_char ),
                            invalid_cla_id() << "Parameter " << m_tag
                                             << " has invalid characters in name." );

        HBOOST_TEST_I_ASSRT( algorithm::all_of( m_value_separator.begin(), m_value_separator.end(), valid_separator_char ),
                            invalid_cla_id() << "Parameter " << m_tag
...
public:
    ...
    virtual void            usage( std::ostream& ostr, cstring negation_prefix_ )
    {
        ...
        HBOOST_TEST_FOREACH( parameter_cla_id const&, id, cla_ids() ) {
...
protected:
    ...
    {
        HBOOST_TEST_I_ASSRT( !tag.is_empty(),
                            invalid_cla_id() << "Parameter can't have an empty name." );

        HBOOST_TEST_I_ASSRT( !prefix.is_empty(),
                            invalid_cla_id() << "Parameter " << tag
                                             << " can't have an empty prefix." );

        HBOOST_TEST_I_ASSRT( !value_separator.is_empty(),
                            invalid_cla_id() << "Parameter " << tag
        ...
        HBOOST_TEST_I_ASSRT( !validate_value_separator || !value_separator.is_empty() || !p_has_optional_value,
                            invalid_cla_id() << "Parameter " << tag
...
public:
    ...
    , m_arg_factory( m )
    {
        HBOOST_TEST_I_ASSRT( !m.has( default_value ) || a == runtime::OPTIONAL_PARAM,
                            invalid_param_spec() << "Parameter " << name
                                                 << " is not optional and can't have default_value." );

        HBOOST_TEST_I_ASSRT( !m.has( optional_value ) || !this->p_repeatable,
                            invalid_param_spec() << "Parameter " << name
...
private:
    ...
    virtual void    produce_argument( cstring token, bool negative_form, arguments_store& store ) const
    {
        if( token.empty() )
            ...
            HBOOST_TEST_I_ASSRT( !negative_form,
...
private:
    ...
    virtual void    value_help( std::ostream& ostr ) const
    {
        if( this->p_value_hint.empty() ) {
            ostr << "<";
            bool first = true;
            HBOOST_TEST_FOREACH( cstring, name, m_valid_names ) {
                if( first )
                    first = false;
                else
                    ostr << '|';
...
public:
    ...
    void                    add( basic_param const& in )
    {
        ...
        HBOOST_TEST_I_ASSRT( m_parameters.insert( std::make_pair( cstring(p->p_name), p ) ).second,
    ...
    basic_param_ptr         get( cstring name ) const
    {
        ...
        HBOOST_TEST_I_ASSRT( found != m_parameters.end(),
...
#endif // HBOOST_TEST_UTILS_RUNTIME_PARAMETER_HPP
