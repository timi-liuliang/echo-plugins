...
#ifndef HBOOST_TEST_UTILS_XML_PRINTER_HPP
#define HBOOST_TEST_UTILS_XML_PRINTER_HPP
...
print_escaped( std::ostream& where_to, const_string value )
{
    ...
    HBOOST_TEST_FOREACH( char, c, value ) {
...
print_escaped_cdata( std::ostream& where_to, const_string value )
{
    ...
    if( pos == const_string::npos )
        ...
        where_to << value.substr( 0, pos+2 ) << cdata_end
                 << HBOOST_TEST_L( "<![CDATA[" ) << value.substr( pos+2 );
    }
}
...
{
    *p << HBOOST_TEST_L( "<![CDATA[" );
    print_escaped_cdata( *p, value );
    return  *p << HBOOST_TEST_L( "]]>" );
}
...
#endif // HBOOST_TEST_UTILS_XML_PRINTER_HPP
