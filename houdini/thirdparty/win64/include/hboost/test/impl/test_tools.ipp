...
#ifndef HBOOST_TEST_TEST_TOOLS_IPP_012205GER
#define HBOOST_TEST_TEST_TOOLS_IPP_012205GER

// Boost.Test
#include <hboost/test/unit_test_log.hpp>
#include <hboost/test/tools/context.hpp>
#include <hboost/test/tools/output_test_stream.hpp>
...
#include <hboost/test/execution_monitor.hpp> // execution_aborted
...
using ::hboost::unit_test::lazy_ostream;
                  ...
                  std::size_t               num_args, ... )
{
    ...
    HBOOST_TEST_I_ASSRT( framework::current_test_case_id() != INV_TEST_UNIT_ID,
    ...
    case REQUIRE:
        ...
        HBOOST_TEST_I_THROW( execution_aborted() );
    }
...
#endif // !defined( HBOOST_NO_CWCHAR )
...
context_frame::context_frame( ::hboost::unit_test::lazy_ostream const& context_descr )
...
: m_pimpl( new Impl )
{
    if( !pattern_file_name.is_empty() ) {
        ...
        if( !m_pimpl->m_pattern.is_open() )
            HBOOST_TEST_MESSAGE( "Can't open pattern file " << pattern_file_name << " for " << (match_or_save ? "reading" : "writing") );
    }
...
#endif // HBOOST_TEST_TEST_TOOLS_IPP_012205GER
