...
#ifndef HBOOST_TEST_FRAMEWORK_IPP_021005GER
#define HBOOST_TEST_FRAMEWORK_IPP_021005GER
...
#include <hboost/test/progress_monitor.hpp>
...
#include <hboost/test/tree/test_case_counter.hpp>
...
assign_sibling_rank( test_unit_id tu_id, order_info_per_tu& tuoi )
{
    ...
    HBOOST_TEST_SETUP_ASSERT( tu.p_sibling_rank != (std::numeric_limits<counter_t>::max)(),
    ...
    HBOOST_TEST_FOREACH( test_unit_id, sibling_id, info.dependant_siblings )
...
invoke_init_func( init_unit_test_func init_func )
{
    ...
    HBOOST_TEST_I_ASSRT( (*init_func)(), std::runtime_error( "test module initialization failed" ) );
#else
...
public:
    ...
    {
#ifdef HBOOST_TEST_SUPPORT_TOKEN_ITERATOR
...
private:
    bool            filter_unit( test_unit const& tu )
    {
        ...
        // look for match
        return std::find_if( filters.begin(), filters.end(), bind( &component::pass, _1, hboost::ref(tu) ) ) != filters.end();
    }
    ...
    virtual bool    visit( test_unit const& tu )
    {
        ...
        if( m_dep_collector ) {
            HBOOST_TEST_FOREACH( test_unit_id, dep_id, tu.p_dependencies.get() ) {
                ...
                HBOOST_TEST_MESSAGE( "Including test " << dep.p_type_name << ' ' << dep.full_name() <<
...
parse_filters( test_unit_id master_tu_id, test_unit_id_list& tu_to_enable, test_unit_id_list& tu_to_disable )
{
    ...
    HBOOST_TEST_FOREACH( const_string, filter, filters ) {
        HBOOST_TEST_SETUP_ASSERT( !filter.is_empty(), "Invalid filter specification" );
...
public:
    ...
    void       deduce_siblings_order( test_unit_id tu_id, test_unit_id master_tu_id, impl::order_info_per_tu& tuoi )
    {
        ...
        HBOOST_TEST_FOREACH( test_unit_id, chld_id, ts.m_children )
    ...
    bool            finalize_default_run_status( test_unit_id tu_id, test_unit::run_status parent_status )
    {
        ...
        if( tu.p_type == TUT_SUITE ) {
            ...
            HBOOST_TEST_FOREACH( test_unit_id, chld_id, static_cast<test_suite const&>(tu).m_children )
                                        ...
                                        random_generator_helper const * const p_random_generator = 0)
    {
        ...
        if( timeout == TIMEOUT_EXCEEDED ) {
            ...
            HBOOST_TEST_FOREACH( test_observer*, to, m_observers )
        ...
        HBOOST_TEST_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
            result = unit_test_monitor.execute_and_translate( hboost::bind( &test_unit_fixture::setup, F ) );
        ...
        if( result == unit_test_monitor_t::test_ok ) {
            ...
            if( tu.p_type == TUT_SUITE ) {
                ...
                if( runtime_config::get<unsigned>( runtime_config::RANDOM_SEED ) == 0 ) {
                    ...
                    HBOOST_TEST_FOREACH( value_type, chld, ts.m_ranked_children ) {
                    ...
                    while( it != ts.m_ranked_children.end() ) {
                        ...
                        HBOOST_TEST_FOREACH( test_unit_id, chld, children_with_the_same_rank ) {
                            unsigned chld_timeout = child_timeout( timeout, tu_timer.elapsed() );
        ...
        if( !unit_test_monitor.is_critical_error( result ) ) {
            ...
            HBOOST_TEST_REVERSE_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
                result = (std::min)( result, unit_test_monitor.execute_and_translate( hboost::bind( &test_unit_fixture::teardown, F ), 0 ) );
        ...
        HBOOST_TEST_REVERSE_FOREACH( test_observer*, to, m_observers )
    ...
    hboost::execution_monitor m_aux_em;
...
init( init_unit_test_func init_func, int argc, char* argv[] )
{
    ...
    HBOOST_TEST_I_TRY {
        s_frk_state().m_aux_em.vexecute( hboost::bind( &impl::invoke_init_func, init_func ) );
    }
    HBOOST_TEST_I_CATCH( execution_exception, ex )  {
        HBOOST_TEST_SETUP_ASSERT( false, ex.what() );
    }
}
...
finalize_setup_phase( test_unit_id master_tu_id )
{
    ...
    private:
        ...
        virtual bool    visit( test_unit const& tu )
        {
            HBOOST_TEST_FOREACH( decorator::base_ptr, d, tu.p_decorators.get() )
...
#  if HBOOST_WORKAROUND(HBOOST_MSVC,  <= 1600 ) && !defined(_DLL) && defined(_DEBUG)
...
{
    HBOOST_TEST_SETUP_ASSERT( tc->p_id == INV_TEST_UNIT_ID, HBOOST_TEST_L( "test case already registered" ) );

    test_unit_id new_id = impl::s_frk_state().m_next_test_case_id;

    HBOOST_TEST_SETUP_ASSERT( new_id != MAX_TEST_CASE_ID, HBOOST_TEST_L( "too many test cases" ) );
...
{
    HBOOST_TEST_SETUP_ASSERT( ts->p_id == INV_TEST_UNIT_ID, HBOOST_TEST_L( "test suite already registered" ) );

    test_unit_id new_id = impl::s_frk_state().m_next_test_suite_id;

    HBOOST_TEST_SETUP_ASSERT( new_id != MAX_TEST_SUITE_ID, HBOOST_TEST_L( "too many test suites" ) );
...
int
add_context( ::hboost::unit_test::lazy_ostream const& context_descr, bool sticky )
{
...
get( test_unit_id id, test_unit_type t )
{
    ...
    HBOOST_TEST_I_ASSRT( (res->p_type & t) != 0, internal_error( "Invalid test unit type" ) );
...
run( test_unit_id id, bool continue_test )
{
    ...
    HBOOST_TEST_SETUP_ASSERT( tcc.p_count != 0 , runtime_config::get<std::vector<std::string> >( runtime_config::RUN_FILTERS ).empty()
        ? HBOOST_TEST_L( "test tree is empty" )
        : HBOOST_TEST_L( "no test cases matching filter or all test cases were disabled" ) );
    ...
    if( call_start_finish ) {
        HBOOST_TEST_FOREACH( test_observer*, to, impl::s_frk_state().m_observers ) {
            HBOOST_TEST_I_TRY {
                impl::s_frk_state().m_aux_em.vexecute( hboost::bind( &test_observer::test_start, to, tcc.p_count ) );
            }
            HBOOST_TEST_I_CATCH( execution_exception, ex ) {
                HBOOST_TEST_SETUP_ASSERT( false, ex.what() );
            }
    ...
    default:
        HBOOST_TEST_MESSAGE( "Test cases order is shuffled using seed: " << seed );
        std::srand( seed );
    }
    ...
    if( call_start_finish ) {
        HBOOST_TEST_REVERSE_FOREACH( test_observer*, to, impl::s_frk_state().m_observers )
            to->test_finish();
    }
...
#endif // HBOOST_TEST_FRAMEWORK_IPP_021005GER
