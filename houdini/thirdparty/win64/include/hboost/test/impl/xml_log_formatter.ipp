...
#ifndef HBOOST_TEST_XML_LOG_FORMATTER_IPP_020105GER
#define HBOOST_TEST_XML_LOG_FORMATTER_IPP_020105GER

// Boost.Test
#include <hboost/test/output/xml_log_formatter.hpp>
...
#include <hboost/test/utils/xml_printer.hpp>
...
{
    ostr  << "<BuildInfo"
            << " platform"  << utils::attr_value() << HBOOST_PLATFORM
            << " compiler"  << utils::attr_value() << HBOOST_COMPILER
            << " stl"       << utils::attr_value() << HBOOST_STDLIB
...
xml_log_formatter::test_unit_start( std::ostream& ostr, test_unit const& tu )
{
    ...
    if( !tu.p_file_name.empty() )
        ostr << HBOOST_TEST_L( " file" ) << utils::attr_value() << tu.p_file_name
             << HBOOST_TEST_L( " line" ) << utils::attr_value() << tu.p_line_num;
...
xml_log_formatter::log_entry_start( std::ostream& ostr, log_entry_data const& entry_data, log_entry_types let )
{
    ...
    ostr << '<' << m_curr_tag
         << HBOOST_TEST_L( " file" ) << utils::attr_value() << entry_data.m_file_name
         << HBOOST_TEST_L( " line" ) << utils::attr_value() << entry_data.m_line_num
         << HBOOST_TEST_L( "><![CDATA[" );
...
xml_log_formatter::log_entry_finish( std::ostream& ostr )
{
    ...
    ostr << HBOOST_TEST_L( "</" ) << m_curr_tag << HBOOST_TEST_L( ">" );
...
{
    ostr << HBOOST_TEST_L( "<Frame>" ) << utils::cdata() << context_descr << HBOOST_TEST_L( "</Frame>" );
}
...
#endif // HBOOST_TEST_XML_LOG_FORMATTER_IPP_020105GER
