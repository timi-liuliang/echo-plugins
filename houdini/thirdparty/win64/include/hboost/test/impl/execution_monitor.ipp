...
#ifndef HBOOST_TEST_EXECUTION_MONITOR_IPP_012205GER
#define HBOOST_TEST_EXECUTION_MONITOR_IPP_012205GER
...
#include <hboost/core/ignore_unused.hpp> // for ignore_unused
#ifndef HBOOST_NO_EXCEPTION
#include <hboost/exception/get_error_info.hpp> // for get_error_info
#include <hboost/exception/current_exception_cast.hpp> // for current_exception_cast
#endif
...
#ifdef HBOOST_SEH_BASED_SIGNAL_HANDLING
#  include <windows.h>
...
#  if defined(UNDER_CE) && HBOOST_WORKAROUND(_MSC_VER,  < 1500 )
...
#    include <crtdbg.h>
#    define HBOOST_TEST_CRT_HOOK_TYPE    _CRT_REPORT_HOOK
#    define HBOOST_TEST_CRT_ASSERT       _CRT_ASSERT
#    define HBOOST_TEST_CRT_ERROR        _CRT_ERROR
#    define HBOOST_TEST_CRT_SET_HOOK(H)  _CrtSetReportHook(H)
#  else
#    define HBOOST_TEST_CRT_HOOK_TYPE    void*
#    define HBOOST_TEST_CRT_ASSERT       2
#    define HBOOST_TEST_CRT_ERROR        1
#    define HBOOST_TEST_CRT_SET_HOOK(H)  (void*)(H)
#  endif

#  if (!HBOOST_WORKAROUND(_MSC_VER,  >= 1400 ) && \
      !defined(HBOOST_COMO)) || defined(UNDER_CE)
...
#  if HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x0564)) || defined(UNDER_CE)

namespace { void _set_se_translator( void* ) {} }

#  endif

#elif defined(HBOOST_HAS_SIGACTION)

#  define HBOOST_SIGACTION_BASED_SIGNAL_HANDLING
...
#      define HBOOST_TEST_LIMITED_SIGNAL_DETAILS
...
#    define HBOOST_TEST_USE_ALT_STACK
#  endif
...
#  if defined(SIGPOLL) && !defined(__CYGWIN__)                              && \
      ...
      !defined(__NetBSD__)                                                  && \
      !defined(__QNXNTO__)
#    define HBOOST_TEST_CATCH_SIGPOLL
#  endif

#  ifdef HBOOST_TEST_USE_ALT_STACK
#    define HBOOST_TEST_ALT_STACK_SIZE SIGSTKSZ
#  endif


#else

#  define HBOOST_NO_SIGNAL_HANDLING
...
#if defined(__GNUC__) && !defined(HBOOST_NO_TYPEID)
#  include <cxxabi.h>
#endif
...
#ifdef __BORLANDC__
#  define HBOOST_TEST_VSNPRINTF( a1, a2, a3, a4 ) std::vsnprintf( (a1), (a2), (a3), (a4) )
#elif HBOOST_WORKAROUND(_MSC_VER, <= 1310) || \
      HBOOST_WORKAROUND(__MWERKS__, HBOOST_TESTED_AT(0x3000)) || \
      defined(UNDER_CE)
#  define HBOOST_TEST_VSNPRINTF( a1, a2, a3, a4 ) _vsnprintf( (a1), (a2), (a3), (a4) )
#else
#  define HBOOST_TEST_VSNPRINTF( a1, a2, a3, a4 ) vsnprintf( (a1), (a2), (a3), (a4) )
#endif
...
extract( hboost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = hboost::get_error_info<ErrorInfo>( *ex );
...
static void
report_error( execution_exception::error_code ec, hboost::exception const* be, char const* format, va_list* args )
{
    static const int REPORT_ERROR_BUFFER_SIZE = 4096;
    static char buf[REPORT_ERROR_BUFFER_SIZE];

    HBOOST_TEST_VSNPRINTF( buf, sizeof(buf)-1, format, *args );
    buf[sizeof(buf)-1] = 0;

    va_end( *args );

    HBOOST_TEST_I_THROW(execution_exception( ec, buf, execution_exception::location( extract<throw_file>( be ),
...
static void
report_error( execution_exception::error_code ec, hboost::exception const* be, char const* format, ... )
{
    va_list args;
...
struct fpe_except_guard {
    ...
    : m_detect_fpe( detect_fpe )
    {
        ...
        m_previosly_enabled = fpe::disable( fpe::HBOOST_FPE_ALL );
        if( m_previosly_enabled != fpe::HBOOST_FPE_INV && detect_fpe != fpe::HBOOST_FPE_OFF )
            fpe::enable( detect_fpe );
    }
    ~fpe_except_guard()
    {
        if( m_detect_fpe != fpe::HBOOST_FPE_OFF )
            fpe::disable( m_detect_fpe );
        if( m_previosly_enabled != fpe::HBOOST_FPE_INV )
...
#if defined(HBOOST_SIGACTION_BASED_SIGNAL_HANDLING)
...
// ************************************************************************** //
// **************    hboost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
system_signal_exception::report() const
{
    ...
    case SIGILL:
        switch( m_sig_info->si_code ) {
#ifndef HBOOST_TEST_LIMITED_SIGNAL_DETAILS
        case ILL_ILLOPC:
...
#if defined(HBOOST_TEST_CATCH_SIGPOLL)
...
// ************************************************************************** //
// **************         hboost::detail::signal_action         ************** //
// ************************************************************************** //

// Forward declaration
extern "C" {
static void hboost_execution_monitor_jumping_signal_handler( int sig, siginfo_t* info, void* context );
static void hboost_execution_monitor_attaching_signal_handler( int sig, siginfo_t* info, void* context );
}
...
, m_installed( install )
{
    ...
    HBOOST_TEST_SYS_ASSERT( ::sigaction( m_sig , sigaction_ptr(), &m_new_action ) != -1 );
    ...
    m_new_action.sa_sigaction  = attach_dbg ? &hboost_execution_monitor_attaching_signal_handler
                                            : &hboost_execution_monitor_jumping_signal_handler;
    HBOOST_TEST_SYS_ASSERT( sigemptyset( &m_new_action.sa_mask ) != -1 );
    ...
    HBOOST_TEST_SYS_ASSERT( ::sigaction( m_sig, &m_new_action, &m_old_action ) != -1 );
}
...
#ifdef HBOOST_TEST_CATCH_SIGPOLL
...
, m_ALRM_action( SIGALRM, timeout > 0        , attach_dbg, alt_stack )
{
    ...
    if( alt_stack ) {
        ...
        HBOOST_TEST_SYS_ASSERT( ::sigaltstack( 0, &sigstk ) != -1 );

        if( sigstk.ss_flags & SS_DISABLE ) {
            sigstk.ss_sp    = alt_stack;
            sigstk.ss_size  = HBOOST_TEST_ALT_STACK_SIZE;
            sigstk.ss_flags = 0;
            HBOOST_TEST_SYS_ASSERT( ::sigaltstack( &sigstk, 0 ) != -1 );
        }
    }
#endif
}
...
static void hboost_execution_monitor_attaching_signal_handler( int sig, siginfo_t* info, void* context )
{
    if( !debug::attach_debugger( false ) )
        hboost_execution_monitor_jumping_signal_handler( sig, info, context );

    // debugger attached; it will handle the signal
    HBOOST_TEST_SYS_ASSERT( ::signal( sig, SIG_DFL ) != SIG_ERR );
}
...
int
execution_monitor::catch_signals( hboost::function<int ()> const& F )
{
    ...
    if( !!p_use_alt_stack && !m_alt_stack )
        m_alt_stack.reset( new char[HBOOST_TEST_ALT_STACK_SIZE] );
#else
    p_use_alt_stack.value = false;
#endif

    signal_handler local_signal_handler( p_catch_system_errors,
                                         p_catch_system_errors || (p_detect_fp_exceptions != fpe::HBOOST_FPE_OFF),
                                         p_timeout,
                                         p_auto_start_dbg,
    ...
    if( !sigsetjmp( signal_handler::jump_buffer(), 1 ) )
        ...
        HBOOST_TEST_I_THROW( local_signal_handler.sys_sig() );
}

//____________________________________________________________________________//

#elif defined(HBOOST_SEH_BASED_SIGNAL_HANDLING)
...
assert_reporting_function( int reportType, char* userMessage, int* )
{
    ...
    if( reportType == HBOOST_TEST_CRT_ASSERT || reportType == HBOOST_TEST_CRT_ERROR )
        detail::report_error( reportType == HBOOST_TEST_CRT_ASSERT ? execution_exception::user_error : execution_exception::system_error, userMessage );
...
execution_monitor::catch_signals( hboost::function<int ()> const& F )
{
    ...
    HBOOST_TEST_CRT_HOOK_TYPE old_crt_hook = 0;

    if( p_catch_system_errors ) {
        old_crt_hook = HBOOST_TEST_CRT_SET_HOOK( &detail::assert_reporting_function );
    ...
    __finally {
        if( l_catch_system_errors ) {
            HBOOST_TEST_CRT_SET_HOOK( old_crt_hook );
...
, p_detect_fp_exceptions( fpe::HBOOST_FPE_OFF )
{}

//____________________________________________________________________________//

int
execution_monitor::execute( hboost::function<int ()> const& F )
{
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<hboost::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
#if HBOOST_WORKAROUND(__BORLANDC__, <= 0x0551)
...
#undef CATCH_AND_REPORT_STD_EXCEPTION
    ...
    catch( hboost::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
#ifdef HBOOST_NO_TYPEID
                              "unknown hboost::exception" ); }
#else
                              typeid(ex).name()          ); }
#endif
...
struct forward {
    explicit    forward( hboost::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    hboost::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( hboost::function<void ()> const& F )
{
...
: m_error_code( ec_ )
, m_what( what_msg_.empty() ? HBOOST_TEST_L( "uncaught exception, system error or abort requested" ) : what_msg_ )
...
enable( unsigned mask )
{
    ...
    ::_controlfp( old_cw & ~mask, HBOOST_FPE_ALL );
#else
    unsigned old_cw;
    if( ::_controlfp_s( &old_cw, 0, 0 ) != 0 )
        return HBOOST_FPE_INV;

    // Set the control word
    if( ::_controlfp_s( 0, old_cw & ~mask, HBOOST_FPE_ALL ) != 0 )
        return HBOOST_FPE_INV;
#endif

    return ~old_cw & HBOOST_FPE_ALL;
#elif defined(__GLIBC__) && defined(__USE_GNU) && !defined(HBOOST_CLANG) && !defined(HBOOST_NO_FENV_H)
    feclearexcept(HBOOST_FPE_ALL);
    int res = feenableexcept( mask );
    return res == -1 ? (unsigned)HBOOST_FPE_INV : (unsigned)res;
#else
    /* Not Implemented  */
    return 0;
#endif
...
#endif // HBOOST_TEST_EXECUTION_MONITOR_IPP_012205GER
