...
#ifndef HBOOST_TEST_RESULTS_COLLECTOR_IPP_021105GER
#define HBOOST_TEST_RESULTS_COLLECTOR_IPP_021105GER
...
results_collector_t::test_unit_finish( test_unit const& tu, unsigned long )
{
    if( tu.p_type == TUT_SUITE ) {
        ...
        if( !num_failures_match )
            HBOOST_TEST_MESSAGE( "Test case " << tu.full_name() << " has fewer failures than expected" );

        bool check_any_assertions = tr.p_aborted || (tr.p_assertions_failed != 0) || (tr.p_assertions_passed != 0);
        if( !check_any_assertions )
            HBOOST_TEST_MESSAGE( "Test case " << tu.full_name() << " did not check any assertions" );
    }
}
...
#endif // HBOOST_TEST_RESULTS_COLLECTOR_IPP_021105GER
