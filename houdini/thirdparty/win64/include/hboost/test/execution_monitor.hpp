...
#ifndef HBOOST_TEST_EXECUTION_MONITOR_HPP_071894GER
#define HBOOST_TEST_EXECUTION_MONITOR_HPP_071894GER
...
///
/// All symbols in the Execution Monitor implementation are located in the namespace hboost. To use the Execution Monitor you need to:
/// -# include @c hboost/test/execution_monitor.hpp
...
typedef hboost::shared_ptr<translator_holder_base> translator_holder_base_ptr;

class HBOOST_TEST_DECL translator_holder_base {
protected:
...
public:
    ...
    virtual int operator()( hboost::function<int ()> const& F ) = 0;
    ...
    translator_holder_base_ptr erase( translator_holder_base_ptr this_, hboost::type<ExceptionType>* = 0 )
    {
        if( m_next )
...
class HBOOST_TEST_DECL execution_exception {
    typedef hboost::unit_test::const_string const_string;
public:
    ...
    struct HBOOST_TEST_DECL location {
...
class HBOOST_TEST_DECL execution_monitor {
    typedef hboost::unit_test::const_string const_string;
public:
    ...
    /// @see vexecute
    int         execute( hboost::function<int ()> const& F );
    ...
    /// @param[in] F  Function to monitor
    /// @see execute
    void         vexecute( hboost::function<void ()> const& F );
    // @}
    ...
    void        register_exception_translator( ExceptionTranslator const& tr, const_string tag = const_string(), hboost::type<ExceptionType>* = 0 );
    ...
    template<typename ExceptionType>
    void        erase_exception_translator( hboost::type<ExceptionType>* = 0 )
    {
...
private:
    ...
    int         catch_signals( hboost::function<int ()> const& F );

    // Data members
    detail::translator_holder_base_ptr  m_custom_translators;
    hboost::scoped_array<char>           m_alt_stack;
}; // execution_monitor
...
public:
    ...
    // translator holder interface
    virtual int operator()( hboost::function<int ()> const& F )
    {
        HBOOST_TEST_I_TRY {
            return m_next ? (*m_next)( F ) : F();
        }
        HBOOST_TEST_I_CATCH( ExceptionType, e ) {
            m_translator( e );
            return hboost::exit_exception_failure;
        }
    }
#ifndef HBOOST_NO_RTTI
...
template<typename ExceptionType, typename ExceptionTranslator>
void
execution_monitor::register_exception_translator( ExceptionTranslator const& tr, const_string tag, hboost::type<ExceptionType>* )
{
...
#define HBOOST_TEST_SYS_ASSERT( cond ) HBOOST_TEST_I_ASSRT( cond, ::hboost::system_error( HBOOST_STRINGIZE( exp ) ) )
...
#ifdef HBOOST_SEH_BASED_SIGNAL_HANDLING
    HBOOST_FPE_DIVBYZERO = EM_ZERODIVIDE,
    HBOOST_FPE_INEXACT   = EM_INEXACT,
    HBOOST_FPE_INVALID   = EM_INVALID,
    HBOOST_FPE_OVERFLOW  = EM_OVERFLOW,
    HBOOST_FPE_UNDERFLOW = EM_UNDERFLOW|EM_DENORMAL,

    HBOOST_FPE_ALL       = MCW_EM,
#elif defined(HBOOST_NO_FENV_H) || defined(HBOOST_CLANG)
    HBOOST_FPE_ALL       = 1,
#else
    HBOOST_FPE_DIVBYZERO = FE_DIVBYZERO,
    HBOOST_FPE_INEXACT   = FE_INEXACT,
    HBOOST_FPE_INVALID   = FE_INVALID,
    HBOOST_FPE_OVERFLOW  = FE_OVERFLOW,
    HBOOST_FPE_UNDERFLOW = FE_UNDERFLOW,

    HBOOST_FPE_ALL       = FE_ALL_EXCEPT,
#endif
    HBOOST_FPE_INV       = HBOOST_FPE_ALL+1
};

//____________________________________________________________________________//

// return the previous set of enabled exceptions when successful, and HBOOST_FPE_INV otherwise
unsigned HBOOST_TEST_DECL enable( unsigned mask );
unsigned HBOOST_TEST_DECL disable( unsigned mask );
