...
#ifndef HBOOST_TEST_TOOLS_OLD_IMPL_HPP_012705GER
#define HBOOST_TEST_TOOLS_OLD_IMPL_HPP_012705GER

// Boost.Test
#include <hboost/test/unit_test_log.hpp>
#include <hboost/test/tools/assertion_result.hpp>
#include <hboost/test/tools/floating_point_comparison.hpp>
...
#include <hboost/numeric/conversion/conversion_traits.hpp> // for numeric::conversion_traits
...
#ifndef HBOOST_TEST_PROD
#define TEMPL_PARAMS( z, m, dummy ) , typename HBOOST_JOIN( Arg, m )

#define FUNC_PARAMS( z, m, dummy )                                                  \
 , HBOOST_JOIN( Arg, m ) const& HBOOST_JOIN( arg, m )                                 \
 , char const* HBOOST_JOIN( HBOOST_JOIN( arg, m ), _descr )                           \
/**/

#define PRED_PARAMS( z, m, dummy ) HBOOST_PP_COMMA_IF( m ) HBOOST_JOIN( arg, m )

#define ARG_INFO( z, m, dummy )                                                     \
 , HBOOST_JOIN( HBOOST_JOIN( arg, m ), _descr )                                       \
 , &static_cast<const unit_test::lazy_ostream&>(unit_test::lazy_ostream::instance() \
        << ::hboost::test_tools::tt_detail::print_helper( HBOOST_JOIN( arg, m ) ))    \
/**/

#define IMPL_FRWD( z, n, dummy )                                                    \
template<typename Pred                                                              \
         HBOOST_PP_REPEAT_ ## z( HBOOST_PP_ADD( n, 1 ), TEMPL_PARAMS, _ )>            \
inline bool                                                                         \
check_frwd( Pred P, unit_test::lazy_ostream const& assertion_descr,                 \
            const_string file_name, std::size_t line_num,                           \
            tool_level tl, check_type ct                                            \
            HBOOST_PP_REPEAT_ ## z( HBOOST_PP_ADD( n, 1 ), FUNC_PARAMS, _ )           \
)                                                                                   \
{                                                                                   \
    return                                                                          \
    report_assertion( P( HBOOST_PP_REPEAT_ ## z(HBOOST_PP_ADD(n, 1), PRED_PARAMS,_) ),\
                assertion_descr, file_name, line_num, tl, ct,                       \
                HBOOST_PP_ADD( n, 1 )                                                \
                HBOOST_PP_REPEAT_ ## z( HBOOST_PP_ADD( n, 1 ), ARG_INFO, _ )          \
    );                                                                              \
}                                                                                   \
/**/

#ifndef HBOOST_TEST_MAX_PREDICATE_ARITY
#define HBOOST_TEST_MAX_PREDICATE_ARITY 5
#endif

HBOOST_PP_REPEAT( HBOOST_TEST_MAX_PREDICATE_ARITY, IMPL_FRWD, _ )
...
#if !defined( HBOOST_NO_CWCHAR )
assertion_result        HBOOST_TEST_DECL equal_impl( wchar_t const* left, wchar_t const* right );
        ...
        for( ; left_begin != left_end && right_begin != right_end; ++left_begin, ++right_begin, ++pos ) {
            if( *left_begin != *right_begin ) {
                ...
                pr.message() << "\nMismatch at position " << pos << ": "
                  << ::hboost::test_tools::tt_detail::print_helper(*left_begin)
                  << " != "
                  << ::hboost::test_tools::tt_detail::print_helper(*right_begin);
            }
        }
...
struct comp_supertype {
    ...
    HBOOST_STATIC_ASSERT_MSG( !is_integral<type>::value, "Only floating-point types can be compared!");
};
...
struct HBOOST_TEST_DECL check_is_close_t {
    // Public typedefs
...
struct HBOOST_TEST_DECL check_is_small_t {
    // Public typedefs
...
#endif // HBOOST_TEST_TOOLS_OLD_IMPL_HPP_012705GER
