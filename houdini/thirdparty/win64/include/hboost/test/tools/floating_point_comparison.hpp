...
#ifndef HBOOST_TEST_FLOATING_POINT_COMPARISON_HPP_071894GER
#define HBOOST_TEST_FLOATING_POINT_COMPARISON_HPP_071894GER
...
template<typename FPT>
struct fpt_limits : hboost::conditional<std::numeric_limits<FPT>::is_specialized,
 ...
 * by the @ref hboost::math::fpc::strength given at construction.
 */
template<typename FPT>
...
public:
    ...
    , m_tested_rel_diff( 0 )
    {
        HBOOST_ASSERT_MSG( m_fraction_tolerance >= FPT(0), "tolerance must not be negative!" ); // no reason for tolerance to be negative
    }
 ...
 *
 * Serves the same purpose as hboost::math::fpc::close_at_tolerance, but used when one
...
public:
    ...
    {
        HBOOST_ASSERT( m_tolerance >= FPT(0) ); // no reason for the tolerance to be negative
    }
...
#endif // HBOOST_FLOATING_POINT_COMAPARISON_HPP_071894GER
