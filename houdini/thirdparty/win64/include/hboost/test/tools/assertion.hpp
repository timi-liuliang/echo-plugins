...
#ifndef HBOOST_TEST_TOOLS_ASSERTION_HPP_100911GER
#define HBOOST_TEST_TOOLS_ASSERTION_HPP_100911GER
...
#define HBOOST_TEST_FOR_EACH_COMP_OP(action) \
    action( < , LT, >= )                    \
    action( <=, LE, >  )                    \
    action( > , GT, <= )                    \
    action( >=, GE, <  )                    \
...
#ifndef HBOOST_NO_CXX11_DECLTYPE

#define HBOOST_TEST_FOR_EACH_CONST_OP(action)\
    action(->*, MEMP, ->* )                 \
                                            \
    action( * , MUL, * )                    \
    action( / , DIV, / )                    \
    ...
    action( >>, RSH, >> )                   \
                                            \
    HBOOST_TEST_FOR_EACH_COMP_OP(action)     \
                                            \
    action( & , BAND, & )                   \
    action( ^ , XOR, ^ )                    \
    action( | , BOR, | )                    \
...
#define HBOOST_TEST_FOR_EACH_MUT_OP(action)  \
    action( = , SET , =  )                  \
    action( +=, IADD, += )                  \
    action( -=, ISUB, -= )                  \
    action( *=, IMUL, *= )                  \
...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(hboost::declval<Lhs>() oper hboost::declval<Rhs>() ) optype; \
    typedef typename hboost::remove_reference<optype>::type              \
/**/
#else
...
HBOOST_TEST_FOR_EACH_CONST_OP( DEFINE_CONST_OPER )
...
#define ADD_OP_SUPPORT( oper, name, _ )                         \
    template<typename T>                                        \
    binary_expr<ExprType,typename hboost::decay<T const>::type,  \
        op::name<ValType,typename hboost::decay<T const>::type> >\
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename hboost::decay<T const>::type Rhs;       \
...
/**/
    ...
    HBOOST_TEST_FOR_EACH_CONST_OP( ADD_OP_SUPPORT )
    #undef ADD_OP_SUPPORT

#ifndef HBOOST_NO_CXX11_AUTO_DECLARATIONS
    // Disabled operators
    template<typename T>
    ExprType&
    operator ||( T const& /*rhs*/ )
    {
        HBOOST_MPL_ASSERT_MSG(false, CANT_USE_LOGICAL_OPERATOR_OR_WITHIN_THIS_TESTING_TOOL, () );
    ...
    operator &&( T const& /*rhs*/ )
    {
        HBOOST_MPL_ASSERT_MSG(false, CANT_USE_LOGICAL_OPERATOR_AND_WITHIN_THIS_TESTING_TOOL, () );
    ...
    operator bool()
    {
        HBOOST_MPL_ASSERT_MSG(false, CANT_USE_TERNARY_OPERATOR_WITHIN_THIS_TESTING_TOOL, () );
...
/**/
    ...
    HBOOST_TEST_FOR_EACH_MUT_OP( ADD_OP_SUPPORT )
#undef ADD_OP_SUPPORT
...
#undef HBOOST_TEST_FOR_EACH_CONST_OP
...
#endif // HBOOST_TEST_TOOLS_ASSERTION_HPP_100911GER
