...
#ifndef HBOOST_MULTI_INDEX_HASHED_INDEX_HPP
#define HBOOST_MULTI_INDEX_HASHED_INDEX_HPP
...
#include <hboost/multi_index/detail/auto_space.hpp>
#include <hboost/multi_index/detail/bucket_array.hpp>
#include <hboost/multi_index/detail/do_not_copy_elements_tag.hpp>
#include <hboost/multi_index/detail/hash_index_iterator.hpp>
#include <hboost/multi_index/detail/index_node_base.hpp>
#include <hboost/multi_index/detail/modify_key_adaptor.hpp>
#include <hboost/multi_index/detail/promotes_arg.hpp>
...
#include <hboost/multi_index/hashed_index_fwd.hpp>
...
#define HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT_OF(x)                 \
...
#define HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT                       \
  HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT_OF(*this)
#else
#define HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT_OF(x)
#define HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT
#endif
...
public:
  ...
  const_iterator begin()const HBOOST_NOEXCEPT
    {return make_iterator(node_type::from_impl(header()->next()->prior()));}
  iterator       end()HBOOST_NOEXCEPT{return make_iterator(header());}
  const_iterator end()const HBOOST_NOEXCEPT{return make_iterator(header());}
  const_iterator cbegin()const HBOOST_NOEXCEPT{return begin();}
  const_iterator cend()const HBOOST_NOEXCEPT{return end();}
  ...
  {
    HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT;
  ...
  iterator insert(iterator position,HBOOST_RV_REF(value_type) x)
  {
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    HBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
    HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT;
  ...
  {
    HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT;
    HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT_OF(x);
  ...
  size_type bucket_count()const HBOOST_NOEXCEPT{return buckets.size();}
  size_type max_bucket_count()const HBOOST_NOEXCEPT{return static_cast<size_type>(-1);}
  ...
  float max_load_factor()const HBOOST_NOEXCEPT{return mlf;}
...
HBOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS:
  ...
  bool equals(const hashed_index& x,hashed_non_unique_tag)const
  {
    ...
    for(const_iterator it=begin(),it_end=end();it!=it_end;){
      ...
      hboost::tie(it2,it2_last)=x.equal_range(key(*it));
   ...
   * in HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT. Actually,
...
private:
  ...
  std::pair<iterator,bool> emplace_impl(HBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT;
    ...
    iterator position,HBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    ...
    std::pair<final_node_type*,bool>p=
      this->final_emplace_hint_(
        ...
        HBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
    return make_iterator(p.first);
  }
...
inline hboost::mpl::true_* hboost_foreach_is_noncopyable(
  hboost::multi_index::detail::hashed_index<
...
#undef HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT
#undef HBOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT_OF
