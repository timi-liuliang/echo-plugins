...
#ifndef HBOOST_MULTI_INDEX_DETAIL_INDEX_BASE_HPP
#define HBOOST_MULTI_INDEX_DETAIL_INDEX_BASE_HPP
...
#include <hboost/multi_index/detail/copy_map.hpp>
#include <hboost/multi_index/detail/do_not_copy_elements_tag.hpp>
#include <hboost/multi_index/detail/node_type.hpp>
...
#include <hboost/multi_index/detail/index_loader.hpp>
#include <hboost/multi_index/detail/index_saver.hpp>
#endif
...
protected:
  ...
  final_node_type* insert_(const value_type& v,final_node_type*& x,lvalue_tag)
  {
    ...
    HBOOST_TRY{
      hboost::detail::allocator::construct(&x->value(),v);
    }
    HBOOST_CATCH(...){
  ...
  final_node_type* insert_(const value_type& v,final_node_type*& x,rvalue_tag)
  {
    ...
    HBOOST_TRY{
       ...
       * hboost::detail::allocator::construct, but 
       * <hboost/detail/allocator_utilities.hpp> is too old and venerable to
      ...
      new (&x->value()) value_type(hboost::move(const_cast<value_type&>(v)));
    }
    HBOOST_CATCH(...){
      final().deallocate_node(x);
      HBOOST_RETHROW;
  ...
  {
    x->value()=hboost::move(const_cast<value_type&>(v));
    return true;
  }
  ...
  {
    return final().emplace_(HBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
  }
  ...
  std::pair<final_node_type*,bool> final_emplace_hint_(
    final_node_type* position,HBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    return final().emplace_hint_(
      position,HBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
  }
