...
#ifndef HBOOST_MULTI_INDEX_DETAIL_SAFE_MODE_HPP
#define HBOOST_MULTI_INDEX_DETAIL_SAFE_MODE_HPP
 ...
 * !defined(HBOOST_MULTI_INDEX_ENABLE_SAFE_MODE).
 */

#if !defined(HBOOST_MULTI_INDEX_ENABLE_SAFE_MODE)
#undef HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT
#define HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(expr,error_code) ((void)0)
#else
#if !defined(HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT)
#include <hboost/assert.hpp>
#define HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(expr,error_code) HBOOST_ASSERT(expr)
#endif
#endif

#define HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(it)                           \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_valid_iterator(it),                                     \
    safe_mode::invalid_iterator);

#define HBOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(it)                 \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_dereferenceable_iterator(it),                           \
    safe_mode::not_dereferenceable_iterator);

#define HBOOST_MULTI_INDEX_CHECK_INCREMENTABLE_ITERATOR(it)                   \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_incrementable_iterator(it),                             \
    safe_mode::not_incrementable_iterator);

#define HBOOST_MULTI_INDEX_CHECK_DECREMENTABLE_ITERATOR(it)                   \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_decrementable_iterator(it),                             \
    safe_mode::not_decrementable_iterator);

#define HBOOST_MULTI_INDEX_CHECK_IS_OWNER(it,cont)                            \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_is_owner(it,cont),                                      \
    safe_mode::not_owner);

#define HBOOST_MULTI_INDEX_CHECK_SAME_OWNER(it0,it1)                          \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_same_owner(it0,it1),                                    \
    safe_mode::not_same_owner);

#define HBOOST_MULTI_INDEX_CHECK_VALID_RANGE(it0,it1)                         \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_valid_range(it0,it1),                                   \
    safe_mode::invalid_range);

#define HBOOST_MULTI_INDEX_CHECK_OUTSIDE_RANGE(it,it0,it1)                    \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_outside_range(it,it0,it1),                              \
    safe_mode::inside_range);

#define HBOOST_MULTI_INDEX_CHECK_IN_BOUNDS(it,n)                              \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_in_bounds(it,n),                                        \
    safe_mode::out_of_bounds);

#define HBOOST_MULTI_INDEX_CHECK_DIFFERENT_CONTAINER(cont0,cont1)             \
  HBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
...
#include <hboost/multi_index/detail/iter_adaptor.hpp>
#include <hboost/multi_index/safe_mode_errors.hpp>
...
inline void detach_equivalent_iterators(Iterator& it)
{
  if(it.valid()){
    {
#if defined(HBOOST_HAS_THREADS)
      hboost::detail::lightweight_mutex::scoped_lock lock(it.cont->mutex);
#endif
...
HBOOST_MULTI_INDEX_PRIVATE_IF_MEMBER_TEMPLATE_FRIENDS:
...
#if defined(HBOOST_HAS_THREADS)
  hboost::detail::lightweight_mutex mutex;
#endif
};
...
void safe_iterator_base::attach(safe_container_base* cont_)
{
  ...
  if(cont){
#if defined(HBOOST_HAS_THREADS)
    hboost::detail::lightweight_mutex::scoped_lock lock(cont->mutex);
#endif
...
public:
  ...
  {
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(x);
...
private:
  friend class hboost::multi_index::detail::iter_adaptor_access;

  reference dereference()const
  {
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    HBOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(*this);
  ...
  bool equal(const safe_iterator& x)const
  {
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(x);
    HBOOST_MULTI_INDEX_CHECK_SAME_OWNER(*this,x);
  ...
  {
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    HBOOST_MULTI_INDEX_CHECK_INCREMENTABLE_ITERATOR(*this);
    ++(this->base_reference());
  }
  ...
  {
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    HBOOST_MULTI_INDEX_CHECK_DECREMENTABLE_ITERATOR(*this);
    --(this->base_reference());
  }
  ...
  {
    HBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    HBOOST_MULTI_INDEX_CHECK_IN_BOUNDS(*this,n);
...
template<typename Iterator,typename Container>
struct version<
  hboost::multi_index::safe_mode::safe_iterator<Iterator,Container>
>
{
  HBOOST_STATIC_CONSTANT(
    int,value=hboost::serialization::version<Iterator>::value);
};
} /* namespace serialization */
#endif
...
#endif /* HBOOST_MULTI_INDEX_ENABLE_SAFE_MODE */
