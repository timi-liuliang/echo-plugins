...
#ifndef HBOOST_MULTI_INDEX_DETAIL_COPY_MAP_HPP
#define HBOOST_MULTI_INDEX_DETAIL_COPY_MAP_HPP
...
public:
  ...
  ~copy_map()
  {
    if(!released){
      for(std::size_t i=0;i<n;++i){
        hboost::detail::allocator::destroy(&(spc.data()+i)->second->value());
  ...
  void clone(Node* node)
  {
    ...
    HBOOST_TRY{
      hboost::detail::allocator::construct(
...
private:
  typedef typename hboost::detail::allocator::rebind_to<
    Allocator,Node
  >::type                                               allocator_type;
