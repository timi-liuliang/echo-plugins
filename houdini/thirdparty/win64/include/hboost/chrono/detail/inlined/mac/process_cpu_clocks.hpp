  ...
  namespace chrono
  {
    namespace chrono_detail
    {
      ...
      //  to nanoseconds; -1 if unknown
      {
        ...
        if (!factor)
        {
          ...
          {
            HBOOST_ASSERT(factor <= 1000000000l); // doesn't handle large ticks
    ...
    process_real_cpu_clock::time_point process_real_cpu_clock::now(system::error_code & ec)
    {
      ...
      if (c == clock_t(-1)) // error
      {
        ...
        {
          hboost::throw_exception(system::system_error(errno, HBOOST_CHRONO_SYSTEM_CATEGORY, "chrono::process_real_cpu_clock"));
        } else
        {
    ...
    process_user_cpu_clock::time_point process_user_cpu_clock::now(system::error_code & ec)
    {
      ...
      if (c == clock_t(-1)) // error
      {
        if (HBOOST_CHRONO_IS_THROWS(ec))
        {
          hboost::throw_exception(system::system_error(errno, HBOOST_CHRONO_SYSTEM_CATEGORY, "chrono::process_user_cpu_clock"));
        } else
        {
          ec.assign(errno, HBOOST_CHRONO_SYSTEM_CATEGORY);
          return time_point();
    ...
    process_system_cpu_clock::time_point process_system_cpu_clock::now(system::error_code & ec)
    {
      ...
      if (c == clock_t(-1)) // error
      {
        if (HBOOST_CHRONO_IS_THROWS(ec))
        {
          hboost::throw_exception(system::system_error(errno, HBOOST_CHRONO_SYSTEM_CATEGORY, "chrono::process_system_cpu_clock"));
        } else
        {
          ec.assign(errno, HBOOST_CHRONO_SYSTEM_CATEGORY);
          return time_point();
    ...
    process_cpu_clock::time_point process_cpu_clock::now(system::error_code & ec)
    {
      ...
      if (c == clock_t(-1)) // error
      {
        if (HBOOST_CHRONO_IS_THROWS(ec))
        {
          hboost::throw_exception(system::system_error(errno, HBOOST_CHRONO_SYSTEM_CATEGORY, "chrono::process_clock"));
        } else
        {
          ec.assign(errno, HBOOST_CHRONO_SYSTEM_CATEGORY);
          return time_point();
