...
#ifndef HBOOST_DYNAMIC_BITSET_CONFIG_HPP_GP_20040424
#define HBOOST_DYNAMIC_BITSET_CONFIG_HPP_GP_20040424
...
#if defined(__GNUC__)
# define HBOOST_DYNAMIC_BITSET_WRAP_CONSTANT(expr) \
         (hboost::detail::make_non_const(expr))
#else
# define HBOOST_DYNAMIC_BITSET_WRAP_CONSTANT(expr) (expr)
#endif

//
#if (defined __BORLANDC__ && HBOOST_WORKAROUND(__BORLANDC__, HBOOST_TESTED_AT(0x564)))  \
                             || (defined HBOOST_NO_MEMBER_TEMPLATE_FRIENDS)
#define HBOOST_DYNAMIC_BITSET_DONT_USE_FRIENDS
#endif

// if we can't use friends then we simply expose private members
//
#if defined(HBOOST_DYNAMIC_BITSET_DONT_USE_FRIENDS)
#define HBOOST_DYNAMIC_BITSET_PRIVATE public
#else
#define HBOOST_DYNAMIC_BITSET_PRIVATE private
#endif
...
// to its parameter c.
//
#if defined (HBOOST_USE_FACET)

#define HBOOST_DYNAMIC_BITSET_CTYPE_FACET(ch, name, loc)     \
            const std::ctype<ch> & name =                   \
            HBOOST_USE_FACET(std::ctype<ch>, loc)         /**/

#define HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, c)             \
           (fac.widen(c))
#else

#define HBOOST_DYNAMIC_BITSET_CTYPE_FACET(ch, name, loc) /**/
#define HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, c)          c
