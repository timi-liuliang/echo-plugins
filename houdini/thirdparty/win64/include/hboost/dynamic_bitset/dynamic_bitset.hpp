...
#ifndef HBOOST_DYNAMIC_BITSET_DYNAMIC_BITSET_HPP
#define HBOOST_DYNAMIC_BITSET_DYNAMIC_BITSET_HPP
...
#include "hboost/dynamic_bitset/config.hpp"

#ifndef HBOOST_NO_STD_LOCALE
#  include <locale>
#endif

#if defined(HBOOST_OLD_IOSTREAMS)
#  include <iostream.h>
...
#include "hboost/dynamic_bitset_fwd.hpp"
#include "hboost/detail/dynamic_bitset.hpp"
#include "hboost/detail/iterator.hpp" // used to implement append(Iter, Iter)
#include "hboost/move/move.hpp"
#include "hboost/limits.hpp"
#include "hboost/pending/lowest_bit.hpp"
...
class dynamic_bitset
{
    ...
    HBOOST_STATIC_ASSERT((bool)detail::dynamic_bitset_impl::allowed_block_type<Block>::value);
...
public:
    ...
    HBOOST_STATIC_CONSTANT(block_width_type, bits_per_block = (std::numeric_limits<Block>::digits));
    HBOOST_STATIC_CONSTANT(size_type, npos = static_cast<size_type>(-1));
...
public:
    ...
    :m_bits(alloc),
     m_num_bits(0)
    {
        using hboost::detail::dynamic_bitset_impl::value_to_type;
        using hboost::detail::dynamic_bitset_impl::is_numeric;
    ...
    void m_append(BlockInputIterator first, BlockInputIterator last, std::forward_iterator_tag)
    {
        ...
        std::size_t d = hboost::detail::distance(first, last);
    ...
    size_type count() const HBOOST_NOEXCEPT;
    ...
    size_type num_blocks() const HBOOST_NOEXCEPT;
...
#if !defined HBOOST_DYNAMIC_BITSET_DONT_USE_FRIENDS
...
private:
    HBOOST_STATIC_CONSTANT(block_width_type, ulong_width = std::numeric_limits<unsigned long>::digits);
    ...
    block_width_type count_extra_bits() const HBOOST_NOEXCEPT { return bit_index(size()); }
    static size_type block_index(size_type pos) HBOOST_NOEXCEPT { return pos / bits_per_block; }
    static block_width_type bit_index(size_type pos) HBOOST_NOEXCEPT { return static_cast<block_width_type>(pos % bits_per_block); }
    static Block bit_mask(size_type pos) HBOOST_NOEXCEPT { return Block(1) << bit_index(pos); }
        ...
        size_type num_bits)
    {
        ...
        HBOOST_DYNAMIC_BITSET_CTYPE_FACET(CharT, fac, std::locale());
        const CharT one = HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '1');
        ...
        for( ; i < m; ++i) {
            ...
            assert( Tr::eq(c, one)
                    || Tr::eq(c, HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0')) );
                                 ...
                                 const Allocator& alloc*/)
    {
        ...
        typedef hboost::detail::dynamic_bitset_impl
...
dynamic_bitset(dynamic_bitset<Block, Allocator>&& b)
  : m_bits(hboost::move(b.m_bits)), m_num_bits(hboost::move(b.m_num_bits))
{
...
{
    if (hboost::addressof(b) == this) { return *this; }

    m_bits = hboost::move(b.m_bits);
    m_num_bits = hboost::move(b.m_num_bits);
    // Required so that assert(m_check_invariants()); works.
    assert((b.m_bits = buffer_type()).empty());
    b.m_num_bits = 0;
    return *this;
...
{
    assert(pos < m_num_bits);
#if defined __MWERKS__ && HBOOST_WORKAROUND(__MWERKS__, <= 0x3003) // 8.x
...
dynamic_bitset<Block, Allocator>::count() const HBOOST_NOEXCEPT
{
...
#if HBOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ == 3) && (__GNUC_PATCHLEVEL__ == 3)
                      ...
                      bool dump_all)
{
    ...
    HBOOST_DYNAMIC_BITSET_CTYPE_FACET(Ch, fac, std::locale());
    const Ch zero = HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0');
    const Ch one  = HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '1');
...
to_ulong() const
{
  ...
  if (find_next(ulong_width - 1) != npos)
    HBOOST_THROW_EXCEPTION(std::overflow_error("hboost::dynamic_bitset::to_ulong overflow"));
...
dynamic_bitset<Block, Allocator>::size() const HBOOST_NOEXCEPT
{
    return m_num_bits;
}
...
template <typename Block, typename Allocator>
inline typename dynamic_bitset<Block, Allocator>::size_type
dynamic_bitset<Block, Allocator>::num_blocks() const HBOOST_NOEXCEPT
{
    return m_bits.size();
}
...
template <typename Block, typename Allocator>
inline typename dynamic_bitset<Block, Allocator>::size_type
dynamic_bitset<Block, Allocator>::max_size() const HBOOST_NOEXCEPT
{
...
template <typename Block, typename Allocator>
inline bool dynamic_bitset<Block, Allocator>::empty() const HBOOST_NOEXCEPT
{
  return size() == 0;
}
...
dynamic_bitset<Block, Allocator>::m_do_find_from(size_type first_block) const
{
    ...
    return i * bits_per_block + static_cast<size_type>(hboost::lowest_bit(m_bits[i]));
           ...
           const dynamic_bitset<Block, Alloc>& b)
{
    ...
    if (cerberos) {
        ...
        HBOOST_DYNAMIC_BITSET_CTYPE_FACET(Ch, fac, os.getloc());
        const Ch zero = HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0');
        const Ch one  = HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '1');
        ...
        } HBOOST_CATCH (...) { // see std 27.6.1.1/4
            bool rethrow = false;
            HBOOST_TRY { os.setstate(ios_base::failbit); } HBOOST_CATCH (...) { rethrow = true; } HBOOST_CATCH_END
...
operator>>(std::basic_istream<Ch, Tr>& is, dynamic_bitset<Block, Alloc>& b)
{
    ...
    if(cerberos) {
        ...
        HBOOST_DYNAMIC_BITSET_CTYPE_FACET(Ch, fac, is.getloc());
        const Ch zero = HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0');
        const Ch one  = HBOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '1');
        ...
        HBOOST_CATCH (...) {
            ...
            HBOOST_TRY { is.setstate(ios_base::badbit); }
            HBOOST_CATCH(...) { rethrow = true; }
            HBOOST_CATCH_END
