...
#ifndef HBOOST_UUID_0552D49838DD11DD90146B8956D89593
#define HBOOST_UUID_0552D49838DD11DD90146B8956D89593
...
#include <hboost/exception/get_error_info.hpp>
...
#include <hboost/exception/current_exception_cast.hpp>
namespace
hboost
    {
    namespace
    exception_detail
        {
        std::string diagnostic_information_impl( hboost::exception const *, std::exception const *, bool, bool );
        }
    ...
    current_exception_diagnostic_information( bool verbose=true)
        {
        hboost::exception const * be=current_exception_cast<hboost::exception const>();
...
hboost
    ...
    exception_detail
        ...
        inline
        exception const *
        get_hboost_exception( exception const * e )
            {
            return e;
            }
        ...
        inline
        std::string
        diagnostic_information_impl( hboost::exception const * be, std::exception const * se, bool with_what, bool verbose )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef HBOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<hboost::exception const *>(se);
            if( !se )
            ...
            if( be && verbose )
                ...
                if( !f && !l && !fn )
                    tmp << "Throw location unknown (consider using HBOOST_THROW_EXCEPTION)\n";
                else
                    {
                    if( f )
                        {
            ...
            if ( verbose )
                tmp << std::string("Dynamic exception type: ") <<
                    core::demangle((be?(HBOOST_EXCEPTION_DYNAMIC_TYPEID(*be)):(HBOOST_EXCEPTION_DYNAMIC_TYPEID(*se))).type_->name()) << '\n';
#endif
    ...
    diagnostic_information( T const & e, bool verbose=true )
        {
        return exception_detail::diagnostic_information_impl(exception_detail::get_hboost_exception(&e),exception_detail::get_std_exception(&e),true,verbose);
        }
    ...
    diagnostic_information_what( exception const & e, bool verbose=true ) throw()
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce hboost::diagnostic_information_what()";
#ifndef HBOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
