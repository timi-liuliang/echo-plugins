...
public:
    ...
    void        buildFromOp(const OP_Node *node, fpreal time, DEP_MicroNode *depnode)
    {
        ...
        if (true)
            OP_Utils::evalOpParm(myIgnoreattribs, node, "ignoreattribs", time, 0);
        else myIgnoreattribs = false;
        if (true)
            OP_Utils::evalOpParm(myIgnoregroups, node, "ignoregroups", time, 0);
        else myIgnoregroups = false;
        if (true)
            OP_Utils::evalOpParm(myMinpolys, node, "minpolys", time, 0);
        else myMinpolys = 1;
        if (true)
        ...
        else myUsemaxsides = false;
        if (true && ( (!(((getConvex()==0))||((getUsemaxsides()==0)))) ) )
            OP_Utils::evalOpParm(myMaxsides, node, "maxsides", time, 0);
        else myMaxsides = 3;
        if (true)
            OP_Utils::evalOpParm(myMergeverts, node, "mergeverts", time, 0);
        else myMergeverts = false;
