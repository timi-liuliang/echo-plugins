...
class DEP_MicroNode;
namespace SOP_VolumeBoundEnums
{
    enum class Comp
    {
        GREATER = 0,
...
class SOP_API SOP_VolumeBoundParms  : public SOP_NodeParms
{
public:
    ...
    explicit SOP_VolumeBoundParms(const SOP_VolumeBoundParms &) = default;

    virtual ~SOP_VolumeBoundParms() {}

    bool operator==(const SOP_VolumeBoundParms &src) const
    {
        if (myGroup != src.myGroup) return false;
        if (myComp != src.myComp) return false;
        if (myValue != src.myValue) return false;
    ...
    using Comp = SOP_VolumeBoundEnums::Comp;
    ...
    void        buildFromOp(const OP_Node *node, fpreal time, DEP_MicroNode *depnode)
    {
        ...
        if (true)
            OP_Utils::evalOpParm(myComp, node, "comp", time, 0);
        else myComp = 0;
        if (true)
            OP_Utils::evalOpParm(myValue, node, "value", time, 0);
        else myValue = 0;
        if (true)
    ...
    {
        *this = *((const SOP_VolumeBoundParms *)src);
    }
    ...
    doSetParmValue(exint idx, const T &value) 
    {
        switch (idx)
        {
            case 0:
                ...
                break;
            case 1:
                coerceValue(myComp, value);
                break;
            case 2:
                coerceValue(myValue, value);
                break;
            case 3:
    ...
    Comp getComp() const { return Comp(myComp); }
    void setComp(Comp val) { myComp = int64(val); }
    ...
    fpreal64 getValue() const { return myValue; }
    void setValue(fpreal64 val) { myValue = val; }
    fpreal64 opValue(const SOP_NodeVerb::CookParms &cookparms) const
    { 
        SOP_Node *thissop = cookparms.getNode();
        if (!thissop) return getValue();
        fpreal64 result;
        OP_Utils::evalOpParm(result, thissop, "value", cookparms.getCookTime(), 0);
        return result;
    }
