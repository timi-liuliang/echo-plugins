...
public:
    ...
    virtual ~SOP_BoneCaptureBiharmonicParms() {}
    ...
    void        buildFromOp(const OP_Node *node, fpreal time, DEP_MicroNode *depnode)
    {
        ...
        if (true)
            OP_Utils::evalOpParm(myMaxIterations, node, "maxiter", time, 0);
        else myMaxIterations = 1;
        if (true)
            OP_Utils::evalOpParm(myDiffTol, node, "difftol", time, 0);
        else myDiffTol = 1e-4;
        if (true)
        ...
        if (true)
            OP_Utils::evalOpParm(myPointColoring, node, "color", time, 0);
        else myPointColoring = 0;
        if (true)
            OP_Utils::evalOpParm(myZeroWeightColor, node, "zeroweightcolor", time, 0);
        else myZeroWeightColor = UT_Vector3D(1,1,1);
        if (true)
            OP_Utils::evalOpParm(myOutputCaptureTets, node, "outputcapturetets", time, 0);
        else myOutputCaptureTets = false;
        if (true)
            OP_Utils::evalOpParm(myVerbose, node, "verbose", time, 0);
        else myVerbose = false;
    ...
    doSetParmValue(exint idx, const T &value) 
    {
        switch (idx)
        {
            case 0:
                ...
                break;
            case 1:
                coerceValue(myMaxIterations, value);
                break;
            case 2:
            ...
            case 4:
                ...
                break;
            case 5:
                coerceValue(myPointColoring, value);
                break;
            case 6:
