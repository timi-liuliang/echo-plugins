 ...
 *
 * NAME:        UT_SolidAngle.h (UT Library, C++)
 *
 * COMMENTS:    Functions for computing solid angles.
 */

#ifndef __UT_SolidAngle_h__
#define __UT_SolidAngle_h__
...
#if !SOLID_ANGLE_USE_BVH
// Forward declaration, to avoid having to include UT_RTree.h
...
private:
    ...
    UT_BVH<BVH_N> myTree;
#else
    UT_UniquePtr<UT_RTreeGeneric<2>> myTree;
#endif
    int myNBoxes;
    int myOrder;
...
/// NOTE: This is currently only instantiated for <float,float>.
template<typename T,typename S>
class UT_API UT_SubtendedAngle
{
public:
    /// This is outlined so that we don't need to include UT_BVHImpl.h
    UT_SubtendedAngle();
