    ...
    GusdUSD_TraverseControl& ctl) const
{
    ...
    if(ip) {
        ...
        if( GusdPurposeInSet( purpose, purposes )) {
            if(HBOOST_UNLIKELY(Visitor()(prim, time, ctl))) {
                if(!Recursive)
                    ctl.PruneChildren();
                return true;
            }
...
TraverseTaskT<Visitor>::run()
{
    ...
    if(HBOOST_UNLIKELY(_visited)) return NULL;
    _visited = true;
    ...
    if(!_skipPrim) {
        ...
        if(HBOOST_UNLIKELY(_visitor.AcceptPrim(_prim, _time, _purposes, ctl))) {
        ...
        if(HBOOST_UNLIKELY(!ctl.GetVisitChildren())) {
            return NULL;
        }
    }
